...
#ident	"@(#)vipw.c	1.17	92/07/14 SMI"	/* SVr4.0 1.1	*/
...
main()
{
	...
	if (o_mtime < n_mtime) {
		...
		fprintf(stdout,"Press 'e' to edit the shadow file for consistency,\n 'q' to quit: ");
		if (( c = getchar()) == 'q') {
			if (chmod(ptemp, 0444) < 0) {
...
char *from, *to;
{
	...
	fd = open(to, O_WRONLY|O_CREAT|O_EXCL, 0644);
	if (fd < 0) {
		if (errno == EEXIST) {
