...
/*LINTLIBRARY*/
#ident	"@(#)verify.c	1.22	95/03/22 SMI"	/* SVr4.0  1.21.1.1	*/
...
averify(int fix, char *ftype, char *path, struct ainfo *ainfo)
{
	...
	if (retcode) {
		...
		if (fix) {
			/*
			 * If the file exists and it isn't a directory,
			 * unlink it. If it is a directory, this is a
			 * more complicated issue than we should deal with
			 * here.
			 */
	...
	if (strchr("cb", myftype)) {
		...
		if (status.st_rdev != makedev(ainfo->xmajor, ainfo->xminor)) {
			reperr(pkg_gt(ERR_MAJMIN), ainfo->xmajor, ainfo->xminor,
