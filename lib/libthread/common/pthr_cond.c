...
#pragma	" @(#)pthr_cond.c 1.8 94/11/03 "
...
{
	if (attr == NULL || attr->pthread_condattrp == NULL ||
			_free_attr(attr->pthread_condattrp) < 0)
		return (EINVAL);
	attr->pthread_condattrp = NULL;
	return (0);
}
						...
						int pshared)
{
	...
	if (attr != NULL && (ap = attr->pthread_condattrp) != NULL &&
						...
						int *pshared)
{
	...
	if (pshared != NULL && attr != NULL &&
			(ap = attr->pthread_condattrp) != NULL) {
		*pshared = ap->pshared;
		return (0);
	} else {
		return (EINVAL);
...
_pthread_cond_init(pthread_cond_t *cond, const pthread_condattr_t *attr)
{
	...
	if (attr != NULL) {
		if (attr->pthread_condattrp == NULL)
			return (EINVAL);

		type = ((cvattr_t *)attr->pthread_condattrp)->pshared;
	} else {
		type = DEFAULT_TYPE;
	}
