...
#pragma	ident	"@(#)sparcsubr.s	1.31	95/08/08	SMI"
...
#else
	retl
	ld	[%g7 + T_TPDP], %o0	! return thread probe data
#endif
 ...
 */
	...
	mov	%o0, %g7
#endif
	ld	[%o0 + T_PC], %i7		! return pc
	ld	[%o0 + T_SP], %fp		! return sp
	mov	%o1, %i0
	call	_resume_ret
	restore
	SET_SIZE(_threadjmp)
...
#endif
#ifdef ITRACE
	lduh	[THREAD_REG + T_USROPTS], %l1
	set	THR_BOUND, %l2
	andcc	%l1, %l2, %g0
	bz	1f			! branch if unbound
	nop
	ld	[THREAD_REG + T_LWPID], %o2	! else if bound
...
_SHAREDFRAME:
	...
	ld	[%o2+%o0], %o0		! %o0 points to _reaplockp
#else
	set	_SHAREDFRAME, %o0	! stop running on thread's stack
	mov	%o0, %sp		! use SHAREDFRAME as stack
					! NOTE: AFTER THIS POINT, USE ONLY THE
					! %o REGISTERS: %o0, %o2, %o3, %o4, %o5
					! AS SCRATCH REGISTERS
	set	_reaplockp, %o0		! here, %o0 is the address of _reaplockp
#endif
					! IMPORTANT NOTE:
	...
	ld	[%o0], %o0		! read _reaplockp for _reaplock address
	...
	set _lwp_exitp, %o3
#endif
	ld 	[%o3], %o3		! Read _lwp_exitp for addr. of _lwp_exit
 ...
 */
FPTR	= (14*4)	! offset of frame ptr in frame
PC	= (15*4)	! offset of PC in frame
WSIZE	= (16*4)	! size of frame

	ENTRY(_ex_unwind_local)
	ta	ST_FLUSH_WINDOWS	! save all regs on stack
	sub	%sp, WSIZE, %sp		! create 16 word frame for traps
	ld	[%fp+PC], %i7		! load ret addr of second frame below
	ld	[%fp+FPTR], %fp		! load fp second frame below
...
/*
 * _ex_clnup_handler(void (*tcancel)(void), void  (*clnup)(void *),
 *								void *arg)
 *
