...
#ident	"@(#)regex.c	1.18	95/09/05 SMI"	/* SVr4.0 1.1.3.4	*/
...
static char *__execute(), *__advance();
static int __xpush(), __getrnge(), __cclass(), __bcclass(), __abcclass();
static char *__xpop();

static char	*__braslist[NBRA];
static char	*__braelist[NBRA];
static int	__bravar[NBRA];
static char 	*__st[SSIZE + 1];
static char * 	*__eptr_;
static char *  *__lptr_;
static int	__cflg;
...
_get_vars_storage(thread_key_t *key)
{
	...
	if (_thr_getspecific(*key, &vars) != 0) {
...
register char *lp, *ep;
{
	...
	case TGRP|A256:
		...
		if (__xpush(1, curlp = lp) == -1)
			return ((char *)-1);
		while (dcnt--)
			if (!(dp = __advance(lp, ep)))
				break;
			else
			if (__xpush(1, lp = dp) == -1)
					return ((char *)-1);
		ep = __xpop(0);
		goto star;
	case MCCHR|RNGE:
	...
	case NBCCL|STAR:
		rp = lp;
		curlp = lp;
		while ((d = __abcclass(ep, &rp, ((ep[-1] == (BCCL|STAR)) ||
			(ep[-1] == (BCCL|PLUS))))) == 1);
		ep += *ep;
		lp = rp;
		goto star;
	...
	case NMCCL|STAR:
		curlp = rp = lp;
		while ((d = __bcclass(ep, &rp, ((ep[-1] == (MCCL|STAR)) ||
			(ep[-1] == (MCCL|PLUS))))) == 1);
		if (d == -2)
			return (0);
		ep += *ep;
	...
	mstar:
		do {
			...
			if ((unsigned char)*lp >= 0200) {
				...
				if (p2 >= curlp && (unsigned char)*p2 == SS3)
					lp = p2;
				else
					lp = lp -eucw1 + 1;
			        if (ptr = __advance(lp, ep))
