#ifndef lint
static char	sccsid[] = "@(#)generic.c 1.5 93/12/13 SMI";
#endif
...
void
aug_save_tid(dev_t port, u_long machine)
{
	aug_tid.port = port;
...
aug_save_namask()
{
	...
	aug_na = -1;
	if (getacna(naflags, 255) < 0) {
		return (-1);
	}
	if (getauditflagsbin(naflags, &mask) < 0) {
		return (-1);
	}
...
aug_audit(void)
{
	...
	au_write(ad, au_to_return((aug_sorf == 0) ? 0 : -1, aug_sorf));
...
aug_selected()
{
	...
	if (au_user_mask(user, &mask)) {
		return (-1);
	}

	return (selected(aug_event, &mask, aug_sorf));
}
