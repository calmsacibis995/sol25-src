...
#ident	"@(#)setupterm.c	1.13	95/01/09 SMI"	/* SVr4.0 1.54	*/
...
next:
	...
	for (n = 0; n < sizeof(offsets); n++)
	{
		...
		((n > 0) && off_sequences[0] && (strcmp(off_sequences[n], off_sequences[0]) == 0)) ||
		((n == 2) && (exit_attribute_mode) && (strcmp(exit_attribute_mode, off_sequences[n]) == 0)))
	    {
