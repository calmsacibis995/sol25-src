...
#pragma	ident	"@(#)ioctl.c	1.17	95/08/29 SMI"
...
_bc_ioctl(int des, int request, int arg)
{
	...
	switch (nreq) {
		case ((int) 't'):
			if (s_ioctl(des, N_I_FIND, "ttcompat") == 0)
				if (s_ioctl(des, N_I_PUSH, "ttcompat") == -1)
					perror("ioctl/I_PUSH");
			switch(request) {
				case TIOCSETD:
					     /* added for sunview */
				...
				case TIOCGETP:			
					...
					ret = s_ioctl(des, request&0xFFFF, &nsg);
					if (ret != -1) {
		...
		case ((int) 'd'):
			switch (request) {
				...
				case F_RAW:
					...
					break;
				case DKIOCINFO:
					ret = s_ioctl(des, S5DKIOCINFO, &newArgs);
					if (ret != -1) {
		...
		case ((int) 'u'):
			switch (request) {
				case USCSICMD:
					...
					s5_cmd.uscsi_flags =
						cmd->uscsi_flags;
					ret = s_ioctl(des, request, &s5_cmd);
	...
	return(s_ioctl(des, request, arg));
}
...
handle_dkio_partitions(int des, int request, int arg)
{
	...
	ret = s_ioctl(des, S5DKIOCINFO, &cinfo);
	...
	if (ret != -1) {
		ret = s_ioctl(des, S5DKIOCGAPART, &map);
		if (ret != -1) {
			if (request == DKIOCGPART) {
				...
				map.dka_map[cinfo.dki_partition].dkl_nblk =
					part->dkl_nblk;
				ret = s_ioctl(des, S5DKIOCSAPART, &map);
			}
		}
	}
	return (ret);
	...
	int		arg;
{
	...
	if (termios != NULL) {
		...
		return (s_ioctl(des, request, &s5termios));
	} else
		return (s_ioctl(des, request, NULL));
	...
	int		arg;
{
	...
	ret = s_ioctl(des, request, &s5termios);
