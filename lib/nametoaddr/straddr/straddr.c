...
#ident	"@(#)straddr.c	1.17	94/06/15 SMI"
...
static int searchhost();
static int searchserv();
static char *mergeaddr();
	...
	if (nd_hostservp->h_serv &&
		...
		if (nd_hostservp->h_serv && (nd_hostservp->h_serv[0] != '\0')) {
			...
			if (((fp = fopen(searchfile, "r")) == NULL) ||
				...
				fulladdr + strlen(fulladdr))) == FALSE)) {
				if (fp != NULL) {
					(void) fclose(fp);
				}
				_nderror = ND_NOSERV;
...
struct netbuf	 *netbufp;
{
	...
	retp->h_cnt = nhost;
	if ((retp->h_hostservs = (struct nd_hostserv *)malloc((unsigned)nhost *
				sizeof (struct nd_hostserv))) == NULL) {
		_nderror = ND_NOMEM;
		return (NULL);
	}
...
struct netconfig *netconfigp;
{
	...
	holdp = malloc((unsigned)strlen(uaddr) + 1);
	if (holdp == (char *)NULL) {
		_nderror = ND_NOMEM;
