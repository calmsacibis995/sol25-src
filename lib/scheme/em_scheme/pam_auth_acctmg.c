...
#ident  "@(#)pam_auth_acctmg.c 1.20     95/02/10 SMI"
...
static int
sa_check_for_login_inactivity(pwd, shpwd, remote_host, ttyn)
	struct 	passwd 	*pwd;
	struct 	spwd 	*shpwd;
	char 	*remote_host;
	char	*ttyn;
...
static int
sa_do_new_passwd_if_needed(pwd, shpwd, passwd_req)
	struct 	passwd 	*pwd;
	struct 	spwd 	*shpwd;
	int 	passwd_req;
{
...
static	int
sa_perform_passwd_aging_check(shpwd, ia_convp)
	struct 	spwd 	*shpwd;
	struct	ia_conv	*ia_convp;
{
	...
	if ((shpwd->sp_lstchg == 0) ||
		...
		if ((Idleweeks == 0) ||
			...
			display_errmsg(ia_convp->cont_conv, 0, 1,
			messages, NULL);
			return (IA_TOK_EXPIRED);
		} else {
			return (IA_NEWTOK_REQD);
		}
	}
...
static void
sa_warn_user_passwd_will_expire(shpwd, ia_convp)
	struct 	spwd 	*shpwd;
	struct	ia_conv	*ia_convp;
{
	int 	now	= DAY_NOW;
	...
	struct ia_status *ia_status;
{
	struct 	spwd *shpwd;
	int 	error;
	char    *pg, *user, *ttyn, *rhost;
	struct  ia_conv *ia_convp;
	int	err;

	if ((err = sa_getall(iah, &pg, &user, &ttyn, &rhost, &ia_convp))
	    != IA_SUCCESS)
		return (err);
	 ...
	 */
	if (error = sa_check_for_login_inactivity(*pwd, shpwd, rhost, ttyn)) {
		return (IA_NOPERM);
	}
	 ...
	 */
	if (error = sa_do_new_passwd_if_needed(*pwd, shpwd, flags)) {
		return (error);
	}
	 ...
	 */
	if (error = sa_perform_passwd_aging_check(shpwd, ia_convp)) {
		return (error);
	}
