	...
	register struct callinfo *ci;
{
	 ...
	 */
	*(u_long *) ci->ci_outbuf = htonl(++ci->ci_xid);
#ifdef	PRINTFS
	...
	register int flag;
{
	...
	printf("clnt_dg_receive:  moreflag %d, udata.len %d, recvsz %d\n",
		 ...
		 */
		pktxid = ntohl(*(u_long *) (tmp_trdata->udata.buf));
		if (pktxid != ci->ci_xid) {
			...
			if (p && p->ci_trdata == (struct t_unitdata *) NULL) {
				if (flag == CI_ASYNC) {
					...
					if (!netbuf_copy(&tmp_trdata->addr,
							&p->ci_trdata->addr,
							T_ADDR)) {
						t_free ((char *) p->ci_trdata, T_UNITDATA);
						return (RS_ERROR);
					}
	...
	char *info;
{
	...
	case CLGET_XID:
		...
		*(u_long *)info = cu->cu_xidseed;
		break;

	case CLSET_XID:
		/* This will set the xid seed of the NEXT call */
		cu->cu_xidseed = *(u_long *) info - 1;
	...
	case CLGET_VERS:
		 ...
		 */
		*(u_long *)info = ntohl(*(u_long *)(cu->cu_mcallproto +
						    4 * BYTES_PER_XDR_UNIT));
		break;
	case CLSET_VERS:
		*(u_long *)(cu->cu_mcallproto + 4 * BYTES_PER_XDR_UNIT)
			= htonl(*(u_long *)info);
		break;
	...
	case CLSET_PROG:
		*(u_long *)(cu->cu_mcallproto + 3 * BYTES_PER_XDR_UNIT)
			= htonl(*(u_long *)info);
		break;
