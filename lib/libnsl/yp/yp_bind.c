...
CLIENT *__clnt_create_loopback();
static CLIENT *getclnt();
...
static ypbind_resp *get_cached_domain();
static int get_cached_transport();
static int ypbind_running();
...
#ifdef DEBUG
printf("yp_bind: clnt_tli_create suceeded\n");
#endif
	...
	int ulen;
{
	...
	sprintf(uaddress, "%s/xprt.%s.%d", BINDING, nconf->nc_netid, vers);
	...
	char *domain;
{
	...
	sprintf(filename, "%s/%s/cache_binding", BINDING, domain);
	fp = fopen(filename, "r");
	if (fp == 0)
		return (0);
