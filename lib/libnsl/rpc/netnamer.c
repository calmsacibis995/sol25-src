...
#ident	"@(#)netnamer.c	1.22	95/06/14 SMI"
/*
...
struct netid_userdata *argp;
{
	...
	while (! feof(fd)) {
		...
		while (isspace(*name))
			name++;
		if (*name == NUL)	/* blank line continue */
			continue;
		...
		*value++ = NUL; /* nul terminate the name */
	...
	int hostlen;
{
	...
	if (*p == NUL)			/* check for null hostname */
		goto bad_netname;
