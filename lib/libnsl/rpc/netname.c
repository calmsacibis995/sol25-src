...
#ident	"@(#)netname.c	1.21	94/10/28 SMI"
	...
	char *domain;
{
	...
	/*
	 * 2.  use nis+ principal name to get netname by getting the DES entry.
	 *
	...
	sprintf(buf, "[cname=\"%s\",auth_type=DES],%s.%s",
	...
	if (nres->objects.objects_len > 1) {
		/*
		 * Principal with more than one DES entry?
		 * Something wrong with cred table. Should be unique.
		 * Warn user and continue.
		 */
		syslog(LOG_ALERT,
			"user2netname: DES entry for %s not unique", principal);
	}
	...
	strncpy(netname, ENTRY_VAL(nres->objects.objects_val, 2), len);
	netname[len] = NUL;
	nis_freeresult(nres);
	*err = __NSW_SUCCESS;
	trace1(TR_user2netname_nisplus, 1);
	...
	const char *domain;
{
	...
	if (host == NULL) {
		strncpy(hostname, nis_local_host(), sizeof (hostname));
