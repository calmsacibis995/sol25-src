...
#pragma ident	"@(#)nis_groups.c	1.19	95/01/13 SMI"
...
extern 		nis_flush_table(NIS_HASH_TABLE *, nis_flush_func);
...
typedef nis_result *(*nis_lookup_func)(nis_name, u_long);
...
int
__nis_group_cache_stats(grpcachecall, grpcachehits, grpcachemisses)
	int	*grpcachecall;
	int	*grpcachehits;
	int	*grpcachemisses;
{
...
printf_explicit(varp, title)
	const g_varieties	*varp;
	const char		*title;
{
	if (varp->explicit == 0) {
		printf("    No explicit %smembers\n", title);
	} else {
		printf("    Explicit %smembers:\n", title);
...
{
	if (varp->implicit == 0) {
		printf("    No implicit %smembers\n", title);
	} else {
		printf("    Implicit %smembers:\n", title);
...
static enum ismem
lookup_recursive(varp, princp, refname, lookup)
	g_varieties	*varp;
	nis_name	princp;
...
{
	if (varp->recursive == 0) {
		printf("    No recursive %smembers\n", title);
	} else {
		printf("    Recursive %smembers:\n", title);
		print_namelist(varp->recursive);
	}
}

static void
printf_varieties(varp, title)
	g_varieties	*varp;
	const char	*title;
{
	printf_explicit(varp, title);
...
static g_entry *
transform_group(gname, gobj)
	nis_name	gname;
	nis_object	*gobj;	/* Assumed to be a GROUP_OBJ */
{
	g_entry		*ge;
	struct timeval	tv;
	int		nm;
...
static g_entry *
cached_group_entry(group, refname, lookup)
	nis_name	group;
	nis_name	refname;
	nis_lookup_func	lookup;
{
...
g_entry		*ge;
{
	...
	if (_thr_main()) {
		...
		v->next = visit_list;
		thr_setspecific(visit_log_key, (void *) v);
	}
}
...
g_entry		*ge;
{
	...
	if (_thr_main()) {
		...
		visit_list = v->next;
		thr_setspecific(visit_log_key, (void *) visit_list);
	}
	free(v);
}
 ...
 */
static enum ismem
do_ismember_2(princp, group, refname, lookup)
	...
	nis_name	group; /* Name of the group to print */
{
	...
	printf("Group entry for \"%s\" group:\n", ge->hdata.name);
 ...
 */
void
__nis_flush_one_group(groupname)	/* === new kinda public interface */
	nis_name groupname;
{
	rw_wrlock(&g_cache_lock);
	if (groups_cache != 0) {
 ...
 */
void
__nis_flush_group_exp_name(groupname)
	nis_name groupname;
{
	char *domainname;
...
nis_name
__nis_splice_name_r(name, splice, buf, bufsize)
	const nis_name	name;
	const char	*splice;
	char		*buf;
	size_t		bufsize;
{
	...
	*p++ = '.';
	...
	strcpy(p, nis_domain_of(name));
	return (buf);
}

nis_name
__nis_map_group_r(name, buf, bufsize)
	const nis_name	name;
	char		*buf;
	size_t		bufsize;
{
...
static nis_object *
get_group(name, refname, lookup, stat)
...
"nislib:get_group() object \"%s\" is not reachable.",
	...
	if (__type_of(NIS_RES_OBJECT(res)) != GROUP_OBJ) {
		if (refname)
			syslog(LOG_ERR,
	...
	nis_name	group;	/* NIS group name */
{
	...
	char		mod_princip_name[NIS_MAXNAMELEN];
	...
	sprintf(name, "%s.%s", obj->zo_name, obj->zo_domain);
	...
	nis_name	group;	/* NIS group name */
{
	...
	if ((res->status == NIS_SUCCESS) || (res->status == NIS_S_SUCCESS)) {
		if (__type_of(res->objects.objects_val) == GROUP_OBJ)
			result = NIS_SUCCESS;
		else
	...
	u_long		flags;
{
	...
	grpobj.zo_data.zo_type = GROUP_OBJ;
