...
#pragma ident	"@(#)nis_perror.c	1.12	93/02/12 SMI"
	...
	nis_error	stat;
{
	...
	switch (stat) {
		...
		case NIS_COLDSTART_ERR :
			res =
	    "Error in accesing NIS+ cold start file, is NIS+ installed?";
			break;
		case NIS_RESYNC :
...
void destroy_err_buf(void *err_buf)
{
	if (err_buf)
		free(err_buf);
}
...
char *
nis_sperror_r(stat, str, buf, len)
	nis_error	stat;
	char		*str;
	char 		buf[];
	int 		len;
{
	...
	len--; /* reserve 1 space for null terminater */
...
#define	ERR_BUF_SIZE 128
	...
	if (_thr_main())
		...
		err_buf = (char *)
			thr_get_storage(&err_buf_key, ERR_BUF_SIZE, destroy_err_buf);
		if (err_buf == NULL) {
