...
#pragma ident	"@(#)nis_db.cc	1.14	94/11/14 SMI"
#include <syslog.h>
...
* operation.
 */
db_result * db_first_entry(char* table_name, int numattrs, nis_attr * attrname)
{
	int jmp_status;
 ...
 */
db_result *
db_add_entry(char* tab, int numattrs, nis_attr* attrname, entry_obj * newobj)
{
	int jmp_status;
	db_result * safety = empty_result(DB_SUCCESS);
	...
	if ((jmp_status = setjmp(dbenv)) == 0) {
		...
		if (tbl == NULL || dbase == NULL)
			...
			db_query *query = InUseDictionary->translate_to_query(tbl,
							numattrs, attrname);
			if (query == NULL)
				return (set_result(safety, DB_BADQUERY));
			db_result * res = dbase->execute(DB_ADD, query,
