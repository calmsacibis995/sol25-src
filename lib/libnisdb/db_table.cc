...
#pragma ident	"@(#)db_table.cc	1.7	93/04/23 SMI"
...
db_table::reset()
{
	...
	for (i = 0; i <= last_used && i < table_size && done < count; i++) {
		if (tab[i]) {
			free_entry(tab[i]);
			++done;
		}
...
db_table::grow()
{
	...
	table_size += DB_TABLE_GROWTH_INCREMENT;
...
{
	long i;
	if (prev >= table_size || tab[prev] == NULL)
		return (NULL);
...
{
	if (where < table_size && tab[where] != NULL)
		return (tab[where]);
	else
		return (NULL);
}
...
{
	if (where < DB_TABLE_START || where >= table_size || tab[where] == NULL)
		return (FALSE);
