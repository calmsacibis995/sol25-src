...
#pragma ident	"@(#)db_index_entry.cc	1.6	93/04/23 SMI"
 ...
 */
db_index_entry_p
db_index_entry::join(long size1, long size2, /* useful ? */
...

{
	...
	for (prev = curr = this; curr != NULL; curr = curr->next) {
		...
		/*
		 * found an entry with the same hash value and key.  Try to find
		 * one with the same record number.
		 */
			for (rp = curr; rp != NULL;  rp = rp->next) {
				if (rp->hashval == hval &&
					rp->key->equal(i, casein) &&
					rp->location == recnum)
