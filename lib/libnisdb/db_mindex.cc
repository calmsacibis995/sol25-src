...
#pragma ident	"@(#)db_mindex.cc	1.9	93/04/23 SMI"
...
db_mindex::remove_aux(entryp recloc)
{
	...
	for (i = 0; i < indices.indices_len; i++) {
		...
		indices.indices_val[curr_ind].remove(comps[i].index_value,
							recloc);
	}
...
db_mindex::add(db_query *q, entry_object * obj)
{
	...
	int i, curr_ind;
	bool_t valid;
	db_index_entry *rp = satisfy_query(q, &count, &valid);
	db_status rstat;
	...
	if (count == 0) { 	/* not found, insert */
		...
		for (i = 0; i < indices.indices_len; i++) {
			...
			indices.indices_val[curr_ind].add(comps[i].index_value,
							recloc);
		}
		delete cq;  // clean up
		return (DB_SUCCESS);
