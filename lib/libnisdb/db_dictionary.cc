...
#pragma ident	"@(#)db_dictionary.cc	1.15	93/10/05 SMI"
	...
	for (i = 0; i < dictionary->tables.tables_len; i++) {
		...
		if (bucket) {
			...
			while (np != NULL) {
				...
				/* We know we don't have a log file, so we will just
				 * add to the in-memory database and dump all of it
				 * once we are done.
				 */
				status = add_to_dictionary (FreeDictionary->dictionary, 
								clone);
				if (status != DB_SUCCESS ) {
					delete_table_desc (clone);
					return (DB_INTERNAL_ERROR);	
...
int
db_dictionary::incorporate_log(char* filename)
{
	db_dictlog f(filename, PICKLE_READ);
...
db_dictionary::open_log()
{
	...
	if ((logfile->open()) == NULL){
		WARNING_M (
			"db_dictionary::open_log: could not open log file: ");
		delete logfile;
		logfile = NULL;
		return (-1);
	}
...
db_dictionary::db_standby(char* tab)
{
	...
	if (!initialized) return (DB_BADDICTIONARY);
...
db_dictionary::find_table(char* tab, db_table_desc **where)
{
	...
	if (tbl == NULL) return (NULL);		// not found

	if (tbl->database != NULL) return (tbl->database);  // return handle
