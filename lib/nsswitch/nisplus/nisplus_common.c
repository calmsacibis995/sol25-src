...
#pragma ident "@(#)nisplus_common.c	1.28	95/10/14 SMI"
...
_nss_nisplus_list(name, extra_flags, res_p)
	...
	nis_result	**res_p;
{
	*res_p = nis_list((char *)name, NIS_LIST_COMMON | extra_flags, 0, 0);
	...
	const char		*keyname;
{
	...
	sprintf(namebuf, "[%s=%s]%s", column_name, keyname,
		be->table_name);
	r = nis_list(namebuf, NIS_LIST_COMMON | USE_DGRAM, 0, 0);
	...
	const char		*table; /* one component table name (no dots) */
{
	...
	r = nis_list(namebuf, EXPAND_NAME | USE_DGRAM | NIS_LIST_COMMON, 0, 0);
	res = process_val(argp, be, r);
	if (r != 0)
		nis_freeresult(r);
