...
#pragma ident	"@(#)files_common.c	1.16	95/07/26 SMI"
...
retry:
	...
	if ((fhp->fh_file_start = malloc(st.st_size + 1)) == NULL)
		goto unavail;

	if ((fd = open(be->filename, O_RDONLY)) < 0)
		goto retry;

	if (read(fd, fhp->fh_file_start, st.st_size) != st.st_size) {
		close(fd);
		goto retry;
	}
	...
	fhp->fh_file_end = fhp->fh_file_start + st.st_size;
	*fhp->fh_file_end = '\n';
	 ...
	 * For example, root and smtp both have uid 0; but we certainly don't
	 * want getpwuid(0) to return smtp.
	 */
	for (ht = 0; ht < fhp->fh_nhtab; ht++) {
