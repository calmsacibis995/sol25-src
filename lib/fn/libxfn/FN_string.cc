...
#pragma ident "@(#)FN_string.cc	1.3 94/10/03 SMI"
...
{
	if (use_mb())
		constr(new string_wchar(0, 0, 0, DEFAULT_CODESET));
	else
		constr(new string_char(0, 0, 0, DEFAULT_CODESET));
}
	...
	unsigned int *status)
{
	...
	case LATIN1_CODESET:
		r = new string_char((const char *)contents,
				    charcount, bytecount, code_set);
		if (constr(r, locale_info, locale_bytes))
			sts = FN_SUCCESS;
		else
...
unsigned long
FN_string::code_set(const void **locale_info, size_t *locale_bytes) const
{
	if (rep)
		return (rep->code_set(locale_info, locale_bytes));
	else
		return (0);
}
	...
	...)
{
	...
	if (s1 == 0) {
		r = new string_char(0, 0, 0, DEFAULT_CODESET);
		if (!constr(r)) {
			if (status)
				*status = FN_E_INSUFFICIENT_RESOURCES;
		}
	...
	// make sure all the instances are alive and of the same type.
	...
	int total_charcount = s1->charcount() + s2->charcount();
	va_start(ap, s2);
	while (sn = va_arg(ap, const FN_string *)) {
