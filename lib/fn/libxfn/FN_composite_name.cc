...
#pragma ident	"@(#)FN_composite_name.cc	1.6	94/08/03 SMI"
...
extract_comp(unsigned char *comp, const unsigned char *&str)
{
	for (; *str; str++)
		...
		case FN_COMPOSITE_NAME_QUOTE1:
			for (str++; *str && *str != FN_COMPOSITE_NAME_QUOTE1;
			    str++)
				switch (*str) {
				case FN_COMPOSITE_NAME_ESCAPE:
					*comp++ = *++str;
...
stringify_comp(unsigned char *&str, const unsigned char *comp)
{
	...
	while (*comp) {
		if (*comp == FN_COMPOSITE_NAME_ESCAPE ||
		    *comp == FN_COMPOSITE_NAME_QUOTE1)
...
list_to_string(List &l)
{
	...
	for (size = 0, i = (const NameListItem *)(l.first(ip)); i;
		...
		size += 2+2*strlen((const char *)(i->name.str(&status)));
