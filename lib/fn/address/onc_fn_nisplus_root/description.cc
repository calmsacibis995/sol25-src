...
#pragma ident "@(#)description.cc	1.1 95/01/29 SMI"
...
static const
FN_identifier onc_fn_nisplus_root_type((unsigned char *)"onc_fn_nisplus_root");
...
extern "C"
FN_string_t *
onc_fn_nisplus_root__fn_ref_addr_description(const FN_ref_addr_t *addrt,
    unsigned detail, unsigned *more_detail)
{
	...
	int is_okaddr = (*tp == onc_fn_nisplus_root_type);
	if (!is_okaddr || contents == 0 || detail > 1) {
		if (more_detail)
			*more_detail = detail;
		char *msg = 0;
		if (!is_okaddr)
			msg = "Address type is not 'onc_fn_nisplus_root'";
		else if (contents == 0)
