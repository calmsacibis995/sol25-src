...
#pragma ident "@(#)to_ref.cc	1.1 95/01/29 SMI"
...
// module: fn_inet_nisplus.so
//
// function: nisplus__txt_record_to_addrs()
//
// 1 OK
// 0 failed
...
//
//	TXT XFNNISPLUS NIS+_root_domain server_name [server_ipaddr]
//
// NIS+_root_domain is the fully qualified name of the NIS+ root domain.
//
// NIS+_root_domain, server_name and server_ipaddr
// are passed to this routine, which uses
// this information to construct an address:
// type:	onc_fn_nisplus_root
// contents:	NIS+_root_domain server_name server_ipaddr
//
//
// For example, if the text record is
//
//	TXT XFNNISPLUS Test.COM. alto
//
...
static const FN_identifier
    onc_fn_nisplus_root_type((unsigned char *)"onc_fn_nisplus_root");
...
extern "C" int
nisplus__txt_records_to_addrs(int num_records, const char **records,
    FN_ref_t *cref, FN_status_t *cstatus)
{
	...
	for (i = 0; i < num_records; i++) {
		// %%% no syntactic checks on record contents
		// %%% should do syntatic check
		// XDR encode text information
		...
		addr = new FN_ref_addr(onc_fn_nisplus_root_type,
