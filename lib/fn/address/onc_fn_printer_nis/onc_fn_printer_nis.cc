...
#pragma ident	"@(#)onc_fn_printer_nis.cc	1.1	94/11/02 SMI"
...
static inline int
FNSP_printer_nis_address_p(const FN_ref_addr &addr)
{
	return ((*addr.type()) ==
		FNSP_printer_nis_address_type);
}
...
extern "C"
FN_string_t *
onc_fn_printer_nis__fn_ref_addr_description(const FN_ref_addr *addrt,
	unsigned detail, unsigned *more_detail)
{
	...
	int is_FNSP_addr = FNSP_printer_nis_address_p(addr);
