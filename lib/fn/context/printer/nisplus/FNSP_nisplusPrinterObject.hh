...
#pragma ident	"@(#)FNSP_nisplusPrinterObject.hh	1.4	94/11/29 SMI"

#include <xfn/fn_spi.hh>
#include "../FNSP_printer_Address.hh"

class FNSP_nisplusPrinterObject : public FN_ctx_csvc_strong {
public:
	~FNSP_nisplusPrinterObject();

	static FNSP_nisplusPrinterObject* from_address(const FN_ref_addr&,
	    const FN_ref&, FN_status& stat);

	FN_ref* get_ref(FN_status&) const;
#ifdef DEBUG
	// probably not used (only for testing)
	FNSP_nisplusPrinterObject(const FN_string&);
	FNSP_nisplusPrinterObject(const FN_ref&);
#endif
