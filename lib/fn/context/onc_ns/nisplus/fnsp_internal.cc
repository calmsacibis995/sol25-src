...
#pragma ident "@(#)fnsp_internal.cc	1.20 95/01/31 SMI"
...
static const char *nis_default_path = "";
...
static const char *FNSP_attrid_col_label = "attridentifier";
static const char *FNSP_attrvalue_col_label = "attrvalue";
...
const FN_string FNSP_nns_name((unsigned char *)"_FNS_nns_");
    ...
    unsigned string_case)
{
	...
	tobj.TA_data.ta_type = (char *) nis_default_table_type;
	tobj.TA_data.ta_maxcol = num_cols;
	tobj.TA_data.ta_sep = nis_default_separator;
	tobj.TA_data.ta_path = (char *) nis_default_path;
    ...
    unsigned nisflags)
{
	...
	case FNSP_shared_table_impl:
		return (FNSP_lookup_binding_shared(parent.get_table_name(),
		    FNSP_self_name, aname,
		    status, btype, nisflags));
    ...
    const FN_string *cname = 0)
{
	...
	eo = &(obj.EN_data);
	eo->en_type = (char *)nis_default_table_type;
...
unsigned
FNSP_remove_binding(const FNSP_Address& parent, const FN_string &aname)
{
	FNSP_binding_type btype;
	unsigned status;
...
//
unsigned
FNSP_destroy_context(const FNSP_Address &parent, const FN_string *dirname)
{
    ...
    int find_legal_childname)
{
	...
	if (iname) {
		answer = new FNSP_Address(*iname, context_type, repr_type);
		delete iname;
	}
    ...
    const FN_identifier *ref_type)
{
	...
	FNSP_Address *child_addr = compose_child_addr(parent, childname,
    ...
    void *add_params)
{
	...
	if (res->status == NIS_RPCERROR) {
		...
		unsigned long new_flags =
		    access_flags|(FNSP_nisflags&(~USE_DGRAM));
		res = nis_list(dirname_nn, new_flags, add_func, add_params);
	}
    ...
    FN_status &stat)
{
	...
	const FN_string *old_longest_name = 0;
...
// ***************** host and user functions **************************

/*
 * For supporting hostname and username, and host and user contexts.
 */
...
extract_hostent(nis_result *res, unsigned &status)
{
	struct hostent * hi;
	nis_object *obj;
	int nobjs, i, j, len, addr_found;
	char *addr;

	hi = (struct hostent *) calloc(1, sizeof (struct hostent));
	if (hi == 0) {
		status = FN_E_INSUFFICIENT_RESOURCES;
		return (0);
	}
	...
	for (i = 0; i < nobjs; i++) {
		...
		if (hi->h_name == 0) {
			...
			hi->h_name = (char *)malloc(len+1);
			if (hi->h_name == 0) {
				status = FN_E_INSUFFICIENT_RESOURCES;
				break;
			}
			memcpy(hi->h_name, ENTRY_VAL(obj, HOST_CNAME_COL),
			    len);
			hi->h_name[len] = '\0';
		}
		/* Aliases */
		if (hi->h_aliases == 0) {
			...
			// the alias name
			// found in each entry should be identical
			hi->h_aliases = (char **) calloc(2, sizeof (char *));
			if (hi->h_aliases == 0) {
				status = FN_E_INSUFFICIENT_RESOURCES;
				break;
			}
			len = ENTRY_LEN(obj, HOST_ALIAS_COL);
			hi->h_aliases[0] = (char *)malloc(len+1);
			if (hi->h_aliases[0] == 0) {
				status = FN_E_INSUFFICIENT_RESOURCES;
				break;
			}
			memcpy(hi->h_aliases[0],
			    ENTRY_VAL(obj, HOST_ALIAS_COL), len);
			hi->h_aliases[0][len] = '\0';
		}

		/* Address */
		if (hi->h_addr_list == 0) {
			// allocate maximum
			hi->h_addr_list = (char **)calloc(nobjs,
			    sizeof (char *));
			if (hi->h_addr_list == 0) {
				status = FN_E_INSUFFICIENT_RESOURCES;
				break;
		...
		len = ENTRY_LEN(obj, HOST_ADDR_COL);
		addr = ENTRY_VAL(obj, HOST_ADDR_COL);
		addr_found = 0;
		for (j = 0; j < hi->h_length && addr_found == 0; j++) {
			if (memcmp(hi->h_addr_list[j], addr, len) == 0) {
				addr_found = 1;
				break;
			}
		}
		if (addr_found == 0) {
			hi->h_addr_list[hi->h_length] = (char *)malloc(len+1);
			if (hi->h_addr_list[hi->h_length] == 0) {
				status = FN_E_INSUFFICIENT_RESOURCES;
				break;
			}
			memcpy(hi->h_addr_list[hi->h_length], addr, len);
			hi->h_addr_list[hi->h_length][len] = '\0';
			hi->h_length++;  // advance to next cell
		}
	}
    ...
    struct hostent **hostinfo)
{
	...
	sprintf(sname, "[name=\"%s\"],%s.%s",
	    hostname.str(), HOST_TABLE, dirname.str());
	if (sname[strlen(sname)-1] != '.')
		strcat(sname, ".");
	// use same flags as gethostbyname()
	res = nis_list(sname,
	    FOLLOW_PATH|FOLLOW_LINKS|USE_DGRAM|access_flags,
	    NULL, NULL);
	status = FNSP_map_result(res, 0);
...
//
// Uses FOLLOW_PATHS and FOLLOW_LINKS for passwd table lookup.
//
FN_string *
    ...
    unsigned int access_flags, unsigned &status)
{
	...
	sprintf(sname, "[name=\"%s\"],%s.%s",
	    username.str(), PASSWD_TABLE, dirname.str());
	if (sname[strlen(sname)-1] != '.')
		strcat(sname, ".");
	// use same flags as getpwnam()
	pass_res = nis_list(sname,
	    FOLLOW_PATH|FOLLOW_LINKS|USE_DGRAM|access_flags,
	    NULL, NULL);
...
int
FNSP_decompose_index_name(const FN_string &name,
...
int
FNSP_change_binding_ownership(const FN_ref &parent,
    const FN_string &name,
    const FN_string &owner)
{
	...
	FNSP_Address parent_addr(parent);
	...
	case FNSP_shared_table_impl:
		sprintf(sname, "[%s=\"%s\",%s=\"%s\"],%s",
		    FNSP_name_col_label, name.str(),
		    FNSP_ctx_col_label, FNSP_self_name.str(),
		    tablename);
		break;
	case FNSP_entries_impl:
		sprintf(sname, "[%s=\"%s\",%s=\"%s\"],%s",
...
// Change ownership of context of 'ref' to 'owner'
// return 1 for success; 0 failure.
//
// Currently supports only single and shared table implementations.
//
// Note that for 'shared_table_impl', subcontexts that
// share the same table are also changed.
//
int
FNSP_change_context_ownership(const FN_ref &ref, const FN_string &owner)
{
	unsigned status = 0;
	nis_result *res, *mres;
	nis_object newobj;
	...
	FNSP_Address addr(ref);

	if (addr.get_context_type() == 0)
		// could not construct proper address
		return (0);

	switch (addr.get_impl_type()) {
	case FNSP_shared_table_impl:
	case FNSP_single_table_impl:
		break;
	default:
		// the other implementation types are not supported yet
		return (0);
	}

	tablename = (nis_name)addr.get_table_name().str();

	// modify ownership of table object
	res = nis_lookup(tablename, MASTER_ONLY|FNSP_nisflags);
	if (res == 0) {
#ifdef DEBUG
		fprintf(stderr,
		    "Lookup of table '%s' failed: (no status returned)\n",
		    tablename);
#endif /* DEBUG */
		free_nis_result(res);
		return (0);
	...
	if (res->status != NIS_SUCCESS) {
#ifdef DEBUG
		fprintf(stderr, "Lookup of table '%s' failed: %s\n",
		    tablename, nis_sperrno(res->status));
#endif /* DEBUG */
		free_nis_result(res);
		return (0);
		// could not lookup table
	}


	newobj = res->objects.objects_val[0];
	saveowner = newobj.zo_owner;  // save for cleanup
	newobj.zo_owner = (nis_name)owner.str();
	mres = nis_modify(tablename, &newobj);
...
#ifdef DEBUG
		fprintf(stderr, "Could not modify ownership of '%s': %s\n",
		    tablename, nis_sperrno(mres->status));
#endif /* DEBUG */
		free_nis_result(mres);
		return (0);
	}
	free_nis_result(mres);
	status = 0;
	// modify ownership of table contents (bindings)
	res = __nis_list_localcb(tablename, MASTER_ONLY|FNSP_nisflags,
	    change_entry_ownership, (void *)owner.str());
    ...
    unsigned &status, unsigned nisflags)
{
	...
	if (res->status == NIS_RPCERROR) {
		...
		res = nis_list(sname, new_flags|nisflags,
...
FNSP_get_attrset(const FNSP_Address &context, const FN_string &atomic_name,
    unsigned &status)
{
	return (FNSP_get_attrset_aux(context, atomic_name, status,
...
int
FNSP_set_attrset(const FNSP_Address &context,
    const FN_string &atomic_name,
    FN_attrset &attrset)
{
	unsigned status = FN_E_UNSPECIFIED_ERROR;
    ...
    const FN_attribute &attribute)
{
	...
	howmany = attribute.valuecount();
	value = attribute.first(ip);
	for (i = 0; value && i < howmany; i++) {
    ...
    unsigned int &status, FN_ref **ref)
{
	...
	if (status == FN_SUCCESS) {
		name = strdup(
			ENTRY_VAL(res->objects.objects_val, FNSP_NAME_COL));
		if (name == 0) {
			iter_pos.n_len = 0;
			iter_pos.n_bytes = 0;
			if (ref)
