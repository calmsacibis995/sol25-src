...
#ifndef _FNSP_NULLCONTEXT_HH
#define	_FNSP_NULLCONTEXT_HH

#pragma ident	"@(#)FNSP_NullContext.hh	1.6	94/11/28 SMI"
...
public:
	...
	FNSP_NullContext(const FN_identifier &addr,
	    const FN_ref&, ReferenceType from = copy,
	    unsigned context_type = FNSP_null_context);
	~FNSP_NullContext();

	static FNSP_NullContext* from_ref(const FN_ref&, FN_status&);
	FN_ref* get_ref(FN_status&) const;

private:
	FN_ref *my_reference;
	FN_ref *nns_reference;
	const FN_identifier *my_address_type;

#ifdef NOTNEEDED
	FNSP_NullContext(const FN_identifier &addr,
	    const FN_string&, const FN_string&,
	    unsigned, unsigned);
#endif NOTNEEDED

	// internal function
	const FN_ref* resolve(const FN_string& name, FN_status_csvc&);
