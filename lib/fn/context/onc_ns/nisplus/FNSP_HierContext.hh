...
#ifndef _FNSP_HIERCONTEXT_HH
#define	_FNSP_HIERCONTEXT_HH

#pragma ident	"@(#)FNSP_HierContext.hh	1.6	94/11/28 SMI"
...
class FNSP_HierContext : public FN_ctx_asvc_strong,
public FNSP_Context {
public:
	~FNSP_HierContext();
...
private:
	FN_ref *resolve(const FN_string &name, FN_status_asvc&);

protected:
	FN_ref *my_reference;  /* encoded */
	FNSP_Address *my_address;  /* decoded */
	const FN_syntax_standard *my_syntax;

	FNSP_HierContext(const FN_ref_addr &, const FN_ref &);

	// atomic name service interface
	FN_string *c_component_parser(const FN_string &,
	    FN_string **rest,
	    FN_status_csvc&);
	...
	FN_attribute *a_attr_get(const FN_string &,
	    const FN_identifier &,
	    FN_status_asvc&);
	int a_attr_modify(const FN_string &, unsigned int,
	    const FN_attribute&,
	    FN_status_asvc&);
	...
	int a_rename_nns(const FN_string &, const FN_composite_name &newn,
	    unsigned BindFlags, FN_status_asvc&);
