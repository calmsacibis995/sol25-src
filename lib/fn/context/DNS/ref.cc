...
#pragma ident "@(#)ref.cc	1.6 94/10/20 SMI"
	...
	for (first = 0; first < ac; first += count) {
		...
		if (first + 1 < ac) {
			...
			while (first + count < ac) {
				if (strncmp(av[first], av[first + count], len) == 0 &&
				    (av[first + count][len] == ' ' ||
				    av[first + count][len] == '\t'))
					count += 1;
				else
...
demangle(int count, const char **av, FN_ref &r, FN_status &s)
{
	...
	sprintf(func, "%s__txt_records_to_addrs", tag);

	if (fn = (int (*)(int, const char *[], FN_ref_t *, FN_status_t *))
	    fns_link_symbol(func)) {
		rval = fn(count, txtv, (FN_ref_t *)&r, (FN_status_t *)&s);
		delete[] txtv;
		return (rval);
	}

	sprintf(module, "fn_inet_%s.so", tag);
