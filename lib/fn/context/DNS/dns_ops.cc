...
#pragma ident "@(#)dns_ops.cc	1.10 94/10/24 SMI"
...
/*
 * this bit should really be in resolv.h
 *
...
extern int res_mkquery(
	...
	int datalen,
	struct rrec *newrr,
	void *buf,		// must be HEADER aligned
	int buflen);
	// MT: reads _res.options
extern int res_send(const char *qbuf, int qlen, char *abuf, int alen);
extern int res_query(const char *name,
	int _class, int type,
	unsigned char *answer, int anslen);
// the following 3 seem to be MT safe
extern int dn_skipname(const u_char *comp_dn, const u_char *eom);
extern int dn_expand(
	const u_char *msg, const u_char *eomorig,
	const u_char *comp_dn,
	u_char *exp_dn, int exp_dn_length);
extern int dn_comp(
	u_char *exp_dn, u_char *comp_dn,
	int length,
...
static int txt_expand(unsigned char *cp, int dlen, unsigned char *txt, int txt_len);
...
dns_client::list_names(const char *dom, dns_rr_vec &dom_vec)
{
	...
	if (get_servers(dom, server_rr, &qual) < 0) {
		if (dns_trace)
			fprintf(stderr, "dns_client::list_names: get_dom_servers_inet(\"%s\") failed\n", dom);
		mutex_unlock(&class_mutex);
		return (-1);	
	}
	if (qual) {
		...
		if (qual_len > 0 && qual[qual_len - 1] == '.')
			sprintf(qual_dom, "%s.%.*s", dom, qual_len - 1, qual);
		else
	...
	if (qlen < 0) {
		if (dns_trace)
			fprintf(stderr, "dns_client::list_names: res_mkquery() failed\n");
		mutex_unlock(&class_mutex);
		return (-1);	
	}
	...
	if ((dns_fd = connect_inet(server_rr.in_vec)) < 0) {
		if (dns_trace)
			fprintf(stderr, "dns_client::list_names: connect_inet failed\n");
		mutex_unlock(&class_mutex);
		return (-1);	
	}
	...
	if (dns_trace)
		fprintf(stderr, "dns_client::list_names: no SOA in response?\n");
	disconnect(dns_fd);
	mutex_unlock(&class_mutex);
	return (-1);	
}
	...
	const char **soa)
{
	...
	if ((cc = mread(dns_fd, &pkt, s)) != s) {
		if (dns_trace)
			fprintf(stderr, "mread data part failed (%d != expected %d)\n", cc, s);
		goto error;
	}
	...
	if (ntohs(pkt.hdr.qdcount) > 0) {
		nlen = dn_skipname((const u_char *)p, (const u_char *)abuf_bound);
		if (nlen < 0)
			goto error;
		p += nlen + QFIXEDSZ;
	}
	...
	rrname[0] = '\0';
	if ((nlen = dn_expand((u_char *)&pkt, (u_char *)abuf_bound,
			(const u_char *)p, (u_char *)rrname, sizeof (rrname))) < 0)
		goto error;
	p += nlen;
	...
	if (type == T_SOA) {
		...
		if (*soa == 0) {
			*soa = strdup(rrname);
			...
			if (dns_trace)
				fprintf(stderr, "unexpected SOA <%s>\n", rrname);
			goto error;
		}
		blen = dn_expand((u_char *)&pkt, (const u_char *)abuf_bound,
			(const u_char *)p, (u_char *)buf, sizeof (buf));
		if (blen <= 0) {
			if (dns_trace)
	...
	if (parse_rrdata(type, (const char *)&pkt, (const char *)abuf_bound, p, *rr) < 0)
		goto error;
...
dns_client::get_servers(const char *dom, dns_rr &rr, const char **qualp)
{
	...
	alen = query(dom, T_NS, (unsigned char *)&abuf, sizeof (abuf), qualp);
	if (alen < 0) {
		if (dns_trace)
			fprintf(stderr, "get_server_names: res_query() failed\n");
		return (-1);
	}
