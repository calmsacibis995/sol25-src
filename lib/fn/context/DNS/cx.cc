...
#pragma ident "@(#)cx.cc	1.9 94/10/20 SMI"
...
DNS_ctx::DNS_ctx(const FN_ref_addr &a)
{
	int		len;
	const char	*dom;
...
DNS_ctx::c_lookup(const FN_string &name, unsigned int, FN_status_csvc &cs)
{
	...
	if (dns_cl->lookup_name(qname, rr) < 0) {
...
DNS_ctx::c_list_names(const FN_string &name, FN_status_csvc &cs)
{
	...
	if ((nl = new dns_nl) == 0) {
		cs.set_error(FN_E_INSUFFICIENT_RESOURCES, *self_reference, name);
		return (0);
	}

	if (get_names(dns_cl, qname, nl->names, 0) < 0) {
		if (trace)
			fprintf(stderr,
...
DNS_ctx::c_list_bindings(const FN_string &name, FN_status_csvc &cs)
{
	...
	if (get_names(dns_cl, qname, bl->names, 1) < 0) {
		if (trace)
			fprintf(stderr,
...
my_dom(const char *domain, const char *n, const char **qnamep, char *qbuf)
{
	if (domain && domain[0] != '\0') {
		if (n && n[0] != '\0') {
			sprintf(qbuf, "%.*s.%.*s", MAXDNAME, n, MAXDNAME, domain);
			*qnamep = qbuf;
		} else
			*qnamep = domain;
	} else {
...
int
DNS_ctx::get_names(dns_client *dns_cl, const char *dom, dns_rr_vec &names, int all)
{
	...
	if (all) {
		...
		for (r = 0; r < rc; ++r) {
			rp = (dns_rr *)rv[r];
			rp->ns_vec.get_vec(nc, nv);
			if (nc > 0)
				dns_cl->lookup_name(rp->rname(), *rp);
#ifdef DEBUG
			{
				int	i, ac;
				dns_el	**av;
...
make_host_ref(FN_ref *&ref, const dns_rr &rr)
{
	...
	rr.in_vec.get_vec(ec, ev);
	if ((rval = add_inet_addr(ref, ec, (dns_in **)ev)) < 0) {
		delete ref;
		ref = 0;
		return (-1);
	}
...
make_dom_ref(FN_ref *&ref, const dns_rr &rr)
{
	...
	rr.in_vec.get_vec(ec, ev);
	if (add_inet_addr(ref, ec, (dns_in **)ev) < 0) {
		delete ref;
		ref = 0;
		return (-1);
	}
