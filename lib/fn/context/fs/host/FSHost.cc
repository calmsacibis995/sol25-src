...
#pragma ident	"@(#)FSHost.cc	1.2	94/12/16 SMI"
...
FSHost::FSHost(const FN_ref &ref)
{
...
FSHost::cn_get_syntax_attrs(const FN_composite_name &, FN_status_cnsvc &cnstat)
{
	...
	static const FN_syntax_standard
	    syntax(FN_SYNTAX_STANDARD_DIRECTION_LTR,
		    FN_STRING_CASE_SENSITIVE, &slash, NULL, NULL, NULL);
...
FN_attribute *
FSHost::cn_attr_get(const FN_composite_name &name, const FN_identifier &attr,
    FN_status_cnsvc &cnstat)
{
	static FN_identifier
	    syntax((const unsigned char *)"fn_attr_syntax_ascii");

	if (attr != attr_exported) {	// "exported" is the only attribute
		return (NULL);
	}
...
FSHost::cn_attr_modify(const FN_composite_name &name, unsigned int,
    const FN_attribute &, FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (0);
}
...
FSHost::cn_attr_get_values(const FN_composite_name &name,
    const FN_identifier &, FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (NULL);
}
...
FN_attrset *
FSHost::cn_attr_get_ids(const FN_composite_name &name, FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (NULL);
}
...
FN_multigetlist *
FSHost::cn_attr_multi_get(const FN_composite_name &name, const FN_attrset *,
    FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (NULL);
...
FSHost::cn_attr_multi_modify(const FN_composite_name &name,
    const FN_attrmodlist &, FN_attrmodlist **, FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (0);
}
...
FSHost::cn_attr_get_nns(const FN_composite_name &name,
    const FN_identifier &attr, FN_status_cnsvc &cnstat)
{
	return (cn_attr_get_nns(name, attr, cnstat));
}
...
FSHost::cn_attr_multi_get_nns(const FN_composite_name &name,
    const FN_attrset *, FN_status_cnsvc &cnstat)
{
	cnstat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (NULL);
}
