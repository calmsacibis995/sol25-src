...
#pragma ident "@(#)FN_ctx_asvc_strong.cc	1.5 94/11/20 SMI"
...
FN_ctx_asvc_strong::c_attr_modify(const FN_string &n, unsigned int i,
    const FN_attribute &a, FN_status_csvc &cs)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_modify(n, i, a, as);
	}
	return (ret);
}
...
FN_ctx_asvc_strong::c_attr_get_values(const FN_string &n,
    const FN_identifier &i, FN_status_csvc &cs)
{
	FN_valuelist *ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_get_values(n, i, as);
	}
	return (ret);
}
...
FN_ctx_asvc_strong::c_attr_multi_modify(const FN_string &n,
    const FN_attrmodlist &m, FN_attrmodlist **a, FN_status_csvc &cs)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_multi_modify(n, m, a, as);
	}
	return (ret);
}
...
FN_ctx_asvc_strong::c_attr_modify_nns(const FN_string &n,
    unsigned int i, const FN_attribute &a, FN_status_csvc &cs)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_modify_nns(n, i, a, as);
	}
	return (ret);
}
    ...
    const FN_identifier &i, FN_status_csvc &cs)
{
	...
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_get_values_nns(n, i, as);
	}
	return (ret);
}
...
FN_ctx_asvc_strong::c_attr_multi_get_nns(const FN_string &n,
    const FN_attrset *i, FN_status_csvc &cs)
{
	FN_multigetlist *ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_multi_get_nns(n, i, as);
	}
	return (ret);
}
...
FN_ctx_asvc_strong::c_attr_multi_modify_nns(const FN_string &n,
    const FN_attrmodlist &a, FN_attrmodlist **l, FN_status_csvc &cs)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cs)) {
		// get final contexts attrs
		FN_status_asvc as(cs);
		ret = a_attr_multi_modify_nns(n, a, l, as);
	}
	return (ret);
}
