...
#pragma ident "@(#)fn_spi.hh	1.6 94/11/20 SMI"
...
public:
	...
	FN_attribute *attr_get(const FN_composite_name &,
	    const FN_identifier &, FN_status &);
	int attr_modify(const FN_composite_name &, unsigned int,
	    const FN_attribute&, FN_status &);
	FN_valuelist *attr_get_values(const FN_composite_name &,
	    const FN_identifier &, FN_status &);
	FN_attrset *attr_get_ids(const FN_composite_name &, FN_status &);
	FN_multigetlist *attr_multi_get(const FN_composite_name &,
	    const FN_attrset *, FN_status &);
	int attr_multi_modify(const FN_composite_name &,
	    const FN_attrmodlist&, FN_attrmodlist **, FN_status &);
	// get_ref is inherited directly by subclass
	// construct FN_ctx_svc from a reference
	static FN_ctx_svc* from_ref(const FN_ref &, FN_status &);
	static FN_ctx_svc* from_initial(FN_status &);
	...
	virtual FN_attribute *p_attr_get(const FN_composite_name &,
	    const FN_identifier &, FN_status_psvc&) = 0;
	virtual int p_attr_modify(const FN_composite_name &,
	    unsigned int, const FN_attribute&, FN_status_psvc&) = 0;
	...
	virtual FN_multigetlist *p_attr_multi_get(const FN_composite_name &,
	    const FN_attrset *, FN_status_psvc&) = 0;
	virtual int p_attr_multi_modify(const FN_composite_name &,
	    const FN_attrmodlist&, FN_attrmodlist **, FN_status_psvc&) = 0;
};
...
public:
	...
	virtual FN_ref *cn_lookup(const FN_composite_name &name,
	    unsigned int f, FN_status_cnsvc&) = 0;
	...
	virtual FN_attribute *cn_attr_get(const FN_composite_name &,
	    const FN_identifier &, FN_status_cnsvc&) = 0;
	virtual int cn_attr_modify(const FN_composite_name &,
	    unsigned int, const FN_attribute&, FN_status_cnsvc&) = 0;
	...
	virtual FN_multigetlist *cn_attr_multi_get(const FN_composite_name &,
	    const FN_attrset *, FN_status_cnsvc&) = 0;
	virtual int cn_attr_multi_modify(const FN_composite_name &,
	    const FN_attrmodlist&, FN_attrmodlist **, FN_status_cnsvc &) = 0;
	virtual FN_ref *cn_lookup_nns(const FN_composite_name &name,
	    unsigned int flags, FN_status_cnsvc &) = 0;
...
public:
	...
	FN_ref *p_lookup(const FN_composite_name &, unsigned int f,
	    FN_status_psvc &);
	...
	FN_attribute *p_attr_get(const FN_composite_name &,
	    const FN_identifier &, FN_status_psvc &);
	int p_attr_modify(const FN_composite_name &,
	    unsigned int, const FN_attribute &, FN_status_psvc &);
	...
	FN_multigetlist *p_attr_multi_get(const FN_composite_name &,
	    const FN_attrset *, FN_status_psvc &);
	int p_attr_multi_modify(const FN_composite_name &,
	    const FN_attrmodlist &, FN_attrmodlist **, FN_status_psvc &);
};
...
protected:
	...
	FN_ref *cn_lookup(const FN_composite_name &,
	    unsigned int f, FN_status_cnsvc &);
	...
	int cn_attr_modify(const FN_composite_name &,
	    unsigned int, const FN_attribute &, FN_status_cnsvc &);
	...
	virtual int c_attr_modify(const FN_string &, unsigned int,
	    const FN_attribute&, FN_status_csvc &) = 0;
	...
	virtual FN_multigetlist *c_attr_multi_get(const FN_string &,
	    const FN_attrset *, FN_status_csvc &) = 0;
	virtual int c_attr_multi_modify(const FN_string &,
	    const FN_attrmodlist &, FN_attrmodlist **, FN_status_csvc &) = 0;
	...
	virtual FN_attribute *c_attr_get_nns(const FN_string &,
	    const FN_identifier &, FN_status_csvc &) = 0;
	virtual int c_attr_modify_nns(const FN_string &, unsigned int,
	    const FN_attribute &, FN_status_csvc &) = 0;
...
public:
	...
	virtual FN_attribute *a_attr_get(const FN_string &,
	    const FN_identifier &, FN_status_asvc &) = 0;
	virtual int a_attr_modify(const FN_string &,
	    unsigned int, const FN_attribute &, FN_status_asvc &) = 0;
...
public:
	...
	FN_ref *cn_lookup(const FN_composite_name &name, unsigned int f,
	    FN_status_cnsvc &);
