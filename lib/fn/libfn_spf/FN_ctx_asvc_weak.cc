...
#pragma ident "@(#)FN_ctx_asvc_weak.cc	1.5 94/11/20 SMI"
...
FN_ctx_asvc_weak::cn_attr_get_values(const FN_composite_name &n,
    const FN_identifier &a, FN_status_cnsvc &cns)
{
	FN_valuelist *ret = 0;
	if (resolve_to_penultimate_context(this, n, cns)) {
		FN_status_asvc as(cns);
...
FN_ctx_asvc_weak::cn_attr_multi_modify(const FN_composite_name &n,
    const FN_attrmodlist &m, FN_attrmodlist **u, FN_status_cnsvc &cns)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cns)) {
		FN_status_asvc as(cns);
		FN_string *fn = n.string();
		if (fn) {
			ret = a_attr_multi_modify(*fn, m, u, as);
			delete fn;
		} else {
			cns.set(FN_E_INSUFFICIENT_RESOURCES);
		}
    ...
    unsigned int i, const FN_attribute &a, FN_status_cnsvc &cns)
{
	...
	if (resolve_to_penultimate_context(this, n, cns)) {
		...
		FN_string *fn = n.string();
		if (fn) {
			ret = a_attr_modify_nns(*fn, i, a, as);
			delete fn;
		} else {
			cns.set(FN_E_INSUFFICIENT_RESOURCES);
		}
    ...
    const FN_identifier &i, FN_status_cnsvc &cns)
{
	...
	if (resolve_to_penultimate_context(this, n, cns)) {
		...
		FN_string *fn = n.string();
		if (fn) {
			ret = a_attr_get_values_nns(*fn, i, as);
			delete fn;
		} else {
			cns.set(FN_E_INSUFFICIENT_RESOURCES);
		}
    ...
    const FN_attrset *a, FN_status_cnsvc &cns)
{
	...
	if (resolve_to_penultimate_context(this, n, cns)) {
		...
		FN_string *fn = n.string();
		if (fn) {
			ret = a_attr_multi_get_nns(*fn, a, as);
			delete fn;
		} else {
			cns.set(FN_E_INSUFFICIENT_RESOURCES);
		}
...
FN_ctx_asvc_weak::cn_attr_multi_modify_nns(const FN_composite_name &n,
    const FN_attrmodlist &i, FN_attrmodlist **l, FN_status_cnsvc &cns)
{
	int ret = 0;
	if (resolve_to_penultimate_context(this, n, cns)) {
		FN_status_asvc as(cns);
		FN_string *fn = n.string();
		if (fn) {
			ret = a_attr_multi_modify_nns(*fn, i, l, as);
			delete fn;
		} else {
			cns.set(FN_E_INSUFFICIENT_RESOURCES);
		}
