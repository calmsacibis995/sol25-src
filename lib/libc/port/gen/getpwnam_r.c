...
#pragma	ident	"@(#)getpwnam_r.c	1.12	95/08/28 SMI"
	...
	nsc_data_t *sptr, int ndata)
{
	...
	sptr->nsc_ret.nsc_u.pwd.pw_name += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_passwd += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_age += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_comment += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_gecos += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_dir += (int) buffer;
	sptr->nsc_ret.nsc_u.pwd.pw_shell += (int) buffer;

	*result = sptr->nsc_ret.nsc_u.pwd;

	if (sptr->nsc_ret.nsc_bufferbytesused > buflen) {
		errno = ERANGE;
		return (NULL);
	}
...

{
	...
	arg.key.uid = uid;
	nss_search(&db_root, _nss_initf_passwd, NSS_DBOP_PASSWD_BYUID, &arg);
    ...
    size_t bufsize, struct passwd **result)
{
	...
	errno = 0;
	if ((*result = _getpwuid_r(uid, pwd, buffer,
					(int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
...

{
	...
	NSS_XbyY_INIT(&arg, result, buffer, buflen, str2passwd);
	arg.key.name = name;
	nss_search(&db_root, _nss_initf_passwd, NSS_DBOP_PASSWD_BYNAME, &arg);
	return (struct passwd *) NSS_XbyY_FINI(&arg);
}
    ...
    size_t bufsize, struct passwd **result)
{
	...
	errno = 0;
	if ((*result = _getpwnam_r(name, pwd, buffer,
					(int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
...
struct passwd *
getpwent_r(result, buffer, buflen)
	...
	int		buflen;
{
	...
	do {
		NSS_XbyY_INIT(&arg, result, buffer, buflen, str2passwd);
		/* No key to fill in */
		nss_getent(&db_root, _nss_initf_passwd, &context, &arg);
...
struct passwd *
fgetpwent_r(f, result, buffer, buflen)
	FILE		*f;
	struct passwd	*result;
	char		*buffer;
	int		buflen;
	...
	int	buflen;
{
	...
	if (!black_magic) {
		...
		if (passwd->pw_uid < 0 || passwd->pw_uid > MAXUID) {
			/*
			 * Previous code did this, but is it
			 * really a good idea?  What about
			 * pw_gid and gr_gid fields?
			 * (There is no definition for MAXGID).
			 */
			passwd->pw_uid = UID_NOBODY;
		}
	}
