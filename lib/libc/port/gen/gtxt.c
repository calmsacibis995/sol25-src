...
const char *cat;
{
	...
	if (cat) {
		...
		strncpy(cur_cat, cat, sizeof (cur_cat) - 1);
		cur_cat[sizeof (cur_cat) - 1] = '\0';
	}
out:
	rw_unlock(&_rw_cur_cat);
...
const char *dflt;
{
	...
	for (i = 0, cur_flag = 0;;) {
		...
		for (;;) {
			...
			if (!(db->flag & DB_OPEN)) {
				db->flag |= DB_OPEN;
				strcpy(db->saved_locale,
					_cur_locale[LC_MESSAGES]);
				strcpy(pathname, P_locale);
				strcpy(pathname + L_locale - 1,
					(db->flag & DB_DFLT) ?
					DEF_LOCALE : db->saved_locale);
				strcat(pathname, MESSAGES);
				strcat(pathname, db->db_name);
