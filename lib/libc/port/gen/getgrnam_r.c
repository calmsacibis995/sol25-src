...
#pragma	ident	"@(#)getgrnam_r.c	1.19	95/08/28 SMI"
    ...
    size_t bufsize, struct group **result)
{
	...
	errno = 0;
	if ((*result = _getgrgid_r(gid, grp, buffer,
					(int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
    ...
    size_t bufsize, struct group **result)
{
	...
	if ((*result = _getgrnam_r(name, grp, buffer,
					(int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
...
struct group *
getgrent_r(result, buffer, buflen)
	struct group	*result;
	char		*buffer;
	int		buflen;
{
...
struct group *
fgetgrent_r(f, result, buffer, buflen)
	FILE		*f;
	struct group	*result;
	char		*buffer;
	int		buflen;
...
int
_getgroupsbymember(username, gid_array, maxgids, numgids)
	const char	*username;
	gid_t		gid_array[];
	int		maxgids;
	int		numgids;
 ...
 */
int
str2group(instr, lenstr, ent, buffer, buflen)
	const char		*instr;
	int			lenstr;
	void	*ent;
	char	*buffer;
...
static nss_status_t
process_cstr(instr, instr_len, gbm)
	const char		*instr;
	int			instr_len;
