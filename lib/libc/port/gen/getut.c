...
#ident	"@(#)getut.c	1.21	94/11/03	SMI"	/*	SVr4.0 1.19.1.9	*/
...
/* a pointer to it. */
		if (cur->ut_type != EMPTY &&
			...
			strncmp(&entry->ut_line[0], &cur->ut_line[0],
...
/*
 * invoke_utmp_update		- This function invokes the utmp_update
 *				  program which has the privilege to write
...
struct utmp *
invoke_utmp_update(entry, entryx)
	struct utmp *entry;
	struct utmpx *entryx;
{
	int status;
	...
	if (childpid = fork()) {
		if (childpid == -1)
			...
			while ((w = waitpid(childpid, &status, WEXITED)) !=
						childpid && w != (pid_t)-1)
				;
			...
			if (use_utmpx == 1) {
				...
				while ((curx = getutxent()) != NULL) {
					...
					(strncmp(
					&entryx->ut_line[0], &curx->ut_line[0],
					sizeof (curx->ut_line)) == 0))
					    return ((struct utmp *) curx);
				}
			} else {
				setutent();
		...
		if (! use_utmpx) {
			 ...
			 * arguments.
			 */
			strncpy(user, entry->ut_user, sizeof (entry->ut_user));
			user[sizeof (entry->ut_user)] = '\0';
			strncpy(id, entry->ut_id, sizeof (entry->ut_id));
			id[sizeof (entry->ut_id)] = '\0';
			strncpy(line, entry->ut_line, sizeof (entry->ut_line));
			line[sizeof (entry->ut_line)] = '\0';
			sprintf(pid, "%d", entry->ut_pid);
			sprintf(type, "%d", entry->ut_type);
			sprintf(term, "%d", entry->ut_exit.e_termination);
			sprintf(exit, "%d", entry->ut_exit.e_exit);
			sprintf(time, "%d", entry->ut_time);

			execl(UTMP_UPDATE, UTMP_UPDATE, "-u",
			 ...
			 * arguments.
			 */
			strncpy(user, entryx->ut_user,
						sizeof (entryx->ut_user));
			user[sizeof (entryx->ut_user)] = '\0';
			strncpy(id, entryx->ut_id, sizeof (entryx->ut_id));
			id[sizeof (entryx->ut_id)] = '\0';
			strncpy(line, entryx->ut_line,
						sizeof (entryx->ut_line));
			line[sizeof (entryx->ut_line)] = '\0';
			sprintf(pid, "%d", entryx->ut_pid);
			sprintf(type, "%d", entryx->ut_type);
			sprintf(term, "%d", entryx->ut_exit.e_termination);
			sprintf(exit, "%d", entryx->ut_exit.e_exit);
			sprintf(time, "%d", entryx->ut_tv.tv_sec);
			sprintf(time_usec, "%d", entryx->ut_tv.tv_usec);
			sprintf(session_id, "%d", entryx->ut_session);
			for (i = 0; i < sizeof (entryx->pad); i += 2) {
				pad[i] = bin2hex[ (entryx->pad[i] >> 4) & 0xF];
				pad[i+1] = bin2hex[(entryx->pad[i]) & 0xF];
			}
			pad[sizeof (entryx->pad)] = 0;
			sprintf(syslen, "%d", entryx->ut_syslen);
			sprintf(host, "%s", entryx->ut_host);


			execl(UTMP_UPDATE, UTMP_UPDATE, "-x",
				user, id, line, pid, type, term, exit, time,
				time_usec, session_id, pad, syslen, host, 0);

		}
		/*
		 * This statement executed only if execl fails
		 */
		return ((struct utmp *) NULL);
	}
}
...
/* the address of "ubuf". */
	...
	if (changed_name == 0 && geteuid())
		return (invoke_utmp_update(entry, (struct utmpx *) 0));
...
#define	MSG	"Second getutid() failed.  fd: %d loc_utmp: %ld\n"
			gdebug(MSG, fd, loc_utmp);
#endif
...
int arg1, arg2, arg3, arg4, arg5, arg6;
{
	...
	if ((fp = fopen("/etc/dbg.getut", "a+")) == NULL)
		return;
	fprintf(fp, format, arg1, arg4, arg3, arg4, arg5, arg6);
	fclose(fp);
}
#endif
