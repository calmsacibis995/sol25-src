...
#ident	"@(#)trwctype.c 1.1	94/10/12 SMI"
...
/*
 *	__trwctype(c) converts lower/upper-case character to the opposite
...
{
	if (c > 255)
		return (__trwctype((wchar_t)c, _L));
	else
...
{
	if (c > 255)
		return (__trwctype((wchar_t)c, _U));
	else
