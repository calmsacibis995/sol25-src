#ident	"@(#)pmap.c	1.23	92/03/05 SMI"
...
{
	/* functions */
	extern enum clnt_stat	rpc_call(u_long, u_long, u_long,
				xdrproc_t, caddr_t, xdrproc_t, caddr_t,
				int, int, struct sainet *, u_int);
	struct in_addr *	get_netip(enum netaddr_type);
	void			pmap_addport(u_long, u_long, u_long);
	void			pmap_delport(u_long, u_long);
	int			uaddr2port(caddr_t);
	bool_t			xdr_rmtcall_args(XDR *,
					struct rmtcallargs *);
	bool_t			xdr_rmtcallres(XDR *,
					struct rmtcallres *);
	bool_t			xdr_rpcb_rmtcallargs(XDR *,
					struct rpcb_rmtcallargs *);
	bool_t			xdr_rpcb_rmtcallres(XDR *,
					struct rpcb_rmtcallres *);
	/* variables */
	enum clnt_stat		status;	/* rpc_call status */
	u_long			port;	/* returned port # */
	...
	status = rpc_call((u_long)PMAPPROG, (u_long)PMAPVERS,
	    (u_long)PMAPPROC_CALLIT, xdr_rmtcall_args, (caddr_t)&pmap_a,
	...
	status = rpc_call((u_long)RPCBPROG, (u_long)RPCBVERS,
	    (u_long)RPCBPROC_CALLIT, xdr_rpcb_rmtcallargs, (caddr_t)&rpcb_a,
	    xdr_rpcb_rmtcallres, (caddr_t)&rpcb_r, rexmit, wait, 0,
	    AUTH_NONE);
 ...
 */
u_long
pmap_getport(u_long prog, u_long vers,
		enum clnt_stat *rpc_stat)	/* for RPC status */
{
	extern enum clnt_stat 		rpc_call();
	extern bool_t			xdr_wrapstring();
	bool_t				xdr_pmap(), xdr_rpcb();
	void				pmap_addport();
	static int			uaddr2port();
	...
	for (walk = map_head; walk != 0; walk = walk->pml_next) {
		if ((walk->pml_map.pm_prog == prog) &&
		    ...
		    (walk->pml_map.pm_prot == (u_long)IPPROTO_UDP)) {
#ifdef DEBUG
	...
	pmap_send.pm_prot = (u_long)IPPROTO_UDP;
	...
	*rpc_stat = rpc_call(PMAPPROG, PMAPVERS, PMAPPROC_GETPORT,
	...
	if (*rpc_stat == RPC_PROGUNAVAIL) {
		...
		*rpc_stat = rpc_call(RPCBPROG, RPCBVERS, RPCBPROC_GETADDR,
		    xdr_rpcb, (caddr_t)&rpcb_send, xdr_wrapstring, &ua,
		    0, 0, 0, AUTH_NONE);
	...
	printf("pmap_getport: prog: %x, vers: %x; returning port: %d.\n",
	    prog, vers, dport);
#endif /* DEBUG */
		pmap_addport(prog, vers, (u_long)dport);
		return ((u_long)dport);
	} else {
#ifdef DEBUG
 ...
 */
void
pmap_addport(u_long prog, u_long vers, u_long port)
{
	...
	new->pml_map.pm_prot = (u_long)IPPROTO_UDP;
 ...
 */
void
pmap_delport(u_long prog, u_long vers)
{
...
pmap_memfree()
{
	...
	while (current != (struct pmaplist *)0) {
		...
		(void) kmem_free((caddr_t)current,
		    sizeof (struct pmaplist));
		current = tmp;
	}
}
