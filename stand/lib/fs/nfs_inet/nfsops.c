...
#pragma ident	"@(#)nfsops.c	1.34	94/09/20 SMI"
...
static int	boot_nfs_read(int fd, caddr_t buf, int size);
...
boot_nfs_closeall(int flag)
{
	...
	/* zero bootdev_t */
	bzero((caddr_t)&bootd, sizeof (bootd));
}
...
boot_nfs_open(char *path, int flags)
{
	...
	if (!got_filep) {
		...
		bzero((caddr_t)newfilep, sizeof (struct nfs_files));
		filep->next = newfilep;
		filep = newfilep;
		filep->desc = file_desc++;
	}
 ...
 */
static int
boot_nfs_read(int fd, char *buf, int size)
{
	/* functions */
	...
	register u_int readcnt = 0;	/* # of bytes read by nfs */
	register u_int count = 0;	/* # of bytes transferred to buf */
	register u_int done = 0;	/* last block has come in */
	register int rexmit;		/* retransmission interval (secs) */
	register int resp_wait;		/* how long to wait for a resp */
	register char *buf_offset;	/* current buffer offset */
	...
	do {
...
#ifdef i386
		read_res.readres_u.reply.data.data_val = readbuf;
#else
		...
		do {
			read_stat = rpc_call((u_long)NFS_PROGRAM,
			    (u_long)NFS_VERSION, (u_long)NFSPROC_READ,
...
#ifdef i386
		/* load the data read into the actual buffer */
		memcpy(buf_offset, readbuf, readcnt);
#endif
...
boot_nfs_getattr(fid_t *fidp, struct vattr *vap)
{
	...
	getattr_stat = rpc_call((u_long)NFS_PROGRAM,
	    (u_long)NFS_VERSION, (u_long)NFSPROC_GETATTR,
...
/*
 * This version of fstat supports mode and size only.
...
{	struct vattr va;
	...
	if (fd == 0) {
		printf("nfs_getattr(): Bad file number 0.\n");
		return (-1);
	}
