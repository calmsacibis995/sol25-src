#ident	"@(#)pathname.c	1.8	92/04/12 SMI" /* from SunOS 4.1 2.24 89/08/16 */
...
{
	if (pnp->pn_buf == NULL) {
		printf("pn_alloc: pn_buf must point to a valid buffer.\n");
		return (-1);
	}
...
{
	/* nop */
	printf("pn_free(): you shouldn't be calling pn_free()!\n");
	return;
}
