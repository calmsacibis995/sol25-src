...
#ident	"@(#)sac.c	1.9	93/03/24 SMI"       /* SVr4.0 1.14*/
 ...
 */
        ...
        if ( (ia_start ("sac",".sac", NULL,NULL,NULL,&iah)) != IA_SUCCESS) {
                (void) sprintf(buf, "error removing utmp entry for <%s>",
                                    sp->sc_tag);
                log(buf);
                return;
        }
         ...
         */
        if (( error = ia_close_session(iah, 0,
            sp->sc_pid, sp->sc_exit, sp->sc_utid, &out)) != IA_SUCCESS) {
                switch (error) {
                        case IA_NOENTRY:
                                (void) sprintf(buf,
                                    "missing utmp entry for <%s>", sp->sc_tag);
                                break;
                        case IA_ENTRYFAIL:
                                (void) sprintf(buf,
                                    "could not remove utmp entry for <%s>",
                                    sp->sc_tag);
                        default:
                                (void) sprintf(buf,
                                    "error removing utmp entry for <%s>",
                                    sp->sc_tag);
                                break;
                }
                log(buf);
