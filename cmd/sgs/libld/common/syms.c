 ...
 */
#pragma ident	"@(#)syms.c	1.46	94/10/24 SMI"
...
static const char
	* Errmsg_lsbv = "local symbol `%s' has bogus value:"
			"\n\t(file %s value=%s);",
	* Errmsg_gsib = "global symbol `%s' has non-global binding:"
			"\n\t(file %s value=%s);",
	* Errmsg_rsad = "reserved symbol `%s' already defined in file %s";
...
sym_copy(Sym_desc * sdp)
{
	...
	if (sdp->sd_flags & FLG_SY_CLEAN) {
		if ((nsym = (Sym *)malloc(sizeof (Sym))) == 0)
			return (S_ERROR);
		*nsym = *(sdp->sd_sym);
		sdp->sd_sym = nsym;
 ...
 */
Sym_desc *
sym_find(const char * name, unsigned long hash, Ofl_desc * ofl)
{
	Word		bkt = 0;
	Sym_cache *	scp;
	Sym_desc *	sdp;
	...
	if (hash == SYM_NOHASH)
		hash = elf_hash((const char *)name);
	if (hash)
		bkt = hash % ofl->ofl_symbktcnt;
...
sym_enter(const char * name, Sym * osym, Word hash, Ifl_desc * ifl,
	Ofl_desc * ofl, int ndx)
{
	Word		bkt;
	Sym_desc *	sdp;
	Sym_aux *	sap;
	Sym_cache *	scp, * _scp = 0;
	Sym_names *	snp, * _snp = 0;
	char *		_name;
	Sym *		nsym;
	int		len;
	 ...
	 */
	if ((sap = (Sym_aux *)calloc(sizeof (Sym_aux), 1)) == 0)
		return ((Sym_desc *)S_ERROR);
	sdp->sd_file = ifl;
	sdp->sd_aux = sap;
	sap->sa_hash = hash;
	...
	*nsym = *osym;
	...
	/*
	 * Determine the next available free space in the symbol dictionary.
	 * We maintain a name cache to localize the symbol string names rather
	 * than reference them directly from their associated files (when there
	 * are a large number of files the string names become scattered,
	 * resulting in frequent page faulting and slower performance).
	 */
	len = strlen(name) + 1;
	for (snp = ofl->ofl_symdict; snp; _snp = snp, snp = snp->sn_next) {
		_name = snp->sn_free;
		if (((int)snp->sn_end - (int)snp->sn_free) >= len)
			break;
	}

	/*
	 * If no dictionary space is available allocate a new one and link it
	 * into the dictionary list.
	 */
	if (snp == 0) {
		if ((snp = (Sym_names *)malloc(SYM_DICTSZ)) == 0)
			return ((Sym_desc *)S_ERROR);
		snp->sn_next = 0;
		snp->sn_free = _name = (char *)(snp + 1);
		snp->sn_end = (char *)((int)snp + SYM_DICTSZ);

		/*
		 * _snp is the parent symbol dictionary, a null value indicates
		 * this is the first symbol dictionary to be added.
		 */
		if (_snp == 0)
			ofl->ofl_symdict = snp;
		else
	...
	/*
	 * Add the new symbol name to the symbol dictionary and update the
	 * symbol cache and symbol descriptor to use this entry.
	 */
	...
	if (shndx == SHN_UNDEF) {
		...
		if ((ofl->ofl_flags & FLG_OF_GENMAP) &&
			...
			if (list_append(&sap->sa_dfiles, ifl->ifl_name) == 0)
				return ((Sym_desc *)S_ERROR);
	}
    ...
    Half shndx, Ofl_desc * ofl)
{
	...
	if (name && (sdp = sym_find(name, SYM_NOHASH, ofl)) &&
		 ...
		 */
		sdp->sd_ref = REF_REL_NEED;
		sdp->sd_sym->st_shndx = shndx;
		sdp->sd_isc = NULL;
		sdp->sd_sym->st_size = 0;
		sdp->sd_sym->st_value = 0;
		sdp->sd_aux->sa_symspec = sdaux_id;
...
sym_undef_title()
{
	eprintf(ERR_NONE, "Undefined\t\t\tfirst referenced");
	eprintf(ERR_NONE, " symbol  \t\t\t    in file");
...
static const char * format[] = {
	"%-35s %s",
	"%-35s %s  (symbol has no version assigned)",
	"%-35s %s  (symbol belongs to implicit dependency %s)",
	"%-35s %s  (symbol belongs to unavailable version %s (%s))"
};
...
sym_undef_entry(Sym_desc * sdp, Type type)
{
	...
	eprintf(ERR_NONE, format[type], sdp->sd_name, name1, name2, name3);
}
...
sym_spec(Ofl_desc * ofl)
{
	...
	if (!(flags & FLG_OF_RELOBJ)) {
		...
		if (sym_add_spec(Etext_sym, Etext_usym, SDAUX_ID_ETEXT,
		    SHN_ABS, ofl) == S_ERROR)
			return (S_ERROR);
		if (sym_add_spec(Edata_sym, Edata_usym, SDAUX_ID_EDATA,
		    SHN_ABS, ofl) == S_ERROR)
			return (S_ERROR);
		if (sym_add_spec(End_sym, End_usym, SDAUX_ID_END,
		    SHN_ABS, ofl) == S_ERROR)
			return (S_ERROR);
		 ...
		 */
		if (sym_add_spec(Dyn_sym, Dyn_usym, SDAUX_ID_DYN,
		    SHN_ABS, ofl) == S_ERROR)
			return (S_ERROR);

		if (flags & FLG_OF_DYNAMIC)
			if (sym_add_spec(Plt_sym, Plt_usym, SDAUX_ID_PLT,
			    SHN_ABS, ofl) == S_ERROR)
				return (S_ERROR);

		if (sym_find(Got_usym, SYM_NOHASH, ofl))
			if (sym_add_spec(Got_sym, Got_usym, SDAUX_ID_GOT,
			    SHN_ABS, ofl) == S_ERROR)
				return (S_ERROR);
	}
	return (1);
...
sym_validate(Ofl_desc * ofl)
{
	...
	Sym *		sym;
	Word		flags = ofl->ofl_flags;
	int		undef = 0, needed = 0, verdesc = 0;
	Half		align = 0;
	size_t		size = 0;
	Ifl_desc *	ifl;
	...
	for (bkt = 0; bkt < ofl->ofl_symbktcnt; bkt++) {
		...
		for (scp = ofl->ofl_symbkt[bkt]; scp; scp = scp->sc_next) {
			for (sdp = (Sym_desc *)(scp + 1);
				...
				/*
				 * Test for undefined symbols.  Note that for
				 * building things like shared objects,
				 * undefined symbols are allowed, however if
				 * these undefines have resulted from a mapfile
				 * or command line symbol entry then we should
				 * inform the user.
				 */
				 ...
				 * supplied from implicit shared objects.
				 */
				if (needed && (sdp->sd_ref == REF_DYN_NEED) &&
				    (sdp->sd_flags & FLG_SY_NOTAVAIL)) {
	 ...
	 */
	if (size || !(ofl->ofl_flags & FLG_OF_RELOBJ)) {
	...
	if (ofl->ofl_entry) {
		if (((sdp = sym_find(ofl->ofl_entry, SYM_NOHASH, ofl)) ==
			...
			eprintf(ERR_FATAL, "entry point symbol `%s' is "
			    "undefined", ofl->ofl_entry);
			return (S_ERROR);
		}
		ofl->ofl_entry = (void *)sdp;
		sdp->sd_flags |= FLG_SY_UPREQD;
	} else if (((sdp = sym_find("_start", SYM_NOHASH, ofl)) != NULL) &&
			(sdp->sd_ref == REF_REL_NEED)) {
		ofl->ofl_entry = (void *)sdp;
		sdp->sd_flags |= FLG_SY_UPREQD;
	} else if (((sdp = sym_find("main", SYM_NOHASH, ofl)) != NULL) &&
			(sdp->sd_ref == REF_REL_NEED)) {
		ofl->ofl_entry = (void *)sdp;
		sdp->sd_flags |= FLG_SY_UPREQD;
	}
...
sym_process(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	 ...
	 */
	if ((ifl->ifl_oldndx = malloc(total * sizeof (Sym_desc *))) == 0)
		return (S_ERROR);
	if ((etype == ET_REL) && local) {
		if ((ifl->ifl_locs = calloc(sizeof (Sym_desc), local)) == 0)
			return (S_ERROR);
	...
	if (ifl->ifl_locscnt) {
		for (sym++, ndx = 1; ndx < local; sym++, ndx++) {
			...
			if (type == STT_SECTION) {
				if (shndx == SHN_UNDEF) {
					eprintf(ERR_WARNING, Errmsg_lsbv,
						sdp->sd_name, ifl->ifl_name,
						conv_shndx_str(shndx));
					sdp->sd_isc = NULL;
			 ...
			 * image record their number and name string size.  Skip
			 * any section symbols as new versions of these will be
			 * created.  Globals are counted after all input file
			 * processing (and hence symbol resolution) is complete
			 * during sym_validate().
			 */
			...
			ofl->ofl_locscnt++;
			if (sdp->sd_name)
				ofl->ofl_locsstrsz += strlen(sdp->sd_name) + 1;
		}
	}
	...
	for (ndx = local; ndx < total; sym++, ndx++) {
		...
		if ((bind != STB_GLOBAL) && (bind != STB_WEAK)) {
			eprintf(ERR_WARNING, Errmsg_gsib, name,
		...
		if ((ELF_ST_TYPE(sym->st_info) == STT_OBJECT) &&
		    ...
		    (name[0] == '_') && ((name[1] == 'e') ||
		    (name[1] == 'D') || (name[1] == 'P')) &&
		    ((strcmp(&name[2], &Etext_usym[2]) == 0) ||
		    (strcmp(&name[2], &Edata_usym[2]) == 0) ||
		    (strcmp(&name[2], &End_usym[2]) == 0) ||
		    (strcmp(&name[2], &Dyn_usym[2]) == 0) ||
		    (strcmp(&name[2], &Plt_usym[2]) == 0))) {
	...
	for (ndx = local; ndx < total; sym++, ndx++) {
		...
		if ((ELF_ST_BIND(sym->st_info) == STB_WEAK) &&
			...
			_sym += local;
			for (_ndx = local; _ndx < total; _sym++, _ndx++) {
				if (_sym == sym)
					continue;
