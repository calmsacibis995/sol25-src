...
#pragma ident	"@(#)main.c	6.11	95/02/11 SMI"
...
# endif
			...
			case 'n':
				...
				break;
			case 'w': case 'W':
				woption = 1; handleeuc=1; widecio=1; 
				break;
			case 'e': case 'E':
				eoption = 1; handleeuc=1; widecio=0; 
				break;
			default:
# ifdef DEBUG
				(void)fprintf(stderr,"Usage: lex [-ewdyctvnVY] [-Q(y/n)] [file]\n");
# else
				(void) fprintf(stderr,"Usage: lex [-ewctvnVY] [-Q(y/n)] [file]\n");
# endif
				exit(1);
		}
	}
	if(woption && eoption){
		error("You may not specify both -w and -e simultaneously.");
	}
	...
	if (no_input) {
		...
		if (strcmp(argv[optind], "-") == 0)
			...
			if(fin == NULL) error ("Can't open input file -- %s",
			     argv[optind]);
	 	}
	} else fin = stdin;
...
# endif
	if (path == NULL) {
		...
		if (find_run_directory(sargv[0], current_work_directory, run_directory,(char **) 0, getenv("PATH")) != 0)        {
			(void)fprintf(stderr,"Error in finding run directory. Using default %s\n",current_work_directory);
...
get1core(){
	...
	if(ccl == 0 || def == 0 || pchar == 0 || subs == 0 
		|| dchar == 0 || sname == 0 || exclusive == 0 || schar == 0)
