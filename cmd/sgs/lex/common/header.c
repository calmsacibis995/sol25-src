...
#pragma ident	"@(#)header.c	6.12	95/02/21 SMI"
...
# endif
	...
	if (widecio)
		(void)fprintf(fout,"# define output(c) (void)putwc(c,yyout)\n");
	else
		(void)fprintf(fout,"# define output(c) (void)putc(c,yyout)\n");
	(void)fprintf(fout,"#else\n");
	if (widecio)
		(void)fprintf(fout,"# define lex_output(c) (void)putwc(c,yyout)\n");
	else
		(void)fprintf(fout,"# define lex_output(c) (void)putc(c,yyout)\n");
	...
	(void)fprintf(fout,"#endif\n");
	(void)fprintf(fout,"#if defined(__cplusplus) && defined(__EXTERN_C__)\n");
	(void)fprintf(fout,"}\n");
	(void)fprintf(fout,"#endif\n\n");
...
"# define unput(c) {yytchar= (c);if(yytchar=='\\n')yylineno--;*yysptr++=yytchar;}\n");
  ...
  "# define lex_input() (((yytchar=yysptr>yysbuf?U(*--yysptr):getc(yyin))==",
	 ...
	 "?(yylineno++,yytchar):yytchar)==EOF?0:yytchar)");
		...
		if (handleeuc) {
			(void)fprintf(fout,"# define REJECT { nstr = yyreject_e(); goto yyfussy;}\n");
			(void)fprintf(fout,"int yyleng;\n");
			(void)fprintf(fout,"int yywleng;\n");
			/*
			 * XCU4:
			 * If %array, yytext[] contains the token.
			 * If %pointer, yytext is a pointer to yy_tbuf[].
			 */
			if (isArray) {
				(void)fprintf(fout,"unsigned char yytext[YYLMAX];\n");
				(void)fprintf(fout,"wchar_t yywtext[YYLMAX*MB_LEN_MAX];\n");
			} else {
				(void)fprintf(fout,"wchar_t yy_twbuf[YYLMAX*MB_LEN_MAX];\n");
				(void)fprintf(fout,"wchar_t yy_tbuf[YYLMAX];\n");
				(void)fprintf(fout,"unsigned char * yytext = yy_tbuf;\n");
