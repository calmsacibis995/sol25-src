...
#pragma ident	"@(#)sub2.c	6.10	95/02/11 SMI"
...
# endif
	...
	for(s = 0; s <= stnum; s++){
		...
		sfall[s] = -1;
		acompute(s);
		for(i=0; i<ncg; i++) symbol[i] = 0;
		npos = *state[s];
		for(i = 1; i<=npos; i++){
			curpos = *(state[s]+i);
                        if(!ISOPERATOR(name[curpos])) symbol[name[curpos]] = TRUE;
	...
	if(tryit){	/* ccl's used */
		...
		for(i=0;i<cnt;i++)
			...
			if ((index >= 0) && (index < NCH))
				...
				{
				fprintf(stderr, "lex`sub2`packtran: tch[%d] out of bounds (%d)\n", i, tch[i]);
				}
			}
		for(i=0; i<cnt;i++){
			c = match[tch[i]];
...
# endif
	...
	for(i=0; i<st; i++){	/* get most similar state */
		...
		while(ach[j] && p < upper){
			...
			if(ach[j] == 0)break;
			if(ach[j] > nchar[p]){diff=ncg;break;}
			/* ach[j] == nchar[p] */
...
acompute(s)	/* compute action list = set of poss. actions */
  int s; {
	...
	for(i=0;i<cnt;i++){
		q=*p;
		if(name[q] == FINAL)temp[k++] = left[q];
		else if(name[q] == S1FINAL){temp[k++] = left[q];
			if ((r=left[q]) >=NACTIONS)
				error("INTERNAL ERROR:left[%d]==%d>=NACTIONS", q, r);
				extra[r] = 1;
			}
		else if(name[q] == S2FINAL)neg[n++] = left[q];
		p++;
		}
	atable[s] = -1;
	if(k < 1 && n < 1) return;
