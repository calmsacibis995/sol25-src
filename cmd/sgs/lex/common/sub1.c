...
#pragma ident	"@(#)sub1.c	6.11	95/02/11 SMI"
  ...
  int a,d;
{
	...
	case RCCL:
	case RNCCL:
		if(slength((CHR *)d) == 0) nullstr[tptr] = TRUE;
		break;
	case STAR:
	case QUEST:
		nullstr[tptr] = TRUE;
...
# ifdef DEBUG
	else error("Unrecognized munput option %wc",t);
# endif
	...
	if(chset == TRUE){
		...
		for(i=1;i<NCH;i++){
			if(i != ctable[i]){
				allprint(i);
				(void) putchar(' ');
				iswprint(ctable[i]) ? (void) putwc(ctable[i], stdout) : (void)printf("%d",ctable[i]);
				(void) putchar('\n');
				}
			}
		}
