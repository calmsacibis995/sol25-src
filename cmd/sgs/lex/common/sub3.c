...
#pragma ident	"@(#)sub3.c	1.4	93/11/19 SMI"
...
{
/*	return ((*pc1)-(*pc2)); 
	This doesn't work properly because unsigned minus unsigned cannot
	yield negative result.
 */
	if( *pc1 > *pc2 ) return 1;
     ...
     wchar_t c;
{
	...
	if(!handleeuc){/* User-friendliness consideration:
			* Make sure no EUC chars are used in reg. exp.
			*/
		
		if(!isascii(c))
		    if(iswprint(c)) warning("\
...
Non-ASCII character of value %#x in pattern; use -w or -e lex option.", c);
		return;/* In any case, we don't need to construct ncgidtbl[].*/
	}

	lsearch(&lc, yycgidtbl, (unsigned *)&ncgidtbl, sizeof(lchar), (int (*)())cmplc);
}

sortcgidtbl()
{
	if(!handleeuc) return;
	qsort(yycgidtbl, ncgidtbl, sizeof(lchar), (int (*)())cmplc);
}
     ...
     wchar_t	c;
{
	...
	error("system error in yycgid():binary search failed for c=0x%04x\n", c);
}
...
repbycgid()
{
	...
	for(i=0; i<tptr; ++i){
		c=name[i];
		if(!ISOPERATOR(c)){/* If not an operator, it must be a char.  */
			...
			for(j=0;j<ncg;++j) symbol[j] = FALSE;
			...
			ccltoken[m] = 0;
			ccp = ccl;
			while (ccp < ccptr && scomp(ccltoken, ccp) != 0) ccp++;
...
setsymbol(i)
     int	i;
{
	if(i>sizeof(symbol)) error("setsymbol: (SYSERR) %d out of range", i);
	symbol[i]=TRUE;
}
