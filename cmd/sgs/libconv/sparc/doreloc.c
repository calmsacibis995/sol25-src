...
#pragma	ident	"@(#)doreloc.c	1.3	95/06/05 SMI"
...
/*
 * This table reperesents the current relocations that do_reloc
 ...
 */
reloc_entry	reloc_table[R_SPARC_NUM] = {
...
/* R_SPARC_PC10 */	{0x3ff, FLG_RE_PCREL | FLG_RE_RELPC | FLG_RE_SIGN,
				4, 0, 13},
/* R_SPARC_PC22 */	{0x0, FLG_RE_PCREL | FLG_RE_RELPC |
				FLG_RE_VERIFY,
				4, 10, 22},
...
/* R_SPARC_PLT32 */	{0x0, FLG_RE_PLTREL | FLG_RE_VERIFY, 4, 0, 0},
 ...
 *
 * Upon successful completion of do_reloc() *value will be set
 * to the 'bit-shifted' value that will be or'ed into memory.
 */
int
	...
	const char * symname, const char * filename)
{
	...
	if (field_size == 0) {
		eprintf(ERR_FATAL, Errmsg_brdr, filename,
			conv_reloc_type_str(rtype), symname);
		return (0);
	}
	...
	if (re_flags & FLG_RE_UNALIGN) {
		...
		if ((Word)off & 0x3) {
			eprintf(ERR_FATAL,
				(const char *)"file=%s: relocation [%s] "
				"(symbol %s) against non-aligned "
				"offset (0x%x)", filename,
				conv_reloc_type_str(rtype), symname, off);
			return (0);
		}
		switch (field_size) {
		case 1:
		...
		default:
			eprintf(ERR_FATAL,
				(const char *)"file=%s: relocation [%s] "
				"(symbol `%s'), unsupported # of bits for "
				"relocation: %d", filename,
				conv_reloc_type_str(rtype), symname,
				rep->re_fsize * 8);
			return (0);
		}
	...
	if (sigbit_mask) {
		 ...
		 * it's bits are not all contiguous.  We have to
		...
		/*
		 * If value is signed make sure that we signextend the
		 * uvalue.
		 */
	...
	if (bshift) {
		 ...
		 * jump to a byte on a 4 byte boundary.
		 */
		if ((bshift == 2) && (uvalue & 0x3)) {
			eprintf(ERR_FATAL,
				(const char *)"file=%s: relocation [%s] "
				"(symbol %s) value 0x%x looses %d bits "
				"at 0x%x", filename,
				conv_reloc_type_str(rtype), symname,
				uvalue, 2, off);
			return (0);
	...
	if ((re_flags & FLG_RE_VERIFY) && sigbit_mask) {
		if ((sigfit_mask & S_ABS(uvalue)) != S_ABS(uvalue)) {
			eprintf(ERR_FATAL,
				(const char *)"file=%s: relocation [%s] "
				"value 0x%x (symbol `%s') does not fit",
				filename,
				conv_reloc_type_str(rtype), uvalue,
				symname);
			return (0);
		}
	}
	...
	if (sigbit_mask) {
		/*
		 * Again the R_SPARC_WDISP16 relocation takes
		 * special processing because of its non-continguous
		 * bits.
		 */
		if (re_flags & FLG_RE_WDISP16)
