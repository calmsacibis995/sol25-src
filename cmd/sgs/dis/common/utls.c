#pragma ident	"@(#)utls.c	1.5	93/06/04 SMI"
	...
	if (Rel_data)
	{
		...
		while(no_rel_entries1--)
		{
			if (in_offset == rel_data1->r_offset)
			{
				...
				{
					sprintf(*pos, oflag?"0%o":"0x%x",val);
					*pos+=10;
					return;
				}
			}
...
#if I386
		sprintf(*pos,oflag?"0%o+%s":"0x%x+%s",val,regname[regno]);
					*pos+=strlen();
#else
		sprintf(*pos,"%s",regname[val]); 
		*pos+=strlen(regname[val]);
		sprintf(*pos, oflag?"(0%o+%s)":"(0x%x+%s)",
					val,regname[regno]); 
			...
			*/
				 ...
				 val == sym_data->st_value) )
			{
				...
				if (val - sym_data->st_value) {
				  ...
				  sprintf(*pos, "(%s+%ld)", 
					sym_name?sym_name:"(null)", val - sym_data->st_value);
				  *pos+=(sym_name?strlen(sym_name):strlen("(null)")) + 10;
				}
				else {
				  sprintf(*pos, "%s", sym_name?sym_name:"(null)");
				  *pos+=sym_name?strlen(sym_name):strlen("(null)");
				}
				return;
			}
			sym_data++;	
...
char	*message;
{
	...
	if (archive)
		(void)fprintf(stderr, "\n%s: %s[%s]: %s\n", TOOL, fname, mem_header->ar_name, message);
	else
		(void)fprintf(stderr, "\n%s: %s: %s\n", TOOL, fname,  message);
