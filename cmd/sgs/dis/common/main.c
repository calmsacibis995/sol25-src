#pragma ident	"@(#)main.c	1.12	94/12/13 SMI"
...
char	*argv[];
{
	...
	while (( optchar = getopt(argc, argv, optstr)) != -1) 
	{
		switch (optchar) 
		{
				...
				case 'l':
					lflag++;
					if ( ((libstr = getenv(LIBDIR)) == NULL) || (*libstr == '\0'))			
						libstr = LIBDEFAULT;
...
each_file()
{
	...
	while ((elf = elf_begin(fd, cmd, arf)) != 0) {
		...
		if ((ehdr = elf32_getehdr(elf)) != 0) {
			...
			{
				if (archive)
					(void) fprintf(stderr, "dis: %s: Warning - internal conversion of COFF file to ELF\n" ,mem_header->ar_name);
				else
					(void) fprintf(stderr, "dis: %s: Warning - internal conversion of COFF file to ELF\n", fname);	
			}
...
disassembly()
{
	...
	if (symtab == 0 && sflag)
	{
		...
		if (archive)
                         (void) fprintf(stderr,
			 "\ndis: %s[%s]:" ,fname, mem_header->ar_name);
		else
			(void) fprintf(stderr,"\ndis: %s:" ,fname);
