 ...
 */
#pragma ident	"@(#)malloc.c	1.9	95/01/04 SMI"
...
static void
split(struct block * block, unsigned int size)
{
	PRF_MCOUNT(78, split);
 ...
 */
static
align(unsigned int size, int bound)
{
	if (size < bound)
		return (bound);
	else
...
found:
	 ...
	 */
	if (!page) {
		unsigned int	totsize = size + HDR_PAGE;
		int		totpage = align(totsize, syspagsz);
		int		dz_fd;
