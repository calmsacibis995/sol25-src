 ...
 */
#pragma ident	"@(#)cache.c	1.10	94/04/18 SMI"
	...
	const char *file;
{
	...
	if (strncmp(SO, n + liblen, SOLEN))
		return (NULL);

	mnp = mjp = ((char *)file + LIBLEN + liblen + SOLEN);
	...
	if (verscmp(file + to_min,
		...
		eprintf(ERR_WARNING, "%s has older revision than expected %s",
