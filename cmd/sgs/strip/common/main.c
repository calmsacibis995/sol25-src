...
#pragma ident	"@(#)main.c	6.16	94/11/13 SMI"
...
static	Elf32_Off *off_table;
				/*
				* array maintains section's offset;
				* set to retain old offset, otherwise 0
				*/
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if not previously defined */
	...
	if (elf_version(EV_CURRENT) == EV_NONE) {
		(void) fprintf(stderr, gettext(
"strip: FATAL: elf_version() failed - libelf.a out of date.\n"));
		exit(FAILURE);
	}
	...
	while ((optchar = getopt(argc, argv, optstr)) != -1) {
		...
		case 'x':	xflag++; break;
		case 'r':	(void) fprintf(stderr, gettext(
"strip: Warning: the -r option has no effect and will be"
" removed in the next release.\n"));
		...
		case 'b':	(void) fprintf(stderr, gettext(
"strip: Warning: the -b option has no effect and will be"
" removed in the next release.\n"));
				break;
		case '?': errflag ++; break;
		default:	break;
...
each_file()
{
	...
	if ((fd = open(fname, O_RDONLY)) == -1) {
		(void) fprintf(stderr, gettext(
		"strip: %s: cannot open file\n"), fname);
		return (FAILURE);
	}

	if ((arf = elf_begin(fd, cmd, (Elf *)0)) == 0) {
		(void) fprintf(stderr, gettext(
		"strip: libelf error: %s\n"), elf_errmsg(-1));
		(void) elf_end(arf);
	...
	if ((elf_kind(arf) == ELF_K_COFF)) {
		(void) fprintf(stderr, gettext(
		"strip: %s: cannot strip a COFF file\n"),
		fname);
		(void) elf_end(arf);
		(void) close(fd);   /* done processing this file */
		return (FAILURE);
		...
		if ((fdartmp = open(artmpfile,
			O_WRONLY |
			O_APPEND |
			O_CREAT, (mode_t) 0644)) == NULL) {
			(void) fprintf(stderr, gettext(
			"strip: %s: cannot open temporary file\n"),
			artmpfile);
			(void) elf_end(arf);
			(void) close(fd);
			exit(FAILURE);
	...
	while ((elf = elf_begin(fd, cmd, arf)) != 0) {
		if (ar_file) /* get header info */ {
			if ((mem_header = elf_getarhdr(elf)) == NULL) {
				(void) fprintf(stderr, gettext(
				    "strip: %s: malformed archive at %ld\n"),
			...
			if ((cur_filenm = (char *)
					malloc((strlen(fname) + 3 +
					strlen(mem_header->ar_name)) *
					sizeof (char))) == NULL) {
				(void) fprintf(stderr, gettext(
				"strip: malloc memory failure\n"));
				strip_exit(FAILURE);
			}
	...
	if (ar_file) {
		...
		if (found_ar_sym_tab) {
			(void) fprintf(stderr,
		"strip: WARNING: %s: symbol table deleted from archive \n",
			fname);
			(void) fprintf(stderr, gettext(
...
char *fname;
{
	...
	if ((fdtmp3 = open(elftmpfile, O_RDONLY)) == -1) {
		(void) fprintf(stderr, gettext(
		"strip: %s: can't open temporary file\n"),
		elftmpfile);
		strip_exit(FAILURE);
	}
	...
	if ((buf = (char *)
		malloc(ROUNDUP(stbuf.st_size) * sizeof (char))) == NULL) {
		(void) fprintf(stderr, gettext(
		"strip: malloc failure\n"));
		strip_exit(FAILURE);
	}
	if (read(fdtmp3, buf,
		(unsigned) stbuf.st_size) != (unsigned) stbuf.st_size) {
		(void) fprintf(stderr, gettext(
		"strip: read failure\n"));
			strip_exit(FAILURE);
	}

	(void) sprintf(mem_header_buf,
		FORMAT,
	...
	if (write(fdartmp, mem_header_buf,
		...
		(void) fprintf(stderr, gettext(
		"strip: %s: write system failure: %s: file not stripped\n"),
		artmpfile, fname);
		strip_exit(FAILURE);
	}
	...
	if (stbuf.st_size & 0x1) {
		buf[stbuf.st_size] = '\n';
		if (write(fdartmp, buf,
			(unsigned) ROUNDUP(stbuf.st_size)) == -1) {
			(void) fprintf(stderr, gettext(
			"strip: %s: write system failure:"
			" %s: file not stripped\n"),
			artmpfile, fname);
...
char *fname;
{
	...
	if (strcmp(mem_header->ar_name, "/") != 0) {
		...
		if ((file_buf =
			(char *) malloc(ROUNDUP(mem_header->ar_size))) ==
			NULL) {
			(void) fprintf(stderr, gettext(
			"strip: malloc failure\n"));
			strip_exit(FAILURE);
...
char *temp_file_name;
{
	uid_t group;			/* Group of original file.	*/
	register int i;
	register int fnew;		/* file descriptor of original file */
	gid_t owner;			/* Owner of original file.	*/

	for (i = 0; signum[i]; i++)	/* Started writing, cannot interrupt */
		(void) signal(signum[i], SIG_IGN);

	(void) stat(fname, &stbuf);
	mode = stbuf.st_mode;		/* For mode of original file.	*/
	owner = stbuf.st_uid;		/* For owner of original file.	*/
	group = stbuf.st_gid;		/* For group of original file.	*/
	...
	if (read(fdtmp2, buf, (unsigned) stbuf.st_size) !=
		(unsigned) stbuf.st_size) {
		(void) fprintf(stderr, gettext(
		"strip: %s: read failure\n"), temp_file_name);
		strip_exit(FAILURE);
	}

	if ((fd = open(fname,
		O_WRONLY | O_TRUNC | O_CREAT, (mode_t) 0644)) == -1) {
		(void) fprintf(stderr, gettext(
		"strip: %s: can't open file for writing\n"), fname);
		strip_exit(FAILURE);
	}

	(void) chmod(fname, mode);

	if (write(fd, buf,
		(unsigned) stbuf.st_size) != (unsigned) stbuf.st_size) {
		(void) fprintf(stderr, gettext(
		"strip: %s: write system failure: %s:"
		" file not stripped\n"),
		fname, fname);
		strip_exit(FAILURE);
	}
...
strip_file()
{
	...
	if (traverse_file() == FAILURE) {
		(void) fprintf(stderr, gettext(
		"strip: WARNING: %s: Cannot strip file\n"), cur_filenm);
		error = FAILURE;
...
initialize()
{
	...
	if ((sec_table = (int *) calloc(ehdr->e_shnum, sizeof (int))) == NULL) {
		(void) fprintf(stderr, gettext(
		"strip: memory allocation failure on calloc\n"));
		strip_exit(FAILURE);
	}

	if ((off_table = (Elf32_Off *)
		calloc(ehdr->e_shnum, sizeof (Elf32_Off))) == NULL) {
		(void) fprintf(stderr, gettext(
		"strip: memory allocation failure on calloc\n"));
		strip_exit(FAILURE);
	}
	...
	if ((nobits_table =
		(int *) calloc(ehdr->e_shnum, sizeof (int))) == NULL) {
		(void) fprintf(stderr, gettext(
		"strip: memory allocation failure on calloc\n"));
		strip_exit(FAILURE);
	}
...
build_segment_table()
{
	...
	if ((b_e_seg_table = (Elf32_Off *)
		calloc(ehdr->e_phnum*2, sizeof (Elf32_Off))) == NULL) {
		(void) fprintf(stderr, gettext(
		"strip: memory allocation failure on calloc\n"));
		strip_exit(FAILURE);
	}
	...
	for (i = 1, j = 0; i <= ehdr->e_phnum; i++, j += 2)
		(void) printf(
		"DEBUG: b_e_seg_table[%d] is %x\t b_e_seg_table[%d] is %x\n",
	...
	while ((scn = elf_nextscn(elf, scn)) != 0) {
		if ((shdr = elf32_getshdr(scn)) == 0) {
			(void) fprintf(stderr, gettext(
			"strip: %s: no section header table\n"),
			cur_filenm);
			return (FAILURE);
		}

		if (shdr->sh_type == SHT_NOBITS) {
			for (i = 1, j = 0; i <= (int)ehdr->e_phnum;
				i++, j += 2) {
		/*
		* Indented
		*/
...
traverse_file()
{
	 ...
	 * be safely deleted from the file, then mark its corresponding
	...
	while ((scn = elf_nextscn(elf, scn)) != 0) {
		...
		if (shdr->sh_type == SHT_REL || shdr->sh_type == SHT_RELA) {
			...
			if (shdr->sh_info != SHN_UNDEF && (temp_scn =
				elf_getscn(elf, shdr->sh_info)) != 0) {
				if ((temp_shdr =
					elf32_getshdr(temp_scn)) != 0) {
					temp_name = elf_strptr(elf,
						ndx,
					...
					if (temp_shdr->sh_type == SHT_NOBITS)
						...
						temp_shdr->sh_size);
					if ((((
				strcmp(temp_name, ".debug") == 0 ||
				strcmp(temp_name, ".stab") == 0 ||
				strcmp(temp_name, ".stab.excl") == 0 ||
				strcmp(temp_name, ".stab.index") == 0 ||
				strcmp(temp_name, ".stab.sbfocus") == 0) &&
				!lflag) || strcmp(temp_name, ".line") == 0) &&
				(y != IN) && (x != IN)) {
						sec_table[scn_no] = -1;
						no_of_del_sec++;
#if DEBUG
(void) printf("\n");
(void) printf("DEBUG: Deleting Section .rela%s\n", temp_name);
#endif
					} else {
			...
			strcmp(name, ".stab.excl") == 0 ||
			strcmp(name, ".stab.exclstr") == 0 ||
			strcmp(name, ".stab") == 0 ||
			strcmp(name, ".stabstr") == 0 ||
			strcmp(name, ".stab.index") == 0 ||
			strcmp(name, ".stab.indexstr") == 0 ||
			strcmp(name, ".stab.sbfocus") == 0 ||
			strcmp(name, ".stab.sbfocusstr") == 0) &&
			!lflag) ||
			strcmp(name, ".line") == 0) &&
			sec_table[scn_no] == 0 &&
...
#if DEBUG
(void) printf("\n");
(void) printf("DEBUG: Deleting Section %s\n", name);
#endif
			...
			if (shdr->sh_type == SHT_SYMTAB && x != IN) {
				/* eliminate ST later if possible */
				SYM = scn_no;
	...
	if (SYM && !xflag && (!REL || DYN)) {
		...
		if (SYM_LINK) {
			...
			no_of_del_sec++;
			for (i = SYM_LINK + 1; i < ehdr->e_shnum; i++) {
				if (sec_table[i] != -1)
					sec_table[i] = sec_table[i] - 1;
			}
		}
	...
	(void) printf(
	"DEBUG: sec_table[%d] = %d off_table[%d] = %x nobits_table[%d] = %d\n",
...
build_file()
{
	...
	Elf32_Sym *p, *q, *temp_p;
	unsigned int    c = 0;
	Elf32_Off	new_offset = 0, r;
	char *name = "";
	int ndx = ehdr->e_shstrndx;

	if ((fdtmp = open(elftmpfile,
		O_RDWR | O_TRUNC | O_CREAT, (mode_t) 0644)) == -1) {
		(void) fprintf(stderr, gettext(
		"strip: %s: can't open temporary file\n"),
		elftmpfile);
		strip_exit(FAILURE);
	...
	if (phdr != NULL) {
		...
		if ((elf_phdr = elf32_newphdr(elffile, ehdr->e_phnum)) ==
				NULL) {
			(void) fprintf(stderr, gettext(
			"strip: libelf error: %s\n"),
			elf_errmsg(-1));
...
#if DEBUG
(void) printf("\n");
(void) printf("DEBUG: Retaining Section %s\n", name);
#endif
			...
			if (shdr->sh_type == SHT_HASH ||
				...
				if (sec_table[shdr->sh_link] < 0) {
					elf_shdr->sh_link = 0;
					(void) fprintf(stderr, gettext(
"strip: Warning: %s: %s linked to a section that has been removed \n"),
					fname, name);
				} else
					elf_shdr->sh_link = (Elf32_Word)
						sec_table[shdr->sh_link];
...
"strip: Warning: %s: %s linked to a section that has been removed \n"),
				...
				if (sec_table[shdr->sh_info] < 0) {
					elf_shdr->sh_info = 0;
					(void) fprintf(stderr, gettext(
"strip: Warning: %s: %s info field set to a section"
" that has been removed \n"),
			...
			if (no_of_del_sec > 0 && shdr->sh_type == SHT_SYMTAB &&
				shdr->sh_entsize > 0) {
				/*
				 * copy this data onto elf_data then modify
				 * elf_data fields rather than data fields
				 */
				...
				for (c = 0; c < no_of_symbols;
					c++, p++, temp_p++) {
				temp_p->st_shndx = p->st_shndx;
				temp_p->st_name = p->st_name;
				temp_p->st_value = p->st_value;
				temp_p->st_size = p->st_size;
				temp_p->st_info = p->st_info;
				temp_p->st_other = p->st_other;
				if (p->st_shndx <= ehdr->e_shnum &&
					p->st_shndx  > 0) {
						if (sec_table[p->st_shndx] !=
							-1)
						temp_p->st_shndx =
							sec_table[p->st_shndx];
						else {
						temp_p->st_name = 0;
						temp_p->st_value = 0;
						temp_p->st_size = 0;
				...
				/* CSTYLED */
				elf_data->d_buf = (VOID *)q;
				elf_data->d_type = data->d_type;
				elf_data->d_size = data->d_size;
				elf_data->d_off = data->d_off;
				elf_data->d_align = data->d_align;
				elf_data->d_version = data->d_version;
			}
			else
				*elf_data = *data;
...
Elf32_Off offset;
{
	...
	for (i = 1, j = 0; i <= (int) ehdr->e_phnum; i++) {
		if (offset >= b_e_seg_table[j] &&
			offset <= b_e_seg_table[j + 1]) {
			return (IN);
