/*
	Copyright 05 Nov 1993 Sun Microsystems, Inc. All Rights Reserved
*/
%{
/***************************************************************** 

  @(#)cafe_dem.y	1.23 05 Nov 1993 15:11:09
...
int cafe_dem(char *in, NDEM_name *d, char *mem)
{
    int rtn = 0;

    if (!CHECK_OLD_PREFIX(in) && !CHECK_PREFIX(in)) return 0;

    if (in[0] == '$' && in[1] == 'U' && strcmp(in+2, "__ARG") == 0 ||
	in[0] == '_' && in[3] == 'U' && strcmp(in+4, "__ARG") == 0)
    {
	d->type = NDEM_unnamed_arg;
	d->qual_class = NULL;
	d->raw_name = NULL;
	return 1;
    }
...
enum DEM_TYPE cafe_getfieldtype(NDEM_name *d)
{
    switch(d->type) {
	case NDEM_constructor:
	    return DEM_CTOR;
...
/* A buffer is needed for printing out (possible) template arguments.
*/
#define	 TBUFSIZE	4096
static char tmem[TBUFSIZE];

char *cafe_getclass(NDEM_name *d, char *out)
{

    if (! d->qual_class) return NULL;

    out_buffer = out;
    obx = 0;	/* reset out_buffer index */
    startup(NULL, tmem, 0);	/* NO NEED to zero out tmem! */
    print_class(d->qual_class, 1);
    out_buffer[obx] = '\0';
    return out;
}

char *cafe_getname(NDEM_name *d, char *out)
{
    if (d->type == NDEM_constructor ||
		d->type == NDEM_destructor || d->type == NDEM_conversion)
    {
	out_buffer = out;
	obx = 0;
	startup(NULL, tmem, 0);		/* NO NEED to zero out tmem! */
	if (d->type == NDEM_constructor)
	    print_class(d->qual_class, 0);
	else if (d->type == NDEM_destructor)
	{
	    put_characters("~", 1);
	    print_class(d->qual_class, 0);
	}
...
char **cafe_getparentclass(NDEM_name *d, char **class_names)
{
    /* obsolete function !! */
...
char *cafe_gettemplatename(NDEM_name *d)
{
    NDEM_class *q;
    ...
    q = d->qual_class;
    while (q->qual_class) q = q->qual_class;

    return q->t_args? q->raw_class_name : NULL;
}

int is_cafe_symbol(char *s)
{
	return CHECK_OLD_PREFIX(s) || CHECK_PREFIX(s);
}

#endif

#ifdef CAFE_DEM_DEBUG
static largest = 0;
#endif
...
#ifdef CAFE_DEM_DEBUG
	    if (mem_reservoir - mem > largest)
	    {
		fprintf(stderr, "largest: %d\n", largest);
		fprintf(stderr, "\tname: %s\n", out);
		largest = mem_reservoir - mem;
	    }
#endif
	    return 0;
	}
...
char *cafe_demangle_and_format(char *in, char *out)
{
