...
#pragma ident	"@(#)m4macs.c	6.10	95/03/21 SMI"
...
register char 	**ap;
{
	...
	if ((ap[2] != NULL) && (strcmp(ap[1], ap[2]) == 0))
		error(gettext(
		"macro defined as itself"));
...
char	**ap;
{
	...
	char	*wrapbuf;		/* buffer for wrapstr */

	/* allocate m4wrap string */
	wrapbuf = xcalloc(strlen(a)+1, sizeof (char));
	if ((wrapentry = (struct Wrap *) malloc(sizeof (struct Wrap))) == NULL)
		error(gettext("out of storage"));
	wrapentry->wrapstr = wrapbuf;
	/* store m4wrap string */
	strcpy(wrapentry->wrapstr, a);
...
#define	MAX_PID	50
	...
	register char *l;
	char *first_X;
	register unsigned xcnt = 0;
	char my_pid[MAX_PID+1];
	int pid_len;
	int i = 0;
