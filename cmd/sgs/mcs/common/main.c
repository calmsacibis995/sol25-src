 ...
 */
#pragma ident	"@(#)main.c	6.11	95/03/06 SMI"
...
char	**argv;
{
	...
	if ((Action = (struct action *) malloc((unsigned)optbufsz*sizeof(struct action))) == NULL){
		(void) fprintf(stderr,
		"mcs: malloc memory allocation failure");
		exit(FAILURE);
		}
	...
	{
		(void) fprintf(stderr,
			"mcs: elf_version() failed - libelf.a out of date.\n");
		exit(FAILURE);
	}
...
char *cur_file;
{
	...
	{
                (void) fprintf(stderr, "mcs: %s: Cannot open file\n",cur_file);
                return(FAILURE); 
        }
        ...
        if ((elf_kind(arf) == ELF_K_AR))
	{
		...
		{  	/* open ar_temp_file */
			   ...
			   open(artmpfile, O_WRONLY | O_APPEND | O_CREAT, (mode_t) 0644)) == NULL)
                	{
                        	(void) fprintf(stderr,
                               	 "mcs: %s: Cannot open temporary file\n", artmpfile);
                        	(void) elf_end(arf);
                        	(void) close(fd);
                        	exit(FAILURE);
                	}
                	...
                	if ((write(fdartmp, ARMAG, SARMAG)) != SARMAG)
			{
				...
				(void) fprintf(stderr,
				"mcs: %s: write system failure: %s: file not manipulated.\n"
				,artmpfile, cur_file);
				mcs_exit(FAILURE);
			}
		}
	...
	elftmpfile = tempnam(TMPDIR, "mcs1"); /* holds temporary file;   */
        ...
        while ((elf = elf_begin(fd, cmd, arf)) != 0)
        {
                if (ar_file) /* get header info */
                {
                        ...
                        {
                                (void) fprintf(stderr, 
				"mcs: %s: malformed archive at %ld\n"
				,cur_file, elf_getbase(elf) );
				(void) elf_end(elf);
				(void) elf_end(arf);
				(void) close(fd);
		...
		if ((elf_kind(elf) == ELF_K_COFF)){
			(void) fprintf(stderr,
			"mcs: %s: Warning - internal conversion of COFF file to ELF\n"
			,cur_filenm);
                        if (elf_update(elf, ELF_C_NULL) == -1) {
                                (void) fprintf(stderr,
                                "mcs: trouble translating COFF file %s: %s\n"
                                ,cur_file, elf_errmsg(-1));
				update_failure++;	
                        }
		}
		...
		if ( elf_kind(elf) == ELF_K_COFF || elf_kind(elf) == ELF_K_ELF
			...
			if (ar_file && Might_chg )
			{
		      ...
		      /* or a COFF file that failed on a elf_update */
				...
				if (update_failure)
					(void)fprintf(stderr,
			   		"mcs: ERROR: %s: Cannot manipulate file.\n",cur_filenm);
				else
					(void)fprintf(stderr, 
			   		"mcs: %s: invalid file type\n",cur_filenm);
				(void) close(fd);
				return(FAILURE);
			}
			else
...
char *cur_file;
{
	...
	int	x;
        ...
        {
		(void) fprintf(stderr, 
		"mcs: WARNING: %s: Cannot manipulate file.\n",cur_file);
                error = FAILURE;
        }
        else 
...
initialize()
{
        ...
        {
                (void) fprintf(stderr,
                        "mcs: calloc memory allocation failure\n");
                exit(FAILURE);
        }
...
char *cur_file;
{
	...
	if ((b_e_seg_table = (Elf32_Off *) calloc(ehdr->e_phnum*3, sizeof(Elf32_Off))) == NULL)
       	{
		(void) fprintf(stderr,
		"mcs: calloc memory allocation failure\n");
		mcs_exit(FAILURE);
	...
	for (i=1, j=0; i<=ehdr->e_phnum; i++, phdr1++ )
	{
		...
		b_e_seg_table[j+2] = phdr1->p_offset + phdr1->p_filesz;
		j += 3;
	}

#if DEBUG
	for (i=1, j=0; i<=ehdr->e_phnum; i++, j+=3 )
	(void) printf("DEBUG: b_e_seg_table[%d] is start:%x\t memend: %x\t "
		      "filend:%x\n", j, b_e_seg_table[j], 
		      b_e_seg_table[j+1], b_e_seg_table[j+2]);
#endif
	return(SUCCESS);
}
...
for (i=0; i<=ehdr->e_shnum-1; i++)
	...
	for (act_index=0; act_index < actmax; act_index++)
	{
		...
		case ACT_PRINT:
					...
					{
						(void) fprintf(stderr, 
		"mcs: %s: Cannot print contents of a NOBITS section (%s)\n"
						,cur_file, Sect_name);
						break;
					}
		...
		case ACT_DELETE:
					if ((elf_kind(elf) == ELF_K_COFF))
					{
						error = CODE4;
						(void) fprintf(stderr, 
			"mcs: %s: Warning: Cannot delete section (%s)\n" 
						,cur_file, Sect_name);
						(void) fprintf(stderr,
		...
		case ACT_APPEND:
					if ((elf_kind(elf) == ELF_K_COFF))
						...
						error = CODE4;
                                                (void) fprintf(stderr,
			"mcs: %s: Warning: Cannot append to section (%s)\n"
						,cur_file, Sect_name);
						(void) fprintf(stderr,
					...
					if ( NOBITS_Sect )
                                        {
                                                error = CODE4;
                                                (void) fprintf(stderr,
                        "mcs: %s: Cannot append to a NOBITS section (%s)\n"
                                                ,cur_file, Sect_name);
                                        	break;
					}
					else 
		...
		case ACT_COMPRESS:
					if ((elf_kind(elf) == ELF_K_COFF))
						...
						error = CODE4;
                                                (void) fprintf(stderr,
				"mcs: %s: Cannot compress section (%s)\n"
						,cur_file, Sect_name);
						(void) fprintf(stderr,
						"\t\tin a COFF object file\n");
						 break;
					...
					if ( NOBITS_Sect) 
					{
						error = CODE4;
						(void) fprintf(stderr,
			"mcs: % s: Cannot compress a NOBITS section (%s)\n"
						,cur_file, Sect_name);
						 break;	
					}
					else
                                        if (sec_loc == IN)
                                        {
                                                error = CODE4;
                                                (void) fprintf(stderr,
                        "mcs: %s: Warning: Cannot compress a section (%s)\n"
                                                ,cur_file, Sect_name);
                                                (void) fprintf(stderr,
                                                "\t\tin a segment\n");
                                                break;
...
docompress()
{
                ...
                if (hash_num == hash_end) 
		{
			...
			{
				(void) fprintf(stderr,
				"mcs: realloc memory allocation failure\n");
				mcs_exit(FAILURE);
			}
                }
...
static int
location(int offset, int mem_search)
{
        int i,j, ind;
...
shdr_location(Elf32_Shdr * shdr)
{
	 ...
	 */
	if (shdr->sh_addr == 0)
		return (location(shdr->sh_offset + shdr->sh_size, 0));

	return (location(shdr->sh_offset + shdr->sh_size, 1));
}
...
build_file()
{
	...
	Elf32_Word	new_sh_name= 0;  /* to hold the offset for the new 
        ...
        if ((fdtmp = open(elftmpfile, O_RDWR | O_TRUNC | O_CREAT, (mode_t) 0644)) == -1){
                (void) fprintf(stderr,
			"mcs: %s: can't open temporary file\n", elftmpfile);
                return(FAILURE);
        }
	...
	{
		(void) fprintf(stderr,"mcs: libelf error: %s\n",elf_errmsg(-1));
		return(FAILURE);
	}	
        ...
        while ((scn = elf_nextscn(elf, scn)) != 0)
        {
			...
			if ( Sect_index != scn_no &&
				...
				|| (sec_loc == PRIOR && (string_size > 
					...
					else if (ELF32_ST_TYPE(p->st_info)
						 != STT_SECTION) {
                                                p->st_name = 0;
                                                p->st_value = 0;
                                                p->st_size = 0;
			...
			*elf_data = *data;
			...
			if ( Sect_index == scn_no )
			{
				...
				{ 
						...
						{
                        				(void) fprintf(stderr, "mcs: malloc memory allocation failure\n");
							mcs_exit(FAILURE);
                				}
				}
	...
	if ( elf_ehdr->e_shstrndx > 0 && elf_ehdr->e_shnum > 0 && 
		...
		/* UPDATE location of program header table */
                if (location(elf_ehdr->e_phoff, 0) == AFTER)
                {
                        r = new_offset % 4;
                        if (r)
                                new_offset += 4 - r;
		...
		/* UPDATE location of section header table */
                if  ( (location(elf_ehdr->e_shoff, 0) == AFTER) ||
			((location(elf_ehdr->e_shoff, 0) == PRIOR) &&
		...
		if ((notesegndx != -1) && (notesctndx != -1) &&
		    (sec_table[notesctndx])) {
			Elf32_Phdr *	newphdr;
			Elf_Scn *	notescn;
			Elf32_Shdr *	noteshdr;
			notescn = elf_getscn(elffile, sec_table[notesctndx]);
...
char *cur_file;
{
        ...
        {
                (void) fprintf(stderr,"mcs: %s: can't open temporary file\n"
			,elftmpfile);
                mcs_exit(FAILURE);
        }
	...
	{
		(void) fprintf(stderr, 
		"mcs: %s: read system failure: %s: file not manipulated.\n"
		,elftmpfile,cur_file);
		mcs_exit(FAILURE);
	}
...
char *cur_file;
{
		...
		{
		  	(void) fprintf(stderr,
		      "mcs: write system failure: %s: file not manipulated.\n"
			,cur_file);
                        mcs_exit(FAILURE);
		}
			...
			"mcs: malloc memory allocation failure\n");
                ...
                {
                        (void) fprintf(stderr, 
		      	"mcs: lseek system failure: %s: file not manipulated\n"
			, cur_file);
                        mcs_exit(FAILURE);
                }
			...
			!= (unsigned) ROUNDUP(mem_header->ar_size))
                {
                        (void) fprintf(stderr, 
			"mcs: read system failure: %s: file not manipulated\n"
			,cur_file);
                        mcs_exit(FAILURE);
                }
	...
	{
                (void) fprintf(stderr,
		"mcs: WARNING: %s: symbol table deleted from archive \n"
		,fname);
                (void) fprintf(stderr,
...
char *temp_file_name;
{
	...
	temp_fname = get_dir(fname);
        ...
        if ((fd=open(temp_fname, O_WRONLY | O_TRUNC | O_CREAT, (mode_t) 0644)) == -1)        {
                (void) fprintf(stderr,"mcs: %s: can't open file for writing\n"
			,temp_fname);
                mcs_exit(FAILURE);
        }
