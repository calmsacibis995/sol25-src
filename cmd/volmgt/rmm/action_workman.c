...
#pragma ident	"@(#)action_workman.c	1.11	95/08/10 SMI"
...
action(struct action_arg **aa, int argc, char **argv)
{
	...
	dprintf("action_workman: %s\n", aa[0]->aa_rawpath);
	if ((fd = open(aa[0]->aa_rawpath, O_RDONLY)) < 0) {
		dprintf("action_workman: open %m\n");
		return (FALSE);
	}

	if (ioctl(fd, CDROMREADTOCHDR, &th) < 0) {
		dprintf("action_workman: ioctl %m\n");
		close(fd);
		return (FALSE);
	}

	te.cdte_format = CDROM_MSF;
	for (i = (uint_t)th.cdth_trk0; i < (uint_t)th.cdth_trk1+1; i++) {
	...
	dprintf("action_workman: found audio (%d tracks)\n",
	...
	if (fork() == 0) {
...
#ifdef	DEBUG
		(void) fprintf(stderr, "DEBUG: child is running (pid = %d)\n",
		    getpid());
#endif
		/* child */
		chdir(rmm_dsodir);
...
#ifdef	DEBUG
		(void) fprintf(stderr,
		    "DEBUG: child: after switching output\n");
#endif
		/* clean up args */
		clean_args(&argc, argv);
#ifdef	DEBUG
		(void) fprintf(stderr,
		    "DEBUG: child: after cleaning up args\n");
#endif
		...
		(void) fprintf(stderr, "exec of %s failed; %s", argv[1],
		    strerror(errno));
		exit(1);
		/*NOTREACHED*/
	}
	 ...
	 */
#ifdef	DEBUG
	(void) fprintf(stderr, "DEBUG: sleeping a while -- just wait\n");
	(void) sleep(15);
#endif
	return (FALSE);
}
...
#ifdef	DEBUG
	(void) fprintf(stderr, "DEBUG: set_user_env(): entering\n");
...
#ifdef	DEBUG
	(void) fprintf(stderr, "DEBUG: set_user_env(): display found = %s\n",
	...
	char **src_argv)			/* src argv */
{
	...
	if ((arr = (char **)malloc((amt + 1) * sizeof (char *))) == NULL) {
		(void) fprintf(stderr,
		    gettext("error: can't allocate space!\n"));
