...
#pragma ident	"@(#)voltestdrv.c	1.26	95/08/23 SMI"
...
#define	VTLONGNAME	"Volume Management Test Driver"
						/* long driver name */
...
static struct vt_tab {
	...
	u_int		vt_flags;	/* per-device flags */
	char		*vt_name;	/* "label" for this dev */
	u_int		vt_namelen;	/* length of vt_name */
	u_int		vt_tag;		/* magic cookie */
...
static int		vt_evcnt;	/* number of events on the list */
...
vtstrategy(struct buf *bp)
{
	...
	DPRINTF("vt: strategy: bp %x dev %d.%d off %d len %d\n",
...
vtread(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	if (tp->vt_label != NULL) {
		...
		while ((uiop->uio_resid != 0) &&
			...
			DPRINTF2("vt: read: returning %d bytes\n", byte_count);
		...
		DPRINTF2("vt: read: returning name/label \"%s\" (%d bytes)\n",
...
vtioctl(dev_t dev, int cmd, int arg, int mode, cred_t *credp, int *rvalp)
{
	...
	if (unit == 0) {
		...
		case VTIOCTAG: {
			...
			DPRINTF2("vt: VTIOCTAG: unit %d tag set to 0x%x\n",
