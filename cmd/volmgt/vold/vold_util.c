...
#pragma ident	"@(#)vold_util.c	1.56	95/02/14 SMI"
...
static u_int	part_to_bits(char *val, u_char *);
...
#define	OLD_V_MAXPART	8	/* 8 slices max */


static char	*unsafe_list[DEFAULT_UNSAFE];
static u_int	unsafe_ind = 0;
...
minor_free(minor_t minr)
{
	...
	mt = minor_getmt((u_int)minr);
	REMQUE(mnr_q_hash[(u_int)minr % MNR_HASH_SIZE], mt);
	free(mt);
 ...
 */
void
minor_clrvol(u_int mnr)
{
	struct mnr_track *mt;


	if ((mt = minor_getmt(mnr)) != NULL) {
		mt->mt_vol = NULL;
	}
 ...
 * taking messages from the driver and figruing out what volume
 * they refer to.
 */
vol_t *
minor_getvol(u_int mnr)
{
	struct mnr_track *mt;
 ...
 * differtiate between different types of devices.  They should
...
props_get(vol_t *v)
{
	char 		*ps = NULL;
	char 		*s;
	char		tmpstr[MAXNAMELEN];
	char		partstr[MAXNAMELEN];
	u_int		parts;
	u_int		i;
	struct vnwrap	*vw;
	char		*path;
 ...
 * keeping track of paritions.
 */
static u_int
part_to_bits(char *val, u_char *npart)
{
	u_int 	parts = 0;
	char	*p, *q;
...
crcgentab()
{
	...
	crctab = (u_long *)malloc(256 * sizeof (u_long));
 ...
 */
u_long
calc_crc(u_char *buf, size_t len)
{
	u_long crc;
	u_long *crctab;
 ...
 */
void
calc_md4(u_char *buf, size_t len, u_longlong_t *sig)
{
	MD4_CTX		context;
 ...
 * a unique key, they can use the db_findkey function directly to
...
unique_time(char *medtype, char *labtype)
{
	...
	for (;;) {
		...
		/*
		 * If we try more than 10 times, we just
		 * give up... something is very wrong.  Hopefully we
		 * will not be hosing things too badly.
		 */
		if (loopcnt++ > 10) {
...
static void
print_vtoc(char *tag, int odl, int part_cnt, u_int maxoff, struct vtoc *vtoc)
{
	/*
	 * print the vtoc info
	 */
 ...
 * There is also code to detect for a cannonical floppy partition.
 ...
 *	this routine, for compatibilty
 */
void
partition_conv_2(struct vtoc *v, u_int maxoff, u_long *pmask, u_char *npart)
{
	int		i;
	int		j;
#ifdef	DEBUG_VTOC
	...
	*pmask = 0;
	...
	/*
	 * Remove duplicates.  The cannonical cd-rom label is captured
	 * here, since the driver returns the A and C partitions
	 * as the same (on sparc, at least).
	 */
	for (i = 0; i < V_MAXPART-1; i++) {
		if ((*pmask & (1 << i)) == 0) {
			continue;
		}
		for (j = i+1; j < V_MAXPART; j++) {
	...
	/* check for cannonical floppy parition. */
	/*
 ...
 * "bar" user, this function will return: "bar.rmtc.central.sun.com.",
 * if "rmtc.central.sun.com." is your domain name.
 */
char *
...
/*
 * Take apart a name like: "bar.rmtc.central.sun.com." and return the
 ...
 * "foo" group, this function will return: "foo@rmtc.central.sun.com.",
 * if "rmtc.central.sun.com." is your domain name.
 */
char *
...
/*
 * Take apart a name like: "foo@rmtc.central.sun.com." and return the
...
hash_string(char *s)
{
	...
	for (rotor = 0; *s; rotor = (rotor+1) % sizeof (rval)) {
...
dso_load(char *name, char *funcname, int vers)
{
	...
	(void) sprintf(namebuf, "%s/%s.%d", vold_devdir, name, vers);
	...
	if (stat(namebuf, &sb) < 0) {
		...
		if (stat(namebuf, &sb) < 0) {
			...
			(void) sprintf(namebuf, "%s/%s", vold_devdir, name);
	...
	for (; vw; vw = vw->vw_next) {
		...
		if (v->v_ndev > 1) {
			...
			for (i = 0; i < (int)v->v_ndev; i++) {
				(void) sprintf(namebuf, "%s/s%d",
				    path, dev_to_part(v, i));
...
match_path(char *p, int (*testpath)(char *))
{
	...
	char		**rval = NULL;
	int		nrval;
	char		*re = NULL;		/* real regexp (vs. sh type) */
	...
	int		nents;
	int		i;			/* loop index */
	int		re_ret;			/* return value from regcomp */
	size_t		err_len;		/* for getting RE err */
	char		*err_buf;		/* for getting RE err */
	...
	if (stat(p, &sb) == 0) {
		...
		rval = (char **)malloc(sizeof (char *) * 2);
		rval[0] = strdup(p);
		rval[1] = NULL;
		goto dun;	/* found a single match */
	}
	...
	if ((bn = strrchr(dn, '/')) == NULL) {
		debug(3, "no '/' in %s\n", p);
		goto dun;	/* pathname syntax error */
	}
	...
	*bn++ = NULL;
	...
	re = sh_to_regex(bn);
	if ((re_ret = regcomp(&re_cmp, re, REG_NOSUB)) != REG_OK) {
		/* get the RE err msg, then print it and exit */
		err_len = regerror(re_ret, &re_cmp, NULL, 0);
		err_buf = malloc(err_len+1);
		(void) regerror(re_ret, &re_cmp, err_buf, err_len);
	...
	if ((dirp = opendir(dn)) == NULL) {
		...
		regfree(&re_cmp);
		goto dun;	/* no such directory? */
	}

	/*
	 * ugly two pass algorithm: find out how many entries, then
	 * allocate an array of char ** to hold them.
	 */
	nents = 0;
	...
	/* allocate room for a full array of pointers */
	ents = (char **)malloc(sizeof (char *) * nents);
	...
	rewinddir(dirp);
	i = 0;
	while ((i < nents) && ((dp = readdir(dirp)) != NULL)) {
		if ((strcmp(dp->d_name, ".") == 0) ||
		    (strcmp(dp->d_name, "..") == 0)) {
			continue;
		}
		ents[i++] = strdup(dp->d_name);
	}

	/*
	 * sort the directory entries.
	 */
	qsort((char *)ents, nents, sizeof (char **), charcmp);

	/*
	 * see which ones match our RE.  instead of doing this
	 * two-pass, we simply allocate rval to be nents+1, based
	 * on the assumption that it will get freed soon after we
	 * return it anyway.
	 */

	/*
	 * array must be zeroed, since we probably won't use it all, and
	 * we count on it being null-ptr terminated
	 */
	rval = (char **)calloc(nents+1, sizeof (char *));

	nrval = 0;
	dnlen = strlen(dn);
	for (i = 0; i < nents; i++) {
		re_ret = regexec(&re_cmp, ents[i], 0, NULL, 0);
		if (re_ret == REG_NOMATCH) {
			free(ents[i]);
			continue;
		}
		if (re_ret != REG_OK) {
		...
		rval[nrval] = (char *)malloc(strlen(ents[i]) + dnlen + 2);

		(void) strcpy(rval[nrval], dn);
		(void) strcat(rval[nrval], "/");
		(void) strcat(rval[nrval], ents[i]);

		/* see if it's the kind of device we hope it is */
		if ((testpath != NULL) &&
		    ((*testpath)(rval[nrval]) != TRUE)) {
	...
	rval[nents] = NULL;			/* null terminate */
