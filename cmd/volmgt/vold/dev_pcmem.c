...
#pragma ident	"@(#)dev_pcmem.c	1.22	95/02/06 SMI"
...
/*
 *  XXX: Remove this when compiling with latest dkio.h.
 */
#ifndef DKC_PCMCIA_MEM
 ...
 */
#define	PCMEM_PIPE_PATH		"/tmp/.pcmcia/pcram"
...
#define	PCMEM_NAMEPROTO		"%s%s"

#define	PCMEM_NAME_TAIL		"d0s2"	/* on end of every pcmem card */
 ...
 *
 * put any library initializatio here
 */
bool_t
dev_init(void)
{
...
pipe_thread(void)
{
	...
	if (stat(p, &sb) < 0) {
		...
		if ((bn = pcmp_dirname(p)) != NULL) {
			if (makepath(bn, 0777) < 0) {
				debug(5,
				    "pipe_thread: makepath failed for: %s\n",
				    bn);
				goto dun;
			}
		}
		if (mknod((const char *)p, S_IFIFO|0666, NULL) < 0) {
...
open_again:
	...
	for (;;) {
		...
		if (strcmp(act_buf, PCMEM_PIPE_ACTION) == 0) {
			...
			(void) sprintf(sym_buf, symname, sym_num);

			/* set up the mode */
			(void) sprintf(mode_buf, "0%o", DEFAULT_MODE);
...
pcmem_testpath(char *p)
{
	...
	if (dev_getdp(sb.st_rdev)) {
		debug(5, "pcmem_testpath: \"%s\" already being managed\n", p);
		goto mem_dun;
	}
...
pcmem_use(char *path, char *symname)
{
	...
	static int		pcmem_get_sn(char *);
	void			*pcmem_thr_stk;
	struct stat		statbuf;
	char			namebuf[MAXNAMELEN+1];
	char			namebuf1[MAXNAMELEN+1];
	struct devs		*dp;
	struct mem_priv		*mem;
	char			*s;
	char			*p;
	...
	if (stat(path, &statbuf) < 0) {
		 ...
		 * We fill in the rest.
		 */
		(void) sprintf(namebuf, PCMEM_NAMEPROTO, path,
		    PCMEM_NAME_TAIL);
		...
		/* got a good path -- it'd better have a "d0s2" */
		if ((s = strstr(path, PCMEM_NAME_TAIL)) == NULL) {
			/* the full path didn't have a "d0s2" in it! */
			warning(gettext("pcmem: %s is an invalid path\n"),
			    path);
			res = FALSE;
			goto dun;
		}
		*s = NULLC;		/* truncate after "s2" */
	...
	if (dev_getdp(statbuf.st_rdev)) {
		debug(1, "pcmem_use: \"%s\" already being managed\n", namebuf);
		res = TRUE;
		goto dun;
	}
	...
	(void) strcpy(path_save, namebuf);

	/* stick some good stuff in the device hierarchy */
	if ((s = strstr(path, "rdsk")) != 0) {
		...
		(void) sprintf(namebuf1, PCMEM_NAMEPROTO, path,
		    PCMEM_NAME_TAIL);
		...
		if ((p = strchr(s, '/')) != 0) {
			p++;
			(void) sprintf(namebuf, "%s/%s", PCMEM_DSK_PATH, p);
		} else {
			/* no slash after rdsk? */
			debug(1,
		...
		(void) sprintf(namebuf1, PCMEM_NAMEPROTO, namebuf,
		    PCMEM_NAME_TAIL);
		mem->mem_blockpath = strdup(namebuf1);

	} else if (s = strstr(path, "dsk")) {

		/* he gave us the block path */

		/* save pointer to block vv-node */
		bvn = dev_dirpath(path);
		...
		if ((p = strchr(s, '/')) != 0) {
			p++;
			(void) sprintf(namebuf, "%s/%s", PCMEM_RDSK_PATH, p);
		} else {
			/* no slash after "dsk"? */
			debug(1,
		"pcmem_use: using malformed pathname (no '/') \"%s\"\n",
	...
	/*
	 * dereference the /dev pathname to find out the real pcmem
	 * socket number
	 */
	if ((sn = pcmem_get_sn(path_save)) >= 0) {
		debug(3, "pcmem_use: path_save=\"%s\", sn=%d\n",
		    path_save, sn);
		free(dp->dp_symname);
		(void) sprintf(namebuf, PCMEM_SYMNAME, sn);
...
/*
 * Get socket number from path:
 *	/dev/[r]dsk/cNtNd0s2
 * which is linked to:
 *	/devices/pcmcia/pcmem@socketN/pcram@tN,dN:dev[,raw]
 */
static int
pcmem_get_sn(char *path)
{
	DIR		*dskdirp;
	...
	/* open the /dev/rdsk directory */
	if ((dskdirp = opendir(PCMEM_RDSK_PATH)) == NULL) {
		debug(10, "pcmem_get_sn: can't open directory \"%s\"; %m\n",
		    PCMEM_RDSK_PATH);
		return (NULL);
	}

	/* Get cntndnsn from /dev/rdsk/cntndnsn */
	if ((cpath = strrchr(path, '/')) == NULL) {
		cpath = path;
	} else {
		cpath++;	/* skip to cntndnsn */
	}
	...
	while (dskentp = readdir(dskdirp)) {
		 ...
		 * stating with "c"
		 */
		if (dskentp->d_name[0] != 'c') {
			continue;
		}

		/*
		 * Skip if cpath [cNtNd0] is not a subset of
		 * dskentp->d_name [cNtNd0s2]
		 */
		if (strncmp(dskentp->d_name, cpath, strlen(cpath)) != 0) {
			continue;
		}
		...
		(void) sprintf(namebuf, "%s/%s", PCMEM_RDSK_PATH,
		    dskentp->d_name);

		if (lstat(namebuf, &sb) < 0) {
			debug(10, "pcmem_get_sn: can't stat \"%s\"; %m\n",
			    namebuf);
			continue;
		}
		...
		if (S_ISLNK(sb.st_mode)) {
			...
			if (linksize <= 0) {
				debug(10,
			"pcmem_get_sn: realink failed on \"%s\"; %m\n",
				    namebuf);
				continue;
			}
			linkbuf[linksize] = NULLC;

			/* Skip [../..]/devices/pcmcia/... */
			devp = strstr(linkbuf, PCMEM_DEVICES_DIR);
			if (devp == NULL) {
				debug(10,
				    "pcmem_get_sn: no \"%s\" in \"%s\"\n",
				    PCMEM_DEVICES_DIR, linkbuf);
				continue;
			}

			devp += strlen(PCMEM_DEVICES_DIR);
			(void) sprintf(dtype, "/%s@", PCMEM_MTYPE);
	...
	if (ioctl(mem->mem_fd, FDGETDRIVECHAR, &fdchar) < 0) {
		...
		if (fdchar.fdd_ejectable == 0) {
			debug(1, "pcmem_thread: \"%s\" manually ejectable\n",
			    mem->mem_rawpath);
			dp->dp_dsw->d_flags |= D_MEJECTABLE;
		}
	}
...
{
	extern int		cond_timedwait(cond_t *, mutex_t *,
					timestruc_t *);
	extern bool_t		dev_present(struct devs *);
	timestruc_t		ts;
	struct timeval		tv;
	...
	/* get the current time (for timed wait) */
	if (gettimeofday(&tv, NULL) < 0) {
		warning(gettext("pcmem: can't get time of day; %m\n"));
		(void) cond_wait(&dp->dp_lock->dp_vol_vg_cv,
		    &dp->dp_lock->dp_vol_vg_mutex);
		(void) mutex_unlock(&dp->dp_lock->dp_vol_vg_mutex);
		debug(10, "wait_til_signalled: cond_wait succeeded\n");
		return;
	}

	/* set up time delay */
	(void) memset(&ts, 0, sizeof (timestruc_t));
	ts.tv_sec = tv.tv_sec + PCMEM_OVERLAP_TIMEOUT_SECS;

	debug(10,
	    "wait_til_signalled: waiting for ejection (%d sec timeout)\n",
	    PCMEM_OVERLAP_TIMEOUT_SECS);

	if ((ret_val = cond_timedwait(&dp->dp_lock->dp_vol_vg_cv,
	    &dp->dp_lock->dp_vol_vg_mutex, &ts)) != 0) {
		(void) mutex_unlock(&dp->dp_lock->dp_vol_vg_mutex);
		debug(10, "wait_til_signalled: cond_timedwait error: %d\n",
		    ret_val);
		return;
	}
	...
	debug(10, "wait_til_signalled: cond_timedwait succeeded\n");
}
