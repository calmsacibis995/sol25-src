...
#pragma ident	"@(#)dev_floppy.c	1.46	94/11/09 SMI"
...
floppy_use(char *path, char *symname)
{
	...
	if (stat(path, &statbuf) >= 0) {
		 ...
		 */
		(void) sprintf(namebuf, FLOPPY_NAMEPROTO, path);
	...
	if (s = strstr(path, "diskette")) {
		...
		(void) sprintf(namebuf, "/dev/rdiskette%d", unit);
		...
		(void) sprintf(namebuf, "/dev/diskette%d", unit);
		...
		(void) sprintf(namebuf, "/dev/rfd%d", unit);
		rvn = dev_dirpath(path);

		/* make the block name */
		(void) sprintf(namebuf, "/dev/fd%d", unit);
		bvn = dev_dirpath(namebuf);

		/* make the full raw path name */
		(void) sprintf(namebuf, "/dev/rfd%dc", unit);
		flp->fl_rawpath = strdup(namebuf);
		/* make the full raw path name */
		(void) sprintf(namebuf, "/dev/fd%dc", unit);
		flp->fl_blockpath = strdup(namebuf);
...
#ifdef MT
	flp->fl_tid = thread_create(0, 0,
		(void *(*)(void *))floppy_thread, (void *)dp);
...
floppy_close(char *path)
{
	...
	if (stat(path, &sb) < 0) {
		...
		if (stat(namebuf, &sb) < 0) {
			warning(gettext("floppy_close: %s; %m\n"), namebuf);
			return;
		}
	}
	 ...
	 */
	(void) mutex_enter(&flp->fl_mutex);
	(void) thread_kill(flp->fl_tid, SIGUSR1);
	/* apparently we have to kick it out of the cv_wait */
	(void) cv_broadcast(&flp->fl_cv);
	(void) mutex_exit(&flp->fl_mutex);
	(void) thread_join(flp->fl_tid);
...
floppy_check(struct devs *dp)
{
	 ...
	 */
	(void) time(&tloc);
	if ((tloc - flp->fl_lastcheck) < 2) {
		goto out;
	}
	flp->fl_lastcheck = tloc;
...
open_exclusive(struct fl_priv *flp, int unit)
{
	...
	(void) sprintf(namebuf, "/dev/rfd%da", unit);
	flp->fl_fd_a = open(namebuf, O_RDWR|O_EXCL|O_NDELAY);
	(void) sprintf(namebuf, "/dev/rfd%db", unit);
