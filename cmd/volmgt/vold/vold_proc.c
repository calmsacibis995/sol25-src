...
#pragma ident	"@(#)vold_proc.c	1.23	94/08/26 SMI"
...
nfsproc_readlink_2_svc(nfs_fh *fh, struct svc_req *req)
{
	...
	res.readlinkres_u.data = strdup(vn->vn_sym->sla_ptr);
	res.status = NFS_OK;
	return (&res);
}
...
nfsproc_symlink_2_svc(symlinkargs *args, struct svc_req *req)
{
	...
	if (args->attributes.uid == -1)
		args->attributes.uid = cred->aup_uid;
	if (args->attributes.gid == -1)
		args->attributes.gid = cred->aup_gid;
...
nfsproc_readdir_2_svc(readdirargs *args, struct svc_req *req)
{
	...
	int		entrycount;
	/*LINTED: alignment ok*/
	u_int		cookie = *(u_int *)args->cookie;
...
static bool_t
checkaccess(vvnode_t *dvn, vvnode_t *vn, struct authsys_parms *aup, u_int acc)
{
