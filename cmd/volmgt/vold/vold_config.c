...
#pragma ident	"@(#)vold_config.c	1.27	95/01/17 SMI"
...
static struct cmds {
	char	*name;
	bool_t	(*func)(int argc, char **argv, u_int ln);
	bool_t	present;
} cmd_list[] = {
	{ "use", conf_use, FALSE },
	{ "eject", conf_action, TRUE },
	{ "insert", conf_action, TRUE },
	{ "notify", conf_action, TRUE },
	{ "error", conf_action, TRUE },
	{ "label", conf_label, FALSE },
	{ "db", conf_db, FALSE },
	{ "unsafe", conf_unsafe, TRUE },
	{ 0, 0}
};
...
static struct flags {
	...
	void	*fl_value;	/* value of the flag */
	void	*fl_default;	/* default value of the flag */
...
static void 	*getflag(u_int, struct flags *);
...
config_read(void)
{
	 ...
	 */
	(void) stat(vold_config, &sb); /* XXX: what if this fails ?? */
	...
	if ((cfp = fopen(vold_config, "r")) == NULL) {
		warning(gettext("config_init: %s; %m\n"), vold_config);
		return (1);
	}
	...
	while (fgets(buf, BUFSIZ, cfp) != NULL) {
		...
		if (!found) {
			warning(gettext(
			    "config_init: unknown directive %s, line %d\n"),
			    av[0], lineno);
		}
		if (wholeline != buf) {
			free(wholeline);
...
conf_use(int argc, char **argv, u_int ln)
{
	...
	rval = dev_use(argv[1], argv[2], argv[3], argv[5],
	    getflag(FLAG_UID, flagp), getflag(FLAG_GID, flagp),
	    getflag(FLAG_MODE, flagp), (bool_t)getflag(FLAG_TEMP, flagp),
	    (bool_t)getflag(FLAG_FORCE, flagp));
...
conf_action(int argc, char **argv, u_int ln)
{
	...
	ap->ap_args[i] = 0;
	ap->ap_uid = network_uid(getflag(FLAG_UID, flagp));
	ap->ap_gid = network_gid(getflag(FLAG_GID, flagp));
	ap->ap_line = ln;
	ap->ap_maptty = (u_int)getflag(FLAG_MAPTTY, flagp);
	return (TRUE);
}


static void *
getflag(u_int tag, struct flags *flagp)
{
	struct flags *fp;
	for (fp = flagp; fp->fl_tag != 0; fp++) {
		if (fp->fl_tag == tag) {
			if (fp->fl_set == TRUE) {
				...
				return (fp->fl_default);
			}
		}
	}
	debug(1, "unknown flag tag %d (bug in vold_config:getflag)\n",
		fp->fl_tag);
	return (0);
}
...
parseflagvalue(struct flags *fp, char *value)
{
	...
	case FLAGTYPE_INT:
		if (value) {
			fp->fl_value = (void *)malloc(sizeof (int));
			*(int *)fp->fl_value = atoi(value);
			fp->fl_set = TRUE;
		}
		break;
	case FLAGTYPE_STRING:
		if (value) {
			fp->fl_value = (void *)strdup(value);
			fp->fl_set = TRUE;
		}
		break;
	case FLAGTYPE_BOOL:
		if ((value == NULL) ||
		    (strcmp(value, "true") == 0) ||
		    (*value == NULLC)) {
			fp->fl_value = (void *)TRUE;
		} else {
			fp->fl_value = (void *)FALSE;
		}
		fp->fl_set = TRUE;
		break;
	default:
		debug(1, "unknown flag type %d: vold_config:parseflagvalue\n",
		    fp->fl_type);
		break;
	}
}
