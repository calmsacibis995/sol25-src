...
#pragma ident	"@(#)vold_vol.c	1.58	95/05/26 SMI"
...
vol_event(struct vioc_event *vie)
{
	...
	case VIE_SYMNAME: {
		...
		symname = dev_symname(dev);

		debug(11, "vol_event: VIE_SYMNAME: (%d,%d) -> \"%s\"\n",
		    major(dev), minor(dev), symname);

		(void) ioctl(vol_fd, VOLIOCDSYMNAME, symname);
		break;
	}
	...
	case VIE_SYMDEV: {
		...
		symdev = symname_to_dev(symname);
		debug(11, "vol_event: VIE_SYMDEV: \"%s\" -> \"%s\"\n",
		    symname, symdev);

		(void) ioctl(vol_fd, VOLIOCDSYMDEV, symdev);
		break;
	}
	...
	"vol_event: created vol_insert() tid %d (%d,%d) (for EJ w/new lab)\n",
			    id, major(vid->vi_dev), minor(vid->vi_dev));
#endif
	...
	case VIE_CANCEL:
		v = minor_getvol(vie->vie_cancel.viec_unit);

		debug(5, "cancel on unit %d (vol %s)\n",
		    vie->vie_cancel.viec_unit, v->v_obj.o_name);
		...
		v->v_ej_inprog = TRUE;
		v->v_clue.c_volume = vie->vie_cancel.viec_unit;
	...
	case VIE_NEWLABEL:
		debug(5, "newlabel on unit %d\n",
		    vie->vie_newlabel.vien_unit);
		v = minor_getvol(vie->vie_newlabel.vien_unit);
		v->v_flags |= V_NEWLABEL;
		break;
	...
	case VIE_GETATTR: {
		...
		v = minor_getvol(vie->vie_attr.viea_unit);
		props = props_get(v);
		...
		vda.vda_unit = vie->vie_attr.viea_unit;
...
#ifdef	DEBUG
	static const char	*laread_res_to_str(enum laread_res);
...
#ifdef	DEBUG
		debug(1,
"vol_foundlabel: examing \"missing\" vol \"%s\" (looking for \"%s\")\n",
...
#endif
	err = waitpid(r->r_pid, &stat, WNOHANG);
	if (err == 0) {
