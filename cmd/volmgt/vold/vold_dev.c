...
#pragma ident	"@(#)vold_dev.c	1.61	95/08/16 SMI"
...
dev_makedp(struct devsw *dsw, char *path)
{
	...
	(void) mutex_setup(&dp->dp_lock->dp_vol_vg_mutex);
	(void) cv_setup(&dp->dp_lock->dp_vol_vg_cv);
...
dev_eject(vol_t *v, bool_t ans)
{
	...
	(void) mutex_enter(&dp->dp_lock->dp_vol_vg_mutex);
#ifdef	DEBUG
	debug(5, "dev_eject: signalling that eject's done on unit %d\n",
	    viej.viej_unit);
#endif
	(void) cv_broadcast(&dp->dp_lock->dp_vol_vg_cv);
	(void) mutex_exit(&dp->dp_lock->dp_vol_vg_mutex);
}
...
{
	extern void	minor_clrvol(u_int);
	u_int		i;
...
dev_map(vol_t *v, bool_t ndelay)
{
	...
	for (i = 0; i < v->v_ndev; i++) {
		...
		if (v->v_confirmed) {
			...
			vim.vim_pathlen = strlen(vim.vim_path)+1;
...
#ifdef	DEBUG
			debug(1,
			"dev_unmap: calling VOLIOCFLAGS(VFL_ENXIO), unit %d\n",
			    volume);
#endif
			if (ioctl(vol_fd, VOLIOCFLAGS, &vfl) < 0) {
				debug(1, "dev_unmap: VOLIOCFLAGS; %m\n");
			}
		}
 ...
 */
vol_t *
dev_unlabeled(struct devs *dp, enum laread_res rres)
{
	vol_t	*v;
	...
	case L_NOTUNIQUE:
		...
		break;
	default:
		v->v_obj.o_name = strdup("broken_label_code");
