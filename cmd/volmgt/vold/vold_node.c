...
#pragma ident	"@(#)vold_node.c	1.41	95/07/10 SMI"
...
static obj_t 	*node_mkpartat(vol_t *v, int devno);
static mode_t	node_dirmode(mode_t mode);
static void	node_newdir(vvnode_t *dvn);
static void	node_updtime(vvnode_t *);

/* local variables */
static vvnode_t	*dskroot;			/* top of dsk */
static vvnode_t	*rdskroot;			/* top of rdsk */
...
node_uniqname(vvnode_t *dvn, obj_t *obj)
{
	...
	while (1) {
		...
		(void) sprintf(newname, "%s#%d", obj->o_name, iter++);
 ...
 */
vvnode_t *
node_mkobj(vvnode_t *dvn, obj_t *obj, u_int flags, u_int *err)
{
	vvnode_t 	*vn, *tvn;
	u_int		nflags;
	...
	*err = 0;				/* initialize to "no error" */
	...
	if (obj->o_dir == NULL) {
		if ((dvn != NULL) && (dvn != root)) {
			...
			(void) sprintf(buf, "%s/%s", dvn->vn_obj->o_dir,
			    dvn->vn_obj->o_name);
	...
	case VV_CHR: {
		...
		if (vn->vn_vol->v_ndev > 1) {
			if (flags & NODE_BLK) {
				...
				vn->vn_otype = vn->vn_type;
			}
			vn->vn_type = VV_DIR;
			for (i = 0; i < vn->vn_vol->v_ndev; i++) {
				tvn = node_mkobj(vn,
				    node_mkpartat(vn->vn_vol, i),
				    NODE_TMPID|flg, &lerr);
				tvn->vn_num = i;
			}
			vn->vn_obj->o_mode = node_dirmode(vn->vn_obj->o_mode);
		}
		break;
	}
	case VV_LINK:
...
/*
 * Build an obj_t to represent device number "devno" of volume "v".
 */
static obj_t *
node_mkpartat(vol_t *v, int devno)
{
	partat_t	*pa;
	char		namebuf[MAXNAMELEN];


	pa = (partat_t *)calloc(1, sizeof (partat_t));
	/* XXX: need to support changing partition names... */
	(void) sprintf(namebuf, "s%d", dev_to_part(v, devno));
	pa->pa_obj.o_name = strdup(namebuf);
	pa->pa_obj.o_type = VV_PART;
	pa->pa_obj.o_uid = v->v_obj.o_uid;
	pa->pa_obj.o_gid = v->v_obj.o_gid;
	pa->pa_obj.o_mode = v->v_obj.o_mode;
	pa->pa_obj.o_atime = v->v_obj.o_atime;
	pa->pa_obj.o_ctime = v->v_obj.o_ctime;
	pa->pa_obj.o_mtime = v->v_obj.o_mtime;
	pa->pa_obj.o_nlinks = 1;
	return ((obj_t *)pa);
}
 ...
 */
void
node_remove(obj_t *obj, bool_t dbup, u_int *err)
{
	struct vnwrap 	*vw;
	struct vnwrap 	*nvw;
	vvnode_t 	*vn;
	vvnode_t 	*vn_next = NULL;
...
node_move(vvnode_t *vn, vvnode_t *tdvn, char *name)
{
	...
	if (vn->vn_parent != tdvn) {
		...
		(void) sprintf(namebuf, "%s/%s", tdvn->vn_obj->o_dir,
		    tdvn->vn_obj->o_name);
...
node_newdir(vvnode_t *dvn)
{
	...
	(void) sprintf(namebuf, "%s/%s", dvn->vn_obj->o_dir,
	    dvn->vn_obj->o_name);
	for (vn = dvn->vn_child; vn; vn = vn->vn_sib) {
		change_dir(vn->vn_obj, namebuf);
...
/*
 * Take a mode_t for what is assumed to be a "regular" file and
 * make it reasonable for a directory.
 * read and write access stay the same as the regular file,
 * we just add execute permission wherever there is read
 * permission.
 */
static mode_t
node_dirmode(mode_t mode)
{
	if (mode & S_IRUSR) {
		mode |= S_IXUSR;
	}
...
node_fattr(vvnode_t *vn, struct fattr *fat)
{
	...
	case VV_CHR:
		...
		fat->mode = node_mapmode(vn->vn_vol);
		if (vn->vn_vol->v_devmap == 0) {
			dev_devmap(vn->vn_vol);
		}
		fat->rdev = vn->vn_vol->v_devmap[vn->vn_num].dm_voldev;
		ASSERT(fat->rdev != 0);
	...
	case VV_PART:
		...
		fat->mode = node_mapmode(vn->vn_parent->vn_vol);
	...
	fat->uid = vn->vn_obj->o_uid;
	fat->gid = vn->vn_obj->o_gid;
...
node_findlabel(struct devs *dp, label *la)
{
	...
	/* We already have one... can't have any more! */
	if (v->v_confirmed == TRUE) {
	...
	if (vw == NULL) {
		(void) sprintf(namebuf, "%s/%s", v->v_obj.o_dir,
		    v->v_obj.o_name);
...
/*ARGSUSED*/
struct vnwrap *
node_findnode(u_longlong_t nid, u_char off, u_char dir, u_char type)
{
	struct vvnode *vn;
...
{
	/*LINTED: alignment ok*/
	struct internal_fh	*vifh, *ifh = (struct internal_fh *)fh;
	struct vvnode		*vn;
	u_int			hash_line;



	hash_line = ifh->fh_id % FH_HASH_SIZE;
	vn = HEAD(struct vvnode, fh_q_hash[hash_line]);
...
node_fid(vvnode_t *vvnode)
{
	...
	if (ifh->fh_id > TMPID_BASE) {
		 ...
		 */
		return ((u_int)(ifh->fh_id - TMPID_BASE)|0x80000000);
	} else {
		return ((u_int)ifh->fh_id+1);
	}
}
...
node_setup()
{
	...
	da = node_mkdirat("rdsk", DEFAULT_TOP_UID,
	    DEFAULT_TOP_GID,  DEFAULT_TOP_MODE);
