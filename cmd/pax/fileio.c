...
#ident	"@(#)fileio.c	1.2	94/10/27 SMI"
...
static char rcsid[] = "@(#)$RCSfile: fileio.c,v $ $Revision: 1.2.4.2 $ (OSF) $Date: 1992/10/30 20:15:34 $";
#endif
/* 
...
#define	FIO_EXISTS	"The file exists and will not be overwritten."
#define	FIO_SAME	"The file is the same."
#define	FIO_NEWER	"The file exists and is newer."
#define	FIO_DIR		"Directories are not being created."
#define	FIO_UKNOWN	"The filetype is unknown."
...

{
	...
	if (link(linkp->l_name, name) != 0) {
		...
		if (f_dir_create) {
			...
			int se = errno;	/* sprintf can change errno */
			sprintf(err,"link: %s", linkp->l_name);
