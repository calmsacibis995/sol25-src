...
#ident	"@(#)names.c	1.3	95/03/01 SMI"
...
static char rcsid[] = "@(#)$RCSfile: names.c,v $ $Revision: 1.2.2.2 $ (OSF) $Date: 1991/10/01 15:54:49 $";
#endif
/* 
...
finduname(int uuid)
{
	...
	pw = getpwuid(uuid);
	if (pw) {
	    strncpy(saveuname, pw->pw_name, TL_UNAME);
	}
    }
    return(saveuname);
}
...
finduid(char *uname)
{
	...
	strncpy(saveuname, uname, TL_UNAME);
	pw = getpwnam(uname);
	if (pw) {
	    saveuid = pw->pw_uid;
	} else {
...
findgname(int ggid)
{
	...
	gr = getgrgid(ggid);
	if (gr) {
	    strncpy(savegname, gr->gr_name, TL_GNAME);
	}
    }
    return(savegname);
}
...
findgid(char *gname)
{
    ...
    if (gname[0] != savegname[0] || strncmp(gname, savegname, TL_GNAME) != 0) {
	strncpy(savegname, gname, TL_GNAME);
	gr = getgrnam(gname);
	if (gr) {
	    savegid = gr->gr_gid;
	} else {
