...
#ident	"@(#)buffer.c	1.3	94/12/06 SMI"
...
#define	BUF_PRESERVE	"An error occurred attempting to preserve the owner and group ids."
#define	BUF_EOF		"An unexpected end-of-file has occurred."
#define	BUF_OVERFLOW	"An overflow condition exists in the write buffer."
#define	BUF_BLKSIZE	"The blocksize value must be greater than zero."
#define	BUF_IOBUF	"Cannot allocate memory for the input/output buffer."
#define	BUF_BUFSKIP	"An overflow condition has occurred within the write buffer."
#define	BUF_BUF_READ	"An overflow condition exists in the read buffer."
#define	BUF_CORRUPT	"The archive file appears to be corrupt."
#define	BUF_WRITE	"A write error has occurred."
#define	BUF_INPUT	"The archive is empty."
#define	BUF_AVAIL	"An overflow condition exists within the write buffer."
...

{
	...
	if (chown(name, asb->sb_uid, asb->sb_gid) < 0) 
	    warn(name, MSGSTR(BUF_PRESERVE, "unable to preserve owner/group"));
	else if (f_mode)
...

{
	...
	if (got < chunk) {
	    ...
	    warn(name, MSGSTR(BUF_EOF, "Early EOF"));
	    while (got < chunk) {
		buf[got++] = '\0';
	    }
	}
...

{
    ...
    while (want = endx - idx) {
	if (bufend - bufidx < 0) {
	    fatal(MSGSTR(BUF_OVERFLOW, "Buffer overflow in write"));
	}
...
{
    if (size <= 0) {
	fatal(MSGSTR(BUF_BLKSIZE, "Invalid value for blocksize"));
    }
    if ((bufstart = malloc((unsigned) size)) == (char *)NULL) {
	fatal(MSGSTR(BUF_IOBUF, "Cannot allocate I/O buffer"));
    }
...

{
    ...
    while (len) {
	if (bufend - bufidx < 0) {
	    fatal(MSGSTR(BUF_BUFSKIP, "Buffer overflow in skip"));
	}
...

{
    ...
    while (want = endx - dst) {
	if (bufend - bufidx < 0) {
	    fatal(MSGSTR(BUF_BUF_READ, "Buffer overflow in read"));
	}
...

{
    ...
    if (corrupt) {
	warn(name, MSGSTR(BUF_CORRUPT, "Corrupt archive data"));
    }
    if (oops) {
	warn(name, oops);
...
static void outflush(void)
{
	...
	for (buf = bufstart, len = bufidx - buf;
	    ...
	    if ((got = write(archivefd, buf, f_blocking ? bufend -
		    ...
		    fatal(MSGSTR(BUF_WRITE, "Tape write error"));
	    }
	}
...

{
	...
	if (areof) {
	    if (total == 0) {
		fatal(MSGSTR(BUF_INPUT, "No input"));
	    } else {
		next(AR_READ);
	    }
	}
...

{
    ...
    if (bufend - bufidx < 0) {
	fatal(MSGSTR(BUF_AVAIL, "Buffer overflow in buf_out_avail"));
    }
    if ((have = bufend - bufidx) == 0) {
	outflush();
    }
