...
#ident	"@(#)extract.c	1.4	95/03/20 SMI"
...
static char rcsid[] = "@(#)$RCSfile: extract.c,v $ $Revision: 1.3.1.2 $ (OSF) $Date: 1991/10/09 14:12:04 $";
#endif
/* 
...
#define	EX_CORRUPT	"The archive file appears to be corrupt."
#define	EX_NOTAR	"This doesn't look like a tar archive."
#define	EX_SKIP		"         Skipping to the next file..."
#define	EX_UNREG	"The archive format is not recognizable."
#define	EX_BADMAG	"The magic number is not recognizable."
#define	EX_MAY		"The file may be corrupt."
#define	EX_RESYNC	"The extraction appears to have resynchronized."
#define	EX_CONT		"Continuing"
#define	EX_SYM		"A symbolic link is corrupted."
#define	EX_OWN		"An error occurred attempting to restore the owner and group ids."
#define	EX_HDR		"The swapped header is corrupted."
#define	EX_PLEN1	"The swapped pathname length is bad."
#define	EX_PATH		"The swapped pathname is corrupted."
#define	EX_BADPATH	"The swapped pathname is bad."
#define	EX_ASCII	"A cpio ASCII header is corrupted."
#define	EX_BADHDR	"A cpio ASCII header is bad."
#define	EX_PLEN2	"A cpio ASCII pathname length is bad."
#define	EX_APATH	"A cpio ASCII pathname is corrupted."
#define	EX_BADAPATH	"A cpio ASCII pathname is bad."
#define	EX_BHDR		"A cpio binary header is corrupted."
#define	EX_PLEN3	"A cpio binary pathname length is bad."
#define	EX_BPATH	"A cpio binary pathname is corrupted."
#define	EX_BADP		"A cpio binary pathname is bad."
...
read_archive(void)
{
	...
	while (get_header(name, &sb) == 0) {
		if ((match = name_match(name,((sb.sb_mode & S_IFMT) ==
					      S_IFDIR))) == -1)
		    break;
		else
		...
		if (rplhead != NULL) {
			...
			if (strlen(name) == 0) {
				if (((ar_format == TAR) 
				     ? buf_skip(ROUNDUP((OFFSET)
							sb.sb_size,
							BLOCKSIZE)) 
				     : buf_skip((OFFSET) sb.sb_size)) < 0) {
					warn(name, MSGSTR(EX_CORRUPT,
							  "File data is "
							  "corrupt"));
				}
				continue;
...
inswab(char *magic, char *name, Stat *asb)
{
    ...
    if ((namesize = SWAB(binary.b_name)) == 0 || namesize >= (size_t) PATH_MAX+1) {
