...
#ident	"@(#)accesssr.c	1.39	95/08/30 SMI"
...
int reg;
{
	...
	case Reg_PSR:
		if (v9flag) {
			ra->ra_raddr = (int *)&arp_v9->r_tstate;
			ra->ra_mode = RA_64BIT;
		} else {
	...
	case Reg_G0:
	 ...
	 case Reg_G7:
		if (v9flag) {
			ra->ra_raddr = (int *)&arp_v9->r_globals[ reg - Reg_G1 ];
			ra->ra_mode = RA_64BIT;
		} else {
	 ...
	 case Reg_O7:
		if (v9flag) {
			ra->ra_raddr = &(adb_oreg_v9[reg - Reg_O0] );
		} else {
	 ...
	 case Reg_FQ:	/* Can't get the FQ */
		...
		break;
	 case Reg_FSR:
		ra->ra_raddr = (int *) &Prfpregs.pr_fsr;
		ra->ra_type = r_normal;
		break;
	 default:
		if (reg >= Reg_F0  &&  reg <= Reg_F31) {
			ra->ra_type = r_floating;
			ra->ra_raddr = (int *)&Prfpregs.pr_fr.pr_regs[reg - Reg_F0];
		} else if (reg >= Reg_F32  &&  reg <= Reg_F63) {
			ra->ra_type = r_floating;
			ra->ra_raddr = (int *)&xregs.pr_un.pr_v8p.pr_xfr.pr_regs[reg - Reg_F32];
		}
#ifdef KADB
...
regs_to_core()
{
	...
	Prstatus.pr_reg[R_PSR] = (int)a->r_tstate;
	Prstatus.pr_reg[R_PC] = a->r_pc;
	Prstatus.pr_reg[R_nPC] = a->r_npc;
	Prstatus.pr_reg[R_Y] = a->r_y;
	Prstatus.pr_reg[R_G1] = (int)a->r_globals[0];
	Prstatus.pr_reg[R_G2] = (int)a->r_globals[1];
	Prstatus.pr_reg[R_G3] = (int)a->r_globals[2];
	Prstatus.pr_reg[R_G4] = (int)a->r_globals[3];
	Prstatus.pr_reg[R_G5] = (int)a->r_globals[4];
	Prstatus.pr_reg[R_G6] = (int)a->r_globals[5];
	Prstatus.pr_reg[R_G7] = (int)a->r_globals[6];

	/* ins of next win == outs of this one */
	win = ((a->r_cwp + 1) % NWINDOW);

	Prstatus.pr_reg[R_O0] = a->r_window[win].rw_in[0];
	Prstatus.pr_reg[R_O1] = a->r_window[win].rw_in[1];
	Prstatus.pr_reg[R_O2] = a->r_window[win].rw_in[2];
	Prstatus.pr_reg[R_O3] = a->r_window[win].rw_in[3];
	Prstatus.pr_reg[R_O4] = a->r_window[win].rw_in[4];
	Prstatus.pr_reg[R_O5] = a->r_window[win].rw_in[5];
	Prstatus.pr_reg[R_O6] = a->r_window[win].rw_in[6];
	Prstatus.pr_reg[R_O7] = a->r_window[win].rw_in[7];
	...
	db_printf(1, "Copy regs to core from adb_regs");
	Prstatus.pr_reg[R_PSR] = a->r_psr;
	Prstatus.pr_reg[R_PC] = a->r_pc;
	Prstatus.pr_reg[R_nPC] = a->r_npc;
	Prstatus.pr_reg[R_Y] = a->r_y;
	Prstatus.pr_reg[R_G1] = a->r_globals[0];
	Prstatus.pr_reg[R_G2] = a->r_globals[1];
	Prstatus.pr_reg[R_G3] = a->r_globals[2];
	Prstatus.pr_reg[R_G4] = a->r_globals[3];
	Prstatus.pr_reg[R_G5] = a->r_globals[4];
	Prstatus.pr_reg[R_G6] = a->r_globals[5];
	Prstatus.pr_reg[R_G7] = a->r_globals[6];
	Prstatus.pr_reg[R_O0] = a->r_window[0].rw_in[0];
	Prstatus.pr_reg[R_O1] = a->r_window[0].rw_in[1];
	Prstatus.pr_reg[R_O2] = a->r_window[0].rw_in[2];
	Prstatus.pr_reg[R_O3] = a->r_window[0].rw_in[3];
	Prstatus.pr_reg[R_O4] = a->r_window[0].rw_in[4];
	Prstatus.pr_reg[R_O5] = a->r_window[0].rw_in[5];
	Prstatus.pr_reg[R_O6] = a->r_window[0].rw_in[6];
	Prstatus.pr_reg[R_O7] = a->r_window[0].rw_in[7];
...
core_to_regs ()
{
	...
	db_printf(1, "Copy regs from Prstatus.pr_reg to adb_regs");
	if (v9flag) {
		register struct allregs_v9 *a = &adb_regs_v9;
		register prxregset_t *xp = (prxregset_t *)&xregs;

		a->r_tstate = xp->pr_un.pr_v8p.pr_tstate;
		a->r_pc  = Prstatus.pr_reg[R_PC];
		a->r_npc = Prstatus.pr_reg[R_nPC];
		a->r_y   = Prstatus.pr_reg[R_Y];
		for (reg = 0; reg <= 7; reg++) {
			a->r_outs[reg] =
			    MAKE_LL(xp->pr_un.pr_v8p.pr_xo[reg],
			    Prstatus.pr_reg[R_O0 + reg]);
			a->r_locals[reg] = Prstatus.pr_reg[R_L0 + reg];
			a->r_ins[reg] = Prstatus.pr_reg[R_I0 + reg];
			if (reg < 7)
				a->r_globals[reg] =
				    MAKE_LL(xp->pr_un.pr_v8p.pr_xg[reg],
				    Prstatus.pr_reg[R_G1 + reg]);
		}
	} else {
		register struct allregs *a = &adb_regs;

		a->r_psr = Prstatus.pr_reg[R_PSR];
		a->r_pc  = Prstatus.pr_reg[R_PC];
		a->r_npc = Prstatus.pr_reg[R_nPC];
		a->r_y   = Prstatus.pr_reg[R_Y];

		for (reg = 0; reg <= 7; reg++) {
			a->r_outs[reg] = Prstatus.pr_reg[R_O0 + reg];
	                a->r_locals[reg] = Prstatus.pr_reg[R_L0 + reg];
	                a->r_ins[reg] =	Prstatus.pr_reg[R_I0 + reg];
			if (reg < 7)
				a->r_globals[reg] = Prstatus.pr_reg[R_G1 + reg];
		}
	}
        db_printf(1, "Done copying regs from Prstatus.pr_reg to adb_regs");
} 
...
{        
        ...
        db_printf(1, "Copy regs to Prstatus.pr_reg from adb_regs\n" );
	if (v9flag) {
	        register struct allregs_v9 *a = &adb_regs_v9;
	        ...
	        Prstatus.pr_reg[R_G6] = a->r_globals[5];
	        Prstatus.pr_reg[R_G7] = a->r_globals[6];
	        Prstatus.pr_reg[R_O0] = a->r_outs[0];
	        Prstatus.pr_reg[R_O1] = a->r_outs[1];
	        Prstatus.pr_reg[R_O2] = a->r_outs[2];
	        Prstatus.pr_reg[R_O3] = a->r_outs[3];
	        Prstatus.pr_reg[R_O4] = a->r_outs[4];
	        Prstatus.pr_reg[R_O5] = a->r_outs[5];
	        Prstatus.pr_reg[R_O6] = a->r_outs[6];
	        Prstatus.pr_reg[R_O7] = a->r_outs[7];
	        for (reg = Reg_L0; reg <= Reg_L7; reg++){
	                put(Prstatus.pr_reg[R_O6] + FR_LREG(reg), SSP,
				a->r_locals[ reg - Reg_L0]);
	        }
	        for (reg = Reg_I0; reg <= Reg_I7; reg++){
	                put(Prstatus.pr_reg[R_O6] + FR_IREG(reg), SSP,
				a->r_ins[ reg - Reg_I0]);
	        }
	} else {	
	        register struct allregs *a = &adb_regs;
	...
	register int i;
{
	...
	ptrace(PTRACE_SETREGS, pid, &Prstatus.pr_reg, 0, 0);
...
{ extern int adb_debug;
    ...
    if( adb_debug  &&  spos->k_fp != Prstatus.pr_reg[R_SP] ) {
	printf( "stacktop -- fp %X != sp %X.\n", spos->k_fp,
		Prstatus.pr_reg[R_SP] );
    }
}
#endif DEBUG
...
{ extern int adb_debug;
     ...
     * it's now ok not to.
     */
    if( adb_debug  &&  (usp != Prstatus.pr_reg[R_SP] ||
	upc != Prstatus.pr_reg[R_PC] )) {
	printf( "is_leaf_proc -- usp or upc confusion:   usp %X upc %X\n",
	    usp, upc );
	printf( "\tPrstatus.pr_reg[R_SP %X, R_PC %X ).\n",
	    Prstatus.pr_reg[R_SP], Prstatus.pr_reg[R_PC] );
    }
}
#endif DEBUG
