 ...
 */
#ident  "@(#)disasm.c 1.24     95/10/19 SMI"
...
{"sub",  OP(2) + OP3(4) + RS1(0) + I(0),
	 ...
	 /* spoof SUB */
{"dec",  OP(2) + OP3(4) + I(1) + RS2(1), OPOP3MSK + IMSK + RS2MSK, itInc, V9_MODE},
	...
	FUNCPTR			prtAddress; /* Routine to print addresses */
{
	...
	if (checkLo(atAddress, bits, nBits) != 1) {
		 ...
		 *					(nBits & LOBITS)));
		 */
		 cp = dis_print(cp, "0x%x", ((bits & IMM22) << IMM22_SHIFT_CT));
	}
	return (cp);
}
