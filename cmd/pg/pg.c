...
#pragma ident	"@(#)pg.c	1.11	92/12/23 SMI"	/* SVr4.0 1.31	*/
...
static	int	fgetputc();
	int	eucscol(const unsigned char *);
...
static	int 	peekc;		/* pushback character for read functions */
...
ttyin()
{
	...
	sptr = cmdbuf;
	while (state != 10) {
		if ((length = _mbftowc(multic, &ch, readch, &peekc)) < 0 ||
		    !iswascii(ch) && !iswprint(ch)) {
			BEEP();
			continue;
		}
		if (ch == '\n' && !slash)
...
register char *pc;
{
	...
	case 3: /* recieved digit, waiting for the rest of the number */
		if (iswascii(c) && isdigit(c))
			return (1);
		if (iswascii(c) && strchr("h\014.wz\004dqQfl np$", c)) {
			pcommand = pc;
			if (nflag)
				*pstate = 10;
			else
...
register FILE *f;
{
	...
	for (column = 0, p = Line + savlength; ; ) {
		if ((length = _mbftowc(multic, &c, mrdchar, &peekc2)) == 0) {
			clearerr(f);
...
{
	if (count)
		promptlen += eucscol((const unsigned char *)m);
