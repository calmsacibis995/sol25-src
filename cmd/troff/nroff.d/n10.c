...
#ident	"@(#)n10.c	1.6	92/08/21 SMI"	/* SVr4.0 1.1	*/
...
#ifdef EUC
#ifdef NROFF
		if (euclocale && ((k & MBMASK) || (k & CSMASK))) {
			cnt = 0;
			while ((*q & MBMASK) && (cnt + 1 < (int)MB_CUR_MAX)) {
				cnt++;
				q++;
			}
			if ((cnt && !(*q & CSMASK)) || (*q & MBMASK)) {
				q -= cnt;
				cnt = 0;
				*q &= ~0xfe00;
			}
			k = cbits(i = *q);
			phyw = w = t.Char * eucsw[cs(i)];
		} else {
#endif /* NROFF */
#endif /* EUC */
