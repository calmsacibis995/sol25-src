...
#ident  "@(#)cron.c 1.25     94/08/26 SMI"
...
begin:
	...
	while (TRUE) {		/* MAIN LOOP */
		...
		/*
		 * Make sure that the user/group of the next_event is
		 * still ok.
		 */

		switch (userCred = verify_user_cred(next_event->u)) {
		case VUC_BADUSER:
			msg("No such user as %s(%d)",
			    (next_event->u)->name,
			    (next_event->u)->uid);
			break;

		case VUC_NOTINGROUP:
			msg("User %s is not a member of group %d",
			    (next_event->u)->name,
			    (next_event->u)->gid);
			break;
		...
		default:
			msg("Cron internal error: Bad VUC value: %d",
			    userCred);
			break;
		}

		if (userCred != VUC_OK) {
			msg("Removing user %s from cron/at event database",
			    next_event->u->name);
			clean_out_user(next_event->u);
			continue;
		}

		if (stat(next_event->u->home, &buf) == -1) {
			msg("can't stat home dir(%s) for user %s.",
			    next_event->u->home,
			    next_event->u->name);
			msg("cmd \"%s\" not executed", next_event->cmd);
			next_event = NULL;
			continue;
		}
...
time_t tflag;			/* fix for bugid 1104537	*/
{
	...
	if (tflag == 0) {		/* start fix for bugid 1104537	*/
		t = time((long *) 0);	/* original way of doing things	*/
	} else {
		t =  tflag;
...
struct event *e;
{
...

	{
		...
		if ((dhltime - e->time) < 0) {
			...
			delayed++;
			t_old = time((long *) 0); /* fix for 1106565	*/
			return (0);
		}
	}
	/*
	...
	if (e->etype == CRONEVENT) {
		...
		if (nexthr != tmnow->tm_hour || nextday != tmnow->tm_mday)
		    return (1);
	}
