...
#ident	"@(#)iconv.c	1.13	94/12/20 SMI"
	...
	char 		*cmd;
{
	...
	ileft = 0;
	while ((ileft += fread(from + ileft, 1, BUFSIZ - ileft, fp)) > 0) {
		fptr = from;
		while (1) {
			tptr = to;
			oleft = BUFSIZ;
...
Cannot access conversion table %s %d\n"), table, errno);
			exit(1);
		}
		process(t, fd, 0);
	} else {
...
search_dbase(o_file, o_table, d_data_base, f_data_base, this_table, fcode,
	tcode)
	char *o_file;
	char *o_table;
	char *d_data_base;
	...
	char *tcode;
{
	...
	for (row = 1; fgets(buff, MAXLINE, dbfp) != NULL; row++) {
		...
		if (buff[MAXLINE-2] != NULL) {
			fprintf(stderr, gettext("\
Database Error : row %d has more than %d characters\n"), row, MAXLINE-2);
			exit(1);
		}
		...
		if (fields < MINFLDS) {
			fprintf(stderr, gettext("\
Database Error : row %d cannot retrieve required %d fields\n"), row, MINFLDS);
			exit(1);
		}
