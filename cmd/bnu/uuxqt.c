...
#ident	"@(#)uuxqt.c	1.10	95/09/12 SMI"	/* from SVR4 bnu:uuxqt.c 2.12.1.12 */
...
#define BAD_FILE	2
#define USAGE	"Usage: %s [-x DEBUG] [-s SYSTEM]\n"
...
char	*user, *rmt, *file, *cmd, *buf, *errfile;
{
	...
	(void) sprintf(msg,
	    gettext("remote execution\t[uucp job %s (%s)]\n\t%s\n%s\n"),
	...
	mailst(ruser, gettext("remote execution status"), msg, file, errfile);
	return;
}
	...
	*incmd = *retaddr = *retuser = *Jobid = NULLCHAR;
	...
	while (fgets(buf, BUFSIZ, xfp) != NULL) {
		...
		case X_STDOUT:	/* standard output */
			    (void) sscanf(&buf[1], "%s%14s", fout, sysout);
		...
		case X_STDERR:	/* standard error */
			    (void) sscanf(&buf[1], "%s%14s", ferr, syserr);
	...
	if (retuser[0] != NULLCHAR)
	    (void) strcpy(user, retuser);	/* pick on this guy */
 ...
 */
	...
	if (strncmp(incmd, "uucp ", 5) == 0) {
		(void) sprintf(_Cmd, "%s %s UU_MACHINE=%s UU_USER=%s "
