...
#ident	"@(#)uudecode.c	1.13	95/03/02 SMI"	/* from SVR4 bnu:uudecode.c 1.2 */
/*
 * uudecode [-p] [input]
 *
...
decode(FILE *in, FILE *out)
{
	...
	for (line = 1; ; line++) {
		...
		/*
		 * The maximum # of octets to be encoded on each line is 45.
		 * The line is bogus if a greater number is indicated.
		 * If the character is out of range then it will be decoded
		 * as 0 and can be ignored.
		 */
		if ((n < 0) || (n > 45))
			continue;
