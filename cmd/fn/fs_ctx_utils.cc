...
#pragma ident	"@(#)fs_ctx_utils.cc	1.3	95/02/26 SMI"
...
static const FN_identifier
    addrtype_user((unsigned char *)"onc_fn_fs_user_nisplus");
...
static FN_ref *set_fs_binding(FNSP_Context *ctx, const FN_string *name,
...
static FN_ref *fs_binding_into_context(FNSP_Context *ctx,
...
static FN_ref *create_fs_context(FNSP_Context *ctx, const FN_string *name,
...
static void bind_fs_ref(FNSP_Context *ctx, const FN_string *name, FN_ref *ref);


// Unbind name in ctx.

static void unbind_fs_ref(FNSP_Context *ctx, const FN_string *name);


// Return the parent context of the given name.  Set parent_is_fs to true
// if this is a file system context.

static FNSP_Context *parent_ctx(const FN_composite_name *name,
    bool_t &parent_is_fs);


// If ref contains an address of type addrtype_mount, addrtype_user,
// or addrtype_host, replace it with addr if addr is not empty, or
// delete it if addr is empty.  Return TRUE if ref has been modified.
...
penultimate_ctx(FN_composite_name *name, bool_t &nsid)
{
	...
	FNSP_Context *ctx = FNSP_Context::from_ref(*ref, status);
	if (ctx == NULL) {
    ...
    const FN_composite_name *remaining)
{
	...
	for (; name != NULL; name = remaining->next(iter)) {
		...
		ctx = FNSP_Context::from_ref(*ref, status);
		if (ctx == NULL) {
...
void
update_namespace(FNSP_Context *ctx, Dir *input, bool_t nsid)
{
	FN_status status;
	...
	if (context) {
		FNSP_Context *subctx = FNSP_Context::from_ref(*ref, status);
		if (subctx == NULL) {
...
static FN_ref *
set_fs_binding(FNSP_Context *ctx, const FN_string *name, bool_t context,
...
static FN_ref *
fs_binding_into_context(FNSP_Context *ctx, const FN_string *name, FN_ref *ref)
{
	if (verbose >= 1) {
...
static FN_ref *
create_fs_context(FNSP_Context *ctx, const FN_string *name,
    const FN_ref_addr *addr)
{
	...
	FN_status status;
	FN_ref *ref = ctx->create_fnsp_subcontext(*name, FNSP_generic_context,
			status, FNSP_normal_repr, &reftype);
	if (ref == NULL) {
...
static void
bind_fs_ref(FNSP_Context *ctx, const FN_string *name, FN_ref *ref)
{
	if (ref->addrcount() == 0) {
...
static void
unbind_fs_ref(FNSP_Context *ctx, const FN_string *name)
{
	if (verbose >= 1) {
		if (name == &_fs_string) {
...
destroy_ref(const FN_composite_name *fullname, FN_ctx *parent, FN_ref *ref)
{
	...
	FN_status status;
	FN_ctx *ctx = FN_ctx::from_ref(*ref, status);
