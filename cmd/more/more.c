...
#ident	"@(#)more.c	1.22	95/02/27 SMI"	/* SVr4.0 1.6	*/
...
#define ret(val) retval=val;done++;break
	...
	case ctrl('B'):
		...
		if (clreol)
			cleareol ();
		printf (gettext("...back %d page"), nlines);
		if (nlines > 1)
			pr ("s\n");
		else
			pr ("\n");
        ...
        case 'f':
            ...
            if (clreol)
                cleareol ();
            printf (gettext("...skipping %d line"), nlines);
            if (nlines > 1)
                pr ("s\n");
            else
                pr ("\n");
...
colon(char *filename, int cmd, int nlines)
{
        ...
        case 'f':
                kill_line ();
                if (!no_intty)
                        promptlen = printf (gettext("\"%s\" line %d"),
					    fnames[fnum], Currline);
                else
                        promptlen = printf(
			 gettext("[Not a file] line %d"), Currline);
                fflush (stdout);
                return (-1);
        case 'n':
                if (nlines == 0) {
