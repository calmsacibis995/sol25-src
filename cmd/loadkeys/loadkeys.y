...
#ifndef lint
#ident	"@(#)loadkeys.y	1.4	93/11/22 SMI"
#endif
	...
	char **argv;
{
	...
	if (ioctl(kbdfd, KIOCTYPE, &type) < 0) {
		perror("loadkeys: ioctl(KIOCTYPE)");
		return (1);
	}
	...
	if (argc == 1 || (argc == 2 && doeeprom)) {
		 ...
		 * The "layout_N" tables are Type 4 only.
		 */
		if (type != KB_SUN4) {
			return (0);
		}
	...
	if ((infile = fopen(infilename, "r")) == NULL) {
		if (errno == ENOENT && *infilename != '/') {
			if ((int)(strlen(keytable_dir) +
				strlen(infilename) + 1) >= MAXPATHLEN) {
				(void) fprintf(stderr,
