...
#ident	"@(#)renice.c	1.7	95/02/21 SMI"	/* SVr4.0 1.3	*/
...
static int donice(int which, int who, int prio, int increment, char *who_s);
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (isdigit(argv[1][0])) {
		if (strtol(argv[1], (char **)NULL, 10) > PRIO_MAX) {
			argc--;			/* renice pid ... */
			argv++;
...
static int
donice(int which, int who, int prio, int increment, char *who_s)
{
	int oldprio;
	extern int errno;
