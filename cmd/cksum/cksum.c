...
#ident	"@(#)cksum.c	1.1	94/11/30 SMI"	/* MKS 1.23	*/
...
static	uchar_t	many;
static	char	asis[] = "%lu";	/* No leading zero padding format */
static	char	*fmt;		/* one of above strings */
static	CRC	filecrc;	/* CRC for this file */
static	CRC	totcrc;		/* Total of all files */
static	ulong	totbytes;	/* Total bytes read */
static	ulong	bsize = 1;	/* block size */

static	int	sum(const char *fn);
static	int	usage(void);
static	CRC	docrcposix(CRC crcval, uchar_t * bp, ulong n);
static	CRC	postprocess(CRC S, ulong n);

static	CRC	(*crcfunc)(CRC crcval, uchar_t * bp, ulong n) = docrcposix;
static void	m_crcposix(ulong *, const uchar_t *, size_t);
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (tflag) {
		printf(fmt, postprocess(totcrc, totbytes));
		printf(gettext("\t%lu\t   Total\n"),
			(totbytes + bsize - 1) / bsize);
	}
	return (exstat);
}
...
sum(const char *fn)
{
	...
	if (fn == NULL || (fn[0] == '-' && fn[1] == '\0'))
		...
		fprintf(stderr, gettext("cksum: input file \"%s\": "), fn);
		perror("");
		return (1);
	}
	filecrc = 0;
	...
	close(fd);
	totbytes += bytes;
	printf(fmt, postprocess(filecrc, bytes));
	printf("\t%lu", (bytes + bsize - 1) / bsize);
	if (many) {
		printf("\t%s", fd == 0 ? gettext("[standard input]") : fn);
	}
	if (n < 0)
		fputs(gettext(" [read error]"), stdout);
	putchar('\n');
	return (n == 0 ? 0 : 1);
}
 ...
 */
static	CRC
docrcposix(CRC crcval, uchar_t * bp, ulong n)
{
...
usage()
{
	fprintf(stderr, gettext("Usage: cksum [file ...]\n"));
	return (2);
}
