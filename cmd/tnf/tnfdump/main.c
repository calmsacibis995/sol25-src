...
#pragma ident	"@(#)main.c	1.8	95/07/31 SMI"
...
read_tnf_file(int raw, char *path)
{
	...
	if ((fd = open(path, O_RDONLY, 0777)) == -1)
		fail(1, gettext("cannot open %s"), path);
	if (fstat(fd, &st) != 0)
		fail(1, gettext("fstat error on %s"), path);
	if (st.st_size == 0) {
	...
	if ((p = mmap(NULL, st.st_size, PROT_READ, MAP_SHARED, fd, 0))
	    == (caddr_t)-1)
		fail(1, gettext("mmap error on %s"), path);
	...
	if ((err = tnf_reader_begin(p, st.st_size, &tnf)) != TNF_ERR_NONE)
		fail(0, gettext("error in %s: %s"), path,
...
describe_scalar(tnf_datum_t datum)
{
	...
	case TNF_K_INT8:
		...
		break;
	case TNF_K_UINT8:
		(void) printf("%u",		tnf_get_int8(datum));
		break;
	case TNF_K_INT16:
		(void) printf("%d",		tnf_get_int16(datum));
		break;
	case TNF_K_UINT16:
		(void) printf("%u",		tnf_get_int16(datum));
		break;
	case TNF_K_INT32:
	...
	case TNF_K_UINT64:
		/* lint not updated, it complains: malformed format string */
		(void) printf("%llu", 	tnf_get_int64(datum));
		break;
	case TNF_K_FLOAT32:
