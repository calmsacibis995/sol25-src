...
#ident	"@(#)ftpd.c	1.26	95/09/27 SMI"        /* SVr4.0 1.11  */
...
void *iad;                              /* Authentication descriptor */

char passbuf[256];                      /* save password for ia_conv */
	...
	char *passwd;
{
	...
	passbuf[sizeof(passbuf)-1] = '\0';
        ...
        if (ia_start("in.ftpd",pw->pw_name,ttyline,remotehost,&conv,&iad) !=
                                                                IA_SUCCESS) {
                exit(1);
        }
	...
	if (!guest) {		/* "ftp" is only account allowed no password */
                if ((error = ia_auth_user(iad, AU_PASSWD_REQ, &pwd, &out)) !=
                    IA_SUCCESS) {
                        reply(530, "Login incorrect."); /* XXX Redundant? */
			...
			seteuid(euid);
                ...
                if ((error = ia_auth_acctmg(iad, AU_PASSWD_REQ, &pwd, &out)) !=
                    IA_SUCCESS) {
			switch (error) {
			case IA_NEWTOK_REQD:
        ...
        /*
         * We have to write utmp first because ia_setcred changes our uid
         * and we can't access /etc/utmp
         */
        logwtmp(ttyline, pw->pw_name, remotehost);

                /* can not set effective UID until after chroot(2) */
        if (ia_setcred(iad,
            SC_INITGPS | SC_SETEGID, pw->pw_uid, pw->pw_gid, 0, NULL, &out)) {
                reply(530, "Login incorrect.");
                pw = NULL;
                goto bad;
        }
	...
	logged_in = 1;
        ...
        if (ia_setcred(iad,
            SC_SETEUID, pw->pw_uid, pw->pw_gid, 0, NULL, &out)) {
                reply(530, "Login incorrect.");
                pw = NULL;
                goto bad;
        }
	...
	FILE *instr, *outstr;
{
	...
	case TYPE_I:
	case TYPE_L:
		while ((rcnt = read(fileno(instr), buf, sizeof buf)) > 0) {
			if ((wcnt = write(fileno(outstr), buf, rcnt)) != rcnt) {
				transflag = 0;
				return (1);
			}
		}
		if ((rcnt == 0) && (fsync(fileno(outstr)) == -1)) {
			transflag = 0;
			return (1);
		}
		transflag = 0;
	...
	int status;
{
	if (logged_in) {
		(void) seteuid(0);
		(void) ia_start ("in.ftpd", NULL, ttyline, NULL, NULL, &iad);
		logwtmp(ttyline, "", "");
		ia_end(iad);
	}
...
static int
ftpd_conv(conv_id, num_msg, msg, response, appdata_ptr)
	int conv_id;
	int num_msg;
	...
	void *appdata_ptr;
{
	...
	*response = (struct ia_response *)calloc(num_msg,
	 ...
	 */
	if (m->msg_style == IA_PROMPT_ECHO_OFF) {
		if ( passbuf[0] != '\0') {
			r->resp = malloc(strlen(passbuf)+1);
			if (r->resp == NULL) {
				free_resp(num_msg, *response);
				*response = NULL;
				return (IA_CONV_FAILURE);
			}
			strcpy(r->resp, passbuf);
			r->resp_len  = strlen(passbuf);
		} else
			reply(530, "Login incorrect.");
