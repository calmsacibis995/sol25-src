...
#ident	"@(#)input.c	1.8	92/09/25 SMI"	/* SVr4.0 1.1	*/
	...
	case RIPCMD_REQUEST:		/* broadcasted request */
		...
		while (size > 0) {
			...
			if (msg->rip_vers > 0) {
				...
				n->rip_metric = htonl((u_long)n->rip_metric);
			}
...
fprintf(ftrace,"route in unsupported address family (%d), from %s (af %d)\n",
	...
	case RIPCMD_POLLENTRY:
		...
		if (rt) {	/* don't bother to check rip_vers */
			...
			e->rtu_flags = ntohs((u_short)rt->rt_flags);
			e->rtu_state = ntohs((u_short)rt->rt_state);
			e->rtu_timer = ntohl((u_long)rt->rt_timer);
			if (rt->rt_state & RTS_INTERFACE)
				e->rtu_metric = ntohl((u_long)rt->rt_metric - 
						      IFMETRIC(rt->rt_ifp));
			else 
				e->rtu_metric = ntohl((u_long)rt->rt_metric);
			ifp = rt->rt_ifp;
			if (ifp) {
				e->int_flags = ntohl((u_long)ifp->int_flags);
