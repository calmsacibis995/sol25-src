...
#ident	"@(#)main.c	1.10	92/08/14 SMI"	/* SVr4.0 1.5	*/
...
int	save_space = 0;		/* If one and not a supplier we install 
				   default route only */
int	deamon = 1;		/* Fork off a detached deamon */
	...
	char *argv[];
{
	...
	(void) signal(SIGHUP, ifinit);
	(void) signal(SIGTERM, hup);
#ifdef DEBUG
	(void) signal(SIGUSR1, if_dump);
	(void) signal(SIGUSR2, rtdump);
#endif
#ifdef SYSV
	srand((u_int)getpid());
#else
