...
#ident	"@(#)tables.c 1.13	93/09/08 SMI"	/* SVr4.0 1.2	*/
...
void
log_change(level, orig, new)
	int level;
	struct rt_entry *orig, *new;
{
 ...
 */
struct rt_entry *
rtlookup2(dst, router)
	struct sockaddr *dst, *router;
{
	register struct rt_entry *rt;
	register struct rthash *rh;
	register u_int hash;
	...
	if (install && rt->rt_state != RTS_INTERNAL) {
		if (doioctldelete) {
			if (ioctl(iosoc, SIOCADDRT, (char *)&rt->rt_rt) < 0)
				if (errno != EEXIST) {
					syslog(LOG_ERR, "rtchange SIOCADDRT: %m");
					log_change(LOG_ERR, rt,
						(struct rt_entry *)&oldroute);
				}
			if (ioctl(iosoc, SIOCDELRT, (char *)&oldroute) < 0) {
