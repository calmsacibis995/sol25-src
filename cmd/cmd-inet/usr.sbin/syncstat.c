...
#ident  "@(#)syncstat.c 1.6     92/09/11 SMI"
...
char errstr[128] = "syncstat: /dev/";
char *ifdevice = errstr + 10;
char *ifname = errstr + 15;
int fd;
	...
	char **argv;
{
	...
	*cp = '\0';	/* drop number, leaving name of clone device. */
	...
	dlattachreq(fd, ppa);
	dlokack(fd, buf);
	printf("syncstat: control device: %s, ppa=%d\n", ifdevice, ppa);
	...
	if (ioctl(fd, I_STR, &sioc) < 0) {
		perror("S_IOCGETMODE");
		fprintf(stderr, "syncstat: can't get sync mode info for %s\n",
			ifname);
		exit(1);
	}
	if (do_clear) {
		...
		if (ioctl(fd, I_STR, &sioc) < 0) {
			...
			fprintf(stderr, "syncstat: can't clear stats for %s\n",
				ifname);
			exit(1);
		}
	}
	...
	if (ioctl(fd, I_STR, &sioc) < 0) {
		...
		fprintf(stderr, "syncstat: can't get stats for %s\n",
			ifname);
		exit(1);
	}
	if (period) {
		if (sm.sm_baudrate == 0) {
			fprintf(stderr, "syncstat: baud rate not set\n");
			exit(1);
		}
		for (count = 0; ; count++) {
			fflush(stdout);
...
"    ipkts   opkts  undrun  ovrrun   abort     crc   iutil   outil\n");
	printf(" %7d %7d %7d %7d %7d %7d %6d%% %6d%%\n",
		st.ipack,
		st.opack,
		st.underrun,
		st.overrun,
