...
#ident	"@(#)db_save.c	1.6	93/04/16 SMI"	/* SVr4.0 1.2 */
	...
	register struct hashbuf *oldhtp;
{
	...
	if (oldhtp == NULL)
		...
		for (n = 0; newsize = hashsizes[n++]; /*EMPTY*/)
	...
	htp->h_size = newsize;
#ifdef SYSV
	memset((void *) htp->h_tab, 0, newsize * sizeof (struct hashbuf *));
#else
...
saveinv()
{
	...
	ip->i_next = NULL;
#ifdef SYSV
	memset((void *)ip->i_dname, 0, sizeof (ip->i_dname));
#else
