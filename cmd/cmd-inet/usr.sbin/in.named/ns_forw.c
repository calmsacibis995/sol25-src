...
#ident	"@(#)ns_forw.c	1.6	93/04/18 SMI"	/* SVr4.0 1.1 */
	...
	struct qinfo **qpp;
{
	...
	for (qp = qhead; qp != QINFO_NULL; qp = qp->q_link) {
		if (qp->q_id == id &&
#ifdef SYSV
		memcmp((void *)&qp->q_from, fp, sizeof (qp->q_from)) == 0 &&
#else
		bcmp((char *)&qp->q_from, fp, sizeof (qp->q_from)) == 0 &&
#endif
		((qp->q_cmsglen == 0 && qp->q_msglen == msglen &&
#ifdef SYSV
		memcmp((void *)(qp->q_msg+2), msg+2, msglen-2) == 0) ||
#else
		bcmp((char *)qp->q_msg+2, msg+2, msglen-2) == 0) ||
#endif
		(qp->q_cmsglen == msglen &&
#ifdef SYSV
		memcmp((void *)(qp->q_cmsg+2), msg+2, msglen-2) == 0))) {
#else
...
found:
	...
	hp->rd = 1;	/* leave set to 0 for dup detection */
#ifdef STATS
	stats[S_OUTPKTS].cnt++;
#endif
	unsched(qp);
