...
#ident	"@(#)ns_req.c	1.8	95/04/07 SMI"	/* SVr4.0 1.3 */
	...
	int dfd;
{
	...
	extern void sq_done(), sort_response();
...
try_again:
		 ...
		 */
#ifdef SYSV
		if (np == NULL && localdomain && strchr(dname, (int)'.') ==
									NULL) {
#else
...
#ifdef DEBUG
		if (debug >= 3) {
		    fprintf(ddt, "req: foundname = %d count = %d ", foundname,
									count);
...
fetchns:
		...
		if (cname) {
			...
			hp->ancount = htons(hp->ancount);
#ifdef SYSV
			memcpy(omsg, msg, omsglen = msglen);
#else
	...
	case IQUERY: {
...
#ifdef SYSV
		memcpy(anbuf, fname, alen = (char *)cp - fname);
#else
		...
		for (ip = invtab[dhash(data, dlen)]; ip != NULL;
			...
			for (dp = np->n_data; dp != NULL; dp = dp->d_next) {
				...
				if (dp->d_size != dlen ||
#ifdef SYSV
				    (memcmp(data, dp->d_data, dlen) ? 1 : 0))
#else
...
#ifdef SYSV
			memcpy(cp, cp1,
				(size_t)(n = dp->d_size - (cp1 - dp->d_data)));
#else
			bcopy(cp1, cp,
