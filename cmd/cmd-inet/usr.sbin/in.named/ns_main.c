...
#ident	"@(#)ns_main.c	1.12	94/05/25 SMI"	/* SVr4.0 1.4 */
...
static	void	usage(), opensocket(), setdebug(), sq_query();

void	getnetconf(), sqrm(), gettime();
#ifdef DEBUG
void	printnetinfo();
#endif
	...
	char *argv[], *envp[];
{
...
#else
	extern SIG_FN maint_alarm(), endxfer();
#endif
	...
	extern char Version[];
	extern void doachkpt(), doadump(), db_reload(), retry(),
			ns_init(), ns_maint(), ns_req(), prime_cache(),
			buildservicelist(), buildprotolist();
#ifdef STATS
	extern void ns_stats();
#endif STATS
	...
	*/
	...
	(void) setsockopt(vs, SOL_SOCKET, SO_REUSEADDR, (char *)&on,
								sizeof (on));
	...
	for (;;) {
		...
		*/
		if (FD_ISSET(vs, &tmpmask)) {
			from_len = sizeof (from_addr);
			rfd = accept(vs, (struct sockaddr *)&from_addr,
								&from_len);
			gettime(&tt);
...
#ifdef SIOCGIFNUM
	if (ioctl(vs, SIOCGIFNUM, (char *)&numifs) < 0)
#endif
		numifs = MAXIFS;
