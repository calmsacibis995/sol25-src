...
#ident	"@(#)db_dump.c	1.7	93/12/20 SMI"	/* SVr4.0 1.1 */
	...
	while (npp < nppend) {
		for (np = *npp++; np != NULL; np = np->n_next) {
			...
			for (dp = np->n_data; dp != NULL; dp = dp->d_next) {
				...
				case T_UNSPEC:
					/*
					 * Dump binary data out in an
					 * ASCII-encoded format
					*/
					{
						/*
						 * Allocate more than enough
						 * space:  actually need 5/4
						 * size + 20 or so
						 */
						int TmpSize = 2 * dp->d_size +
									30;
						char *TmpBuf = (char *)
								malloc(TmpSize);
						if (TmpBuf == NULL) {
