...
#ident	"@(#)db_update.c	1.6	93/04/16 SMI"	/* SVr4.0 1.1 */
	...
	struct hashbuf *htp;
{
	    ...
	    (odp->d_type == T_A)))
	{
		...
		if (db_update(name, dp, dp, (flags|DB_NOHINTS), fcachetab) !=
									OK) {
#ifdef DEBUG
			if (debug > 2)
...

{
...
#ifdef SYSV
		return ((memcmp(dp2->d_data, dp1->d_data, dp1->d_size)?1:0));
#else
	...
	case T_MINFO:
		...
		cp2 += strlen(cp2) + 1;
#ifdef SYSV
		return ((memcmp(cp2, cp1, sizeof (u_long) * 5) ? 1 : 0));
#else
