...
#ident	"@(#)ns_resp.c	1.8	95/04/07 SMI"	/* SVr4.0 1.1 */
...
extern	int priming;
extern void schedretry(), unsched(), qremove(), qfree();

struct qinfo *sysquery();

static void check_root(), check_ns();
#ifdef notdef
static void prime();
#endif
...
#ifdef SYSV
		if (memcmp((void *)&fwd->fwdaddr.sin_addr, &from_addr.sin_addr,
...
#ifdef SYSV
			if (memcmp((void *)&qs->ns_addr.sin_addr,
	...
	int  msglen, zone, flags;
{
	...
	case T_SOA:
		...
		if (type == T_SOA) {
#ifdef SYSV
			memcpy(cp1, cp, n = 5 * sizeof (u_long));
#else
	...
	case UPDATEDA:
		 ...
		 */
		if ((n = dn_expand(msg, msg+msglen, cp, dname,
							sizeof (dname))) < 0) {
#ifdef DEBUG
			if (debug)
	...
	/*
	 * Bogon check from SunOS 4.1.1 source code
	 * we don't want to cache a wildcard
	 */
...
#ifdef SYSV
	    memcmp((void *)(oqp->q_msg+2), qp->q_msg+2, qp->q_msglen-2) == 0) {
#else
