...
#ident	"@(#)ns_init.c	1.7	93/04/18 SMI"	/* SVr4.0 1.1 */
...
extern  void endline(), get_sort_list(), gettime(), remove_zone();
static void zoneinit(), get_forwarders(), free_forwarders();
#ifdef DEBUG
extern  void printzoneinfo();
static  void content_zone();
#endif
	...
	char *bootfile;
{
	...
	while (!feof(fp) && !ferror(fp)) {
		 ...
		 */
		if (!getword(buf, sizeof (buf), fp)) {
			syslog(LOG_ERR, "%s: line %d: missing source or addr\n",
						bootfile, lineno);
			continue;
		}
...
#endif DEBUG
				/*
				 * Realloc() not used since it might damage
				 * zones if an error occurs
				 */
			    if ((zp = (struct zoneinfo *)
				malloc((64 + nzones) *
					sizeof (struct zoneinfo))) == NULL) {
...
#ifdef SYSV
			    memcpy((void *)zp, (void *)zones,
					    nzones * sizeof (struct zoneinfo));
			    memset((void *)&zp[nzones], 0,
...
#ifdef DEBUG
		if (debug)
			fprintf(ddt, "zone[%d] type %d: '%s'", zp-zones, type,
