...
#ident	"@(#)in.telnetd.c	1.17	95/10/23 SMI"	/* SVr4.0 1.7	*/
 ...
 * publication.
 *
 * 	(c) 1986 to 1994  Sun Microsystems, Inc
...
gotpty:
	...
	if (ioctl(f, I_POP, "sockmod") < 0)
		fatalperror(f, "ioctl I_POP sockmod", errno);
...
rmut()
{
	...
	if ((ia_start("in.telnetd", ".telnet", NULL, NULL, NULL, &iah)) !=
	    IA_SUCCESS) {
		fprintf(stderr, "in.telnetd: ia_start failure");
		_exit(1);
	}
	 ...
	 */
	(void) ia_close_session(iah, 0, pid, 0, NULL, &ia_status);
