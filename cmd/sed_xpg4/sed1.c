...
int	seekto = 1;	/* to calculate where to seek to at QUIT command */
long    lnum;
char    *spend;
int	nflag;
long    tlno[NLINES];
		...
		case ACOM:
			*aptr++ = ipc;
			if (aptr >= &abuf[ABUFSIZE]) {
				(void) fprintf(stderr,
		gettext("sed: Too many appends after line %ld\n"), lnum);
				aptr--;
			}
			*aptr = 0;
			break;
	...
	cpcom:
		...
		case RCOM:
			...
			*aptr++ = ipc;
			if (aptr >= &abuf[ABUFSIZE]) {
				(void) fprintf(stderr,
		gettext("sed: Too  many reads after line %ld.\n"),
					lnum);
				aptr--;
			}
			*aptr = 0;
