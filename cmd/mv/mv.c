...
#pragma ident	"@(#)mv.c	1.41	95/09/22 SMI"
...
copy:
			...
			if (ISREG(s1)) {
				/*
				 * Determine size of initial mapping.  This
				 * will determine the size of the address
				 * space chunk we work with.  This initial
				 * mapping size will be used to perform
				 * munmap() in the future.
				 */
				mapsize = MAXMAPSIZE;
			...
			if (mapsize != 0) {
				...
				for (;;) {
					...
					/*
					 * if we write less than the mmaped
					 * size it's due to a media error
					 * on the input file.
					 */
					if ((nbytes >= 0) &&
					    (nbytes != (int)mapsize)) {
					...
					if (nbytes < 0) {
					/*
					 * although the write manual page
					 * doesn't specify this as a possible
					 * errno, it is set when the nfs read
					 * via the mmap'ed file is accessed,
					 * so report the problem as a source
					 * access problem, not a target file
					 * problem
					 */
						if (errno == EACCES)
							Perror(source);
...
copydir(char *source, char *target)
{
	...
	if (fixmode != (mode_t)0) {
		 ...
		 */
		if (pflg) {
			if ((ret = chg_mode(target, UID(s1save), GID(s1save),
			    FMODE(s1save))) == 0)
				ret = chg_time(target, s1save);
		} else
