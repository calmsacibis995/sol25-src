...
#ident "@(#)pcontrol.h	1.13	94/01/02 SMI"	/* SVr4.0 1.2	*/
...
/*
 * If we are compiling with ANSI-C, define CONST to be const,
 * else define it to be empty.  This relies on this header file
 * being the first (local) one included in all the .c files.
 */
#if	defined(__STDC__)
#	define	CONST	const
...
extern CONST char * CONST regname[];	/* register name strings */
...
#define	SYSCALL	((syscall_t)0xd900)	/* syscall (OST) instruction */
#define	FRMSZ	13			/* frame size, in words */
#endif

#if u3b2 || u3b15
typedef	unsigned short	syscall_t;	/* holds a syscall instruction */
#define	SYSCALL	((syscall_t)0x3061)	/* syscall (GATE) instruction */
...
#if mc68k
typedef	unsigned short	syscall_t;	/* holds a syscall instruction */
#define	SYSCALL	((syscall_t)0x4e40)	/* syscall (trap 0) instruction */
...
#define	OSYSCALL ((syscall_t)0x91d02000)/* old syscall (ta 0) instruction */
#define	SYSCALL	((syscall_t)0x91d02008)	/* syscall (ta 8) instruction */
#define	ERRBIT	PSR_C		/* bit in psr indicating syscall error */
#endif
