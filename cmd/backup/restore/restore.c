...
#ifndef lint
static char sccsid[] =
	"@(#)restore.c 1.11 90/11/12 SMI"; /* from UCB 5.4 5/22/88 */
#endif not lint
...
#define	MODECHG	0x8	/* mode of inode changed */
	...
	if (np != NIL) {
		...
		if (ip == NULL)
			panic(gettext("corrupted symbol table\n"));
		if (ip != np)
			lookuptype = LINK;
	}
	/*
	...
	char *symtabfile;
{
	...
	while (curfile.ino < maxino) {
		...
		if (ep == NIL)
			panic(gettext("unknown file on volume\n"));
	...
	int type;
{
	...
	for (; np != NIL; np = np->e_links)
		if (np == ep)
			break;
	if (np == NIL)
		panic(gettext("missing inumber %d\n"), ino);
