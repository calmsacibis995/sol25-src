...
#pragma ident	"@(#)server.c	1.13	95/10/16 SMI"
...
server_create()
{
	...
	mutex_unlock(&create_lock);
	thr_create(NULL, 0, server_tsd_bind, NULL, THR_BOUND, NULL);
}
...
main(int argc, char ** argv)
{
	...
	else if(client_getadmin(&current_admin)) {
		fprintf(stderr, "Cannot contact nscd properly(?)\n");
		exit(1);
	}

	while ((opt = getopt(argc, argv, "Kf:c:ge:p:n:i:l:d:s:h:o:")) != EOF) {
		nsc_stat_t * cache;
		char * cacheopt;
	...
	if (current_admin.debug_level) {
		...
		if(strlen(current_admin.logfile) == 0) {
			nscd_set_lf(&current_admin, NULL);  /* default no log file */
	        }
		else {
		    nscd_set_lf(&current_admin, current_admin.logfile);  
	        }
	      ...
	      int n_desc)
{
	...
	door_return(&u.data,
		    u.data.nsc_ret.nsc_bufferbytesused,
		    u.data.nsc_ret.nsc_bufferbytesused,
		    0);
}
...
nscd_set_lf(admin_t * ptr, char * s)
{
	...
	if (s == NULL) {
		...
		if ((newlogfd = open(s, O_WRONLY|O_CREAT|O_APPEND,-1)) < 0) {
