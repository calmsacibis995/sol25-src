...
#pragma ident "@(#)subr.c   1.12     95/03/03 SMI"
/*
...
cachefs_resource_size(int maxinodes, struct cachefs_rinfo *rinfop)
{
	...
	fsize += maxinodes * sizeof (struct lru_pointers);
	fsize = ((fsize + MAXBSIZE - 1) / MAXBSIZE) * MAXBSIZE;

	rinfop->r_ptrsize = fsize - rinfop->r_ptroffset;
	rinfop->r_identoffset = fsize;

	fsize += maxinodes * sizeof (struct lru_idents);
	fsize = ((fsize + MAXBSIZE - 1) / MAXBSIZE) * MAXBSIZE;

	rinfop->r_identsize = fsize - rinfop->r_identoffset;
 ...
 *	lockid		lockid if we already have lock (and lockfile
 *			and cachedir already exist), or -1.
 * Returns:
...
cachefs_create_cache(char *dirp, struct cachefs_user_values *uv,
    struct cache_label *clabel, int lockid)
{
	int xx;
	...
	/* make the networker "don't back up" file */
	xx = 0;
	...
	cnt += (clabel->cl_maxinodes * sizeof (struct lru_idents) +
	    MAXBSIZE - 1) / MAXBSIZE;
	cnt += (clabel->cl_maxinodes * sizeof (struct lru_pointers) +
	    MAXBSIZE - 1) / MAXBSIZE;
 ...
 *	keeplock keep the lock file (and the directory itself)
 * Returns:
...
int
cachefs_delete_all_cache(char *dirp, int keeplock)
{
	DIR *dp;
	struct dirent *dep;
	int xx;
	...
	if (xx == -1) {
		...
		xx = nftw(path, cachefs_delete_file, 16,
...
int
cachefs_delete_file(const char *namep, const struct stat *statp, int flg,
    struct FTW *ftwp)
{
	/* ignore . and .. */
    ...
    struct cache_label *clp, const char *dirp)
{
	...
	clp->cl_maxblks = uvp->uv_maxblocks / 100.0 * fs.f_blocks * ftmp + .5;
	clp->cl_blockmin = uvp->uv_minblocks / 100.0 * fs.f_blocks * ftmp + .5;
	clp->cl_blocktresh = uvp->uv_threshblocks / 100.0 * fs.f_blocks *
	    ftmp + .5;
	clp->cl_maxinodes = uvp->uv_maxfiles / 100.0 * fs.f_files + .5;
	clp->cl_filemin = uvp->uv_minfiles / 100.0 * fs.f_files + .5;
	clp->cl_filetresh = uvp->uv_threshfiles / 100.0 * fs.f_files + .5;
...
#define	BOUND(yy) \
	...
	xx = clp->cl_maxblks * ftmp / fs.f_blocks * 100. + .5;
	BOUND(xx);
	uvp->uv_maxblocks = xx;

	xx = clp->cl_blockmin * ftmp / fs.f_blocks * 100. + .5;
	BOUND(xx);
	uvp->uv_minblocks = xx;

	xx = clp->cl_blocktresh * ftmp / fs.f_blocks * 100. + .5;
	BOUND(xx);
	uvp->uv_threshblocks = xx;

	xx = ((double)clp->cl_maxinodes / fs.f_files) * 100. + .5;
	BOUND(xx);
	uvp->uv_maxfiles = xx;

	xx = ((double)clp->cl_filemin / fs.f_files) * 100. + .5;
	BOUND(xx);
	uvp->uv_minfiles = xx;

	xx = ((double)clp->cl_filetresh / fs.f_files) * 100. + .5;
	BOUND(xx);
	uvp->uv_threshfiles = xx;
