...
#pragma ident "@(#)stats_dbm.c   1.9     94/11/11 SMI"
...
stats_dbm_open(stats_cookie_t *st)
{
	...
	(void) sprintf(st->st_dbm_name, "%s/%s-%d",
...
stats_dbm_rm(stats_cookie_t *st)
{
	...
	(void) strcpy(buffy, st->st_dbm_name);
	eobase = buffy + strlen(buffy);

	(void) strcpy(eobase, ".dir");
	(void) unlink(buffy);

	(void) strcpy(eobase, ".pag");
	(void) unlink(buffy);
}
...
fid_info *
stats_dbm_fetch_byfid(stats_cookie_t *st, fid_t *fidp)
{
	datum key, value;
	fid_info *rc;
...
void
stats_dbm_store_byfid(stats_cookie_t *st, fid_t *fidp, fid_info *fi)
{
	datum key, value;
    ...
    u_long fileno, u_int count)
{
	...
	(void) sprintf(keystring, "%s.%d", cacheid,
	    fileno / mi->mi_filegrp_size);
	fileno %= mi->mi_filegrp_size;
	...
	if ((fileno / NBBY) < sizeof (fgp->fg_bits)) {
		tbits = 1 << (fileno % NBBY);
		if (! (fgp->fg_bits[fileno / NBBY] & tbits))
			fgp->fg_bcount++;
		fgp->fg_bits[fileno / NBBY] |= tbits;
	}
