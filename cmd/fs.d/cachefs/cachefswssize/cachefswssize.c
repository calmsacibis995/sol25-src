...
#pragma ident "@(#)cachefswssize.c   1.12     94/11/11 SMI"
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	for (key = stats_dbm_firstkey(sc);
		...
		printf("\t       end size: %6dk\n", mip->mi_current / 1024);
		printf("\thigh water size: %6dk\n", mip->mi_high / 1024);
		free(mip);
	}

	printf(gettext("\n    total for cache\n"));
	printf(gettext("\t   initial size: %6dk\n"),
	    stats_log_wssize_init(sc) * lh->lh_maxbsize / 1024);
	printf(gettext("\t       end size: %6dk\n"),
	    stats_log_wssize_current(sc) / 1024);
	printf(gettext("\thigh water size: %6dk\n"),
	    stats_log_wssize_high(sc) / 1024);
