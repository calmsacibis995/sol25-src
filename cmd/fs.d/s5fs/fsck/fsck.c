...
#define	BSIZE  2048
#define	INOPB  32
#define	ININDIR  (BSIZE/ sizeof(daddr_t))
#define BSHIFT 11
#define BMASK  03777
#define INOSHIFT 5
...
struct bufarea {
	...
	union {
		...
		daddr_t	b_indir[ININDIR];	/* indirect block */
...
char    *use = "s5fs Usage:\nfsck [-F s5] [generic_options] [special ...]\nfsck [-F s5fs] [generic_options] [-y] [-n] [-p] [-sX] [-SX] [-t file] [-l] [-q] [-D] [-f] [special ...]\n";
...
char	*argv[];
{
	...
	if (sanity_only) {
		if ((argc - 3) > atoi(&sanity_only)) {
			error("s5fs fsck: Incompatible options: cannot use -m with other options\n");
			errexit(use);
		}
	}
	if(sflag && csflag)
 ...
 */
	...
	||  (superblk.s_type < 1))
		errexit ("s5fs fsck: %s is not an s5fs file system\n",argv[optind]);
...
char	*filename;
{
	...
	if (stat(filename, &stbd) < 0) {
		fprintf(stderr, "s5fs fsck: sanity check failed : cannot stat %s\n", filename);
		exit(34);
	}
	stat("/", &stbr);
	if (stbr.st_dev == stbd.st_rdev) {	/* root file system */
		if (superblk.s_state != FsACTIVE) {
			fprintf(stderr, "s5fs fsck: sanity check: root file system needs checking\n");
			exit(32);
		} else {
			fprintf(stderr, "s5fs fsck: sanity check: root file system okay\n");
			exit(0);
		}
	}
	if (ustat(stbd.st_rdev, &usb) == 0) {
		fprintf(stderr, "s5fs fsck: sanity check: %s already mounted\n", filename);
		exit(33);
	}

	if ((superblk.s_state + (long)superblk.s_time) != FsOKAY) {
		fprintf(stderr, "s5fs fsck: sanity check: %s needs checking\n", filename);
		exit(32);
	}
	fprintf(stderr, "s5fs fsck: sanity check: %s okay\n", filename);
	exit(0);
}
...
char *dev;
{
	...
	sprintf(ff_buf, "/usr/sbin/ff -F s5fs -I -i");
