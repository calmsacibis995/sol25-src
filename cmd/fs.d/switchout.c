...
#ident	"@(#)switchout.c	1.11	92/10/23 SMI"	/* SVr4.0 1.19	*/
...
/*
 * note regarding localization: the usage strings in the c_usgstr of the
 * following structures should be given a translation; the call to gettext
 * is in the usage() function.
 */
...
struct commands {
	...
	"mkfs", "F:o:mb:?V", 
		"[-F FSType] [-V] [-m] [-o specific_options] special [operands]",
	"dcopy", "F:o:s:a:f:?dvV", 
		"[-F FSType] [-V] [-o specific_options] inputfs outputfs",
	"fsdb", "F:o:z:?V", 
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	if (argc == 1) {
		for (c_ptr = cmd_data; ((c_ptr->c_basename != NULL) && 
		    (strcmp(c_ptr->c_basename, basename) != 0)); c_ptr++) 
		;
