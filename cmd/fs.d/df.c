...
/*
 * XXX: the code has been almost completely rewritten; only trivial
 *	functions (like xmalloc) remain the same. Should the above
 *	copyright be changed ?
 */

#ident	"@(#)df.c	1.35	95/08/10 SMI"	/* SVr4.0 1.34	*/
...
typedef void (*output_func)(struct df_request *, struct statvfs *);
...
#else
	"Usage: %s [-F FSType] [-abegklntV] [-o FSType-specific_options]"
		" [directory | block_device | resource]", program_name);
#endif
	exit(1);
	/* NOTREACHED */
...
#else
	while ((arg = getopt(argc, argv, "F:o:abekVtgnl")) != EOF) {
#endif
		if (arg == 'F') {
			if (F_option)
...
/*
 * Convert an unsigned long to a string representation and place the
 ...
 * must be powers of 2.
 * The value "(unsigned long)-1" is a special case and is always
 * converted to "-1".
...
number_to_string(
			...
			unsigned long number,	/* convert this number */
			int unit_from,		/* from units of this size */
			int unit_to)		/* to units of this size */
{
	if ((long)number == (long)-1)
		(void) strcpy(buf, "-1");
	else {
		if (unit_from == unit_to)
			(void) sprintf(buf, "%lu", number);
		else if (unit_from < unit_to)
			(void) sprintf(buf, "%lu",
				number / (unit_to / unit_from));
		else
			(void) sprintf(buf, "%lu",
 ...
 */
static void
g_output(struct df_request *dfrp, struct statvfs *fsp)
{
	unsigned long	available_blocks	= fsp->f_bavail;
...
#define	SCOL4_WIDTH	(int) MAX(frag_size_len, total_files_len)
	...
	(void) printf("%*s %-*s %#*.*lx %-*s %*s %-*s\n\n",
		...
		NCOL3_WIDTH, number_to_string(fname_buf, fsp->f_namemax, 1, 1),
			SCOL3_WIDTH, fname_str);
}


static void
k_output(struct df_request *dfrp, struct statvfs *fsp)
{
	unsigned long	total_blocks		= fsp->f_blocks;
	unsigned long	free_blocks		= fsp->f_bfree;
	unsigned long	available_blocks	= fsp->f_bavail;
	 ...
	 * number of blocks.
	 */
	if (free_blocks == (unsigned long)-1) {
		used_blocks = (unsigned long)-1;
		(void) strcpy(capacity_buf, "  100%");
	} else {
		unsigned long reserved_blocks = free_blocks - available_blocks;
...
static void
t_output(struct df_request *dfrp, struct statvfs *fsp)
{
	numbuf_t	total_blocks_buf;
	numbuf_t	total_files_buf;
	numbuf_t	free_blocks_buf;
...
static void
eb_output(struct df_request *dfrp, struct statvfs *fsp)
{
	numbuf_t free_files_buf;
...
static void
e_output(struct df_request *dfrp, struct statvfs *fsp)
{
	numbuf_t free_files_buf;
...
static void
b_output(struct df_request *dfrp, struct statvfs *fsp)
{
	numbuf_t free_blocks_buf;
...
/* ARGSUSED */
static void
n_output(struct df_request *dfrp, struct statvfs *fsp)
{
	(void) printf("%-*s: %-*s\n",
		MOUNT_POINT_WIDTH, DFR_MOUNT_POINT(dfrp),
...
static void
default_output(struct df_request *dfrp, struct statvfs *fsp)
{
	numbuf_t free_blocks_buf;
	numbuf_t free_files_buf;
...
/* ARGSUSED */
static void
V_output(struct df_request *dfrp, struct statvfs *fsp)
{
	char temp_buf[LINEBUF_SIZE];
