...
#ident	"@(#)umount.c	1.29	95/09/25 SMI"	/* SVr4.0 1.17	*/
...
int	aflg, oflg, Vflg, dashflg, dflg;
 ...
 */
static const char   *keeplist[] = { "/", "/usr", "/usr/kvm", "/var",
					"/proc", "/dev/fd", "/tmp", 0};
...
#define	TEXT_DOMAIN "SYS_TEST"
	 ...
	 * "-d" for compatibility
	 */
	while ((cc = getopt(argc, argv, "ado:V?")) != -1)
		switch (cc) {
		case 'a':
			aflg++;
			break;
	 ...
	 */
	while ((n = readlink(mnttab, symfile, PATH_MAX)) != -1) {
		symfile[n] = '\0';
		mnttab = symfile;
	}
	 ...
	 * is forced to mount and mnttab is updated. If realpath
	 * is called later, then the actual mnttab and the in-memory
	 * copy of it in mntll don't match leaving the unmounted
	 * entry in the mnttab;
	 */
...
doexec(struct mnttab *ment)
{
	...
	/*
	 * if the umount is done, and the user aborts there is a
	 * possibility that mnttab is not updated. Then that
	 * entry will never get removed; so disable signals.
	 */
	 ...
	 */
	if (((ret = umount(ment->mnt_mountp)) < 0) && (errno != EBUSY))
		ret = umount(ment->mnt_special);
	...
	fsrmfrommtab(ment);	/* remove entry from mnttab */
...
"Usage:\n%s [-V] [-o specific_options] {special | mount-point}\n"),
		myname);
	fprintf(stderr, gettext(
"%s -a [-V] [-o specific_options] [mount_point ...]\n"), myname);
	exit(1);
}
