...
#ident	"@(#)sharetab.c	1.6	93/08/13 SMI"	/* SVr4.0 1.2	*/
...
alloc_failed:
	/*
	 * make sure we don't reference sharefree with NULL nsh
	 */
	if (nsh != NULL)
		sharefree(nsh);
	return (NULL);
...
/*
 * sharefree() should NEVER be referenced will a NULL pointer sh.
 * It is currently referenced in remshare() and sharedup() only
 */
static void
sharefree(sh)
	struct share *sh;
