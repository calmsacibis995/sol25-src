...
#ident	"@(#)ff.c	1.12	92/07/14 SMI"	/* SVr4.0 1.9 */
	...
	register struct dinode *ip;
{
	...
	pr:
		if (Sflg || uflg || Aflg || Mflg || Cflg || Nflg || Lflg)
			dip = ginode(dp->d_ino);
		if ((!Aflg || 
		cmp((Today - dip->di_un.di_icom.ic_atime)/DAY, Adelay, Asign)) &&
			...
			if (Sflg)
				(void) printf("\t%6d",
				    dip->di_un.di_icom.ic_size.val[1]);
			if (uflg)
				(void) printf("\t%s",
