 ...
 * "limit" is used for RUSERSVERS_3 only.  Only this many entries is
 *	processed.
 *
...
getutmpx_3(all, version, limit)
	...
	int limit;		/* in v3: limits how many users we return */
{
	struct utmpx *utent;
	...
	while ((utent = getutxent()) != NULL && (cnt < MAXUSERS ||
	...
	register SVCXPRT *transp;
{
	...
	case RUSERSPROC_ALLNAMES:
		if (rqstp->rq_vers == RUSERSVERS_IDLE) {
			 ...
			 */
			if (utmp_array_res.utmp_array_len != 0) {
				for (i = 0; i < utmp_array_res.utmp_array_len;
						i++)
					free_ua_entry(
					    &utmp_array_res.utmp_array_val[i]);
			}
			...
			if (cnt != utmp_array_res.utmp_array_len) {
				...
				utmp_array_res.utmp_array_len = cnt;
			}
...
int sig;
{
	if (_rpcsvcrecent) {
		...
		if (_rpcsvcdirty == 0) {
			...
			for (i = 0, openfd = 0; i < rl.rlim_max && openfd < 2;
					i++)
