	...
	char *argv[];
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, BOOTPARAMPROG, BOOTPARAMVERS, bootparamprog_1, 0)) {
			msgout("unable to register (BOOTPARAMPROG, BOOTPARAMVERS).");
			exit(1);
		}
		if (pmclose) {
	...
	if (!svc_create(bootparamprog_1, BOOTPARAMPROG, BOOTPARAMVERS, "netpath")) {
 		msgout("unable to create (BOOTPARAMPROG, BOOTPARAMVERS) for netpath.");
		exit(1);
	}
