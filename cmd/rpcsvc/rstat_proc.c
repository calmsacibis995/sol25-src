...
#pragma	ident	"@(#)rstat_proc.c	1.15	94/07/22 SMI"
...
static	kstat_named_t *lbolt_knp;
...
stat_init(void)
{
	...
	/*
	 * boottime = currenttime - lbolt / hz
	 */
	hz = sysconf(_SC_CLK_TCK);
	gettimeofday(&btm, (struct timezone *)0);
	btm.tv_sec -= lbolt_knp->value.l / hz;
...
extern int _rpcsvcdirty;	 /* Still serving ? */
	...
	stats_s2.cp_time[0] =
	stats_s3.cp_time[0] =
	stats_s4.cp_time.cp_time_val[0] =
		cpu_sysinfo_all.cpu[CPU_USER];
	stats_s2.cp_time[1] =
	stats_s3.cp_time[1] =
	stats_s4.cp_time.cp_time_val[1] =
		cpu_sysinfo_all.cpu[CPU_WAIT];
	stats_s2.cp_time[2] =
	stats_s3.cp_time[2] =
	stats_s4.cp_time.cp_time_val[2] =
		cpu_sysinfo_all.cpu[CPU_KERNEL];
	stats_s2.cp_time[3] =
	stats_s3.cp_time[3] =
	stats_s4.cp_time.cp_time_val[3] =
		cpu_sysinfo_all.cpu[CPU_IDLE];

#ifdef DEBUG
	fprintf(stderr, "cpu: %d %d %d %d\n", cpu_sysinfo_all.cpu[0],
		cpu_sysinfo_all.cpu[1], cpu_sysinfo_all.cpu[2],
		cpu_sysinfo_all.cpu[3]);
	fprintf(stderr, "cp_time: %d %d %d %d\n", stats_s3.cp_time[0],
		stats_s3.cp_time[1], stats_s3.cp_time[2],
		stats_s3.cp_time[3]);
#endif
