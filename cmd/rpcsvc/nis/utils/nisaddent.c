...
#pragma ident "@(#)nisaddent.c	1.36	95/04/10 SMI"
	...
	void *udata;
{
	...
	*(p++) = 0;
	...
	sprintf(aname, "[cname=%s,auth_type=DES],%s", cname, ta_name);
	...
	void *udata;
{
	...
	nis_object eobj;
	entry_col ecol[4];
	char *alias, aname[NIS_MAXNAMELEN];
	...
	sprintf(aname, "[alias=%s],%s", alias, ta_name);
...
struct ttypelist_t ttypelist[] = {
	...
	{"publickey", "publickey.byname", "cred.org_dir", "cred_tbl",
	    "auth_type=DES", 2, 0, " \t",
	    dbmniskey, nisdbmkey, filedbmkey, genent_publickey,
	    dump_publickey, 0,
	    filedbmline_comment, filetodbm_keyvalue, fetchdbm_addkey,
	    printfkeystr_01 },
	...
	int merge;
{
	...
	while (tt->filedbmline(&line, etcf, &lineno, &loc)) {
		...
		sprintf(keyname, "[%s],%s",  tt->dbmniskey(key), ta_name);
		eres = nis_list(keyname, fpath|MASTER_ONLY, 0, 0);
		switch (eres->status) {
...
adddbm()
{
	...
	for (key = dbm_firstkey(dbmf); key.dptr; key = dbm_nextkey(dbmf)) {
		...
		if (line = tt->fetchdbm(key)) {
			sprintf(keyname, "[%s],%s", tt->dbmniskey(key),
					ta_name);
			eres = nis_list(keyname, fpath|MASTER_ONLY, 0, 0);
			switch (eres->status) {
			case NIS_BADATTRIBUTE:
	...
	char *argv[];
{
	...
	while ((c = getopt(argc, argv, "D:PAMvapqrmdt:f:y:Y:")) != -1) {
		switch (c) {
		case 'D':
			defstr = optarg;
			break;
	...
	if (ta_obj->zo_data.zo_type != TABLE_OBJ) {
