...
#pragma ident	"@(#)nistbladm.c	1.23	95/10/06 SMI"
	...
	char *argv[];
{
	...
	char *p, *p1;
	table_col *tcol, *ocol;
	char mpred[NIS_MAXNAMELEN], spred[NIS_MAXNAMELEN];
	char sname[NIS_MAXNAMELEN], tname[NIS_MAXNAMELEN];
	nis_error s;
	ib_request ibr;
	entry_col *ecol;
	int nrights;
	...
	if (obj->zo_data.zo_type != TABLE_OBJ) {
	...
	case OP_DESTROY:
		sprintf(sname, "[]%s", tname);
		rres = nis_remove_entry(sname, 0, REM_MULTIPLE);
		switch (rres->status) {
		case NIS_SUCCESS:
		case NIS_NOTFOUND:
			break;
