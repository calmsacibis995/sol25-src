...
#pragma ident	"@(#)nismatch.c	1.10	94/10/14 SMI"
...
{
	fprintf(stderr,
		"usage: nismatch [-PAMchvo] key tablename\n");
	fprintf(stderr,
		"       nismatch [-PAMchvo] colname=key ... tablename\n");
	fprintf(stderr,
		"       nismatch [-PAMchvo] indexedname\n");
	exit(EXIT_ERROR);
}
	...
	char *argv[];
{
	...
	while ((c = getopt(argc, argv, "PAMchvo")) != -1) {
		switch (c) {
		case 'P':
			fpath = FOLLOW_PATH;
			break;
