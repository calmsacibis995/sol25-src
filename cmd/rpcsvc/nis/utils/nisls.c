...
#pragma ident	"@(#)nisls.c	1.11	94/10/15 SMI"
	...
	void		*udata;
{
	...
	if ((flags & F_RECURSE) && (et == DIRECTORY_OBJ) && !(flags & F_DIR))
		nlpush(strdup(fname));
	...
	char	*argv[];
{
	...
	while (name = nlpop()) {
		...
		if ((res->objects.objects_val[0].zo_data.zo_type != DIRECTORY_OBJ) || (flags & F_DIR)) {
