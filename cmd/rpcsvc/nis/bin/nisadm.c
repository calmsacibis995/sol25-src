...
#pragma ident	"@(#)nisadm.c	1.8	94/10/14 SMI"
...
oar_mask masks[6] = {
		{DEFAULT_RIGHTS, DIRECTORY_OBJ},
		{DEFAULT_RIGHTS, GROUP_OBJ},
		{DEFAULT_RIGHTS, LINK_OBJ},
		{DEFAULT_RIGHTS, ENTRY_OBJ},
		{DEFAULT_RIGHTS, TABLE_OBJ},
		{0L, BOGUS_OBJ}
};
	...
	int	*buflen;
{
	...
	*(s-1) = NUL; /* Nul terminate the string */
	*buf = s;
...
#define	BUFLENGTH 4096
	...
	if (type == BOGUS_OBJ) {
		tmp.zo_data.zo_type = BOGUS_OBJ;
		while (tmp.zo_data.zo_type == BOGUS_OBJ) {
			printf("Enter the objects type : ");
			xask(tmpval);
			if (LOWER(tmpval[0]) == 'g')
				tmp.zo_data.zo_type = GROUP_OBJ;
			else if (LOWER(tmpval[0]) == 't')
				tmp.zo_data.zo_type = TABLE_OBJ;
			else if (LOWER(tmpval[0]) == 'd')
				tmp.zo_data.zo_type = DIRECTORY_OBJ;
			else if (LOWER(tmpval[0]) == 'e')
				tmp.zo_data.zo_type = ENTRY_OBJ;
			else if (LOWER(tmpval[0]) == 'l')
				tmp.zo_data.zo_type = LINK_OBJ;
			else if (LOWER(tmpval[0]) == 'p')
				tmp.zo_data.zo_type = PRIVATE_OBJ;
		}
	} else
	...
	switch (tmp.zo_data.zo_type) {
		...
		case TABLE_OBJ :
			...
			xask(tmpval);
			ta->ta_sep = (tmpval[0] == NUL) ? ' ' : tmpval[0];
		...
		case ENTRY_OBJ :
			...
			for (i = 0; i < np; i++) {
				...
				len = strlen((char *) val)+1; /* inc the NUL */
				if (flags & EN_BINARY) {
					for (j = 0; j < len; j += 2)
						*(val + (j>>1)) =
							getbyte(val+j);
