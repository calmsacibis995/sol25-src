#!/bin/sh
#
# @(#)nisclient.sh	1.16 95/03/10	Copyright 1992 Sun Microsystems Inc.
...
init()
{
	...
	# - during the restore process, both files must be removed if the
	...
	#
	DELETE_FILES="/var/nis/NIS_SHARED_DIRCACHE"
	...
	#
	# The following section is need for 4.x support.
	#
	if [ $OS -eq 5 ]
	then
...
setup_backup()
{
	...
	do
		if [ -f $FILE.$BACKUP ]
		then
			$ECHO mv $FILE.$BACKUP $FILE.$BACKUP.$$
		fi
	done
...
setup_domain()
{
	...
	if [ "$DEFDOM" != $NODOT ]
	then
		$ECHO "cp /etc/defaultdomain /etc/defaultdomain.$BACKUP"
...
do_nisinit()
{
	...
	$ECHO "/bin/rm -f /var/nis/NIS_SHARED_DIRCACHE 2> /dev/null"
	$ECHO "mv -f /var/nis/NIS_COLD_START /var/nis/NIS_COLD_START.$BACKUP"
...
init_client()
{
	...
	then
		...
		if [ "$SEC" != "0" ]
		then
			echo "as an authenticated NIS+ client."
		else
			echo "as an unauthenticated NIS+ client."
		fi
	fi
	if [ -d /var/nis/data ]
	then
	...
	then
		if nistest '[cname='`nisdefaults -h`'],cred.org_dir'.$DOM;
		then
...
#
# restore_nis(): removes the NIS_SHARED_DIRCACHE file.
#
restore_nis()
{
	CACHE="/var/nis/NIS_SHARED_DIRCACHE"
...
restore_tmp()
{
	...
	do
		...
		then
			...
			$ECHO mv -f $FILE.$BACKUP.$$ $FILE.$BACKUP
		fi
	done
	exit 1
}
