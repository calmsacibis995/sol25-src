...
#
#
# @(#)nisserver.sh	1.13 95/03/10	Copyright 1992 Sun Microsystems Inc.
...
check_host()
{
	...
	echo "**ERROR: host $1 is not a valid NIS+ principal in domain"
	echo "$LDOM.  This $MESS $1 must must be defined in the credential"
	echo "table in domain $LDOM .  Use nisclient -c to create the"
	echo "the host credential."
	return 0
}
...
root_master()
{
	...
	then
		echo "WARNING: this script removes directories and files related to"
		echo "NIS+ under /var/nis directory with the exception of the"
		echo "NIS_COLD_START and NIS_SHARED_DIRCACHE files which will be"
		echo "renamed to <file>.${BACKUP}.  If you want to save these files,"
		echo "you should abort from this script now to save these files first."
		echo ""
		if [ -d /var/nis/data ]
		then
			...
			echo "restore your NIS+ client environment using \"nisclient -i\""
			echo "with the proper domain name and server information."
		else
			echo "Use \"nisclient -i\" to restore your NIS+ client environment."
		fi
		echo ""
	else
	...
	echo "(creating standard directories & tables)"
	$ECHO "nissetup $YP"
	if [ $? -ne 0 ]
	then
	...
	*)	;;
	...
	then
		...
		echo "	  1. /usr/sbin/nisgrpadm -c $GROUP"
		echo "	  2. /usr/sbin/nisgrpadm -a $GROUP $HOST.$DOM"
	else
		...
		then
			...
			echo "	  1. /usr/sbin/nisgrpadm -a $GROUP $HOST.$DOM"
		fi
	fi
	echo ""
	...
	echo "restarting root server at security level $SEC ..."
	if [ $OS -ne 5 -o $OSVER -lt 4 ]
	then
	...
	$ECHO restart_process nis_cachemgr
	setup_rpc
...
nonroot_master()
{
	...
	$ECHO "/usr/lib/nis/nisping $PAR_DOM > /dev/null"
	sleep 5
	...
	then
		...
		then
			...
			echo "	You will need to create this group manually:"
			echo "	  1. /usr/sbin/nisgrpadm -c $GROUP"
			echo "	  2. /usr/sbin/nisgrpadm -a $GROUP $ALLSERVERS"
		else
			...
			then
				...
				echo ""
				echo "	You will need to add this member manually:"
				echo "	  1. /usr/sbin/nisgrpadm -a $GROUP $ALLSERVERS"
			else
	...
	echo "The system ${HOST} is now configured as a non-root server for"
	echo "domain $DOM.  You can now populate the standard NIS+ tables by"
	echo "using the nispopulate or /usr/lib/nis/nisaddent"
	echo "commands."
}
