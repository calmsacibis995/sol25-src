#pragma ident	"@(#)nis_db.c	1.21	94/12/22 SMI"
/*
 * Ported from
 ...
 */
static bool_t
xdr_nis_name_abbrev(xdrs, namep, domainname)
	register XDR	*xdrs;
	nis_name	*namep;
 ...
 */
static bool_t
xdr_nis_fetus_object(xdrs, objp, tobj)
	register XDR	*xdrs;
	...
	nis_object	*tobj;	/* Table object */
{
	...
	objp->zo_data.zo_type = ENTRY_OBJ;
	...
	char		*res;
{
	...
	else if (nis_dir_cmp(name, nis_local_directory())  == SAME_NAME)
	...
	if (t[0] == '"') {
		...
		if (t[1] != '"') {
			...
			for (i = 0; i < j; i++)
				t[i] = t[i+1];
			t[j-2] = NUL; /* Trounce trailing dquote */
		}
	}
	/*
 ...
 */
static obj_list *
cvt2object(tablename, ep, num, got, mangle)
	...
	nis_name	name;
{
	...
	if (dbres == 0 || dbres->status == DB_MEMORY_LIMIT) {
		...
		res->status = NIS_SUCCESS; /* presume success */
		xdrmem_create(&xdrs, ec->ENVAL, ec->ENLEN, XDR_DECODE);
	...
	else if (dbres->status == DB_BADTABLE)
		res->status = NIS_NOSUCHTABLE;
	else {
	...
	if (res->obj) {
		if ((__type_of(res->obj) == TABLE_OBJ) &&
			strstr(name, "org_dir")) {
			/*
	...
	int		modop;	 /* Modify operation flag */
{
	...
	if ((__type_of(obj) == TABLE_OBJ) && (!modop)) {
	...
	if (dbres == 0 || dbres->status == DB_MEMORY_LIMIT)
		res = NIS_NOMEMORY;
	else if (dbres->status == DB_STORAGE_LIMIT)
		res = NIS_NOFILESPACE;
	else if (dbres->status == DB_SUCCESS) {
		res =  NIS_SUCCESS;
		if (modop) {
	...
	nis_attr	*a;	/* An array of attributes 	*/
{
	...
	if (dbres == 0 || dbres->status == DB_MEMORY_LIMIT)
		...
		syslog(LOG_ERR, "No table %s for object %s, status = %d",
				table, name, dbres->status);
		res->status = NIS_SYSTEMERROR;
	}
	if (verbose && dbres)
		syslog(LOG_INFO,
 ...
 */
nis_fn_result *
db_firstib(name, na, a, flags, table)
 ...
 */
nis_fn_result *
db_nextib(name, cookie, flags, table)
	nis_name	name;
	netobj		*cookie;
	int		flags;
	char		*table;
{
	...
	res->ticks = __stop_clock(1);
	if (dbres == NULL || dbres->status == DB_MEMORY_LIMIT)
		res->status = NIS_NOMEMORY;
	else if (dbres->status != DB_SUCCESS) {
		switch (dbres->status) {
			case DB_NOTFOUND :
 ...
 */
nis_error
__db_addib(name, numattr, attrs, obj)
	...
	nis_object	*obj;		/* Entry to add to the table */
{
	...
	if (CHILDPROC) {
		syslog(LOG_INFO,
			"__db_addib: non-parent process attempting an add.");
		return (NIS_SYSTEMERROR);
	}
	...
	if (verbose && dbres)
		syslog(LOG_INFO, "__db_addib: done. (%d)", dbres->status);
	free_db_result(dbres);
	return (res);
}
 ...
 */
nis_db_result *
db_addib(name, numattr, attrs, obj, tobj)
	nis_name	name;		/* Name of the table.	*/
	int		numattr;	/* Number of attributes	*/
	nis_attr	*attrs;		/* array of attributes	*/
	nis_object	*obj,		/* Entry to add to the table */
			*tobj;		/* Table to add it too.	*/
{
	nis_attr	*attr_list;
	int		i, mc, na;
	table_col	*tc;
	...
	nis_attr	*attrs;
{
	...
	if (verbose && dbres)
		syslog(LOG_INFO, "__db_remib: done. (%d)", dbres->status);
	free_db_result(dbres);
	return (res);
}
 ...
 */
nis_db_result *
db_remib(name, nattrs, attrs, olist, numo, tobj, xid_time)
	nis_name	name;
	int		nattrs;
	nis_attr	*attrs;
	obj_list	*olist;
	...
	nis_name	name;	/* name of directory to checkpoint */
{
	...
	while (fnr->status == NIS_SUCCESS) {
		if (__type_of(fnr->obj) == TABLE_OBJ ||
		    __type_of(fnr->obj) == DIRECTORY_OBJ) {
