...
#else
	memcpy(&(temp->theaddr.s_addr), addr, 4);
#endif
	if (temp != NULL) {
		temp->h_errno = TRY_AGAIN;
		if (nres_dosrch(temp) >= 0)
	...
	struct rpc_as  *as;
{
	...
	if (status > 0) {
		...
		if (verbose && verbose_out) p_query(temp->answer);
	} else if (status < 0) {
	...
	if (theans) {
		if (temp->reverse == REVERSE_PTR) {
			...
			for (a = (struct in_addr **) theans->h_addr_list;
								*a; a++)
#ifdef TDRPC
				if (bcmp((char *) *a, (char *) &(temp->theaddr),
						theans->h_length) == 0) {
#else
			...
			if (!found_addr) {  /* weve been spoofed */
				prnt(P_ERR, "nres_gethostbyaddr: %s != %s.\n",
