#pragma ident	"@(#)nis_ns_proc.c	1.22	94/05/23 SMI"
/*
 * Ported from :
	...
	struct svc_req *reqstp;
{
	...
	if (nis_name_of(argp->ns_name) == NULL) {
		int want_root;
	...
	nis_result	*res;
{
	...
	if (res->status != NIS_SUCCESS)
		 ...
		 * inside db_remove
		 */
		if (__type_of(old) == DIRECTORY_OBJ) {
			/*
	...
	nis_name	princp;	/* Principal making the request */
{
	 ...
	 */
	if (root_server &&
	    (nis_dir_cmp(name, nis_local_directory()) == SAME_NAME)) {
	...
	struct svc_req *reqstp;
{
	...
	if (! root_server && (nis_name_of(argp->ns_name) == NULL))
		return (nis_make_error(NIS_BADNAME, 0, 0, 0, __stop_clock(0)));
