#pragma ident	"@(#)yp_ns_proc.c	1.20	95/02/23 SMI"
/*
 *	yp_ns_proc.c
 *
...
static nis_error
cook_record_from_entry(entry_obj *, char **, int *, char *);
static void
cook_an_ngroup(entry_obj *, char **, int *);
static nis_error
cook_host_record(nis_object *, int, char **, int *);
	...
	char *domain;
{
	...
	if (domain == 0 || (n = strlen(domain)) == 0 || n >= YPMAXDOMAIN)
		return (0);
	return (1);
}
	...
	struct svc_req *rqstp;
{
	...
	status = getzobj_orgdir(req->domain, &org_zobj);
	...
	if ((__type_of(dbres->obj) != TABLE_OBJ) ||
					(column &&
	...
	long *err;
{
	...
	if ((ret_en_objs == NULL) || !(num_ntrees) ||
				(__type_of(ret_en_objs) != ENTRY_OBJ)) {
		*err = YP_NOKEY;
		return (NULL);
	}
	...
	struct svc_req *rqstp;
{
	...
	if ((table_req.ibr_name = (char *) XCALLOC(1, \
			(strlen(table) + ORGLEN + strlen(req->domain) + 1))) ==
									NULL) {
	...
	strcat(table_req.ibr_name, req->domain);
	if (*(req->domain + strlen(req->domain) - 1) != '.')
	 ...
	 * going through this is to enforce nis_return_list's censorship.
	 */
	nis_getprincipal(princp, rqstp->rq_cred.oa_flavor, rqstp->rq_clntcred);
	...
	if (strcmp(table, "hosts") == 0) {
		status = cook_host_record(ret_en_objs, num_ntrees, &t, &len);
	} else {
		status = cook_record_from_entry(e, &t, &len, table);
	}
...
doagain:
	...
	if (__type_of(ret_en_objs) != ENTRY_OBJ) {
	...
	struct svc_req *rqstp;
{
	...
	nis_getprincipal(princp, rqstp->rq_cred.oa_flavor,
							rqstp->rq_clntcred);
	all_readable = __can_do(NIS_READ_ACC, dbres->obj->zo_access,
					dbres->obj, princp);
	fnr = db_nextib(full_tblnm, &cookie, 0, NULL);
...
((resp.key_column_ndx = get_keyndx(&(dbres->obj->TA_data), column)) < 0 ||
	...
	return (&resp); /* xdr_ypresp_all will do the real work ! */
...
ypproc_maplist_svc(domain, rqstp)
	string_t *domain;
	struct svc_req *rqstp;
{
...
static nis_error
cook_record_from_entry(e, record_dat, record_len, table)
	entry_obj *e;
	char **record_dat;
	int *record_len;
	char *table;
{
	int i, len, num_cols = e->en_cols.en_cols_len;
	...
	if (strcmp(table, "hosts") == 0)
		...
		/*
		 * For unrecognized maps we build a "dumb" datum out of the
		 * the column 1.
		 */
		sprintf(entrydat, "%s", COLVAL(1));
	}
 ...
 */
static nis_error
cook_host_record(objs, num_objs, record_dat, record_len)
	nis_object *objs;
	int num_objs;
	char **record_dat;
	int *record_len;
{
	...
	*record_len = 0;
	...
	qsort((void *)objs, num_objs, sizeof (nis_object), host_object_compar);
	...
	struct svc_req *rqstp;
{
	...
	if (rqstp == NULL)
		return ("");
	sock = svc_getcaller(rqstp->rq_xprt);
	pnet = inet_ntoa(sock->sin_addr);
	return (pnet);
}
#else
static char *
