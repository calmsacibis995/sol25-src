...
#pragma ident	"@(#)nisinit.c 1.26	95/03/24 SMI"
	...
	char	*host;
{
	...
	/*
	 * This will be updated by nisupdkeys.
	 */
...
usage(s)
	char	*s;
{
	fprintf(stderr, "usage: \t%s -c -H host|-B|-C coldstart\n", s);
	fprintf(stderr, "\t%s -r\n", s);
...
"For more information, see secure_rpc(3N), publickey(3N), & nisaddcred(1).\n");
	...
	op = MAKE_NONE;
	src = SRC_NONE;
	while ((c = getopt(argc, argv, "rcp:C:BMH:")) != -1) {
		switch (c) {
			case 'r' :
				if (op != MAKE_NONE)
					usage(argv[0]);
...
"Error: Parent object cannot be created before creating a root object.\n");
	...
	switch (op) {
		case MAKE_ROOT :
			...
			d_obj.zo_data.zo_type = DIRECTORY_OBJ;
	...
	"\nERROR: could not open file %s for storing directories served.\n",
		...
		case MAKE_CLIENT :
			...
			case SRC_BCAST :
				...
				dummy = rpc_broadcast(NIS_PROG, NIS_VERSION,
						...
						(resultproc_t)bc_init_data,
									NULL);
#else
				clnt_broadcast(NIS_PROG, NIS_VERSION,
						NIS_FINDDIRECTORY,
						(char *) &fdarg, xdr_fd_args,
						(char *) &fdres, xdr_fd_result,
						bc_init_data);
#endif
			...
			__nis_CacheRestart();
			unlink(COLDSTART);
			res = nis_lookup(directory, NO_AUTHINFO);
			if (res->status != NIS_SUCCESS) {
				fprintf(stderr,
		"\nError: Could not create a valid NIS+ coldstart file\n");
				nis_perror(res->status, "lookup");
				exit(1);
			}
