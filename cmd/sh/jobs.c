...
#ident	"@(#)jobs.c	1.15	95/05/02 SMI"	/* SVr4.0 1.17.15.1	*/
/*
...
collect_fg_job()
{
	...
	while (1) {
		errno = 0;
		/*
		 * This is not interupted by the session leader receiving
		 * a signal, so do not need to be changed to handle the
		 * situation where it is intrupted.
		 *
		 */
...
register struct job *jp;
{
	...
	do {
		errno = 0;
		ret = waitpid(pid, &stat, WUNTRACED|WNOWAIT);
		err = errno;
		if (ret == -1 && err == EINTR)
			chktrap();
		if (ret == -1 && err == ECHILD)
