...
#pragma	ident	"@(#)uscsi_cmd.c 1.3     94/12/30 SMI"
 ...
 */
int
uscsi_cmd(int file, struct uscsi_cmd *command, int read_flag)
{
int	status, i;
	...
	command->uscsi_flags = USCSI_ISOLATE | USCSI_DIAGNOSE |
		USCSI_RQENABLE;
	if (read_flag == USCSI_READ) command->uscsi_flags |= USCSI_READ;
