...
#pragma ident	"@(#)cpio.c	1.44	95/09/26 SMI"
...
static union b_block *bar_hdr_p, *bar_Vhdr;
struct gen_hdr Gen_bar_vol;
...
ckname()
{
	...
	if ((Args & OCr) && !Adir) { /* rename interactively */
		(void)fprintf(Wtty_p, gettext("Rename \"%s\"? "), G_p->g_nam_p);
		(void)fflush(Wtty_p);
	...
	if (mask & OCH) {
		...
		if (!strcmp("odc", Hdr_p)) {
			...
			if ((Args & OCo) || (Args & OCp))
				msg(ERR, "Header type bar can only be used with -i");
			if (Args & OCP)
				msg(ERR, "Can't preserve using bar header");
			Hdr_type = BAR;
			Max_namesz = TNAMLEN - 1;
			Onecopy = 0;
		} else
...
creat_hdr()
{
	...
	case USTAR:
		...
		if ((split = strlen(Gen.g_nam_p)) > MAXNAM) {
			...
			Gen.g_tname = junkbuf;
			if ((Gen.g_prefix = (char *)malloc(strlen(abuf) + 1)) == NULL)
				msg(EXT, "Out of memory.");
		...
		if (result >= 0) {
			...
			if (Pflag && aclp != NULL) {
                        	if (acl(nam_p, SETACL, aclcnt, aclp) < 0) {
                                        msg(ERRN, "\"%s\": failed to set acl", nam_p);
                                }
                        	free(aclp);
                        	aclp = NULL;
                	}
	...
	if (G_p->g_typeflag == 0 &&
		...
		if (S_ISDIR(DesSt.st_mode)) {
			...
			if (remove_dir(nam_p) < 0) {
				msg(ERRN, "Cannot remove the directory \"%s\"", nam_p);
				(void)unlink(Over_p);
				*Over_p = '\0';
				/*
...
data_out()
{
	...
	if (Hdr_type == CRC && (csum = cksum(CRC, 0)) < 0) {
		G_p->g_cksum = csum;
	...
	if (Hdr_type == USTAR || Hdr_type == TAR) {
		...
		if (G_p->g_nlink == 1) {		/* hard link */
			if (ckname() != F_SKIP) {
			    ...
			    strncpy(lname, Thdr_p->tbuf.t_linkname, NAMSIZ);
...
file_out()
{
	...
	if (Hdr_type == TAR || Hdr_type == USTAR) { /* TAR and USTAR */
		 ...
		 */
		 if ((linklen = strlen(T_lname)) > NAMSIZ) {
	...
	while (l_p != &Lnk_hd) {
		...
		if (stat(Gen.g_nam_p, &SrcSt) == 0) { /* check if file exists */
			...
			while (tl_p != (struct Lnk *)NULL) {
				...
				write_hdr(0, 0); /* archive header only */
...
gethdr()
{
	...
	if ((Gen.g_mode == SECMODE) || ((Hdr_type == USTAR || Hdr_type == TAR) &&
	    Thdr_p->tbuf.t_typeflag == 'A')) {
		...
		if (Pflag) {
			...
			tp = secp;
                	do {
					    ...
					    &aclcnt);
                                	/* header is 8 */
                                	attrsize = 8 + strlen(&attr->attr_info[0]) + 1;
        ...
        if (Pflag) {
                ...
                if (aclcnt > MIN_ACL_ENTRIES) {
                        if ((aclp = (aclent_t *)malloc(
                                ...
                                msg(ERRN, "Error with malloc() of \"%d\"", aclcnt);
                        }
                        if (acl(Gen.g_nam_p, GETACL, aclcnt, aclp) < 0) {
                                msg(ERRN, "Error with getacl() of \"%s\"", Gen.g_nam_p);
                        }
                }
...
openout()
{
	...
	do {
		...
		if (Hdr_type == TAR && Thdr_p->tbuf.t_typeflag == '2') {
			if ((result = symlink(Thdr_p->tbuf.t_linkname, nam_p)) >= 0) {
				cnt = 0;
				if (Over_p != NULL) {
					(void)unlink(Over_p);
					*Over_p = '\0';
		...
		if (lstat(G_p->g_nam_p, &sbuf) == 0) {
			if ((sbuf.st_mode & Ftype) != S_IFLNK) {
				...
				if (chmod(G_p->g_nam_p, new_mask) < 0)
					msg(ERRN, "Cannot chmod() \"%s\"", G_p->g_nam_p);
				umask(orig_mask);
			}
		}
	}
...
register char *nam_p;
{
	...
	if ((Args & OCt) && (Args & OCv)) {
		...
		if ((Hdr_type == USTAR || Hdr_type == TAR) &&
			...
			(void)printf(gettext(" linked to %s"), Thdr_p->tbuf.t_linkname);
		}
		(void)printf("\n");
...
write_hdr(int secflag, int len)
{
	...
	case CHR:
		if (secflag) {
			...
			G_p->g_magic, G_p->g_dev, G_p->g_ino, SECMODE, G_p->g_uid, G_p->g_gid, 
			G_p->g_nlink, MK_USHORT(G_p->g_rdev), G_p->g_mtime, G_p->g_namesz, len, G_p->g_nam_p);
		} else {
			(void)sprintf(Buffr.b_in_p, "%.6lo%.6lo%.6lo%.6lo%.6lo%.6lo%.6lo%.6lo%.11lo%.6lo%.11lo%s",
			G_p->g_magic, G_p->g_dev, G_p->g_ino, G_p->g_mode, G_p->g_uid, G_p->g_gid, 
			G_p->g_nlink, MK_USHORT(G_p->g_rdev), G_p->g_mtime, G_p->g_namesz, G_p->g_filesz, G_p->g_nam_p);
		}
		break;
	case ASC:
	case CRC:
		if (secflag) {
			...
			G_p->g_magic, G_p->g_ino, SECMODE, G_p->g_uid, G_p->g_gid, G_p->g_nlink, G_p->g_mtime,
			len, major(G_p->g_dev), minor(G_p->g_dev), major(G_p->g_rdev), minor(G_p->g_rdev),
	...
	case USTAR: /* USTAR */
		...
		if (secflag)
			(void)sprintf(Thdr_p->tbuf.t_size, "%011lo", len);
		else
			(void)sprintf(Thdr_p->tbuf.t_size, "%011lo", G_p->g_filesz);
		(void)sprintf(Thdr_p->tbuf.t_mtime, "%011lo", G_p->g_mtime);
		if (secflag)
			Thdr_p->tbuf.t_typeflag = 'A';	/* ancillary file type */
		else
			Thdr_p->tbuf.t_typeflag = G_p->g_typeflag;
		if (T_lname[0] != '\0') {
			/*
			 * if not a symblic link
			 */
			if ((G_p->g_mode & Ftype) != S_IFLNK) {
				Thdr_p->tbuf.t_typeflag = '1';
				(void)sprintf(Thdr_p->tbuf.t_size, "%011lo", 0L);
			}
	...
	case TAR:
		...
		if (secflag)
			(void)sprintf(Thdr_p->tbuf.t_mode, "%07o ", SECMODE);
		else
			(void)sprintf(Thdr_p->tbuf.t_mode, "%07o ", G_p->g_mode);
		(void)sprintf(Thdr_p->tbuf.t_uid, "%07o ", G_p->g_uid);
		(void)sprintf(Thdr_p->tbuf.t_gid, "%07o ", G_p->g_gid);
...
convert_to_old_stat()
{
	...
	if ((TmpSt = svr32lstat(Gen.g_nam_p))) {
		if (TmpSt->st_rdev == (o_dev_t)NODEV) {
			msg(ERR, "Error old format can't support expanded types on %s", Gen.g_nam_p);
		} else {
			goodfile = 1;
			if ((TmpSt->st_ino == 0) ||
				...
				if (((TmpSt->st_mode & Ftype) != S_IFDIR) &&
				    TmpSt->st_nlink > 1)
					msg(POST, "Warning: file %s has large inode/device number - linked files will be restored as separate files", Gen.g_nam_p);
				TmpSt->st_nlink = 1;	/* ensure no links as no ino */
			}
			if ((TmpSt->st_mode & Ftype) == S_IFLNK &&
				...
				if (TmpSt->st_ino == 0) {
					TmpSt->st_nlink = 1;
					msg(POST, "Warning: file %s is a link to a file with a large inode - it will be restored as a separate file", Gen.g_nam_p);
				}
			}
		}
		if(TmpSt->st_dev < 0)
...
read_bar_vol_hdr()
{
	...
	if (tmp_hdr->dbuf.bar_magic[0] == BAR_VOLUME_MAGIC) {
		...
		if (bar_Vhdr == NULL) {
			if ((bar_Vhdr = (union b_block *)malloc(TBLOCK)) == NULL) {
				fprintf(stderr, gettext(
			"error: cannot malloc for bar volume header\n"));
				exit(1);
			}
		}
		memcpy(&(bar_Vhdr->dbuf), &(tmp_hdr->dbuf), TBLOCK);
	} else {
		fprintf(stderr, gettext(
	...
	(void)sscanf(bar_Vhdr->dbuf.uid, "%8d", &Gen_bar_vol.g_uid);
	(void)sscanf(bar_Vhdr->dbuf.gid, "%8d", &Gen_bar_vol.g_gid);
	(void)sscanf(bar_Vhdr->dbuf.size, "%12lo", &Gen_bar_vol.g_filesz);
	...
	if (strcmp(bar_Vhdr->dbuf.volume_num, "1") != 0) {
		fprintf(stderr, gettext("error: This is not volume 1.  "));
		fprintf(stderr, gettext("This is volume %s.  "),
...
read_bar_file_hdr()
{
	...
	(void)sscanf(tmp_hdr->dbuf.size, "%12lo", &Gen.g_filesz);
...
FILE **pipef;
{
	char *tmp_filename, *cmd_buf;

	if ((cmd_buf = malloc(MAXPATHLEN*2)) == NULL) {
		fprintf(stderr, gettext("can't malloc\n"));
		exit(1);
	}

	if (access(Gen.g_nam_p, W_OK) !=0) {
		sprintf(cmd_buf, "chmod +w '%s'; uncompress -c > '%s'; chmod 0%o '%s'", Gen.g_nam_p, Gen.g_nam_p, G_p->g_mode, Gen.g_nam_p);
	} else
		sprintf(cmd_buf, "uncompress -c > '%s'", Gen.g_nam_p);
...
skip_bar_volhdr()
{
	...
	if ((buff = malloc(Bufsize)) == NULL) {
		fprintf(stderr, gettext("Cannot malloc memory in skip_bar_volhdr\n"));
		exit(1);
	}
	if ((rv = g_read(Device, Archive, buff, Bufsize)) < 0) {
		fprintf(stderr, gettext("error in skip_bar_volhdr\n"));
	} else {
		...
		if (tmp_hdr->dbuf.bar_magic[0] == BAR_VOLUME_MAGIC) {
			...
			fprintf(stderr, gettext(
				"cpio error: cannot read bar volume header\n"));
			exit(1);
		}

		(void)sscanf(bar_Vhdr->dbuf.mode, "%8lo", &Gen_bar_vol.g_mode);
		(void)sscanf(bar_Vhdr->dbuf.uid, "%8lo", &Gen_bar_vol.g_uid);
		(void)sscanf(bar_Vhdr->dbuf.gid, "%8lo", &Gen_bar_vol.g_gid);
		(void)sscanf(bar_Vhdr->dbuf.size, "%12lo", &Gen_bar_vol.g_filesz);
		(void)sscanf(bar_Vhdr->dbuf.mtime, "%12lo", &Gen_bar_vol.g_mtime);
		(void)sscanf(bar_Vhdr->dbuf.chksum, "%8lo", &Gen_bar_vol.g_cksum);
		if (bar_Vhdr->dbuf.compressed == '1')
...
bar_file_in()
{
	...
	case '3':
		...
		break;
	default:
		fprintf(stderr, gettext("error: unknown file type\n"));
		break;
	}
	return;
}
	...
	*/
	...
	if (Hdr_type == BAR) {
		if (is_tape(*fdes)) {
			...
			fprintf(stderr, "Archiving to tape");
			fprintf(stderr, " blocking factor 126\n");
		}
		else if (is_floppy(*fdes)) {
			bufsize = BAR_FLOPPY_SIZE;
			fprintf(stderr, "Archiving to floppy");
			fprintf(stderr, " blocking factor 18\n");
		}
	}
...
write_ancillary(char *secinfo, int len)
{
	...
	while (len > 0) {
        	...
        	FLUSH(cnt);
        	errno = 0;
        	memcpy(Buffr.b_in_p, secinfo, (unsigned)cnt);
        	Buffr.b_in_p += cnt;
        	Buffr.b_cnt += (long)cnt;
        	len -= (long)cnt;
        }
