...
#ident	"@(#)fixpath.c	1.19	95/06/24 SMI"
...
#define	ERR_IRSET	"Install_root has already been set and used."
#define	ERR_IRNOTABS	"Install_root(-R option) requires an absolute pathname."
#define	ERR_ALLOCFAILED	"insuffient memory in %s"
		...
		if (RELATIVE(path)) {
			if (relocatable) {
				...
				*client_ptr = *server_ptr + client_offset;
				...
				sprintf(*server_ptr, rel_fmt[base_sepr],
				    basedir, path);
			} else {
