...
#ident	"@(#)merginfo.c	1.28	95/06/24 SMI"	/* SVr4.0 1.8.2.1 */
...
extern char	instdir[], pkgbin[], pkgloc[], savlog[], tmpdir[], **environ;
...
merginfo(struct cl_attr **pclass)
{
	...
	for (i = 0; environ[i]; i++) {
		...
		if ((strncmp(environ[i], "PKGSAV=", 7) == 0) && ir && *ir) {
			tmppt = &pkgsav[0];
			(void) fprintf(fp, "PKGSAV=%s\n", orig_path_ptr(tmppt));
			continue;
		}
