...
#ident	"@(#)sortmap.c	1.21	94/10/21 SMI"	/* SVr4.0 1.11.1.1 */
...
int
sortmap(struct cfextra ***extlist, struct mergstat **mstat, FILE *pkgmapfp,
	FILE *mapfp, FILE *tmpfp)
{
	...
	*extlist = procmap_x(pkgmapfp, 2, NULL);
	...
	if (*mstat)
		free(*mstat);
	*mstat =
	    (struct mergstat *)calloc((unsigned)i, sizeof (struct mergstat));

	/* truncate the t.contents file to 0 length */
	(void) sprintf(t_contents, "%s/t.contents", get_PKGADM());
	if (truncate(t_contents, 0) != 0) {
		progerr(gettext("unable to truncate temporary contents file"));
		quit(99);
	}
	dbchg = pkgdbmerg(mapfp, tmpfp, *extlist, *mstat, 60);
	if (dbchg < 0) {
		progerr(
