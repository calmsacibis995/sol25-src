...
#ident	"@(#)dofinal.c	1.19	94/10/21 SMI"	/* SVr4.0 1.10.1.1	*/
		...
		if (pinfo) {
			if (rmflag && (pinfo->status == RM_RDY)) {
				...
				if (is_remote_fs_n(fs_entry) &&
					...
					/*
					 * Check the new entry for existence
					 * and attributes. If there's a
					 * problem, mark it NOT_FND;
					 * otherwise, ENTRY_OK.
					 */
					pinfo->status =
					    (finalck(&(entry.cf_ent), 1, 1) ?
					    NOT_FND : ENTRY_OK);
