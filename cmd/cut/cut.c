...
#ident	"@(#)cut.c	1.17	95/01/11 SMI"	/* SVr4.0 1.11	*/
#
...
bnfunc() /* called when -b -n is used */
{
	...
	while ((c = fgetwc(inptr)) != EOF) {
		if (c == '\n') {
			...
			/*
			 * if the end of the range is before the last byte
			 * of the char, don't print the char.
			 * the byte of the last char is at 'pos -1'
			 */
			if (inrange)
				if (rend[rndx] < (pos - 1)) {
					inrange = 0;
