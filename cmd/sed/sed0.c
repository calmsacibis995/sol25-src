...
#ident	"@(#)sed0.c	1.12	94/08/12 SMI"	/* SVr4.0 1.15	*/
#include <stdio.h>
#include <sys/types.h>
#include <locale.h>
#include <stdarg.h>
...
jtcommon:
			...
			case 'w':
				rep->r1.command = WCOM;
				if(*cp++ != ' ') {
					fprintf(stderr, gettext(CGMES), linebuf);
					exit(2);
				}
				if(nfiles >= 10){
					fprintf(stderr,
		...
		if(++rep >= ptrend) {
			fprintf(stderr,
			    gettext("Too many commands, last: %s\n"), linebuf);
			exit(2);
		}
...
char    *lbuf;
{
	...
	while((t = getc(fin)) != EOF) {
		*++p = t;
		if(p == lbend){
                        fprintf(stderr, gettext("sed: line too long\n"));
                        exit(1);
		}
		if(*p == '\\') {
			if((t = getc(fin)) == EOF)
