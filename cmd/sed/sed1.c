...
#ident	"@(#)sed1.c	1.11	93/08/09 SMI"	/* SVr4.0 1.10	*/
#include <stdio.h>
#include <ctype.h>
#include <libintl.h>
#include "sed.h"
...
int	n;
{
	...
	if (sp >= &genbuf[LBSIZE - 1]) {
		fprintf(stderr, gettext("Output line too long.\n"));
		genbuf[LBSIZE - 1] = '\0';
	}
	lp = linebuf;
	sp = genbuf;
		...
		case ACOM:
			*aptr++ = ipc;
			if(aptr >= &abuf[ABUFSIZE]) {
				fprintf(stderr,
				    gettext("Too many appends after line %ld\n"),
					lnum);
			}
			*aptr = 0;
			break;
			...
			*hspend++ = '\n';
			...
			p2 = linebuf;
                        if((hspend + strlen(p2)) > &holdsp[LBSIZE]){
                                fprintf(stderr,
				    gettext("sed: hold space overflow !\n"));
                                exit(1);
                        }
			while(*p1++ = *p2++);
			hspend = p1-1;
	...
	cpcom:
		...
		case RCOM:
			...
			*aptr++ = ipc;
			if(aptr >= &abuf[ABUFSIZE])
				fprintf(stderr,
				    gettext("Too many reads after line%ld\n"),
				    lnum);
...
register char **ep;
{
	...
	*pcp = '\0';
	...
	while(regerrno == 50) {
		...
		if(p2 == (char *)0) {
			fprintf(stderr, gettext(TMMES), linebuf);
			exit(2);
		}
		respace = *ep = p2;
