#ident	"@(#)ypxfrd_client.c	1.5	95/07/11 SMI"
/*
...
xdr_myfyl(xdrs, objp)
	...
	int *objp;
{
	if (!xdr_answer(xdrs, (answer *)objp)) return (FALSE);
	...
	char *map;
{
	...
	if (secure_map == TRUE) {
		...
		logprintf("ypxfr: cannot bind to reserved port for tcp\n%s\n",
			  netdir_sperror(""));
		return (-1);
	    }
	}
	...
	if ((clnt = clnt_tli_create(fd, nconf, svcaddr,
		YPXFRD, 1, 0, recvsiz)) == (CLIENT *) NULL) {
		clnt_pcreateerror(
