...
#ident	"@(#)yp_b_subr.c	1.25	95/01/10 SMI"
...
CLIENT *clnt;
{
	...
	if (resp_3->ypbind_status == YPBIND_SUCC_VAL) {
		...
		/* XXX - check sin_family */
		sin = (struct sockaddr_in *)
		    resp_3->ypbind_resp_u.ypbind_bindinfo->ypbind_svcaddr->buf;
		bi = &resp.ypbind_respbody_2.ypbind_bindinfo;
		memcpy(&(bi->ypbind_binding_port), &sin->sin_port, 2); /* XXX */
		memcpy(&(bi->ypbind_binding_addr), &sin->sin_addr, 4); /* XXX */
	} else {
...
struct domain *domain_struct; /* to pass back */
{
	...
	*/
	...
	if ((nconf = __rpc_getconf(handle)) == (struct netconfig *)NULL) {
		syslog(LOG_ERR,
			"ypbind: RPC operation on /etc/netconfig failed");
		free_listofnames(lin);
		return (-1);
	...
	for (; list; list = list->nextname) {
		 ...
		 * VERY IMPORTANT: __clnt_tp_create_bootstrap is a hacked up
		 * version that does not do netdir_getbyname.
		 */
		clnt2 =
	__clnt_tp_create_bootstrap(servername, YPPROG, YPVERS, nconf);
		if (clnt2 == NULL) {
			if (rpc_createerr.cf_stat == RPC_UNKNOWNHOST) {
				syslog(LOG_ERR,
		"NIS server %s is not in /etc/hosts file !", servername);
			}
			perror(servername);
...
argp->ypsetdom_bindinfo->ypbind_servername, who->buf);
				if (uaddr)
					free(uaddr);
				freenetconfigent(nconf);
				svcerr_noprog(transp);
