...
#ident	"@(#)ls.c	1.27	95/02/22 SMI"	/* SVr4.0 1.38.5.17	*/
...
static struct cachenode * findincache(struct cachenode **, int);
static void		euc_pprintf(unsigned char *);
static void		pprintf(char *, char *);
static int		isvalid(int, int);
static int		compar(); /* no args prototypes to avoid cc warnings */
...
static unsigned long tblocks; /* number of blocks of files in a directory */
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	maxfils = quantn;
	if (((flist = (struct lbuf **)malloc((unsigned)(maxfils *
	    sizeof (struct lbuf *)))) == NULL) ||
	    ((nxtlbf = (struct lbuf *)malloc((unsigned)(quantn *
	    sizeof (struct lbuf)))) == NULL)) {
		perror("ls");
		exit(2);
	}
...
int lp;
{
	...
	if (lflg || sflg)
		curcol += printf(gettext("total %u"), tblocks);
...
struct lbuf *ap;
{
	...
	if (lflg) {
		...
		if (p->ltype == 'b' || p->ltype == 'c')
			curcol += printf("%3d,%3d", major((dev_t)p->lsize),
			    minor((dev_t)p->lsize));
		else
...
/* all flist/lbuf pair assigned files time to get some more space */
		...
		if (((flist = (struct lbuf **)realloc((char *)flist,
		    ...
		    ((nxtlbf = (struct lbuf *)malloc((unsigned)
		    (quantn * sizeof (struct lbuf)))) == NULL)) {
			perror("ls");
			nomocore = 1;
			return (NULL);
		}
...
char *s1, *s2;
{
	euc_pprintf((unsigned char *)s1);
	euc_pprintf((unsigned char *)s2);
}
...
unsigned char	*s;
{
	...
	if (!qflg && !bflg) {
		...
		while (*cp) {
			mbuf[0] = *cp++;
			n = csetno(mbuf[0]);
			size = csetlen(n) + ((n == 2 || n == 3)? 1 :0);
			i = 1;
			while (--size)
				if (*cp)
					mbuf[i++] = *cp++;
			mbuf[i] = 0;
			if (mbtowc(&wc, mbuf, i) == i) {
				if (iswprint(wc)) {
					for (n = 0; n < i; n++)
						(void) putchar(mbuf[n]);
					curcol += strcol((unsigned char *)mbuf);
					continue;
				}
			}
			for (n = 0; n < i; n++) {
...
int	f;
{
	if (ISASCII(c))
		...
		if ((!f) && wp._eucw3)
			...
			return (0);
	else if ((c >= 0200 && c <= 0237) || (c == 0240) || (c == 0377))
		return (0);
	else
		return (1);
}
