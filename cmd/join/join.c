...
#ident	"@(#)join.c	1.13	95/08/15 SMI"	/* SVr4.0 1.5	*/
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	while ((opt = getopt(argc, argv, "1:2:a:e:j:t:v:")) != EOF) {
		...
		case 'v':
			opt = (int) strtol(optarg, (char **)NULL, 10);
			if ((opt != 1) && (opt != 2))
				Usage();
...
input(int n)		/* get input line and split into fields */
{
	...
	} else if (((wchar_t *) wcsrchr((wchar_t *) bp, L'\n')
