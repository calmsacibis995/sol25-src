...
#pragma ident	"@(#)ctlr_xd7053.c	1.7	93/11/22 SMI"
	...
	caddr_t	bufaddr;
{
	...
	if (dir == DIR_READ)
		cmdblk.hdkc_cmd = XD_READ;
	else
		cmdblk.hdkc_cmd = XD_WRITE;
	/*
...
xd_ck_format()
{
	...
	for (i = 0; i <= ncyl; i += ncyl/4) {
		/*
		 * Fill in a command block with the command info. We
		 * run the command in silent mode because the error
		 * message is not important, just the success/failure.
		 */
		cmdblk.hdkc_cmd = XD_READ;
	...
	struct	defect_list *list;
{
	...
	for (curnt = start; curnt <= end; curnt += size) {
		 ...
		 */
		cmdblk.hdkc_cmd = XD_WEXT | XD_FORMVER;
		cmdblk.hdkc_blkno = curnt;
		cmdblk.hdkc_secnt = size / nsect;
	...
	for (i = 0; i < list->header.count; i++) {
		...
		do {
			...
			if (sect == (XY_HDR_SLIP & 0xffff) ||
				...
				continue;
			/*
			 * if it is mapped then we don't need to do
			 * any more
			 */
			 ...
			 * actual sector number and repair it with
			 * xd_fixblk.
			 */
			blkno = chs2bn(cyl, head, sect);
			status = xd_fixblk(blkno, index, hdr_cnt,
				pattern_buf, F_SILENT);
			if (status) {
				err_print("Unable to repair defect #%d.\n", i);
				error = -1;
			}
		/*
	...
	struct	defect_list *list;
{
	 ...
	 * Note that this doesn't include any runt on the 7053.
	 */
	status = ioctl(cur_file, HDKIOCGTYPE, &type);
	if (status) {
	...
	for (cyl = 0; cyl < pcyl; cyl++) {
		...
		for (head = 0; head < nhead; head++) {
			 ...
			 */
			if (status && (!ignore)) {
				err_print("Disk is not fully formatted.\n");
	...
	struct	defect_list *list;
{
	 ...
	 */
	if ((trkbuf = malloc((u_int) HDRSIZE)) == (char *)0) {
		err_print("Could not allocate header buffer\n");
		return (-1);
	}
	for (i = 0; i < HDRSIZE; i++)
		trkbuf[i] = (char)0xee;
	...
	for (cyl = 0; cyl < pcyl; cyl++) {
		...
		for (head = 0; head < nhead; head++) {
			...
			if (xd_putdefs(cyl, head, list)) {
				 ...
				 * running from a command file.
				 */
				err_print("Disk could not be encoded with ");
				err_print("defect info.\n");
...
"Unable to find suitable alternate sector for blkno %d.\n", bn);
		 ...
		 */
		bzero((char *)&header.xdhdr, sizeof (struct xdhdr));
		header.xdhdr.xdh_cyl_hi = XD_CYL_HI(cyl);
		header.xdhdr.xdh_cyl_lo = XD_CYL_LO(cyl);
		header.xdhdr.xdh_head = (u_char)head;
		header.xdhdr.xdh_sec = (u_char)sect;
		/*
	...
	if (must_map) {
		...
		/*
		 * Write the data from the bad sector out
		 * to its replacement.
		 */
		if (!(flags & F_SILENT)) {
			cmdblk.hdkc_cmd = XD_WRITE;
	...
	daddr_t	blkno;
{
	...
	switch (severity) {
		...
		break;
	    default:
		err_print("), Unknown severity value from xd driver.\n");
		fullabort();
	}
	/*
	...
	struct	hdk_cmd *cmdblk;
{
	...
	if (status || diag.hdkd_severe == HDK_NOERROR) {
		err_print("Error: unexpected ioctl error from xd driver.\n");
		fullabort();
	}
	/*
	 * Look up the error.
	 */
	errptr = xd_finderr(diag.hdkd_errno);
	/*
	 ...
	 */
	if (!(flags & F_SILENT))
		xd_printerr(diag.hdkd_severe, errptr->errtype, errptr->errmsg,
	...
	u_int	hdr;
{
	...
	switch (hdr & 0xffff0000) {
	    case ((u_int) XY_HDR_SLIP & 0xffff0000):
		return ((u_int) XY_HDR_SLIP);
	    case ((u_int) XY_HDR_SPARE & 0xffff0000):
		return ((u_int) XY_HDR_SPARE);
	    case ((u_int) XY_HDR_ZAP & 0xffff0000):
		return ((u_int) XY_HDR_ZAP);
	}
	/*
	...
	u_int	*buf;
{
	...
	hdr = xd_match(buf[i]);
	if (hdr == (u_int) XY_HDR_SLIP ||
			hdr == (u_int) XY_HDR_SPARE ||
				hdr == (u_int) XY_HDR_ZAP) {
		return (hdr & 0xffff);
	} else {
		return (ls);
	}
	...
	void	*buf;
{
	 ...
	 * Execute the command and return status.
	 */
	cmdblk.hdkc_cmd = XD_WEXT | XD_THEAD;
	cmdblk.hdkc_blkno = bn;
	cmdblk.hdkc_secnt = cnt;
	cmdblk.hdkc_bufaddr = (caddr_t)buf;
	cmdblk.hdkc_buflen = cnt * XD_HDRSIZE;
	...
	struct	defect_list *list;
{
	...
	for (cnt = 0; cnt < 3; cnt++) {
		 ...
		 */
		cmdblk.hdkc_cmd = XD_REXT | XD_DEFECT;
	...
	if (cnt >= 3) {
		...
		for (cnt = 0; cnt < 3; cnt++) {
			 ...
			 * Execute the read defect command for the track.
			 */
			cmdblk.hdkc_cmd = XD_REXT | XD_EXTDEF;
			cmdblk.hdkc_blkno = cyl * spc() + head * nsect;
			cmdblk.hdkc_secnt = 1;
			cmdblk.hdkc_bufaddr = (caddr_t)ptr;
			cmdblk.hdkc_buflen = sizeof (struct xydefinfo);
	 ...
	 * because of a missing address mark.  These controllers don't
	 * care about that.
	 */
	if (def_cnt == 4 && XY_TRK_BAD(ptr)) {
		err_print("track is bad. cyl = %d head = %x\n",
			def.cyl, def.head);
		return (-1);
	}
	return (0);
 ...
 * return an error.
 * We assume that the list points to the first error on the specified
 * track.  If there are more than 4 defects on a track the track is
 * marked as defective and this information is output to the user
 */
	...
	struct	defect_list *list;
{
	...
	/*
	 * If there is a defect in the defect section then try to do
	 * the extended defect stuff first
	 */

	if (XY_MAN_LEN(ptr, 0) &&
			(int)XY_MAN_BFI(ptr, 0) >= XY_MAN_DEFECT_BEGIN &&
			(int)XY_MAN_BFI(ptr, 0) <= XY_MAN_DEFECT_END)
		state = !state;
	for (cnt = 1; cnt; cnt--, state = !state) {
		status = XD_WRITE_DEFECT(ptr);
		if (!status) {
			status = XD_READ_DEFECT(tmp);
			if (!status) {
				if (!(status = bcmp((char *)ptr,
						(char *)tmp,
						sizeof (struct xydefinfo))))
					break;
			}
		}
	}
	...
	int	cyl, head;
{
	...
	cmdblk.hdkc_blkno = cyl * spc() + head * nsect;
	cmdblk.hdkc_secnt = 1;
	cmdblk.hdkc_bufaddr = (caddr_t)trkbuf;
	cmdblk.hdkc_buflen = HDRSIZE;
	return (xdcmd(cur_file, &cmdblk, F_NORMAL));
}
	...
	u_char *ptr;
{
	...
	cmdblk.hdkc_cmd = XD_WEXT | XD_DEFECT;
	cmdblk.hdkc_blkno = cyl * spc() + head * nsect;
	cmdblk.hdkc_secnt = 1;
	cmdblk.hdkc_bufaddr = (caddr_t)ptr;
	cmdblk.hdkc_buflen = sizeof (struct xydefinfo);
	...
	u_char *ptr;
{
	...
	cmdblk.hdkc_cmd = XD_WEXT | XD_EXTDEF;
	cmdblk.hdkc_blkno = cyl * spc() + head * nsect;
	cmdblk.hdkc_secnt = 1;
	cmdblk.hdkc_bufaddr = (caddr_t)ptr;
	cmdblk.hdkc_buflen = sizeof (struct xydefinfo);
 ...
 * it.  The user is then given the option of continuing (and losing some
 * defect information) or aborting the task.  Aborting the task leaves
 * the disk in its original state since we haven't done anything to
 * the disk yet
 */
	...
	struct	defect_list *list;
{
	 ...
	 */
	for (ptr = list->list, cnt = 0;
			ptr - list->list < list->header.count; ptr++) {
		...
		if (++cnt > 3) { /* over the limit */
			err_print(
"Defect list has more than 4 defects per track.\n\
Defect info will be lost\n");
			if (!check(
			"Do you wish stop writing the defect info")) {
				return (-1);
			} else {
				return (0);
			}
	...
	struct defect_entry *def;
{
	...
	/*
	 * Return the fact that no defect was found.  We get
	 * here if no match was found by falling through the for loop
	 */
not_found:
