...
#pragma ident	"@(#)auto_sense.c	1.11	94/04/04 SMI"
	...
	char			*disk_name)
{
	...
	/*
	 * We reserve 2 cylinders for our use, and set
	 * the number of data cylinders to pcyl-acyl.
	 */
	acyl = 2;
	ncyl = pcyl - acyl;
	...
	if ((pcyl * nhead * nsect) > nblocks) {
		...
		if (option_msg && diag_msg) {
			err_print("Geometry after adjusting for capacity:\n");
			err_print("    pcyl:    %d\n", pcyl);
			err_print("    ncyl:    %d\n", ncyl);
			err_print("    heads:   %d\n", nhead);
			err_print("    nsects:  %d\n", nsect);
