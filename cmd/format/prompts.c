...
#ifndef lint
#pragma ident	"@(#)prompts.c	1.4	93/08/31 SMI"
#endif	lint
	...
	u_long		*options;
{
	...
	if (cur_ctype->ctype_flags & CF_450_TYPES) {
		...
		if (status) {
			err_print("\
Warning: unable to read drive configuration type.\n");
			deflt = 0;
		} else
			deflt = dktype.hdkt_drtype;
		*options |= SUP_DRTYPE;
		ioparam.io_bounds.lower = 0;
		ioparam.io_bounds.upper = 3;
		return (input(FIO_INT, "Enter drive type",
			':', &ioparam, &deflt, DATA_INPUT));
	}
