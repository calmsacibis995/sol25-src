...
#pragma ident	"@(#)io.c	1.9	94/09/25 SMI"
...
reprompt:
	...
	if (deflt != NULL) {
		...
		case FIO_SLIST:
			...
			break;
		case FIO_CYL:
			fmt_print("[%db, %dc, %1.2fmb]", *deflt, bn2c(*deflt),
				bn2mb(*deflt));
			break;
		case FIO_OPINT:
...
partition size must be a multiple of %d blocks to lie on a cylinder boundary\n",
					spc());
				err_print("\
%d blocks is approximately %d cylinders, or %1.2f megabytes\n",
					bn, bn2c(bn), bn2mb(bn));
				break;
			}
			return (bn);
...
#ifdef sparc
		if (disk->disk_dkinfo.dki_ctype == DKC_PANTHER) {
			fmt_print("<drive may be reserved or type unknown>");
		} else
#endif sparc
