...
#ifndef lint
#pragma ident	"@(#)ctlr_xy450.c	1.7	94/09/25 SMI"
#endif	lint
	...
	caddr_t	bufaddr;
{
	...
	if (dir == DIR_READ)
		cmdblk.hdkc_cmd = XY_READ;
	else
		cmdblk.hdkc_cmd = XY_WRITE;
	/*
...
xy_ck_format()
{
	...
	for (i = 0; i <= ncyl; i += ncyl/4) {
		/*
		 * Fill in a command block with the command info. We run
		 * the command in silent mode because the error message
		 * is not important, just the success/failure.
		 */
		cmdblk.hdkc_cmd = XY_READ;
		cmdblk.hdkc_blkno = (daddr_t)i;
		cmdblk.hdkc_secnt = 4;
	...
	struct	defect_list *list;
{
	 ...
	 */
	status = ioctl(cur_file, HDKIOCGTYPE, &type);
	if (status) {
		err_print("Unable to read drive configuration.\n");
		return (-1);
	}
	hdr_cnt = type.hdkt_hsect;
	if (hdr_cnt < nsect) {
		err_print("Hard sector count less than nsect.\n");
		return (-1);
	}
	/*
	...
	for (curnt = start; curnt <= end; curnt += size) {
		/*
		 * Break up zone into reasonable chunks so
		 * XY communication does not freeze for extended
		 * periods.
		 */
		if ((end - curnt) < nsect * 10)
			size = end - curnt + 1;
		else
			size = nsect * 10;
		/*
		 ...
		 */
		cmdblk.hdkc_cmd = XY_FORMAT;
		cmdblk.hdkc_blkno = curnt;
		cmdblk.hdkc_secnt = size;
		cmdblk.hdkc_bufaddr = NULL;
	...
	for (bn = start; bn < end; bn += nsect) {
		 ...
		 */
		status = xy_readhdr(bn, hdr_cnt, (char *)pattern_buf);
		if (status) {
			err_print("Unable to read track headers.\n");
			return (-1);
		}
		 ...
		 */
		(void) memset((char *)hp, 0, sizeof (struct xyhdr));
		...
		if (status) {
			err_print("Unable to write track headers.\n");
			return (-1);
		}
	}
	...
	for (i = 0; i < list->header.count; i++) {
		...
		do {
			 ...
			 */
			status = xy_readhdr(bn, hdr_cnt,
				(char *)pattern_buf);
			if (status) {
				err_print("Unable to locate defect #%d.\n",
					i);
				error = -1;
				continue;
			}
			...
			if (bfi == UNKNOWN) {
				sect = def->sect;
				index = xy_ls2index(sect,
						(u_int *)pattern_buf);
			/*
			 * Else use bfi to calculate index into headers
			 * and that to calculate logical sector value.
			 */
			} else {
				index = bfi / bps - head;
				if (index < 0)
					index += hdr_cnt;
				sect = xy_index2ls(index,
						(u_int *)pattern_buf);
				/*
				 * If defect spans to the next sector,
				 * set up state to go around again.
				 */
				if (((bfi % bps) + len) > bps) {
					span = 1;
					bfi += bps;
			...
			/*
			 * We get funny here for drives without runts.
			 * If the defect is in the last "sector" and
			 * there is no runt then we just ignore the error
			 */
			if ((index + head) == hdr_cnt) {
			    for (j = 0; j < hdr_cnt; j++) {
				if (xy_match((u_int)((int *)
					pattern_buf)[j]) == XY_HDR_RUNT)
					break;
			    }
				/* if no runt then ignore it */
			    if (j == hdr_cnt)
				continue;
			}
			/*
			 * If we have impossible values, assume the
			 * defect can't be found.
			 */
			if (index >= hdr_cnt) {
				err_print(
				"Unable to locate defect #%d.\n", i);
				error = -1;
				continue;
			}
			/*
			 * If the sector we located has already been
			 * removed from use, nothing needs to be done.
			 */
			 ...
			 * as zapped so it won't get used later by
			 * accident.
			 */
			if (sect == (XY_HDR_SPARE & 0xffff)) {
				ptr[index] = (u_int) XY_HDR_ZAP;
				status = xy_writehdr(bn, hdr_cnt,
					pattern_buf);
				if (status) {
					err_print(
					"Unable to repair defect #%d.\n",
					    i);
					error = -1;
				}
				continue;
			...
			/*
			 * The defect is in a data sector. Calculate
			 * the actual sector number and repair it
			 * with xy_fixblk.
			 */
			blkno = chs2bn(cyl, head, sect);
			status = xy_fixblk(blkno, index, hdr_cnt,
				pattern_buf, F_SILENT);
			if (status) {
				err_print(
				"Unable to repair defect #%d.\n", i);
				error = -1;
		...
		/*
		 * If the current defect spans to the next sector,
		 * loop again.
		 */
		} while (span);
	}
 ...
 * This will only work on an unformatted disk, but we let the user try
 * on any disk in case something bad happens and the disk ends up
 * partially formatted.  This will allow the user to get the list off
 * the unformatted part.  The manufacturer's list is encoded on each
 * track of the drive.
 */
int
xy_ex_man(list)
	struct	defect_list *list;
{
	...
	if ((int)type.hdkt_promrev < (int)('a' - '@')) {
		err_print("\n\
Controller firmware is outdated and doesn't support extraction.\n");
	...
	for (cyl = 0; cyl < pcyl; cyl++) {
		 ...
		 */
		nolog_print("   ");
		pr_dblock(nolog_print, (daddr_t)(cyl * spc()));
		nolog_print("  \015");
		(void) fflush(stdout);
		/*
		...
		for (head = 0; head < nhead; head++) {
			...
			if (xy_getdefs(cyl, head, list)) {
				...
				if (cnt < 0)
					continue;
				/*
				 * Keep track of how many failures
				 * in a row we have had.
				 */
				cnt++;
				if (cnt < 3)
					continue;
				 ...
				 */
				err_print("Disk is not fully encoded with ");
				err_print("defect info.\n");
	...
	struct	defect_list *list;
{
	...
	for (cyl = 0; cyl < pcyl; cyl++) {
		...
		for (head = 0; head < nhead; head++) {
			 ...
			 * could be half-formatted.  Note that the
			 * polarity of the message MUST be this way so
			 * that it will abort the command when running
			 * from a command file.
			 */
			if (status && (!ignore)) {
				err_print(
				...
				if (!check(
					"Do you with to stop extracting"))
					return (-1);
				/*
			 ...
			 * we still skip the extract for the header in
			 * error.
			 */
			if (status)
				continue;
	...
	int	flag;
{
	...
	status = xy_readhdr(bn, hdr_cnt, (char *)pattern_buf);
	if (status) {
		err_print("Unable to locate defect.\n");
		return (-1);
	}
	/*
...
/*
 * The routines from here on are internal to the xy controller code,
 * they are not referenced from anywhere outside this file.
 */
	...
	int	flags;
{
	 ...
	 */
	if (xy_isbad(cyl, head, sect)) {
		err_print("Can't repair sector that is already mapped.\n");
		return (-1);
	}
	/*
	...
	for (i = index + 1; i < hdr_cnt; i++) {
		hdr = xy_match(ptr[i]);
		if (hdr == (u_int) XY_HDR_SPARE)
			spare_avail = i;
		if (hdr == (u_int) XY_HDR_ZAP)
			must_map = 1;
		if (hdr == (u_int) XY_HDR_RUNT)
			runt_sec = i;
	}
	/*
	...
	if (!(flags & F_SILENT)) {
		for (i = 0; i < 10; i++) {
			cmdblk.hdkc_cmd = XY_READ;
			cmdblk.hdkc_blkno = bn;
			cmdblk.hdkc_secnt = 1;
			cmdblk.hdkc_bufaddr = (caddr_t)cur_buf;
			cmdblk.hdkc_buflen = SECSIZE;
			status = xycmd(cur_file, &cmdblk, F_SILENT);
			if (!status)
				break;
		}
		if (i >= 10)
			err_print(
"Warning: unable to save defective data for blkno %d.\n", bn);
	}
	/*
	...
	if (!must_map && ns > 0 && !(flags *F_SILENT)) {
		cmdblk.hdkc_cmd = XY_READ;
		cmdblk.hdkc_blkno = bn + 1;
		cmdblk.hdkc_secnt = ns;
		cmdblk.hdkc_bufaddr = (caddr_t)cur_buf + SECSIZE;
		cmdblk.hdkc_buflen = ns * SECSIZE;
		status = xycmd(cur_file, &cmdblk, F_NORMAL);
		if (status)
			err_print("Warning: unable to save track data.\n");
	}
	/*
	 ...
	 */
	if (must_map) {
		if (bn == 0 || (bn >= datasects() &&
					bn < totalsects())) {
			err_print(
"Unable to find suitable alternate sector for blkno %d.\n", bn);
			return (-1);
		}
		ptr[index] = (u_int) XY_HDR_ZAP;
	} else {
		 ...
		 */
		(void) memset((char *)&u_xy.header, 0,
			sizeof (struct xyhdr));
		u_xy.header.xyh_sec_hi = XY_SEC_HI(sect);
		u_xy.header.xyh_sec_lo = XY_SEC_LO(sect);
		u_xy.header.xyh_cyl_hi = XY_CYL_HI(cyl);
		u_xy.header.xyh_cyl_lo = XY_CYL_LO(cyl);
		u_xy.header.xyh_head = (u_char)head;
		u_xy.header.xyh_type = cur_dtype->dtype_dr_type;
		/*
		 ...
		 */
		ptr[index + 1] = u_xy.sector;
		ptr[index] = (u_int) XY_HDR_SLIP;
		/*
	...
	if (status) {
		err_print("Unable to repair track headers.\n");
		return (-1);
	}
	if (must_map) {
		/*
		...
		if (status) {
			err_print("Bad block map table overflow.\n");
			return (-1);
		}
		/*
		 * Write the data from the bad sector
		 * out to its replacement.
		 */
		if (!(flags & F_SILENT)) {
			cmdblk.hdkc_cmd = XY_WRITE;
			cmdblk.hdkc_blkno = newblk;
			cmdblk.hdkc_secnt = 1;
			cmdblk.hdkc_bufaddr = (caddr_t)cur_buf;
			cmdblk.hdkc_buflen = SECSIZE;
			status = xycmd(cur_file, &cmdblk, F_NORMAL);
		...
		/*
		 * Write out the bad sector plus all the sectors after
		 * it.  This is necessary because slipping moves all
		 * those sectors.
		 */
		if (!(flags & F_SILENT)) {
			cmdblk.hdkc_cmd = XY_WRITE;
			cmdblk.hdkc_blkno = bn;
			cmdblk.hdkc_secnt = nsect - sect;
			cmdblk.hdkc_bufaddr = (caddr_t)cur_buf;
			cmdblk.hdkc_buflen = (nsect - sect) * SECSIZE;
			status = xycmd(cur_file, &cmdblk, F_NORMAL);
			if (status)
				err_print(
				"Warning: unable to save track data.\n");
		}
	}
	return (0);
}
	...
	daddr_t	blkno;
{
	...
	switch (severity) {
		...
		break;
	    default:
		err_print("), Unknown severity value from xy driver.\n");
		fullabort();
	}
	/*
	 ...
	 */
	if (type == HDK_ISMEDIA)
		err_print("media error");
	else
		err_print("non-media error");
	/*
	 * Print out the rest of the error info.
	 */
	if (option_msg)
		err_print(" (%s) during %s\n", msg, xycmdnames[cmd >> 8]);
	else
		err_print(" (%s)\n", msg);
	...
	struct	hdk_cmd *cmdblk;
{
	 ...
	 */
	cmdblk->hdkc_flags = HDK_SILENT | HDK_ISOLATE | HDK_DIAGNOSE;
	/*
	 ...
	 */
	status = ioctl(file, HDKIOCGDIAG, &diag);
	if (status || diag.hdkd_severe == HDK_NOERROR) {
		err_print("Error: unexpected ioctl error from xy driver.\n");
		fullabort();
	}
	/*
	 * Look up the error.
	 */
	errptr = xy_finderr(diag.hdkd_errno);
	/*
	 ...
	 */
	if (!(flags & F_SILENT))
		xy_printerr(diag.hdkd_severe, errptr->errtype, errptr->errmsg,
		    diag.hdkd_errcmd, diag.hdkd_errsect);
	/*
	 * Return status.
	 */
	if (diag.hdkd_severe == HDK_FATAL || flags & F_ALLERRS)
		return (-1);
	else
		return (0);
}
	...
	u_int	hdr;
{
	 ...
	 */
	switch (hdr) {
	    case (u_int) XY_HDR_SLIP:
	    case (u_int) XY_HDR_RUNT:
	    case (u_int) XY_HDR_SPARE:
	    case (u_int) XY_HDR_ZAP:
		return (hdr);
	}
	/*
	...
	switch (hdr & 0x000000ff) {
		...
		return ((u_int) XY_HDR_SLIP);
	    case (XY_HDR_RUNT & 0x000000ff):
		return ((u_int) XY_HDR_RUNT);
	    case (XY_HDR_SPARE & 0x000000ff):
		return ((u_int) XY_HDR_SPARE);
	    case (XY_HDR_ZAP & 0x000000ff):
		return ((u_int) XY_HDR_ZAP);
	}
	switch (hdr & 0xff000000) {
	    case ((u_int) XY_HDR_SLIP & 0xff000000):
		return ((u_int) XY_HDR_SLIP);
	    case ((u_int) XY_HDR_RUNT & 0xff000000):
		return ((u_int) XY_HDR_RUNT);
	    case ((u_int) XY_HDR_SPARE & 0xff000000):
		return ((u_int) XY_HDR_SPARE);
	    case ((u_int) XY_HDR_ZAP & 0xff000000):
		return ((u_int) XY_HDR_ZAP);
	}
	if ((hdr & 0x38000000) == 0)
		return (hdr);
	...
	void	*buf;
{
	 ...
	 * the bottom track).
	 */
	cmdblk.hdkc_cmd = XY_READHDR;
	cmdblk.hdkc_blkno = nsect;
	cmdblk.hdkc_secnt = cnt;
	cmdblk.hdkc_bufaddr = (caddr_t)buf;
	cmdblk.hdkc_buflen = cnt * XY_HDRSIZE;
	status = xycmd(cur_file, &cmdblk, F_NORMAL);
	if (status)
		return (-1);
	/*
	 ...
	 */
	if (spare_sector == 0)
		err_print("Warning: disk not set for slip-sectoring.\n");
	/*
	...
	struct	defect_list *list;
{
	...
	for (lsec = i = 0; i < cnt && lsec < (nsect + spare_sector); i++) {
		...
		def.cyl = bn2c(bn);
		def.head = bn2h(bn);
		def.bfi =  bps * ((lsec + bn2h(bn)) % (nsect + spare_sector));
		/*
	...
	struct	defect_list *list;
{
	...
	for (cnt = 0; cnt < 3; cnt++) {
		 ...
		 */
		cmdblk.hdkc_cmd = XY_READALL | XY_DEFLST;
		cmdblk.hdkc_blkno = cyl * spc() + head * nsect;
		cmdblk.hdkc_secnt = 1;
		cmdblk.hdkc_bufaddr = (caddr_t)ptr;
		cmdblk.hdkc_buflen = sizeof (struct xydefinfo);
		status = xycmd(cur_file, &cmdblk, F_NORMAL);
		/*
...
/*
 * Check the xy450 drive type.  If the type as reported by the
 * disk does not match the type specified in the format.dat
 * file, remember the specified type in case the user formats,
 * but for the time being, continue to use the type as reported
 * by the disk, so we can continue to talk to it.
 */
void
check_xy450_drive_type()
{
	...
	assert(cur_ctype->ctype_flags & CF_450_TYPES);
	...
	if (dktype.hdkt_drtype >= NXYDRIVE) {
		/*
		 * The HDKIOCGTYPE returned a invalid drive type.
		 * (The valid values are 0 to NXYDRIVE-1)
		 * This happens when the driver attach routine attempts
		 * to read the label from the drive, using the drive type
		 * values from 0 to NXYDRIVE in a for loop, but fails for all.
		 * So the best bet is to use the drive type from format.dat.
		 */
		cur_dtype->dtype_dr_type_data = cur_dtype->dtype_dr_type;
	} else if (cur_dtype->dtype_dr_type != dktype.hdkt_drtype) {
		/*
		* Print warning of conflict between data file drive
		* type and drive type presently formatted into the
		* disk.  Ignore the data file drive type so dialog
		* with disk can continue.  Give the option later on
		* (if the user formats) to change to the drive type
		* in the data file.
		*/
		err_print("\n"
"Warning: The label on the disk matches a label entry in the disk data file,\n"
"however, the current drive type (%d) of the disk does not match\n"
"the associated drive type entry (%d) in the data file.  The data\n"
"file drive type entry will be ignored so dialog with the disk can\n"
"be maintained.  If you choose to format the disk later, an option\n"
"to change the drive type value will be given.  An alternative to\n"
"this would be to quit format, update the drive type entry in the\n"
"data file and start format again.\n\n",
			dktype.hdkt_drtype, cur_dtype->dtype_dr_type);
		/*
		 * Save the type in the data file
		 */
		cur_dtype->dtype_dr_type_data = cur_dtype->dtype_dr_type;
		/*
		 * Change to type on disk
		 */
...
/*
 * Give user opportunity to change xy450 drive types before formatting
 */
int
want_xy450_drive_type_change(start, end)
	daddr_t		start;
	daddr_t		end;
{
	char		tmpstr[180];
	...
	if ((cur_dtype->dtype_dr_type != cur_dtype->dtype_dr_type_data) &&
		(end == (physsects() - 1)) && (start == 0)) {
		err_print("\n"
"Warning: The current drive type (%d) does not match the default drive\n"
"\ttype value (%d) in the data file entry.\n\n", cur_dtype->dtype_dr_type,
			cur_dtype->dtype_dr_type_data);

		(void) sprintf(tmpstr,
"Do you wish to change the current drive type (%d) to\n"
"the default drive type (%d) in the data file", cur_dtype->dtype_dr_type,
			cur_dtype->dtype_dr_type_data);
		change_drive_type = 1;
		ioparam.io_charlist = confirm_list;
		if (!input(FIO_MSTR, tmpstr, '?',
			&ioparam, &change_drive_type, DATA_INPUT)) {
			change_drive_type = 1;
		}
		else
			change_drive_type = 0;
	...
	return (change_drive_type);
}


/*
 * Change xy450 drive types before formatting.
 * Return -1 if operation failed and we should not continue.
 */
int
change_xy450_drive_type()
{
	struct hdk_type	dktype;
	int		status;

	assert(cur_ctype->ctype_flags & CF_450_TYPES);
	assert(cur_dtype->dtype_dr_type != cur_dtype->dtype_dr_type_data);

	dktype.hdkt_drtype = cur_dtype->dtype_dr_type_data;
	status = ioctl(cur_file, HDKIOCSTYPE, &dktype);
	if (status) {
		err_print("\nWarning: error setting drive type to %d\n",
			cur_dtype->dtype_dr_type_data);
		/*
		 * Since we got an error, make sure it's set
		 * to the original drive type.
		 */
		dktype.hdkt_drtype = cur_dtype->dtype_dr_type;
		status = ioctl(cur_file, HDKIOCSTYPE, &dktype);
		if (status) {
			err_print(
"\nWarning: error setting drive type back to %d\n",
				cur_dtype->dtype_dr_type);
			/*
			 * If this doesn't work, user has to live
		...
		err_print("\n\
Error: Cannot change drive types.  Check drive types of attached\n\
disks.  Each unique geometry must have a different drive type.\n");
		if (check("Continue"))
			return (-1);
	} else {
		/* change was sucessful */
		cur_dtype->dtype_dr_type = cur_dtype->dtype_dr_type_data;
	}
