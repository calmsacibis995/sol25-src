...
#ifndef lint
#pragma ident	"@(#)misc.c	1.8	94/09/25 SMI"
#endif	lint
 ...
 */
static char		*disk_4x_identifiers[] = { "sd", "xd", "xy", "id" };
...
#ifdef i386
/*
 * Return true if a device name matches the intel physcial name conventions
 * for the particular system.
 */
int
intel_physical_name(name)
	char	*name;
{
	must_be(name, 'c');
	skip_digits(name);
...
bn2mb(daddr_t nblks)
{
	...
	n = nblks;
	n *= DEV_BSIZE;
	return ((float)((float)n/(float)(1024*1024)));
}
