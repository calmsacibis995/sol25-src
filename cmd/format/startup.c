...
#pragma ident	"@(#)startup.c	1.26	94/09/25 SMI"
...
sup_init()
{
	...
	/*
	 * Create a singlely-linked list of controller types so that we may
		...
		if (strcmp(ident, "ctlr") == 0) {
			...
			if (mlp == NULL) {
				for (i = 0; i < OTHER_CTLRS; i++) {
				    if (strcmp(other_ctlrs[i], cleaned) == 0) {
...
sup_setpart()
{
	...
	for (;;) {
		...
		if (((flags & (SUP_DISK|SUP_CTLR)) == (SUP_DISK|SUP_CTLR)) &&
			...
			while (mlp != NULL) {
				if (strcmp(mlp->ctlr_type->ctype_name, ctlr)
				    == 0)
					break;
				mlp = mlp->next;
			}
...
#ifdef i386
				if (!intel_physical_name(dp->d_name)) {
#endif
				(void) strcpy(path, directory);
				(void) strcat(path, "/");
				(void) strcat(path, dp->d_name);
				add_device_to_disklist(dp->d_name, path);
				}
#ifdef i386
			}
#endif
 ...
 */
void
search_for_logical_dev(devname)
	char		*devname;
{
	char		path[MAXPATHLEN];
	char		*directory = "/dev/rdsk/";
 ...
 */
void
add_device_to_disklist(devname, devpath)
	char	*devname;
	char	*devpath;
{
	...
	if (mlp == NULL) {
		...
		strcpy(tctlr->ctype_name, dkinfo.dki_cname);
	 ...
	 * really exists.  A result of ID_MAY_BE_RESERVED
	 * means the disk may be reserved. Unlike scsi we can
	 * not be sure here since ipi controller firmware does
	 * reserve/release internally for short durations.
	 */
	if (dkinfo.dki_ctype == DKC_PANTHER) {
		i = id_rdwr(DIR_READ, search_file, (daddr_t) 0,
			1, (char *) &search_label, F_SILENT);
		switch (i) {
		case ID_MAY_BE_RESERVED:
	...
	/*
	 * Do the similar check for SCSI disks. In future, it will be good
...
#elif defined(i386)
			if (search_ctlr->ctlr_ctype->ctype_ctype != DKC_DIRECT)
			{
				search_dtype->dtype_flags |= DT_NEED_SPEFS;
			}
#else
	...
	if (search_parts == NULL) {
		...
		for (i = 0; i < NDKMAP; i++) {
			...
			search_parts->pinfo_map[i].dkl_cylno =
			    search_label.dkl_vtoc.v_part[i].p_start / ((int)
			    (search_label.dkl_nhead * search_label.dkl_nsect));
...
{
	int	x;
	struct	mctlr_list	*ctlrp, *tmpctlrp;
