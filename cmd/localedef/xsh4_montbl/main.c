...
/*
 *  Monetary table
 *  xsh4_montbl -i input_fname [-f charmap] [-o output_fname] [-s skip_count]
 */
#include "montbl.h"
#include "extern.h"
#include <unistd.h>

#define	DEFAULT_OUTPUT	"monetary"
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	if ((montbl_fd = creat(output_fname, 0777)) == -1) {
		(void) fprintf(stderr, gettext(
			"%s: Can't create %s.\n"),
			program, output_fname);
		exit(4);
	}
	if (output(montbl_fd) != 0) {
		ret_val = 1;
	}
	(void) close(montbl_fd);
	exit(ret_val);
}
