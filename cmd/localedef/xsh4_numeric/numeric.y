 ...
 */
%{
#ident	"@(#)numeric.y	1.9	95/03/06 SMI"
...
%token <key> _LC_NUMERIC END
%token <key> N_WORD_STR
%token <key> N_WORD_GRP
%start numeric
...
numeric		: starter n_words ender
		...
		;
starter		: _LC_NUMERIC eol
		;
ender		: END _LC_NUMERIC
		;
n_words 	: n_words n_word
		| n_word
		;
n_word		: N_WORD_STR quoted_string eol
		{
			(void) set_string($1->ktype, $2);
			free($2);
		}
		| N_WORD_GRP n_grp_list eol
		{
			(void) set_group(num_list, num_grp);
			num_list = 0;
		}
		;
n_grp_list	: n_grp_list ';' NUM
		{
...
coll_elements	: coll_elements coll_symbol
		{
			if (set_enbuf($2) == ERROR) {
				undefined("LC_NUMERIC", $2, lineno, &errorcnt);
			}
			$$ = &enbuf;
		}
		| coll_symbol
		{
			...
			if (set_enbuf($1) == ERROR) {
				undefined("LC_NUMERIC", $1, lineno, &errorcnt);
			}
			$$ = &enbuf;
		}
		;
...
{
	int		x;
	x = _yylex_common(input_file, _XSH4_NUMERIC);
	return (x);
}
