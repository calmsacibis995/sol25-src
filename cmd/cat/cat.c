...
#ident	"@(#)cat.c	1.20	95/06/15 SMI"	/* SVr4.0 1.17	*/
/*
**	Concatenate files.
*/
	...
	char *filenm;
{
	...
	if (mapsize != 0) {
		...
		if (read(fi_desc, &x, 1) == -1)
			read_error = 1;
		(void) madvise(bufferp, mapsize, MADV_SEQUENTIAL);
		mapoffset = 0;
		...
		for (;;) {
			...
			if (mmap(bufferp, mapsize, PROT_READ,
				...
				(void) munmap(bufferp, munmapsize);
				(void) lseek(fi_desc, mapoffset, SEEK_SET);
				return (1);
			}
	...
	while ((c = getc(fi)) != EOF) {
...
#endif EUC
		{
			...
			for (i = 1; --size > 0; i++) {
				if ((tc = getc(fi)) == EOF)
					goto _unget;
				mbuf[i] = (unsigned char)tc;
			}
			if (mbtowc(&wc, (const char *)mbuf, i) == i) {
				if (iswprint(wc)) {
					for (n = 0; n < i; n++)
						(void) putchar(mbuf[n]);
					continue;
