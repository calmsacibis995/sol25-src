...
#ident	"@(#)lpr.c	1.12	93/08/11 SMI"	/* SVr4.0 1.4	*/
...
#define TEXT_DOMAIN "SYS_TEST"
	...
	while(argc > 1 && argv[1][0] == '-') {
		...
		case 'r':       /* currently not supported */
			break;
		...
		default:
			printf(gettext("usage: lpr [-Pprinter] [-#num] [-Cclass] [-Jjob] [-Ttitle] [-i [indent]] [-1234 font] [-wcols] [-m] [-h] [-s] [-pltndgvcf]\n"));
			exit(0);
		}
	...
	** 	from the PRINTER or LPDEST environment variable, if set.
	**	Otherwise, use the system default printer.
	*/
	if (!Printer)
		if (!(Printer = (char *)getenv("PRINTER")))
			if (!(Printer = (char *)getenv("LPDEST")))
	...
	*/
	...
	if (stdinp > 0) {
		if (insize = savestdin()) {
			...
			printf(gettext("%s: standard input: empty input file\n"), Name);
			done(1);
			/*NOTREACHED*/
		}
	}
		...
		*p = 0;
    	...
    	for (filep = files; filep && *filep; filep++) {
			...
			if (f = fopen(*filep, "r")) {
				...
				printf(gettext("%s: cannot access %s\n"), Name, *filep);
	    	} else {
			if (**filep == '/')
				...
				fatal(gettext("can't get current directory\n"));
				/*NOTREACHED*/
			}
	    	}
	}
...
#endif
{
	...
	switch (status) {
		case MOK:
			return(strdup(pfix));
		case MNOMEM:
			printf(gettext("too many files - break up the job\n"));
			done(1);
			/*NOTREACHED*/
			break;
		default:
...
#endif
{
	...
	case MNOMEM:
		...
		break;
    	case MERRDEST:
		fatal(gettext("Printer queue is disabled"));
		/*NOTREACHED*/
		break;
   	case MNOOPEN:
...
#endif
{
	...
	if (nfiles > 1)
		if (stdinp > 0)
			printf(gettext("%d files and standard input"), nfiles);
		else
			printf(gettext("%d files"), nfiles);
	else if (nfiles == 1)
		if (stdinp > 0)
			printf(gettext("1 file and standard input"));
		else
			printf(gettext("1 file"));
	else
		if (stdinp > 0)
...
{
#if 0
	/* Formerly the defines required here were in filehdr.h */
	...
	if (stat(file, &stbuf)) {
		printf(gettext("%s: cannot stat %s\n"), Name, file);
		return(-1);
	}
	if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
		printf(gettext("%s: %s is a directory.\n"), Name, file);
		return(-1);
	}
	if (stbuf.st_size == 0) {
		printf(gettext("%s: %s is an empty file\n"), Name, file);
		return(-1);
	}
	...
	if (read(fd, buf, FILHSZ) == FILHSZ) {
		...
		*/
		...
		case NMAGIC:
		case ZMAGIC:
                        printf(gettext("%s: %s is an executable program"), Name, file);
			goto error1;
		case ARMAG:
                        printf(gettext("%s: %s is an archive file") , Name, file);
			goto error1;
                default:
			/*
			...
			if ( (magic > 0400 ) && (magic <0600)) {
       				printf(
				gettext("%s: %s is an archive or an executable program"),
								Name, file);
				goto error1;
			}
			if (buf[0] == '\100' && 
			   ...
			   (strcmp(cont_type, OTROFF) != 0)) {
				printf(gettext("%s: %s is a troff file, assuming '-t'\n"),
				Name, file);			
				cont_type = OTROFF;
			}
			break;
...
error1:
	printf(gettext(" and is unprintable\n"));
 	(void)close(fd);                
 	return(-1);       
}
