...
#ident	"@(#)printer.c	1.10	93/10/18 SMI"	/* SVr4.0 1.13	*/
...
do_printer(char **list)
{
	int msg;
	msg = (remote_cmd ?
		S_INQUIRE_REMOTE_PRINTER : S_INQUIRE_PRINTER_STATUS);
	while (*list) {
	...
	char *character_set)
{
	...
	*/
	...
	if (D && !(verbosity & (V_LONG|V_BITS)))
		...
		printf((prbufp->banner & BAN_ALWAYS ? 
                           gettext("\tBanner required\n"):
                           gettext("\tBanner not required\n")));
 ...
 */
static
int put_flt(char *printer, char *flt)
{
	char *dup, *p;
