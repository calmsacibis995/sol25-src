...
#ident	"@(#)svChild.c	1.16	93/11/19 SMI"	/* SVr4.0 1.9	*/
...
for (;;)
{
	...
	case	EINTR:
		...
		*/
		...
		if (AlarmTrapFlag)
		{
			...
			if (retryCount++ >= 5) {
				logit(LOG_ERR,
					"Could not connect to remote, exiting");
				exit(0);
			}
			logit(LOG_INFO, "Connection Attempt Retry(%d)",
				retryCount);
			...
			if (ConnectToRemoteChild ())
			{
				...
				*/
				...
				/* fix for bugid 1094795:
				 * it's possible for LpExecEvent() to lose
				 * the connection; in which case CIP is
				 * set to NULL via FreeConnectionInfo().
				 * thus you can't depend on CIP being
				 * non-null.
				 *
				 * so, if CIP is now NULL, we reschedule
				 * the attempt to contact the remote 
				 * system.
				 *
				 * N.B.. SIP->retry is ignored under 
				 * this circumstance, and the event is
				 * *always* rescheduled relatively
				 * quickly. This decision is based on
				 * field experience; if it fails again, 
				 * we'll end up going through the normal 
				 * sequence of events, anyway.
				 */

				if (CIP == NULL) {
					WriteLogMsg ("Dropped remote child, ");
					WriteLogMsg ("in LpExecEvent! Reschedule");
					alarm(25);
					continue;
				}
	...
	*/
	...
	if (SIP->retry == 0)
	{
		...
		{
			(void)	alarm (12);  /*  immediate retries.  */
		}
		continue;
	}
	else
	...
	if (retryCount++ >= 5) {
		logit(LOG_ERR, "Could not connect to remote, exiting");
		exit(0);
	}
	logit(LOG_INFO, "Connection Attempt Retry(%d)", retryCount);
...
_POLLIN:
	...
	*/
	...
	/*---------------------------------------------------------*/
	/*
	**  At this point we have a message to service.  It should be
	**  a S_SEND_JOB message.  
	*/
_switch:
	switch (mtype (msgp)) {
	case	S_SEND_JOB:
...
#ifdef DEBUG
                sprintf(debugBuf,"lpNet sendJob %d\n", jobId);
                WriteLogMsg(debugBuf);
#endif
...
#ifdef DEBUG
                sprintf(debugBuf,"lpNet receivedJob %d\n", jobId);
                WriteLogMsg(debugBuf);
#endif DEBUG
...
else
{
	...
	*/
	jobId = SendJob (CIP, listOfReplies_p, NULL, NULL, 0, 0);
...
*/
	...
	case	1:
		...
		if (fileList_pp == NULL) {
			char mesg[BUFSIZ];
			sprintf(mesg, "getjobfiles(%s)", requestFileName_p);
			TrapError (Fatal, Unix, FnName, mesg);
		}
		...
		sec = getsecure(requestFileName_p);
		if (sec != 0) {
			gid = sec->gid;
			uid = sec->uid;
		}
	...
	*/
sendJob:
	jobId = SendJob(CIP, dataList_p, srcFileList_p, destFileList_p, uid,
			gid);
	...
	do
	{
		...
		case	R_INQUIRE_REQUEST_RANK:
			if (getmessage (msgBuffer, R_INQUIRE_REQUEST_RANK,
			    &statusCode, &fooChar_p, &fooChar_p, &fooLong,
			    &fooLong, &fooShort, &fooChar_p, &fooChar_p,
			    &fooChar_p, &fooShort, &fooChar_p) == -1)
			{
				TrapError (Fatal, Unix, FnName, "getmessage");
			}
			if (statusCode != MOKMORE)
