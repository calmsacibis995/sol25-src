...
#ident	"@(#)parent.c	1.11	93/12/26 SMI"	/* SVr4.0 1.6	*/
...
static	int	NewChild (connectionInfo *, systemInfo *, process_Rank);
static	int	s5NewChild (connectionInfo *, systemInfo *, process_Rank);
static	int	bsdNewChild (connectionInfo *, systemInfo *, process_Rank);
...
static  void    SoftwarePollTrap(int); /* BUG_1103421 */
...
static  void    SoftwarePollTrap(); /* BUG_1103421 */
...
#ifdef	DEBUG
{
	...
	*/
	...
	{
		endpwent ();
		TrapError (Fatal, Unix, FnName, "getpwnam");
	}

	Lp_Uid = passwdp->pw_uid;
	Lp_Gid = passwdp->pw_gid;
...
_POLLIN:
	...
	*/
	...
	sip->systemType = SystemVSystem;  /* fix for sunsoft bugid 1103573*/
	FreeNetworkMsg (&networkMsg_p);
...
_POLLIN:
	...
	*/
	...
	sip->systemType = BerkeleySystem; /* fix for sunsoft bugid 1103573*/
...
#ifdef	DEBUG
{
	...
	*/
	...
	if (mkdir (pathp, (mode_t) 0751) < 0 && errno != EEXIST)
	{
		TrapError (Fatal, Unix, FnName, "mkdir");
	}
	if (chown (pathp, Lp_Uid, Lp_Gid) < 0)
	{
		TrapError (Fatal, Unix, FnName, "chown");
	}
	free (pathp);
	...
	{
		char tmbuf[120];	/* start fix for bugid 1112617	*/

		sprintf(tmbuf, "mkdir failed, pathp=%s, errno=%d\n", pathp, errno);
		WriteLogMsg(tmbuf);
		sprintf(tmbuf, "systemname=%s\n", SIP->systemName_p);
		WriteLogMsg(tmbuf);
		if (mkdir (Lp_Requests, (mode_t) 0775) < 0 && errno != EEXIST)
		{
			sprintf(tmbuf, "Tried to recover by making %s\n", Lp_Requests);
			WriteLogMsg(tmbuf);
			sprintf(tmbuf, "Failed! errno=%d\n", errno);
			WriteLogMsg(tmbuf);
			TrapError (Fatal, Unix, FnName, "mkdir");
		}
		sprintf(tmbuf, "Recovered by making %s!\n", Lp_Requests);
		WriteLogMsg(tmbuf);
		if (mkdir (pathp, (mode_t) 0770) < 0 && errno != EEXIST)
		{
			sprintf(tmbuf, "Couldn't recover 2nd time, errno=%d, ", errno);
			WriteLogMsg(tmbuf);
			sprintf(tmbuf, " path=%s\n", pathp);
			WriteLogMsg(tmbuf);
			TrapError (Fatal, Unix, FnName, "mkdir");
		}			/* end fix for bugid 1112617	*/
	}
	if (chown (pathp, Lp_Uid, Lp_Gid) < 0)
	{
		TrapError (Fatal, Unix, FnName, "chown");
...
/* begin BUG_1103421 */
/*
 * if the pipe closes, send us a SIGPOLL
 */
		if (ioctl(ProcessInfo.lpExec, I_SETSIG, S_HANGUP) < 0)
		        TrapError (Fatal, Unix, FnName,
				   "ioctl(lpExec, I_SETSIG, S_HANGUP)");
		else
		        signal(SIGPOLL, SoftwarePollTrap);
/* end  BUG_1103421 */
...
/*
 * For BUG_1103421,
 *  SoftwarePollTrap will be installed as a signal handler for
 *   SIGPOLL.  This will only get called when the LpExecPipe is
 *   closed by the parent (ie. parent exits or "Unmounts" pipe
 */
void SoftwarePollTrap(int sig)
{
#if defined(DEBUG)
        WriteLogMsg("shutdown requested via SIGPOLL, exiting");
#endif
        Exit(0);
}
