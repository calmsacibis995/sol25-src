...
#ident	"@(#)faults.c	1.7	93/10/18 SMI"	/* SVr4.0 1.2.1.4	*/
...
shortenReason(char *reason)
{
	...
	if ((ptr = strchr(reason,']')) && (strncmp(ptr,"]%%",3) == 0))
		*ptr = 0;
	 ...
	 int err)
{
	...
	if (pps->status & PS_SHOW_FAULT)
		...
		DEBUGLOG1("do alert instead of showing fault\n");
...
clear_printer_fault(register PSTATUS *pps, char *alert_text)
{
	...
	DEBUGLOG4("clear printer fault type: %s pr stat %x\nmsg (%s)\n",
		(pps->printer->fault_alert.shcmd ?
		    pps->printer->fault_alert.shcmd : "??"),
		pps->status, shortWhy);
