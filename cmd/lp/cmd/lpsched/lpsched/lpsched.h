...
#ident	"@(#)lpsched.h	1.15	94/12/12 SMI"	/* SVr4.0 1.9.1.11	*/
...
#endif	/* CHECK_CHILDREN */
#define	WHEN_STATUS	(30/CLOCK_TICK)	/* wait for reply from get status */

/*
 * Threshold of outstanding jobs to BSD system before poll.
 * Normally we don't expect to hit this threshold, since status queries
 * will cause old jobs to be removed from the local queue.
 */
#define MAX_BSD_JOBS 10
...
#else
#if	defined(DEBUG)
# define ENTRY(X)	int xyzzy = schedtrace("Trace: %s\n", X);
...
#define DEBUGLOG1(A)		schedlog(A)
#define DEBUGLOG2(A,B)		schedlog(A,B)
#define DEBUGLOG3(A,B,C)	schedlog(A,B,C)
#define DEBUGLOG4(A,B,C,D)	schedlog(A,B,C,D)
#define DEBUGLOG5(A,B,C,D,E)	schedlog(A,B,C,D,E)
#else
#define DEBUGLOG1(A)
...
void		alert ( int , ... );
void		askforstatus ( SSTATUS * , MESG *,short, ...);
...
void		load_bsd_stat(SSTATUS *, PSTATUS *, short, short, PSTATUS *);
...
void		remote_status_failed(SSTATUS *);
