...
#ident	"@(#)validate.c	1.15	94/06/17 SMI"	/* SVr4.0 1.11.1.10	*/
	 ...
	 int moving)
{
	...
	DEBUGLOG5("_validate(%x, %x, %x, %x, ",
		prs, pps, stop_pps, prefixp);
	DEBUGLOG2("%d)\n", moving );
	 ...
	 * form knowledge.
	 */
	if (prs && prs->request && prs->request->form &&
		(pcs || (pps && !(pps->status & PS_REMOTE)))) {
		...
		if ((pfs = search_ftable(prs->request->form))) {
			if (lp_or_root || allowed(prs->secure->user,
				...
				DEBUGLOG4(
				"couldn't allow (%s) isroot %d user (%s)\n",
					prs->request->form,lp_or_root,
					(prs ? prs->secure->user : "NULL"));
				ret = MDENYMEDIA;
				goto Return;
			}
		} else {
...
_chkopts(RSTATUS *prs, CANDIDATE *pc, FSTATUS *pfs)
{
	...
	if (wants_nobanner && !lp_or_root &&
	    (pc->pps->printer->banner & BAN_ALWAYS))
	{	/* if banner is required, remove "nobanner" from options */
		register char		**list;
