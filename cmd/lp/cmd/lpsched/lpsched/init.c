...
#ident	"@(#)init.c	1.14	94/03/02 SMI"	/* SVr4.0 1.10.1.8	*/
...
init_requests(void)
{
	...
	while((name = next_file(sysdir, &sysaddr)) != NULL) {
	    ...
	    *(table[count]->request) = *r;
		...
		DEBUGLOG2("Old %s with RS_NOTIFY\n", s->req_id);
		if (Access(file, F_OK) == -1) {
		    DEBUGLOG2("No notification file (%s)\n", file);
		    if (!(r->outcome & RS_CANCELLED)) {
			DEBUGLOG1("Defunct, removing request\n");
			Free(file);
			rmfiles(table[count], 0);
			freerstatus(table[count]);
			continue;
...
init_systems(void)
{
    ...
    ST_Size = ACLUSTERSIZE;
    ST_Count = 0;
    SStatus = Malloc(ACLUSTERSIZE * sizeof (void **));
    SStatus[0] = NULL;
	...
	*(ssp->system) = *s;
    ...
    DEBUGLOG2("Loaded %d system(s)\n", ST_Count);
}

void
init_remote_printer(PSTATUS *psp, PRINTER *p)
{
...
void
addone(void ***table, void *elem, int *tsize, int *tocc)
{
    ENTRY ("addone")
...
void
delone(void ***table, void *elem, int *tsize, int *tocc)
{
    ENTRY ("delone")
