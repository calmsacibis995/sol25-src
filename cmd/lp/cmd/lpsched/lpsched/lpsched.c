...
#pragma ident	"@(#)lpsched.c	1.19	94/09/30 SMI" 
...
#if defined(DEBUG)
    getrlimit(RLIMIT_NOFILE, &rlim);	/* Crank up the number of fds avail. */
    printf("old fd limit(hard:%d, soft:%d)\n", rlim.rlim_max, rlim.rlim_cur);
#endif

    rlim.rlim_max = rlim.rlim_cur = 1024 ; 
    setrlimit(RLIMIT_NOFILE, &rlim);

#if defined(DEBUG)
    getrlimit(RLIMIT_NOFILE, &rlim);
    printf("new fd limit(hard:%d, soft:%d)\n", rlim.rlim_max, rlim.rlim_cur);
#endif
...
#if	defined(MDL)
# ident	"@(#)lpsched.c	1.19	94/09/30 SMI"
    /*
...
process()
{
    ...
    DEBUGLOG1("Starting poll for work to do\n");
    for (EVER) {
	RESTART_ENTRY
