...
#ident	"@(#)daisyforms.c	1.12	94/03/04 SMI"
		/* SVr4.0 1.5.1.4	*/
...
queue_form(register RSTATUS *prs, FSTATUS *pfs)
{
	...
	if ((prs->form = pfs)) {
		prs->form->requests++;
		DEBUGLOG4("queue_form %d %s %x\n", prs->printer,
			prs->printer->printer->name, prs->printer->status);
...
#define NALERT	pf->alert
	...
	DEBUGLOG3("check_form_alert: trigger %d max requ %d\n", trigger,
		requests_waiting);
...
{
	ENTRY ("isFormUsableOnPrinter")

	int i;

	i = trayWithForm(pps,pfs,0,1);
	DEBUGLOG4("isFormUsable %s on %s: %d\n",
		(pfs ? pfs->form->name : "none"), pps->printer->name, i);
	return(i >= 0 );
}
int
isFormMountedOnPrinter(PSTATUS *pps, FSTATUS *pfs)
{
	ENTRY ("isFormMountedOnPrinter")

	int i;

	i = trayWithForm(pps,pfs,0,0);
	DEBUGLOG4("isFormMounted %s on %s: %d\n",
		(pfs ? pfs->form->name : "none"), pps->printer->name, i);
	return(i >= 0 );
}
...
max_requests_needing_form_mounted(FSTATUS *pfs)
{
	...
	BEGIN_WALK_BY_FORM_LOOP (prs, pfs)
		if ((pps = prs->printer) && (!isFormMountedOnPrinter(pps,pfs)))
			if (++pps->nrequests >= max)
				max = pps->nrequests;
	END_WALK_LOOP
	if (NewRequest)
		if ((pps = NewRequest->printer) &&
