...
#ident	"@(#)msgs.c	1.7	93/10/18 SMI"	/* SVr4.0 1.9.1.5	*/
		...
		case MD_CHILD:
			...
			if (slot < ET_Size) {
				...
				if (debug & (DB_EXEC|DB_DONE)) {
					execlog("DROP! slot %d, pid %d\n",
						slot, Exec_Table[slot].pid);
					execlog("%e", &Exec_Table[slot]);
				}
#endif
				DEBUGLOG3("DROP! slot %d, pid %d\n",
					slot, Exec_Table[slot].pid);
			} else {
				DEBUGLOG2("do SHUTDOWN event %d\n", md->event);
...
# endif
	...
	DEBUGLOG2("MESSAGE ACCEPTED: client %#0x\n", md);
#endif
	if (type != S_GOODBYE)
	{
	    md->wait = 0;
