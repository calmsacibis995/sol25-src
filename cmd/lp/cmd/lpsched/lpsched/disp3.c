...
#ident        "@(#)disp3.c 1.13     93/10/19 SMI"        /* SVr4.0 1.10.1.3      */
# include	"dispatch.h"
...
static void
remount_remote_form(char *m, MESG *md, short type, register PSTATUS *pps,
	char *formName, short trayNum)
{
	short returnType;
	returnType = type+1; /* R_ type always follow S_ types */
	...
	if ( pps->system ) {
		...
		DEBUGLOG4("%s -> remote for (%s) (%s)\n", dispatchName(type),
			pps->printer->name,
			(pps->remote_name ? pps->remote_name : "??"));
		if (Redispatch  == 0) {
			DEBUGLOG5("askforstatus %d %d %s %d\n", type, pps,
				(formName ? formName : "NULL"), trayNum);
			askforstatus(pps->system, md, type, pps, formName,
				trayNum);
			if (waitforstatus(m, md) == 0)
				return;
		} 
		if ((ppm = pps->system->pmlist) &&
		    ((ii = mtype(*ppm)) == returnType)) {
			(void) getmessage(*ppm, returnType, &h1);
			DEBUGLOG3("forward message %s %d\n",
				dispatchName(ii), h1);
			mputm(md, returnType, h1);
		} else {
			DEBUGLOG2("send  %s MNOINFO (null pmlist)\n",
				dispatchName(returnType));
			mputm(md, returnType, MNOINFO);
		}
	} else {
		DEBUGLOG2("send  %s MNODEST (null pmlist)\n",
			dispatchName(returnType));
		mputm(md, returnType, MNODEST);
	}
}
...
remount_form(register PSTATUS *pps, FSTATUS *pfs, short trayNum)
{
	...
	DEBUGLOG5("remount_form(%x, %x(%s), %d)\n", 
		pps, pfs, (pfs ? pfs->form->name : "NULL"), trayNum);
	trayNum--; /* make zero based */
	DEBUGLOG2("remount_form was %d\n", pps->forms);
	if (pps->forms && (pps->forms[trayNum].form == pfs)) {
		pps->forms[trayNum].isAvailable = ((pfs ||
					 (!LP_TRAY_UNMOUNT)) ? 1 : 0);
					 /* force it */
		DEBUGLOG2("remount_form unchanged %d\n", trayNum);
		return;	/* nothing to do */

	} else if ((!pps->forms) && (!pfs)) {
		DEBUGLOG1("remount_form unchanged, null pfs\n");
		return;	/* nothing to do */
	}
	...
	if (pps->forms && pps->forms[trayNum].form) {
		...
		DEBUGLOG2("remount_form remove old %d\n", trayNum);
	 ...
	 */
	if (pfs) {
		DEBUGLOG4("remount_form add %x(%s) to tray %d\n",
			 pfs, (pfs ? pfs->form->name : "NULL"), trayNum);
               
/*
 * BUGID 1137632 - add "trays" to printer if none exist, so forms can 
 *	be mounted
 */ 
		if (pps && !pps->forms) {
			DEBUGLOG5("remount_form(%s, %s, %d) adding %d trays\n",
				pps->printer->name, pfs->form->name, trayNum,
				(trayNum +1)); 
...
/*
 * BUGID 1123879 - added pps to check, so that it would short circuit if
 *	pps was NULL, instead of SEGV
 */
		if (pps && pps->forms && (pps->numForms > trayNum)) {
			DEBUGLOG3("remount_form: add(form: %s tray: %d)\n",
				pfs->form->name, trayNum);
			pps->forms[trayNum].form = pfs;
			pps->forms[trayNum].isAvailable = 1;
			pfs->mounted++;
		} else {
			DEBUGLOG3("remount_form: add(form: %s, tray: %d) - NOTRAY\n",
				pfs->form->name, trayNum+1 );
...
s_max_trays(char *m, MESG *md)
{
	...
	/* If the printer is currently printing, we can't disturb it. */
		...
		if (!(ppfs = Realloc(pps->forms,numTrays * sizeof(PFSTATUS))))
			...
			DEBUGLOG3("s_max_trays realloc %d -> %d\n",
...
s_mount(char *m, MESG *md)
{
	...
	DEBUGLOG5("s_mount: getmessage(%s, S_MOUNT, %s, %s, %s)\n",
		m, printer, form, pwheel_name);
...
s_mount_tray(char *m, MESG *md)
{
	...
	else if ( pps->status & PS_REMOTE ) {
		remount_remote_form(m, md, S_MOUNT_TRAY, pps, form, trayNum);
		return;
	}
...
s_unmount_tray(char *m, MESG *md)
{
	 ...
	 */
		...
		remount_remote_form (m,md,S_UNMOUNT_TRAY,pps, NULL,trayNum);
		return;
		}
	/*
	 * How about the tray?
...
_unload_form(register FSTATUS *pfs)
{
	 ...
	 */
	for (; pps < ppsend; pps++)
		if ((ppfs = pps->forms) && ((numForms = pps->numForms) > 0))
