...
#ident	"@(#)lpsystem.c	1.15	94/06/16 SMI"	/* SVr4.0 1.8	*/
...
#define TEXT_DOMAIN "SYS_TEST"
	...
	case 't':
		...
		{
			(void)	fprintf (stderr,
		gettext("ERROR: Supported protocols are \"%s\" and \"%s\".\n"),
...
/*
*/
char *s5warn = "Warning: Using the s5 protocol.\n \
		...
		if (Protocolp)
		{
			...
			if ( STREQU(NAME_S5PROTO, Protocolp) ) {
				sysbuf.protocol = S5_PROTO;
					(void) fprintf(stderr, gettext(s5warn));
			}
			else
				sysbuf.protocol = BSD_PROTO;
		}
		else {
			if (sysbuf.protocol == S5_PROTO)
				(void) fprintf(stderr, gettext(s5warn));
		}
		if (Timeoutp)
		{
...
#endif
{
	...
	*/
	...
	{
		(void)	fprintf (stderr, gettext("ERROR:  "));
		(void)	perror (gettext("putmessage"));
		(void)	exit (2);
	}
	if (msend (msgbuf) < 0)
	{
