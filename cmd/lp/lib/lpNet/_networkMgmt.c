...
/*
*/
#ident	"@(#)_networkMgmt.c	1.19	94/03/31 SMI"	/* SVr4.0 1.6	*/
...
char	*systemName_p;
{
	...
	/*
	*/
		int		acflag = 0; /* 1112869: anon client flag */
		char		buffer [128],
				*token_p;
		boolean		found = False;
		systemInfo	*sip;
	...
	*/
	do
	{
		if (! fgets (buffer, sizeof (buffer), NetworkInfoFile_p))
			goto	errorReturn_1;
		...
		*/
		...
		/* start changes for bugid's 1103573 & 1112869.
		 * if the /etc/lp/Systems file contains a line of the form:
		 *	+:x:-:s5:-:n:1:-:-:
		 * I.e. the system name field is "+", then we accept *all*
		 * remote connections, unless the remote system
		 * is otherwise specified in /etc/lp/Systems.
		 */
		if (strcmp ("+", token_p) == 0) {
			TRACEP ("Postive MATCH for ALL incoming connections")

			/* assign system name from the name of the
			 * incoming connection.
			 */
			if ((sip->systemName_p=strdup(systemName_p)) == NULL)
				TrapError (Fatal, Unix, FnName, "strdup");

			/* Password */
			if ((sip->systemPassword_p=strdup ("x")) == NULL)
				TrapError (Fatal, Unix, FnName, "strdup");

			/* Skip Reserved field */

			/* System Type is irrelevant here. It will be
			 * picked up later on in listen[S5,BSD]Event().
			 * default to S5.
			 */
			...
			/* Timeout - use default of Never		*/
			sip->timeout = -1;		/* Never timeout */

			/* Retry - use default of 10 minutes	*/
			sip->retry = 10;	/* Default:  10 minutes	*/

			/* skip the next two reserved fields, and comments */

			sip->ncProtoFamily_p = NULL;
			sip->ndHostServ.h_host = strdup (sip->systemName_p);
			sip->ndHostServ.h_serv = "listen";
			sip->ndOption = 0;

			/* fix for bugid 1112869:
			 * DON'T return here! keep scanning the Systems
			 * file. This information gets used if and only
			 * if no other entry in the Systems file can 
			 * be found!
			 */
			 acflag = 1;
		}
		/* end of changes for sunsoft bugid 1103573. */
	...
	*/
errorReturn_1:
	/* start fix for bugid 1112869:
	 * if acflag is non-zero, then we're supposed to use the
	 * default values. otherwise, no system entry for this name
	 * is known. we *only* get here if we're finished scanning
	 * /etc/lp/Systems. acflag *only* gets set if we've found
	 * a "+" system name.
	 */
	if (acflag == 1) {
		TRACEP ("Using default parameters for anon. client support")
		EXITP
		return sip;
	}
	/* end fix for bugid 1112869	*/
...
char		*netidp;
{
	...
	*/
	...
	DEFINE_FNNAME("GetSystemInfoByAddr")
	...
	*/
	...
	for (i=0; i < hostservlistp->h_cnt; i++)
	{
		...
		fprintf(stderr, "host names: of %d (%s)\n",
			 hostservlistp->h_cnt,
			 hostservlistp->h_hostservs[i].h_host);
		sip =
...
systemInfo	*sip;
{
	...
	*/
	...
	TRACEs (sip->ndHostServ.h_host)
	TRACEs (sip->ndHostServ.h_serv)
...
PrintTransportInfo (cip->providerInfo_p);
...
#ifdef	DEBUG
		{
		char *returnBinding_addr;

		if ((returnBinding_addr = taddr2uaddr(cip->netConfig_p,
		    &cip->returnBinding_p->addr)) != NULL)
				TRACEs(returnBinding_addr);
		}
#endif
		...
		*/
		...
		for (i=0; i < cip->ndAddrList_p->n_cnt; i++)
		{
			...
			if ((receiveCall_addr = taddr2uaddr(cip->netConfig_p,
			    &cip->receiveCall_p->addr)) != NULL)
				TRACEs(receiveCall_addr);
			...
			if (t_getname (cip->fd, &nbuf, LOCALNAME) == -1) {
				...
				if ((nbuf_addr = taddr2uaddr(cip->netConfig_p,
				    &nbuf)) != NULL)
					TRACEs(nbuf_addr);
			}
			}
...
{
	void			*handle_p;
	struct netconfig	*pnetconf;
	char			*addr;

	if ((handle_p = setnetpath ()) != NULL) {
	        while ((pnetconf = getnetpath(handle_p)) != NULL) {
			printf("pnetconf->nc_device: %s", pnetconf->nc_device);
 
			if (pnetconf->nc_semantics == NC_TPI_CLTS)
				continue;
			if (strcmp(NC_INET, pnetconf->nc_protofmly) != 0)
				continue;

			if ((addr = taddr2uaddr(pnetconf,
			    &cip->receiveCall_p->addr)) != NULL)
				TRACEs(addr);
			break;
		}
...
connectionInfo	*cip;
{
	...
	/*
	*/
	DEFINE_FNNAME("DisconnectSystem")
...
int		timeout;
{
	...
	*/
		pollfd_t	pfd;
	DEFINE_FNNAME("PollNetworkEvent")
...
systemInfo	**sipp;
{
	...
	if ((*sipp)->extensions != NULL)
		free ((*sipp)->extensions);
	free (*sipp);
...
connectionInfo	*cip;
{
	...
	/*
	*/
	DEFINE_FNNAME("FreeConnectionInfoMembers")
	...
	*/
	...
	if (cip->returnBinding_p != NULL) {
		t_free ((char *)cip->returnBinding_p, T_BIND);
		cip->returnBinding_p = NULL;
	}
	if (cip->sendCall_p != NULL) {
		t_free ((char *)cip->sendCall_p, T_CALL);
		cip->sendCall_p = NULL;
	}
	if (cip->receiveCall_p != NULL) {
		t_free ((char *)cip->receiveCall_p, T_CALL);
