...
/*
*/
#ident	"@(#)networkMgmt.c	1.9	93/11/18 SMI"	/* SVr4.0 1.6	*/
...
static boolean
SendFile(connectionInfo *, boolean, char *, char *, uid_t, gid_t);
...
*/
int
SendJob(connectionInfo *cip, list *dataList_p, list *srcFileList_p,
	list *destFileList_p, uid_t uid, gid_t gid)
{
	/*----------------------------------------------------------*/
	/*
	*/
	...
	*/
	...
	for (i=0; i < listLength-1; i++) {
		if (! SendFile(cip, endOfJob, ListMember(srcFileList_p, i),
		    destFileList_p == NULL ? NULL :
			ListMember(destFileList_p, i), uid, gid))
		{
			int	save = errno;
			if (Connected (cip))
				SendJobControlMsg (cip, JobAborted);
	...
	if (! SendFile(cip, endOfJob, ListMember (srcFileList_p, i),
		destFileList_p == NULL ? NULL : ListMember (destFileList_p, i),
		uid, gid))
	{
		int	save = errno;
		if (Connected (cip))
...
*/
static boolean
SendFile(connectionInfo *cip, boolean lastFileOfJob, char *srcPath_p,
	char *destPath_p, uid_t uid, gid_t gid)
{
	/*----------------------------------------------------------*/
	/*
	*/
...
#define FRAG_BUFF_SIZE 32768
	...
	*/
	...
	{
		FragmentBuffer_p = malloc (FRAG_BUFF_SIZE);
	...
	*/
	...
	switch(rstat(srcPath_p, new_path, &statInfo, uid, gid)) {
	case -1:
		TRACE (errno)
		EXITP
		return	False;
	...
	*/
	...
	fragmentMsg.destPathp		  =
		destPath_p == NULL ? src_path : destPath_p;

	nBytes = 0;
	do {
		int len;

		if ((len = read(fd, FragmentBuffer_p, FRAG_BUFF_SIZE)) == -1) {
			int	save = errno;
			(void) close (fd);
			errno = save;
			TRACE (errno)
		...
		fragmentMsg.fragment.fragment_len = len;
		nBytes += len;
...
_open:
	...
	{
		if ((fd = open (destPath_p, O_WRONLY|O_APPEND|O_EXCL, 0600)) == -1)
        	{
	...
	*/
	...
	do {
		...
		FreeNetworkMsg((void **)&fileFragmentMsg_p);
...
int		sizeOfData;
{
	...
	*/
	...
	JobControl.endOfJob    = (u_char) endOfJob;
...
int	priority;
{
	JobControl.priority = (u_char) priority;
	return;
}
/*==================================================================*/

...
networkMsgType	msgType;
{
	...
	*/
	static	int		MsgId		= 0;
	DEFINE_FNNAME("EncodeNetworkMsgTag")
...
{
	DEFINE_FNNAME("SendJobControlMsg")

	ENTRYP

	JobControl.controlCode = (u_char) controlCode;
...
fileFragmentMsg	*fileFragmentMsg_p;
{
	...
	/*
	*/
	DEFINE_FNNAME("SendFileFragmentMsg")
...
void		**networkMsg_pp;
{
	...
	/*
	*/
	DEFINE_FNNAME("ReceiveNetworkMsg")
...
*/
void
FreeNetworkMsg(void **networkMsg_pp)
{
...
/*
 * root stat
 * lpNet will try and access the users file in order to send it to the print
 * server.  The path lpNet uses is /var/spool/lp/tmp/.net/tmp/<name>/<file>
 * which has restricted permissions on many of the components.  <file> is
 * a symlink to the users file, which may not be accesible since lpNet runs
 * as root but the users file might have a restricted mode (i.e. 400).
 * NFS will map root to NOBODY, which will prevent access.  Thus, we try
 * access the file through the link as root, but if that fails, we set our
 * permissions down to the user and try again.
 *
 * Return -1 if fail, 0 if normal stat is ok and 1 if we had to follow the
 * symlink and the caller should use newpath as the file to open for printing.
 * newpath is set to the real file that
 * /var/spool/lp/tmp/.net/tmp/<name>/<file> is symlinked to.  It should always
 * be a buffer of MAXPATHLEN bytes.  On exit from this routine, the euid and
 * egid might be set to the user, so the caller should set it back when it's
 * done (i.e. the open has completed).
 */
int
rstat(char *path, char *newpath, struct stat *buf, uid_t uid, gid_t gid)
{
	int len;
	...
	if (lstat(path, buf) != 0 || ! (buf->st_mode & S_IFLNK))
		return (-1);

	len = readlink(path, newpath, MAXPATHLEN - 1);
	if (len == -1)
		return (-1);
