...
#ident	"@(#)dd.c	1.19	95/09/14 SMI"	/* SVr4.0 1.18	*/
/*
**	convert and copy
*/
...
#define	USAGE\
	...
	"	   [,ebcdic][,ibm][,block|unblock][,lcase|ucase][,swab]\n"\
...
char **argv;
{
	...
	register int trunc;		/* whether output file is truncated */
...
long big;
{
	register char *cs;
	register long n;
	register long cut = BIG / 10;	/* limit to avoid overflow */
...
*flsh()
{
	...
	if (obc)			/* don't flush if the buffer is empty */
	{
		...
		if (bc != oc) {
			...
			(void) fprintf(stderr,
				"dd: %s wrote %d bytes, expected %d\n",
				gettext("unexpected short write"), bc, oc);
			term(2);
		}
		obc -= oc;
		op = obuf;
...
stats()
{
	(void) fprintf(stderr, gettext("%u+%u records in\n"), nifr, nipr);
	(void) fprintf(stderr, gettext("%u+%u records out\n"), nofr, nopr);
	if (ntrunc) {
		(void) fprintf(stderr,
		    gettext("%u truncated record%s\n"),
		    ntrunc, ntrunc != 1 ? "s" : "");
	}
}
