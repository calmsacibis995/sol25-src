...
#ident	"@(#)ps.c	1.30	95/07/21 SMI"	/* SVr4.0 1.62.20.1	*/
...
#define	MAXGROUP 8	/* max number of chars in group that will be printed */
...
struct gdata {
	...
	char	name[MAXGROUP];	/* group name, may not be null terminated */
};
...
static	int	ntty = 0;
static	pid_t	pid[SIZ];	/* for p option */
static	int	npid = 0;
static	pid_t	grpid[SIZ];	/* for g option */
static	int	ngrpid = 0;
static	pid_t	sessid[SIZ];	/* for s option */
...
static	char	*gettty(prpsinfo_t *, register int *);
static	int	prcom(int, prpsinfo_t *);
static	void	prtpct(u_short, int);
static	void	print_time(time_t, int);
static	void	print_field(prpsinfo_t *, struct field *, char *);
static	void	pr_fields(prpsinfo_t *, char *);
static	int	search(pid_t *, int, pid_t);
static	int	uconv(struct udata *, int *);
static	int	gconv(struct gdata *, int *);
static	int	ufind(uid_t, struct udata *, int);
static	int	gfind(gid_t, struct gdata *, int);
static	int	psread(int, char *, unsigned int);
static	void	pswrite(int, char *, unsigned int);
static	void	prtime(timestruc_t, int, int);
static	void	przom(prpsinfo_t *);
static	int	num(char *);
static	int	namencnt(char *, int, int);

main(int argc, char **argv)
{
	struct udata euid_tbl[SIZ];	/* table to store selected euid's */
	int	neut = 0;		/* counter for euid_tbl */

	struct udata ruid_tbl[SIZ];	/* table to store selected real uid's */
	int	nrut = 0;		/* counter for ruid_tbl */

	struct gdata egid_tbl[SIZ];	/* table to store selected egid's */
	int	negt = 0;		/* counter for egid_tbl */

	struct gdata rgid_tbl[SIZ];	/* table to store selected real gid's */
	int	nrgt = 0;		/* counter for rgid_tbl */
...
#define	TEXT_DOMAIN	"SYS_TEST"	/* Use this only if it weren't */
	...
	while ((c = getopt(argc, argv, "jlfceAadt:p:g:u:U:G:n:s:o:")) != EOF)
		switch (c) {
		case 'l':		/* long listing */
			lflg++;
			break;
...
#define	TSZ	30
	...
	gettext("ps: %s is an invalid non-numeric argument for -g option\n"),
		...
		case 'u':		/* effective user name or number */
			...
			do {
				getarg();
				if (neut < SIZ)
					(void) strncpy(euid_tbl[neut++].name,
					    parg, MAXLOGIN);
			} while (*p1);
			break;
		case 'U':		/* real user name or number */
			...
			do {
				getarg();
				if (nrut < SIZ)
					(void) strncpy(ruid_tbl[nrut++].name,
					    parg, MAXLOGIN);
			} while (*p1);
			break;
		case 'G':		/* real group name or number */
			...
			do {
				getarg();
				if (nrgt < SIZ)
					(void) strncpy(rgid_tbl[nrgt++].name,
					    parg, MAXGROUP);
			} while (*p1);
			break;
	...
	if (!(aflg|Aflg|dflg|Gflg|Uflg|uflg|tflg|pflg|gflg|sflg)) {
		...
		(void) sprintf(pname, "%s/%ld", procdir, getpid());
		if ((procfd = open(pname, O_RDONLY)) < 0 ||
		    ioctl(procfd, PIOCPSINFO, (char *) &info) != 0 ||
	...
	i += uconv(euid_tbl, &neut);
	i += uconv(ruid_tbl, &nrut);
	i += gconv(egid_tbl, &negt);
	i += gconv(rgid_tbl, &nrgt);
	if (i)
		exit(1);
	...
	while (dentp = readdir(dirp)) {
		...
		prpsinfo_t info;	/* process information from /proc */
...
retry:
		...
		else if (uflg && ufind(info.pr_euid, euid_tbl, neut))
			found++;	/* puid in u option arg list */
		else if (Uflg && ufind(info.pr_uid, ruid_tbl, nrut))
			found++;	/* puid in U option arg list */
#ifdef NOT_YET
		else if (gflg && gfind(info.pr_egid, egid_tbl, negt))
			found++;	/* pgid in g option arg list */
#endif	/* NOT_YET */
		else if (Gflg && gfind(info.pr_gid, rgid_tbl, nrgt))
			found++;	/* pgid in G option arg list */
		else if (gflg && search(grpid, ngrpid, info.pr_pgrp))
...
{
	static char usage1[] =
	    "ps [ -aAdeflcj ] [ -o format ] [ -t termlist ]";
	static char usage2[] =
 ...
 */
static char *
gettty(prpsinfo_t *psinfo, register int *ip)
{
	register int	i;
 ...
 */
static int
prcom(int found, prpsinfo_t *psinfo)
{
	register char	*cp;
	register char	*tp;
	long	tm;
	...
	if (lflg) {
		(void) printf("%2lx %c",			/* F S */
		    psinfo->pr_flag & 0377, psinfo->pr_sname);
		if (fflg)
			(void) printf(" ");
	}
	if (fflg) {
	...
	if (jflg) {
		(void) printf("%6ld", psinfo->pr_pgrp);		/* PGID */
		(void) printf("%6ld", psinfo->pr_sid);		/* SID  */
	}
	if (cflg) {
		(void) printf("%5s", psinfo->pr_clname);	/* CLS  */
		(void) printf("%4ld", psinfo->pr_pri);		/* PRI	*/
	} else if (lflg || fflg) {
		(void) printf("%3d", psinfo->pr_cpu & 0377);	/* C   */
		if (lflg) {					/* PRI NI */
			/*
			 ...
			 */
			(void) printf("%4d", psinfo->pr_oldpri);
			if (psinfo->pr_oldpri != 0)
				(void) printf("%3d", psinfo->pr_nice);
			else
				(void) printf(" %2.2s", psinfo->pr_clname);
		}
	}
	if (lflg) {
		(void) printf("%9lx%7ld",
		    (long)psinfo->pr_addr, psinfo->pr_size);	/* ADDR SZ */
		if (psinfo->pr_wchan)
			(void) printf("%9lx",
			    (long)psinfo->pr_wchan);		/* WCHAN */
		else
			(void) printf("         ");
	}
	if (fflg)						/* STIME */
	...
	if (psinfo->pr_time.tv_nsec > 500000000)
		tm++;
	(void) printf("%2ld:%.2ld", tm / 60, tm % 60);
	 ...
	 */
	cp = psinfo->pr_psargs;
	while (cp < &psinfo->pr_psargs[PRARGSZ]) {
		if (*cp == 0)
			break;
...
static void
print_field(prpsinfo_t *psinfo, struct field *f, char *tp)
{
	...
	case F_PPID:
		...
		break;
	case F_PGID:
		(void) printf("%*ld", width, psinfo->pr_pgrp);
		break;
	case F_SID:
		(void) printf("%*ld", width, psinfo->pr_sid);
		break;
	case F_OPRI:
		(void) printf("%*d", width, psinfo->pr_oldpri);
		break;
	case F_PRI:
		(void) printf("%*ld", width, psinfo->pr_pri);
		break;
	case F_F:
		mask = 0xffffffffUL;
		if (width < 8)
		...
		break;
	case F_S:
		(void) printf("%*c", width, psinfo->pr_sname);
		break;
	case F_C:
		(void) printf("%*d", width, psinfo->pr_cpu);
		break;
	case F_PCPU:
	...
	case F_PMEM:
		...
		break;
	case F_OSZ:
		(void) printf("%*ld", width, psinfo->pr_size);
		break;
	case F_VSZ:
		(void) printf("%*ld", width, psinfo->pr_bysize / 1024);
		break;
	case F_RSS:
		(void) printf("%*ld", width, psinfo->pr_byrssize / 1024);
		break;
	case F_NICE:
		/* if pr_oldpri is zero, then this class has no nice */
		if (psinfo->pr_oldpri != 0)
			(void) printf("%*d", width, psinfo->pr_nice);
		else
			(void) printf("%*.*s", width, width, psinfo->pr_clname);
		break;
	case F_CLASS:
		(void) printf("%*.*s", width, width, psinfo->pr_clname);
		break;
	...
	case F_WCHAN:
		if (psinfo->pr_wchan)
			(void) printf("%*lx", width, (long)psinfo->pr_wchan);
		else
	...
	case F_ARGS:
		 ...
		 * PRARGSZ == length of cmd arg string.
		 */
		psinfo->pr_psargs[PRARGSZ] = '\0';
		/* LINTED may be used before set: length */
		...
		if (*cp == '\0') {
			...
			(void) printf("[ %.*s ]", width-4, psinfo->pr_fname);
		}
...
static void
pr_fields(prpsinfo_t *psinfo, char *tp)
{
	struct field *f;
...
static int
uconv(struct udata *utbl, int *pn)
{
	struct passwd *pwd;
	int i, j;
	int n = *pn;
	...
	for (i = 0; i < n; i++) {
		/*
		 * If not found and name is numeric, ask for numeric id
		 */
		if ((pwd = getpwnam(utbl[i].name)) == NULL &&
		    utbl[i].name[0] >= '0' &&
		    utbl[i].name[0] <= '9')
		 ...
		 */
		if (pwd != NULL) {
			utbl[i].uid = pwd->pw_uid;
			(void) strncpy(utbl[i].name, pwd->pw_name, MAXLOGIN);
		} else {
			rval++;
			(void) fprintf(stderr,
			...
			for (j = i + 1; j < n; j++) {
				(void) strncpy(utbl[j-1].name,
				    utbl[j].name, MAXLOGIN);
			}
			n--;
			i--;
		}
	}
	*pn = n;	/* in case it changed */
	return (rval);
}

static int
gconv(struct gdata *gtbl, int *pn)
{
	struct group *grp;
	int i, j;
	int n = *pn;
	...
	for (i = 0; i < n; i++) {
		 ...
		 * If not found and name is numeric, ask for numeric id
		 */
		if ((grp = getgrnam(gtbl[i].name)) == NULL &&
		    gtbl[i].name[0] >= '0' &&
		    gtbl[i].name[0] <= '9')
		 ...
		 */
		if (grp != NULL) {
			gtbl[i].gid = grp->gr_gid;
			(void) strncpy(gtbl[i].name, grp->gr_name, MAXGROUP);
		} else {
			rval++;
			(void) fprintf(stderr,
 ...
 */
static int
ufind(uid_t puid, struct udata *utbl, int n)
{
	register int i;
...
/*
 * Return 1 if pgid is in table, otherwise 0.
 */
static int
gfind(gid_t pgid, struct gdata *gtbl, int n)
{
	register int i;
