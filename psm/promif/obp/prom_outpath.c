...
#pragma	ident	"@(#)prom_outpath.c	1.25	94/12/10 SMI"
...
prom_dnode_to_pathname(register dnode_t nodeid, register char *outbuf)
{
	...
	/*
	 * If we are a direct child of sbus, on a sun4c with a V0 PROM,
	 * then we have to normalize the addresses in the reg property:
	 *
	 * V2high = (V0low >> 25) & 3	(i.e. extract bits 25 and 26)
	 * V2low  = V0low & 1ffffff	(i.e. low 25 bits)
	 *
	 * For sun4e: the shift is 26 bits and the mask is 3ffffff.
	 */

	v0_shift = 25;
	v0_mask = 0x1ffffff;
	if ((prom_getversion() == 0) &&
	    (prom_getprop(parent, OBP_NAME, namebuf) != -1) &&
	    (prom_strcmp(namebuf, "sbus") == 0))
		(void) prom_sprintf(p, "@%x,%x",
		    (regs[0].lo >> v0_shift) & 3,
		    regs[0].lo & v0_mask);
	else
		(void) prom_sprintf(p, "@%x,%x", regs[0].hi,
		    regs[0].lo);
}
