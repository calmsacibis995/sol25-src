...
#pragma ident	"@(#)prom_init.c	1.21	94/12/10 SMI"
 ...
 *
 * XXX	We should also lookup the cputype here on SUNMON machines
 *	so that we -don't- have to pass the cputype across the
 *	interface (e.g. in prom_stdoutpath())
 */
/*ARGSUSED*/
void
prom_init(char *pgmname, void *cookie)
{
	 ...
	 *  Any prom with no such number is deemed "SUNMON".
	 */
	...
	case SUNMON_ROMVEC_VERSION:
		PROMIF_DPRINTF(("%s: SUNMON romvec\n", pgmname));
		break;
	...
	case OBP_V3_ROMVEC_VERSION:
	{
		...
		node = prom_findnode_byname(prom_nextnode(0), "openprom", stk);
		prom_stack_fini(stk);
