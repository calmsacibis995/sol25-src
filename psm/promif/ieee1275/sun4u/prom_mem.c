...
#pragma ident	"@(#)prom_mem.c	1.3	95/01/18 SMI"
 ...
 * Returns *base_hi, *base_lo only if successful.
 *
 * This routine is suitable for platforms with 2-cell physical addresses
 * and a single size cell in the "memory" node.
 */
int
prom_allocate_phys(u_int size, u_int align, u_int *base_hi, u_int *base_lo)
{
	cell_t ci[10];
	int rv;
	...
	ci[6] = p1275_uint2cell(size);		/* Arg4: SA2: size */
	...
	*base_hi = p1275_cell2uint(ci[8]);	/* Res2: SR1: base.hi */
	*base_lo = p1275_cell2uint(ci[9]);	/* Res3: SR2: base.lo */
	return (0);
}
 ...
 */
int
prom_claim_phys(u_int size, u_int phys_hi, u_int phys_lo)
{
	cell_t ci[10];
	int rv;
	ihandle_t imemory = prom_memory_ihandle();
	...
	ci[6] = p1275_uint2cell(size);		/* Arg4: SA2: len */
	ci[7] = p1275_uint2cell(phys_hi);	/* Arg5: SA3: phys.hi */
	ci[8] = p1275_uint2cell(phys_lo);	/* Arg6: SA4: phys.lo */
 ...
 */
void
prom_free_phys(u_int size, u_int phys_hi, u_int phys_lo)
{
	cell_t ci[8];
	ihandle_t imemory = prom_memory_ihandle();
	...
	ci[4] = p1275_ihandle2cell(imemory);	/* Arg2: memory ihandle */
	ci[5] = p1275_uint2cell(size);		/* Arg3: SA1: size */
	ci[6] = p1275_uint2cell(phys_hi);	/* Arg4: SA2: phys.hi */
	ci[7] = p1275_uint2cell(phys_lo);	/* Arg5: SA3: phys.lo */
