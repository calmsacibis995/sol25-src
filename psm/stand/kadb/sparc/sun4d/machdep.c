...
#pragma ident	"@(#)machdep.c	1.27	95/07/18 SMI" /* From SunOS 4.1.1 */
...
#ifdef KERNEL_AGENT
/*
 * without the kernel agent the kernel stops the other cpus before
 * they get here.  The kernel agent code in the kernel disables the
 * stopping of processors so it happens here. This code flags the
 * kernel to save the registers of the other processors as they stop.
 */
	extern int *kdbx_stopcpus_addr;
#endif
	...
	if (kdbx_stopcpus_addr != (int *) 0) {
		*kdbx_stopcpus_addr = 1; /* tell other processors to save regs */
	} else {
		printf("idle_other_cpus: kdbx_stopcpus_addr not set\n");
		printf("registers for other cpus may not be correct\n");
	}
#endif /* KERNEL_AGENT */
