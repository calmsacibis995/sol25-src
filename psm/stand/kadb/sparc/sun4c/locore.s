...
#pragma ident	"@(#)locore.s	1.54	95/06/27 SMI" /* from SunOS 4.1 */
...
kadbcpu:
	.word	-1			! ordinal of cpu holding above lock.
first:
	.word	1			! flags first entry into kadb
...
scb:
	...
	BAD_TRAP;		! FF monitor breakpoint (gets overlaid)
 ...
 * itself yet.  If it has all ready, trap in to kadb.  Otherwise, take
 * care of the initialization.
 * NOTE: all that old position-independent code that dealt with relocation
...
enter:
	...
	tst	%o4			! been though here before?
	bne,a	init			! non-zero --> no
	st	%g0, [%o5 + %lo(first)]	! clear the flag
	!
...
init:
	...
	call	early_startup
	nop
	sethi	%hi(kadblock), %o0	! be sure that the trap mutex is clear
	...
	std	%o0, [%l5+8]
	!
	! Save monitor's breakpoint trap.
	!
	bclr    0xfff, %l4              ! remove tt
 ...
 */
	...
	ENTRY(trap)
#ifdef	SINISTER_FORCE
	ld	[%g7 + T_CPU], %l5	! get this cpuid
	ldub	[%l5 + C_CPUID], %l5
	!
	! Try the lock.
	!
...
2:
	...
	! save fpu
	sethi	%hi(fpu_exists), %g1
	ld	[%g1 + %lo(fpu_exists)], %g1
...
1:
	...
	!
	mov	-1, %g1
	sethi	%hi(kadbcpu), %g2	! clobber cpu ordinal
	st	%g1, [%g2 + %lo(kadbcpu)]
	set	regsave, %g7
	ld	[%g7 + R_WIM], %g1
	mov	%g1, %wim
	...
	sethi	%hi(kadblock), %l3	! clear the mutex
	stb	%g0, [%l3 + %lo(kadblock)]
...
/* Don't use the "=" pseudo-op for these!  It induces aprocryphal
 * symbols in the linked program.
 */
#define	LPSR	(0*4)
#define	LPC	(1*4)
#define	LNPC	(2*4)
