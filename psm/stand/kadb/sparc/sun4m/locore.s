...
#pragma ident	"@(#)locore.s	1.13	95/06/27 SMI" /* From SunOS 4.1.1 */
...
scb:
	...
	BAD_TRAP; 				! 24
	WIN_TRAP(unimpl_iflush);		! 25 - unimp iflush
						! For ROSS 625 which has
						! FlushTrapDisables off
						! to perform an iflush bcast
 ...
 * Check the "fist" flag to see if kadb has had a chance to initialize
 ...
 *
 * XXX - should really be in the module layer.
 */
#define ASI_IBUF_FLUSH	0x31

	ENTRY(unimpl_iflush)
	sta	%g0,[%g0]ASI_IBUF_FLUSH
	jmp	%l2			! skip iflush instruction
	rett	%l2 + 4
