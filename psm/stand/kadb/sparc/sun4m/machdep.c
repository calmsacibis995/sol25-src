...
#pragma ident	"@(#)machdep.c	1.25	95/07/18 SMI" /* From SunOS 4.1.1 */
...
#define	NCPUS	4
u_int cpu_nodeid[NCPUS];	/* array of nodeids */
#endif MP
...
u_int module_type = -1;		/* type of module being used */
...
int level;
{
	...
	indx[1] = MMU_L1_INDEX((u_int)v);
	indx[2] = MMU_L2_INDEX((u_int)v);
	indx[3] = MMU_L3_INDEX((u_int)v);
	for (i = 1; i < level; i++) {
...
idle_other_cpus()
{
	...
	if (kdbx_stopcpus_addr != (int *)0) {
		...
		printf("idle_other_cpus: registers for other cpus not saved\n");
	}
#endif /* KERNEL_AGENT */
