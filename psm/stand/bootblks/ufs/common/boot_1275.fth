...
\ All Rights Reserved.
\
\ "@(#)boot_1275.fth	1.7	95/08/04 SMI"
\

\ Make 2.x tokenizer understand new FCodes and name changes

tokenizer[  warning off  ]tokenizer

tokenizer[
   also tokens  definitions

   v2 h# 029 01 byte-code: push-package
   v2 h# 02a 01 byte-code: pop-package
   v2 h# 02b 01 byte-code: interpose

   alias decode-string         xdrtostring
   alias to                    is
   alias property              attribute
   alias get-package-property  get-package-attribute
   alias rshift                >>
   alias lshift                <<
   alias invert                not
   alias char+                 ca1+
...
: booter-dirname$ ( -- dir$ )  " /platform/sun4u/"  ;
...
: booter$ ( -- adr,len )
   ...
   over c@ ascii / <>  if               ( filename$ )
      booter-dirname$  $cat-booter-name ( filename$ )
...
: e_entry     ( -- n )  h# 18 +l_elfhdr  ;
: e_phoff     ( -- n )  h# 1c +l_elfhdr  ;
: e_phentsize ( -- n )  h# 2a +w_elfhdr  ;
: e_phnum     ( -- n )  h# 2c +w_elfhdr  ;
...
: load-elf ( filebase -- entry-point )
   dup to elfhdr
   e_phentsize allocate-memory to phdr
   e_phnum 0 ?do
      dup i get-phdr
      p_type pt_load = if
	 ( read it )
	 ...
	 then
      then
   loop drop
   phdr e_phentsize free-memory
   e_entry
;
: sign-on ( -- )
   diagnostic-mode?  if
      ." FCode UFS Reader 1.6 04 Aug 1995 13:01:58. " cr
   then
;
: relocate ( -- )
   loader-base dup check-elf  if  ( base-adr )
      load-elf                    ( entry-point )
      " to load-base init-program" evaluate
   then
;
: reloc&go ( -- )  relocate  " ?go" evaluate  ;
...
\      ." Type  '4000  "" /platform/sun4u/ufsboot"" get-file reloc&go' "
\      ." to continue."
      exit
   then
   loader-base  booter$  ( adr,len )
