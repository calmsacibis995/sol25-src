...
#pragma ident	"@(#)cpr_sun4m.c 1.28	94/12/07 SMI"
...
cpr_move_ptpe(u_int *opap, u_int ptepa, u_int *ptpp)
{
	...
	prom_map((caddr_t)ptva, 0, (opa & 0xfffff000), MMU_PAGESIZE);
...
va_to_pa(u_int va)
{
	...
	if ((pfn = cpr_va_to_pfn(va)) != -1)
		return (PN_TO_ADDR(pfn) | (va & MMU_PAGEOFFSET));
	else
		return ((u_int)-1);
}

void
cpr_get_nvram(char *default_bootfile)
{
	dnode_t node;
	...
	if ((bootf_len = prom_getproplen(node, "boot-file")) <= 0 ||
		...
		errp("Invalid default prom file name len (%d)\n", bootf_len);
		return;
	}
...
void
cpr_set_nvram(char *boot_file, char *silent)
{
	dnode_t node;
	dnode_t sp[OBP_STACKDEPTH];
	pstack_t *stk;
	...
	if ((node == OBP_NONODE) || (node == OBP_BADNODE)) {
		prom_printf("Invalid prom node; no auto resume\n");
		return;
	}
	/* +1 for '\0' */
	if ((bootf_len = strlen(boot_file) + 1) <= 0 ||
	    (bootf_len >= OBP_MAXPATHLEN)) {
		prom_printf("Inval prom file name len (%d); no auto resume\n",
			bootf_len);
		return;
	}
