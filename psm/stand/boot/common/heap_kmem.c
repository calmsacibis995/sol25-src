#ident	"@(#)heap_kmem.c	1.25	94/12/10 SMI" /* From SunOS 4.1 1.10 */
	...
	if (a->size - nbytes < SMALLEST_BLK) {
		...
		if ((int) a->block->data & ALIGNMASK &&
			...
			*/
			...
			if (a->size > nbytes) {
				/*
				rmlog(0, nextblk(a->block, nbytes),
				    a->size - nbytes, caller());
				*/
 ...
 */
caddr_t
bkmem_alloc(register unsigned size)
{
	return (kmem_alloc(size));
}

bkmem_free(caddr_t p, u_int bytes)
{
	return (kmem_free(p, bytes));
}
