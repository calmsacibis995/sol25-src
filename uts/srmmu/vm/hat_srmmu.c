...
#pragma ident   "@(#)hat_srmmu.c 1.192     95/10/06 SMI"
...
static void	srmmu_ptechgprot(struct pte *, u_int, u_int, caddr_t, int,
		    struct hat *, int);
...
static void	s_hat_enter(struct as *);
static void	s_hat_exit(struct as *);
static kmutex_t	*srmmu_mlist_enter(struct page *);
static void	srmmu_mlist_exit(kmutex_t *);

static void	srvec_mlist_enter(struct page *);
static void	srvec_mlist_exit(struct page *);

static int	srmmu_mlist_held(struct page *);
static void	srmmu_cachectl_enter(struct page *);
static void	srmmu_cachectl_exit(struct page *);

struct hat_lockops srmmu_lockops = {
	srmmu_page_enter,
	srmmu_page_exit,
	s_hat_enter,
	s_hat_exit,
 ...
 */
static void srmmu_l1inval(struct as *, ptbl_t *);
...
static int max_ptbl = 100000;
static int min_ptbl = 10;

static kmutex_t free_ptbl_lock;
static kmutex_t pool_ptbl_lock;
 ...
 */
#define	MEM_HASH_SHIFT  0x8
#define	N_MEM_SLOTS	0x100		/* must be a power of 2 */
#define	PFN_HASH(pfn)   ((pfn/mhash_per_slot) & (N_MEM_SLOTS - 1));

static u_int mhash_per_slot;

static struct flat_mem {
	page_t  *pstart;
	u_int   spfn;
} mem_hash[N_MEM_SLOTS];
...
const char *srmmu_version = "06 Oct 1995 1.192";
...
tatoptbl_pte(u_int ta, ptbl_t **ptbl, /* struct pte */ union ptpe **pte)
{
	...
	ASSERT(pp->p_pagenum == (ta >> MMU_STD_PTPSHIFT));
...
#ifdef VAC
extern	void	spt_vacsync(caddr_t, struct as *);
extern	void	spt_mtxinit();
#endif /* VAC */
 ...
 *
 * A pile of mutexes, the various hash tables, and a few other odds
 * and ends are set up.
 */
static void
srmmu_lock_init(void)
{
	...
	hat_lockops = &srmmu_lockops;

	mutex_init(&free_l1srmmu_lock, "free_l1srmmu", MUTEX_DEFAULT, NULL);
	mutex_init(&free_ptbl_lock, "free_ptbl_lock", MUTEX_DEFAULT, NULL);
	mutex_init(&pool_ptbl_lock, "pool_ptbl_lock", MUTEX_DEFAULT, NULL);
	mutex_init(&srmmu_ctx_mtx, "srmmu_ctx_mtx", MUTEX_DEFAULT, NULL);
	...
	for (i = 0; i < SPL_TABLE_SIZE; i++) {
		(void) sprintf(name_buf, "srmmu_page_%d", i);
		mutex_init(&srmmu_page_lock[i], name_buf, MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_PTE2HME_MTX; i++) {
		(void) sprintf(name_buf, "pte2hme_bin_%d", i);
		mutex_init(&hme_hash_mtx[i], name_buf, MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_L1PTBL_MTX; i++) {
		(void) sprintf(name_buf, "l1ptbl_mutex_%d", i);
		mutex_init(&l1ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_L2PTBL_MTX; i++) {
		(void) sprintf(name_buf, "l2ptbl_mutex_%d", i);
		mutex_init(&l2ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_L3PTBL_MTX; i++) {
		(void) sprintf(name_buf, "l3ptbl_mutex_%d", i);
		mutex_init(&l3ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < NCPU; i++) {
		(void) sprintf(name_buf, "hment_pool_%d", i);
		mutex_init(&hment_pool[i].hp_mtx, name_buf,
		    MUTEX_DEFAULT, NULL);
	}

#if defined(SRMMU_TMPUNLOAD)

	for (i = 0; i < N_TMPL1PTBL_MTX; i++) {
		(void) sprintf(name_buf, "t_l1ptbl_mutex_%d", i);
		mutex_init(&tmpl1ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_TMPL2PTBL_MTX; i++) {
		(void) sprintf(name_buf, "t_l2ptbl_mutex_%d", i);
		mutex_init(&tmpl2ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}

	for (i = 0; i < N_TMPL3PTBL_MTX; i++) {
		(void) sprintf(name_buf, "t_l3ptbl_mutex_%d", i);
		mutex_init(&tmpl3ptbl_lock[i], name_buf,
		    MUTEX_DEFAULT, NULL);
	}
	...
	/*
	 * The demap_mutex keeps the hat layer from doing multiple
	 * concurrent demaps.
	 */
	mutex_init(&srmmu_demap_mutex, "srmmu_demap", MUTEX_ADAPTIVE, NULL);
...
srmmu_init(void)
{
	...
	srmmu_kmcache = kmem_cache_create("srmmu_kmcache",
	    sizeof (srmmu_t), sizeof (u_int), NULL, NULL, NULL);
...
/* ARGSUSED */
void
srmmu_memload(hat, as, addr, pp, prot, flags)
	struct hat *hat;
	struct as *as;
	caddr_t addr;
	struct page *pp;
	...
	int flags;
{
	...
	srmmu_pteload(as, addr, pp, &apte, 3, flags);
}
...
/* ARGSUSED */
void
srmmu_devload(hat, as, addr, dp, pf, prot, flags)
	struct hat *hat;
	struct as *as;
	caddr_t addr;
	devpage_t *dp;
	...
	int flags;
{
	...
	if (!(cache & CACHE_VAC)) {
		 ...
		 * tracked) and no HAT_NOCONSIST flag.
		 */
		ptpe.pte.Cacheable = (!(flags & HAT_NOCONSIST) &&
		    (pfn2pp_hash(pf) != NULL));
	}
#endif
	...
	srmmu_pteload(as, addr, (struct page *)dp,
		(struct pte *)&ptpe, 3, flags);
}
	...
	u_int len;
{
	...
	while (len) {
		...
		if (len >= MMU_L1_SIZE && MMU_L1_OFF(addr) == 0 &&
			...
			ret = srmmu_pteload(as, addr, pp, &apte, 1, flags);
			if (ret == 0) {
				cmn_err(CE_PANIC, "load l1 failed?");
			}

			pp += mmu_btop(MMU_L1_SIZE);
			addr += MMU_L1_SIZE;
			len -= MMU_L1_SIZE;

		} else if (len >= MMU_L2_SIZE && MMU_L2_OFF(addr) == 0 &&
			(page_pptonum(pp) & MMU_STD_SECONDMASK) == 0) {
			ret = srmmu_pteload(as, addr, pp, &apte, 2, flags);
			if (ret == 0) {
				cmn_err(CE_PANIC, "load l2 failed?");
			}

			pp += mmu_btop(MMU_L2_SIZE);
			addr += MMU_L2_SIZE;
			len -= MMU_L2_SIZE;

		} else {
load_l3:
			(void) srmmu_pteload(as, addr, pp, &apte, 3, flags);
			pp++;
			addr += MMU_PAGESIZE;
			len -= MMU_PAGESIZE;
		}
...
{
	register caddr_t a;
	register struct devpage *tmp_dp;
	struct vnode *vp;
	off_t off;
	union ptpe ptpe;
	...
	if (dp) {
		...
		if ((vp = dp->p_vnode) == NULL) {
			cmn_err(CE_PANIC, "hat_contigload null vp\n");
		}
		off = dp->p_offset;
		 ...
		 * dp based on a <vnode, offset> devpage look up.
		 */
		for (a = addr; a < addr + len; a += MMU_PAGESIZE,
			...
			if (tmp_dp == NULL) {
				cmn_err(CE_PANIC,
				    "hat_contigload() null devpage\n"
				    "vnode = 0x%x off = 0x%x\n"
				    "devpage_lookup failed\n", vp, off);
			}
			srmmu_devload(hat, as, a, tmp_dp, pf, prot, flags);
...
load_l3:
			srmmu_pteload(as, addr, NULL, &ptpe.pte, 3, flags);
			pf++;
			addr += MMU_PAGESIZE;
			len -= MMU_PAGESIZE;
		}
 ...
 */
void
srmmu_chgprot(as, addr, len, vprot)
	struct as *as;
	caddr_t addr;
	u_int len;
	u_int vprot;			/* virtual page protections */
{
	register struct pte *pte, *lastpte, *tmppte;
	...
	if (vprot != (u_int)~PROT_USER && vprot != (u_int)~PROT_WRITE) {
		pprot = srmmu_vtop_prot(addr, vprot);
	}
	...
	for (; len; len -= span, addr += span) {
		...
		if (pte_valid(&old)) {
			if (level == 3) {
				...
				srmmu_ptechgprot(pte, vprot, pprot, addr, level,
					hat, flag);
			}
		} else {
			span = srmmu_sizes[level];
 ...
 */
static void
srmmu_ptechgprot(pte, vprot, pprot, addr, level, hat, flag)
	struct pte *pte;
	u_int vprot, pprot;
	caddr_t addr;
	int level;
	...
	int flag;
{
	...
	} else if (old.AccessPermissions != pprot) {
		newprot = 1;
	} else {
		newprot = 0;
	}
...
tmpunload_l2(struct as *as, caddr_t addr, struct ptp rl1ptp, ptbl_t *rl2ptbl)
{
	...
	switch (tmp.EntryType) {
		...
		default:
			cmn_err(CE_PANIC, "tmpunload_l2: l1pte = 0x%x",
			    *(u_int *) &tmp);
	}
	...
	struct ptp rl2ptp, ptbl_t *rl3ptbl)
{
	...
	default:
		cmn_err(CE_PANIC, "tmpunload_l3: l2pte = 0x%x",
		    *(u_int *) &tmp);
	}
	/*NOTREACHED*/
}
	...
	int	flags;
{
	 ...
	 * more.
	 */
	if ((flags & HAT_TMPUNLOAD) && (len >= (256 * 1024))) {
		struct hat	*hat;
	 ...
	 * TMPUNLOAD.
	 */
	if (!(flags & HAT_TMPUNLOAD) && (srmmu->s_tmpl1ptbl != NULL)) {
	...
	srmmu_t	*srmmu;
{
	...
	ASSERT((flags & (HAT_TMPUNLOAD | SR_TMP_TREE)) !=
		(HAT_TMPUNLOAD | SR_TMP_TREE));
...
{
	if (hme == NULL) {
		if (((u_int)pp->p_mapping & PP_PTBL) == 0) {
			/*
		...
		*pte = ptbltopt_va(ptbl) + pp->p_index;
	} else {
		if ((u_int)pp->p_mapping & PP_PTBL) {
			/*
	...
	struct hment *vf;
{
	...
	ASSERT((vacflag == VAC_UNLOAD) || se_assert(&pp->p_selock));
	ASSERT((vacflag == 0) ||
	...
	while ((p_mapping = (u_int)pp->p_mapping) != NULL) {
	...
	int vacflag;
{
	...
	ASSERT(srmmu_mlist_held(pp));
	ASSERT(pp != NULL);

	if (hme == NULL) {
		ASSERT(((u_int)pp->p_mapping) & PP_PTBL);
		ptbl = PP2PTBL(pp);
		pte = ptbltopt_va(ptbl) + pp->p_index;
	} else {
		ASSERT(hme->ghme.hme_page == pp);
	...
	u_int clearflag;
{
	...
	if (PP_ISRO(pp) && (clearflag & HAT_STOPON_MOD)) {
		return (HAT_DONE);
	}

	if (pp->p_share > share_trigger &&
	    !(clearflag & HAT_ZERORM)) {
		if (PP_ISRO(pp)) {
			srmmu_page_enter(pp);
			PP_SETREF(pp);
			srmmu_page_exit(pp);
	...
	u_int clearflag;
{
	...
	if (hme == NULL) {
		ASSERT((u_int)pp->p_mapping & PP_PTBL);
		if ((u_int)pp->p_mapping & PP_NOSYNC) {
			return (HAT_DONE);
		}
		ptbl = PP2PTBL(pp);
		pte = ptbltopt_va(ptbl) + pp->p_index;
 ...
 */
u_int
srmmu_getkpfnum(addr)
	caddr_t	addr;
{
	return (srmmu_getpfnum(&kas, addr));
}
...
srmmu_reserve(struct as *as, caddr_t addr, u_int len, u_int load_flag)
{
	...
	while (addr < end) {
		 ...
		 * list, so we correct that here.  When load_flag isn't
		...
		if (load_flag) {
			...
			if (pte_valid(&tpte)) {
				...
				pp, pp->p_mapping, pte->PhysicalPageNumber);
 ...
 */
static
srmmu_pteload(as, addr, pp, pte, level, flags)
	struct	as *as;
	caddr_t	addr;
	struct	page *pp;
	struct	pte *pte;
	...
	int	flags;
{
	 ...
	 */
	ASSERT(pp == NULL || (pp >= pages && pp < epages));

#ifdef sun4m
	if (pp == NULL && !(flags & HAT_NOCONSIST)) {
		/*
	...
	ASSERT((pp == NULL) || se_assert(&pp->p_selock) || panicstr);

#ifdef sun4m
	ASSERT((pp == NULL) ||
	    (pp->p_pagenum == (pte->PhysicalPageNumber & 0xfffff)));
#else sun4m
	ASSERT(pp == NULL || (pp->p_pagenum == pte->PhysicalPageNumber));
#endif sun4m
...
vac_retry:
	 ...
	 */
#ifndef VAC
	if ((flags & PTELD_LOCK) && (as != &kas) && (level == 3) &&
	    !locked) {
		ptbl->ptbl_lockcnt++;
		locked = 1;
	}
	...
	if ((flags & PTELD_LOCK) && (level == 3) && !locked) {
		ptbl->ptbl_lockcnt++;
		locked = 1;
	}
#endif
	...
	if (pte_valid(&oldpte)) {
		...
		if (((*x) & ~PTE_RM_MASK) != ((*y) & ~PTE_RM_MASK)) {
			 ...
			 */
			(void) mmu_writepte(opte, *(u_int *)pte, addr, level,
				as->a_hat, PTE_RM_MASK);
		} else {
#ifdef DEBUG
		...
		if (pp) {
			 ...
			 * held due to VAC.
			 */
			rc = hme_may_add(ptbl, opte, pp, flags, as->a_hat, pte,
			    myhme);
			if (rc != 0) {
				srmmu_mlist_exit(pml);
				unlock_ptbl(ptbl, mtx);
			...
			if (level == 3) {
				...
				as->a_rss += mmu_btop(srmmu_sizes[level]);
			}
	...
	ptbl_t *ptbl;
{
	...
	if (p_mapping & PP_PTBL) {
		...
		opte = ptbltopt_va(optbl) + pp->p_index;
	...
	int resolution;
{
	...
	case UNCACHE_CONFLICT:
		...
		while (p_mapping != NULL) {
			if (p_mapping & PP_PTBL) {
				...
				pte = ptbltopt_va(tptbl) + pp->p_index;
				p_mapping = NULL;
			} else {
				hme = (struct srhment *) p_mapping;
				ASSERT(hme->ghme.hme_page == pp);
	...
	case UNLOAD_CONFLICT:
		 ...
		 * are about to load.
		 */
		while ((p_mapping = (u_int) pp->p_mapping) != 0) {
...
srmmu_vacsync(u_int pfnum)
{
	...
	ASSERT(pp == NULL || srmmu_mlist_held(pp));
	...
	caddr_t addr;
{
	...
	/*
	 * XXX how do we know we are in the correct context??
	 * - it's upto the calller.
	 */
	ptpe.ptpe_int = mmu_probe(addr, NULL);
	return (pte_valid(&ptpe.pte));
}
	...
	int got_mlist_lock;
{
	...
	if (do_pg_coloring && (cache & CACHE_VAC) && pte->Cacheable) {
		flags |= SR_NOPGFLUSH;
		ASSERT(ADDR_2_VCOLOR(ptetovaddr(ptbl, pte)) == pp->p_vcolor);
	}
#endif /* VAC */
...
#ifndef VAC
	if ((flags & HAT_UNLOCKUNLOAD) && (as != &kas) && (level == 3)) {
		ptbl->ptbl_lockcnt--;
	}
#else
	/* Maintain kas lockcnt for VAC. */
	if ((flags & HAT_UNLOCKUNLOAD) && (level == 3)) {
		ptbl->ptbl_lockcnt--;
	}
#endif
	...
	int got_mlist_lock;
{
	...
	p_mapping = (u_int) pp->p_mapping;
	if (p_mapping & PP_PTBL) {
		if ((ptbltopt_va(MAP2PTBL(p_mapping)) + pp->p_index)
		    == pte) {
			 ...
			 * could be due to HAT_NOCONSIST load.
			 */
	...
	struct page *pp;
{
	...
	if (pp->p_mapping == NULL) {
		...
		p_mapping = (u_int) pp->p_mapping;
		if ((p_mapping & PP_PTBL) || (pp->p_share == 1)) {
			if (p_mapping & PP_PTBL) {
	...
	register struct page *pp;
{
	...
	if (oldpte.Cacheable && PP_ISNC(pp)) {
		...
		*pte = oldpte;
		 ...
		 */
		if ((cache & CACHE_VAC) && do_pg_coloring)
			pp->p_vcolor = ADDR_2_VCOLOR(vaddr);
	}
}
#endif	/* VAC */
	...
	int flags;
{
	...
	do {
		...
		if (as == &kas) {
			...
			ASSERT((ptbl[i]->ptbl_flags &
			    (HAT_LOADSHARED | SR_TMP_TREE)) == 0);
		} else {
			ASSERT((flags & (HAT_LOADSHARED | SR_TMP_TREE)) !=
			    (HAT_LOADSHARED | SR_TMP_TREE));
			if (flags & HAT_LOADSHARED) {
	...
	caddr_t addr;
{
	...
	for (i = 0; i < NL2PTEPERPT; i++, fl2ptp++, addr += MMU_L2_SIZE) {
		...
		case MMU_ET_PTE:
		default:
			cmn_err(CE_PANIC, "bad fl2ptp 0x%x", fl2ptp);
		}
	}
}
...

{
	...
	default:
		cmn_err(CE_PANIC, "recover_l2tmp: l1pte 0x%x",
		    *(u_int*)&trl1ptp);
	}
...
again:
	...
	default:
		cmn_err(CE_PANIC, "recover_l3tmp: rl2ptp 0x%x",
		    *(u_int *) &trl2ptp);
	}
...
fake_again:
	...
	case MMU_ET_PTP:
		...
		case MMU_ET_PTE:
		default:
			cmn_err(CE_PANIC, "recover_l3tmp: fl2ptp 0x%x",
			    *(u_int *) &tfl2ptp);
		}
		/*NOTREACHED*/
	...
	case MMU_ET_PTE:
	default:
		cmn_err(CE_PANIC, "recover_l3tmp: fl1ptp 0x%x",
		    *(u_int *) &tfl1ptp);
	}
	/*NOTREACHED*/
}
	...
	register ptbl_t *pl1tbl;
{
	...
	for (i = 0; i < NUSRL1PTEPERPT; i++, addr += MMU_L1_SIZE, l1ptr++) {
		...
		case MMU_ET_PTP:
			/* clear L1 ptp. */
			MOD_VALID_PTP(flags, &(l1ptr->ptp), MMU_ET_INVALID,
			    addr, 1, as->a_hat, 0);

			l2ptbl = tatoptbl(l1tmp.ptp.PageTablePointer);
			ret = srmmu_l2inval(as, (caddr_t) addr, flags, l2ptbl,
			    &l2mtx);
		...
		default:
			cmn_err(CE_PANIC, "bad l1 entry at 0x%x",
				l1tmp.ptpe_int);
		}
	}
	...
	page_t *rpp[];
{
	...
	*pmtx = mtx;
	srmmu_unload_loop(l3ptbl, 0, NL3PTEPERPT, SR_NOFLUSH, rpp);
	...
	u_int		size;
{
	...
	lock_ptbl(l1ptbl, 0, as, addr, 1, &mtx);
	...
	for (i = 0; i < ((size + L2PTOFFSET) >> MMU_STD_RGNSHIFT);
		...
		if (nl1ptbl != l1ptbl) {
			...
			lock_ptbl(nl1ptbl, 0, as, addr, 1, &mtx);
			l1ptbl = nl1ptbl;
		}
...
srmmu_fillin_ptbl_group(ptbl_gr_t *ptbl_gr, pt_gr_t *ptg, int level)
{
	...
	if (pp == NULL) {
		cmn_err(CE_PANIC, "srmmu: could not find page %8x\n", ptg);
	}
	...
	ptbl_gr->pg_pt_pfn = pp->p_pagenum;
	ptbl_gr->pg_page = pp;
...
ialloc_ptbl()
{
	...
	srpt_kmcache = kmem_cache_create("pt_kmcache",
	    MMU_PAGESIZE, MMU_PAGESIZE, NULL, NULL, NULL);
	...
	for (i = 0; i < n_l3pts; i++) {
		...
		if (ptbl == NULL) {
			cmn_err(CE_PANIC, "Can not allocate initial ptbls\n");
		}
...
mismatch:
	...
	cmn_err(CE_PANIC, "fail to lock: ptbl 0x%x flag 0x%x "
		"as 0x%x va 0x%x level %d mtx 0x%x ret %d",
		ptbl, flag, pas, va, level, mtx, ret);
	/* NOTREACHED */
}
...
clr_sx_bits(u_int pfn)
{
	...
	case 0xf:
		...
		if ((x == 0xf0) && (pfn != 0xf00000) && (pfn != 0xf00001)) {
			cmn_err(CE_PANIC, "bad pfn 0x%x", pfn);
		}
		break;
	}
...
{
	u_int index;
	struct flat_mem *phash;
	page_t *pp;

	extern page_t *epages, *pages;
	...
	index = PFN_HASH(pfn);
	phash = &mem_hash[index];

	ASSERT(pfn - phash->spfn >= 0);

	pp = phash->pstart + (pfn - phash->spfn);
	while (pp >= pages && pp < epages) {
		if (pp->p_pagenum == pfn) {
			ASSERT(page_numtopp_nolock(pfn) == pp);
			return (pp);
		}

		/*
		 * We missed. This is due to at least one memseg hole
		 * in this slot- Very unlikely since we spread it to
		 * 0x100 slots.
		 */
		if (pp->p_pagenum < pfn) {
			/*
			 * or should just return NULL??
			 * i think this means the page
			 * is not a pp.
			 */
			break;
		}
	...
	/*
	 * Not only is there a memseg hole in this slot,
	 * the hole(s) are so big that we don't know where pp
	 * is pointing to.
	 */
...
build_pfn_hash()
{
	...
	for (pseg = memsegs; pseg != NULL; pseg = pseg-> next) {
		index = PFN_HASH(pseg->pages_base);
		cur = pseg->pages_base;
		pcur = pseg->pages;
		do {
			ASSERT(index < N_MEM_SLOTS);
			if (mem_hash[index].spfn == 0 ||
			    mem_hash[index].spfn > cur) {
				mem_hash[index].spfn = cur;
				mem_hash[index].pstart = pcur;
			}
			cur += mhash_per_slot;
			pcur += mhash_per_slot;
			index++;
	...
	page_t *pp;
{
	...
	if (p_mapping & PP_PTBL) {
		ASSERT(pp->p_share == 1);
		ASSERT(pte == ptbltopt_va(PP2PTBL(pp)) + pp->p_index);
		ASSERT(hme == NULL);
 ...
 */
#define	KNOWN_HAT_LOAD_FLAGS	(HAT_LOAD | HAT_LOCK | HAT_NOSYNCLOAD | \
	HAT_ADVLOAD | HAT_LOADSHARED)
	...
	srhment_t	*myhme[])
{
	...
	ASSERT(se_assert(&pp->p_selock) || panicstr);
	ASSERT((flags & ~KNOWN_HAT_LOAD_FLAGS) == 0);
	ASSERT(PTBL_IS_LOCKED(ptbl->ptbl_flags));
	ASSERT(srmmu_mlist_held(pp));
	...
	if (t_map == NULL) {
		...
		if (flags & PTELD_NOSYNC) {
			t_map |= PP_NOSYNC;
		}
		pp->p_mapping = (void *)t_map;
		pp->p_index = (u_char)((u_int)pte & 0xff) >> 2;
		pp->p_share = 1;
		if (pte_ronly(pte) && !PP_ISMOD(pp)) {
			srmmu_page_enter(pp);
			if (!PP_ISMOD(pp)) {
				PP_SETRO(pp);
			}
		...
		hme->ghme.hme_nosync = (flags & PTELD_NOSYNC) != 0;
	...
	register page_t *pp;
{
	...
	if (tmp) {
		ASSERT(pp->p_mapping != (struct hment *) hme);
		ASSERT(tmp->hme_page == pp);
		tmp->hme_next = hme->ghme.hme_next;
	} else {
		ASSERT(pp->p_mapping == (struct hment *) hme);
		pp->p_mapping = hme->ghme.hme_next;
	}
...

{
	...
	if (bad) {
		cmn_err(CE_PANIC, "bad entry! bad = %d, ptbl: 0x%x\n",
		    bad, ptbl);
	}

	if (lvl == 3) {
		if (nptp) {
			cmn_err(CE_PANIC, "bad ptp! nptp = %d, ptbl: 0x%x\n",
			    nptp, ptbl);
		}
	...
	if (lvl == 2) {
		if ((npte + nptp) != ptbl->ptbl_validcnt) {
			cmn_err(CE_PANIC, "bad validcnt = %d, npte = %d, "
			    "nptp = %d, ptbl: 0x%x\n",
			    ptbl->ptbl_validcnt, npte, nptp, ptbl);
		}
