...
#pragma ident	"@(#)hat_srmmu.h	1.69	95/02/10 SMI"
...
struct ptbl {
	...
	struct as	*ptbl_as;
	u_short		ptbl_base;
	u_short		ptbl_lockcnt;
	u_char		ptbl_unused;
	u_char		ptbl_index;
	u_char		ptbl_validcnt;
	u_char		ptbl_flags;
};
...
struct srmmu {
	union {
		struct hat   *su_hat;	/* back ptr to the hat */
		struct srmmu *su_next;	/* next srmmu on free/lru list */
	} srmmu_u;
	ptbl_t		*s_l1ptbl;	/* first ptbl of the 4 for the l1 pt */
	u_int		srmmu_root;	/* Left around for the leo pigs */
...
#define	s_hat		srmmu_u.su_hat
#define	s_next		srmmu_u.su_next
 ...
 */
#define	SR_RMSYNC	0x0100000
#define	SR_INVSYNC	0x0200000
#define	SR_RMSTAT	0x0400000
#define	SR_NOFLUSH	0x0800000
#define	SR_NOPGFLUSH	0x1000000 /* only tlb flush for vac & pg_color */
 ...
 */
#define	SR_TMP_TREE	0x2000000
#define	SR_UNLOAD_ALL	0x4000000

#define	SR_FLAGS	0x7f00000

/*
 * Flags to pass to srmmu_pteload() and segkmem_mapin().
 *
 ...
 *	  don't convert flags specified by the caller to SRMMU specific
 *	  flags.
 */
 ...
 */
#define	ptetooff(a)	(((u_int)(a) & 0xff) >> 2)

#define	PTBL_GROUP	16

struct ptbl_gr {
	u_int		pg_pt_pfn;	/* pfn for first pt in the group */
...
void srmmu_devload(struct hat *, struct as *, caddr_t, devpage_t *,
		    u_int, u_int, int);
void srmmu_mempte(struct page *, u_int, struct pte *, caddr_t);
u_int srmmu_vtop_prot(caddr_t, u_int);
u_int srmmu_ptov_prot(struct pte *);
struct pte *srmmu_ptefind(struct as *, caddr_t, int *, struct ptbl **,
	kmutex_t **, int);
struct pte *srmmu_ptefind_nolock(struct  as *, caddr_t, int *);
void srmmu_reserve(struct as *as, caddr_t addr, u_int len, u_int load_flag);

void lock_l1ptbl(struct ptbl *, u_int, struct as *, kmutex_t **);
...
struct ptbl *srmmu_ptblreserve(caddr_t, u_int, struct ptbl *, kmutex_t **);
...
int lock_ptbl(struct ptbl *pptbl, u_int flag, struct as *pas, caddr_t va,
	int level, kmutex_t **);
