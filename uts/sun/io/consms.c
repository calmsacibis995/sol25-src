...
#ident	"@(#)consms.c	5.33	94/08/30 SMI"	/* SunOS4.0 5.15	*/
...
static int consms_prop_op(dev_t, dev_info_t *, ddi_prop_op_t, int,
	    char *, caddr_t, int *);
...
static u_long	consms_timestamp;	/* time of last use */
static u_char	consms_suspended;	/* True, if suspended */
...
_init(void)
{
	...
	mutex_init(&consmslock, "consms autoconfig lock", MUTEX_DRIVER, NULL);
...
consms_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	drv_getparm(TIME, &consms_timestamp);
	if (ddi_prop_create(DDI_DEV_T_NONE, devi, DDI_PROP_CANSLEEP,
	    "pm_timestamp", (caddr_t) &consms_timestamp,
	    sizeof (consms_timestamp)) != DDI_SUCCESS)
		cmn_err(CE_WARN, "consms: Can't create property\n");
	power = 1;
	if (ddi_prop_create(DDI_DEV_T_NONE, devi, DDI_PROP_CANSLEEP,
	    "pm_norm_pwr", (caddr_t) &power, sizeof (power)) != DDI_SUCCESS)
		cmn_err(CE_WARN, "consms: Can't create property\n");
...
static int
consms_prop_op(dev_t dev, dev_info_t *dip, ddi_prop_op_t prop_op,
		int mod_flags, char *name, caddr_t valuep, int *lengthp)
{
	if (strcmp(name, "pm_timestamp") == 0 &&
	    ddi_prop_modify(DDI_DEV_T_NONE, consms_dip, mod_flags, name,
	    (caddr_t)&consms_timestamp, sizeof (consms_timestamp))
	    != DDI_PROP_SUCCESS) {
		cmn_err(CE_WARN, "consms: Can't modify property");
	}
...
{
	if (consms_suspended)
		ddi_dev_is_needed(consms_dip, 0, 1);
	drv_getparm(TIME, &consms_timestamp);
