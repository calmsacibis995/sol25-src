...
#ident	"@(#)ms.c	1.81	94/08/29 SMI"	/* SunOS4.0 1.52	*/
...
struct msdata {
	...
	short	msd_jitter;
	int	msd_timeout_id;	/* id returned by timeout() */
	int	msd_reioctl_id;	/* id returned by bufcall() */
	int	msd_resched_id;	/* id returned by bufcall() */
...
static int	msjittertimeout; /* Timeout used when mstimeout in effect */
...
static void	msresched(long msdptr);
static void	msreioctl(long msdptr);
...
static void	msincr(struct msdata *msd);
	...
	cred_t	*credp;
{
	 ...
	 * Setup tty modes.
	 */
	while ((mp = allocb(sizeof (struct iocblk), BPRI_HI)) == NULL) {
		int id;
	...
	if (ms->ms_buf == 0) {
		...
		b = kmem_zalloc((u_int)ms->ms_bufbytes, KM_SLEEP);
	...
	cred_t		*credp;
{
	...
	/* Free mouse buffer */
	if (ms->ms_buf != NULL)
		kmem_free((void *)ms->ms_buf, (u_int)ms->ms_bufbytes);
		...
		case MS_VUID_FORMAT: {
			...
			case EVENT_X:
				...
				if ((bp = allocb(sizeof (Firm_event),
				    ...
				    fep->pair = (u_char)LOC_X_ABSOLUTE;
				    fep->value = mi->mi_x;
				    fep->time = mi->mi_time;
				} else {
				    if (msd->msd_resched_id)
			...
			case EVENT_Y:
				...
				if ((bp = allocb(sizeof (Firm_event),
				    ...
				    fep->pair_type = FE_PAIR_ABSOLUTE;
				    fep->pair = (u_char)LOC_Y_ABSOLUTE;
				    fep->value = -mi->mi_y;
				    fep->time = mi->mi_time;
				} else {
				    if (msd->msd_resched_id)
 ...
 * button byte. Collect button, dx, dy, and send.
	...
	char c;
{
	...
	if (mi->mi_buttons == msd->msd_oldbutt) {
		...
		if (ABS((int) mi->mi_x) <= jitter_radius &&
			...
			msd->msd_jitter = 1;
			msd->msd_timeout_id =
				qtimeout(msd->msd_readq, (void (*)())msincr,
					(caddr_t) msd, msjittertimeout);
			return;
		}
	}
