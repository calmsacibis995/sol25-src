...
#pragma ident	"@(#)dbri.c	1.44	94/07/12 SMI"
...
dbri_ioctl(aud_stream_t *as, queue_t *q, mblk_t *mp)
{
	...
	case AUDIO_GETDEV:
	{
		...
		strcpy(devtp->name, DBRI_DEV_NAME);
		devtp->version[0] = unitp->version;
		devtp->version[1] = '\0';
		strcpy(devtp->config, ds->config);
	...
	case ISDN_GET_CONFIG:
	{
		...
		case 1:		/* first copyout done, new perform second */
			...
			audio_copyout(q, mp, (caddr_t)as->dioctl.private,
			    sizeof (isdn_conn_tab_t));
			return (AUDRETURN_NOCHANGE);
		} /* switch on ioctl state */
	}
...
dbri_start(aud_stream_t *as)
{
	...
	for (dcp = AsToDs(as)->cmdptr; dcp != NULL; dcp = dcp->nextio) {
		...
		if (!ISXMITDIRECTION(as)) {
			...
			DBRI_SYNC_MD(unitp, dcp->md, DDI_DMA_SYNC_FORCPU);
			if (dcp->rxmd.com)
				continue;
		}
		break;
	...
	cmd.arg[0] = (uint_t)DBRI_IOPBIOADDR(unitp, &dcp->txmd);
...
#define	DBRI_FIFO_SIZE	(88) /* bytes, 22 words */
	...
	if (ISXMITDIRECTION(as)) {
		...
		DBRI_SYNC_MD(unitp, dcp->md, DDI_DMA_SYNC_FORDEV);
	} else {
...
{
	struct timeval now;
	uint_t usecs_btwn_smples, usecs_gone_by;
		...
		if (Modify(ip->monitor_gain)) {
			...
			if (old_monitor_gain != unitp->distate.monitor_gain) {
				if (old_monitor_gain == 0) {
					unitp->timestamp[2] = 0;
					ddi_dev_is_needed(unitp->dip, 2, 1);
				} else if (unitp->distate.monitor_gain == 0 &&
				    pipep->refcnt <= 1) {
					drv_getparm(TIME, unitp->timestamp+2);
				}
			}
		}
	...
	for (dcp = (dbri_cmd_t *)(void *)cmdp;
		...
		if (e != DDI_SUCCESS) {
			...
			if (dcp->buf_dma_handle) {
				ddi_dma_free(dcp->buf_dma_handle);
		...
		dcp->rxmd.bufp = (uchar_t *)dcp->buf_dma_cookie.dmac_address;
		dcp->rxmd.bcnt = dcp->buf_dma_cookie.dmac_size;

		DBRI_SYNC_BUF(dcp, dcp->rxmd.cnt, DDI_DMA_SYNC_FORDEV);
		...
		if (AsToDs(as)->cmdptr == NULL) {
			...
			DBRI_SYNC_MD(unitp, AsToDs(as)->cmdlast->md,
			    DDI_DMA_SYNC_FORDEV);
...
dbri_txqcmd(aud_stream_t *as, aud_cmd_t *cmdp)
{
	...
	for (dcp = (dbri_cmd_t *)(void *)cmdp;
		...
		if (dcp->cmd.done || dcp->cmd.skip || !activated) {
			...
			dcp->txmd.bufp = (uchar_t *)
			    dcp->buf_dma_cookie.dmac_address;
			dcp->txmd.cnt = dcp->buf_dma_cookie.dmac_size;
			DBRI_SYNC_BUF(dcp, dcp->txmd.cnt, DDI_DMA_SYNC_FORDEV);
			...
			if (packet.head == NULL) {
				...
				packet.tail->txmd.fp = (union dbri_md *)
	...
	if (packet.head) {
		if (ISTEDCHANNEL(as)) {
			...
			DBRI_SYNC_MD(unitp, packet.head->md,
			    DDI_DMA_SYNC_FORDEV);
		} else {
			...
			DBRI_SYNC_MD(unitp, packet.tail->md,
			    DDI_DMA_SYNC_FORDEV);
		}
		...
		if (packet.tail->md2 != NULL) {
			...
			packet.tail->txmd2.bufp =
			    (uchar_t *)DBRI_IOPBIOADDR(unitp, buf);
		...
		if (AsToDs(as)->cmdptr == NULL) {
			...
			if (AsToDs(as)->cmdlast->md2 != NULL) {
				...
				DBRI_SYNC_MD(unitp, AsToDs(as)->cmdlast->md2,
				    DDI_DMA_SYNC_FORDEV);
			}
