...
#ident	"@(#)kbd.c	1.96	95/03/14 SMI"	/* SunOS-4.0 1.60	*/
...
struct	kbddata {
	...
	int	kbdd_bufcallid;		/* bufcall id */
	int	kbdd_rptid;		/* timeout id for kbdrpt() */
	int	kbdd_layoutid;		/* timeout id for kbdlayout() */
...
kbdopen(queue_t *q, dev_t *devp, int oflag, int sflag, cred_t *crp)
{
	 ...
	 * Setup tty modes.
	 */
	while ((mp = allocb((int)sizeof (struct iocblk), BPRI_HI)) == NULL) {
		int id;

		id = qbufcall(q, sizeof (struct iocblk), BPRI_HI,
				dummy_callback, 0);
		if (!qwait_sig(q)) {
			qunbufcall(q, id);
			return (EINTR);
	...
	while ((datap = allocb((int)sizeof (struct termios), BPRI_HI)) ==
		NULL) {
		int id;

		id = qbufcall(q, sizeof (struct termios), BPRI_HI,
				dummy_callback, 0);
		if (!qwait_sig(q)) {
			qunbufcall(q, id);
			freemsg(mp);
...
kbdclose(register queue_t *q, int flag, cred_t *crp)
{
	...
	kmem_free((void *)kbdd->kbdd_downs, kbdd->kbdd_downs_bytes);
	kmem_free(kbdd, sizeof (struct kbddata));
	return (0);
}
 ...
 * messages.
 * This routine should be type void, the return value is never
 * checked, but AT&T owns the qinit struct.
 * Consequently, we always return 0.
 */
static void
...
static void
kbdreioctl(long kbdd_addr)
{
	struct kbddata *kbdd = (struct kbddata *) kbdd_addr;
	register queue_t *q;
	register mblk_t *mp;
...
kbdioctl(register queue_t *q, register mblk_t *mp)
{
	...
	case KIOCGLED: {
		...
		if ((datap = allocb((int)sizeof (u_char), BPRI_HI)) == NULL) {
	...
	case KIOCLAYOUT: {
		...
		if (kbdd->kbdd_state.k_id == KB_SUN4) {
			if ((datap = allocb((int)sizeof (int),
			    BPRI_HI)) == NULL) {
				ioctlrespsize = sizeof (int);
				goto allocfailure;
			}
...
allocfailure:
	...
	if (kbdd->kbdd_bufcallid)
		qunbufcall(q, kbdd->kbdd_bufcallid);
	kbdd->kbdd_bufcallid = qbufcall(q, (int)ioctlrespsize, BPRI_HI,
					kbdreioctl, (long)(kbdd));
}
 ...
 */
static void
kbduse(register struct kbddata *kbdd, u_char keycode)
{
 ...
 */
static int
kbdsetkey(register struct kbddata *kbdd, struct kiockey *key)
{
	int	strtabindex, i;
	struct	keymap *km;
 ...
 */
static int
kbdskey(register struct kbddata *kbdd, struct kiockeymap *key)
{
	int	strtabindex, i;
	struct	keymap *km;
...
static void
kbdlayouttimeout(register struct kbddata *kbdd)
{
	register mblk_t *mp;
 ...
 */
static void
kbdinput(register struct kbddata *kbdd, register u_char key)
{
	register struct keyboardstate *k;
	register mblk_t *mp;
	...
	case KID_GOT_PREFACE:
		if (key < (u_char)KB_FIRST || key > (u_char)KB_LAST) {
			cmn_err(CE_WARN, "kbd: Unknown keyboard type, "
					"Type %d assumed.", KB_DEFAULT);
			key = KB_DEFAULT;
		}
		kbdid(kbdd, (int)key);
		...
		default:
			{
				...
				(caddr_t) wrkbuf, &len) == DDI_PROP_SUCCESS &&
				len > 0 && len < 8) {
	...
	case COMPOSE2:
		...
		/*
		 * The states for the LAYOUTKEY and the COMPOSE key
		 * conflict.  If the KIOCLAYOUT ioctl is in progress,
		 * the compose key can't be used.
		 */
		if (key == LAYOUTKEY) {
			k->k_state = LAYOUT;
...
kbdid(register struct kbddata *kbdd, int id)
{
	...
	k->k_shiftmask = 0;
	k->k_buckybits = 0;
	k->k_curkeyboard = keytables[k->k_id - KB_FIRST];
...
static void
kbdrpt(register struct kbddata *kbdd)
{
	register struct keyboardstate *k;
...
static void
kbdtranslate(register struct kbddata *kbdd, register u_char keycode,
    register queue_t *q)
{
	...
	if (newstate == PRESSED && entrytype != (SHIFTKEYS >> 8) &&
		...
		if (k->k_rptkey != keycode) {
			kbdcancelrpt(kbdd);
			kbdd->kbdd_rptid = qtimeout(q, kbdrpt, (caddr_t)kbdd,
			    kbd_repeatdelay);
			k->k_rptkey = keycode;
		}
...
static void
kbdkeypressed(register struct kbddata *kbdd, u_char key_station, Firm_event *fe,
...
static void
kbdqueuepress(register struct kbddata *kbdd, u_char key_station, Firm_event *fe)
{
 ...
 */
static void
kbdputcode(u_int code, register queue_t *q)
{
	register mblk_t *bp;
	...
	if (!canput(q))
		...
		if ((bp = allocb((int)sizeof (u_int), BPRI_HI)) == NULL)
			cmn_err(CE_WARN,
 ...
 */
static void
kbdputbuf(char *buf, register queue_t *q)
{
	register mblk_t *bp;
 ...
 */
static void
kbdqueueevent(register struct kbddata *kbdd, Firm_event *fe)
{
	register queue_t *q;
	register mblk_t *bp;
