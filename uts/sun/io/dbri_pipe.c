...
#pragma ident	"@(#)dbri_pipe.c	1.13	94/01/25 SMI"
...
dbri_basepipe_init(dbri_unit_t	*unitp, dbri_chan_t dchan, int *error)
{
	...
	*error = 0;
	...
	case DBRI_PIPE_CHI:
		...
		unitp->timestamp[2] = 0;	/* Audio is now in use */
	...
	case DBRI_PIPE_NT_D_IN:
		if (!ISPIPEINUSE(bpipep)) {
			...
			unitp->timestamp[1] = 0; /* line drivers are in use */
			ddi_dev_is_needed(unitp->dip, 1, 1);
