...
#pragma	ident  "@(#)consmsconf.c 1.33     94/11/28 SMI" 	/* SVr4 */
...
consconfig(void)
{
	...
	if (prom_stdin_devname(devname) != 0)
		goto not_on_same_zs;

	if (strcmp(devname, "zs") != 0)
		goto not_on_same_zs;

	if ((stdiopath = prom_stdinpath()) == 0)
		goto not_on_same_zs;		/* XXX */
...
not_on_same_zs:
	 ...
	 */
	if (error = strioctl(kbdvp, I_PUSH, (int) "kb", FREAD+FNOCTTY, K_TO_K,
	    kcred, &rval)) {
	...
	(void) strioctl(kbdvp, TIOCSPGRP, (int)&zeropgrp, FREAD+FNOCTTY,
	    K_TO_K, kcred, &rval);

	if (error = strioctl(kbdvp, KIOCTRANSABLE, (int)&kbdtranslatable,
	    FREAD+FNOCTTY, K_TO_K, kcred, &rval))
	...
	if (kbdtranslatable == TR_CANNOT) {
		 ...
		 * XXX: Assuming "zs"
		 */
		(void) strioctl(kbdvp, TCGETS, (int)&termios, FREAD+FNOCTTY,
		    K_TO_K, kcred, &rval);

		termios.c_cflag &= ~(CBAUD|CIBAUD);
		termios.c_cflag |= kbdspeed;

		if (error = strioctl(kbdvp, TCSETSF, (int)&termios,
		    FREAD+FNOCTTY, K_TO_K, kcred, &rval)) {
	...
	if (prom_stdin_is_keyboard())  {
		...
		stdindevname[0] = (char)0;
		(void) prom_stdin_devname(stdindevname);
		is_a_zs = (strcmp("zs", stdindevname) == 0);
		if (is_a_zs)
		...
		if (error = strioctl(stdinvp, I_PUSH, (int) "kb",
		    FREAD+FNOCTTY, K_TO_K, kcred, &rval))  {
			cmn_err(CE_WARN, (char *)discmsg, emsg,
			    "keyboard", error);
		}

		(void) strioctl(stdinvp, TIOCSPGRP, (int)&zeropgrp,
		    FREAD+FNOCTTY, K_TO_K, kcred, &rval);

		if (error = strioctl(stdinvp, KIOCTRANSABLE,
		    (int)&translatable, FREAD+FNOCTTY, K_TO_K, kcred, &rval))
			cmn_err(CE_WARN, "%s: KIOCTRANSABLE failed error: %d",
			    emsg, error);
		...
		if (is_a_zs)  {
			...
			(void) strioctl(stdinvp, TCGETS, (int)&termios,
			    FREAD+FNOCTTY, K_TO_K, kcred, &rval);
...
stdin_conf_done:
	...
	/* now we must close it to make console logins happy */
	ddi_hold_installed_driver(getmajor(rwsconsdev));
