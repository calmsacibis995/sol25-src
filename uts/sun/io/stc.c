...
#pragma ident	"@(#)stc.c 1.92	95/03/14 SMI"
...
/*
 * local driver data
 *    stc_driverid - char pointer to a string that gets displayed
 *	after a sucessful attach
...
static char *stc_driverid = STC_DRIVERID;
static int nstc = 0;
 ...
 * cd180 functions
 */
static char *stc_mkdevname(/* char *proto, int index, int count */);
static int stc_open();
static int stc_close(/* queue_t *q, int flag */);
static int stc_wput(/* queue_t *q, mblk_t *mp */);
static int stc_ioctl(/* queue_t *q, mblk_t *mp */);
static void stc_reioctl(/* struct stc_line_t *line */);
static void stc_restart(/* struct stc_line_t *line */);
static void stc_start(/* struct stc_line_t *line */);
static void stc_param(/* struct stc_line_t *line */);
static int stc_xmit(/* int unit */);
static int stc_rcv(/* int unit */);
static int stc_rcvexp(/* int unit */);
static int stc_modem(/* int unit */);
static void stc_drainsilo(/* struct stc_line_t *line */);
static void stc_draintimeout(/* struct stc_line_t *line */);
static void silocopy(/* struct stc_line_t *line, u_char *buf, int count */);
static void stc_xwait(/* struct stc_line_t *line */);
static void stc_timeout(/* struct stc_line_t *line */);
static u_int stc_poll();
static u_int stc_softint();
static u_int stc_softint_line(/* struct stc_line_t *line */);
static int stc_init(/* int unit */);
static int stc_setdefaults(
	/* struct stc_line_t *line, struct stc_defaults_t *stc_defaults */);
static int stc_getdefaults(/* devinfo_t *dip, u_int tag, stc_defaults_t *
	stc_defaults */);
static int ppc_getdefaults(/* devinfo_t *dip, u_int tag, stc_defaults_t *
	stc_defaults */);
static int stc_extractval(/* char *strbuf, int *offset, char ** token,
	int *value */);
static void stc_txflush(/* struct stc_line_t *line */);
static int stc_identify(/* dev_info_t *, ddi_attach_cmd_t */);
static int stc_attach(/* dev_info_t *dip, ddi_detach_cmd_t cmd */);
static int stc_detach(/* dev_info_t *dip, ddi_detach_cmd_t cmd */);
static int stc_getinfo(/* dev_info_t *, ddi_info_cmd_t, void *, void ** */);

#ifdef	STC_ALIVE
static void stc_alive(struct stc_unit_t *stc);
int stc_alive_id;
#endif	STC_ALIVE
 ...
 * ppc functions
 */
static u_int ppc_poll();
static int ppc_int(/* int unit_no */);
static void ppc_out(/* struct stc_line_t *line */);
static void ppc_acktimeout(/* struct stc_line_t *line */);
...
static struct cb_ops stc_cb_ops = {
	...
	&stc_stab,	/* stream tab */
	D_NEW|D_MTSAFE	/* Driver Compatability Flags */
};
 ...
 */
static u_char stc_break[EMBED_XMIT_LEN] =
...
#define	SBREAK_LEN	2
#define	EBREAK_LEN	2
static u_char stc_sbreak[SBREAK_LEN] = { 0x00, 0x81 };
static u_char stc_ebreak[EBREAK_LEN] = { 0x00, 0x83 };
 ...
 */
unsigned static short stc_baud_10[16] = {
	0x00,	/* B0 */
	0x30d4, /* B50 */
	0x208d, /* B75 */
	0x1632, /* B110 */
 ...
 */
unsigned static short stc_baud_9[16] = {
	0x00,	/* B0 */
	0x3000, /* B50 */
	0x2000, /* B75 */
	0x15d1, /* B110 */
...
/*
 * stc_mkdevname(proto, index, count) - used to return a device node
 *	name filled into the proto array
 */
static char *
stc_mkdevname(proto, index, count)
	char *proto;
	int index, count;
{
	int len;
	...
	switch (count) {
		case 1:
		index &= 0x0f;
		sprintf(&proto[len-count], "%x", index);
		break;

		case 2:
		index &= 0x0ff;
		/* proto[len-count+1] = (char)NULL; */
		sprintf(&proto[len-count], "%2x", index);
		break;
...
/*
 * stc_identify(dev_info_t *dip) - this driver entry point idenifies the
 *	device for the kernel.	If the name of the dip node matches the
 *	driver name the identify function returns successfully.
 *
 *	Returns:	DDI_SUCCESS, if the name matches that of the driver.
 *			DDI_FAILURE, if name does not match.
 */
static int
stc_identify(dev_info_t *dip)
{
	char *name = ddi_get_name(dip);

	SDM(STC_TRACK, (CE_CONT, "stc_identify: name = %s\n", name));

	if (strcmp(name, STC_NAME) == 0) {
	    SDM(STC_TRACK, (CE_CONT,
		"stc_identify: returning (DDI_IDENTIFIED) %d\n",
		DDI_IDENTIFIED));
	    return (DDI_IDENTIFIED);
	} else {
	    SDM(STC_TRACK, (CE_CONT,
		"stc_identify: returning (DDI_NOT_IDENTIFIED) %d\n",
		DDI_NOT_IDENTIFIED));
...
/* ************* LOOK do we really need all of these checks?? ************* */
	...
	/*
	 * in the SunOS 4.X version of this code, we checked the sun4c system
	 *	boot PROM revision and if it was 1.0, we fiddled with the
	 *	interrupt levels given to us in the device tree.  In this
	 *	version of the code, the system boot PROM on a sun4c must
	 *	be at least 1.3.  If it's not, lots of things will break.
	 */
	...
	if (ddi_map_regs(dip, 0, (caddr_t *)&stc->prom, (off_t)0,
		...
		for (line_no = 0; line_no < STC_LINES; line_no++) {
			...
			(u_char *)(stc->prom + DTR_OFFSET + line_no);
		}
		stc->regs =
			(struct stcregs_t *)((caddr_t)stc->prom + CD180_OFFSET);
		stc->iack =
			(struct stciack_t *)((caddr_t)stc->prom + IACK_OFFSET);
		stc->ppc =
			(struct ppcregs_t *)((caddr_t)stc->prom + PPC_OFFSET);
		/*
	...
	error = ddi_add_intr(dip, (u_int)STC_INT_NUM, &stc->stc_blk_cookie,
		...
		/* &stc->stc_dev_cookie, stc_poll, (caddr_t)stc); */
	...
	error = ddi_add_softintr(dip, STC_SOFT_PREF, &stc->softint_id,
		...
		(caddr_t)stc);
		/*
		 * &stc->soft_blk_cookie, &stc->soft_dev_cookie, stc_softint,
		 * (caddr_t)stc);
		 */
	if (error != DDI_SUCCESS) {
	...
	mutex_init(stc->stc_mutex, MUTEX_STC_NAME, MUTEX_DRIVER,
		(void *)stc->stc_blk_cookie);
	stc->ppc_mutex = kmem_zalloc(sizeof (kmutex_t), KM_SLEEP);
	mutex_init(stc->ppc_mutex, MUTEX_PPC_NAME, MUTEX_DRIVER,
	...
	for (line_no = 0; line_no < STC_LINES; line_no++) {
		...
		mutex_init(line->line_mutex, MUTEX_LINE_NAME, MUTEX_DRIVER,
			(void *)stc->soft_blk_cookie);
		line->cvp = kmem_zalloc(sizeof (kcondvar_t), KM_SLEEP);
		cv_init(line->cvp, CV_SOFT_NAME, CV_DRIVER, (void *)NULL);
	}
	 ...
	 */
	for (line_no = 0; line_no < STC_LINES; line_no++) {
	    bzero((caddr_t)&stc->line[line_no].stc_stats,
	...
	for (line_no = 0; line_no < STC_LINES; line_no++) {
	    ...
	    sprintf(stc_ttyname, "%d,cu", line_no);
	    if (ddi_create_minor_node(dip, stc_ttyname, S_IFCHR,
...
#ifdef	STC_ALIVE
	stc_alive_id = timeout(stc_alive, (caddr_t)stc, (STC_TIMEOUT*2));
#endif	STC_ALIVE
	...
	struct stc_unit_t *stc;
{
	...
	stc_alive_id = timeout(stc_alive, (caddr_t)stc, STC_TIMEOUT);
}
#endif	STC_ALIVE
...
stc_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	cmn_err(CE_CONT, "stc%d: detaching unit\n", instance);
...
stc_init(struct stc_unit_t *stc)
{
	 ...
	 */
	if (ddi_pokec(stc->dip, (char *)&stc->regs->givr, 0) != DDI_SUCCESS) {
		SDM(STC_DEBUG, (CE_CONT,
			"stc_init%d: ERROR 1 on pokec of 0x%x\n",
			unit, (char *)&stc->regs->givr));
		return (-1);
	}
	if (ddi_pokec(stc->dip, (char *)&stc->regs->ccr, CD180_RESET_ALL) !=
		DDI_SUCCESS) {
	...
	if (ddi_peekc(stc->dip, (char *)&stc->regs->givr, (char *)&r) !=
		DDI_SUCCESS) {
	...
	if (ddi_peekc(stc->dip, (char *)&stc->regs->gfrcr, (char *)&r) !=
		DDI_SUCCESS) {
	 ...
	 */
	for (i = 0; i < STC_LINES; i++) {
	    if (ddi_pokec(stc->dip, (char *)stc->line[i].dtr, (char)DTR_OFF) !=
		DDI_SUCCESS) {
	...
	for (i = 0; i < STC_LINES; i++) {
	    ...
	    bcopy((char *)&stc_initmodes, (char *)&stc_defbuf,
	...
	line->line_no = 0;
	line->stc = stc;		/* ugh... kludge */
	line->dtr = (u_char *)NULL;
	bcopy((char *)&ppc_initmodes, (char *)&stc_defbuf,
		sizeof (struct stc_defaults_t));
	ppc_getdefaults(stc->dip, unit, &stc_defbuf);
	stc_setdefaults(line, &stc_defbuf);
	line->default_param.pcon = PCON_OPEN_INIT;
...
cd180_init(struct stc_unit_t *stc, int chan)
{
	...
	for (i = start_chan; i < end_chan; i++) {
	    stc->regs->car = (u_char)i; /* set channel number */
 ...
 */
static u_int
stc_softint(struct stc_unit_t *stc)
{
	register struct stc_line_t *line;
	register queue_t *q;
 ...
 */
static u_int
stc_poll(struct stc_unit_t *stc)
{
	int serviced = DDI_INTR_UNCLAIMED, line_no;
	u_char givr;
...
stc_drainsilo(line)
	register struct stc_line_t *line;
{
	register queue_t *q = line->stc_ttycommon.t_readq;
	...
	if (line->state & STC_CANWAIT) {
		UNTIMEOUT(line->stc_draintimeout_id);
		mutex_enter(stc->stc_mutex);
		line->state &= ~STC_CANWAIT;
		mutex_exit(stc->stc_mutex);
		line->stc_qfcnt = 0;
...
"stc_drainsilo: unit %d line %d can't allocate %d byte streams buffer\n",
		...
		"stc_drainsilo: unit %d line %d punting after %d put retries\n",
			...
			line->stc_draintimeout_id =
				timeout(stc_draintimeout, (caddr_t)line,
				QFRETRYTIME);
 ...
 */
static void
silocopy(line, buf, count)
	register struct stc_line_t *line;
	register u_char *buf;
	register count;
{
	register u_char *sink = line->stc_sink;
	register cc = count;
	...
	if (sink < line->stc_source) {
		bcopy((caddr_t)sink, (caddr_t)buf, cc);
	} else {
 ...
 */
static int
stc_open(q, dev, oflag, sflag, credp)
	register queue_t *q;
	dev_t *dev;
	int oflag, sflag;
	cred_t *credp;
...
again:
	...
	if (!(line->state & (STC_LP | STC_CONTROL))) {
		 ...
		 */
		mutex_enter(stc->stc_mutex);
		stc->regs->car = (u_char)line->line_no;
		if (line->flags & (DTR_ASSERT|STC_DTRFORCE)) {
			/*
			 * assert DTR on open
			 */
 ...
 */
static int
stc_close(q, flag, credp)
	register queue_t *q;
	int flag;
	cred_t *credp;
{
	register struct stc_line_t *line;
	register struct stc_unit_t *stc;
	register int s;
	...
	if (!(line->state & STC_LP)) {
		...
		if (line->state & STC_INSOFTINT) {
			cmn_err(CE_CONT,
			"stc_close: ignoring softint on line %d %x\n",
			line->line_no, line);
		}
 ...
 */
static void
stc_mcopy(mp, dp, size, type, cmd, useraddr)
	mblk_t		*mp;
	mblk_t		*dp;
	uint		size;
	unsigned char	type;
	...
	switch (mp->b_datap->db_type) {
		...
		switch (((struct iocblk *) mp->b_rptr)->ioc_cmd) {
			...
			case TIOCMGET:
				...
				if (stp == (struct stc_state_t *) NULL)
	...
	register struct stc_line_t *line;
{
	...
	mutex_enter(line->line_mutex);
	line->stc_wbufcid = 0;
	if (!(q = line->stc_ttycommon.t_writeq)) {
	    mutex_exit(line->line_mutex);
	    return;
	}
	if (mp = line->stc_ttycommon.t_iocpending) {
	...
	register mblk_t *mp;
{
	 ...
	 */
	if (datasize = ttycommon_ioctl(&line->stc_ttycommon, q, mp, &error)) {
	    SDM(STC_DEBUG, (CE_CONT,
		...
		unbufcall(line->stc_wbufcid);
	    if (!(line->stc_wbufcid =
		bufcall(datasize, BPRI_HI, stc_reioctl, (long)line))) {
		cmn_err(CE_CONT,
	 ...
	 */
		 ...
		 * ignore it we could really seperately enable RTS and CTS
		 * flow control, but we don't right now
		 */
		case TCSETX:
		case TCSETXW:
		case TCSETXF: {
		...
		case STC_SREGS:
			if ((line->state & STC_ISROOT) &&
				 ...
				 */
				register struct stc_line_t *Line = line;
	...
	"stc_ioctl: unit %d line %d can't allocate STC_SREGS block\n",
		...
		"stc_ioctl: unit %d line %d can't allocate STC_GSTATS block\n",
				...
				if (lcmd & STAT_CLEAR)
				    bzero((caddr_t)&stc->line[ln].stc_stats,
					sizeof (struct stc_stats_t));
				bcopy((caddr_t)&stc->line[ln].stc_stats,
					(caddr_t)stc_stats,
					sizeof (struct stc_stats_t));
 ...
 */
static int
stc_dodiagregs(line, stc_diagregs, iocmd)
	register struct stc_line_t *line;
	register struct stc_diagregs_t *stc_diagregs;
	register u_int iocmd;
{
	register struct stc_unit_t *stc = line->stc;
	register u_char *cregs;
	register int s;
	...
	switch (stc_diagregs->reg_flag) {
		case STC_IOREG:	    /* dink the cd180 register */
		    if (iocmd == STC_GREGS) {
			mutex_enter(stc->stc_mutex);
			cregs = (u_char *)stc->regs + stc_diagregs->reg_offset;
		...
		case STC_PPCREG:	/* dink the PPC register */
		    if (iocmd == STC_GREGS) {
			mutex_enter(stc->ppc_mutex);
			cregs = (u_char *)stc->ppc + stc_diagregs->reg_offset;
			stc_diagregs->reg_data = *cregs;
			mutex_exit(stc->ppc_mutex);
		    } else {
			mutex_enter(stc->ppc_mutex);
	...
	register struct stc_line_t *line;
{
	...
	if (!(line->state & STC_DRAIN)) {
		 ...
		 */
		line->stc_timeout_id =
			timeout(stc_timeout, (caddr_t)line, STC_TIMEOUT);
		if (!cv_wait_sig(line->cvp, line->line_mutex)) {
			UNTIMEOUT(line->stc_timeout_id);
			SDM(STC_DEBUG, (CE_CONT,
	...
	register struct stc_line_t *line;
{
	register queue_t *q;
	register mblk_t *bp;
	register cc, bytesleft;
	int i = 0;
	register struct stc_unit_t *stc = line->stc;
	register struct stcregs_t *regs = stc->regs;
	register u_char *current;
	...
	while ((bp = getq(q)) != NULL) {
		...
		case M_DELAY:
			...
			line->state |= STC_DELAY;
			mutex_exit(stc->stc_mutex);
			(void) timeout(stc_restart, (caddr_t)line,
...
transmit:
	...
	if (!(line->state & (STC_DRAIN | STC_CONTROL))) {
	    if (cc = (line->stc_txbufsize - bytesleft)) {
		SDM(STC_DEBUG, (CE_CONT,
			"stc_start: xmit %d\n", line->stc_txcount));
		if (line->state & STC_LP) {
			/*
 ...
 */
static int
stc_getdefaults(dip, tag, stc_defaults)
	dev_info_t *dip;
	u_int tag;
	struct stc_defaults_t *stc_defaults;
{
	...
	sprintf(defname, "%s%d", STC_SERIAL_PROP_ID, tag);
 ...
 */
static int
ppc_getdefaults(dip, tag, stc_defaults)
	dev_info_t *dip;
	u_int tag;
	struct stc_defaults_t *stc_defaults;
{
	...
	sprintf(defname, "%s%d", STC_PARALLEL_PROP_ID, tag);
...
static int
stc_extractval(strbuf, offset, token, value)
	char *strbuf;
	int *offset;
	char **token;
	int *value;
 ...
 */
static int
stc_setdefaults(line, stc_defaults)
	register struct stc_line_t *line;
	register struct stc_defaults_t *stc_defaults;
{
	register ispeed, ospeed;
	 ...
	 * termios structure
	 */
	bcopy((caddr_t)&stc_defaults->termios,
		(caddr_t)&line->default_param.sd.termios,
...
/*
 * stc_getlineparams(line, stc_defaults) - get current paramters for a line
 *
 * note that none of the termios settings are returned
 * we assume that we're coming here if the requested line is already open
 */
static int
stc_getlineparams(line, stc_defaults)
	register struct stc_line_t *line;
	register struct stc_defaults_t *stc_defaults;
{
	register struct stc_unit_t *stc = line->stc;
	...
	if (line->state & STC_LP) {
		stc_defaults->strobe_w = line->strobe_w;
		stc_defaults->data_setup = line->data_setup;
		/*
		 * convert the timeouts from ticks to return seconds to the user
		 */
		stc_defaults->ack_timeout =
			((line->ack_timeout * line->ack_maxloops) / hz);
		stc_defaults->error_timeout = (line->error_timeout / hz);
		stc_defaults->busy_timeout =
			((line->busy_timeout * BUSY_COUNT_C) / hz);
	} else {
	    register struct stc_unit_t *stc = line->stc;
	    int s;
		/*
		 ...
		 */
		stc_defaults->drain_size = line->drain_size;
		stc_defaults->stc_hiwater = line->stc_hiwater;
		stc_defaults->stc_lowwater = line->stc_lowwater;
		/*
		 * disable interrupts from the serial port while we get
		 * the RTPR and COR3 (RxFIFO threshold) register values
		 */
		mutex_enter(stc->stc_mutex);
		stc->regs->car = (u_char)line->line_no;
		stc_defaults->rtpr = stc->regs->rtpr;
		stc_defaults->rx_fifo_thld = (stc->regs->cor3 & RX_FIFO_MASK);
		mutex_exit(stc->stc_mutex);
	}

	/*
	 * return the current default flags
	 */
	stc_defaults->flags = line->flags;

	/*
	 * return the line type in the op field
	 * STC_CONTROL is taken care of in stc_ioctl()
	 */
	if (line->state & STC_LP)
		stc_defaults->op = STC_PARALLEL;
	else
		stc_defaults->op = STC_SERIAL;

	return (0);
}

/*
 * stc_setlineparams(line, stc_defaults) - set up paramters for a line
 * immediately
 *
 * note that none of the termios settings are set, neither is the state of
 * the DTR line modified
 *
 * we assume that we're coming here if the requested line is already open
 */
static int
stc_setlineparams(line, stc_defaults)
	register struct stc_line_t *line;
	register struct stc_defaults_t *stc_defaults;
{
	...
	/*
	 * do a simple sanity check on the ppc parameters - isn't there a
	 * better way to do this? (yeah - trust the user to always give
	 * you the right values... ha!)
	 */
	if (line->state & STC_LP) {
		if (FUNKY(stc_defaults->strobe_w, MIN_STBWIDTH, MAX_STBWIDTH) ||
			FUNKY(stc_defaults->data_setup, MIN_DATASETUP,
				MAX_DATASETUP) ||
			FUNKY(stc_defaults->ack_timeout, MIN_ACKTIMEOUT,
				MAX_ACKTIMEOUT) ||
			FUNKY(stc_defaults->busy_timeout, MIN_BUSYTIMEOUT,
				MAX_BUSYTIMEOUT) ||
			FUNKY(stc_defaults->error_timeout, MIN_ERRTIMEOUT,
				MAX_ERRTIMEOUT))
			return (EINVAL);

		line->strobe_w = stc_defaults->strobe_w;
		line->data_setup = stc_defaults->data_setup;
		/*
		 * the user passed us seconds, we convert to ticks
		 */
		line->ack_maxloops = (stc_defaults->ack_timeout / PPC_ACK_POLL);
		line->ack_timeout = (PPC_ACK_POLL * hz);
		line->busy_timeout =
			((stc_defaults->busy_timeout * hz) / BUSY_COUNT_C);
		line->error_timeout = (stc_defaults->error_timeout * hz);
	} else {
		register struct stc_unit_t *stc = line->stc;
		u_char cor3;
		int s;
		if (FUNKY(stc_defaults->drain_size, MIN_DRAIN_SIZE,
			MAX_DRAIN_SIZE) ||
			FUNKY(stc_defaults->stc_hiwater, MIN_HIWATER,
				MAX_HIWATER) ||
			FUNKY(stc_defaults->stc_lowwater, MIN_LOWWATER,
				MAX_LOWWATER) ||
			FUNKY(stc_defaults->rtpr, MIN_RTPR, MAX_RTPR) ||
			FUNKY(stc_defaults->rx_fifo_thld, MIN_RX_FIFO,
				MAX_RX_FIFO))
			return (EINVAL);

		line->drain_size = stc_defaults->drain_size;
		line->stc_hiwater = stc_defaults->stc_hiwater;
		line->stc_lowwater = stc_defaults->stc_lowwater;
		/*
		 * disable interrupts from the serial port while we modify
		 * the RTPR and COR3 (RxFIFO threshold) registers
		 */
		mutex_enter(stc->stc_mutex);
		stc->regs->car = (u_char)line->line_no;
		stc->regs->rtpr = stc_defaults->rtpr;
		cor3 = stc->regs->cor3;
		cor3 &= ~RX_FIFO_MASK;
		cor3 |= (stc_defaults->rx_fifo_thld & RX_FIFO_MASK);
		stc->regs->cor3 = cor3;
		SET_CCR(stc->regs->ccr, COR3_CHANGED|COR_CHANGED);
		mutex_exit(stc->stc_mutex);
	}
	 ...
	 * set the line flags like SOFT_CARR, DTR_ASSERT, etc...
	 */
	line->flags = stc_defaults->flags;
	if ((stc_defaults->flags & SOFT_CARR) || (line->state & STC_LP))
		line->stc_ttycommon.t_flags |= TS_SOFTCAR;
	else
		line->stc_ttycommon.t_flags &= ~TS_SOFTCAR;
 ...
 */
static u_int
ppc_poll(struct stc_unit_t *stc)
{
...
{
	register struct ppcregs_t *regs = stc->ppc;
	register struct stc_line_t *line = &stc->ppc_line;
	...
	register struct stc_line_t *line;
{
	...
	if (ppc_stat(regs, line)) {
		line->ppc_out_id =
			timeout(ppc_out, (caddr_t)line, line->error_timeout);
		mutex_exit(line->line_mutex);
		return;
	}
	...
	if (PPC_BUSY(regs->pstat, line->flags)) {
		...
		if (line->busy_cnt++ >= BUSY_COUNT_C) {
			...
			line->ppc_out_id =
				timeout(ppc_out, (caddr_t)line, busy_timeout);
			mutex_exit(line->line_mutex);
			return;
		}
	}
	...
	regs->pcon = line->pcon_s;
	mutex_exit(stc->ppc_mutex);
	line->ppc_acktimeout_id =
		timeout(ppc_acktimeout, (caddr_t)line, line->ack_timeout);
	mutex_exit(line->line_mutex);
	...
	register struct stc_line_t *line;
{
	...
	if (!(line->state & STC_DRAIN)) {
		...
		if (ppc_stat(regs, line)) {
			line->state |= STC_ACKWAIT;
			line->ppc_acktimeout_id =
				timeout(ppc_acktimeout, (caddr_t)line,
				line->error_timeout);
			mutex_exit(line->line_mutex);
			return;
		}
...
static int
ppc_stat(regs, line)
	register struct ppcregs_t *regs;
	register struct stc_line_t *line;
{
	register int ssig = 0;
