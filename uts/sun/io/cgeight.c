 ...
 * All rights reserved.
 */
#pragma ident	"@(#)cgeight.c	1.16	95/01/10 SMI"	/* SunOS-4.1.1 1.6 */
...
static int cgeightioctl(dev_t, int, int, int, cred_t *, int *);
	...
	ddi_attach_cmd_t cmd;
{
	...
	mutex_init(&softc->softc_mutex, "tc1 softc",
		    MUTEX_DRIVER, softc->cg8_iblock_cookie);

	mutex_init(&softc->busy_mutex, "tc1 busy",
		    MUTEX_DRIVER, softc->cg8_iblock_cookie);
	...
	int	 prot; /* (unused) */
{
	...
	if (off >= 0x00900000) {
		...
		DEBUGF(2, (CE_CONT,
		    "Returning offset %x as phys_off %x and address %x\n",
		    off, phys_off,
...
/*
 * Does the inverse of cg8_attach(), given that cg8_attach() succeeded.
 *
 * XXX	Warning: this is NOT an entry point routine, it's
 *	just convenient structurally, and is roughly how
 *	it might look one day..
 */
#ifdef SOMEDAY
static int
cg8_detach(register dev_info_t *devi)
{
	register int unit;
	register struct Tc1_Softc *softc;
	volatile Tcs_Device_Map  *device_tcs;

	unit = ddi_get_instance(devi);
	softc = getsoftc(unit);

#ifdef DWP_LATER
	if (!softc->mapped_by_prom && softc->ctl_sp != NULL)
		softc->ctl_sp->apu.vsg_ctl &= ~CG8_APU_EN_VID;
#endif

	/* shut down interrupts from board */
	switch (softc->fb_model) {

	default:
		if (softc->device_reg != NULL)
	...
	if (softc->cg8_iblock_cookie != NULL)
		ddi_remove_intr(devi, (u_int)0,
	...
	mutex_destroy(&softc->softc_mutex);
	...
	SOFTC	   *softc; /* ->per-unit data struct to be filled in. */
{
	 ...
	 *  of the board.
	 */
	DEBUGF(5, (CE_CONT, "tcp_attach softc at %x\n", softc));
	...
	DEBUGF(5, (CE_CONT, "tcp_attach reg=0x%x basepage=0x%x (0x%x)\n",
	...
	SOFTC	  *softc; /* -> per-unit data struct to be filled in. */
{
	 ...
	 */
	DEBUG_LEVEL(5);
	DEBUGF(5, (CE_CONT, "tcs_attach softc at %x\n", softc));
	...
	DEBUGF(5, (CE_CONT, "tcs_attach reg=0x%x basepage=0x%x (0x%x)\n",
