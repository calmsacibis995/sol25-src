...
#pragma ident	"@(#)sbus.c	1.27	94/12/02 SMI"	/* SVr4 5.0 */
...
sbus_add_intrspec(dev_info_t *, dev_info_t *,
    ...
    u_int (*int_handler)(caddr_t), caddr_t, int);
    ...
    ddi_ctl_enum_t op, void *arg, void *result)
{
	...
	if (op == DDI_CTLOPS_REPORTDEV) {
		...
		for (i = 0, n = sparc_pd_getnreg(rdip); i < n; i++) {
			...
			/*
			 * XXX	Maybe we should print 'onboard SBus'
			 *	.. though 4c onboard is slot 0 and 4m
			 *	onboard is slot f .. so this is tricky.
			 */
			cmn_err(CE_CONT, "?SBus slot %x 0x%x",
	...
	if (op == DDI_CTLOPS_SLAVEONLY) {
		if (DEVI_PD(rdip) && sparc_pd_getnreg(rdip) > 0) {
			/*
			 * Only early sun4c needs default for slave-only.
			 * Other archs have no slave-only slots. The test
			 * based on romvec version is actually generic.
			 *
			 * XXX: For sun4e, defvalue should be 4, always.
			 * To do this, set up a slave-only root node
			 * property in sun4e's autoconf.c:add_root_props()
			 * table with integer value 4.
			 */
			int defvalue = (prom_getversion() == 0) ? 8 : 0;
