...
#pragma	ident	"@(#)cgthree.c	1.38	95/03/17 SMI"
...
static int cg3_ioctl(dev_t, int, int, int, cred_t *, int *);
...
_init(void)
{
	...
	if ((e = mod_install(&modlinkage)) != 0) {
		...
		DEBUGF(1, (CE_CONT, "done, error mod_install\n", e));
	}
	return (e);
}
...
cg3_attach(register dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	DEBUGF(1, (CE_CONT, "cg3_attach%d fb 0x%x kpfnum 0x%x\n",
		instance, (u_int)softc->fb,
	...
	mutex_init(&softc->softc_lock,
		"cgthree softc", MUTEX_DRIVER, softc->iblkc);
	...
	timestamp[0] = 0;
	(void) drv_getparm(TIME, timestamp+1);
	...
	DEBUGF(2, (CE_CONT,
	    "cg3_attach: softc->size=0x%x; softc->fb=0x%x; softc->reg=0x%x\n",
	    softc->size, softc->fb, softc->reg));
...
cg3_detach(register dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	if (ddi_prop_remove(DDI_DEV_T_NONE, devi, "pm_timestamp")
		...
		"pm_norm_pwr") != DDI_PROP_SUCCESS)
			cmn_err(CE_WARN, "cg3_Detach: Unable to "
			    "remove property");
...
cg3_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg, void **result)
{
	...
	case DDI_INFO_DEVT2INSTANCE:
		DEBUGF(2, (CE_CONT,
		    "cg3_info(DDI_INFO_DEVT2INSTANCE) arg=0x%x\n",
		    arg));
...
{
	register int		instance = getminor(dev);
	register struct cg3_softc *softc = getsoftc(getminor(dev));
	...
	if (off == MFB_REG_MMAP_OFFSET) {
		 ...
		 */
		DEBUGF(2, (CE_CONT, "cg3_mmap() no mapping off=0x%x\n", off));

		DEBUGF(2, (CE_CONT,
		"cg3_mmap: MFB_REG_MMAP_OFFSET=0x%x; CG3_MMAP_OFFSET=0x%x\n",
		    MFB_REG_MMAP_OFFSET, CG3_MMAP_OFFSET));

		DEBUGF(2, (CE_CONT,
		"cg3_mmap: softc->size=0x%x; softc->fb=0x%x; softc->reg=0x%x\n",
		    softc->size, softc->fb, softc->reg));
...
static int
cg3_ioctl(dev_t dev,
    register int cmd, register int arg, int mode, cred_t *cred_p, int *rval_p)
{
	register int instance = getminor(dev);
	register struct cg3_softc *softc = getsoftc(instance);
...
"cg3_ioctl() FBIOget/putCMAP; PIX_ATTRGROUP(index=%d)=0x%x\n",
	...
	case FBIOGATTR: {
		...
		DEBUGF(2, (CE_CONT, "FBIOGATTR; wide=%d; high=%d; size=0x%x\n",
	...
	case FBIOGTYPE: {
		...
		DEBUGF(2, (CE_CONT, "FBIOGTYPE; wide=%d; high=%d; size=0x%x\n",
			softc->_w, softc->_h, softc->size));
...
cg3_intr(register caddr_t int_handler_arg)
{
	...
	int i = mfb_int_pending(softc->reg);
...
{
	DEBUGF(4, (CE_CONT, "cg3_reset_cmap(); cmap=0x%x; entries=%d\n",
	    cmap, entries));
...
cg3_cmap_bcopy(register u_char *bufp, register u_char *rgb, u_int count)
{
	...
	DEBUGF(4, (CE_CONT,
	    "cg3_cmap_bcopy(); bufp=0x%x; rgb=0x%x; count=%d\n",
	    bufp, rgb, count));
...
cg3_pseudoPROM(register dev_info_t *devi)
{
	 ...
	 */
	if (ddi_peekc(devi, (char *)&mfb->status, &status) == DDI_FAILURE) {
		goto failed;
		/*NOTREACHED*/
	}
