...
#pragma ident	"@(#)dbri_subr.c	1.41	95/06/20 SMI"
...
dbri_initchip(dbri_unit_t *unitp)
{
	 ...
	 */
	bzero((caddr_t)unitp->intq.intq_bp,
	    DBRI_NUM_INTQS * sizeof (dbri_intq_t));
	bzero((caddr_t)unitp->cmdqp, sizeof (dbri_cmdq_t));
	...
	for (i = 0; i < (DBRI_NUM_INTQS - 1); ++i, qp++) {
		...
		qp->nextq = DBRI_IOPBIOADDR(unitp, nqp);
	}

	/* last queue points back to first */
	qp->nextq = DBRI_IOPBIOADDR(unitp, unitp->intq.intq_bp);
	 ...
	 */
	bzero((caddr_t)unitp->ptab, sizeof (unitp->ptab));
...

	{
		...
		cmd.arg[0] = (uint_t)DBRI_IOPBIOADDR(unitp,
		    unitp->intq.curqp);
...
dbri_command(dbri_unit_t *unitp, dbri_chip_cmd_t cmd)
{
	...
	case 1:
		...
		DBRI_SYNC_CMDQ(unitp, 1, DDI_DMA_SYNC_FORDEV);
		break;
	...
	case 2:
		...
		cmdq->cmd[--tmp_cmdp] = cmd.opcode;
		DBRI_SYNC_CMDQ(unitp, 2, DDI_DMA_SYNC_FORDEV);
		break;
	...
	case 3:
		...
		cmdq->cmd[--tmp_cmdp] = cmd.arg[0];
		cmdq->cmd[--tmp_cmdp] = cmd.opcode;
		DBRI_SYNC_CMDQ(unitp, 3, DDI_DMA_SYNC_FORDEV);
		break;
...
dbri_keep_alive(caddr_t arg)
{
	...
	if (unitp->keep_alive_running && !unitp->openinhibit) {
		...
		unitp->keepalive_id = timeout(dbri_keep_alive, (caddr_t)unitp,
