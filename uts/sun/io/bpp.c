...
#pragma ident	"@(#)bpp.c	1.21	95/09/25 SMI"
...
static	ddi_dma_lim_t	bpp_limits = {
	...
	(u_long)0xffffffff,	/* upper range limit */
...
static	int	bpp_identify(dev_info_t *dip);
...
static	int	bpp_ioctl(dev_t dev, int cmd, int arg, int flag,
			cred_t *credp, int *rvalp);
static	u_int	bpp_intr();
...
static	u_short check_write_params(struct  bpp_transfer_parms *parms_p,
		int unit, int flags);
static	u_short check_read_params(struct  bpp_transfer_parms *parms_p,
		u_int unit, int flags);
static	u_short check_read_pins(struct  bpp_pins *pins_p,
		int flags, u_int unit, register enum handshake_t handshake);
static	u_short check_write_pins(struct  bpp_pins *pins_p,
		int flags, u_int unit, register enum handshake_t handshake);
...
static	void 	bpp_transfer_timeout(int unit_no);
...
#if	BPP_DEBUG > 0
	"bpp 2.0 Debug @(#) 1.1 91/15/11",		/* name of module  */
#else	BPP_DEBUG
	"bidir parallel port 2.0 @(#) 91/15/11",	/* name of module  */
#endif	BPP_DEBUG
	&bpp_ops				/* *Drv_dev_ops		*/
};
 ...
 */
static int
bpp_identify(dev_info_t *dip)
{
	char *dev_name;
	dev_name = ddi_get_name(dip);

	BPP_PRINT(2, (CE_CONT, "Entering bpp_identify: name %s\n", dev_name));
	if (strcmp(dev_name, BPP_PROM_NAME) == 0) {
		return (DDI_IDENTIFIED);
	}
	BPP_PRINT(2, (CE_CONT,
		"Leaving bpp_identify, %s (REJECTED)\n", dev_name));
...
bpp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_RESUME:
		if (!(bpp_p = (struct bpp_unit *)ddi_get_driver_private(dip)))
			return (DDI_FAILURE);
		mutex_enter(&bpp_p->bpp_mutex);
		if (!bpp_p->unit_suspended) {
			mutex_exit(&bpp_p->bpp_mutex);
			return (DDI_FAILURE);
		}
		bpp_p->unit_suspended = 0;
		mutex_exit(&bpp_p->bpp_mutex);
	...
	ddi_set_driver_private(dip, (caddr_t)bpp_p);
	...
	BPP_PRINT(5, (CE_CONT, "Installed bpp_poll: unit %d\n", unit_no));
        ...
        cv_init(&bpp_p->wr_cv,"multi-write synchron",CV_DRIVER,NULL);

	/*
	 * Initialize the bpp mutex.
	 * This mutex is used for all operations outside of attach.
	 */
	mutex_init(&bpp_p->bpp_mutex, "bpp mutex",
		MUTEX_DRIVER, (void *)bpp_p->bpp_block_cookie);
	...
	bpp_p->bpp_transfer_timeout_ident = -1;
	bpp_p->bpp_fakeout_timeout_ident = -1;
	...
	sprintf(name, "bpp%d", unit_no);
	if (ddi_create_minor_node(dip, name, S_IFCHR,
		unit_no, NULL, NULL) == DDI_FAILURE) {
...
{
	register int	error;
	BPP_PRINT(2, (CE_CONT, "_init: driver compiled at %s on %s\n",
				__TIME__, __DATE__));
...
{
	BPP_PRINT(2, (CE_CONT, "_info: bpp driver compiled at %s on %s\n",
				__TIME__, __DATE__));
...
bpp_close(dev_t dev, int openflags, int otyp, cred_t *credp)
{
	...
	if (bpp_p->timeouts) {			 /* any timeouts pending? */
		...
		if (bpp_p->timeouts & TRANSFER_TIMEOUT) {
			...
			(void) untimeout(bpp_p->bpp_transfer_timeout_ident);
		}
...
bpp_write(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	*bpp_errorstat_p = bpp_default_error_stat;
	...
	while (bpp_p->timeouts & TRANSFER_TIMEOUT)
...
bpp_strategy(register struct buf *bp)
{
	...
	if (bpp_p->unit_suspended) {
		mutex_exit(&bpp_p->bpp_mutex);
		ddi_dev_is_needed(bpp_p->dip, 0, 1);
		mutex_enter(&bpp_p->bpp_mutex);
	}
	...
	BPP_PRINT(5, (CE_CONT,
		"Before dma_buf_setup, b_addr = 0x%x, b_bcount = 0x%x\n",
		paddr(bp), bp->b_bcount));
	/*
	...
	BPP_PRINT(5, (CE_CONT,
		"After dma_buf_setup, b_addr = 0x%x, b_bcount = 0x%x\n",
		paddr(bp), bp->b_bcount));
	...
	bpp_p->bpp_transfer_timeout_ident =
	    timeout(bpp_transfer_timeout, (caddr_t)unit_no,
...
/*ARGSUSED*/
static	int
bpp_ioctl(dev_t dev, int cmd, int arg, int flag, cred_t *credp, int *rvalp)
{
	int	unit_no;
	u_short	retval = 0;	/* return value (errno) for system call */
	...
	switch (cmd) {
		case BPPIOC_SETPARMS:	/* set transfer parameters */
			...
			mutex_enter(&bpp_p->bpp_mutex);
			copyin((caddr_t) arg, (caddr_t)(&temp_parms),
			    sizeof (struct bpp_transfer_parms));
		...
		case BPPIOC_GETPARMS:	/* get transfer parameters */
			...
			mutex_enter(&bpp_p->bpp_mutex);
			copyout((caddr_t)(&bpp_p->transfer_parms),
		...
		case BPPIOC_SETOUTPINS:	/* set output pins */
			...
			/* copy passed parms to temporary storage */
			mutex_enter(&bpp_p->bpp_mutex);
			copyin((caddr_t) arg, (caddr_t)(&temp_pins),
		...
		case BPPIOC_GETERR:	/* get error block status */
			...
			mutex_enter(&bpp_p->bpp_mutex);
			copyout((caddr_t)(&bpp_p->error_stat),
...
bpp_intr(caddr_t unit_no)
{
	...
	if ((bpp_regs_p->dma_csr & BPP_INT_PEND) ||
		...
		ddi_dma_free(bpp_p->bpp_dma_handle);
 ...
 */
static void
bpp_transfer_timeout(int unit_no)
{
	register struct	bpp_unit	*bpp_p;	/* will point to this */
						/* unit's state struct */
	register struct buf		*bp;
...
/*ARGSUSED*/
static	u_short
check_read_params(struct  bpp_transfer_parms *parms_p, u_int unit, int flags)
{
	u_short	retval = 0;	/* return value (errno) for system call */
	static int max_setup = 0;	/* maximum setup time allowed (ns) */
	static int max_width = 0;	/* maximum width time allowed (ns) */
...
/*ARGSUSED*/
static	u_short
check_read_pins(struct  bpp_pins *pins_p, int flags, u_int unit,
		register enum handshake_t handshake)
{
	u_short	retval = 0;	/* return value (errno) for system call */
	register struct	bpp_unit	*bpp_p;	/* will point to this */
...
/*ARGSUSED*/
static	u_short
check_write_pins(struct  bpp_pins *pins_p, int flags, u_int unit,
		register enum handshake_t handshake)
{
	u_short	retval = 0;	/* return value (errno) for system call */
	register struct	bpp_unit	*bpp_p;	/* will point to this */
...
{
	volatile u_long	*l_reg_addr;		/* address of a 32-bit reg */
	volatile u_long	l_reg_contents;		/* contents of a 32-bit reg */
	volatile u_short *s_reg_addr;		/* address of a 16-bit reg */
	volatile u_short s_reg_contents;	/* contents of a 16-bit reg */
	volatile u_char	*c_reg_addr;		/* address of a 8-bit reg */
	volatile u_char	c_reg_contents;		/* contents of a 8-bit reg */
	register struct	bpp_unit	*bpp_p;	/* will point to this */
						/* unit's state struct */
...
/* check the long dma registers */
	...
	if (ddi_peekl(bpp_p->dip,
	    (long *)l_reg_addr, (long *)&l_reg_contents) != DDI_SUCCESS) {
		BPP_PRINT(1, (CE_CONT,
