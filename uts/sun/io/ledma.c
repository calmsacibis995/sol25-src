...
#ident	"@(#)ledma.c	1.20	94/12/22 SMI"
 ...
 */
static	u_int	ledmadelay = 300000;
...
ledmaattach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	lop = (struct leops *)kmem_alloc(sizeof (*lop), KM_SLEEP);
...
{
	u_int	*csr = (u_int *) ddi_get_driver_private(dip);
	int	tcsr;
	int	burst = -1;
