/*
 * Copyright 1991-1995, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma	ident	"@(#)bwtwo.c	1.41	95/01/10 SMI"
/*
...
static int bw2_ioctl(dev_t, int, int, int, cred_t *, int *);
static int bw2_mmap(dev_t, off_t, int);

static u_int bw2_intr(caddr_t);
...
struct bw2_softc {
	...
	u_char	mapped_by_prom;	/* if true, fb mapd by prom */
	u_char	bw2_vstate;	/* original state of video */
	u_char	is_sid;		/* is a self-ident device */
...
static void bw2_set_video(struct bw2_softc *softc, u_int on, u_int intr);
static u_int bw2_get_video(struct bw2_softc *softc);
static u_int bw2_intr(caddr_t i_handler_arg);
...
bw2_wrapup_reg_map(register struct bw2_softc *softc)
{
	...
	DEBUGF(1, (CE_CONT, "bw2_wrapup_probe%d: mfb_reg 0x%x\n",
	    ddi_get_instance(softc->devi), (u_int)softc->reg));
	...
	if (softc->reg) {
		ddi_unmap_regs(softc->devi, (u_int)0,
		    (char **)&(softc->reg),
...
static u_char bw2_mv_1600_1280_ecl[] = {	/* high-res ECL */
...
static u_char bw2_mv_1152_900_ecl[] = {		/* low-res ECL */
...
static u_char bw2_mv_1152_900_analog[] = {	/* low-res analog */
 ...
 */
static u_char bw2_mv_501_1561_66Hz[] = {
 ...
 */
static u_char bw2_mv_501_1561_76Hz[] = {
 ...
 * All we really have is a u_long register
 */
...
bw2_setup_nsid(register struct bw2_softc *softc)
{
	 ...
	 */
	softc->reg_size = (off_t)sizeof (u_long);
	softc->reg_offset = (off_t)0;

	if (ddi_map_regs(softc->devi, (u_int)0, (caddr_t *)&reg,
	 ...
	 */
	if (ddi_peeks(softc->devi, (short *)softc->reg, (short *)0)
					    != DDI_SUCCESS) {
		return (0);
		/*NOTREACHED*/
	}

	DEBUGF(1, (CE_CONT, "bw2_setup_nsid%d: reg 0x%x\n",
	    ddi_get_instance(softc->devi), (u_long)softc->reg));
...
bw2_sid_hw_init(register struct bw2_softc *softc)
{
	...
	DEBUGF(1, (CE_CONT, "bw2_sid_hw_init%d: mfb_reg 0x%x\n",
	    ddi_get_instance(devi), (u_int)softc->reg));
...
bw2_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if (!softc->mapped_by_prom) {
			if (ddi_map_regs(softc->devi, (u_int)0,
		...
		DEBUGF(1, (CE_CONT, "bw2_attach%d fb 0x%x kpfnum 0x%x\n",
		    softc->instance, (u_int)softc->fb,
		...
		if (softc->bw2_type == BW2_MFB) {
			mutex_init(&softc->reg_mutex,
			    "bw2 reg", MUTEX_DRIVER, NULL);
		} else {
			/*
			...
			if (ddi_add_intr(softc->devi,
			    ...
			    (u_int (*)(caddr_t))nulldev,
			    NULL) != DDI_SUCCESS) {
				goto failed;
				/*NOTREACHED*/
			}
			mutex_init(&softc->reg_mutex,
			    "bw2 reg", MUTEX_DRIVER, softc->iblkc);
			ddi_remove_intr(softc->devi, (u_int)0,
			    softc->iblkc);
			if (ddi_add_intr(softc->devi,
			    (u_int)0, &softc->iblkc,
		...
		/* Create power managemnt properties */
		timestamp[0] = 0;
		if (drv_getparm(TIME, timestamp+1) != 0)
			goto failed;
		(void) ddi_prop_create(DDI_DEV_T_NONE,
		    softc->devi, DDI_PROP_CANSLEEP, "pm_timestamp",
		    (caddr_t)timestamp, sizeof (timestamp));
		power[0] = 1;
		power[1] = 255;
		(void) ddi_prop_create(DDI_DEV_T_NONE,
		    softc->devi, DDI_PROP_CANSLEEP,
...
failed:
	...
	case DDI_RESUME:
		...
		/*
		 * Assuming PROM initializes framebuffer as in
		 * bw2_sid_probe, then only need to restore brightness.
		 */
		if (ddi_prop_op(DDI_DEV_T_ANY,
		    softc->devi, PROP_LEN_AND_VAL_BUF, 0,
		    "pm_norm_pwr", (caddr_t)power, &proplen)
		    == DDI_PROP_SUCCESS)
			(void) bw2_power(softc->devi, 1, power[1]);
...
bw2_power(dev_info_t *devi, int cmpt, int level)
{
	...
	if (cmpt != 1 || 0 > level || level > 255 || !softc)
		return (DDI_FAILURE);
	if (level) {
		bw2_set_video(softc, FBVIDEO_ON, 0);
		power[0] = 1;
		power[1] = level;
		if (ddi_prop_modify(DDI_DEV_T_NONE,
		    softc->devi, DDI_PROP_CANSLEEP,
		    "pm_norm_pwr", (caddr_t)power,
		    sizeof (power)) != DDI_SUCCESS) {
			return (DDI_FAILURE);
		}
	} else {
		bw2_set_video(softc, FBVIDEO_OFF, 0);
	}
	(void) ddi_power(softc->devi, 1, level);
	return (DDI_SUCCESS);
}
...
bw2_getinfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg, void **result)
{
	...
	DEBUGF(1,
	    (CE_CONT, "bw2_getinfo cmd 0x%x; arg=0x%08x; result=0x%08x\n",
	    infocmd, arg, result));
...
/*ARGSUSED4*/
static int
bw2_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *cred_p, int *rval_p)
{
	register int instance = getminor(dev);
	register struct bw2_softc *softc = getsoftc(instance);
	register int error = 0;
	...
	case VIS_GETIDENTIFIER:
		if (ddi_copyout((caddr_t) &bw2_ident, (caddr_t) arg,
	...
	case FBIOGTYPE: {
		...
		if (ddi_copyout((caddr_t) &bw2_fb, (caddr_t) arg,
	...
	case FBIOSVIDEO: {
		...
		if (ddi_copyin((caddr_t) arg, (caddr_t) &on,
		    sizeof (int), mode) != 0)
			error = EFAULT;
		else
	...
	case FBIOGVIDEO: {
		...
		if (ddi_copyout((caddr_t) &video, (caddr_t) arg,
		    sizeof (int), mode) != 0)
			error = EFAULT;
	}	break;
...
{
	register struct bw2_softc *softc;
	register caddr_t kvaddr = (caddr_t)0;
#ifdef _DDICT
	ulong_t credv;		/* fake ddict */
#else
	struct cred credv;
#endif  /* _DDICT */
	...
	if (softc->bw2_type == BW2_MFB && off == MFB_REG_MMAP_OFFSET) {
		 ...
		 */
		if (drv_getparm(UCRED, (unsigned long *)&credv) != 0)
			return (-1);
		if (drv_priv((struct cred *)&credv) == EPERM)
			return (-1);
		kvaddr = (caddr_t)softc->reg;
	} else if ((u_int) off < softc->fb_size)
		kvaddr = (caddr_t)softc->fb + off;

	if (kvaddr != 0) {
		DEBUGF(kvaddr != (caddr_t)softc->fb ? 9 : 2, (CE_CONT,
		    "bw2_mmap: pfnum 0x%x (kva 0x%x)\n", hat_getkpfnum(kvaddr),
		    (int)kvaddr));
...
static void
bw2_set_video(register struct bw2_softc *softc, u_int videoon, u_int intrclr)
{
	register u_long bitson = 0;
