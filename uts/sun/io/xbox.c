...
#pragma	ident	"@(#)xbox.c	1.21	94/12/07 SMI"
...
static int xbox_ioctl(dev_t, int, int, int, cred_t *, int *);
...
static int xbox_wait_for_error(struct xbox_state *xac,
	struct xc_errs *epkt_p, long timeout);
...
xbox_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	mutex_init(XAC_MUTEX, "xbox sync",
		MUTEX_DRIVER, (void *)xac->xac_iblkc);

	/*
	 * initialize cv for sundiag test mode
	 */
	cv_init(XAC_CV, "xac_cv", CV_DRIVER, NULL);
	...
	if (xbox_no_cards_in_slot0 == 0 &&
	    ddi_peekl(devi, (long *)xac->xac_xc.xac_write0,
	    (long *)0) != DDI_SUCCESS) {
...
#ifdef DOESNTWORK
/*
 * XXXX frequent flushes cause timeouts when accessing other registers
 * during the flush
 * XXX there is really no need to do this on sun4m or sun4c with
 * Sun SBus controllers qualified for xbox
 */
/*
 * DMA synchronization.
 *
 * The XBox is mostly transparent, but it has this "neato" feature
 * that device interrupts can race back to the cpu ahead of the DVMA
 * transfer.  Thus on a fast enough cpu, we might find ourselves
 * doing a DMA sync operation inside the interrupt handler when the
 * data is still moving through the XBox.
 *
 * So, we intercept the sync & free requests to ensure that the CPU's
 * view of the memory object includes any changes made by the
 * device's DMA engine.
 * We assert that we do not need to worry about DDI_DMA_SYNC_FORDEV
 * (i.e. where the cpu has altered memory, and we want to ensure that
 * the device sees the change) because XBox doesn't really contain
 * a cache - we just have to cope with the interrupt race.
 *
 * XXX the write0 causes a read timeout on cards in the xbox if
 * the write0 is immediately followed by this read
 */
static int
xbox_dma_mctl(dev_info_t *dip, dev_info_t *rdip,
    ddi_dma_handle_t handle, enum ddi_dma_ctlops request,
    off_t *offp, u_int *lenp, caddr_t *objp, u_int cache_flags)
{
	register ddi_dma_impl_t *mp = (ddi_dma_impl_t *)handle;

	if (((request == DDI_DMA_SYNC) &&
	    ((cache_flags == DDI_DMA_SYNC_FORKERNEL) ||
	    (cache_flags == DDI_DMA_SYNC_FORCPU))) ||
	    ((request == DDI_DMA_FREE) &&
	    (mp->dmai_rflags & DDI_DMA_READ))) {

		register int instance;
		register struct xbox_state *xac;
		volatile u_int *errd_loc;

		instance = ddi_get_instance(dip);
		xac = (struct xbox_state *)ddi_get_soft_state(xbh, instance);


		/*
		 * XXX	The mutex is a pain, but is needed because this
		 *	routine may be running concurrently for this xbox
		 *	on an MP, and the operation of flushing the XBox
		 *	DVMA channel is not atomic.
		 */
		mutex_enter(XAC_MUTEX);

		/*
		 * Do a 'flush' of the xbox - this next sequence of write0
		 * commands issues a DVMA test without interrupt
		 * sequence that guarantees to flush all pending DMA packets.
		 */
		errd_loc = &xac->xbc_epkt->xc_errd;
		*errd_loc = NO_PKT;
		xbox_write0(xac, XBC_CTL1_OFFSET, CTL1_DVTE | XBC_CTL1);
		if (xbox_no_cards_in_slot0) {
			ddi_peekl(xac->xac_dev_info,
				(long *)xac->xac_xc.xac_write0, (long *)0);
		} else {
			long junk = *((long *)xac->xac_xc.xac_write0);
#ifdef lint
			junk = junk;
#endif /* lint */
		}
		...
		/*
		 * Let's hope that this next test fails straight away
		 * XXX need a timeout here
		 */
		while (*errd_loc == NO_PKT) {
			XBOX_DMA_SYNC(xac);
		}
		xbox_write0(xac, XBC_CTL1_OFFSET, XBC_CTL1);
 ...
 */
static int
xbox_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *cred_p, int *rval_p)
{
	register int instance = getminor(dev);
	register struct xbox_state *xac = ddi_get_soft_state(xbh, instance);
	int rval = 0;
