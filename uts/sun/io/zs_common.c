...
#pragma ident	"@(#)zs_common.c 4.108	95/09/18 SMI"
...
int	default_dtrlow = 3;	/* hold dtr low nearly this long on close */
...
u_int zs_high_intr(caddr_t argzs);
u_int zsintr(caddr_t intarg);
...
zsprobe(dev_info_t *dev)
{
	...
	if (ddi_peekc(dev, (char *)&zsaddr->zscc_control, &c) != DDI_SUCCESS) {
		goto out;
	}
	drv_usecwait(2);
	 ...
	 */
	if (ddi_pokec(dev, (char *)&zsaddr->zscc_control, '\017')
	    != DDI_SUCCESS) {
		goto out;
	}
	drv_usecwait(2);
...
zsattach(dev_info_t *dev, ddi_attach_cmd_t cmd)
{
	...
	register u_int		s;
	register int		softcd, rtsdtr_bits = 0;
	register u_char		rr;
	short			speed[2];
	...
	ulong_t			timestamp[3];	/* for PM properties */
	...
	if (zscom == NULL) {
		mutex_init(&zs_soft_lock, "zs_soft_lock",
		    MUTEX_DRIVER, (void *)ZS_PL);
		mutex_init(&zs_curr_lock, "zs_curr_lock",
		    MUTEX_DRIVER, (void *)ZS_PL_HI);
		nzs = maxzs;
		zscom = (struct zscom *)
		    kmem_zalloc(maxzs * sizeof (struct zscom), KM_SLEEP);
		zs_prog = (struct zs_prog *)
		    kmem_zalloc(maxzs * sizeof (struct zs_prog), KM_SLEEP);
	...
	for (i = 0; i < 2; i++) {
		...
		mutex_init(zs->zs_excl, "zs_excl lock",
		    MUTEX_DRIVER, (void *)ZS_PL);
	 ...
	 */
	if (ddi_add_intr(dev, (u_int) 0, &zs_hi_iblock,
	...
	if (!GETPROP(zs->zs_dip, "keyboard", 0)) {
		 ...
		 */
		sprintf(name, "%c", (ddi_get_instance(dev) + 'a'));
		if (ddi_create_minor_node(dev, name, S_IFCHR,
		    ddi_get_instance(dev) * 2,
		...
		sprintf(name, "%c", (ddi_get_instance(dev) + 'b'));
		if (ddi_create_minor_node(dev, name, S_IFCHR,
		    (ddi_get_instance(dev) * 2) + 1,
		    serial_line, NULL) == DDI_FAILURE) {
			ddi_remove_minor_node(dev, NULL);
		...
		sprintf(name, "%c,cu", (ddi_get_instance(dev) + 'a'));
		if (ddi_create_minor_node(dev, name, S_IFCHR,
		...
		sprintf(name, "%c,cu", (ddi_get_instance(dev) + 'b'));
		if (ddi_create_minor_node(dev, name, S_IFCHR,
	...
	ddi_report_dev(dev);
	/*
	 * Add power management properties for both channels
	 */
	timestamp[0] = 0;
	drv_getparm(TIME, timestamp+1);
	timestamp[2] = timestamp[1];
	if (ddi_prop_create(DDI_DEV_T_NONE, dev, DDI_PROP_CANSLEEP,
	    "pm_timestamp", (caddr_t)timestamp, sizeof (timestamp))
	    != DDI_SUCCESS)
		cmn_err(CE_WARN, "zsattach: Can't create pm property\n");
	timestamp[0] = timestamp[1] = timestamp[2] = 1;
	if (ddi_prop_create(DDI_DEV_T_NONE, dev, DDI_PROP_CANSLEEP,
	    "pm_norm_pwr", (caddr_t)timestamp, sizeof (timestamp))
	    != DDI_SUCCESS)
		cmn_err(CE_WARN, "zsattach: Can't create pm property\n");
	return (DDI_SUCCESS);
}
...
{
	register int mbits, obits, now, held;
