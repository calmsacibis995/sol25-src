...
#pragma ident  "@(#)esp.c 1.207     95/08/14 SMI"
...
static long esp_tick;			/* esp_watch() interval in Hz */
static int esp_reset_watch;
static struct esp *esp_poll_last;
...
#ifdef ESP_PERF
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_request_count */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_sample_time */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_intr_count */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_ncmds */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_ndisc */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_ncmds_per_esp */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_ncmds_per_esp */
...
#define	MAX_ESPS	80	/* should be enough, even for sun4d */
static int esp_ncmds_per_esp[MAX_ESPS];
#endif

/* VARIABLES PROTECTED BY "Stable Data": scsi_pkt::, esp_cmd::	*/
/* VARIABLES PROTECTED BY "Stable Data": scsi_device::		*/
/* VARIABLES PROTECTED BY "Stable Data": dmaga::, buf::		*/
/* VARIABLES PROTECTED BY "Stable Data": scsi_address::		*/
/* READ-ONLY VARIABLES: espdebug				*/
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_poll_last	*/
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_watchdog_running */
/* VARIABLES PROTECTED BY "No Mutex Needed": esp_scsi_watchdog_tick */
/* FUNCTIONS CALLED THROUGH POINTER esp_cmd::cmd_pkt.pkt_comp:
	"esp.c":esp_start */
/* FUNCTIONS CALLED THROUGH POINTER esp_cmd::cmd_pkt.pkt_comp:
	"esp.c":esp_abort, "esp.c":esp_reset */
/* FUNCTIONS CALLED THROUGH POINTER esp_cmd::cmd_pkt.pkt_comp:
	"esp.c":esp_commoncap */
/* FUNCTIONS CALLED THROUGH POINTER esp_cmd::cmd_pkt.pkt_comp:
	"esp.c":esp_scsi_dmafree */
/* FUNCTIONS CALLED THROUGH POINTER esp_cmd::cmd_pkt.pkt_comp:
	"esp.c":esp_scsi_destroy_pkt, "esp.c":esp_scsi_init_pkt */
...
static void esp_restart_cmd(struct esp *esp);
static int esp_dopoll(struct esp *esp, int timeout);
static u_int esp_poll_loop(caddr_t arg);
static u_int esp_poll_sun4d(caddr_t arg);
...
static void esp_watch(caddr_t arg);
...
static char *esp_state_name(u_short state);
static void esp_update_props(struct esp *esp, int tgt, u_int tickval,
    u_int offset);
...
static int esp_abort_disconnected_cmd(struct esp *esp, struct scsi_address *ap,
    struct esp_cmd *sp, u_char msg, int slot);
static void esp_mark_packets(struct esp *esp, int slot, u_char reason,
    u_long stat);
...
static int esp_create_callback_thread(struct esp *esp);
static void esp_callback(register struct esp *esp);
static void esp_call_pkt_comp(register struct esp *esp,
    register struct esp_cmd *sp);
...
static int esp_next_window(struct esp *esp, struct esp_cmd *sp, u_int end);
static void esp_start_watch_reset_delay();
static void esp_watch_reset_delay(caddr_t arg);
static int esp_watch_reset_delay_subr(register struct esp *esp);
void esp_wakeup_callback_thread(register struct callback_info *cb_info);
static void esp_update_TQ_props(struct esp *esp, int tgt, int value);
static int esp_check_dma_error(register struct esp *esp);
...
static void esp_kmem_cache_constructor(void *buf, size_t size);
static void esp_kmem_cache_destructor(void *buf, size_t size);
 ...
 */
static int esp_info(dev_info_t *dip, ddi_info_cmd_t infocmd,
    void *arg, void **result);
static int esp_identify(dev_info_t *dev);
...
static struct modldrv modldrv = {
	...
	"ESP SCSI Host Bus Adapter Driver", /* Name of the module. */
	&esp_ops,	/* driver ops */
};
...
_init(void)
{
	...
	mutex_init(&esp_global_mutex, "esp_global_mutex", MUTEX_DRIVER, NULL);
...
/*ARGSUSED*/
static int
esp_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg, void **result)
{
	return (DDI_FAILURE);
}

static int
esp_identify(dev_info_t *devi)
{
	/* NO OTHER THREADS ARE RUNNING */
	char *dname = ddi_get_name(devi);

	if (strcmp(dname, "esp") == 0 || strcmp(dname, "SUNW,esp") == 0) {
...
esp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (ddi_regs_map_setup(dip, (u_int)0, (caddr_t *)&ep,
	...
	if ((dmamaj = ddi_name_to_major("dma")) != -1) {
		if (ddi_hold_installed_driver(dmamaj) != NULL)
			dmar = dma_alloc(dip);
	}
	if (dmar == NULL) {
		cmn_err(CE_WARN,
	...
	if (ddi_dma_mem_alloc(esp->e_dmahandle, (u_int)FIFOSIZE,
	...
	if (tran == NULL) {
		cmn_err(CE_WARN, "esp: scsi_hba_tran_alloc failed\n");
		goto fail;
	}
	...
	if (esp->e_dma_rev == ESC1_REV1) {
		register u_int addr1 = (u_int)
			esp->e_dmacookie.dmac_address;
		register u_int addr2 = (u_int)
			roundup(addr1 + FIFOSIZE, MMU_PAGESIZE);
		esp->e_esc_read_count = (u_int) (addr2 - addr1);
	}
	...
	IPRINTF2("esp scsi_options=%x, global=%x\n",
		esp->e_scsi_options, scsi_options);
	if (esp->e_scsi_options != scsi_options) {
		esplog(esp, CE_NOTE, "esp scsi_options=0x%x\n",
		    esp->e_scsi_options);
	}
	...
	for (i = 0; i < NTARGETS; i++) {
		...
		esp->e_target_scsi_options[i] = ddi_prop_get_int(
			DDI_DEV_T_ANY, dip, 0, prop_str, esp->e_scsi_options);
		IPRINTF3("esp target%d_scsi_options=%x, esp scsi_options=%x\n",
			i, esp->e_target_scsi_options[i], esp->e_scsi_options);
		if (esp->e_target_scsi_options[i] != esp->e_scsi_options) {
			esplog(esp, CE_NOTE, "esp target%d_scsi_options=0x%x\n",
	 ...
	 */
	if (ddi_get_iblock_cookie(dip, (u_int)0, &esp->e_iblock)
	    != DDI_SUCCESS) {
		cmn_err(CE_WARN, "esp_attach: cannot get iblock cookie");
		goto fail;
	}

	sprintf(buf, "esp%d_mutex", instance);
	mutex_init(ESP_MUTEX, buf, MUTEX_DRIVER, esp->e_iblock);

	/*
	 * initialize mutex for startQ
	 */
	sprintf(buf, "esp%d_startQ_mutex", instance);
	mutex_init(&esp->e_startQ_mutex, buf, MUTEX_DRIVER,
	    esp->e_iblock);
	...
	/*
	 * on sun4d/sun4u (precise interrupts) the interrupt handler
	 * only checks one esp instance
	 */
	if (strcmp(utsname.machine, "sun4d") == 0 ||
		strcmp(utsname.machine, "sun4u") == 0) {
		if (ddi_add_intr(dip, (u_int)0,
		    &esp->e_iblock, NULL, esp_poll_sun4d,
		    (caddr_t)esp)) {
			cmn_err(CE_WARN, "esp: cannot add intr");
			goto fail;
		}
	/*
	 * on sun4c and sun4m, the interrupt dispatcher always starts with
	 * the first handler in the chain and stops whenever a handler
	 * returns CLAIMED. This causes starvation on the esp's down
	 * at the end of the chain. the esp_poll_loop services all esp's
	 * somewhat round-robin.
	 * Note, that esp's interrupt at different levels, so we steal
	 * interrupts from other levels. We also steal interrupts
	 * from user context sometimes (esp_startcmd())
	 */
	} else {
		struct esp *e = esp_softc;
		int installed = 0;

		/*
		 * if there is already an esp_poll_loop at this level
		 * then don't install another
		 */
		for (e = esp_softc; e != NULL; e = e->e_next) {
			if (e->e_iblock == esp->e_iblock && e != esp) {
				installed++;
				break;
			}
		}
		if (installed == 0) {
			if (ddi_add_intr(dip, (u_int)0,
			    &esp->e_iblock, NULL, esp_poll_loop,
			    (caddr_t)esp)) {
				cmn_err(CE_WARN, "esp: cannot add intr");
				goto fail;
			}
	...
	if (esp_softc == esp) {
		...
		IPRINTF2("esp scsi watchdog tick=%x, esp_tick=%x\n",
		    esp_scsi_watchdog_tick, esp_tick);
		(void) timeout(esp_watch, (caddr_t)0, esp_tick);
	}
	...
	if (!timestamp) {
		timestamp = 1;		/* Workaround for bug 1125533 */
	}
	(void) ddi_prop_update_int(DDI_DEV_T_NONE, dip,
	    "pm_timestamp", timestamp);
	power = 1;
	(void) ddi_prop_update_int(DDI_DEV_T_NONE, dip, "pm_norm_pwr", power);
	 ...
	 */
	mutex_enter(&esp_global_mutex);
	if (esp_create_callback_thread(esp) == -1) {
		mutex_exit(&esp_global_mutex);
		goto fail;
	}
	mutex_exit(&esp_global_mutex);
	 ...
	 */
	sprintf(buf, "esp%d_cache", instance);
...
fail:
	cmn_err(CE_WARN, "esp%d: cannot attach", instance);
	{
		...
		if (add_intr_done) {
			ddi_remove_intr(dip, (u_int)0, &esp->e_iblock);
		}
		if (tran) {
		...
		if (esp->e_cmdarea) {
			if (bound_handle) {
				ddi_dma_unbind_handle(esp->e_dmahandle);
			}
		...
		(void) kmem_free((caddr_t)esp, (sizeof (*esp)));
	}
...
esp_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	case DDI_PM_SUSPEND:
		...
		mutex_enter(ESP_MUTEX);
		/*
		 * When CPR writes out the state file it uses the esp driver.
		...
		/*
		 * In the current implementation of esp power managment, the
 ...
 */
static void
esp_determine_chip_type(register struct esp *esp)
{
	int i;
	u_char clock_conv;
	u_long ticks;
	...
	if ((ep->esp_conf2 & 0xf) == 0xa) {
		esp->e_espconf2 = (u_char) ESP_CONF2_SCSI2;
		ep->esp_conf3 = 0;
		ep->esp_conf3 = 5;
		if (ep->esp_conf3 == 0x5) {
			for (i = 0; i < NTARGETS; i++) {
...
static void
esp_internal_reset(register struct esp *esp, int reset_action)
{
...
esp_hw_reset(struct esp *esp, int action)
{
	...
	if (action & ESP_RESET_ESP) {
		 ...
		 */
		if (esp->e_type == FAST) {
			register u_char fcode;

			esp->e_idcode = ep->esp_id_code;
			fcode =
			    (u_char)(ep->esp_id_code & ESP_FCODE_MASK)>>
			    (u_char) 3;
			if (fcode == ESP_FAS236) {
				esp->e_type = FAS236;
			} else {
		...
		case FAS236:
			...
			if ((u_char)(ep->esp_id_code & ESP_REV_MASK)
			    > (u_char)2) {
				IPRINTF1("FAS236 rev=%x Stack_cmds DISABLED\n",
				    (u_char)(ep->esp_id_code & ESP_REV_MASK));
		...
		if (esp->e_type > ESP100A) {
			...
			EPRINTF2("req_ack_delay (0x%x) = %x\n",
			    &esp->e_req_ack_delay, esp->e_req_ack_delay);
...
static int esp_max_spawn = 2; /* max of 2 extra threads for 4 esps */
static int esp_cb_now_qlen = 5;
...
esp_create_callback_thread(struct esp *esp)
{
	...
	if ((esp_n_esps++ % esp_n_esps_per_callback_thread) == 0) {
		...
		cb_info = (struct callback_info *)kmem_zalloc(
		    sizeof (struct callback_info),  KM_NOSLEEP);
		if (cb_info == NULL) {
			IPRINTF("can't create callback info structure\n");
			return (-1);
		}

		sprintf(buf, "esp%d_callback_cv", esp_n_callback_threads);
		cv_init(&cb_info->c_cv, buf, CV_DRIVER, NULL);

		sprintf(buf, "esp%d_callback_mutex", esp_n_callback_threads);
		...
		t = thread_create((caddr_t)NULL, 0, esp_callback,
			(caddr_t)esp, 0, &p0, TS_RUN, v.v_maxsyspri - 2);

		if (t == NULL) {
			IPRINTF("can't create callback thread\n");
			return (-1);
		}
		cb_info->c_thread = t;
		cb_info->c_spawned = (u_char) esp_max_spawn;
		cb_info->c_cb_now_qlen = (u_char) esp_cb_now_qlen;
	} else {
 ...
 */
void
esp_wakeup_callback_thread(register struct callback_info *cb_info)
{
	register struct esp_cmd *sp;
...
static void
esp_callback(register struct esp *esp)
{
	register struct esp_cmd *sp;
	register struct callback_info *cb_info = esp->e_callback_info;
	register int serviced = 0;
	register int wakeups = 0;
	int id, load;
	int hiload = 0;
	int loload = 0;
#ifdef TRACE
	...
	for (;;) {
		...
		if (load > esp_hi_cb_load) {
			...
			if ((hiload++ > esp_cb_load_count) &&
				 ...
				 * create another thread
				 */
				(void) thread_create((caddr_t)NULL,
				    0, esp_callback,
				    (caddr_t)esp, 0, &p0, TS_RUN,
				    v.v_maxsyspri - 2);
				serviced = wakeups = 0;
				cb_info->c_spawned--;
				/*
...
{
	register struct esp_cmd *cmd = (struct esp_cmd *)pkt;
...
{
	register int i;
	register struct esp_cmd *sp = (struct esp_cmd *)pkt;
	...
	int flags, int (*callback)(), caddr_t arg)
{
	...
	register struct esp_cmd *cmd = (struct esp_cmd *)pkt;
	register struct esp *esp = ADDR2ESP(ap);
	struct esp_cmd		*new_cmd = NULL;

	ASSERT(!esp->e_suspended);
/* #define	ESP_TEST_EXTRN_ALLOC */
	...
	if (pkt == NULL) {
		...
		if (cmd) {
			 ...
			 * the rest is initialized in esp_prepare_pkt()
			 */
			register int *p;
	...
	if (bp) {
		...
		if (cmd->cmd_dmahandle == NULL) {
			if ((rval = ddi_dma_alloc_handle(esp->e_dev,
			    esp->e_dma_attr, callback, arg,
			    &cmd->cmd_dmahandle)) != 0) {
				goto dma_failure;
			}
		}
...
esp_pkt_destroy_extern(struct esp *esp, struct esp_cmd *sp)
{
	if (sp->cmd_flags & CFLAG_FREE) {
		cmn_err(CE_PANIC,
		    "esp_scsi_pktfree: freeing free packet");
		/* NOTREACHED */
	}
 ...
 * allocate the handle because we don't have the esp pointer here
 * When destructing, just free the dma handle
 */
static void
esp_kmem_cache_constructor(void *buf, size_t size)
{
...
/* ARGSUSED */
static void
esp_kmem_cache_destructor(void *buf, size_t size)
{
	struct esp_cmd *cmd = buf;
 ...
 */
static int
esp_prepare_pkt(register struct esp *esp, register struct esp_cmd *sp)
{
	register int size, cmdlen;
 ...
 */
static int
esp_start(struct scsi_address *ap, register struct scsi_pkt *pkt)
{
	register struct esp_cmd *sp = (struct esp_cmd *)pkt;
	register struct esp *esp = ADDR2ESP(ap);
	register int rval;
	...
	TRACE_1(TR_FAC_SCSI, TR_ESP_START_END, "esp_start_end: ncmds %x",
		esp->e_ncmds);
	return (rval);
}
 ...
 */
static int
_esp_start(register struct esp *esp, register struct esp_cmd *sp, int flag)
{
	register short slot;
	int target = Tgt(sp);
	int lun = Lun(sp);
	...
	if (esp_do_kstats) {
		...
		if ((esp->e_slot_stats[slot] == NULL) &&
			...
			sprintf(buf, "esp%dt%dd", CNUM, target);
	...
	if (esp->e_readyf[slot]) {
		if (sp->cmd_pkt.pkt_flags & FLAG_HEAD) {
			...
			register struct esp_cmd *dp = esp->e_readyb[slot];
		...
		if ((esp->e_state == STATE_FREE) &&
			...
			EPRINTF2(
			    "cmd not started: e_slot=%x, throttle=%x\n",
			    esp->e_slots[slot], esp->e_throttle[slot]);
...
static int
esp_alloc_tag(register struct esp *esp, register struct esp_cmd *sp)
{
	register struct t_slots *tag_slots;
	register u_char tag;
	int rval = 0;
	int target = Tgt(sp);
	int lun = Lun(sp);
	register short slot = (target * NLUNS_PER_TARGET) | lun;
...
alloc:
	...
	if ((int)tag_slots != NULL) {
		...
		EPRINTF2("Target %d.%d allocating tag que\n",
		    target, Lun(sp));
		tag_slots = (struct t_slots *)kmem_zalloc(
			(unsigned)sizeof (struct t_slots), KM_NOSLEEP);
		if ((int)tag_slots == NULL) {
			/*
			 * Couldn't get space for tagged que.  Complain
			 * and disable tagged queueing.	 It beats
...
static int
esp_ustart(register struct esp *esp, short start_slot, short flag)
{
	register struct esp_cmd *sp;
	register short slot;
	...
	{
		register found = 0;
		register short dslot = esp->e_dslot;
...
static int
esp_startcmd(register struct esp *esp, register struct esp_cmd *sp)
{
	register volatile struct espreg *ep = esp->e_reg;
	int cmd_len, i, nstate;
	register u_char cmd, tshift, target, lun;
	register caddr_t tp = (caddr_t)esp->e_cmdarea;
	u_char offset, period, conf3;
...
#define	LOAD_CMDP	*(tp++)
	...
	EPRINTF2("esp_startcmd: sp=%x flags=%x\n",
	    sp, sp->cmd_pkt.pkt_flags);

#ifdef ESPDEBUG
	if (esp->e_cur_slot != ((target * NLUNS_PER_TARGET) | lun)) {
		eprintf(esp, "cur_slot=%x, target=%x, lun=%x, sp=%x\n",
		esp->e_cur_slot, target, lun, sp);
	...
	if (((sp->cmd_pkt.pkt_flags & FLAG_TAGMASK) == 0) &&
		...
		if ((sp->cmd_pkt.pkt_flags & FLAG_NOINTR) == 0) {
			register struct esp_cmd *dp;
			register int slot = esp->e_cur_slot;
	...
	if ((sp->cmd_pkt.pkt_flags & FLAG_TAGMASK) &&
		...
		for (i = 0; i < (u_int) esp->e_omsglen; i++) {
			esp->e_cur_msgout[i] =
		...
		if (esp->e_weak & tshift) {
			...
			int period = esp->e_neg_period[target];

			if (period == 0) {
				period = esp->e_default_period[target];
			} else {
				/*
				 * do not backoff if we have
				 * done this before
				 */
				if (esp->e_backoff[target] == 1) {
					/*
					 * increase period by 20 or 100% (FAS)
					 */
	...
	ESP_DMA_READ(esp, esp->e_lastcount,
	    (u_long) esp->e_dmacookie.dmac_address);
	...
	if (DEBUGGING) {
		auto char buf[256];
		buf[0] = '\0';
		sprintf(&buf[strlen(buf)], "sel %d.%d cmd[ ", target, lun);
	 ...
	 */
	if (sp->cmd_pkt.pkt_flags & FLAG_TAGMASK) {
		register short slot = esp->e_cur_slot;
		register struct t_slots *tag_slots = esp->e_tagQ[slot];
 ...
 *
 * This esp_poll_loop is going to cause spurious interrupts since
 * we steal interrupts from higher and lower levels; therefore we
 * reset the spurious interrupt counters to suppress the console messages
 * Since the interrupt dispatcher also qualifies an interrupt that
 * goes away before esp_poll_loop is called as spurious, having
 * esp_poll_loop return CLAIMED in the event of interrupt stealing
 * does not work too well
 * esp's interrupt at level 3, 4, and 5 (sun4c and sun4m)
 * Note that for every interrupt, we will check all esp's so controllers
 * down the line will not be pleased. Fortunately, PIOs aren't very
 * expensive on sun4m/4c
 */
static int esp0_intr_count;
static int espn_intr_count;
/* VARIABLES PROTECTED BY "No Mutex Needed": esp0_intr_count */
/* VARIABLES PROTECTED BY "No Mutex Needed": espn_intr_count */

static u_int
esp_poll_loop(caddr_t arg)
{
	register struct esp *esp;
	register struct esp *start;
	register struct esp *first = (struct esp *)arg;
	register n_intr_serviced = 0;
	register try_enter = 1;
	int failed_try_enters = 0;
	int n = 0;
	int cb_needed = 0;

	TRACE_0(TR_FAC_SCSI, TR_ESP_POLL_LOOP_START, "esp_poll_loop_start");
	...
	/*
	 * do round-robin servicing of esp's; update esp_poll_last
	 * so another CPU or the next time around will start
	 * at the last active esp
	 *
	 * this should be protected by the global mutex but that is
	 * too much overhead
	 */
	esp = start = esp_poll_last;
	if (start->e_next == NULL) {
		esp_poll_last = esp_softc;
	} else {
		esp_poll_last = start->e_next;
	}
...
again:
	for (;;) {
		...
		cb_needed |= esp->e_callback_signal_needed;
...
next:
		...
		if (esp == start) {
			if (n_intr_serviced == 0 && failed_try_enters == 0) {
				/*
				 * no interrupts were serviced and
				 * no mutex_tryenters failed, so we are done
				 */
				break;

			} else if (failed_try_enters) {
				/*
				 * do mutex_enter preferably on a mutex
				 * with an iblock cookie of this intr
				 * level
				 */
				esp = first;
			}
			try_enter = (failed_try_enters == 0);
			failed_try_enters = n_intr_serviced = 0;
		}
	}
	...
	if (cb_needed) {
		...
		for (cb_info = esp_softc->e_callback_info;
		    cb_info != NULL; cb_info = cb_info->c_next) {
	...
	/*
	 * to avoid spurious interrupt messages when we steal interrupts
	 * from a different level, return CLAIMED if we serviced interrupts
	 * before on this level
	 */
	TRACE_0(TR_FAC_SCSI, TR_ESP_POLL_LOOP_END, "esp_poll_loop_end");
...
static int esp_slow_disks = 4;

static u_int
esp_poll_sun4d(caddr_t arg)
{
	register struct esp *esp = (struct esp *)arg;

	TRACE_0(TR_FAC_SCSI, TR_ESP_POLL_SUN4D_START, "esp_poll_sun4d_start");
	...
	/*
	 * for slow disks, we hang around a little longer to catch
	 * some more interrupts
	 */
	if (esp_slow_disks &&
	    ((esp->e_state & STATE_SELECTING) ||
	    ((esp->e_state == STATE_FREE) && esp->e_ndisc) ||
	    (esp->e_state == ACTS_UNKNOWN))) {
		register int i;

		if (mutex_tryenter(ESP_MUTEX)) {
			for (i = 0; i < esp_slow_disks; i++) {
				if (INTPENDING(esp)) {
					do {
						espsvc(esp);
					} while (INTPENDING(esp));
					ESP_CHECK_STARTQ_AND_ESP_MUTEX_EXIT
					    (esp);
					ESP_WAKEUP_CALLBACK_THREAD(esp);
					goto exit;
				}
...
exit:
	TRACE_0(TR_FAC_SCSI, TR_ESP_POLL_SUN4D_END, "esp_poll_sun4d_end");
	return (DDI_INTR_CLAIMED);
}
...
static void
espsvc(register struct esp *esp)
{
...
start_action:
	while (action != ACTION_RETURN) {
		...
		TRACE_3(TR_FAC_SCSI, TR_ESPSVC_ACTION_CALL,
			"espsvc call: state %s, action %d (%d)",
			esp_state_name(esp->e_state), action, i++);
 ...
 */
static int
esp_phasemanage(register struct esp *esp)
{
	...
	do {
		...
		TRACE_2(TR_FAC_SCSI, TR_ESP_PHASEMANAGE_CALL,
			"esp_phasemanage_call: state %s (%d)",
			esp_state_name(esp->e_state), i++);
	...
	if (sp->cmd_pkt.pkt_flags & FLAG_TAGMASK) {
		...
		if (sp->cmd_pkt.pkt_time >= tag_slots->e_timebase) {
			...
			if (--(tag_slots->e_dups) >= 0) {
				tag_slots->e_timeout  = tag_slots->e_timebase;
				EPRINTF("using dup timeout value\n");
			} else if (esp->e_tcmds[slot]) {
				register struct esp_cmd *ssp;
				register u_long n = 0;
				/*
			...
			/*
			 * reset e_timeout to e_timebase to prevent false
			 * cmd timeouts
			 */
			tag_slots->e_dups = 0;
			tag_slots->e_timeout = tag_slots->e_timebase;
 ...
 */
static int
esp_finish(register struct esp *esp)
{
	short last_slot;
	register struct esp_cmd *sp = CURRENT_CMD(esp);
 ...
 */
static void
esp_handle_qfull(register struct esp *esp, struct esp_cmd *sp, int slot)
{
	if ((++sp->cmd_qfull_retries > esp->e_qfull_retries[Tgt(sp)]) ||
		...
		IPRINTF2("%d.%d: status queue full, retries over\n",
			Tgt(sp), Lun(sp));
		esp_set_throttles(esp, slot, NLUNS_PER_TARGET, DRAIN_THROTTLE);
		esp_call_pkt_comp(esp, sp);
	} else {
		if (esp->e_reset_delay[Tgt(sp)] == 0) {
			esp->e_throttle[slot] =
		...
		if (esp->e_throttle[slot] == HOLD_THROTTLE) {
			 ...
			 */
			esp_set_throttles(esp, slot, NLUNS_PER_TARGET,
				QFULL_THROTTLE);
			if (esp->e_restart_cmd_timeid == 0) {
				esp->e_restart_cmd_timeid =
				    timeout(esp_restart_cmd, (caddr_t)esp,
...
static void
esp_restart_cmd(register struct esp *esp)
{
	int i;
	...
	for (i = 0; i < N_SLOTS; i += NLUNS_PER_TARGET) {
		...
		if (esp->e_throttle[i] == QFULL_THROTTLE) {
			esp_set_throttles(esp, i, NLUNS_PER_TARGET,
				CLEAR_THROTTLE);
		}
	}
 ...
 */
static void
esp_call_pkt_comp(register struct esp *esp, register struct esp_cmd *sp)
{
	...
	if (sp->cmd_pkt.pkt_comp && !(sp->cmd_flags & CFLAG_CMDARQ) &&
		...
		if (cb_info->c_qf) {
			 ...
			 * add to tail
			 */
			register struct esp_cmd *dp = cb_info->c_qb;
			ASSERT(dp != NULL);
			cb_info->c_qb = sp;
			sp->cmd_forw = NULL;
			dp->cmd_forw = sp;
		 ...
		 */
		register int slot = Tgt(sp) * NLUNS_PER_TARGET | Lun(sp);
		/*
		 * this recurses!
		 */
 ...
 */
static int
esp_finish_select(register struct esp *esp)
{
	register volatile struct espreg *ep = esp->e_reg;
	register volatile struct dmaga *dmar = esp->e_dma;
	register struct esp_cmd *sp = CURRENT_CMD(esp);
...
step_done:
		...
		if (cmdamt > 0) {
			sp->cmd_pkt.pkt_state |= STATE_SENT_CMD;
			sp->cmd_cdbp =
			    (u_char *) (((u_long)sp->cmd_cdbp) + cmdamt);
		}
...
static char *botched_tag =
	"Target %d.%d botched tagged queueing msg (0x%x, 0x%x)";
...
static int
esp_reconnect(register struct esp *esp)
{
	register volatile struct espreg *ep = esp->e_reg;
	register struct esp_cmd *sp;
...
static int
esp_handle_unknown(register struct esp *esp)
{
	TRACE_1(TR_FAC_SCSI, TR_ESP_HANDLE_UNKNOWN_START,
	    "esp_handle_unknown_start: esp %x", esp);
...
static int
esp_handle_clearing(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
...
esp_handle_data(struct esp *esp)
{
	...
	EPRINTF2("esp_handle_data: sp=%x, flags=%x\n", sp, sp->cmd_flags);
...
bad:
	...
	EPRINTF3("amt=%x, end=%x, cur_addr=%x\n", amt, end, sp->cmd_cur_addr);
	...
	if ((end & ~esp->e_dma_attr->dma_attr_seg) !=
		...
		IPRINTF3("cur_addr %x cur_addr+amt %x cntr_max %x\n",
	...
	if (amt > end) {
		IPRINTF4("ovflow amt=%x end=%x curaddr=%x count=%x\n",
...
static int
esp_handle_data_done(register struct esp *esp)
{
	register volatile struct espreg *ep = esp->e_reg;
	register volatile struct dmaga *dmar = esp->e_dma;
	register struct esp_cmd *sp = CURRENT_CMD(esp);
...
static int
esp_handle_c_cmplt(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	volatile struct espreg *ep = esp->e_reg;
	register u_char sts, msg, msgout, intr, perr;
	register u_char *c = (u_char *) esp->e_cmdarea;
	...
	if (esp->e_options & ESP_OPT_STACKED_CMDS) {
		 ...
		 * expensive; this loop is necessary for xbox and sun4d, see
...
static int
esp_handle_msg_in(register struct esp *esp)
{
 ...
 */
static int
esp_handle_more_msgin(register struct esp *esp)
{
...
static int
esp_handle_msg_in_done(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	register volatile struct espreg *ep = esp->e_reg;
	register sndmsg = 0;
...
static int
esp_onebyte_msg(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	register int msgout = 0;
	register u_char msgin = esp->e_last_msgin = esp->e_imsgarea[0];
	register tgt = Tgt(sp);
...
esp_handle_cmd_start(struct esp *esp)
{
	...
	if (amt > 0) {
		...
		SET_DMAESC_COUNT(esp->e_dma, esp->e_esc_read_count);
		ESP_DMA_READ(esp, amt, (u_long) esp->e_dmacookie.dmac_address);
		 ...
		 */
		cmd_distance =
		    (u_long) sp->cmd_cdbp - (u_long) sp->cmd_pkt.pkt_cdbp;
		if (cmd_distance >= (u_long) CDB_GROUP5) {
			if (sp->cmd_pkt.pkt_reason == CMD_CMPLT)
...
static int
esp_handle_cmd_done(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	register u_char intr = esp->e_intr;
 ...
 */
static int
esp_handle_msg_out(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	register volatile struct espreg *ep = esp->e_reg;
	register u_char *msgout = esp->e_cur_msgout;
...
static int
esp_handle_msg_out_done(register struct esp *esp)
{
	register struct esp_cmd *sp = CURRENT_CMD(esp);
	register volatile struct espreg *ep = esp->e_reg;
 ...
 */
static void
esp_update_props(struct esp *esp, int tgt, u_int tickval, u_int offset)
{
...
static int
esp_multibyte_msg(register struct esp *esp)
{
#ifdef ESPDEBUG
/*
 ...
 */
	...
	if (emsg == MSG_SYNCHRONOUS) {
		...
		if (offset) {
			...
			esp_update_props(esp, tgt, xfer_freq, offset);
...
{
	esplog(esp, CE_WARN, "Unexpected Selection Attempt");
	return (ACTION_RESET);
}
...
esp_set_new_window(struct esp *esp, struct esp_cmd *sp)
{
	...
	IPRINTF4("new window %x: off=%x, len=%x, count=%x\n",
...
static int
esp_next_window(struct esp *esp, struct esp_cmd *sp, u_int end)
{
	...
	if (sp->cmd_cur_win >= sp->cmd_nwin) {
		register int slot = Tgt(sp) * NTARGETS | Lun(sp);
 ...
 */
static int
esp_check_dma_error(register struct esp *esp)
{
	/*
 ...
 */
static void
esp_runpoll(register struct esp *esp, short slot, struct esp_cmd *sp)
{
	int limit, i, n;
	int timeout = 0;
	struct esp_cmd *savesp = esp->e_slots[slot];

	IPRINTF5("runpoll: slot=%x, cmd=%x, e_slots=%x, tcmds=%x, savesp=%x\n",
		slot, *((u_char *) sp->cmd_pkt.pkt_cdbp),
		esp->e_slots[slot], esp->e_tcmds[slot], savesp);
	...
	while ((sp->cmd_flags & CFLAG_COMPLETED) == 0) {
		esp_set_throttles(esp, slot, NLUNS_PER_TARGET, HOLD_THROTTLE);
		...
		if (esp->e_slots[slot] && (esp->e_slots[slot] != sp)) {
			ASSERT(esp->e_slots[slot] == savesp);
	 ...
	 */
	esp_set_throttles(esp, slot, NLUNS_PER_TARGET, CLEAR_THROTTLE);
 ...
 */
static int
esp_dopoll(register struct esp *esp, int limit)
{
	register int i, n;
...
esp_makeproxy_cmd(struct esp_cmd *sp, struct scsi_address *ap, int nmsgs, ...)
{
	...
	va_start(vap, nmsgs);
	for (i = 0; i < nmsgs; i++) {
		sp->cmd_cdb[ESP_PROXY_DATA + 1 + i] = (u_char)va_arg(vap, int);
	}
	va_end(vap);
}
...
{
	register u_char *p = esp->e_cur_msgout + msgout_offset;
...
static void
esp_watchsubr(register struct esp *esp)
{
	register short slot;
	register struct esp_cmd *sp;
	register int d = ((esp->e_dslot == 0)? 1 : esp->e_dslot);
	register struct t_slots *tag_slots;
	...
	for (slot = 0; slot < N_SLOTS; slot += d)  {
		...
		if (esp_force_timeout &&
			...
			esp_force_timeout = 0;
			(void) esp_cmd_timeout(esp, 0, slot);
			return;
		}
#endif /* ESP_TEST_TIMEOUT */
#ifdef ESP_TEST_RESET
...
static void
esp_cmd_timeout(register struct esp *esp, register struct esp_cmd *sp,
    int slot)
{
...
static void
esp_sync_backoff(register struct esp *esp, register struct esp_cmd *sp,
    int slot)
{
	char phase = esp->e_reg->esp_stat & ESP_PHASE_MASK;
	u_short state = esp->e_state;
	u_char tgt = slot / NLUNS_PER_TARGET;
	u_char lun = slot % NLUNS_PER_TARGET;
	...
	if (sp) {
		if (state != ACTS_DATA && state != ACTS_DATA_DONE) {
		    ...
		    (u_char) *(sp->cmd_pkt.pkt_cdbp) == SCMD_REQUEST_SENSE) {
 ...
 */
static int
esp_abort_curcmd(register struct esp *esp)
{
	if (esp->e_cur_slot != UNDEFINED) {
...
static int
esp_abort_cmd(register struct esp *esp, register struct esp_cmd *sp, int slot)
{
...
static int
esp_abort_allcmds(register struct esp *esp)
{
	/*
 ...
 * can be in progess at any point in time
 */
static int
esp_create_arq_pkt(struct esp *esp, struct scsi_address *ap, int create)
{
	 ...
	 */
	register struct esp_cmd *rqpktp;
	...
	if (create == 0) {
		...
		if (esp->e_save_pkt[slot]) {
			...
			scsi_free_consistent_buf((struct buf *)
		 ...
		 */
#ifndef __lock_lint
		rqpktp->cmd_pkt.pkt_comp = esp_complete_arq_pkt;
#endif
	}
	return (rval);
}
...
esp_complete_arq_pkt(struct esp *esp, struct esp_cmd *sp, int slot)
{
	...
	EPRINTF1("completing arq pkt sp=%x\n", sp);
...
esp_start_arq_pkt(struct esp *esp, struct esp_cmd *sp)
{
	...
	EPRINTF1("starting arq for slot %x\n", sp);
	bzero((caddr_t)esp->e_rq_sense_data[slot],
	    sizeof (struct scsi_extended_sense));
	EPRINTF3("slot=%x, arqsp=%x, save_pkt=%x\n", slot, arqsp,
		esp->e_arq_pkt[slot]);

	ASSERT(esp->e_save_pkt[slot] == NULL);
...
_esp_abort(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	register short slot = (ap->a_target * NLUNS_PER_TARGET) | ap->a_lun;
	 ...
	 */
	ASSERT(mutex_owned(ESP_MUTEX));
	IPRINTF4("esp_abort for slot %x, sp=%x, pkt_flags=%x, cur_sp=%x\n",
	    slot, sp, (sp? sp->cmd_pkt.pkt_flags : 0), cur_sp);
	...
	if (sp) {
		...
		if (cur_sp && (esp->e_state != STATE_FREE) &&
			...
			rval = esp_abort_connected_cmd(esp, cur_sp, abort_msg,
				slot);
		} else if (cur_sp || esp->e_tcmds[slot]) {
			rval = esp_abort_disconnected_cmd(esp, ap,
				    (struct esp_cmd *)0, abort_msg, slot);
			abort_disconnected++;
		}
	}
done:
	if (abort_disconnected && cur_sp && !cur_tagged_flag &&
 ...
 */
static void
esp_mark_packets(struct esp *esp, int slot, u_char reason, u_long stat)
{
	struct esp_cmd *sp;
...
esp_remove_readyQ(struct esp *esp, struct esp_cmd *sp, int slot)
{
	...
	IPRINTF3("aborting sp=%x %d.%d (not yet started)\n",
		sp, Tgt(sp), Lun(sp));
 ...
 */
static void
esp_flush_cmd(struct esp *esp, struct esp_cmd *sp, u_char reason,
    u_long  stat)
{
	register short slot = (Tgt(sp) * NLUNS_PER_TARGET) | Lun(sp);
 ...
 */
static int
esp_abort_connected_cmd(struct esp *esp, struct esp_cmd *sp, u_char msg,
    int slot)
{
	int rval = FALSE;
...
esp_save_throttles(struct esp *esp, int slot, int n, short *throttles)
{
	...
	bcopy((caddr_t)&esp->e_throttle[slot], (caddr_t)throttles,
...
_esp_reset(struct scsi_address *ap, int level)
{
	...
	if (level == RESET_ALL) {
		 ...
		 * the current pkt may change during this loop if the current
		 * pkt completes during the sending of the reset msg.
		 * a retry count prevents infinite looping here (not likely)
		 */
		while (cur_sp) {
			savec = cur_sp->cmd_pkt.pkt_comp;
			tagged_cmd = (cur_sp->cmd_pkt.pkt_flags & FLAG_TAGMASK);
			cur_sp->cmd_pkt.pkt_comp = NULL;
			...
			if ((retry ++ < 3) &&
			    ...
			    (ap->a_target ==
				(esp->e_cur_slot/NLUNS_PER_TARGET))) {

				rval = esp_reset_connected_cmd(esp, ap, slot);

				/*
				 * did the current cmd slip thru?
				 */
				if ((rval == FALSE) &&
				    (cur_sp->cmd_flags & CFLAG_COMPLETED)) {
					cur_sp->cmd_flags &= ~CFLAG_COMPLETED;
					...
					esp_decrement_ncmds(esp, cur_sp);
					esp_call_pkt_comp(esp, cur_sp);
					cur_sp = esp->e_slots[slot];
					/*
					 * retry with this cur_sp or just save
					 * new pkt_comp
					 */
					continue;
				}
		...
		if (rval == FALSE) {
			...
			if (cur_sp &&
				...
				if (rval == TRUE) {
					...
					/* paranoia */
					if ((esp->e_slots[slot] != cur_sp) &&
					    !tagged_cmd) {
						cur_sp->cmd_flags |=
							CFLAG_COMPLETED;
						MARK_PKT(cur_sp, CMD_BDR_FAIL,
						    STAT_DEV_RESET);
					}
				}
				/*
				 * if there was a preemption then this
				 * pkt may still be on the ready list
				 */
				(void) esp_remove_readyQ(esp, cur_sp, slot);
			}
		}
...
{
	mutex_enter(&esp_global_mutex);
	if (esp_reset_watch == 0) {
		esp_reset_watch = timeout(esp_watch_reset_delay, (caddr_t)0,
		    drv_usectohz((clock_t)
		    ESP_WATCH_RESET_DELAY_TICK * 1000));
	}
	ASSERT(esp_reset_watch != 0);
	mutex_exit(&esp_global_mutex);
}

/*ARGSUSED*/
static void
esp_watch_reset_delay(caddr_t arg)
{
	struct esp *esp;
	int not_done;
...
static int
esp_watch_reset_delay_subr(register struct esp *esp)
{
	...
	for (slot = 0; slot < N_SLOTS; slot += NLUNS_PER_TARGET)  {
		...
		if (esp->e_reset_delay[s] != 0) {
			...
			if (esp->e_reset_delay[s] <= 0) {
				...
				esp_set_throttles(esp, slot,
				    NLUNS_PER_TARGET, CLEAR_THROTTLE);
...
esp_reset_cleanup(struct esp *esp, int slot)
{
	 ...
	 */
	if (esp_watchdog_running && !panicstr) {
		esp_set_throttles(esp, start, NLUNS_PER_TARGET, HOLD_THROTTLE);
...
esp_reset_connected_cmd(struct esp *esp, struct scsi_address *ap, int slot)
{
	...
	sp->cmd_pkt.pkt_flags |= FLAG_NOINTR;
	esp_assert_atn(esp);
	esp_save_throttles(esp, slot, NLUNS_PER_TARGET, throttles);
	esp_set_throttles(esp, slot, NLUNS_PER_TARGET, HOLD_THROTTLE);
	(void) esp_dopoll(esp, SHORT_POLL_TIMEOUT);
	...
	esp_restore_throttles(esp, slot, NLUNS_PER_TARGET, throttles);
	sp->cmd_pkt.pkt_flags = flags;
	esp->e_omsglen = 0;
...
esp_reset_recovery(struct esp *esp)
{
	 ...
	 */
	bcopy((caddr_t)esp->e_slots, (caddr_t)eslots,
	    (sizeof (struct esp_cmd *)) * N_SLOTS);
	bzero((caddr_t)esp->e_slots,
		(sizeof (struct esp_cmd *)) * N_SLOTS);
	...
	/*
	 * if we did a busy wait reset delay, we are now ready to
	 * start new cmds
	 */
	if (esp->e_ncmds) {
		return (ACTION_SEARCH);
	...
	if (cidx == -1) {
		...
		case SCSI_CAP_SYNCHRONOUS:
			...
			if ((esp->e_target_scsi_options[ap->a_target] &
				...
				if (val) {
					esp->e_offset[ap->a_target] =
						DEFAULT_OFFSET;
				} else {
					esp->e_offset[ap->a_target] = 0;
				}
		...
		case SCSI_CAP_TAGGED_QING:
			/* Must have disco/reco enabled for tagged queueing. */
			if (((esp->e_target_scsi_options[ap->a_target] &
				SCSI_OPTIONS_DR) == 0) ||
				...
				if (val) {
					...
					int start, end, slot;
					int target = ap->a_target;
					u_int size = sizeof (struct t_slots);
		...
		case SCSI_CAP_QFULL_RETRIES:
			if (tgtonly) {
				...
				int i;
				for (i = 0; i < NTARGETS; i++) {
					esp->e_qfull_retries[i] = (u_char)val;
				}
			}
			rval = TRUE;
			break;
...
esp_update_TQ_props(struct esp *esp, int tgt, int value)
{
	...
	if (ddi_prop_exists(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS, property)) {
		...
		if (ddi_prop_create(DDI_DEV_T_NONE, dip, 0, property,
		    (caddr_t)&tmp, 0) != DDI_PROP_SUCCESS) {
			IPRINTF1("cannot create %s property\n", property);
		}
	}
}
...
esplog(struct esp *esp, int level, const char *fmt, ...)
{
	...
	va_end(ap);
	scsi_log(dev, esp_label, level, "%s", buf);
}
...
#ifdef ESPDEBUG
	scsi_log(dev, esp_label, SCSI_DEBUG, "%s", buf);
#else
	{
		char label[32];
		if (dev) {
...
static void
esp_printstate(register struct esp *esp, char *msg)
{
	register volatile struct espreg *ep = esp->e_reg;
	volatile struct dmaga *dmar = esp->e_dma;
	register struct esp_cmd *sp;
	u_char fifo_flag;
	u_int csr = dmar->dmaga_csr;
	u_int count = dmar->dmaga_count;
	u_int addr = dmar->dmaga_addr;
...
esp_dump_cmd(struct esp_cmd *sp)
{
	...
	eprintf((struct esp *)0,
	    ...
	    sp->cmd_pkt.pkt_statistics);
	eprintf((struct esp *)0,
	    "\tcmd_flags=0x%x cmd_timeout=%d\n", sp->cmd_flags,
	    sp->cmd_timeout);
}

static char *
esp_state_name(u_short state)
{
	if (state == STATE_FREE) {
		return ("FREE");
