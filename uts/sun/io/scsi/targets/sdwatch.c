...
#pragma ident	"@(#)sdwatch.c 1.20	95/03/17 SMI"
...
static int sdwatch_io_time = SDWATCH_IO_TIME;
...
/* ALL MEMBERS PROTECTED BY scsi_watch::sw_mutex */
...
/* VARIABLES PROTECTED BY "Unshared data": scsi_watch_request:: */
...
/* NO OTHER THREADS ARE RUNNING */
	mutex_init(&sw.sw_mutex, "scsi_watch_mutex", MUTEX_DRIVER, NULL);
	cv_init(&sw.sw_cv, "scsi_watch_cv", CV_DRIVER, NULL);
	...
	caddr_t			cb_arg)		/* device number */
{
	 ...
	 */
	swr = (struct scsi_watch_request *)
		kmem_zalloc(sizeof (struct scsi_watch_request), KM_SLEEP);
	...
	rqpkt->pkt_time = sdwatch_io_time;
...
scsi_watch_request_terminate(
	opaque_t	token)
{
	register struct scsi_watch_request	*swr =
		(struct scsi_watch_request *)token;
	register struct scsi_watch_request	*sswr;
	 ...
	 */
	SW_DEBUG((dev_info_t *)NULL, sw_label, SCSI_DEBUG,
		"scsi_watch_request_terminate: Entering(%x) ...\n", swr);
	mutex_enter(&sw.sw_mutex);
	...
	struct scsi_pkt	*pkt)
{
	...
	if (pkt == swr->swr_pkt) {
		...
		scsi_log((dev_info_t *)NULL, sw_label, CE_PANIC,
			"scsi_watch_intr: Bad Packet(%x)", pkt);
	}
