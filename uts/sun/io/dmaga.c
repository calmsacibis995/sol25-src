...
#pragma ident	"@(#)dmaga.c	1.24	94/11/25 SMI"
...
static int dmaidentify(dev_info_t *dev);
...
static struct modldrv modldrv = {
	...
	"Direct Memory Access driver",	/* Name of the module. */
	&dma_ops,	/* driver ops */
};
...
_init(void)
{
	...
	mutex_init(&dmaautolock, "dma autoconfig lock",
	    MUTEX_DRIVER, NULL);
...
static int
dmaidentify(dev_info_t *dev)
{
	if ((strcmp(ddi_get_name(dev), "dma") == 0) ||
	    (strcmp(ddi_get_name(dev), "espdma") == 0)) {
...
dmaattach(dev_info_t *dev, ddi_attach_cmd_t cmd)
{
	...
	ddi_set_driver_private(dev, (caddr_t)dp);
...
static u_int restrict_affinity_delta = 0x100000;
...
dma_affinity(dev_info_t *dma, dev_info_t *cdev)
{
	...
	if (strcmp(ddi_get_name(cdev), "esp") != 0) {
		...
		u_int slot = sparc_pd_getreg(dma, 0)->regspec_bustype;
		u_int slot_b =
		    sparc_pd_getreg(cdev, 0)->regspec_bustype;
		u_int addr = sparc_pd_getreg(dma, 0)->regspec_addr;
		u_int addr_b =
