 ...
 */
#pragma	ident	"@(#)cgfourteen.c 1.102	95/08/14 SMI"
...
struct mdi_softc {
	...
	caddr_t	mdi_vrt_vaddr;	/* Shadow page containing vertical */
					/* retrace counter	*/
	devpage_t *mdi_vrt_dp;		/* devpage for vrt cntr shadow page */
...
static struct mdi_data {	/* Per segment private data	*/
	...
	devpage_t	*dp;	/* devpage for this mapping */
};
...
static int mdi_ioctl(dev_t, int, int, int, cred_t *, int *);
...
static int mdi_save_mon_pwr(struct mdi_softc *);
static int mdi_restore_mon_pwr(struct mdi_softc *);
static int mdi_set_mon_pwr(struct mdi_softc *, int);

/* Call back routine for checkpoint/resume. */
static	void	mdi_callb(void *, int);
...
mdi_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	mutex_init(&softc->mdi_mutex, "MDI state vector",
	    MUTEX_DRIVER, softc->mdi_iblkc);
	mutex_init(&softc->pixrect_mutex, "MDI pixrect data",
	    MUTEX_DRIVER, softc->mdi_iblkc);
	mutex_init(&softc->mdi_degammalock, "MDI Degamma lock",
	    MUTEX_DRIVER, NULL);
	...
	sprintf(name, "cgfourteen%d", instance);
	...
	if (ddi_prop_create(DDI_DEV_T_NONE, dip, DDI_PROP_CANSLEEP,
		"pm_timestamp", (caddr_t)timestamp, sizeof (timestamp)) !=
			DDI_PROP_SUCCESS) {
		cmn_err(CE_WARN, "mdi_attach: "
			"Could not create timestamp property.\n");
		return (DDI_FAILURE);
	}
	...
	if (ddi_prop_create(DDI_DEV_T_NONE, dip, DDI_PROP_CANSLEEP,
		"pm_norm_pwr", (caddr_t)power, sizeof (power)) !=
			DDI_PROP_SUCCESS) {
		cmn_err(CE_WARN, "mdi_attach: "
			"Could not create power property.\n");
		return (DDI_FAILURE);
	}
	 ...
	 * XXX Need to change the call back class name after cpr
	 * create a better name than reservation 1.
	 */
	softc->mdi_callb_id = callb_add(mdi_callb, (void *)softc,
		CB_CL_CPR_RES1, "cgfourteen");
...
mdi_power(dev_info_t *dip, int cmpt, int level)
{
	...
	error = mdi_set_mon_pwr(softc, level);
...
static int
mdi_set_mon_pwr(struct mdi_softc *softc, int level)
{
	int error = 0;

	if (((level == 0) && softc->monitor_off) ||
		((level > 0) && !softc->monitor_off))
		return (DDI_SUCCESS);

	if (level == 0)
		error = mdi_save_mon_pwr(softc);
	else
		error = mdi_restore_mon_pwr(softc);
...
mdi_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	callb_delete(softc->mdi_callb_id);
...
/*ARGSUSED*/
static int
mdi_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *credp, int *rvalp)
{
	volatile struct mdi_register_address *mdi;
	...
	case MDI_SET_PIXELMODE:
		 ...
		 */
		mutex_enter(&softc->mdi_mutex);
		error = mdi_set_pixelmode(softc, arg);
		mutex_exit(&softc->mdi_mutex);
		break;

	case MDI_DEGAMMA:
		softc->mdi_gammaflag = arg;
		break;
	...
	case SET_MONITOR_POWER: {
		if (arg < 0 || arg > 1)
			...
			error = mdi_set_mon_pwr(softc, arg);
	}
	break;

	default:
		error = mdi_fbiocmds(dev, cmd, mode, arg);
		break;
	}
	if (error)
...
static	int segmdi_create(struct seg *, void *);
static	int segmdi_dup(struct seg *, struct seg *);
static	int segmdi_unmap(struct seg *, caddr_t, u_int);
static	void segmdi_free(struct seg *);
static	faultcode_t segmdi_fault(struct hat *, struct seg *, caddr_t, u_int,
		enum fault_type, enum seg_rw);
static	faultcode_t segmdi_nofault(struct seg *, caddr_t);
static	int	segmdi_setprot(struct seg *, caddr_t, u_int, u_int);
static	int segmdi_checkprot(struct seg *, caddr_t, u_int, u_int);
static	int segmdi_kluster(struct seg *, caddr_t, int);
static	int segmdi_incore(struct seg *, caddr_t, u_int, char *);
static	int segmdi_getprot(struct seg *, caddr_t, u_int, u_int *);
static off_t segmdi_getoffset(struct seg *, caddr_t);
static	int segmdi_nop();
static u_int segmdi_swapout(struct seg *);
static void segmdi_dump(struct seg *);
static int segmdi_getvp(struct seg *, caddr_t, struct vnode **);
static int segmdi_gettype(struct seg *, caddr_t);
static struct mdi_data *sdpmdi_create(struct mdi_data *, struct mdi_softc *);
...
mdi_mmap(dev_t dev, off_t offset, int prot)
{
	...
	if (mdi_getmapinfo(dev, offset, &objtype, &prot_allowed,
	    &map_allowed, &len_allowed, &pfnum, &dp) == -1)
		return (-1);
	...
	cred_t *cred)
{
	...
	if (mdi_getmapinfo(dev, offset, &objtype, &prot_allowed,
	    &maptype_allowed, &len_allowed, &pfnum, &dp) == -1)
		return (EACCES);
	 ...
	 */
	if ((maptype_allowed & (flags & ~MAP_FIXED)) != (flags & ~MAP_FIXED))
		return (EACCES);
	...
	if ((flags & MAP_FIXED) == 0) {
		 ...
		 */
		(void) map_addr(addr, len, (off_t)0, 0);
		if (*addr == NULL)
			return (ENOMEM);
		 ...
		 */
		as_unmap((struct as *)as, *addr, len);
	}

	error = as_map((struct as *)as, *addr, len, segmdi_create,
		(caddr_t)&seg_priv_data);

	as_rangeunlock(as);

	ASSERT(dp == NULL || objtype == MDI_SHDW_VRT_MAP);
	hat_contig_devload(as->a_hat, as, *addr, dp, pfnum, prot,
	    HAT_LOAD, len);
	...
	enum seg_rw rw)
{
	...
	ASSERT(sdp->dp == NULL || sdp->objtype == MDI_SHDW_VRT_MAP);

	/*
	 *  We want to load up the whole seg.
	 */
	hat_contig_devload(hat, seg->s_as, seg->s_base,
	    sdp->dp, (u_int)pf, sdp->prot,
	    HAT_LOAD | ((type == F_SOFTLOCK) ? HAT_LOCK : 0),
	    seg->s_size);
...
static int
segmdi_unmap(struct seg *seg, caddr_t addr, u_int len)
{
	register struct mdi_data *sdp = (struct mdi_data *)seg->s_data;
	 ...
	 */
	ASSERT(sdp->vp != NULL);
	if (VOP_DELMAP(common_specvp(sdp->vp),
			(offset_t)sdp->offset + (addr - seg->s_base),
			seg->s_as, addr, len, sdp->prot, sdp->maxprot,
			MAP_SHARED, CRED()) != 0)
		cmn_err(CE_WARN, "segmdi_unmap VOP_DELMAP failure");
...
/*ARGSUSED*/
static int
segmdi_setprot(seg, addr, len, prot)
	struct seg *seg;
	caddr_t addr;
	u_int len, prot;
{
...
/*ARGSUSED*/
static int
segmdi_checkprot(seg, addr, len, prot)
	struct seg *seg;
	caddr_t addr;
	u_int len, prot;
{
...
static int
segmdi_getprot(seg, addr, len, protv)
	register struct seg *seg;
...
sdpmdi_create(struct mdi_data *old, struct mdi_softc *softc)
{
	...
	*new = *old;   /* struct copy */
	...
	mutex_init(&new->segmdi_mutex, "MDI segment vector",
		MUTEX_DRIVER, softc->mdi_iblkc);
...
mdi_setupinfo(dev_info_t *dip, int instance)
{
	 ...
	 */
	softc->mdi_vrt_vaddr = (caddr_t)kmem_getpages(1, KM_NOSLEEP);
	...
	softc->mdi_vrt_dp = page_numtopp_nolock(mp[MDI_SHDW_VRT_INDX].pagenum);
	if (softc->mdi_vrt_dp == NULL) {
		return (B_FALSE);
	}
...
mdi_set_pixelmode(struct mdi_softc *softc, int mode)
{
	...
	if (softc->mapped_by_prom) {
		 ...
		 */
		hat_unlock((&kas)->a_hat, &kas, (caddr_t) mdi_console_vaddr,
					size);
		 ...
		 *  current mappings...
		 */
		hat_unload(&kas, mdi_console_vaddr, size, HAT_UNLOAD);
		...
		for (i = 0, a = mdi_console_vaddr;
			...
			hat_devload((&kas)->a_hat, &kas, a, (devpage_t *)NULL,
				(phys_pagenum + i), prot, HAT_LOCK);
		}
...
mdi_free_resources(struct mdi_softc *softc)
{
	...
	if (softc->mdi_vrt_vaddr != NULL) {
		kmem_freepages(softc->mdi_vrt_vaddr, 1);
		softc->mdi_vrt_vaddr = NULL;
		softc->mdi_vrt_dp = NULL;
	}
	if (softc->intr_added) {
...
mdi_set_timing_params(struct mdi_set_counters *cntrs,
	...
	int vvistime)
{
	cntrs->m_hss = (hfporch / 4) - 1;
	cntrs->m_hsc = ((hfporch + hsync) / 4) - 1;
	cntrs->m_hbc = ((hfporch + hsync + hbporch) / 4) - 1;
	cntrs->m_hbs = ((hfporch + hsync + hbporch + hvistime) / 4) - 1;

	cntrs->m_vss = vfporch - 1;
	cntrs->m_vsc = vfporch + vsync - 1;
	cntrs->m_vbc = vfporch + vsync + vbporch - 1;
	cntrs->m_vbs = vfporch + vsync + vbporch + vvistime - 1;
...
mdi_do_resume(struct mdi_softc *softc)
{
	...
	mutex_exit(&softc->mdi_mutex);
	mdi_set_cursor(softc, curs_info);
...
static int
mdi_save_mon_pwr(struct mdi_softc *softc)
{
	volatile struct mdi_register_address *mdi = softc->mdi_ctlp;
	...
	/* Alloc memory space to save VESA register contents */
	softc->mdi_vesa_save = (struct mdi_vesa_regs *)
		kmem_zalloc(sizeof (struct mdi_vesa_regs), KM_SLEEP);
	vesa_save = softc->mdi_vesa_save;

	/*
	 * Save the states of the sync registers and video state
	 */
...
static int
mdi_restore_mon_pwr(struct mdi_softc *softc)
{
	volatile struct mdi_register_address *mdi = softc->mdi_ctlp;
	...
	/* Restore the states of the sync registers */
	 ...
	 * screen jumpping. However if we do so, the coming up time for p4
	 * monitors will be too long because p4 monitors already has the
	 * delay built in.
	 */
	mdi_set_video(mdi, vesa_save->video_state);
	mdi_set_blanking(mdi, 1); /* blank bit active low */

	/* Free up the backup resource */
	kmem_free((struct mdi_vesa_regs *)softc->mdi_vesa_save,
