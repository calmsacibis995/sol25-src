...
#pragma ident	"@(#)dbri_mmcodec.c	1.45	94/12/22 SMI"
 ...
 * Private functions
 */
static void mmcodec_data_mode_ok(dbri_unit_t *);
...
#undef	PROP_VALUE
	...
	unitp->mmcstartup_id = timeout(mmcodec_startup_audio,
	    (caddr_t)unitp, MMCODEC_PDN_WAIT);
} /* mmcodec_reset */
 ...
 */
	...
	mmctrl.r.mb = Dbri_mmcodec_mb;
	mmctrl.r.pio = Dbri_mmcodec_pio;
...
#error	illegal CHI_DATA_MODE_LEN value;
	...
	if (cs->pal_present) {
		...
		ds = DChanToDs(unitp, DBRI_AUDIO_IN);
		ds->as.info.avail_ports |= AUDIO_LINE_IN;
		strcpy(ds->config, DBRI_DEV_SPKRBOX);
 ...
 */
static void
mmcodec_watchdog(dbri_unit_t *unitp)
{
	dbri_chi_status_t *cs;
 ...
 */
static void
mmcodec_data_mode_ok(dbri_unit_t *unitp)
{
	dbri_chi_status_t *cs;
...
mmcodec_setup_data_mode(dbri_unit_t *unitp)
{
	...
	unitp->mmcdataok_id = timeout(mmcodec_data_mode_ok, (caddr_t)unitp,
	    TICKS(mute_time));
 ...
 */
void
mmcodec_startup_audio(dbri_unit_t *unitp)
{
	dbri_chi_status_t *cs;
	dbri_pipe_t *pipep;
	aud_stream_t *as;
 ...
 */
static void
mmcodec_volume_timer(dbri_unit_t *unitp)
{
	boolean_t done;
	dbri_pipe_t *pipep;
	aud_stream_t *control_as;
	...
	if (!done) {
		...
		unitp->mmcvolume_id = timeout(mmcodec_volume_timer,
		    (caddr_t)unitp, sb_button_repeat);
	}
