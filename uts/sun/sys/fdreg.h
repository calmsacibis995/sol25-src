...
#pragma ident	"@(#)fdreg.h	1.11	93/11/01 SMI"
...
#define	ZEROBITS_DMA	0x02	/* reset bits - on DMA platform */
...
/* Interfaces for the 7.1 environment */
#define	Msr(fdc)	ddi_getb(fdc->c_handlep_cont, \
					((uchar_t *)fdc->c_control))
#define	Dsr(fdc, val)   ddi_putb(fdc->c_handlep_cont, \
					((uchar_t *)fdc->c_control),\
					((uchar_t)val))
#define	Dir(fdc)	ddi_getb(fdc->c_handlep_cont, \
					((uchar_t *)fdc->c_dir))
#define	Fifo(fdc)	ddi_getb(fdc->c_handlep_cont, \
					((uchar_t *)fdc->c_fifo))
#define	Set_Fifo(fdc, val) ddi_putb(fdc->c_handlep_cont, \
					((uchar_t *)fdc->c_fifo), \
					((uchar_t)val))
#define	Dor(fdc)	ddi_getb(fdc->c_handlep_cont, ((uchar_t *)fdc->c_dor))
#define	Set_dor(fdc, val, flag) \
	{ if (flag) \
		ddi_putb(fdc->c_handlep_cont, ((uchar_t *)fdc->c_dor), \
			((uchar_t)(Dor(fdc) | (val)))); \
	    else \
		ddi_putb(fdc->c_handlep_cont, ((uchar_t *)fdc->c_dor), \
			((uchar_t)(Dor(fdc) & ~(val)))); }
...
#define	Set_auxio(fdc, val)	ddi_putl(fdc->c_handlep_aux, \
				((ulong_t *)fdc->c_auxio_reg), \
				    ((ulong_t)(val)))

#define	Get_auxio(fdc)		ddi_getl(fdc->c_handlep_aux, \
				((ulong_t *)fdc->c_auxio_reg))
...
#ifndef _ASM
struct fdc_dma_reg {
	u_long fdc_dcsr;	/* Data Control Status Register */
	u_long fdc_dacr;	/* DMA Address Count Registers */
	u_long fdc_dbcr;	/* DMA Byte Count Register */
};

#define	Set_dcsr(fdc, val)	ddi_putl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dcsr), \
				((ulong_t)val))
#define	Get_dcsr(fdc)   ddi_getl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dcsr))
 ...
 */
#define	Reset_dcsr(fdc)	{	Set_dcsr(fdc,  DCSR_RESET); \
				while (Get_dcsr(fdc) & DCSR_CYC_PEND); \
				Set_dcsr(fdc, 0); }

#define	Set_dacr(fdc, val)	ddi_putl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dacr), \
				((ulong_t)val))

#define	Get_dacr(fdc)		ddi_getl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dacr))

#define	Set_dbcr(fdc, val)	ddi_putl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dbcr), \
				((ulong_t)val))

#define	Get_dbcr(fdc)		ddi_getl(fdc->c_handlep_dma, \
				((ulong_t *)&fdc->c_dma_regs->fdc_dbcr))
...
#define	DCSR_BURST_1	0x00000000	/* Burst Size bit 1 */
#else
