...
#pragma ident	"@(#)fasvar.h	1.20	95/08/22 SMI"
...
struct f_slots {
	...
	u_short			f_n_slots; /* number of	a_slots		*/
	u_short			f_size;
...
/*
 * this	structure holds all pkts that need callback
 */
struct callback_info {
	struct fas_cmd		*c_qf;
	struct fas_cmd		*c_qb;
	kmutex_t		c_mutex;
	u_char			c_in_callback;
};
...
struct fas {
	 ...
	 */
	u_char		f_fasconf3[NTARGETS_WIDE];
	u_char		f_fasconf3_reg_last;
	 ...
	 */
	u_char	f_sdtr_sent;	/* Count of sync data negotiation messages: */
				...
				/* sent. */
	u_char	f_wdtr_sent;	/* same	for wide negotations */
	u_char	f_stat;		/* soft	copy of	status register	*/
	u_char	f_stat2;	/* soft	copy of	status2	register */
	u_char	f_intr;		/* soft	copy of	interrupt register */
	u_char	f_step;		/* soft	copy of	step register */
	u_char	f_abort_msg_sent; /* indicates that abort message went out */
	u_char	f_reset_msg_sent; /* indicates that device reset message */
				/* went	out */
	u_char	f_last_cmd;	/* last	cmd sent to fas	chip */

	u_short	f_state;	/* state of the	driver */
	u_short	f_laststate;	/* last	state of the driver */
	u_char	f_suspended;	/* true	if driver is suspended */
	u_char	f_dslot;	/* delta to next slot */
	u_char	f_idcode;	/* chips idcode	*/
...
#define	OMSGSIZE	12
	u_char		f_cur_msgout[OMSGSIZE];
	u_char		f_last_msgout;
	u_char		f_omsglen;
...
#define	IMSGSIZE	8
	 ...
	 */
	u_char	f_offset[NTARGETS_WIDE]; /* synch offset + req-ack delay */
	u_char	f_sync_period[NTARGETS_WIDE]; /* synch period reg val */
	u_char	f_neg_period[NTARGETS_WIDE]; /*	synch periods (negotiated) */
	u_short	f_backoff;		/* sync/wide backoff bit mask */
	u_char	f_req_ack_delay;	/* req ack delay in offset reg */
	u_char	f_offset_reg_last;	/* save	last offset value */
	u_char	f_period_reg_last;	/* save	last period value */

	/*
	 * fifo	length and fifo	contents stored	here before reading intr reg
	 */
	u_char		f_fifolen;
	u_char		f_fifo[2*FIFOSIZE];

	/*
	 * These u_shorts are  bit maps	for targets
	 */
	u_short		f_wide_known;	/* wide	negotiate on	next cmd */
	u_short		f_nowide;	/* no wide for this target */
	u_short		f_wide_enabled;	/* wide	enabled	for this target	*/

	u_short		f_sync_known;	/* sync	negotiate on next cmd */
	u_short		f_nosync;	/* no sync for this target */
	u_short		f_sync_enabled;	/* sync	enabled	for this target	*/
	 ...
	 */
	u_long			f_lastdma;	/* last	dma address */
	u_long			f_lastcount;	/* last	dma count */
	...
	/*
	 * callback  info for this fas
	 */
	struct callback_info	*f_callback_info;
	 ...
	 * qfull handling
	 */
	u_char			f_qfull_retries[NTARGETS_WIDE];
	u_short			f_qfull_retry_interval[NTARGETS_WIDE];
	int			f_restart_cmd_timeid;
	 ...
	 */
	u_int			f_reg_trace_index;
	u_int			f_reg_trace[REG_TRACE_BUF_SIZE+1];
#endif
};
_NOTE(MUTEX_PROTECTS_DATA(fas::f_mutex,	fas))
_NOTE(SCHEME_PROTECTS_DATA("save sharing",
	fas::f_next fas::f_callback_info fas::f_state))
_NOTE(SCHEME_PROTECTS_DATA("save sharing",
	fas::f_dma fas::f_dma_attr fas::f_dma_rev))
...
/*
 * Representations of Driver states (stored in tags f_state && e_laststate).
 *
...
#define	EPRINTF(str)		if (EDEBUGGING)	fas_printf(fas,	str)
#define	EPRINTF1(str, a)	if (EDEBUGGING)	fas_printf(fas,	str, a)
#define	EPRINTF2(str, a, b)	if (EDEBUGGING)	fas_printf(fas,	str, a,	b)
#define	EPRINTF3(str, a, b, c)	if (EDEBUGGING)	fas_printf(fas,	str, a,	b, c)
#define	EPRINTF4(str, a, b, c, d)	\
	if (EDEBUGGING)	fas_printf(fas,	str, a,	b, c, d)
#define	EPRINTF5(str, a, b, c, d, e)	\
	if (EDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e)
#define	EPRINTF6(str, a, b, c, d, e, f)	\
	if (EDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e, f)

#define	DPRINTF(str)		if (DDEBUGGING)	fas_printf(fas,	str)
#define	DPRINTF1(str, a)	if (DDEBUGGING)	fas_printf(fas,	str, a)
#define	DPRINTF2(str, a, b)	if (DDEBUGGING)	fas_printf(fas,	str, a,	b)
#define	DPRINTF3(str, a, b, c)	if (DDEBUGGING)	fas_printf(fas,	str, a,	b, c)
#define	DPRINTF4(str, a, b, c, d)	\
	if (DDEBUGGING)	fas_printf(fas,	str, a,	b, c, d)
#define	DPRINTF5(str, a, b, c, d, e)	\
	if (DDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e)
#define	DPRINTF6(str, a, b, c, d, e, f)	\
	if (DDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e, f)

#define	IPRINTF(str)		if (IDEBUGGING)	fas_printf(fas,	str)
#define	IPRINTF1(str, a)	if (IDEBUGGING)	fas_printf(fas,	str, a)
#define	IPRINTF2(str, a, b)	if (IDEBUGGING)	fas_printf(fas,	str, a,	b)
#define	IPRINTF3(str, a, b, c)	if (IDEBUGGING)	fas_printf(fas,	str, a,	b, c)
#define	IPRINTF4(str, a, b, c, d)	\
	if (IDEBUGGING)	fas_printf(fas,	str, a,	b, c, d)
#define	IPRINTF5(str, a, b, c, d, e)	\
	if (IDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e)
#define	IPRINTF6(str, a, b, c, d, e, f)	\
	if (IDEBUGGING)	fas_printf(fas,	str, a,	b, c, d, e, f)
 ...
 * each	retry to be after 10 ms.
 */
#define	QFULL_RETRIES		10
#define	QFULL_RETRY_INTERVAL	10

/*
 * dma revision
 */
#define	FAS_DMA_REV(fas)	(fas)->f_dma_rev
...
#define	FAS_EMPTY_CALLBACKQ(fas)  fas_empty_callbackQ(fas->f_callback_info)
...
/*
 * 1 ms	timeout	on receiving tag on reconnect
 */
#define	RECONNECT_TAG_RCV_TIMEOUT 1000	/* allow up to 1 ms */
