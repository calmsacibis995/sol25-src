...
#ifndef	_SYS_SCSI_ADAPTERS_NCRCTL_H
#define	_SYS_SCSI_ADAPTERS_NCRCTL_H

#pragma ident	"@(#)ncrctl.h	1.7	93/11/12 SMI"
...
struct ncrdma {
	 ...
	 */
	vaccess	addr;	/* csr.la	- (4/110, SCSI-3 sun3/3x) */
			/* csr.msw	- (SCSI-3 sun4) */
	...
	vaccess	count;	/* csr.la	- (4/110, SCSI-3 sun3/3x) */
			/* csr.msw	- (SCSI-3 sun4) */
			/* csr.lsw	- (SCSI-3 sun4, 3/E) */
			/* csr.ca[0-3]	- not used */
...
#define	SET_DMA_ADDR(ncr, val)	\
	((ncr)->n_type == IS_COBRA) ? \
	((ncr)->n_dma->addr.la = (u_int)(val)) : \
		((ncr)->n_dma->addr.msw = (u_short)(val >> 16), \
...
#define	SET_DMA_COUNT(ncr, val)	\
	((ncr)->n_type == IS_COBRA) ? \
	(ncr)->n_dma->count.la = (u_int)(val) : \
		((ncr)->n_dma->count.msw = (u_short)(val >> 16), \
...
#define	REQ_ASSERTED(ncr)	\
	(ncr_sbcwait((u_char *)&CBSR, NCR_CBSR_REQ, NCR_WAIT_COUNT, 1) == \
	SUCCESS)

#define	REQ_DROPPED(ncr)	\
	(ncr_sbcwait((u_char *)&CBSR, NCR_CBSR_REQ, NCR_WAIT_COUNT, 0) == \
	SUCCESS)
...
#define	Lobyte(x)	(((u_int)(x)) & 0xff)

/*
 * Default value for the address modifier, scsi3 vme version only
 */
#define	VME_AM_DFLT	SP_VME_A24D16_SUPV_D
...
#define	NCRHWSIZE	\
	sizeof (struct ncrsbc) + sizeof (struct ncrdma) + sizeof (struct ncrctl)
...
#endif	/* _SYS_SCSI_ADAPTERS_NCRCTL_H */
