...
#pragma ident	"@(#)plnvar.h	1.14	95/08/28 SMI"
...
typedef struct pln_rsp {
	union {
	    ...
	    u_char		rsp_data[PLN_RSP_DATA_SIZE];
	} rp;
} pln_rsp_t;
...
struct pln_scsi_cmd {
	...
	u_char		cmd_cdblen;	/* SCSI command length */
	u_char		cmd_senselen;	/* SCSI Sense length */
	u_char		cmd_tgtlen;	/* target private len */
	u_long		cmd_flags;	/* flags */
	u_long		cmd_dmacount;	/* Transfer length */
...
typedef struct pln_fc_pkt {
	...
	ddi_dma_handle_t	fp_datahandle;
 ...
 * pd_state
 */
#define	PD_NOT_ATTACHED		0		/* attach hasn't been called */
#define	PD_ATTACHED		1		/* we've attached this one */
 ...
 */
#define	SDEV2TRAN(sd)		((sd)->sd_address.a_cookie)
#define	SDEV2ADDR(sd)		(&((sd)->sd_address))
#define	PKT2TRAN(pkt)		((pkt)->pkt_address.a_cookie)

#define	TRAN2PLN(tran)		((struct pln *)(tran))
#define	SDEV2PLN(sd)		(TRAN2PLN(SDEV2TRAN(sd)))
#define	PKT2PLN(pkt)		(TRAN2PLN(PKT2TRAN(pkt)))
#define	ADDR2PLN(ap)		(TRAN2PLN((ap)->a_cookie))

#define	PKT2DIP(pkt)		(PKT2TRAN(pkt)->tran_dev)
...
struct pln {
	 ...
	 * which defines function entry points to the library. The
	...
	struct scsi_transport	pln_tran;
#else
...
#define	pln_iblock		pln_fc_tran->fc_iblock
	 ...
	 * Instrumentation
	 */
	long		pln_ncmd_ref;		/* varies with # of commands */
	long		pln_maxcmds;		/* max commands we can issue */
	long		pln_throttle_cnt;	/* # of "throttle" responses */
	long		pln_throttle_ncmds;	/* ncmd_ref @ "throttle" rsp */
	volatile long	pln_throttle_flag;	/* this pln has "onhold" cmds */
	...
	/*
	 * For the callback threads
	 */
	kmutex_t		pln_callback_mutex;
	kcondvar_t		pln_callback_cv;
	struct pln_scsi_cmd	*pln_callback_head;
	struct pln_scsi_cmd	*pln_callback_tail;
	kthread_t		*pln_callback_thread;

	/*
	 * Structures and variables for pln_ctlr
	 */

	kcondvar_t		pln_private_cv; /* Condition variable */
	pln_controller_t	pln_controller;
};
...
/*
 * Initial value for pln_maxcmds
 */
#define	PLN_MAX_DEFAULT		1024

/*
 * When throttling, we'll set the throttle point to the current queue
 * depth minus this quantity
 */
#define	PLN_THROTTLE_BACKOFF	16

/*
 * The number of times we receive a response telling us to throttle
 * before we readjust the throttle downwards
 */
#define	PLN_MAX_T_RESP		16

/*
 * The amount we'll let the queue drop down before we search the list
 * of onhold commands
 */
#define	PLN_THROTTLE_SWING	8

/*
 * The amount we'll increment pln_maxcmds each second
 */
#define	PLN_MAXCMDS_INCR	32

/*
 * The maximum difference between pln_maxcmds and pln_ncmd_ref
 */
#define	PLN_MAXCMDS_DELTA	4096
 ...
 */
#define	pln_mutex		pln_tran.tran_mutex
#define	pln_dip			pln_tran.tran_dev
#endif	/* ON1093 */
 ...
 */
#ifdef	ON1093
_NOTE(SCHEME_PROTECTS_DATA("safe sharing", pln::pln_tran.tran_dev))
#else	ON1093
...
_NOTE(MUTEX_PROTECTS_DATA(pln::pln_cr_mutex, pln_cr_pool::waiters_head))
_NOTE(SCHEME_PROTECTS_DATA("safe sharing", pln::pln_throttle_ncmds))
...
_NOTE(MUTEX_PROTECTS_DATA(pln::pln_callback_mutex, pln::pln_callback_head))
_NOTE(MUTEX_PROTECTS_DATA(pln::pln_callback_mutex, pln::pln_callback_tail))
...
_NOTE(SCHEME_PROTECTS_DATA("safe sharing", pln_fc_pkt::fp_datahandle))
...
_NOTE(MUTEX_PROTECTS_DATA(pln::pln_cr_mutex, pln_fc_pkt::fp_cr_next))
...
_NOTE(MUTEX_PROTECTS_DATA(pln_disk::pd_pkt_alloc_mutex, pln_fc_pkt::fp_state))
...
_NOTE(SCHEME_PROTECTS_DATA("safe sharing", pln_fc_pkt::fp_datasegs))
_NOTE(SCHEME_PROTECTS_DATA("safe sharing", pln_fc_pkt::fp_datahandle))
_NOTE(MUTEX_PROTECTS_DATA(pln_disk::pd_pkt_alloc_mutex, pln_fc_pkt::fp_next))
_NOTE(MUTEX_PROTECTS_DATA(pln_disk::pd_pkt_alloc_mutex, pln_fc_pkt::fp_prev))

/*
 * Warlock directives for pln_disk
 */
_NOTE(MUTEX_PROTECTS_DATA(pln_disk::pd_pkt_alloc_mutex, pln_disk::pd_pkt_pool))
_NOTE(MUTEX_PROTECTS_DATA(pln_disk::pd_pkt_alloc_mutex,
...
_NOTE(SCHEME_PROTECTS_DATA("write only in attach", pln_disk::pd_state))
