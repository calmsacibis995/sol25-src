...
#pragma ident	"@(#)plndef.h	1.5	95/02/10 SMI"
...
typedef struct pln_address {
	union {
		...
		struct {
			u_short	pa_entity;	/* entity within pluto */
			u_short	pa_port;	/* port address */
			u_short	pa_target;	/* target address */
			u_short	pa_reserved;	/* reserved */
		} pln_addr;
	} un;
} pln_address_t;
