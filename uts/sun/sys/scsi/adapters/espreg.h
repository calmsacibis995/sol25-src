...
#pragma ident	"@(#)espreg.h	1.19	93/11/12 SMI"
...
#define	SET_ESP_COUNT_16(ep, val)	\
	(ep)->esp_xcnt_lo = (u_char) (val), \
	(ep)->esp_xcnt_mid = (u_char) ((val) >> 8)
...
#define	ESP_CLOCK_TICK(esp)	\
	((u_long) ESP_CLOCK_DELAY * (u_long) (esp)->e_clock_conv * \
	(u_long) (esp)->e_clock_cycle) / (u_long) 1000
...
#define	MIN_SYNC_TIME(esp)	\
	((u_long) MIN_SYNC((esp)) * (u_long) ((esp)->e_clock_cycle)) / \
	    (u_long) 1000
#define	MAX_SYNC_TIME(esp)	\
	((u_long) MAX_SYNC((esp)) * (u_long) ((esp)->e_clock_cycle)) / \
	    (u_long) 1000
