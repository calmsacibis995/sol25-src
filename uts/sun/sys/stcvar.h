...
#pragma ident	"@(#)stcvar.h	1.58	94/06/08 SMI"
...
#define	MUTEX_STC_NAME	"stc_unit_t mutex (stc)"
#define	MUTEX_PPC_NAME	"stc_unit_t mutex (ppc)"
#define	MUTEX_LINE_NAME	"stc_line_t mutex"
#define	CV_SOFT_NAME	"stc_line_t cv"
...
#define	UNTIMEOUT(utt) {		\
	if (utt) {			\
	    ...
	    /* cmn_err(CE_CONT,"UNTIMEOUT: id is 0\n"); */ \
	}				\
}
...
#define	DTR_SET(L, S) {							\
	if (L->dtr) {							\
		if (L->flags & STC_DTRFORCE) {				\
			*(L->dtr) = DTR_ON;				\
			L->dtr_shadow = (u_char)DTR_ON;			\
		} else {						\
			*(L->dtr) = S;					\
			L->dtr_shadow = (u_char)S;			\
		}							\
	} else {							\
		cmn_err(CE_CONT,					\
			"stc%d: line %d NULL pointer in DTR_SET(0x%x, %d)\n", \
			L->unit_no, L->line_no, L, S);			\
	}								\
}
...
struct stc_lineparam_t {
	...
	/* ppc-specific stuff */
	u_char		pcon;		/* ppc's pcon register default value */
	int		ack_maxloops;	/* used in ppc_acktimeout() */
	/* cd-180 specific stuff */
	u_char		cor1;		/* parity/stop bits/char length */
	u_char		cor2;		/* flow control/loopback/embed xmit */
	u_char		cor3;		/* s/w flow cntl/schar detect/Rx fifo */
	u_char		schr1;		/* SCHR1- Special Character Reg 1 */
	u_char		schr2;		/* SCHR2- Special Character Reg 2 */
	u_char		schr3;		/* SCHR3- Special Character Reg 3 */
	u_char		schr4;		/* SCHR4- Special Character Reg 4 */
	u_char		mcor1;		/* modem pins to monitor */
	u_char		mcor2;		/* modem pins to monitor */
	u_char		rbprh;		/* RBPRH-Rcv Baud Rate Period Reg Hi */
	u_char		rbprl;		/* RBPRL-Rcv Baud Rate Period Reg Lo */
	u_char		tbprh;		/* TBPRH-Xmit Baud Rate Period Reg Hi */
	u_char		tbprl;		/* TBPRL-Xmit Baud Rate Period Reg Lo */
	u_char		rtpr;		/* inter-character receive timer */
};
...
struct stc_line_t {
	...
	int			stc_timeout_id;		/* timeout id */
	int			stc_draintimeout_id;	/* timeout id */
	int			ppc_acktimeout_id;	/* timeout id */
	int			ppc_out_id;		/* timeout id */
	struct stc_unit_t	*stc;		/* do this for PUTSILO */
	kcondvar_t		*cvp;		/* used for cv_wait_sig() */
	kmutex_t		*line_mutex;	/* stc_line_t mutex */
	/* cd180 speific stuff */
	u_char			*dtr;		/* DTR latch reg, LSB only */
	u_char			dtr_shadow;	/* shadow of DTR latch */
	u_char			stc_flowc;	/* flow control character */
	int			stc_txbufsize;	/* size of (soft) Tx buffer */
	u_char			stc_txbuf[LINE_TXBUFSIZE]; /* soft tx buffer */
	int			stc_txcount;	/* chars in stc_txbuf */
	int			stc_silosize;	/* size of rx silo */
	int			stc_sscnt;	/* silo count */
	u_char			*stc_source;	/* silo source */
	u_char			*stc_sink;	/* silo sink */
	u_char			stc_ssilo[STC_SILOSIZE];	/* soft silo */
	int			stc_qfcnt;	/* queue full retry count */
	int			stc_wbufcid;	/* id of write-side bufcall */
	...
	u_char			pcon_s;		/* soft copy of control reg */
	u_char			*stc_txbufp;	/* ptr into soft tx buffer */
