...
#pragma ident	"@(#)zsdev.h	1.24	94/12/02 SMI"
...
#define	lbolt_condp	((cond_t *)&lbolt_cv)	/* XXX - DDI/DKI ? */
...
#define	SCC_WRITEA(reg, val) { \
	((struct zscc_device *)((int)zs->zs_addr|ZSOFF))->zscc_control = reg; \
	ZSDELAY(); \
	((struct zscc_device *)((int)zs->zs_addr|ZSOFF))->zscc_control = val; \
	ZSDELAY(); \
	zs->zs_wreg[reg] = val; \
}
...
#define	SCC_READA(reg, var) { \
	((struct zscc_device *)((int)zs->zs_addr|ZSOFF))->zscc_control = reg; \
	ZSDELAY(); \
	var = ((struct zscc_device *)((int)zs->zs_addr|ZSOFF))->zscc_control; \
	ZSDELAY(); \
}
 ...
 */
#define	ZS_MAX_PRIV_STR 256 /* int */
struct zscom {
	 ...
	 */
	u_char		zs_suspended;	/* True, if suspended */
	...
	cond_t		*zs_flags_cv;   /* condition variable for flags */
	u_int 		zs_priv_str[ZS_MAX_PRIV_STR];
	u_char 		zs_flags_need;	/* random flags */
	u_char		zs_flags_doing;	/* random flags */
};
...
struct zs_prog {
	...
	u_char		flags;	/* see definitions below */
	u_char		wr4;	/* misc parameters and modes */
	u_char		wr11;	/* clock mode control */
	u_char		wr12;	/* BRG time constant Lo byte */
	u_char		wr13;	/* BRG time constant Hi byte */
	u_char		wr3;	/* receiver parameters and control */
	u_char		wr5;	/* transmitter parameters and control */
	u_char		wr15;	/* external status interrupt control */
};
