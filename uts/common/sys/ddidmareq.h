...
#pragma ident	"@(#)ddidmareq.h	1.18	94/12/19 SMI"
 ...
 */
struct phy_address {
	u_long	p_addr;		/* base physical address */
	u_long	p_memtype;	/* memory type */
};
 ...
 *
 * On the other hand, if you have a VME card plugged into an older Sun
 * workstation, you shouldn't define the addressing constraints of your
 * VME card to be cognizant that that specific Sun VME implementation maps
 * the bottom megabyte of the VME address space to the top megabyte of
 * the Sun's address space. The VME nexus driver will translate this for
 * you. You should instead define the VME card to have the constraints
 * that are appropriate for the VME bus.
 */
...
/*
 * Use A32 DVMA space on sun4m. Private communication
 * between leaf and vme nexus driver
 * XXX should belong elsewhere
 */
#define	DDI_DMA_VME_USEA32	0x1000
