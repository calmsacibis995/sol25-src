...
#pragma ident	"@(#)ddipropdefs.h	1.9	94/12/05 SMI"	/* SVr4.0 */
...
typedef struct ddi_prop_handle {
	...
	u_int			ph_size;	/* Size of encoded data */
	u_int			ph_flags;	/* See below */
...
typedef struct prop_handle_ops {
	int (*op_prop_int)(prop_handle_t *ph, u_int cmd, int *data);
	int (*op_prop_str)(prop_handle_t *ph, u_int cmd, char *data);
	int (*op_prop_bytes)(prop_handle_t *ph, u_int cmd,
				u_char *data, u_int size);
} prop_handle_ops_t;
