...
#pragma ident	"@(#)sunddi.h	1.54	95/01/20 SMI"
...
#define	DEVI_REV		((int)1)	/* ret from ddi_get_version */
...
/*
 * Shared register access - return values from ddi_get_shared_reg()
 * defined below.
 */
#define	DDI_SHREG_SUCCESS	DDI_SUCCESS	/* register found */
#define	DDI_SHREG_UNKNOWN	-4	/* can't find named register */
#define	DDI_SHREG_BADACCESS	-5	/* cannot satisfy access type */
 ...
 */
int
ddi_map_regs(dev_info_t *dip, u_int rnumber, caddr_t *kaddrp,
...
void
ddi_unmap_regs(dev_info_t *dip, u_int rnumber, caddr_t *kaddrp,
	off_t offset, off_t len);
 ...
 */
int ddi_peekc(dev_info_t *dip, char *addr, char *val_p);
int ddi_peeks(dev_info_t *dip, short *addr, short *val_p);
int ddi_peekl(dev_info_t *dip, long *addr, long *val_p);
int ddi_peekd(dev_info_t *dip, longlong_t *addr, longlong_t *val_p);

int ddi_pokec(dev_info_t *dip, char *addr, char val);
int ddi_pokes(dev_info_t *dip, short *addr, short val);
int ddi_pokel(dev_info_t *dip, long *addr, long val);
int ddi_poked(dev_info_t *dip, longlong_t *addr, longlong_t val);
 ...
 */
void
ddi_remove_intr(dev_info_t *dip, u_int inum,
 ...
 */
int ddi_prop_get_int(dev_t match_dev, dev_info_t *dip, u_int flags,
    char *name, int defvalue);
int ddi_prop_lookup_int_array(dev_t match_dev, dev_info_t *dip, u_int flags,
 ...
 */
int ddi_prop_lookup_string(dev_t match_dev, dev_info_t *dip, u_int flags,
    char *name, char **data);
int ddi_prop_lookup_string_array(dev_t match_dev, dev_info_t *dip, u_int flags,
 ...
 */
int ddi_prop_lookup_byte_array(dev_t match_dev, dev_info_t *dip, u_int flags,
...
int
ddi_get_version(dev_info_t *dip);
...
dev_info_t *
ddi_add_child(dev_info_t *pdip, char *name, u_int nodeid, u_int instance);
...
int
ddi_dma_kvaddrp(ddi_dma_handle_t, off_t, u_int, caddr_t *);
...
int
ddi_dma_movwin(ddi_dma_handle_t, off_t *offp, u_int *lenp, ddi_dma_cookie_t *);
...
int
ddi_dma_curwin(ddi_dma_handle_t handle, off_t *offp, u_int *lenp);
...
/*
 * Post-transfer implementation DMA error gathering
 *
 * When DMA to an object has completed, there are
 * sometimes implementation dependent steps that need
 * to be done in order to clean up after dma and check
 * for implementation dependent errors. The following
 * functional interface provides the means by which
 * the owner of DMA handle may do this.
 *
 * Semantics
 *
 *	The requestor guarantees that DMA from it's device is
 *	now inactive with respect to the object described.
 *
 *	The implementation returns DDI_FAILURE if a fatal error had occurred
 *	and fills in the area pointed to by errblk, if errblk is not
 *	NULL, up to the amount specified by len, else if no fatal
 *	error occurred, the implementation returns DDI_SUCCESS. The error
 *	information latched up by fatal errors stays latched up until
 *	a call to this function is made to retrieve a fatal transfer
 *	error for the DMA object referred to by handle, or until the
 *	handle is freed by a call to ddi_dma_free(). It is up to each
 *	implementation to decide how to deal with multiple errors
 *	occurring before being reported.
 *
 * Usage
 *
 *	For *all* architectures, after completion of DMA activity
 *	(whether noted via an interrupt or via polling some hardware
 *	control register bit), this function should be called by
 *	the driver that 'owns' the DMA engine that is the source
 *	of the DMA master cycles. If there are child devices of
 *	this device that need to be informed of an error condition
 *	then it is up to each specific implementation to define
 *	the protocol by which the child device's device drivers
 *	are informed of the error condition by the parent. That is
 *	to say that such protocols are *not* defined by the Generic
 *	Sun DDI (but will be defined by any addition specific
 *	implementation DDI-s (e.g., SCSA)).
 *
 */

int
ddi_dma_get_error(ddi_dma_handle_t handle, u_int length, caddr_t errblk);
 ...
 * perform the synchronization. A value of (u_int) -1 means that the length
...
int
ddi_dma_sync(ddi_dma_handle_t handle, off_t offset, u_int len, u_int flags);
...
int
ddi_iopb_alloc(dev_info_t *dip, ddi_dma_lim_t *limits, u_int length,
	caddr_t *iopbp);
 ...
 * a halkword (2 byte) transfers would be returned as 0x2 (e.g., a device
 * on a 16 data bit VME bus implementation).
 */
...
ddi_dma_addr_bind_handle(ddi_dma_handle_t handle, struct as *as,
	caddr_t addr, uint_t len, uint_t flags,
	int (*waitfp)(caddr_t), caddr_t arg,
...
int
ddi_dma_set_sbus64(ddi_dma_handle_t handle, uint_t burstsizes);
...
int
ddi_dev_regsize(dev_info_t *dev, u_int rnumber, off_t *resultp);
...
extern int ddi_copyin(caddr_t, caddr_t, size_t, int);
extern int ddi_copyout(caddr_t, caddr_t, size_t, int);
...
extern int ddi_shared_reg_fetch();
extern void ddi_shared_reg_rele();
...
extern caddr_t ddi_get_name();
extern int ddi_get_version();
...
/*
 * Redefines for commonly used named spls in the drivers.
 *
 * SUNDDI_IMPL is used so that implementation files can include this
 * file and still call the real spl routines.
 */
#if defined(sparc) || defined(__sparc)
#ifndef SUNDDI_IMPL
#define	splaudio()	i_ddi_splaudio()
#define	splhigh()	i_ddi_splhigh()
#define	splclock()	i_ddi_splclock()
#define	spltty()	i_ddi_spltty()
#define	splbio()	i_ddi_splbio()
#define	splimp()	i_ddi_splimp()
#define	splnet()	i_ddi_splnet()
#ifdef splstr
#undef splstr
#endif /* splstr */
#define	splstr()	i_ddi_splstr()
#define	splx(s)		i_ddi_splx(s)
#else
#ifdef	__STDC__
extern int splx(int);
extern int splr(int);
...
ddi_device_zero(ddi_acc_handle_t handle, caddr_t dev_addr,
	size_t bytecount, long dev_advcnt, ulong_t dev_datasz);
