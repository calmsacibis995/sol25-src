...
#pragma ident	"@(#)fifonode.h	1.28	95/03/03 SMI"	/* SVr4.0 1.15	*/
...
typedef struct fifolock {
	...
	uint		flk_fill[4];	/* cache align lock structure */
} fifolock_t;
...
struct fifonode {
	...
	ulong		fn_ino;		/* node id for pipes */
	...
	uint		fn_count;	/* Number of bytes on fn_mp */
	...
	ushort		fn_wcnt;	/* number of writers */
	ushort		fn_rcnt;	/* number of readers */
	ushort		fn_open;	/* open count of node */
	ushort		fn_wsynccnt;	/* fifos waiting for open write sync */
	ushort		fn_rsynccnt;	/* fifos waiting for open read sync */
	ushort		fn_fill;
	...
	uint		fn_filler[2];	/* 32 byte cache align */
};
...
extern int	fifo_stropen(vnode_t **, int, cred_t *, int);
