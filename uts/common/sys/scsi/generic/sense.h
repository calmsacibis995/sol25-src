...
#pragma ident	"@(#)sense.h	1.10	94/02/01 SMI"
...
struct scsi_sense {
	...
	u_char	ns_code		: 4,	/* Vendor Uniqe error code 	*/
		ns_class	: 3,	/* Error class 			*/
		ns_valid	: 1;	/* Logical Block Address is val */
	u_char	ns_lba_hi	: 5,	/* High Logical Block Address */
...
#if defined(_BIT_FIELDS_LTOH)
	u_char	es_code		: 4,	/* Vendor Unique error code 	*/
		es_class	: 3,	/* Error Class- fixed at 0x7 	*/
		es_valid	: 1;	/* sense data is valid 		*/

	u_char	es_segnum;		/* segment number: for COPY cmd */

	u_char	es_key		: 4,	/* Sense key (see below) 	*/
				: 1,	/* reserved 			*/
