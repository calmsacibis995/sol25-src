 ...
 * All rights reserved.
 */
#pragma ident	"@(#)cachefs_log.c 1.12	94/10/03 SMI"
...
static void cachefs_log_enqueue(cachefscache_t *, void *, size_t, xdrproc_t);
...
cachefs_log_kstat_snapshot(kstat_t *ksp, void *buf, int rw)
{
	...
	cachefscache_t *cachep = lc->lc_cachep;
	...
	if (rw != KSTAT_WRITE) {
		bcopy((caddr_t) lc, (caddr_t) buflc, sizeof (*lc));
		return (0);
	}
	 ...
	 */
	bcopy((caddr_t) buflc, (caddr_t) lc, sizeof (*lc));
...
cachefs_log_create_cookie(cachefs_log_control_t *lc)
{
	...
	rc = (cachefs_log_cookie_t *)
	    cachefs_kmem_zalloc(sizeof (*rc), KM_NOSLEEP);
	if (rc == NULL)
		return (NULL);
...
cachefs_log_destroy_cookie(cachefs_log_cookie_t *cl)
{
	...
	node = cl->cl_head;
	while (node != NULL) {
		cachefs_kmem_free((caddr_t) node->data, node->size);
		oldnode = node;
		node = node->next;
		cachefs_kmem_free((caddr_t) oldnode, sizeof (*oldnode));
	}
	if (cl->cl_logvp != NULL)
		VN_RELE(cl->cl_logvp);
	cachefs_kmem_free((caddr_t) cl, sizeof (*cl));
}
...
cachefs_log_logfile_open(cachefscache_t *cachep, char *path)
{
	...
	if (newvp->v_op == cachefs_getvnodeops()) {
		error = EINVAL;
		goto out;
	}
	...
	for (i = 0; i < cachefs_kstat_key_n; i++) {
		...
		vfsp = (struct vfs *) k->ks_vfsp;
		fscp = VFS_TO_FSCACHE(vfsp);
		cachefs_log_mount(cachep, 0, vfsp, &fscp->fs_options,
		    k->ks_mountpoint, UIO_SYSSPACE, k->ks_cacheid);
	}
...
cachefs_log_error(cachefscache_t *cachep, int error, int getlock)
{
	...
	if ((cl != NULL) && (cl->cl_logvp != NULL))
		(void) cachefs_log_write_header(cl->cl_logvp, cachep, error);

	cachep->c_log = NULL;
	if (cl != NULL)
		cachefs_log_destroy_cookie(cl);
	bzero((caddr_t) lc, sizeof (cachefs_log_control_t));
	lc->lc_magic = CACHEFS_LOG_MAGIC;
	lc->lc_cachep = cachep;
	(void) VOP_REMOVE(cachep->c_dirvp, LOG_STATUS_NAME, kcred);
...
static void
cachefs_log_enqueue(cachefscache_t *cachep, void *record, size_t size,
    xdrproc_t translate)
{
	...
	if (cl == NULL) { /* someone turned off logging out from under us */
		mutex_exit(&cachep->c_log_mutex);
		cachefs_kmem_free((caddr_t) record, size);
		return;
	}
	ASSERT(cl->cl_magic == CACHEFS_LOG_MAGIC);

	cl->cl_size += size;
	newnode = (cachefs_log_work_list_t)
	    cachefs_kmem_zalloc(sizeof (*newnode), KM_NOSLEEP);
	if ((cl->cl_size > CACHEFS_LOG_MAX_BUFFERED) || (newnode == NULL)) {
		cachefs_log_error(cachep, ENOMEM, 0);
		if (newnode != NULL)
			cachefs_kmem_free((caddr_t) newnode,
			    sizeof (*newnode));
		cachefs_kmem_free((caddr_t) record, size);
		mutex_exit(&cachep->c_log_mutex);
		return;
...
cachefs_log_process_queue(cachefscache_t *cachep, int getlock)
{
	...
	for (work = workhead; work != NULL; work = work->next) {
		...
		error = VOP_WRITE(logvp, &uio, FAPPEND, kcred);
...
out:
	...
	work = workhead;
	while (work != NULL) {
		cachefs_kmem_free((caddr_t) work->data, work->size);
		oldwork = work;
		work = work->next;
		cachefs_kmem_free((caddr_t) oldwork, sizeof (*oldwork));
	}
	if (logvp != NULL)
		VN_RELE(logvp);
	if (error) {
...
cachefs_log_mount(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    struct cachefsoptions *optp, char *upath, enum uio_seg seg, char *cacheid)
{
	...
	size = sizeof (*record) + len1 + len2 -
	    CLPAD(cachefs_log_mount_record, path);
	record = (struct cachefs_log_mount_record *)
	    cachefs_kmem_zalloc(size, KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		goto out;
	}
	...
	record->error = errno;
	record->vfsp = (caddr_t) vfsp;

	if (optp != NULL) {
		record->flags = optp->opt_flags;
		record->popsize = optp->opt_popsize;
		record->fgsize = optp->opt_fgsize;
	}

	record->pathlen = (u_short) len1;
	record->cacheidlen = (u_short) len2;
	if (path != NULL)
...
void
cachefs_log_umount(cachefscache_t *cachep, int errno, struct vfs *vfs)
{
	struct cachefs_log_umount_record *record;

	record = (struct cachefs_log_umount_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
	}
	...
	record->error = errno;
	record->vfsp = (caddr_t) vfs;

	cachefs_log_enqueue(cachep, record, sizeof (*record),
	    cachefs_xdr_umount);
}
...
cachefs_log_getpage(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, long uid, u_int offset, u_int len)
{
	struct cachefs_log_getpage_record *record;

	record = (struct cachefs_log_getpage_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_xdr_getpage(XDR *xdrs, struct cachefs_log_getpage_record *rec)
{
	if ((! xdr_int(xdrs, &rec->type)) ||
	    ...
	    (! xdr_u_long(xdrs, &rec->fileno)) ||
	    (! xdr_long(xdrs, &rec->uid)) ||
	    (! xdr_u_int(xdrs, &rec->offset)) ||
...
cachefs_log_readdir(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, long uid, off_t offset, int eof)
{
	struct cachefs_log_readdir_record *record;

	record = (struct cachefs_log_readdir_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_readlink(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, long uid, size_t length)
{
	struct cachefs_log_readlink_record *record;

	record = (struct cachefs_log_readlink_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_remove(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, long uid)
{
	struct cachefs_log_remove_record *record;

	record = (struct cachefs_log_remove_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_rmdir(cachefscache_t *cachep, int errno,
    struct vfs *vfsp, fid_t *fidp, ino_t fileno, long uid)
{
	struct cachefs_log_rmdir_record *record;

	record = (struct cachefs_log_rmdir_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_truncate(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, long uid, size_t size)
{
	struct cachefs_log_truncate_record *record;

	record = (struct cachefs_log_truncate_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, long uid, u_int offset, u_int len)
{
	...
	record = (struct cachefs_log_putpage_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_create(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *filefidp, ino_t fileno, long uid)
{
	struct cachefs_log_create_record *record;

	record = (struct cachefs_log_create_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_mkdir(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *cfidp, ino_t fileno, long uid)
{
	struct cachefs_log_mkdir_record *record;
	size_t size;

	size = sizeof (*record);
	record = (struct cachefs_log_mkdir_record *)
	    cachefs_kmem_zalloc(size, KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_rename(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *gfp, ino_t fileno, int removed, long uid)
{
	struct cachefs_log_rename_record *record;

	record = (struct cachefs_log_rename_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, long uid, size_t size)
{
	...
	record = (struct cachefs_log_symlink_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_populate(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, u_int off, int popsize)
{
	struct cachefs_log_populate_record *record;

	record = (struct cachefs_log_populate_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_csymlink(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, int size)
{
	struct cachefs_log_csymlink_record *record;

	record = (struct cachefs_log_csymlink_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, int size)
{
	...
	record = (struct cachefs_log_filldir_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_mdcreate(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, u_int count)
{
	struct cachefs_log_mdcreate_record *record;

	record = (struct cachefs_log_mdcreate_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, long uid, u_int offset, u_int len)
{
	...
	record = (struct cachefs_log_gpfront_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, long uid)
{
	...
	record = (struct cachefs_log_rfdir_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_ualloc(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno, u_int off, u_int len)
{
	struct cachefs_log_ualloc_record *record;

	record = (struct cachefs_log_ualloc_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
    ...
    fid_t *fidp, ino_t fileno, u_int off, u_int len)
{
	...
	record = (struct cachefs_log_calloc_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
...
cachefs_log_nocache(cachefscache_t *cachep, int errno, struct vfs *vfsp,
    fid_t *fidp, ino_t fileno)
{
	struct cachefs_log_nocache_record *record;

	record = (struct cachefs_log_nocache_record *)
	    cachefs_kmem_zalloc(sizeof (*record), KM_NOSLEEP);
	if (record == NULL) {
		cachefs_log_error(cachep, ENOMEM, 1);
		return;
