 ...
 *
 */
#pragma ident	"@(#)s5_inode.c	1.5	94/04/19 SMI"
...
extern clock_t lbolt;		/* XXX already in <sys/systm.h> */
	...
	int fstype;
{
	...
	mutex_init(&s5_syncbusy, "s5_syncbusy", MUTEX_DEFAULT, DEFAULT_WT);
	mutex_init(&dirbufp_lock, "s5 dir kmem freelist", MUTEX_DEFAULT,
								DEFAULT_WT);
	mutex_init(&ifree_lock, "ifree_lock", MUTEX_DEFAULT, DEFAULT_WT);
	rw_init(&icache_lock, "inode cache lock", RW_DEFAULT, DEFAULT_WT);
...
tryagain:
	...
	if (ifreeh &&
		...
		mutex_init(&ITOV(ip)->v_lock, "s5 v_lock", MUTEX_DEFAULT,
								DEFAULT_WT);
	...
	struct cred *cr;
{
	...
	if (length > oip->i_size) {
		...
		if (err == 0) {
			...
			if ((boff = blkoff(s5vfs, oip->i_size)) != 0) {
				...
				pvn_vpzero(ITOV(oip), (u_int)oip->i_size,
				    (u_int)(bsize - boff));
	 ...
	 */
	if (boff == 0) {
		(void) pvn_vplist_dirty(ITOV(oip), (u_int)length, s5_putapage,
