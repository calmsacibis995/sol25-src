...
#ident "@(#)s5_dir.c	1.7	94/04/19 SMI"
	...
	struct cred *cr;		/* credentials */
{
	...
	if ((tip->i_mode & IFMT) == IFDIR) {
		...
		if (!s5_dirempty(tip, tdp->i_number, cr) || tip->i_nlink > 2) {
			vn_vfsunlock(ITOV(tip));
 ...
 */
static int
s5_dirmakeinode(tdp, ipp, vap, op, cr)
	struct inode *tdp;
	struct inode **ipp;
	...
	struct cred *cr;
{
	 ...
	 */
	rw_exit(&ip->i_contents);
	error = fbread(ITOV(ip), (off_t)0, (u_int)ip->i_s5vfs->vfs_bsize,
	    S_OTHER, &fbp);
	if (error) {
		rw_enter(&ip->i_contents, RW_WRITER);
		return (error);
	...
	struct fbuf **fbpp;
{
	...
	err = fbread(ITOV(ip), (long)(offset & ~(ip->i_s5vfs->vfs_bmask)),
		bsize, S_OTHER, &fbp);
	if (err) {
...
dirbad(ip, how, offset)
	...
	off_t offset;
{
	cmn_err(CE_NOTE, "bad dir ino %d at offset %d: %s\n",
