...
#pragma ident	"@(#)s5_vnops.c	1.13	95/03/15 SMI"
...
int s5_rdwri(enum uio_rw rw, struct inode *ip, caddr_t base, int len,
    off_t offset, enum uio_seg seg, int *aresid, struct cred *cr);
...
static int s5_getpage_ra(struct vnode *vp, u_int off, struct seg *seg,
    caddr_t addr);
static int s5_getpage_miss(struct vnode *vp, u_int off, u_int len,
    struct seg *seg, caddr_t addr, page_t *pl[], u_int plsz,
...
static int s5_link(register struct vnode *tdvp, struct vnode *svp,
...
static int s5_readdir(struct vnode *vp, struct uio *uiop, struct cred *cr,
    int *eofp);
static int s5_symlink(register struct vnode *dvp, char *linkname,
...
static int s5_frlock(register struct vnode *vp, int cmd, struct flock *bfp,
    int flag, offset_t offset, cred_t *cr);
static int s5_space(struct vnode *vp, int cmd, struct flock *bfp, int flag,
    offset_t offset, struct cred *cr);
static int s5_getpage(struct vnode *vp, offset_t off, u_int len, u_int *protp,
    page_t *plarr[], u_int plsz, struct seg *seg, caddr_t addr,
    enum seg_rw rw, struct cred *cr);
static int s5_putpage(register struct vnode *vp, offset_t off, u_int len,
    int flags, struct cred *cr);
int s5_putapage(struct vnode *vp, page_t *pp, u_int *offp, u_int *lenp,
    int flags, struct cred *cr);
static int s5_putpages(register struct vnode *vp, offset_t off, u_int len,
    int flags, struct cred *cr);
static struct buf *s5_startio(register struct inode *ip, daddr_t bn,
    page_t *pp, u_int len, u_int pgoff, int flags, int io);
static int s5_map(struct vnode *vp, offset_t off, struct as *as,
    caddr_t *addrp, u_int len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr);
static int s5_addmap(struct vnode *vp, offset_t off, struct as *as,
    caddr_t addr, u_int len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr);
...
{
	TRACE_1(TR_FAC_S5, TR_S5_OPEN, "s5_open:vpp %x", vpp);
	return (0);
}
    ...
    struct cred *cr)
{
	...
	TRACE_1(TR_FAC_S5, TR_S5_CLOSE, "s5_close:vp %x", vp);
...
s5_read(struct vnode *vp, struct uio *uiop, int ioflag, struct cred *cr)
{
	...
	TRACE_3(TR_FAC_S5, TR_S5_READ_START,
		"s5_read_start:vp %x uiop %x ioflag %x",
		vp, uiop, ioflag);
...
out:
	...
	TRACE_2(TR_FAC_S5, TR_S5_READ_END,
		"s5_read_end:vp %x error %d", vp, error);
	return (error);
}
...
s5_write(struct vnode *vp, struct uio *uiop, int ioflag, struct cred *cr)
{
	...
	TRACE_3(TR_FAC_S5, TR_S5_WRITE_START,
		"s5_write_start:vp %x uiop %x ioflag %x",
		vp, uiop, ioflag);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_WRITE_END,
		"s5_write_end:vp %x error %d", vp, error);
	return (error);
}
 ...
 */
static int
rwip(register struct inode *ip, register struct uio *uio,
    enum uio_rw rw, int ioflag, struct cred *cr)
{
	register u_int off;
	...
	if (MANDLOCK(vp, ip->i_mode)) {
		...
		if (error != 0) {
			TRACE_2(TR_FAC_S5, TR_S5_RWIP_END,
				"s5_rwip_end:vp %x error %d", vp, error);
			return (error);
		}
	}
	type = ip->i_mode & IFMT;
	...
	if (uio->uio_offset < 0 || (uio->uio_offset + uio->uio_resid) < 0) {
		TRACE_2(TR_FAC_S5, TR_S5_RWIP_END,
			"s5_rwip_end:vp %x error %d", vp, EINVAL);
		return (EINVAL);
	}
	if (uio->uio_resid == 0) {
		TRACE_2(TR_FAC_S5, TR_S5_RWIP_END,
			"s5_rwip_end:vp %x error %d", vp, 0);
		return (0);
	}
...
/* ARGSUSED */
static int
s5_getattr(struct vnode *vp, register struct vattr *vap, int flags,
    struct cred *cr)
{
	register struct inode *ip = VTOI(vp);
	struct ulockfs *ulp;
	...
	TRACE_2(TR_FAC_S5, TR_S5_GETATTR_START,
		"s5_getattr_start:vp %x flags %x", vp, flags);
...
unlock:
	s5_lockfs_end(ulp);
out:
	TRACE_1(TR_FAC_S5, TR_S5_GETATTR_END, "s5_getattr_end:vp %x", vp);

	return (err);
}

static int
s5_setattr(register struct vnode *vp, register struct vattr *vap,
    int flags, struct cred *cr)
{
	int error = 0;
	...
	TRACE_2(TR_FAC_S5, TR_S5_SETATTR_START,
		"s5_setattr_start:vp %x flags %x", vp, flags);
	...
	if (mask & AT_SIZE) {
		...
		if (vp->v_type == VREG &&
			...
			goto update_inode;
		if (error = s5_itrunc(ip, vap->va_size, cr))
			goto update_inode;
	}
	/*
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_SETATTR_END,
		"s5_setattr_end:vp %x error %d", vp, error);
	return (error);
}
...
s5_access(struct vnode *vp, int mode, int flags, struct cred *cr)
{
	...
	TRACE_3(TR_FAC_S5, TR_S5_ACCESS_START,
		"s5_access_start:vp %x mode %x flags %x", vp, mode, flags);
	...
	TRACE_2(TR_FAC_S5, TR_S5_ACCESS_END,
		"s5_access_end:vp %x error %d", vp, error);
	return (error);
}
...
s5_readlink(struct vnode *vp, struct uio *uiop, struct cred *cr)
{
	...
	TRACE_2(TR_FAC_S5, TR_S5_READLINK_START,
		"s5_readlink_start:vp %x uiop %x", uiop, vp);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_READLINK_END,
		"s5_readlink_end:vp %x error %d", vp, error);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_FSYNC_END,
		"s5_fsync_end:vp %x error %d", vp, error);
	return (error);
}
    ...
    struct pathname *pnp, int flags, struct vnode *rdir, struct cred *cr)
{
	...
	TRACE_2(TR_FAC_S5, TR_S5_LOOKUP_START,
		"s5_lookup_start:dvp %x name %s", dvp, nm);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_LOOKUP_END,
		"s5_lookup_end:dvp %x error %d", dvp, error);
	return (error);
}
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_CREATE_END,
		"s5_create_end:dvp %x error %d", dvp, error);
	return (error);
}
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_REMOVE_END,
		"s5_remove_end:vp %x error %d", vp, error);
	return (error);
}
 ...
 */
static int
s5_link(register struct vnode *tdvp, struct vnode *svp,
    char *tnm, struct cred *cr)
{
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_LINK_END,
		"s5_link_end:tdvp %x error %d", tdvp, error);
	return (error);
}
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_RENAME_END,
		"s5_rename_end:sdvp %x error %d", sdvp, error);
	return (error);
}

/*ARGSUSED*/
static int
s5_mkdir(struct vnode *dvp, char *dirname, register struct vattr *vap,
    struct vnode **vpp, struct cred *cr)
{
	register struct inode *ip;
	struct inode *xip;
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_MKDIR_END,
		"s5_mkdir_end:dvp %x error %d", dvp, error);
	return (error);
}
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_RMDIR_END,
		"s5_rmdir_end:vp %x error %d", vp, error);
    ...
    int *eofp)
{
	...
	TRACE_2(TR_FAC_S5, TR_S5_READDIR_START,
		"s5_readdir_start:vp %x uiop %x", vp, uiop);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_READDIR_END,
		"s5_readdir_end:vp %x error %d", vp, error);
	return (error);
}

/*ARGSUSED*/
static int
s5_symlink(register struct vnode *dvp, char *linkname,
    struct vattr *vap, char *target, struct cred *cr)
{
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_SYMLINK_END,
		"s5_symlink_end:dvp %x error %d", dvp, error);
	return (error);
}
...
/* ARGSUSED */
static int
s5_frlock(register struct vnode *vp, int cmd, struct flock *bfp,
    int flag, offset_t offset, cred_t *cr)
{
	register struct inode *ip = VTOI(vp);
...
/* ARGSUSED */
static int
s5_space(struct vnode *vp, int cmd, struct flock *bfp, int flag,
    offset_t offset, struct cred *cr)
{
	int error;
	struct ulockfs *ulp;
 ...
 */
static int
s5_getpage(struct vnode *vp, offset_t off, u_int len, u_int *protp,
    page_t *plarr[], u_int plsz, struct seg *seg, caddr_t addr,
    enum seg_rw rw, struct cred *cr)
{
	struct inode 	*ip = VTOI(vp);
...
out:
	TRACE_2(TR_FAC_S5, TR_S5_GETPAGE_END,
		"s5_getpage_end:vp %x error %d", vp, err);
	return (err);
}
 ...
 */
static int
s5_getpage_miss(struct vnode *vp, u_int off, u_int len, struct seg *seg,
    caddr_t addr, page_t *pl[], u_int plsz, enum seg_rw rw, int seq)
{
	struct inode	*ip = VTOI(vp);
	int		err = 0;
	u_int		io_len;
	 ...
	 */
	if (rw == S_CREATE) {
		if ((pp = page_create(vp, off, PAGESIZE, PG_WAIT)) == NULL)
    ...
    caddr_t addr)
{
	...
	if (addr2 >= seg->s_base + seg->s_size)
		return (0);
	pp = pvn_read_kluster(vp, lfoff, seg, addr2, &io_off, &io_len,
		off & (PAGESIZE-1), PAGESIZE, 1);
	/*
	 * Some other thread has entered the page.
...
/*ARGSUSED*/
static int
s5_putpage(register struct vnode *vp, offset_t off, u_int len,
    int flags, struct cred *cr)
{
	int err = 0;
	...
	if (vp->v_flag & VNOMAP) {
		TRACE_2(TR_FAC_S5, TR_S5_PUTPAGE_END,
			"s5_putpage_end:vp %x error %d", vp, ENOSYS);
		return (ENOSYS);
	}
	...
	if (flags & B_ASYNC) {
		if (s5_delay && len &&
			...
			mutex_exit(&ip->i_tlock);
			TRACE_2(TR_FAC_S5, TR_S5_PUTPAGE_END,
				"s5_putpage_end:vp %x error %d", vp, 0);
			return (0);
		}
		/*
	...
	TRACE_2(TR_FAC_S5, TR_S5_PUTPAGE_END,
		"s5_putpage_end:vp %x error %d", vp, err);
	return (err);
}
...
/*ARGSUSED*/
static int
s5_putpages(register struct vnode *vp, offset_t off, u_int len,
    int flags, struct cred *cr)
{
	register struct inode *ip;
	register page_t *pp;
	...
	if (len == 0) {
		 ...
		 */
		err = pvn_vplist_dirty(vp, (u_int)off, s5_putapage, flags, cr);
	} else {
		/*
		...
		for (io_off = (u_int)off; io_off < eoff; io_off += io_len) {
			...
			if (pp == NULL || pvn_getdirty(pp, flags) == 0)
				...
				err = s5_putapage(vp, pp, &io_off, &io_len,
				    flags, cr);
				if (err != 0)
					break;
				/*
...
/*ARGSUSED*/
int
s5_putapage(struct vnode *vp, page_t *pp, u_int *offp, u_int *lenp,
    int flags, struct cred *cr)
{
	register struct inode *ip;
	 ...
	 */
	off = pp->p_offset & ~(bsize - 1);	/* block align it */
	lfoff = off;
	 ...
	 */
	pp = pvn_write_kluster(vp, pp, &io_off, &io_len, off, PAGESIZE, flags);
...
out:
	...
	TRACE_2(TR_FAC_S5, TR_S5_PUTAPAGE_END,
		"s5_putapage_end:vp %x error %d", vp, err);
	return (err);
}
 ...
 */
static struct buf *
s5_startio(register struct inode *ip, daddr_t bn, page_t *pp,
...
s5_map(struct vnode *vp, offset_t off, struct as *as, caddr_t *addrp,
    u_int len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr)
{
