...
#ident "@(#)s5_alloc.c	1.6	94/04/19 SMI"
	...
	struct cred *cr;
{
	...
	if (fs->s_nfree <= 0 || fs->s_nfree > NICFREE) {
		mutex_exit(&s5vfsp->vfs_lock);
		printf("Bad free count on dev = %d\n", vp->vfs_dev);
		goto nospace;
	}
	if (fs->s_tfree)
		fs->s_tfree--;
...
nospace:
	delay(5*HZ);
	cmn_err(CE_NOTE, "s5alloc: %d: file system full\n", vp->vfs_dev);
	return (ENOSPC);
}
	...
	dev_t dev;
{
	if (bn < (daddr_t)fp->s_isize || bn >= (daddr_t)fp->s_fsize) {
		cmn_err(CE_WARN, "bad block on dev = %d", dev);
		return (1);
	}
	return (0);
}
	...
	struct cred *cr;
{
	...
	rw_enter(&ip->i_contents, RW_WRITER);
	error = s5_itrunc(ip, lp->l_start, cr);
	rw_exit(&ip->i_contents);
	rw_exit(&ip->i_rwlock);
	return (error);
}
