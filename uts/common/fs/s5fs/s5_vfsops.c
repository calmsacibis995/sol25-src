...
#pragma ident	"@(#)s5_vfsops.c	1.5	94/03/31 SMI"
 ...
 * s5 vfs operations.
 */
static int s5_mount(register struct vfs *vfsp, struct vnode *mvp,
    struct mounta *uap, struct cred *cr);
static int s5_unmount(register struct vfs *vfsp, struct cred *cr);
static int s5_root(struct vfs *vfsp, struct vnode **vpp);
static int s5_statvfs(register struct vfs *vfsp, struct statvfs *sp);
...
static int
s5_mount(register struct vfs *vfsp, struct vnode *mvp,
    struct mounta *uap, struct cred *cr)
{
...
s5_mountroot(struct vfs *vfsp, enum whymountroot why)
{
	...
	struct vnode *vp;
	int ovflags;
	register struct s5vfs *s5vfsp = (struct s5vfs *)vfsp->vfs_data;
    ...
    struct cred *cr, int isroot, struct s5_args *argsp)
{
	...
	register struct filsys *fsp, *fspt;
	register struct s5vfs *s5vfsp = 0;
	...
	if (vfsp->vfs_flag & VFS_REMOUNT) {
		...
		bcopy((char *)fsp, tp->b_un.b_addr, (u_int)SBSIZE);
		bwrite(tp);
remountout:
		if (tpt)
			brelse(tpt);
	...
	bp->b_blkno = SBLOCK;
	bp->b_bcount = SBSIZE;
	bcopy((caddr_t)tp->b_un.b_addr, (caddr_t)bp->b_un.b_addr, SBSIZE);
	...
	if (vfsp->vfs_flag & VFS_RDONLY) {
		...
		tp->b_blkno = SBLOCK;
		tp->b_bcount = SBSIZE;
		bcopy((char *)fsp, tp->b_un.b_addr, SBSIZE);
		bwrite(tp);
		tp = 0;
	...
	mutex_init(&s5vfsp->vfs_lock, "fs_lock", MUTEX_DEFAULT, NULL);

	/*
	 * Initialize lockfs structure to support file system locking
	 */
	s5vfsp->vfs_ulockfs.ul_vfsp = vfsp;
	mutex_init(&s5vfsp->vfs_ulockfs.ul_lock, "s5_lockfs_lock",
	    MUTEX_DEFAULT, NULL);
	cv_init(&s5vfsp->vfs_ulockfs.ul_vnops_cnt_cv, "s5_lockfs_cv",
	    CV_DEFAULT, NULL);
 ...
 */
static int
s5_unmount(register struct vfs *vfsp, struct cred *cr)
{
 ...
 */
static int
s5_statvfs(register struct vfs *vfsp, struct statvfs *sp)
{
