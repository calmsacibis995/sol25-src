...
#pragma ident	"@(#)specvnops.c	1.87	95/01/12 SMI"
/*	From:	SVr4.0	"fs:fs/specfs/specvnops.c	1.67.5.5" */
#include <sys/types.h>
...
static int spec_ioctl(struct vnode *, int, int, int, struct cred *, int *);
...
static int spec_frlock(struct vnode *, int, struct flock *, int, offset_t,
    struct cred *);
static int spec_realvp(struct vnode *, struct vnode **);

static int spec_getpage(struct vnode *, offset_t, u_int, u_int *, page_t **,
    u_int, struct seg *, caddr_t, enum seg_rw, struct cred *);
static int spec_putapage(struct vnode *, page_t *, u_int *, u_int *, int,
	struct cred *);
static struct buf *spec_startio(struct vnode *, page_t *, u_int, u_int, int);

static int spec_getapage(struct vnode *, u_int, u_int, u_int *, page_t **,
    u_int, struct seg *, caddr_t, enum seg_rw, struct cred *);
static int spec_map(struct vnode *, offset_t, struct as *, caddr_t *, u_int,
    u_char, u_char, u_int, struct cred *);
static int spec_addmap(struct vnode *, offset_t, struct as *, caddr_t, u_int,
    u_char, u_char, u_int, struct cred *);
static int spec_delmap(struct vnode *, offset_t, struct as *, caddr_t, u_int,
...
static int spec_pageio(struct vnode *, page_t *, u_int, u_int, int, cred_t *);
...
spec_open(struct vnode **vpp, int flag, struct cred *cr)
{
	...
	case VCHR:
		...
		if (STREAMSTAB(maj)) {
			...
			if (error == 0 && oldttyp == NULL && u.u_ttyp != NULL) {
	...
	case VBLK:
		...
		if ((error == 0) && (dev != newdev))  {
			...
			if ((nops == NULL) || (nvp == NULL))  {
				error = ENXIO;
				if (nops)  {
					dev_close(newdev, flag, OTYP_BLK, cr);
					ddi_rele_driver(nmaj);
					error = ENOMEM;
				}
				break;
		 ...
		 */
		if (error == 0) {
			register long sn_size = csp->s_size;
			if (sn_size == 0 || sn_size == UNKNOWN_SIZE) {
				sn_size = bdev_size(dev);
				if (sn_size == -1 || sn_size > btodb(MAXOFF_T))
	...
	if (error != 0) {
		 ...
		 *
		 * Note that e_ddi_get_dev_info() will itself hold the
		 * unsafe_driver mutex if it needs to and that dev_get_dev_info
		 * adds an extra hold to the devops that we need to rele.
		 */
		if (csp->s_dip == NULL) {
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_OPEN,
		"specfs open:maj %d vp %x cvp %x error %d", maj,
		*vpp, csp, error);
	return (error);
}
	...
	struct cred		*cr)
{
	...
	ASSERT(maxoff != -1);	/* only streams return -1 */
	...
	do {
		...
		base = segmap_getmapflt(segkmap, blkvp, off + on, (u_int)n,
		    1, S_READ);
...
static int
spec_ioctl(struct vnode *vp, int cmd, int arg, int mode, struct cred *cr,
    int *rvalp)
{
	struct snode *sp;
	dev_t dev;
...
spec_fsync(struct vnode *vp, int syncflag, struct cred *cr)
{
	...
	if (vp->v_type == VBLK && cvp != vp && cvp->v_pages != NULL &&
		...
		(void) VOP_PUTPAGE(cvp, (offset_t)0, (u_int)0, 0, cr);
	...
	struct cred	*cr)
{
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_GETPAGE,
		"specfs getpage:vp %x off %x len %d snode %x",
		vp, off, len, sp);
	...
	case VBLK:
		...
		if ((u_int)off + len > sp->s_size + PAGEOFFSET)
			return (EFAULT);	/* beyond EOF */

		if (len <= PAGESIZE)
			err = spec_getapage(vp, (u_int)off, len, protp, pl,
			    plsz, seg, addr, rw, cr);
		else
			err = pvn_getpages(spec_getapage, vp, (u_int)off,
			    len, protp, pl, plsz, seg, addr, rw, cr);
		break;

	case VCHR:
		err = 0;
		ASSERT(len <= PAGESIZE && pl != (page_t **)0 && plsz > 1);
		pl[0] = devpage_lookup(vp, off, SE_SHARED);
		pl[1] = NULL;
		break;
	...
	struct cred	*cr)
{
	...
	struct buf *bp;
	page_t *pp, *pp2;
	u_int io_off1, io_len1;
	u_int io_off2, io_len2;
	u_int blksz, blkoff;
	...
	TRACE_3(TR_FAC_SPECFS, TR_SPECFS_GETAPAGE,
		"specfs getapage:vp %x off %x snode %x", vp, off, sp);
reread:
...
again:
	if ((pagefound = page_exists(vp, off)) == 0) {
		if (rw == S_CREATE) {
			...
			pp = pvn_read_kluster(vp, off, seg, addr, &io_off1,
	...
	if (dora && rw != S_CREATE) {
		...
		if (off2 >= size)
			...
			pp2 = pvn_read_kluster(vp, off2, seg, addr2, &io_off2,
	...
	struct cred	*cr)
{
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_PUTPAGE,
		"specfs putpage:vp %x off %x len %d snode %x",
		vp, off, len, sp);
	...
	if (len == 0) {
		 ...
		 */
		err = pvn_vplist_dirty(vp, (u_int)off, spec_putapage,
		    flags, cr);
	} else {
		register u_int eoff;
		 ...
		 */
		eoff = (u_int)off + len;
		for (io_off = (u_int)off; io_off < eoff && io_off < size;
		    io_off += io_len) {
			/*
			...
			if (pp == NULL || pvn_getdirty(pp, flags) == 0)
				...
				err = spec_putapage(vp, pp, &io_off, &io_len,
				    flags, cr);
				if (err != 0)
					break;
				/*
...
spec_putapage(
	struct vnode	*vp,
	page_t		*pp,
	u_int		*offp,		/* return value */
	u_int		*lenp,		/* return value */
	int		flags,
	struct cred	*cr)
{
	register struct snode *sp = VTOS(vp);
	 ...
	 */
	pp = pvn_write_kluster(vp, pp, &io_off, &io_len, blkoff, blksz, flags);
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_PUTAPAGE,
		"specfs putapage:vp %x off %x snode %x err %d",
		vp, *offp, sp, err);
	return (err);
}
	...
	struct cred *cred)
{
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_SEGMAP,
		"specfs segmap:dev %x as %x len %x prot %x",
		dev, as, len, prot);
	...
	struct cred *cred)
{
	...
	if (vp->v_type == VCHR) {
		dev_t dev = vp->v_rdev;
		register int (*segmap)(dev_t, off_t, struct as *,
		 ...
		 */
		segmap = devopsp[getmajor(dev)]->devo_cb_ops->cb_segmap;
		if (segmap == nodev || segmap == NULL || segmap == nulldev) {
			if (devopsp[getmajor(dev)]->devo_cb_ops->cb_mmap
			    == nodev)
				return (ENODEV);
			else
				segmap = spec_segmap;
		...
		return ((*segmap)(dev, (u_int)off, as, addrp, len, prot,
		    maxprot, flags, cred));
...
spec_addmap(
	...
	struct as *as,
	caddr_t addr,
	u_int len,		/* how many bytes to add */
	u_char prot,
	u_char maxprot,
	u_int flags,
	struct cred *cred)
...
#ifdef notdef	/* devpage support */
	if (vp->v_type == VCHR) {
		register u_int pg;

		/*
		 * For VCHR special files, allocate and reference count
		 * the devpages hung off the common vp of dev we're
		 * mapping.  (VBLK devices use seg_vn, not seg_dev, and
		 * thus should not have devpages.  seg_vn uses ordinary
		 * page structures.)
		 *
		 * The open, close, addmap and delmap(dev_close) operations are
		 * serialized through the testing and setting of
		 * SLOCKED and SWANT bits instead of holding the s_lock
		 * mutex.
		 */
		for (pg = 0; pg < npages; pg++) {
			if ((dp = devpage_lookup(vp, off, SE_SHARED)) == NULL &&
			    (dp = devpage_create(vp, off)) == 0) {

				/*
				 * Work backwards freeing what we
				 * just allocated
				 */
				cmn_err(CE_WARN, "no heap for devpages");
				while (pg-- > 0) {
					off -= PAGESIZE;
					devpage_destroy(devpage_find(vp, off));
				}
				error = ENOMEM;
				break;
				/*NOTREACHED*/
...
spec_delmap(
	...
	struct as *as,
	caddr_t addr,
	u_int len,		/* how many bytes to take away */
	u_int prot,
	u_int maxprot,
	u_int flags,
	struct cred *cred)
...
#ifdef notdef	/* devpage support */
	if (vp->v_type == VCHR) {
		register u_int offset, pg;

		/*
		 * For VCHR special files, release the associated
		 * devpages.  We *don't* have to hold the snode mutex
		 * here because devpages that exist are automatically
		 * locked shared and can't go away.
		 *
		 * [This is simply saying that we're never going to try
		 * and delete something we haven't already allocated,
		 * and the act of allocation is atomic]
		 */
		offset = (u_int)off;	/* off is an offset_t */
		for (pg = npages; pg > 0; pg--) {
			if ((dp = devpage_find(vp, offset)) == NULL) {
				cmn_err(CE_PANIC, "spec_delmap: devpage gone?");
				/*NOTREACHED*/
			} else {
				devpage_destroy(dp);
			}
