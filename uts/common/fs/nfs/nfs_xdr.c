...
#ident	"@(#)nfs_xdr.c	1.49	95/10/26 SMI"	/* SVr4.0 1.6	*/
...
static bool_t xdr_fastunion(register XDR *xdrs, enum_t *dscmp, char *unp,
...
xdr_writeargs(XDR *xdrs, struct nfswriteargs *wa)
{
	...
	if (xdrs->x_op == XDR_DECODE) {
		...
		if (ptr != NULL) {
			...
			*fhp++ = *ptr++;
			*fhp = *ptr++;
			wa->wa_begoff = IXDR_GET_U_LONG(ptr);
			wa->wa_offset = IXDR_GET_U_LONG(ptr);
			wa->wa_totcount = IXDR_GET_U_LONG(ptr);
			...
			wa->wa_count = IXDR_GET_U_LONG(ptr);
			...
			if (wa->wa_data == NULL) {
				wa->wa_data =
					(char *)kmem_alloc((u_int)wa->wa_count,
							KM_NOSLEEP);
				if (wa->wa_data == NULL)
					return (FALSE);
			}
			...
			if (ptr == NULL)
				return (xdr_opaque(xdrs, wa->wa_data,
						    (u_int)wa->wa_count));
			bcopy((char *)ptr, wa->wa_data, wa->wa_count);
			return (TRUE);
		}
	}
	...
	if (xdrs->x_op == XDR_ENCODE) {
		...
		if (ptr != NULL) {
			...
			*ptr++ = *fhp++;
			*ptr++ = *fhp;
			IXDR_PUT_U_LONG(ptr, wa->wa_begoff);
			IXDR_PUT_U_LONG(ptr, wa->wa_offset);
			IXDR_PUT_U_LONG(ptr, wa->wa_totcount);
			IXDR_PUT_U_LONG(ptr, wa->wa_count);
			if (wa->wa_count > NFS_MAXDATA)
				return (FALSE);
			ptr = XDR_INLINE(xdrs, RNDUP(wa->wa_count));
			if (ptr == NULL)
				return (xdr_opaque(xdrs, wa->wa_data,
						    (u_int)wa->wa_count));
			bcopy(wa->wa_data, (char *)ptr, wa->wa_count);
			rndup = BYTES_PER_XDR_UNIT -
				(wa->wa_count % BYTES_PER_XDR_UNIT);
			if (rndup != BYTES_PER_XDR_UNIT) {
				cptr = (char *)ptr + wa->wa_count;
				for (i = 0; i < rndup; i++)
					*cptr++ = '\0';
			}
			return (TRUE);
	...
	if (xdrs->x_op == XDR_FREE) {
		if (wa->wa_data != NULL) {
			kmem_free(wa->wa_data, (u_int)wa->wa_count);
			wa->wa_data = NULL;
		}
		return (TRUE);
	}
	...
	if (xdr_fhandle(xdrs, &wa->wa_fhandle) &&
	    ...
	    xdrmblk_getmblk(xdrs, &wa->wa_mblk, (u_int *)&wa->wa_count) :
...
xdr_fattr(XDR *xdrs, struct nfsfattr *na)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			na->na_mode = IXDR_GET_U_LONG(ptr);
			na->na_nlink = IXDR_GET_U_LONG(ptr);
			na->na_uid = IXDR_GET_U_LONG(ptr);
			na->na_gid = IXDR_GET_U_LONG(ptr);
			na->na_size = IXDR_GET_U_LONG(ptr);
			na->na_blocksize = IXDR_GET_U_LONG(ptr);
			na->na_rdev = IXDR_GET_U_LONG(ptr);
			na->na_blocks = IXDR_GET_U_LONG(ptr);
			na->na_fsid = IXDR_GET_U_LONG(ptr);
			na->na_nodeid = IXDR_GET_U_LONG(ptr);
			...
			IXDR_PUT_U_LONG(ptr, na->na_mode);
			IXDR_PUT_U_LONG(ptr, na->na_nlink);
			IXDR_PUT_U_LONG(ptr, na->na_uid);
			IXDR_PUT_U_LONG(ptr, na->na_gid);
			IXDR_PUT_U_LONG(ptr, na->na_size);
			IXDR_PUT_U_LONG(ptr, na->na_blocksize);
			IXDR_PUT_U_LONG(ptr, na->na_rdev);
			IXDR_PUT_U_LONG(ptr, na->na_blocks);
			IXDR_PUT_U_LONG(ptr, na->na_fsid);
			IXDR_PUT_U_LONG(ptr, na->na_nodeid);
...
xdr_fastfattr(XDR *xdrs, struct nfsfattr *na)
{
	...
	na->na_mode = (u_long)htonl(na->na_mode);
	na->na_nlink = (u_long)htonl(na->na_nlink);
	na->na_uid = (u_long)htonl(na->na_uid);
	na->na_gid = (u_long)htonl(na->na_gid);
	na->na_size = (u_long)htonl(na->na_size);
	na->na_blocksize = (u_long)htonl(na->na_blocksize);
	na->na_rdev = (u_long)htonl(na->na_rdev);
	na->na_blocks = (u_long)htonl(na->na_blocks);
	na->na_fsid = (u_long)htonl(na->na_fsid);
	na->na_nodeid = (u_long)htonl(na->na_nodeid);
...
xdr_readargs(XDR *xdrs, struct nfsreadargs *ra)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			*fhp++ = *ptr++;
			*fhp = *ptr++;
			ra->ra_offset = IXDR_GET_LONG(ptr);
			ra->ra_count = IXDR_GET_LONG(ptr);
			ra->ra_totcount = IXDR_GET_LONG(ptr);
		} else {
			fhp = (long *)&ra->ra_fhandle;
			*ptr++ = *fhp++;
			*ptr++ = *fhp++;
			...
			*ptr++ = *fhp++;
			*ptr++ = *fhp;
			IXDR_PUT_LONG(ptr, ra->ra_offset);
			IXDR_PUT_LONG(ptr, ra->ra_count);
			IXDR_PUT_LONG(ptr, ra->ra_totcount);
		}
...
xdr_rrok(XDR *xdrs, struct nfsrrok *rrok)
{
	...
	if (xdrs->x_op == XDR_ENCODE && xdrs->x_ops == &xdrmblk_ops &&
	    rrok->rrok_mp != NULL) {
		/* Normal server side case */
		mblk_t *mp = rrok->rrok_mp;

		mp->b_wptr += rrok->rrok_count;
		if (xdrmblk_putmblk(xdrs, mp, rrok->rrok_count) == FALSE) {
			ret = xdr_bytes(xdrs, (char **)&mp->b_datap->db_base,
			...
			return (TRUE);
	} else if (xdrs->x_op == XDR_ENCODE && rrok->rrok_mp != NULL) {
		/* Weird server side case - x_ops != &xdrmblk_ops */
		mblk_t *mp = rrok->rrok_mp;
		 ...
		 * Client side
		 * or
		 * Server side - No data to encode. This is
		 * the case where the read offset >= size.
		 * In this case, the encode succeeds, but there's no
		 * data. We still need to call xdr_bytes() to encode
		 * the zero byte count.
		 */
		return (xdr_bytes(xdrs, &rrok->rrok_data,
...
#ifdef _LITTLE_ENDIAN
static struct xdr_discrim fastattrstat_discrim[2] = {
	{ (int)NFS_OK, xdr_fastfattr },
	{ __dontcare__, NULL_xdrproc_t }
};
...
xdr_fastattrstat(XDR *xdrs, struct nfsattrstat *ns)
{
	...
	return (xdr_fastunion(xdrs, (enum_t *)&(ns->ns_status),
		(caddr_t)&(ns->ns_attr), fastattrstat_discrim, xdr_void));
}
#endif
...
xdr_srok(XDR *xdrs, struct nfssrok *srok)
{
	...
	if (xdrs->x_op == XDR_DECODE) {
		...
		if (ptr != NULL) {
#ifdef lint
			size = (u_int)IXDR_GET_U_LONG(ptr);
#else
			if (sizeof (u_int) == sizeof (u_long))
				size = (u_int)IXDR_GET_U_LONG(ptr);
			else
				size = (u_int)IXDR_GET_U_SHORT(ptr);
#endif
			srok->srok_count = (u_long)size;
			if (size > NFS_MAXPATHLEN)
				return (FALSE);
			if (size == 0)
				return (TRUE);
			if (srok->srok_data == NULL) {
				srok->srok_data = (char *)kmem_alloc(size,
								KM_NOSLEEP);
				if (srok->srok_data == NULL)
					return (FALSE);
			}
			...
			if (ptr == NULL)
				return (xdr_opaque(xdrs, srok->srok_data,
						    size));
			bcopy((char *)ptr, srok->srok_data, size);
			return (TRUE);
		}
	}
	...
	if (xdrs->x_op == XDR_ENCODE) {
		if (srok->srok_count > NFS_MAXPATHLEN)
			return (FALSE);
		size = (u_int)srok->srok_count;
		ptr = XDR_INLINE(xdrs, 1 * BYTES_PER_XDR_UNIT + RNDUP(size));
		if (ptr != NULL) {
#ifdef lint
			IXDR_PUT_U_LONG(ptr, (u_long)size);
#else
			if (sizeof (u_int) == sizeof (u_long))
				IXDR_PUT_U_LONG(ptr, (u_long)size);
			else
				IXDR_PUT_U_SHORT(ptr, (u_short)size);
#endif
			bcopy(srok->srok_data, (char *)ptr, size);
			rndup = BYTES_PER_XDR_UNIT -
	...
	if (xdrs->x_op == XDR_FREE) {
		if (srok->srok_data != NULL) {
			kmem_free(srok->srok_data, (u_int)srok->srok_count);
			srok->srok_data = NULL;
		}
		return (TRUE);
	}
...
xdr_rddirargs(XDR *xdrs, struct nfsrddirargs *rda)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			*fhp++ = *ptr++;
			*fhp = *ptr++;
			rda->rda_offset = IXDR_GET_U_LONG(ptr);
			rda->rda_count = IXDR_GET_U_LONG(ptr);
		} else {
			fhp = (long *)&rda->rda_fh;
			*ptr++ = *fhp++;
			*ptr++ = *fhp++;
			...
			*ptr++ = *fhp++;
			*ptr++ = *fhp;
			IXDR_PUT_U_LONG(ptr, rda->rda_offset);
			IXDR_PUT_U_LONG(ptr, rda->rda_count);
		}
		return (TRUE);
	}
		...
		if (ptr != NULL) {
			...
			*fhp = *ptr++;
			...
			if (nodesize == 0)
				return (TRUE);
			if (da->da_name == NULL) {
				da->da_name = (char *)kmem_alloc(nodesize,
								KM_NOSLEEP);
				if (da->da_name == NULL)
					return (FALSE);
			...
			bcopy((char *)ptr, da->da_name, size);
			da->da_name[size] = '\0';
			if (strlen(da->da_name) != size) {
				if (da->da_flags & DA_FREENAME) {
		...
		if (ptr != NULL) {
			...
			*ptr++ = *fhp;
			...
			if (sizeof (u_int) == sizeof (u_long))
				...
				IXDR_PUT_U_SHORT(ptr, (u_short)size);
#endif
			bcopy(da->da_name, (char *)ptr, size);
			rndup = BYTES_PER_XDR_UNIT -
				(size % BYTES_PER_XDR_UNIT);
			if (rndup != BYTES_PER_XDR_UNIT) {
				cptr = (char *)ptr + size;
...
#define	BUG_1147798
			...
			if ((size % BYTES_PER_XDR_UNIT) != 0) {
				...
				da->da_name = (char *)ptr;
				bcopy((char *)(ptr + 1), da->da_name, size);
				da->da_name[size] = '\0';
			}
#else
			if ((size % BYTES_PER_XDR_UNIT) != 0)
...
#ifdef _LITTLE_ENDIAN
static struct xdr_discrim fastdiropres_discrim[2] = {
	{ (int)NFS_OK, xdr_fastdrok },
	{ __dontcare__, NULL_xdrproc_t }
};
...
xdr_fastdiropres(XDR *xdrs, struct nfsdiropres *dr)
{
	...
	return (xdr_fastunion(xdrs, (enum_t *)&(dr->dr_status),
		(caddr_t)&(dr->dr_drok), fastdiropres_discrim, xdr_void));
}
#endif
...
xdr_saargs(XDR *xdrs, struct nfssaargs *argp)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			*arg = *ptr++;
			sa = &argp->saa_sa;
			sa->sa_mode = IXDR_GET_U_LONG(ptr);
			sa->sa_uid = IXDR_GET_U_LONG(ptr);
			sa->sa_gid = IXDR_GET_U_LONG(ptr);
			sa->sa_size = IXDR_GET_U_LONG(ptr);
			sa->sa_atime.tv_sec = IXDR_GET_LONG(ptr);
			sa->sa_atime.tv_usec = IXDR_GET_LONG(ptr);
			sa->sa_mtime.tv_sec = IXDR_GET_LONG(ptr);
			sa->sa_mtime.tv_usec = IXDR_GET_LONG(ptr);
		} else {
			arg = (long *)&argp->saa_fh;
			*ptr++ = *arg++;
			*ptr++ = *arg++;
			...
			*ptr++ = *arg;
			sa = &argp->saa_sa;
			IXDR_PUT_U_LONG(ptr, sa->sa_mode);
			IXDR_PUT_U_LONG(ptr, sa->sa_uid);
			IXDR_PUT_U_LONG(ptr, sa->sa_gid);
			IXDR_PUT_U_LONG(ptr, sa->sa_size);
			IXDR_PUT_LONG(ptr, sa->sa_atime.tv_sec);
			IXDR_PUT_LONG(ptr, sa->sa_atime.tv_usec);
			IXDR_PUT_LONG(ptr, sa->sa_mtime.tv_sec);
			IXDR_PUT_LONG(ptr, sa->sa_mtime.tv_usec);
		}
		return (TRUE);
	}
...
xdr_fastcreatargs(XDR *xdrs, struct nfscreatargs **argpp)
{
	...
	if (ptr != NULL) {
		...
		if (! XDR_CONTROL(xdrs, XDR_PEEK, (void *)&size)) {
			...
			nodesize = size + 1;
			if (nodesize == 0)
				(void) XDR_GETLONG(xdrs, (long *)&size);
			else {
				ptr = XDR_INLINE(xdrs, 1 * BYTES_PER_XDR_UNIT +
							RNDUP(size));
				if (ptr != NULL) {
...
xdr_fsok(XDR *xdrs, struct nfsstatfsok *fsok)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			fsok->fsok_tsize = IXDR_GET_LONG(ptr);
			fsok->fsok_bsize = IXDR_GET_LONG(ptr);
			fsok->fsok_blocks = IXDR_GET_LONG(ptr);
			fsok->fsok_bfree = IXDR_GET_LONG(ptr);
			fsok->fsok_bavail = IXDR_GET_LONG(ptr);
		} else {
			IXDR_PUT_LONG(ptr, fsok->fsok_tsize);
			IXDR_PUT_LONG(ptr, fsok->fsok_bsize);
			IXDR_PUT_LONG(ptr, fsok->fsok_blocks);
			IXDR_PUT_LONG(ptr, fsok->fsok_bfree);
			IXDR_PUT_LONG(ptr, fsok->fsok_bavail);
		}
		return (TRUE);
	}
...
#ifdef _LITTLE_ENDIAN
static struct xdr_discrim faststatfs_discrim[2] = {
	{ (int)NFS_OK, xdr_fastfsok },
	{ __dontcare__, NULL_xdrproc_t }
};
...
xdr_faststatfs(XDR *xdrs, struct nfsstatfs *fs)
{
	...
	return (xdr_fastunion(xdrs, (enum_t *)&(fs->fs_status),
		(caddr_t)&(fs->fs_fsok), faststatfs_discrim, xdr_void));
}
#endif
...
/*
 * Fast XDR a descriminated union
 ...
 */
static bool_t
xdr_fastunion(register XDR *xdrs, enum_t *dscmp, char *unp,
	const struct xdr_discrim *choices, xdrproc_t dfault)
{
