...
#pragma ident	"@(#)nfs3_xdr.c	1.13	95/08/22 SMI"
...
xdr_fattr3(register XDR *xdrs, fattr3 *objp)
{
	...
	if (!xdr_size3(xdrs, &objp->size))
		return (FALSE);
	/*
	 * Bugid 1218990 - if we get a large file, we want to
	 * prevent any operations on it at all, because they'll
	 * break.  We really can decode it, but can't use it in
	 * the NFS VFS, so we'll try to get a failure we can handle.
	 */
	if (objp->size > MAXOFF_T)
		return (FALSE);
