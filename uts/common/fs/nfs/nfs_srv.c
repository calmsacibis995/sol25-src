...
#pragma ident	"@(#)nfs_srv.c	1.15	95/08/23 SMI"
/* SVr4.0 1.21 */
...
static void	mblk_to_iov(mblk_t *, struct iovec *);
...
static kmutex_t rfs_rl_list_lock;
static char *rfs_rl_list;
	...
	struct svc_req *req, cred_t *cr)
{
	 ...
	 */
	mutex_enter(&rfs_rl_list_lock);
	rl->rl_data = (char *)kmem_fast_alloc(&rfs_rl_list, NFS_MAXPATHLEN, 4,
						KM_SLEEP);
	mutex_exit(&rfs_rl_list_lock);
...
rfs_rlfree(struct nfsrdlnres *rl)
{
	...
	if (rl->rl_data != NULL) {
		mutex_enter(&rfs_rl_list_lock);
		kmem_fast_free(&rfs_rl_list, (caddr_t)rl->rl_data);
		mutex_exit(&rfs_rl_list_lock);
	}
}

/*
 * nfsreadmap isn't really used anywhere now. Leaving the variable in
 * as suggested by the C-Team, since this won't cause a warning if some
 * sites have nfsreadmap set to 0 in the /etc/system file.
 * This should be removed in the next release.
 */
#ifndef lint
static int nfsreadmap = 0;
#endif
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	 ...
	 * driver).
	 */
	mp = allocb_wait(ra->ra_count, BPRI_MED, STR_NOSIG, &alloc_err);
	ASSERT(mp != NULL);
	ASSERT(alloc_err == 0);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if (wa->wa_data) {
		...
		uio.uio_iovcnt = 1;
		uio.uio_segflg = UIO_SYSSPACE;
		uio.uio_loffset = wa->wa_offset;
		uio.uio_resid = wa->wa_count;
		rlimit = u.u_rlimit[RLIMIT_FSIZE].rlim_cur - wa->wa_offset;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	for (lp = rfs_async_write_head; lp != NULL; lp = lp->next) {
		if (bcmp((caddr_t)&wa->wa_fhandle, (caddr_t)lp->fhp,
			    sizeof (fhandle_t)) == 0)
			break;
	}
	...
	cv_init(&nlp->cv, "rfs_async_write_cv", CV_DEFAULT, NULL);
	nlp->list = nrp;
	nlp->next = NULL;
	...
	do {
...
#endif
			niovp = (struct iovec *)
				kmem_alloc((u_int)(sizeof (*niovp) * iovcnt),
					KM_SLEEP);
		}
		/*
		...
		uio.uio_iovcnt = iovcnt;
		uio.uio_segflg = UIO_SYSSPACE;
		uio.uio_loffset = rp->wa->wa_offset;
		uio.uio_resid = count;
		rlimit = u.u_rlimit[RLIMIT_FSIZE].rlim_cur - rp->wa->wa_offset;
		if (rlimit < uio.uio_resid)
			uio.uio_resid = rlimit;
		/*
		 * The limit is checked on the client. We
		...
		if (niovp != iov) {
			kmem_free((caddr_t)niovp,
				(u_int)(sizeof (*niovp) * iovcnt));
		}
	...
	if (data_written) {
		TRACE_0(TR_FAC_NFS, TR_VOP_PUTPAGE_START,
			"vop_putpage_start:");
		error = VOP_PUTPAGE(vp, off, len, 0, cr);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if ((va.va_mode & IFMT) == IFCHR) {
		if (args->ca_sa->sa_size == (u_long)NFS_FIFO_DEV)
			...
			if ((va.va_size & 0xffff0000) == 0)
				va.va_rdev = (dev_t)nfsv2_expdev(va.va_size);
			else
...
{
	int error;
	register vnode_t *fromvp;
	register vnode_t *tovp;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	rd->rd_entries = (struct dirent *)kmem_alloc(rd->rd_bufsize, KM_SLEEP);
	...
	uio.uio_iovcnt = 1;
	uio.uio_segflg = UIO_SYSSPACE;
	uio.uio_loffset = rda->rda_offset;
...
rfs_rddirfree(struct nfsrddirres *rd)
{
	...
	if (rd->rd_entries != NULL)
		kmem_free((caddr_t)rd->rd_entries, rd->rd_bufsize);
}
...
sattr_to_vattr(struct nfssattr *sa, struct vattr *vap)
{
	 ...
	 * of the mode.
	 */
	if (sa->sa_mode != (u_long)((ushort)-1) &&
	    sa->sa_mode != (u_long)-1) {
		vap->va_mask |= AT_MODE;
		vap->va_mode = sa->sa_mode;
	}
	if (sa->sa_uid != (u_long)-1) {
		vap->va_mask |= AT_UID;
		vap->va_uid = sa->sa_uid;
	}
	if (sa->sa_gid != (u_long)-1) {
		vap->va_mask |= AT_GID;
...
vattr_to_nattr(struct vattr *vap, struct nfsfattr *na)
{
	...
	if (vap->va_uid == (unsigned short) -1)
		...
		na->na_uid = (unsigned long) NFS_UID_NOBODY;
	else
	...
	if (vap->va_gid == (unsigned short) -1)
		...
		na->na_gid = (unsigned long) NFS_GID_NOBODY;
	else
	 ...
	 * nfs_client.c.
	 */
	if (getminor(vap->va_rdev) <= 255 && getmajor(vap->va_rdev) <= 255)
...
void
map_type(struct vattr *vap, struct nfsfattr *na)
{

	if (vap->va_type == VFIFO && (vap->va_mode & VSUID) != 0) {
		na->na_type = NFSOC;
		na->na_mode &= ~IFMT;
		na->na_mode |= IFSOCK;
	}
...
acl_perm(struct vnode *vp, struct exportinfo *exi, struct vattr *va, cred_t *cr)
{
	...
	if (!error) {
		...
		if (vsa.vsa_aclcnt)
			kmem_free((caddr_t)vsa.vsa_aclentp,
...
rfs_srvrinit(void)
{
	...
	mutex_init(&rfs_async_write_lock, "rfs_async_write_lock",
		    MUTEX_DEFAULT, NULL);
	mutex_init(&rfs_rl_list_lock, "rfs_rl_list_lock",
		    MUTEX_DEFAULT, NULL);
}
