...
#pragma ident	"@(#)nfs_acl_srv.c	1.3	95/01/13 SMI"
/* SVr4.0 1.21 */
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (args->access & (ACCESS2_MODIFY|ACCESS2_EXTEND)) {
		error = VOP_ACCESS(vp, VWRITE, 0, cr);
		if (!error && !MANDLOCK(vp, va.va_mode))
			resp->resok.access |=
			    (args->access & (ACCESS2_MODIFY|ACCESS2_EXTEND));
	}
	if ((args->access & ACCESS2_DELETE) && vp->v_type == VDIR) {
		error = VOP_ACCESS(vp, VWRITE, 0, cr);
		if (!error)
