...
#pragma ident	"@(#)hsfs_node.c	1.34	95/02/27 SMI"
	...
	struct vfs *vfsp;
{
	...
	htp = (struct hstable *)kmem_alloc((u_int)size, KM_SLEEP);
	htp -> hs_vfsp = vfsp;
	...
	mutex_init(&htp->hsfree_lock, "hsfree_lock", MUTEX_DEFAULT, DEFAULT_WT);
	rw_init(&htp->hshash_lock, "hshash lock", RW_DEFAULT, DEFAULT_WT);
	...
	for (tp = &htp->hshash[HS_HPASH(hp)]; *tp != NULL;
		...
		if (*tp == hp) {
			...
			if (vp->v_pages != NULL)
				 ...
				 */
				(void) pvn_vplist_dirty(vp, 0, hsfs_putapage,
					B_INVAL, (struct cred *) NULL);
			*tp = hp->hs_hash;
			break;
		}
	}
	...
	struct vfs *vfsp;
{
	...
	if (dp->sym_link != (char *)NULL) {
		kmem_free(dp->sym_link, dp->ext_size + 1);
		dp->sym_link = (char *) NULL;
	}
	...
	struct cred	*cred;
{
	...
	size_t		cmpname_size;	/* how much memory we allocate for it */
	int		cmpnamelen;
	...
	cmpname_size = fsp->hsfs_namemax + 1;
	cmpname = (char *)kmem_alloc(cmpname_size, KM_SLEEP);
...
tryagain:
	...
	while (offset < end) {
		...
		error = fbread(dvp, (off_t) offset & MAXBMASK,
...
#define	rel_offset(offset) ((offset) & MAXBOFFSET)  /* index into cur blk */
	...
	kmem_free(cmpname, cmpname_size);
 ...
 */
int
hs_namecopy(from, to, size, flags)
	char *from;
	char *to;
	int size;
	u_long flags;
	...
	struct hs_direntry *hdp;
{
	...
	if (vp->v_type != VDIR) {
		cmn_err(CE_WARN, "hsfs_filldirent: vp (0x%x) not a directory",
			(int)vp);
		return;
	}
...
process_dirblock(fbp, offset, last_offset, nm, nmlen, fsp, dhp, dvp, vpp,
		error, is_rrip)
...
#define	PD_return(retval)	{ err = retval; goto do_ret; }
	...
	if (is_rrip) {
		...
		rrip_name_str = (char *)kmem_alloc(rrip_name_size, KM_SLEEP);
		rrip_tmp_name = (char *)kmem_alloc(rrip_name_size, KM_SLEEP);
		rrip_name_str[0] = '\0';
		rrip_tmp_name[0] = '\0';
	}
	...
	while (*offset < last_offset) {
		...
		if (!is_rrip && !HSFS_HAVE_LOWER_CASE(fsp) && *nm < *dname) {
			PD_return(WENT_PAST)
		}
