...
#pragma ident	"@(#)hsfs_vfsops.c	1.42	94/05/05 SMI"
...
static int hsfs_statvfs(struct vfs *vfsp, struct statvfs *sbp);
...
static int
hsfsinit(vswp, fstype)
	struct vfssw *vswp;
	int fstype;
{
	vswp->vsw_vfsops = &hsfs_vfsops;
	hsfsfstype = fstype;
	mutex_init(&hs_mounttab_lock, "hsfs mountab lock",
	...
	struct cred *cr;
{
	...
	module_keepcnt++;		/* XXX no lock! must be a bug */
	...
	struct cred *cr;
{
	...
	*tspp = fsp->hsfs_next;
	...
	if (fsp->hsfs_fsmnt != NULL)
		kmem_free((caddr_t)fsp->hsfs_fsmnt, strlen(fsp->hsfs_fsmnt)+1);
	...
	register struct statvfs *sbp;
{
	...
	sbp->f_blocks = fsp->hsfs_vol.vol_size;

	sbp->f_bfree = (u_long) -1;
	sbp->f_bavail = (u_long) -1;
	...
	strcpy(sbp->f_fstr, fsp->hsfs_vol.vol_id);
...
/*ARGSUSED*/
static int
hs_mountfs(vfsp, dev, path, mode, mount_flags, cr, isroot)
	struct vfs	*vfsp;
	dev_t		dev;
	char		*path;
	mode_t		mode;
	...
	int		isroot;
{
	 ...
	 */
	fsp = (struct hsfs *)kmem_alloc(sizeof (*fsp), KM_SLEEP);
	...
	fsp->hsfs_fsmnt = (char *)kmem_alloc(strlen(path)+1, KM_SLEEP);
	strcpy(fsp->hsfs_fsmnt, path);
 ...
 */
static int
hs_getmdev(fspec, flags, pdev, mode)
	char *fspec;
	int flags;
	dev_t *pdev;
	mode_t *mode;
...
static void
hs_copylabel(hvp, label)
	struct hs_volume *hvp;
	unsigned char *label;
{
	/* cdrom volid is at most 32 bytes */
	bcopy((char *)label, hvp->vol_id, 32);
	hvp->vol_id[31] = NULL;
}
...
hs_findvoldesc(dev_t rdev, int desc_sec)
{
	...
	if (e_ddi_getprop(rdev, VCHR, DDI_KERNEL_IOCTL,
		 ...
		 */
		error = cdev_ioctl(rdev, CDROMREADOFFSET, (int) &secno,
