...
#ident	"@(#)fifovnops.c	1.70	95/09/28 SMI"	/* SVr4.0 1.52	*/
/*
...
static int fifo_fastioctl(vnode_t *, int, int, int, cred_t *, int *);
static int fifo_strioctl(vnode_t *, int, int, int, cred_t *, int *);
...
fifo_open(vnode_t **vpp, int flag, cred_t *crp)
{
	 ...
	 */
	if ((flag & FREAD) && fnp->fn_rcnt++ == 0 && fnp->fn_wsynccnt != 0) {
		cv_broadcast(&fnp->fn_wait_cv);
		/*
		 * if non-blocking we can go directly to sync_done
		 * because we know at this point that there is
		 * at least 1 writer waiting for us (fn_wsynccnt > 0)
		 */
		...
		/*
		 * Just wait for sleeping writers to wake up and get going
		 */
		while (fnp->fn_wsynccnt != 0)
			if (!cv_wait_sig(&fnp->fn_wait_cv,
			    &fnp->fn_lock->flk_lock)) {
				/*
				 * call fifo_cleanup with FREAD because we
				 * haven't updated fn_wcnt yet
				 */
				fifo_cleanup(*vpp, FREAD);
	...
	if ((flag & FWRITE) && fnp->fn_wcnt++ == 0 && fnp->fn_rsynccnt != 0) {
		cv_broadcast(&fnp->fn_wait_cv);
		/*
		 * if non-blocking we can go directly to sync_done
		 * because we know at this point that there is
		 * at least 1 reader waiting for us (fn_rsynccnt > 0)
		 */
		if (flag & (FNDELAY|FNONBLOCK)) {
			goto sync_done;
		}
		/*
		 * Just wait for sleeping readers to wake up and get going
		 */
		while (fnp->fn_rsynccnt != 0)
			if (!cv_wait_sig(&fnp->fn_wait_cv,
			    &fnp->fn_lock->flk_lock)) {
				fifo_cleanup(*vpp, flag & FMASK);
				mutex_exit(&fnp->fn_lock->flk_lock);
				return (EINTR);
			}
		goto sync_done;
	...
	/*
	 * if there are no writers and we can block,
	 * increment fn_rsynccnt to indicate we are asleep and wait
	 * for first writer to wake us up
	 */
	if (flag & FREAD) {
		while (fnp->fn_wcnt == 0) {
			if (flag & (FNDELAY|FNONBLOCK))
			...
			/*
			 * if we are the last reader woken up
			 * wake up first writer
			 */
			if (--fnp->fn_rsynccnt == 0)
				cv_broadcast(&fnp->fn_wait_cv);
		}
	...
	/*
	 * if there are no readers increment fn_wsynccnt to indicate
	 * we are asleep and wait for first reader to wake us up
	 */
	if (flag & FWRITE) {
		while (fnp->fn_rcnt == 0) {
			...
			/*
			 * if we are the last writer woken up
...
sync_done:
	 ...
	 */
	mutex_exit(&fn_lock->flk_lock);
	return (fifo_stropen(vpp, flag, crp, 1));
}
...
fifo_write(vnode_t *vp, uio_t *uiop, int ioflag, cred_t *crp)
{
	...
	do  {
		...
		bp->b_rptr += ((uint)uiop->uio_iov->iov_base & 0x7);
...
static int
fifo_ioctl(vnode_t *vp, int cmd, int arg, int mode, cred_t *cr, int *rvalp)
{
	/*
	 * Just a quick check
...
static int
fifo_fastioctl(vnode_t *vp, int cmd, int arg, int mode, cred_t *cr, int *rvalp)
{
	fifonode_t	*fnp		= VTOF(vp);
	fifonode_t	*fn_dest;
	int		error		= 0;
	...
	case I_PEEK:
		...
		error = copyin((caddr_t)arg, (caddr_t)&strpeek,
			sizeof (strpeek));
		if (error)
			return (error);
		...
		error = copyout((caddr_t)&strpeek, (caddr_t)arg,
			sizeof (strpeek));
		if (error == 0)
			*rvalp = 1;
		break;
 ...
 */
static int
fifo_strioctl(vnode_t *vp, int cmd, int arg, int mode, cred_t *cr, int *rvalp)
{
	queue_t		*sd_wrq;
	fifonode_t	*fnp		= VTOF(vp);
	int		error		= 0;
...
/*
 * FIFOPOLLLOCK macro - macro is used instead of just a mutex_enter()
 * to enter the fifo lock. This is done to prevent an A<>B dead-lock
 * with the fifo_lock and ps_lock/pplock. fifo_poll() maybe called
 * with ps_lock/pplock mutex(s) held then tries to enter the fifo
 * lock mutex (A<-B) and another thread has already entered the
 * fifo lock mutex and calls pollwakeup() which tries to enter
 * ps_lock/pplock (A->B).
 *
 * The solution used here is to exit ps_lock/pplock (if need be)
 * and retry the enter of the fifo lock mutext then reenter the
 * ps_lock/pplock (if need be).
 * XXX This could be made into a more generic function provided by
 * poll !!
 */
#define	FIFOPOLLLOCK(poll_list, lock) 					\
	if (! mutex_tryenter(lock)) {					\
	pollstate_t *ps = curthread->t_pollstate;			\
	pollhead_t *php = (poll_list);					\
...
#define	FIFOPOLLUNLOCK(lock) mutex_exit(lock);
	...
	pollhead_t **phpp)
{
	...
	FIFOPOLLLOCK(&stp->sd_pollist, &fn_lock->flk_lock);
	...
	if ((fnp->fn_flag & FIFOISOPEN) == 0) {
		if (((events & (POLLIN | POLLRDNORM | POLLPRI | POLLRDBAND)) &&
		    ...
		    fnp->fn_wcnt == 0)) {
			FIFOPOLLUNLOCK(&fnp->fn_lock->flk_lock);
			*reventsp = POLLERR;
			return (0);
		}
	}
