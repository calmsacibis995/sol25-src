...
#ident	"@(#)fifosubr.c	1.58	95/09/28 SMI"	/* SVr4.0 1.38	*/
/*
 ...
 */
static void
fnode_constructor(void *buf, size_t size)
{
	fifodata_t *fdp = buf;
	fifolock_t *flp = &fdp->fifo_lock;
	fifonode_t *fnp = &fdp->fifo_fnode[0];

	mutex_init(&flp->flk_lock, "per fnode lock", MUTEX_DEFAULT, NULL);
	cv_init(&flp->flk_wait_cv, "fnode wait cv", CV_DEFAULT, NULL);
	flp->flk_ocsync = 0;
	...
	while ((char *)fnp < (char *)buf + size) {
		...
		cv_init(&fnp->fn_wait_cv, "fnode wait cv", CV_DEFAULT, NULL);

		mutex_init(&vp->v_lock, "fifo v_lock", MUTEX_DEFAULT, NULL);
		cv_init(&vp->v_cv, "fifo v_cv", CV_DEFAULT, NULL);
...
static void
fnode_destructor(void *buf, size_t size)
{
	fifodata_t *fdp = buf;
	fifolock_t *flp = &fdp->fifo_lock;
	fifonode_t *fnp = &fdp->fifo_fnode[0];
...
static void
pipe_constructor(void *buf, size_t size)
{
	fifodata_t *fdp = buf;
...
static void
pipe_destructor(void *buf, size_t size)
{
#ifdef DEBUG
	fifodata_t *fdp = buf;
	fifonode_t *fnp1 = &fdp->fifo_fnode[0];
...
fifoinit(struct vfssw *vswp, int fstype)
{
	...
	mutex_init(&ftable_lock, "fnode table lock", MUTEX_DEFAULT, DEFAULT_WT);
	mutex_init(&fino_lock, "fifo_ino lock", MUTEX_DEFAULT, DEFAULT_WT);
 ...
 */
int
fifo_stropen(vnode_t **vpp, int flag, cred_t *crp, int dotwist)
{
	int error = 0;
	vnode_t *oldvp = *vpp;
...
fifo_connld(struct vnode **vpp, int flag, cred_t *crp)
{
	...
	if ((error = fifo_stropen(&vp1, FREAD|FWRITE, filep->f_cred, 0)) != 0 ||
	    (error = fifo_stropen(&vp2, flag, filep->f_cred, 0)) != 0) {
#if DEBUG
	...
	str_mate(strvp2wq(vp1), strvp2wq(vp2));
...
out:
	VTOF(vp2)->fn_flag &= ~FIFOOPEN;
	fifo_close(vp2, flag, 1, (offset_t)0, filep->f_cred);
	VN_RELE(vp2);
	return (error);
}
...
fifo_wakewriter(fifonode_t *fn_dest, fifolock_t *fn_lock)
{
	...
	if ((fn_dflag & (FIFOHIWATW | FIFOISOPEN)) ==
		...
		if (fn_dflag & FIFOSETSIG) {
			str_sendsig(FTOV(fn_dest), S_WRNORM, 0L);
		}
...
fifo_wakereader(fifonode_t *fn_dest, fifolock_t *fn_lock)
{
	...
	if (fn_dflag & FIFOISOPEN) {
		if (fn_dflag & FIFOSETSIG) {
			str_sendsig(FTOV(fn_dest), S_INPUT, 0L);
			str_sendsig(FTOV(fn_dest), S_RDNORM, 0L);
		}
