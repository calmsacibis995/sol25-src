...
#ident	"@(#)ufs_vnops.c 2.235 95/10/20 SMI"
/* from ufs_vnops.c 2.131	90/01/08 SMI */
...
static 	int ufs_getpage_ra(struct vnode *, u_int, struct seg *, caddr_t);
static	int ufs_getpage_miss(struct vnode *, u_int, u_int, struct seg *,
		caddr_t, struct page **, u_int, enum seg_rw, int);
...
static	int ufs_ioctl(struct vnode *, int, int, int, struct cred *, int *);
...
static	int ufs_frlock(struct vnode *, int, struct flock *, int, offset_t,
			struct cred *);
static  int ufs_space(struct vnode *, int, struct flock *, int, offset_t,
		struct cred *);
static	int ufs_getpage(struct vnode *, offset_t, u_int, u_int *,
		struct page **, u_int, struct seg *, caddr_t,
		enum seg_rw, struct cred *);
static	int ufs_putpage(struct vnode *, offset_t, u_int, int, struct cred *);
static	int ufs_putpages(struct vnode *, offset_t, u_int, int, struct cred *);
static	struct buf *ufs_startio(struct inode *, daddr_t, struct page *,
			u_int, u_int, int, int);
static	int ufs_map(struct vnode *, offset_t, struct as *, caddr_t *, uint,
			u_char, u_char, uint, struct cred *);
static	int ufs_addmap(struct vnode *, offset_t, struct as *, caddr_t,  uint,
			u_char, u_char, uint, struct cred *);
static	int ufs_delmap(struct vnode *, offset_t, struct as *, caddr_t,  uint,
...
static	int ufs_pageio(struct vnode *, struct page *, u_int, u_int, int,
			struct cred *);
static	int ufs_dump(vnode_t *, caddr_t, int, int);
static	int ufs_dumpctl(vnode_t *, int);
static	daddr_t *save_dblks(struct inode *, daddr_t *, daddr_t *, int, int);
...
struct dump {
	...
	struct timeval	time;		/* time stamp for the struct */
	daddr_t 	dblk[1];	/* place holder for block info */
};
...
ufs_open(vpp, flag, cr)
	...
	struct cred *cr;
{
	TRACE_1(TR_FAC_UFS, TR_UFS_OPEN, "ufs_open:vpp %x", *vpp);
	return (0);
}
	...
	struct cred *cr;
{
	...
	TRACE_1(TR_FAC_UFS, TR_UFS_CLOSE, "ufs_close:vp %x", vp);

	/*
	 * The following should really be done in cleanlocks.
	 * See bugids: 1081856, 1090535
	 */
	if (vp->v_filocks) {
		ld.l_type = F_UNLCK;
		ld.l_whence = 0;
	...
	struct cred *cr;
{
	...
	TRACE_3(TR_FAC_UFS, TR_UFS_READ_START,
		"ufs_read_start:vp %x uiop %x ioflag %x",
		vp, uiop, ioflag);
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_READ_END,
		"ufs_read_end:vp %x error %d", vp, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	TRACE_3(TR_FAC_UFS, TR_UFS_WRITE_START,
		"ufs_write_start:vp %x uiop %x ioflag %x",
		vp, uiop, ioflag);
	 ...
	 * calls rwip
	 */
	TRANS_WRITE(ip, uiop, ioflag, error, ulp, cr);

	rw_exit(&ip->i_contents);
	if (ulp) {
		if (ioflag & (FSYNC|FDSYNC)) {
			TRANS_END_SYNC(ufsvfsp, error, TOP_WRITE_SYNC,
			    TOP_WRITE_SMALL_SIZE);
		} else {
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_WRITE_END,
		"ufs_write_end:vp %x error %d", vp, error);
	return (error);
}
...
wrip(struct inode *ip, struct uio *uio, int ioflag, struct cred *cr)
{
	...
	if (MANDLOCK(vp, ip->i_mode)) {
		...
		if (error != 0) {
			TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
				"ufs_wrip_end:vp %x error %d", vp, error);
			return (error);
		}
	}
	type = ip->i_mode & IFMT;
	...
	if (uio->uio_loffset > MAXOFF_T) {
		TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
			"ufs_wrip_end:vp %x error %d", vp, EINVAL);
		return (EFBIG);
	}
	...
	if (uio->uio_resid == 0) {
		TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
			"ufs_wrip_end:vp %x error %d", vp, 0);
		return (0);
	}
	...
	do {
		...
		if (uoff + n > ip->i_size) {
			 ...
			 */
			error = bmap_write(ip, uoff,
			    (daddr_t *)0, (int *)0, (int)(on + n),
			    mapon == 0, cr);
			if (error)
				break;
			i_size_changed = 1;
		...
		base = segmap_getmapflt(segkmap, vp, off + mapon, (u_int)n,
		    !pagecreate, S_WRITE);
 ...
 */
static int
rdip(struct inode *ip, struct uio *uio, int ioflag)
{
	u_int off;
	caddr_t base;
	int n, on, mapon;
	...
	long oresid = uio->uio_resid;
	int dofree;
	extern int pages_before_pager, freemem, lotsfree;

	vp = ITOV(ip);

	TRACE_1(TR_FAC_UFS, TR_UFS_RWIP_START,
		"ufs_rdip_start:vp %x", vp);
	...
	if (MANDLOCK(vp, ip->i_mode)) {
		...
		if (error != 0) {
			TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
				"ufs_rdip_end:vp %x error %d", vp, error);
			return (error);
		}
	}
	...
	if (uio->uio_loffset > MAXOFF_T) {
		TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
			"ufs_rdip_end:vp %x error %d", vp, EINVAL);
		return (EFBIG);
	}
	if (uio->uio_offset < 0 || (uio->uio_offset + uio->uio_resid) < 0) {
		TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
	...
	if (uio->uio_resid == 0) {
		TRACE_2(TR_FAC_UFS, TR_UFS_RWIP_END,
			"ufs_rdip_end:vp %x error %d", vp, 0);
		return (0);
	}
	...
	struct cred *cr;
{
	...
	if (rw == UIO_READ)
		return (rdip(ip, uio, ioflag));
	else
		return (wrip(ip, uio, ioflag, cr));
}
	...
	int *rvalp;
{
	...
	switch (cmd) {
		case _FIOLFS:
			...
			if (!suser(cr))
				return (EPERM);
			if (copyin((caddr_t)arg, (caddr_t)&lockfs,
				sizeof (struct lockfs)))
				return (EFAULT);
			if (lockfs.lf_comlen) {
				if (lockfs.lf_comlen > LOCKFS_MAXCOMMENTLEN)
					return (ENAMETOOLONG);
				comment = (caddr_t)kmem_alloc(lockfs.lf_comlen,
						KM_SLEEP);
			...
			if ((error = ufs_fiolfs(vp, &lockfs)) == 0) {
				lockfs.lf_comment = original_comment;
				copyout((caddr_t) &lockfs, (caddr_t)arg,
				    sizeof (struct lockfs));
			}
			return (error);
		...
		case _FIOLFSS:
			...
			if (copyout((caddr_t) &lockfs, (caddr_t)arg,
				sizeof (struct lockfs)))
				return (EFAULT);
			if (lockfs.lf_comlen &&
		...
		case _FIOAI:
			...
			error = ufs_fioai(vp, (struct fioai *)arg, cr);
		...
		case _FIOSATIME:
			...
			if (ulp) {
				trans_size = TOP_SETATTR_SIZE(VTOI(vp));
		...
		case _FIOSDIO:
			 ...
			 * set delayed-io
			 */
			return (ufs_fiosdio(vp, (u_long *)arg, cr));

		case _FIOGDIO:
			/*
			 * get delayed-io
			 */
			return (ufs_fiogdio(vp, (u_long *)arg, cr));
	...
	struct cred *cr;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_GETATTR_START,
		"ufs_getattr_start:vp %x flags %x", vp, flags);
...
out:
	TRACE_1(TR_FAC_UFS, TR_UFS_GETATTR_END, "ufs_getattr_end:vp %x", vp);
	...
	struct cred *cr;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_SETATTR_START,
		"ufs_setattr_start:vp %x flags %x", vp, flags);
	...
	if (ulp) {
		trans_size = TOP_SETATTR_SIZE(ip);
	...
	if (mask & (AT_UID|AT_GID)) {
		...
		if (mask & AT_UID) {
			...
			if (ip->i_uid == vap->va_uid)
				...
				change = ip->i_blocks;
			(void) chkdq(ip, -change, 1, cr);
			(void) chkiq(
			    (struct ufsvfs *)(ip->i_vnode.v_vfsp->vfs_data),
			    ip, (uid_t)ip->i_uid, 1, cr);
			dqrele(ip->i_dquot);
#endif
			ip->i_uid = vap->va_uid;
		}
		...
		if (mask & AT_UID) {
			...
			(void) chkiq(
			    (struct ufsvfs *) (ip->i_vnode.v_vfsp->vfs_data),
			    (struct inode *)NULL, (uid_t)ip->i_uid, 1, cr);
		}
#endif
	}
	/*
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_SETATTR_END,
		"ufs_setattr_end:vp %x error %d", vp, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	TRACE_3(TR_FAC_UFS, TR_UFS_ACCESS_START,
		"ufs_access_start:vp %x mode %x flags %x", vp, mode, flags);
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_ACCESS_END,
		"ufs_access_end:vp %x error %d", vp, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_READLINK_START,
		"ufs_readlink_start:vp %x uiop %x", uiop, vp);
...
again:
	...
	if (!fastsymlink) {
		...
		error = rwip(ip, uiop, UIO_READ, 0, cr);
		...
		if (ip->i_size <= FSL_SIZE && ip->i_size == size) {
			...
			if (error == 0) {
				...
				/*
				 * free page
				 * i_owner prevents recursive locks
				 * in ufs_putpage
				 */
...
nolockout:
	TRACE_2(TR_FAC_UFS, TR_UFS_READLINK_END,
		"ufs_readlink_end:vp %x error %d", vp, error);
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_FSYNC_END,
		"ufs_fsync_end:vp %x error %d", vp, error);
	return (error);
}
...
out:
	TRACE_3(TR_FAC_UFS, TR_UFS_LOOKUP_END,
		"ufs_lookup_end:dvp %x name %s error %d", *vpp, nm, error);
	return (error);
}
		...
		if (error) {
			...
			if (ip->i_size == 0) {
				...
				if (TRANS_ISTRANS(ufsvfsp))
					...
					rw_enter(&ip->i_rwlock, RW_WRITER);
					rw_enter(&ip->i_contents, RW_WRITER);
					(void) ufs_itrunc(ip, (u_long)0, 0, cr);
					rw_exit(&ip->i_rwlock);
				}
			}
		}
...
unlock:
	...
	if (!error && truncflag) {
		rw_enter(&ip->i_rwlock, RW_WRITER);
		(void) TRANS_ITRUNC(ip, (u_long)0, 0, cr);
		rw_exit(&ip->i_rwlock);
	}
...
out:
	TRACE_3(TR_FAC_UFS, TR_UFS_CREATE_END,
		"ufs_create_end:dvp %x name %s error %d", *vpp, name, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	TRACE_1(TR_FAC_UFS, TR_UFS_REMOVE_START,
		"ufs_remove_start:vp %x", vp);

	/*
	 * don't let the delete queue get too long
	 */
	if (ufsvfsp->vfs_delete.uq_ne > ufs_ninode)
	...
	if (ulp)
		TRANS_BEGIN_CSYNC(ufsvfsp, issync, TOP_REMOVE,
		    trans_size = TOP_REMOVE_SIZE(VTOI(vp)));
...
out:
	TRACE_3(TR_FAC_UFS, TR_UFS_REMOVE_END,
		"ufs_remove_end:vp %x name %s error %d", vp, nm, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	if (ulp)
		TRANS_BEGIN_CSYNC(ufsvfsp, issync, TOP_LINK,
		    trans_size = TOP_LINK_SIZE(VTOI(tdvp)));
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_LINK_END,
		"ufs_link_end:tdvp %x error %d", tdvp, error);
	return (error);
}

kmutex_t	rename_lock;	/* Serialize all renames in UFS */
	...
	struct cred *cr;
{
	...
	if (ulp)
		TRANS_BEGIN_CSYNC(ufsvfsp, issync, TOP_RENAME,
		    trans_size = TOP_RENAME_SIZE(sdp));
...
out:
	TRACE_5(TR_FAC_UFS, TR_UFS_RENAME_END,
		"ufs_rename_end:sdvp %x snm %s tdvp %x tnm %s error %d",
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_MKDIR_END,
		"ufs_mkdir_end:dvp %x error %d", dvp, error);
	return (error);
}
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_RMDIR_END,
		"ufs_rmdir_end:vp %x error %d", vp, error);

	return (error);
}

kmutex_t dirbufp_lock;	/* protects kmem_fast_alloc freelist for dirbufp */
	...
	int *eofp;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_READDIR_START,
		"ufs_readdir_start:vp %x uiop %x", vp, uiop);
	...
	if (offset >= ip->i_size || ip->i_nlink <= 0) {
		if (eofp)
			*eofp = 1;
		error = 0;
		goto unlock;
	...
	if (uiop->uio_segflg != UIO_SYSSPACE || uiop->uio_iovcnt != 1) {
		 ...
		 */
		fastalloc = (total_bytes_wanted == 1048);
		bufsize = total_bytes_wanted;
		if (fastalloc) {
			mutex_enter(&dirbufp_lock);
			outbuf = (caddr_t) kmem_fast_alloc(&dirbufp, bufsize, 1,
				KM_SLEEP);
			mutex_exit(&dirbufp_lock);
		} else
...
nextblk:
	 ...
	 */
	error = fbread(vp, (off_t)offset, bytes_wanted, S_OTHER, &fbp);
	...
	while (incount < bytes_wanted) {
		...
		if (idp->d_ino != 0 && offset >= uiop->uio_offset) {
			u_short this_reclen = (direntsz + idp->d_namlen + 1 +
				(NBPW-1)) & ~(NBPW-1);
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_READDIR_END,
		"ufs_readdir_end:vp %x error %d", vp, error);
	return (error);
}
	...
	struct cred *cr;		/* user credentials */
{
	...
	if (ulp)
		TRANS_BEGIN_CSYNC(ufsvfsp, issync, TOP_SYMLINK,
		    trans_size = TOP_SYMLINK_SIZE(dip));

	/* check for space - need at least 1 fragment */
	...
	if (error == 0) {
		 ...
		 * or EDQUOT.
		 */
		error = ufs_rdwri(UIO_WRITE, FWRITE, ip, target,
		    (int)strlen(target), (off_t)0, UIO_SYSSPACE, &leftover, cr);
		if (error) {
			rw_exit(&ip->i_contents);
			rw_exit(&ip->i_rwlock);
			VN_RELE(ITOV(ip));
		...
		if (ip->i_size <= FSL_SIZE) {
			...
			if (kcopy((caddr_t) target, (caddr_t) &ip->i_db[1],
			    (u_int) ip->i_size) == 0) {
				ip->i_flag |= IFASTSYMLNK;
			} else {
				int i;
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_SYMLINK_END,
		"ufs_symlink_end:dvp %x error %d", dvp, error);
	return (error);
}
	...
	struct cred *cr;
{
	...
	error = rwip(ip, &auio, rw, ioflag, cr);
	if (rw == UIO_WRITE) {
	...
	struct cred *cr;
{
	...
	if (cmd != F_FREESP)
		error =  EINVAL;
	else if ((error = convoff(vp, bfp, 0, (off_t)offset)) == 0)
...
retrylock:
	...
	if ((rw == S_WRITE || rw == S_CREATE) && (has_holes || beyond_eof)) {
		 ...
		 */
		if (dolock &&
		    (rwtype == RW_READER && !rw_tryupgrade(&ip->i_contents))) {
			rwtype = RW_WRITER;
			rw_exit(&ip->i_contents);
			goto retrylock;
		}
		 ...
		 * in rwip, not here. We are not dealing with frags
		 * in this case.
		 */

		offset = uoff & fs->fs_bmask;
		while (offset < uoff + len) {
			blk_size = (int)blksize(fs, ip, lblkno(fs, offset));
			err = bmap_write(ip, offset, &bn, &contig,
				blk_size, 0, cr);
			if (err)
				goto update_inode;
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_GETPAGE_END,
		"ufs_getpage_end:vp %x error %d", vp, err);
	return (err);
}
	...
	int 		seq;
{
	...
	if (rw == S_CREATE)
		...
		if (bn == UFS_HOLE && rw == S_WRITE) {
			return (ufs_fault(vp,
	"ufs_getpage_miss: bn == UFS_HOLE and rw == S_WRITE (vp: %d ufsvfs %d)",
				(int) vp, (int) ip->i_ufsvfs));
		}
	}
	...
	if (crpage) {
		...
		bp = ufs_startio(ip, bn, pp, io_len, 0, B_READ,
		    seg == segkmap);
	...
	caddr_t 	addr;
{
	...
	if ((pp = pvn_read_kluster(vp, io_off, seg, addr2, &io_off, &io_len,
	    io_off, contig, 1)) == NULL)
		return (0);
	...
	(void) ufs_startio(ip, bn, pp, io_len, 0, (B_READ | B_ASYNC),
	    seg == segkmap);
	...
	struct cred *cr;
{
	...
	if (((flags & B_ASYNC) == 0) &&
	    ((dontblock = curthread->t_flag & T_DONTBLOCK) == 0)) {
		curthread->t_flag |= T_DONTBLOCK;
		TRANS_BEGIN_SYNC(ufsvfsp, TOP_PUTPAGE, TOP_PUTPAGE_SIZE);
	}

	err = ufs_putpages(vp, off, len, flags, cr);

	if (((flags & B_ASYNC) == 0) && (dontblock == 0)) {
		TRANS_END_SYNC(ufsvfsp, err, TOP_PUTPAGE, TOP_PUTPAGE_SIZE);
		curthread->t_flag &= ~T_DONTBLOCK;
	}

errout:
	TRACE_2(TR_FAC_UFS, TR_UFS_PUTPAGE_END,
		"ufs_putpage_end:vp %x error %d", vp, err);
	return (err);
}
...
/*ARGSUSED*/
static int
ufs_putpages(vp, off, len, flags, cr)
	register struct vnode *vp;
	offset_t off;
	u_int len;
	int flags;
	struct cred *cr;
{
	...
	if (len == 0) {
		 ...
		 */
		err = pvn_vplist_dirty(vp, (u_int)off, ufs_putapage, flags, cr);
	} else {
		/*
		...
		for (io_off = (u_int)off; io_off < eoff; io_off += io_len) {
			...
			if (pp == NULL || pvn_getdirty(pp, flags) == 0)
				...
				err = ufs_putapage(vp, pp, &io_off, &io_len,
				    flags, cr);
				if (err != 0)
					break;
				/*
...
/*ARGSUSED*/
int
ufs_putapage(vp, pp, offp, lenp, flags, cr)
	struct vnode *vp;
	page_t *pp;
	u_int *offp, *lenp;		/* return values */
	int flags;
	struct cred *cr;
{
	register struct inode *ip;
	 ...
	 */
	off = pp->p_offset & fs->fs_bmask;		/* block align it */
	 ...
	 */
	if (io_len == 0) {
		ASSERT(pp->p_offset >= roundup(ip->i_size, PAGESIZE));
		io_len = PAGESIZE;
	}

	bp = ufs_startio(ip, bn, pp, io_len, 0, B_WRITE | flags, 0);

	/*
	 * bp == NULL means that the sync user data has been logged
	 */
	if (((flags & B_ASYNC) == 0) && bp) {
		/*
...
out:
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_PUTAPAGE_END,
		"ufs_putapage_end:vp %x error %d", vp, err);
	return (err);
}
...
/* ARGSUSED6 */
static struct buf *
ufs_startio(ip, bn, pp, len, pgoff, flags, io)
	register struct inode *ip;
	daddr_t bn;
	page_t *pp;
	u_int len;
	...
	int io;
{
	...
	TRACE_1(TR_FAC_UFS, TR_UFS_STARTIO_START,
		"ufs_startio_start:vp %x", ITOV(ip));
	...
	if (((flags & B_READ) == 0) && TRANS_ISTRANS(ufsvfsp)) {
		if ((ip->i_mode & IFMT) == IFSHAD) {
			TRANS_BUF(ufsvfsp, 0, len, bp, DT_SHAD);
			...
			TRANS_UD_BUF(ufsvfsp, islogged, 0, len, bp, DT_UD);
			if (islogged)
				bp->b_flags |= B_ASYNC;
		}
	}
	...
	TRACE_1(TR_FAC_UFS, TR_UFS_STARTIO_END,
		"ufs_startio_end:vp %x", ITOV(ip));

	/*
	 * sync user data has been logged; don't wait on it
	 */
	return ((islogged) ? NULL : bp);
}
...
out:
	TRACE_2(TR_FAC_UFS, TR_UFS_MAP_END,
		"ufs_map_end:vp %x error %d", vp, error);
	return (error);
}
	...
	cred_t *cr;
{
	...
	while (done_len < io_len) {
		ASSERT(cpp);
		if (err = bmap_read(ip, io_off + done_len, &bn, &contig))
			break;
...
ufs_dumpctl(vnode_t *vp, int free)
{
	...
	if (!free) {
		 ...
		 */
		entries = lblkno(fs, blkroundup(fs, ip->i_size));
		if ((dump_info = (struct dump *)
		    kmem_alloc(sizeof (struct dump) +
		    (entries - 1) * sizeof (daddr_t), KM_NOSLEEP)) == NULL) {
			    rw_exit(&ip->i_contents);
			    return (ENOMEM);
		}
 ...
 */
static daddr_t *
save_dblks(struct inode *ip, daddr_t *storeblk, daddr_t *dblk,
    int level, int entries)
{
	...
	for (i = 0; i < NINDIR(fs); i++) {
		if (storeblk - dump_info->dblk >= entries)
			break;
		bp = bread(ip->i_dev, fsbtodb(fs, dblk[i]), fs->fs_bsize);
	...
	struct cred *cr;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_GETSECATTR_START,
		"ufs_getsecattr_start:vp %x, vsap %x, flags %x", vp, flag);
	...
	struct cred *cr;
{
	...
	TRACE_2(TR_FAC_UFS, TR_UFS_SETSECATTR_START,
		"ufs_setsecattr_start:vp %x, vsap %x, flags %x", vp, flag);
