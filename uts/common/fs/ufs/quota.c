...
#ident	"@(#)quota.c	1.38	95/10/20 SMI"
...
loop:
	...
	"getdiskquota: dqp->dq_cnt: %d != 0 || dqp->dq_flags: 0x%x != 0 (%s)",
	...
	insque(dqp, dhp);
	mutex_exit(&dq_cachelock);
	if (dqoff(uid) >= 0 && dqoff(uid) < qip->i_size) {
		/*
	...
	register struct dquot *dqp;
{
	...
	if (dqp->dq_cnt || (dqp->dq_flags & DQ_MOD)) {
		(void) ufs_fault(dqp->dq_ufsvfsp && dqp->dq_ufsvfsp->vfs_root?
				    dqp->dq_ufsvfsp->vfs_root: NULL,
	"dqinval: dqp->dq_cnt:%d != 0 || (dqp->dq_flags:0x%x & DQ_MOD)",
				    dqp->dq_cnt, dqp->dq_flags);
		return;
	}
	/*
