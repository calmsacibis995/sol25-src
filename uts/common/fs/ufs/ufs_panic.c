...
#ident	"@(#)ufs_panic.c	1.8	94/12/01 SMI"
...
/* external references */
extern long	ufs_thread_run(struct ufs_q *);		/* in ufs_thread.c */
...
static void	ufsfx_kill_fix_failure_thread(time_t);
...
static void		 real_panic(ufs_failure_t *, char *, va_list);
...
init_failure(vnode_t *vp, char *fmt, va_list adx)
{
	...
	new = (ufs_failure_t *) kmem_zalloc(sizeof (ufs_failure_t), KM_NOSLEEP);
	if (!new) {
	...
	mutex_init(&new->uf_mutex, "ufs failure mutex", MUTEX_DEFAULT,
								    DEFAULT_WT);
...
queue_failure(ufs_failure_t *new)
{
	...
	ufs_fix.uq_ne = ufs_fix.uq_maxne = uf_stats.ufst_num_failed;
	mutex_exit(&uf_stats.ufst_mutex);
...
void
real_panic(ufs_failure_t *f, char *fmt, va_list adx)
{
...
ufsfx_init(void)
{
	...
	for (uap = &ufsd_addr_tab[0]; uap->ua_devname; uap++) {
		...
		sprintf(buf, "ufsd addr \"%s\" mutex",
						    NULLSTR(uap->ua_netid));

		mutex_init(&uap->ua_mutex, buf, MUTEX_DEFAULT, DEFAULT_WT);
	}

	mutex_init(&uf_stats.ufst_mutex, "ufs failure stats mutex",
...
trylock_time_exceeded(ufs_failure_t *f)
{
	...
	toolong = ufsfx_tune.uft_too_long + f->uf_entered_tm;
	if (time > toolong)
...
get_lockfs_status(ufs_failure_t *f, struct lockfs *lfp)
{
	...
	ASSERT(!MUTEX_HELD(&f->uf_vfsp->vfs_reflock));
	...
	ufs_failure_states_t	 s)
{
	...
	case UFA_SET:
		...
		bcopy((caddr_t) f->uf_ufsvfsp->vfs_fs->fs_fsmnt,
			    (caddr_t) f->uf_fsname, MAXMNTLEN);
...
sf_found_lock_fix_cmn(ufs_failure_t *f, ufs_failure_states_t s)
{
	 ...
	 */
	binval(f->uf_dev);
	bp = bread(f->uf_dev, SBLOCK, SBSIZE);
	if (bp) {
	...
	toolong = (gb_size == 0? 1: gb_size) * SecondsPerGig;
...
sf_term_cmn(ufs_failure_t *f, ufsa_t a, ufs_failure_states_t s)
{
	...
	case UFA_SET:
		...
		{	int need_lock_vfs;
			...
			timeout(&ufsfx_kill_fix_failure_thread,
		    (caddr_t) (ufsfx_tune.uft_short_err_period * HZ),
...
alloc_lockfs_comment(ufs_failure_t *f, struct lockfs *lfp)
{
	...
	if (f->uf_lf.lf_comment) {
		...
		bcopy((caddr_t) from, (caddr_t) f->uf_lf.lf_comment, len);
...
set_lockfs(ufs_failure_t *f, struct lockfs *lfp)
{
	...
	ASSERT(!MUTEX_HELD(&f->uf_ufsvfsp->vfs_ulockfs.ul_lock));
...
fsck_active(ufs_failure_t *f)
{
	...
	size_t		 comlen;
	const int	 PIDSTRLEN = strlen(PIDSTR);
	...
	errlocked = ULOCKFS_IS_ELOCK(ulp);
...
{
	return (xdr_u_long(xdrs, &objp->ua_fsid.fs_dev)			&&
		xdr_string(xdrs, &objp->ua_fsid.fs_name, MAXMNTLEN)	&&

		xdr_u_long(xdrs, &objp->ua_attempts));
}
...
fs_name(ufs_failure_t *f)
{
	...
	if (MUTEX_HELD(f->uf_vfs_lockp)) {
		if (f->uf_bp != f->uf_ufsvfsp->vfs_bufp) {
			HIDEOUS((": vfs_bufp mutated from 0x%lx to 0x%lx\n",
				    (long) f->uf_bp,
				    (long) f->uf_ufsvfsp->vfs_bufp));
			return ((char *) mutated_vfs_bufp);
		}
		if (f->uf_fs != f->uf_ufsvfsp->vfs_fs) {
			HIDEOUS((": vfs_bufp mutated from 0x%lx to 0x%lx\n",
				    (long) f->uf_fs,
				    (long) f->uf_ufsvfsp->vfs_fs));
...
dump_uf(ufs_failure_t *f, int i)
{
	...
	printf("\t\"%s\"\tAddr: 0x%lx\n", f->uf_panic_str, (long) f);
	printf("\tNext: 0x%lx\t\tPrev: 0x%lx\n",
		    (long) f->uf_next, (long) f->uf_prev);

	if (f->uf_orig)
		printf("\tOriginal failure: 0x%lx \"%s\"\n",
		    (long) f->uf_orig, f->uf_orig->uf_panic_str);

	printf("\tUfsvfs: 0x%lx\t\tVfs_lockp: 0x%lx\n",
		    (long) f->uf_ufsvfsp, (long) f->uf_vfs_lockp);
	printf("\tVfs_fsfxp: 0x%lx\n", (long) f->uf_vfs_ufsfxp);
	printf("\tVfs_bufp: 0x%lx", (long) f->uf_bp);
 ...
 */
static void
ufsfx_kill_fix_failure_thread(clock_t odelta)
{
	int		qlen;
	...
	if (qlen < 0) {
		...
		timeout(ufsfx_kill_fix_failure_thread, (caddr_t) delta, delta);
		MAJOR((": rescheduled"));
...
ufsd_connect(struct ufsd_addr *uap)
{
	...
	if (!(uap->ua_flags & UAFLG_CONNECTED)) {
		...
		if (!uap->ua_kn.knc_protofmly) {
			...
			uap->ua_addr.maxlen = strlen(uap->ua_symaddr);
		}
			...
			sprintf(uap->ua_symaddr, "%d.%d.%d.%d.%d.%d",
					(int) ((prefix >> 24) & 0xff),
					(int) ((prefix >> 16) & 0xff),
					(int) ((prefix >> 8) & 0xff),
					(int) (prefix & 0xff),
...
create_ufsd_client(struct ufsd_addr *uap)
{
	...
	if (!uap->ua_cl) {
		...
		if (rc == 0 && uap->ua_cl) {
			uap->ua_cl->cl_auth = authkern_create();
		} else {
			CLNT_GETERR(uap->ua_cl, &re);
			TRIVIA((
	": clnt_tli_kcreate(%s:%s) failed(%s) rc:%d, re_errno:%d, re_status:%d",
					uap->ua_netid, uap->ua_symaddr,
					err_name(rc), rc,
