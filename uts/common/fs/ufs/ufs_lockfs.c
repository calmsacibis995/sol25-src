...
#ident	"@(#)ufs_lockfs.c	1.51	95/08/29 SMI"	/* SunOS-4.1.1 */
...
ufs_unfreeze(struct ulockfs *ulp, struct lockfs *lockfsp)
{
	 ...
	 * reset lock
	 */
	bcopy((caddr_t)lockfsp, (caddr_t)&ulp->ul_lockfs,
		(u_int)sizeof (struct lockfs));

	ulp->ul_fs_lock = (1 << lockfsp->lf_lock);
out:
	if (comment && comlen != 0)
		kmem_free(comment, (size_t) comlen);
}
/*
 * ufs_quiesce
...
ufs_flush(struct vfs *vfsp)
{
	 ...
	 */
	if ((error = VOP_PUTPAGE(ufsvfsp->vfs_devvp, (offset_t) 0, 0, B_INVAL,
	    CRED())) > 0)
		saverror = error;

	(void) bflush((dev_t)vfsp->vfs_dev);
	(void) binval((dev_t)vfsp->vfs_dev);
...
{
	int		error	= 0;
	int		noidel	= (ulp->ul_flag & ULOCKFS_NOIDEL);
...
ufs_reconcile_fs(struct vfs *vfsp, struct ufsvfs *ufsvfsp, int errlck)
{
	 ...
	 */
	bp = bread(vfsp->vfs_dev, SBLOCK, SBSIZE);
	 ...
	 */
	kmem_free((caddr_t)mfs->fs_csp[0], (size_t) mfs->fs_cssize);
...
ufs_reconcile_inode(struct inode *ip, void *arg)
{
	 ...
	 */
	fs = ip->i_fs;
	bp = bread(ip->i_dev, (daddr_t)fsbtodb(fs, itod(fs, ip->i_number)),
	    (int)fs->fs_bsize);
	if (bp->b_flags & B_ERROR) {
		brelse(bp);
		return (EIO);
...
#if defined(DEBUG)
	set_icksum(ip);	/* recalculate inode checksum */
#endif /* DEBUG */
 ...
 */
int
ufs_fiolfs(struct vnode *vp, struct lockfs *lockfsp)
{
	return (ufs__fiolfs(vp, lockfsp, /* from_user */ 1));
}

/* kernel-internal interface, also used by fix-on-panic */
int
ufs__fiolfs(struct vnode *vp, struct lockfs *lockfsp, int from_user)
{
	struct ulockfs	*ulp;
	struct lockfs	lfs;
	int		error;
	 ...
	 */
	bcopy((caddr_t) &ulp->ul_lockfs, (caddr_t) &lfs,
		(u_int) sizeof (struct lockfs));

	/*
	 * Freeze the file system (pend future accesses)
	 */
	if (error = ufs_freeze(ulp, lockfsp))
		goto errexit;
	...
	if (lfs.lf_comment && lfs.lf_comlen != 0) {
		kmem_free((caddr_t) lfs.lf_comment, (size_t) lfs.lf_comlen);
		lfs.lf_comment = NULL;
		lfs.lf_comlen = 0;
	}
