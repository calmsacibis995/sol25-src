 ...
 */
#ident	"@(#)ufs_acl.c	1.40	95/10/15 SMI"
...
static int ufs_sectobuf(si_t *, caddr_t *, int *);
static int acl_count(ufs_acl_t *);
...
static int si_get(si_t **);
static int si_free(si_t **);
static int acl_get(ufs_acl_t **);
static void acl_free(ufs_acl_t **);

extern void ksort(caddr_t, int, int, int (*)());
extern int cmp2acls(aclent_t *, aclent_t *);
extern int rwip(struct inode *, uio_t *, uio_rw_t, int, cred_t *);
kmutex_t ufs_acl_freelist_lock;	/* used to lock the acl free list */

int 	acl_HW = 32 * 1024;
int 	acl_LW = 16 * 1024;
int	acls_total = 0;			/* total number of acls in system */
int	acls_free_count = 0;		/* number on free list */
ufs_acl_t *acl_free_list = NULL;
int 	si_HW = 1024;
int 	si_LW = 2048;
int	si_total = 0;			/* total number of ic_si in system */
int	si_free_count = 0;		/* number on free list */
krwlock_t si_cache_lock;		/* Protects si_cache */
int	si_cachecnt = 64;		/* # buckets in si_cache[a|i] */
si_t	*si_free_list = NULL;
si_t	**si_cachea;			/* The 'by acl' cache chains */
si_t	**si_cachei;			/* The 'by inode' cache chains */
long	si_cachehit = 0;
long	si_cachemiss = 0;

#define	SI_HASH(S)	((S) & (si_cachecnt - 1))
...
/* first check lru cache for a si to free */

	ASSERT(spp);
	mutex_enter(&ufs_acl_freelist_lock);
	if (!si_free_list) {
		mutex_exit(&ufs_acl_freelist_lock);
		sp = (si_t *) kmem_zalloc(sizeof (si_t), KM_SLEEP);
		rw_init(&sp->s_lock, "inode acl lock", RW_DEFAULT, DEFAULT_WT);
		*spp = sp;
		mutex_enter(&ufs_acl_freelist_lock);
		si_total++;
		mutex_exit(&ufs_acl_freelist_lock);
	...
	si_free_list = si_free_list->s_next;
	si_free_count--;
	mutex_exit(&ufs_acl_freelist_lock);
	sp->s_next = 0;
	sp->s_forw = 0;
...
si_free(si_t **spp)
{
	...
	if ((si_total > si_HW) ||
	    (si_total > si_LW && si_free_count > si_LW)) {
	...
	/*
	 * put the si on the free list since the free list is smaller than
	 * si_LW.
	 */
	mutex_enter(&ufs_acl_freelist_lock);
	sp->s_next = si_free_list;
	si_free_list = sp;
	si_free_count++;
	*spp = NULL;
	mutex_exit(&ufs_acl_freelist_lock);
...
int
acl_get(ufs_acl_t **aclpp)
{

/* first check lru cache for a acl to free */
	...
	if (!acl_free_list) {
		ufs_acl_t *aclp = NULL;
		mutex_exit(&ufs_acl_freelist_lock);
		aclp = (ufs_acl_t *)kmem_zalloc(sizeof (ufs_acl_t), KM_SLEEP);
		*aclpp = aclp;
		mutex_enter(&ufs_acl_freelist_lock);
		acls_total++;
		mutex_exit(&ufs_acl_freelist_lock);
	...
	ASSERT(acl_free_list);
	*aclpp = acl_free_list;
	acls_free_count--;
	acl_free_list = acl_free_list->acl_next;
	mutex_exit(&ufs_acl_freelist_lock);
...
void
acl_free(ufs_acl_t **aclpp)
{
	ufs_acl_t *aclp = *aclpp, *acl1p;

	while (aclp) {
		ASSERT(aclp);
		acl1p = aclp->acl_next;
		if ((acls_total > acl_HW) ||
		    (acls_total > acl_LW && acls_free_count > acl_LW)) {
			...
			*aclpp = NULL;
			...
			/*
			 * put the acl on the free list since the free list is
			 * smaller than acl_LW.
			 */
			mutex_enter(&ufs_acl_freelist_lock);
			aclp->acl_next = acl_free_list;
			acl_free_list = aclp;
			acls_free_count++;
			*aclpp = NULL;
			mutex_exit(&ufs_acl_freelist_lock);
...
loop:
	...
	imode = MAKEIMODE(VREG, 0);
	if ((err = ufs_ialloc(ip, ip->i_number, imode, &sip, cr)) != 0) {
		(void) ufs_si_free_mem(sp);
		return (err);
	}
	rw_enter(&sip->i_contents, RW_WRITER);
	...
	if ((err = ufs_rdwri(UIO_WRITE, FWRITE|FSYNC, sip, acldata,
	    acldatalen, 0, UIO_SYSSPACE, (int *)0, cr)) != 0) {
...
switchshadows:
	 ...
	 */
	ip->i_mode &= ~0777;
	ip->i_mode |= (sp->aowner->acl_perm & 07) << 6;		/* Owner */
	ip->i_mode |= (sp->agroup->acl_perm & 07) << 3;		/* Group */
	ip->i_mode |= (sp->aother->acl_perm & 07);		/* Other */
	if (sp->aclass.acl_ismask)
		ip->i_mode = (ip->i_mode & ~070) |
		    (((sp->aclass.acl_maskbits & 07) << 3) &
		    ip->i_mode);
	ip->i_uid = sp->aowner->acl_who;
	ip->i_gid = sp->agroup->acl_who;
 ...
 */
{
	...
	acldata	 = kmem_zalloc(sip->i_size, KM_SLEEP);
 ...
 */
{
	 ...
	 */
	if (cr->cr_uid == sp->aowner->acl_who) {
		return (MODE_CHECK(mode, (sp->aowner->acl_perm << 6)));
	}
	 ...
	 */
	if (sp->ausers)
		for (acl = sp->ausers; acl != NULL; acl = acl->acl_next) {
			if (acl->acl_who == cr->cr_uid) {
	 ...
	 */
	if (groupmember((uid_t)sp->agroup->acl_who, cr)) {
		ngroup++;
		gperm = (sp->agroup->acl_perm);
		if (!ismask)
	 ...
	 */
	if (sp->agroups)
		for (acl = sp->agroups; acl != NULL; acl = acl->acl_next)
		{
			if (groupmember(acl->acl_who, cr)) {
				ngroup++;
				gperm |= acl->acl_perm;
			}
		}
	 ...
	 */
	return (MODE_CHECK(mode, (sp->aother->acl_perm << 6)));
}
...
ufs_acl_set(struct inode *ip, vsecattr_t *vsap, int flag, cred_t *cr)
{
	 ...
	 * in the inode.
	 *
	 * XXX is this redundant with the XXX below
	 */
	if (sp->aowner)
		sp->aowner->acl_who = ip->i_uid;
	if (sp->agroup)
		sp->agroup->acl_who = ip->i_gid;
	/*
	 * if there are only the three owner/group/other then do not
	 * create a shadow inode.  If there is already a shadow with
	 * the file, remove it.
	 */
	if (sp->aowner && sp->agroup && sp->aother &&
	    sp->aclass.acl_ismask == 0 &&
	    !sp->ausers && !sp->agroups &&
	    !sp->downer && !sp->dgroup && !sp->dother &&
	    sp->dclass.acl_ismask == 0 && !sp->dusers && !sp->dgroups) {
		if (ip->i_ufs_acl)
			err = ufs_si_free(ip->i_ufs_acl, ITOV(ip)->v_vfsp, cr);
		ip->i_ufs_acl = NULL;
		ip->i_shadow = 0;
		ip->i_flag |= IMOD | IACC;
		ip->i_mode = (ip->i_smode & ~777) |
		    ((sp->aowner->acl_perm & 07) << 6) |
		    ((sp->agroup->acl_perm & 07) << 3) |
		    (sp->aother->acl_perm & 07);
	 ...
	 *
	 * XXX are the first 2 code lines redundant with the XXX above
	 */
	sp->aowner->acl_who = ip->i_uid;
	sp->agroup->acl_who = ip->i_gid;
...
static int
formacl(ufs_acl_t **aclpp, aclent_t *aclentp)
{
	ufs_acl_t *uaclp;

	if (acl_get(&uaclp) != 0)
		return (ENOMEM);
	uaclp->acl_perm = aclentp->a_perm;
	uaclp->acl_who = aclentp->a_id;
	uaclp->acl_next = *aclpp;
	*aclpp = uaclp;
	return (0);
}
...
void
formvsec(int obj_type, ufs_acl_t *aclp, aclent_t **aclentpp)
{
	for (; aclp; aclp = aclp->acl_next) {
		(*aclentpp)->a_type = obj_type;
		(*aclentpp)->a_perm = aclp->acl_perm;
		(*aclentpp)->a_id = aclp->acl_who;
		(*aclentpp)++;
	}
}
...
{
	ASSERT(!sp->s_ref && !(sp->s_flags & SI_CACHED));
	ASSERT(sp);
	ASSERT(!RW_LOCK_HELD(&sp->s_lock));
	ASSERT(sp->s_ref == 0);
	/*
...
void
acl_cpy(ufs_acl_t *saclp, ufs_acl_t *daclp)
{
	ufs_acl_t  *aclp, *prev_aclp = NULL, *aclp1;
	...
	for (aclp = saclp; aclp != NULL; aclp = aclp->acl_next) {
		(void) acl_get(&aclp1);
		ASSERT(aclp1);
		aclp1->acl_next = NULL;
		aclp1->acl_who = aclp->acl_who;
		aclp1->acl_perm = aclp->acl_perm;
		prev_aclp->acl_next = aclp1;
		prev_aclp = (ufs_acl_t *)&aclp1->acl_next;
	}
}

/*
 *	ufs_si_inherit takes a prarent acl structure (saclp) and the inode
 ...
 */
{
	 ...
	 * just return since no shadow inode is required.
	 */
	if (sp->dclass.acl_ismask == 0 && !sp->dusers && !sp->dgroups) {
		rw_exit(&sp->s_lock);
		rw_exit(&tdp->i_contents);
		return (0);
	}
	...
	ASSERT(RW_WRITE_HELD(&sp->s_lock));
	acl_cpy(sp->downer, (ufs_acl_t *)&tsp->aowner);
	acl_cpy(sp->dgroup, (ufs_acl_t *)&tsp->agroup);
	acl_cpy(sp->dother, (ufs_acl_t *)&tsp->aother);
	acl_cpy(sp->dusers, (ufs_acl_t *)&tsp->ausers);
	acl_cpy(sp->dgroups, (ufs_acl_t *)&tsp->agroups);
	...
	if ((ip->i_mode & IFMT) == IFDIR) {
		acl_cpy(sp->downer, (ufs_acl_t *)&tsp->downer);
		acl_cpy(sp->dgroup, (ufs_acl_t *)&tsp->dgroup);
		acl_cpy(sp->dother, (ufs_acl_t *)&tsp->dother);
		acl_cpy(sp->dusers, (ufs_acl_t *)&tsp->dusers);
		acl_cpy(sp->dgroups, (ufs_acl_t *)&tsp->dgroups);
...
ufs_acl_cp(si_t *sp)
{
	...
	acl_cpy(sp->aowner, (ufs_acl_t *)&dsp->aowner);
	acl_cpy(sp->agroup, (ufs_acl_t *)&dsp->agroup);
	acl_cpy(sp->aother, (ufs_acl_t *)&dsp->aother);
	acl_cpy(sp->ausers, (ufs_acl_t *)&dsp->ausers);
	acl_cpy(sp->agroups, (ufs_acl_t *)&dsp->agroups);

	dsp->aclass.acl_ismask = sp->aclass.acl_ismask;
	dsp->aclass.acl_maskbits = sp->aclass.acl_maskbits;

	acl_cpy(sp->downer, (ufs_acl_t *)&dsp->downer);
	acl_cpy(sp->dgroup, (ufs_acl_t *)&dsp->dgroup);
	acl_cpy(sp->dother, (ufs_acl_t *)&dsp->dother);
	acl_cpy(sp->dusers, (ufs_acl_t *)&dsp->dusers);
	acl_cpy(sp->dgroups, (ufs_acl_t *)&dsp->dgroups);
...
ufs_acl_setattr(struct inode *ip, struct vattr *vap, cred_t *cr)
{
	...
	if (mask & AT_MODE) {
		sp->aowner->acl_perm = (o_mode_t) (ip->i_mode & 0700) >> 6;
		sp->aother->acl_perm = (o_mode_t) (ip->i_mode & 07);
		if (sp->aclass.acl_ismask)
...
static int
acl_count(ufs_acl_t *p)
{
	ufs_acl_t	*acl;
	int		count;

	for (count = 0, acl = p; acl; acl = acl->acl_next, count++)
		;
	return (count);
}
 ...
 */
static int
ufs_sectobuf(si_t *sp, caddr_t *buf, int *len)
{
	int		acl_size;
	int		def_acl_size;
	caddr_t		buffer;
	...
	if (sp->aclass.acl_ismask) {
		...
		bufaclp->acl_perm =
		    (o_mode_t) sp->aclass.acl_maskbits;
		bufaclp++;
	}
	fsdp->fsd_size = acl_size;
...
wrtdefs:
	...
	if (sp->dclass.acl_ismask) {
		...
		bufaclp->acl_perm =
		    (o_mode_t) sp->dclass.acl_maskbits;
		bufaclp++;
	}
	fsdp->fsd_size = def_acl_size;
...
ufs_si_free(si_t *sp, struct vfs *vfsp, cred_t *cr)
{
	...
	if ((err = ufs_iget(vfsp, shadow, &sip, cr)) == 0) {
		...
		rw_enter(&sp->s_lock, RW_WRITER);
	/* Dec ref & use counts on si referenced by this ip */
	sp->s_use = usecnt;
	refcnt = --sp->s_ref;
	ASSERT(sp->s_ref >= 0 && sp->s_ref <= sp->s_use);
	rw_exit(&sp->s_lock);
...
si_cache_init(void)
{
	mutex_init(&ufs_acl_freelist_lock, "acl freelist lock", MUTEX_DEFAULT,
	    DEFAULT_WT);
	rw_init(&si_cache_lock, "acl cache lock", RW_DEFAULT, DEFAULT_WT);

	/* The 'by acl' headers */
	si_cachea = (si_t **)(kmem_zalloc(si_cachecnt * sizeof (si_t *),
	    KM_SLEEP));
	/* The 'by inode' headers */
	si_cachei = (si_t **)(kmem_zalloc(si_cachecnt * sizeof (si_t *),
	    KM_SLEEP));
 ...
 */
static int
aclcksum(ufs_acl_t *s_aclp)
{
	ufs_acl_t *aclp;
	int signature = 0;
	for (aclp = s_aclp; aclp; aclp = aclp->acl_next) {
		signature += aclp->acl_perm;
		signature += aclp->acl_who;
	}
	return (signature);
}
 ...
 */
static int
aclcmp(ufs_acl_t *aclin1p, ufs_acl_t *aclin2p)
{
	ufs_acl_t *aclp1;
	ufs_acl_t *aclp2;
	...
	for (aclp1 = aclin1p, aclp2 = aclin2p; aclp1 && aclp2;
	    aclp1 = aclp1->acl_next, aclp2 = aclp2->acl_next) {
		if (aclp1->acl_perm != aclp2->acl_perm ||
		    aclp1->acl_who != aclp2->acl_who)
			return (1);
	}
	/*
