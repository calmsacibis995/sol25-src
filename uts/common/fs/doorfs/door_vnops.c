...
#pragma ident	"@(#)door_vnops.c	1.5	95/07/20 SMI"
...
)
{
	...
	/*
	 * If we are exiting, revoke the door
	 */
	if (dp->door_target == curproc && curthread->t_proc_flag & TP_LWPEXIT) {
	...
	if (count == 2 && vp->v_count == 1 && (dp->door_flags & DOOR_UNREF)) {
		...
		if (dp->door_active == 0) {
			...
			/* do the unref later */
			dp->door_flags |= DOOR_DELAY_UNREF;
		}
		mutex_exit(&door_knob);
	}
	return (0);
...
door_getattr(struct vnode *vp, struct vattr *vap, int flags, struct cred *cr)
{
	...
	vap->va_nodeid = 0;		/* node id */
	vap->va_nlink = vp->v_count;	/* number of references to file */
	vap->va_size0 = 0;		/* file size pad (for future use) */
	...
	vap->va_nblocks = 0;		/* # of blocks allocated */
