...
#ident "@(#)cis_handlers.c	1.25	95/07/21 SMI"
 ...
 *	particular tuple handler called.
 *
 */
int
cis_tuple_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
	...
	while (co->type != (cisdata_t)CISTPL_END) {
		if (co->type == tp->type&&
			...
			(tp->type == CISTPL_FUNCE && co->subtype == subtype))) {
			    ...
			    /* XXX be nice if we could prototype this */
 ...
 *
 * We return CISTPLF_NOERROR always to indicate that this is a
 *	recognized tuple.
 */
int
cis_no_tuple_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
 ...
 */
int
cistpl_vers_1_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
 ...
 *
 * For the first four config registers we set the present flags in the
...
int
cistpl_config_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	cisdata_t tpcc_sz;
	int i, n, nrb, na, hr = 0;
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		 ...
		 *	aren't.
		 * For the first four config registers, set the appropriate
...
/*
 * cistpl_device_handler - handler for the CISTPL_DEVICE and CISTPL_DEVICE_A
 *				tuples
 *
 *	void *arg - points to a cistpl_device_t * where the
 *			information is stuffed into
 *
 * XXX - we don't handle more than once device described in the tuple
 */
int
cistpl_device_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	cisdata_t dev_id;
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		struct cistpl_device_node *cdn;

		/* XXX - fix this to look for more than one device definition */
		dt->num_devices = 1;
		cdn = &dt->cistpl_device_node[0];
 ...
 *	void *arg - points to a XXX where the information is stuffed into
 */
extern int cistpl_cftable_io_size_table[];
extern int cistpl_cftable_shift_table[];
int

cistpl_cftable_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		/*
		 * Check for and parse any miscellanious information.
		 *
		 * XXX - we throw away any extension bytes we see
		 */
		if (tpce_fs & CISTPL_CFTABLE_TPCE_FS_MISCM) {
			...
			/*
			 * We only understand the first byte, so we toss any
			 *	extension bytes that might exist.
			 */
			first = 1;
			do {
				if (first) {
					first = 0;
					misc->flags = GET_BYTE(tp);
				}
...
/*
 * cis_tuple_text - return a string describing the passed tuple
 *
 *	cistpl_callout_t *co - pointer to callout structure
 *				array to use to find this tuple
 *	cisdata_t type - tuple type code
 */
char *
cis_tuple_text(cistpl_callout_t *co, cisdata_t type, cisdata_t subtype)
{

	while (co->type != (cisdata_t)CISTPL_END) {
		if (co->type == type &&
		    ((type != CISTPL_FUNCE) ||
			(type == CISTPL_FUNCE && co->subtype == subtype)))
			break;
		co++;
	}
...
ulong_t
cistpl_expd_parse(cistpl_t *tp, int *flags)
{
	cisdata_t pdesc;
...
ulong_t
cistpl_devspeed(cistpl_t *tp, cisdata_t spindex, int flags)
{
	int scale = 1, first;
	cisdata_t exspeed;
 ...
 */
int
cistpl_vers_2_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		cs->nhdr = GET_BYTE(tp);
		cs->oem = cis_getstr(tp);
		if (GET_LEN(tp) > 0) {
			cs->info = tp->read.byte;
		} else {
			cs->info = NULL;
		}
	}
 ...
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_jedec_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
...
cistpl_format_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
...
/*
 * cistpl_geometry_handler - handler for disk geometry tuples
 *
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_geometry_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
								void *arg)
{
	/*
	 * nothing special about our flags, so just call the
...
/*
 * cistpl_byteorder_handler - handler for memory byte order tuple
 *
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_byteorder_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
								void *arg)
{
	/*
	 * nothing special about our flags, so just call the
 ...
 *			information is stuffed into
 */
cistpl_date_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
 ...
 *			information is stuffed into
 */
cistpl_battery_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
 ...
 *			information is stuffed into
 */
cistpl_org_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		RESET_TP(tp);
		cs->type = GET_BYTE(tp);
		/* need to fix string things XXX */
		cs->desc = (char *)tp->read.byte;
	}
	return (CISTPLF_NOERROR);
}
 ...
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_manfid_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
...
/*
 * cistpl_funcid_handler - handler for function ID tuple
 *
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_funcid_handler(cistpl_callout_t *co, cistpl_t *tp, int flags, void *arg)
{
	/*
	 * nothing special about our flags, so just call the
	 *	generic handler for this
...
/*
 * cistpl_funce_serial_handler - handler for function extention (serial) tuple
 *
 *	void *arg - points to a XXX where the information is stuffed into
 */
...
/*
 * cistpl_funce_lan_handler - handler for function extention (LAN) tuple
 *
 *	void *arg - points to a XXX where the information is stuffed into
 */
cistpl_funce_lan_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
								void *arg)
{
	int subfunction;
 ...
 */
cistpl_linktarget_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
								void *arg)
{
	/*
	 * nothing special about our flags, so just call the
 ...
 */
int
cistpl_longlink_ac_handler(cistpl_callout_t *co, cistpl_t *tp, int flags,
								void *arg)
{
	/*
	 * nothing special about our flags, so just call the
...
/*
 * cis_validate_longlink_ac - Validates the secondary tuple chain pointed
 *				to by cisptr and specified by a previous
 *				CISTPL_LONGLINK_A or CISTPL_LONGLINK_C
 *				tuple.
 *
 *	cisptr->base must be setup to point to the first address in the
 *		secondary tuple chain to validate
 ...
 */
int
cis_validate_longlink_ac(volatile cisptr_t *cisptr)
{
	...
	if ((cb[0] == CISTPL_LINKTARGET) && (cb[1] >= MIN_LINKTARGET_LENGTH)) {
		...
		for (tl = 0; tl < MIN_LINKTARGET_LENGTH; tl++)
...
cis_malloc(size_t len)
{
	...
	addr = (caddr_t)kmem_alloc(len+sizeof (int), KM_NOSLEEP);
