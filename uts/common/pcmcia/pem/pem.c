 ...
 * All rights reserved.
 */
#pragma ident "@(#)pem.c	1.34	95/08/04 SMI"
...
char _depends_on[] = "drv/pcmcia"; /* change to misc/pcmcia later */
extern pcmcia_set_em_handler();
 ...
 */
extern	pem_open(queue_t *, dev_t *, int, int, cred_t *);
extern	pem_close(queue_t *, int, cred_t *);
extern	pem_wput(queue_t *, mblk_t *);
static	int pem_wsrv(queue_t * q);
static	int pem_rsrv(queue_t *q);
extern  pem_attach(dev_info_t *, ddi_attach_cmd_t);
extern  pem_detach(dev_info_t *, ddi_detach_cmd_t);
...
extern  u_int pem_soft_intr(caddr_t);
...
static int pem_get_first_tuple(queue_t *, mblk_t *,
				    em_get_first_tuple_req_t *);
static int pem_get_next_tuple(queue_t *, mblk_t *,
...
DDI_DEFINE_STREAM_OPS(pem_ops, pem_identify, nulldev, pem_attach, pem_detach,
			nodev, pem_devinfo, D_MP, &pem_info);
/*
...
#ifndef BUILD_STATIC
#define	pem_init int _init
#define	pem_fini int _fini
#define	pem_info int _info
#endif

int created_pem_mutexii = 0;
...
pem_init(void)
{
	...
	if (e == 0) {
		mutex_init(&pem_global_lock, "pem global lock",
				MUTEX_DRIVER, NULL);
		created_pem_mutexii = 1;
	} else {
...
pem_fini(void)
{
	...
	if ((ret = mod_remove(&modlinkage)) == 0) {
		if (created_pem_mutexii != 0) {
			mutex_destroy(&pem_global_lock);
			created_pem_mutexii = 0;
		}
...
#ifndef BUILD_STATIC
pem_info(struct modinfo *modinfop)
{
...
pem_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		for (i = 0; i < EM_EVENT_SIZE; i++)
			events[i] = (u_char) ~0;
		i = pcmcia_set_em_handler(pem_event_handler, events,
						sizeof (events), 0x1234,
						&cardservices,
						&Socket_Services);
		if (i != 0) {
		...
		ddi_add_softintr(dip, DDI_SOFTINT_MED, &pem_intr_id, &pem_iblock,
					0,
					pem_soft_intr, (caddr_t) dip);

		mutex_init(&pem_intr_lock, "pem interrupt lock",
				MUTEX_DRIVER, pem_iblock);
		break;
...
pem_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		pcmcia_set_em_handler(NULL, NULL, 0, 0x1234, NULL);
	...
	*dev = makedevice(getmajor(*dev), minordev);
	...
	if (pem_cs_handle == 0) {
		...
		if (pem_debug && result != CS_SUCCESS)
			printf("pem: couldn't register for CS\n");
#else
	...
	mblk_t *mp;		/* message pointer */
{
	...
	default:
		...
		if (pem_debug & PEMERRS)
			printf("pem: Unexpected packet type from queue: %x\n",
				DB_TYPE(mp));
#endif
		freemsg(mp);
		break;
...
#ifdef PEM_DEBUG
			if (pem_debug & PEMERRS)
				printf("pem_wsrv: db_type(%x) not supported\n",
					mp->b_datap->db_type);
#endif
	...
	mblk_t *mp;		/* message pointer */
{
...
#ifdef PEM_DEBUG
	if (pem_debug & PEMTRACE)
		printf("pem_cmds(%x, %x):pem=(N/A), prim->em_primitive=%d\n",
			q, mp, prim->em_primitive);
#endif
	...
	case EM_MODIFY_EVENT_MASK_REQ:
		...
		break;
	case EM_GET_FIRST_TUPLE_REQ:
		result = pem_get_first_tuple(q, mp, &prim->get_first_tuple_req);
		break;
	case EM_GET_NEXT_TUPLE_REQ:
		result = pem_get_next_tuple(q, mp, &prim->get_next_tuple_req);
		break;
...
#ifdef PEM_DEBUG
		if (pem_debug & PEMERRS)
			printf("pem_cmds: unknown M_PROTO message: %d\n",
				prim->em_primitive);
#endif
		result = EM_BADPRIM;
	}
...
pem_get_first_tuple(queue_t *q, mblk_t *mp, em_get_first_tuple_req_t *treq)
{
	...
	mp = mexchange(q, mp, len, M_PCPROTO, EM_GET_FIRST_TUPLE_ACK);
	if (mp == NULL)
		return (PEME_OK);
...
/* ARGSUSED */
static int
pem_get_next_tuple(queue_t *q, mblk_t *mp, em_get_next_tuple_req_t *treq)
{
	return (0);
}
...
static void
pemprintf(str, a, b, c, d, e, f, g, h, i)
	char   *str;
{
	char	buff[256];
...
pem_event_ind(queue_t *q, long event, long socket, void *arg)
{
	...
	case PCE_DEV_IDENT:
		...
		strncpy((char *)mp->b_rptr + sizeof (em_event_ind_t),
...
pem_soft_intr(caddr_t arg)
{
	...
	do {
		i = ddi_enter_critical();
		mutex_enter(&pem_intr_lock);
		pending = pem_softint_pend;
		pem_softint_pend = 0;
		mutex_exit(&pem_intr_lock);
		ddi_exit_critical(i);
...
#if	defined(PEM_DEBUG)
	if (pem_debug)
		printf("pem_event_dispatch(%x, %x, %x, %x)\n",
			id, event, socket, arg);
#endif
	if (pem_instances == NULL)
		return;
...
#if	defined(PEM_DEBUG)
			if (pem_debug)
				printf("\tflags=%x, id=%x, wanted=%d\n",
					pem->pem_flags, pem->pem_id,
...
#if	defined(PEM_DEBUG)
	if (pem_debug)
		printf("pem_event_handler(%x, %x, %x, %x)\n",
			id, event, socket, arg);
#endif
	for (i = 0, pe = pem_events; i < PEM_MAX_EVENTS; i++, pe++) {
		if (pem_claim(pe)) {
