...
#pragma ident	"@(#)tnf_writer.c	1.12	95/03/21 SMI"
...
TNF_STD_DERIVED_TAG(tnf_self_size, tnf_size,
		tnf_derived_properties, TNF_UINT32);

TNF_STD_DERIVED_TAG(tnf_size, tnf_uint32,
		tnf_derived_properties, TNF_UINT32);
...
TNF_STD_DERIVED_TAG(tnf_type_size, tnf_size,
		tnf_derived_properties, TNF_UINT32);
...
/*ARGSUSED0*/
tnf_ref32_t
tnf_ref32_1(tnf_ops_t *ops, tnf_record_p item, tnf_reference_t reference)
{
	...
	if (((char *)dest_header_p < (file_start + TNFW_B_FW_ZONE)) ||
		...
		offset_delta = (tnf_ref32_t)(item - (tnf_record_p) reference);

		src_header_p =  (tnf_block_header_t *)
			(reference & TNF_BLOCK_MASK);
...
tnf_reference_t
tnf_string_1(tnf_ops_t *ops, const char *string, tnf_reference_t reference,
...
tnf_reference_t
tnf_string_array_1(tnf_ops_t *ops, char	**strings, tnf_reference_t reference,
			tnf_tag_data_t	*tag_data)
{
	tnf_record_p 	tag_index;
	size_t		record_size;
	...
	while (*tmp) {
		*ref_p = tnf_string(ops, *tmp, (tnf_reference_t)ref_p);
		tmp++;
		ref_p++;
	}
...
tnf_tag_element_1(tnf_ops_t *ops, tnf_tag_data_t **tag_data_p,
		tnf_reference_t reference, tnf_tag_data_t *aux_tag_data)
{
	tnf_tag_data_t	*tag_data;
		...
		tnf_tag_data_t		*tag_data)
{
	...
	ref_p = (tnf_reference_t *)((char *)bufhdr + sizeof (*bufhdr));
	while (*tmp) {
		*ref_p = tnf_tag_element_1(ops, *tmp, (tnf_reference_t)ref_p,
		    TNF_NULL);
		tmp++;
		ref_p++;
	}
