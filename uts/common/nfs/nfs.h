...
#pragma ident	"@(#)nfs.h	1.53	95/08/23 SMI"	/* SVr4.0 1.5	*/
...
#define	NFS_COTS_TIMEO	100	/* initial timeout for cots in 10th of a sec */
...
#define	nfsv2_cmpdev(x) \
	(unsigned long)((((x)>>L_BITSMINOR) > SO4_MAXMAJ || \
	    ((x)&L_MAXMIN) > SO4_MAXMIN) ? NODEV : \
	    ((((x)>>L_BITSMINOR)<<SO4_BITSMINOR)|((x)&SO4_MAXMIN)))

/* convert from over-the-wire format to SVR4 device number format */

#define	nfsv2_expdev(x) \
	(unsigned long)(((((x)>>SO4_BITSMINOR)&SO4_MAXMAJ)<<L_BITSMINOR) | \
	    ((x)&SO4_MAXMIN))
...
struct nfsrrok {
	...
	u_int		rrok_bufsize;	/* size of kmem_alloc'd buffer */
...
extern void	nfs_purge_access_cache(vnode_t *);
extern void	nfs_purge_rddir_cache(vnode_t *);
extern void	nfs_cache_check(vnode_t *, timestruc_t, timestruc_t,
				u_long, long *, cred_t *);
extern void	nfs_attrcache(vnode_t *, struct nfsfattr *, long);
extern void	nfs_attrcache_va(vnode_t *, struct vattr *, long);
...
extern int	nfs_addcllock(vnode_t *vp, struct flock *bfp);
extern void	nfs_rmcllock(vnode_t *vp, struct flock *bfp);
extern int	nfs_lockrelease(vnode_t *vp, int flag,
		    offset_t offset, cred_t *credp);
extern void	vattr_to_nattr(struct vattr *, struct nfsfattr *);
extern void	map_type(struct vattr *, struct nfsfattr *);
extern int	mount_root(struct knetconfig *, char *, struct netbuf *,
			fhandle_t *, char *, char *);
extern int	nfsrootvp(vnode_t **, vfs_t *, struct knetconfig *,
			struct netbuf *, struct netbuf *, fhandle_t *,
			char *, char *, int, int, cred_t *);
extern void	nfs_lockcompletion(vnode_t *vp, int cmd, struct flock *bfp);
extern void	nfs_add_locking_pid(vnode_t *, pid_t);
...
extern ulong_t		rfsproccnt_v2_ndata;
...
extern void	nfs3_cache_check(vnode_t *, wcc_attr *, long *, cred_t *);
extern void	nfs3_cache_check_fattr3(vnode_t *, fattr3 *, long *, cred_t *);
extern void	nfs3_cache_post_op_attr(vnode_t *, post_op_attr *, cred_t *);
extern void	nfs3_cache_wcc_data(vnode_t *, wcc_data *, cred_t *);
extern void	nfs3_attrcache(vnode_t *, fattr3 *, long);
...
extern int	mount3_root(struct knetconfig *, char *, struct netbuf *,
			nfs_fh3 *, char *, char *);
extern int	nfs3rootvp(vnode_t **, vfs_t *, struct knetconfig *,
			struct netbuf *, struct netbuf *, nfs_fh3 *,
			char *, char *, int, int, cred_t *);

extern vtype_t		nf3_to_vt[];
extern kstat_named_t	*rfsproccnt_v3_ptr;
extern ulong_t		rfsproccnt_v3_ndata;
extern struct vfsops	nfs3_vfsops;
extern struct vnodeops	nfs3_vnodeops;
extern vnode_t		nfs3_notfound;
extern u_long		nfs3_root_rsize;
#ifdef DEBUG
