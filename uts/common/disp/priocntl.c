...
#pragma ident	"@(#)priocntl.c	1.38	95/10/05 SMI"	/* from SVr4.0 1.15 */
...
rval_t				*rvp;
{
	...
	case PC_GETCID:
		 ...
		 * In the dynamic world we need to return the number
		 * of loaded classes, not the max number of
		 * available classes that can be loaded.
		 */
		if (uap->arg == NULL) {
			rvp->r_val1 = loaded_classes;
			break;
		} else {
	...
	case PC_SETPARMS:
		 ...
		 * checking here because it's done on a per thread
		 * basis by parmsset().
		 */
		 ...
		 * cases where the current thread is the only one in the
		 * set.  In such cases we can call parmsset() directly,
		 * avoiding the relatively lengthy path through dotoprocs().
		 * The underlying classes expect pidlock to be held.
		 */
		if (cur_inset_only(&procset) == B_TRUE) {
			 ...
			 * to be called for each thread in the specified
			 ...
			 * one or more of the threads it returns EPERM in
			 * stp_error so dotoprocs() will continue through
			 * the thread set.  If dotoprocs() returned an error
	...
	case PC_GETPARMS:
		...
		if (cur_inset_only(&procset) == B_TRUE) {
			 ...
			 * is required to make sure that the process we chose
			 * as the winner doesn't go away i.e. retthreadp has
			 * to be a valid pointer.
			 *
			 * The case below can only happen if the thread with
			 * the highest priority was not in your process.
			 * In that case, dotolwp will return holding p_lock
			 * for both your process as well as the process in
			 * which retthreadp is a thread.
			 */
			if ((retthreadp != NULL) &&
		...
		/*
		 * And finally, return the tid of the selected thread.
		 */
		rvp->r_val1 = ttoproc(retthreadp)->p_pid;
		mutex_exit(&(ttoproc(retthreadp)->p_lock));
		ASSERT(MUTEX_NOT_HELD(&(curproc)->p_lock));
 ...
 * cl_globpri() function to select one thread from a specified
 ...
 * Basic strategy is dotoprocs() calls us once for each thread in
 * the set.  We in turn call the class specific function to compare
 * the current thread from dotoprocs to the "best" (according to
 * the class criteria) found so far.  We keep the "best" thread
 * in *pcmp_retthreadp.
 */
static int
