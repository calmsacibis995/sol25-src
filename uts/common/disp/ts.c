...
#ident	"@(#)ts.c	1.70	95/09/11 SMI"	/* from SVr4.0 1.23 */
...
#define	tscpupri	(tspp->ts_cpupri)
...
static int	ts_parmsin(), ts_parmsout(), ts_parmsset();
static int	ts_donice();
static void	ts_exitclass(), ts_forkret();
static void	ts_nullsys(), ts_parmsget(), ts_preempt();
static void	ts_setrun(), ts_sleep();
static pri_t	ts_swapin(kthread_id_t, int), ts_swapout(kthread_id_t, int);
static void	ts_tick(), ts_trapret(), ts_update(), ts_wakeup();
static cpu_t	*ts_cpu_choose(kthread_id_t t, pri_t tpri);
static pri_t	ts_globpri(tsproc_t *tsprocp);
static void	ts_set_process_group(pid_t sid, pid_t bg_pgid, pid_t fg_pgid);
extern tsdpent_t *ts_getdptbl();
extern pri_t *ts_getkmdpris();
extern pri_t td_getmaxumdpri();
extern pri_t ts_maxkmdpri; /* maximum kernel mode ts priority */

void		ia_init();
static int	ia_getclinfo(), ia_parmsin(), ia_parmsset();
static void	ia_parmsget();
static void	ia_set_process_group(pid_t sid, pid_t bg_pgid, pid_t fg_pgid);
...
static kmutex_t	ts_list_lock;	/* protects time sharing thread list */
	...
	size_t		*clprocsz;
{
	...
	*clprocsz = sizeof (tsproc_t);
	mutex_init(&ts_dptblock, "ts dispatch tbl lock", MUTEX_DEFAULT, NULL);
	mutex_init(&ts_list_lock, "ts list lock", MUTEX_DEFAULT, NULL);
}
...
/* ARGSUSED */
static int
ts_enterclass(t, cid, tsparmsp, reqpcredp, bufp)
	kthread_id_t	t;
	id_t		cid;
	tsparms_t	*tsparmsp;
	cred_t		*reqpcredp;
	void		*bufp;
{
	...
	if (t == curthread) {
		t->t_pri = ts_dptbl[tsumdpri].ts_globpri;
		ASSERT(t->t_pri >= 0 && t->t_pri <= ts_maxglobpri);
		if (DISP_PRIO(t) > DISP_MAXRUNPRI(t)) {
			tspp->ts_timeleft = ts_dptbl[tscpupri].ts_quantum;
		} else if (!(tspp->ts_flags & TSIA)) {
	...
	tsproc_t *tsprocp;
{
	...
	mutex_exit(&ts_list_lock);
	kmem_free((caddr_t)tsprocp, sizeof (tsproc_t));
}
...
ts_fork(t, ct, bufp)
	...
	void	*bufp;
{
	register tsproc_t	*ptspp; /* ptr to parent's tsproc structure */
	 ...
	 */
	thread_lock(t);
	if ((ptspp->ts_flags & (TSIA | TSIASET)) == TSIA)
		ctspp->ts_timeleft = IA_OFF_QUANTUM;
	else
	...
	ctspp->ts_dispwait = 0;
	ctspp->ts_flags = ptspp->ts_flags;
	if (ctspp->ts_flags & TSKPRI) {
		ct->t_trapret = 1;		/* so ts_trapret will run */
		aston(ct);
	}
	ctspp->ts_tp = ct;
	thread_unlock(t);
	...
	register kthread_id_t	ct;
{
	...
	tspp = (tsproc_t *)(curthread->t_cldata);
	tspp->ts_cpupri = ts_dptbl[tscpupri].ts_tqexp;
	TS_NEWUMDPRI(tspp);
	tspp->ts_timeleft = ts_dptbl[tscpupri].ts_quantum;
	tspp->ts_dispwait = 0;
	curthread->t_pri = ts_dptbl[tsumdpri].ts_globpri;
	ASSERT(curthread->t_pri >= 0 && curthread->t_pri <= ts_maxglobpri);
...
static void
ia_parmsget(tsprocp, iaparmsp)
	tsproc_t	*tsprocp;
	iaparms_t	*iaparmsp;
{
	iaparmsp->ia_uprilim = tsprocp->ts_uprilim;
	iaparmsp->ia_upri = tsprocp->ts_upri;
	if (tsprocp->ts_flags & TSIASET)
		iaparmsp->ia_mode = IA_SET_INTERACTIVE;
	else
		iaparmsp->ia_mode = IA_INTERACTIVE_OFF;
	iaparmsp->ia_nice = tsprocp->ts_nice;
}
...
/* ARGSUSED */
static int
ia_parmsin(iaparmsp, reqpcid, reqpcredp, targpcid, targpcredp, tspp)
	register iaparms_t	*iaparmsp;
	id_t			reqpcid;
	cred_t			*reqpcredp;
	id_t			targpcid;
	cred_t			*targpcredp;
	...
	cred_t			*reqpcredp;
{
	...
	ASSERT(MUTEX_HELD(&(ttoproc(tspp->ts_tp))->p_lock));
	 ...
	 */
	nice = NZERO - (tsparmsp->ts_upri * NZERO) / ts_maxupri;
	if (nice >= 2 * NZERO)
		nice = 2 * NZERO - 1;
	...
	tspp->ts_dispwait = 0;
	if (tx == curthread) {
		tx->t_pri = ts_dptbl[tsumdpri].ts_globpri;
		ASSERT(tx->t_pri >= 0 && tx->t_pri <= ts_maxglobpri);
		...
		if (wasonq == B_TRUE) {
			if (tspp->ts_flags & TSIASET) {
				tspp->ts_timeleft =
				    ts_dptbl[tscpupri].ts_quantum;
				setfrontdq(tx);
			} else {
				tspp->ts_timeleft = IA_OFF_QUANTUM;
				setbackdq(tx);
			}
		} else {
			tspp->ts_flags |= TSBACKQ;
...
/* ARGSUSED */
static int
ia_parmsset(iaparmsp, tspp, reqpcid, reqpcredp)
	register iaparms_t	*iaparmsp;
	register tsproc_t	*tspp;
	id_t			reqpcid;
	cred_t			*reqpcredp;
{
	...
	if (iaparmsp->ia_upri != IA_NOCHANGE ||
		...
		return (ts_parmsset((tsparms_t *)iaparmsp, tspp, reqpcid,
		    reqpcredp));
	}
	...
	if ((pid == sid) && (p->p_sessp->s_vp != NULL) &&
		...
		if ((stp->sd_pgidp != NULL) && (stp->sd_sidp != NULL)) {
			...
			(void) ia_set_process_group(sid, off, on);
		}
	}
	if (sess_held)
		TTY_RELE(p->p_sessp);
 ...
 */
static pri_t
ts_globpri(tsproc_t *tsprocp)
{
	register pri_t	tspri;

	tspri = tsmedumdpri + tsprocp->ts_upri;
	...
	kthread_id_t	t;
{
	...
	TRACE_4(TR_FAC_DISP, TR_PREEMPT,
	    "preempt:tid %x (%T) old pri %d new pri %d",
	    t, t, oldpri, t->t_pri);
}
	...
	kthread_id_t	t;
{
	...
	if (tspp->ts_dispwait > ts_dptbl[tsumdpri].ts_maxwait) {
		tspp->ts_cpupri = ts_dptbl[tscpupri].ts_slpret;
		TS_NEWUMDPRI(tspp);
		tspp->ts_timeleft = ts_dptbl[tscpupri].ts_quantum;
		tspp->ts_dispwait = 0;
		if ((tspp->ts_flags & TSKPRI) == 0) {
	...
	short			disp;
{
	...
	if (t->t_kpri_req) {
		...
		t->t_trapret = 1;		/* so ts_trapret will run */
		aston(t);
	} else if (tspp->ts_dispwait > ts_dptbl[tsumdpri].ts_maxwait) {
		/*
	...
	TRACE_5(TR_FAC_DISP, TR_SLEEP,
	    "sleep:tid %x (%T) old pri %d new pri %d class %S",
	...
	int		flags;
{
	...
	if (t->t_state == TS_RUN && (t->t_schedflag & TS_LOAD) == 0) {
		...
		if (INHERITED(t) || (tspp->ts_flags & (TSKPRI | TSIASET)))
			 ...
			 */
			epri = ts_dptbl[tsumdpri].ts_globpri;
	...
	int		flags;
{
	...
	if (flags == SOFTSWAP) {
		...
		if ((t->t_state == TS_SLEEP && swapin_time > ts_minslp) ||
			...
			pri = ts_dptbl[tsumdpri].ts_globpri;
	...
	kthread_id_t	t;
{
	...
	if (--tspp->ts_timeleft <= 0) {
		...
		TRACE_4(TR_FAC_DISP, TR_TICK,
		    "tid %x (%T) old pri %d new pri %d", t, t, oldpri,
		    t->t_pri);
	}
}
...
ts_trapret()
{
	tsproc_t	*tspp = (tsproc_t *)curthread->t_cldata;
#ifdef TRACE
	...
	TRACE_5(TR_FAC_DISP, TR_TRAPRET,
	    "trapret:tid %x (%T) old pri %d new pri %d class %S",
	    curthread, curthread, old_pri, curthread->t_pri,
	    sclass[curthread->t_cid].cl_name)
}
...
ts_update()
{
	...
	for (tspp = ts_plisthead.ts_next; tspp != &ts_plisthead;
		...
		tspp->ts_cpupri = ts_dptbl[tscpupri].ts_lwait;
		TS_NEWUMDPRI(tspp);
		tspp->ts_dispwait = 0;
		/*
		 ...
		 */
		if (tx->t_pri != ts_dptbl[tsumdpri].ts_globpri) {
#ifdef TRACE
			oldpri = tx->t_pri;
#endif
			wasonq = dispdeq(tx);
			...
			TRACE_4(TR_FAC_DISP, TR_UPDATE,
			    "update:tid %x (%T) old pri %d new pri %d",
 ...
 */
#define	TS_RECHOOSE_INTERVAL	3

/*
 * Select a CPU for this thread to run on.
 */
static cpu_t *
ts_cpu_choose(kthread_id_t t, pri_t tpri)
{
	cpu_t   *bestcpu;
#ifdef TRACE
	cpu_t	*oldcpu = t->t_cpu;
#endif
	...
	if (tpri >= kpreemptpri ||
	    ((lbolt - t->t_disp_time) > TS_RECHOOSE_INTERVAL && t != curthread))
		bestcpu = disp_lowpri_cpu(bestcpu->cpu_next_onln);
	TRACE_5(TR_FAC_DISP, TR_CPU_CHOOSE,
	    "tid %x (%T) pri %d old cpu %d new cpu %d",
	    t, t, t->t_pri, oldcpu->cpu_id, bestcpu->cpu_id);
	return (bestcpu);
}

/* ARGSUSED */
static void
ts_set_process_group(pid_t sid, pid_t bg_pgid, pid_t fg_pgid)
{
	/* nop function */
}
 ...
 * either strioctl or priocntlsys.
 */
/* ARGSUSED */
static void
