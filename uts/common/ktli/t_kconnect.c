...
/* SVr4.0 1.4  */
#pragma	ident	"@(#)t_kconnect.c	1.10	93/12/12 SMI"
	...
	register struct	t_call	*rcvcall)
{
	...
	if (sndcall->addr.len) {
		bcopy(sndcall->addr.buf, (caddr_t)(bp->b_wptr+msgsz),
		    sndcall->addr.len);
	...
	if (sndcall->opt.len) {
		bcopy(sndcall->opt.buf, (caddr_t)(bp->b_wptr+msgsz),
		    sndcall->opt.len);
	...
	case T_CONN_CON:
		...
		if (rcvcall != NULL) {
			...
			len = min(pptr->conn_con.RES_length,
			    rcvcall->addr.maxlen);
			bcopy((caddr_t)(bp->b_rptr + pptr->conn_con.RES_offset),
			    (caddr_t)rcvcall->addr.buf, len);
			rcvcall->addr.len = len;

			len = min(pptr->conn_con.OPT_length,
			    rcvcall->opt.maxlen);
			bcopy((caddr_t)(bp->b_rptr + pptr->conn_con.OPT_offset),
			    (caddr_t)rcvcall->opt.buf, len);
			rcvcall->opt.len = len;
