...
#pragma	ident	"@(#)swapgeneric.c	5.91	95/01/27 SMI"
...
static int loaddrv_hierarchy(char *drvname, major_t prev_major);
...
dumpinit(struct vnode *vp, char *name)
{
	...
	if (dumpfile.bo_vp == NULL) {
		 ...
		 */
		dumpfile.bo_vp = vp;
		strncpy(dumpfile.bo_name, name, BO_MAXOBJNAME);
	}
	...
	VN_HOLD(dumpvp);
	dumphdr_init();
	cmn_err(CE_CONT, "?dump on %s size %dK\n", dumpfile.bo_name,
...
loop:
	 ...
	 *
	 * XXX	The need to "go backwards" should be reassessed!
	 */

	if (strcmp(rootfs.bo_fstype, "cachefs") == 0) {
		if ((this = getlastfrompath(backfs.bo_name)) == (char *)0) {
			err = ENXIO;
		} else if ((err = loaddrv_hierarchy(this, (major_t) 0)) == 0) {
			/*
			 * By now the single call of the recursive
			 * 'loaddrv' will have loaded all possible
			 * parent drivers of the named leaf device
			 * if the .conf file descriptions are correct.
			 *
			 * For h/w devinfo nodes, there need not be .conf
			 * files thus we must do this on the entire pathname.
			 */
			for (this = getlastfrompath(NULL);
			    this != NULL && err == 0;
			    this = getlastfrompath(NULL)) {
				err = loaddrv_hierarchy(this, (major_t) 0);
			}
		}
#if 0
printf("backdev loaded: err is %x\n", err);
#endif
		name = backfs.bo_name;
		if (frontfs.bo_fstype[0] != '\0' && err == 0) {
			if ((this = getlastfrompath(frontfs.bo_name)) ==
			    (char *)0)
				err = ENXIO;
			else if ((err = loaddrv_hierarchy(this,
			    (major_t) 0)) == 0) {
				/*
				 * See above
				 */
				for (this = getlastfrompath(NULL);
				    this != NULL && err == 0;
				    this = getlastfrompath(NULL))
					err = loaddrv_hierarchy(this,
					    (major_t) 0);
			}
			name = frontfs.bo_name;
		}
	} else {
		if ((this = getlastfrompath(rootfs.bo_name)) == (char *)0)
			err = ENXIO;
		else if ((err = loaddrv_hierarchy(this, (major_t) 0)) == 0) {
			/*
			 * By now the single call of the recursive
	 ...
	 * XXX: from the PROM is "network". (but what about sunmon?)
	 */
...
getphysdev(char *askfor, char *name)
{
	 ...
	 */
	if (strcmp(askfor, "root") == 0) {
		if (BOP_GETPROP(bootops, "boot-path", defaultpath) == -1)
...
retry:
	...
	if ((strcmp(askfor, "root") != 0) &&
	    (strcmp(askfor, "front") != 0) &&
	    (strcmp(askfor, "back") != 0) &&
	    (ddi_pathname_to_dev_t(name) == NODEV)) {
		printf("Not a supported device for %s.\n",
		    askfor);
		boothowto |= RB_ASKNAME | RB_VERBOSE;
		goto retry;
	}

	if (*name != '\0' && strcmp(askfor, "swap") == 0) {

		/*
		 * Get the name of the leaf driver from the
		 * physical device path
		 */
		if ((drvname = getlastfrompath(name)) == NULL)
			goto broken;

		/*
		 * Ensure that we're not trying to swap on the floppy.
		 */
		if (strncmp(drvname, "fd", 2) == 0) {
			printf("Too dangerous to swap on the floppy\n");
			goto broken;
		}

		/*
		 * Ensure the named driver is loaded and attached,
		 * together with all it's parent drivers.
		 */
		if (mod_find_by_filename("drv", drvname) == NULL &&
		    modload("drv", drvname) == -1)
			goto broken;
	}
