...
#pragma ident   "@(#)kstat_fr.c 1.21     95/08/22 SMI"
...
extern	ulong_t		segmapcnt_ndata;
extern	kstat_named_t	*biostats_ptr;
extern	ulong_t		biostats_ndata;
extern	kstat_named_t	*vmhatstat_ptr;
extern	ulong_t		vmhatstat_ndata;
...
struct {
	...
	{ "ncpus",		KSTAT_DATA_ULONG },
	{ "lbolt",		KSTAT_DATA_ULONG },
	{ "deficit",		KSTAT_DATA_ULONG },
	{ "clk_intr",		KSTAT_DATA_ULONG },
	{ "vac",		KSTAT_DATA_ULONG },
	{ "nproc",		KSTAT_DATA_ULONG },
	{ "avenrun_1min",	KSTAT_DATA_ULONG },
	{ "avenrun_5min",	KSTAT_DATA_ULONG },
	{ "avenrun_15min",	KSTAT_DATA_ULONG }
};
...
kstat_init(void)
{
	...
	mutex_init(&kstat_chain_lock, "kstat_chain_lock", MUTEX_DEFAULT, NULL);
	...
	if (ksp) {
		...
		cmn_err(CE_PANIC, "cannot create kstat 'kstat_headers'");
	}
	...
	ksp = kstat_create("unix", 0, "vmhatstat", "hat", KSTAT_TYPE_NAMED,
		vmhatstat_ndata, KSTAT_FLAG_VIRTUAL);
	if (ksp) {
		ksp->ks_data = (void *) vmhatstat_ptr;
		kstat_install(ksp);
	}
	...
	ksp = kstat_create("unix", 0, "flckinfo", "misc", KSTAT_TYPE_RAW,
		sizeof (struct flckinfo), KSTAT_FLAG_VIRTUAL);
	if (ksp) {
		ksp->ks_data = (void *) &flckinfo;
		kstat_install(ksp);
	}
...
default_kstat_snapshot(kstat_t *ksp, void *buf, int rw)
{
	...
	switch (ksp->ks_type) {
	    ...
	    case KSTAT_TYPE_IO:
		/*
		 * apply corrections to deal with
...
header_kstat_snapshot(kstat_t *header_ksp, void *buf, int rw)
{
	...
	for (ksp = kstat_chain; ksp; ksp = ksp->ks_next) {
		kbuf--;
		bcopy((char *)ksp, (char *)kbuf, sizeof (kstat_t));
	}
	return (0);
}
...
system_misc_kstat_update(kstat_t *ksp, int rw)
{
	if (rw == KSTAT_WRITE)
		return (EACCES);
	system_misc_kstat.ncpus.value.ul		= (ulong_t)ncpus;
	system_misc_kstat.lbolt.value.ul		= (ulong_t)lbolt;
	system_misc_kstat.deficit.value.ul		= (ulong_t)deficit;
	system_misc_kstat.clk_intr.value.ul		= (ulong_t)clk_intr;
	system_misc_kstat.vac.value.ul			= (ulong_t)vac;
	system_misc_kstat.nproc.value.ul		= (ulong_t)nproc;
	system_misc_kstat.avenrun_1min.value.ul		= (ulong_t)avenrun[0];
	system_misc_kstat.avenrun_5min.value.ul		= (ulong_t)avenrun[1];
	system_misc_kstat.avenrun_15min.value.ul	= (ulong_t)avenrun[2];
	return (0);
}

extern int	freemem;
extern u_int	econtig;
extern int	lotsfree, desfree, minfree;
extern int	fastscan, slowscan;
extern int	nscan, desscan;
extern page_t	*pages;
...
system_pages_kstat_update(kstat_t *ksp, int rw)
{
	...
	int pagesfree, pageslocked, pagesio;
	...
	system_pages_kstat.pp_kernel.value.ul	= (ulong_t)pages_pp_kernel;
	...
	while (pp < epages) {
		...
		if (pp->p_selock || pp->p_lckcnt) {
			pageslocked++;
		}

		if (sema_held(&(pp->p_iolock))) {
			pagesio++;
		}
	...
	system_pages_kstat.pagestotal.value.ul	= (ulong_t)(epages - pages);
	return (0);
}
...
{
	ulong_t ks_data_size, allocation_size;
	kstat_t *new;
	ekstat_t *enew;
	...
	if (ks_name == NULL) {
		...
		sprintf(namebuf, "%s%d", buf, ks_instance);
		ks_name = namebuf;
	}
	...
	if (ks_ndata < kstat_data_type[ks_type].min_ndata ||
		...
		cmn_err(CE_WARN, "kstat_create('%s', %d, '%s'): "
			...
			(int)kstat_data_type[ks_type].min_ndata,
			(int)kstat_data_type[ks_type].max_ndata);
		return (NULL);
	}
...
kstat_install(kstat_t *new)
{
	...
	if ((new->ks_flags & KSTAT_FLAG_VAR_SIZE) &&
		...
		mutex_exit(&kstat_chain_lock);
		cmn_err(CE_PANIC, "kstat_install('%s', %d, '%s'): "
...
kstat_delete(kstat_t *kspdel)
{
	...
	if (lp != NULL && MUTEX_HELD(lp)) {
		cmn_err(CE_PANIC,
		    "kstat_delete(%x): caller holds data lock %x",
		    (int)kspdel, (int)lp);
	}
	...
	if (kspdel == kstat_chain) {
		...
		if (ksp == NULL) {
			 ...
			 */
			mutex_exit(&kstat_chain_lock);
			cmn_err(CE_WARN, "kstat_delete(%x): "
			    "does not exist in kstat chain", (int)kspdel);
			return;
		}
	}
...
kstat_hash_delete(ekstat_t *edel, int hash_type)
{
	...
	if (eksp == edel) {
		...
		if (eksp == NULL) {
			cmn_err(CE_WARN,
			    "can't find kstat %x in hash table %d",
			    (int)edel, hash_type);
			return;
		}
	}
...
/*
 * wait queue exit + run queue enter, in one function
 */
void
...
/*
 * Move a request from the run queue back to the wait queue
 */
void
