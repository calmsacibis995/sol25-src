...
#ident	"@(#)sched.c	1.52	95/05/23 SMI"	/* from SVr4.0 1.16 */
...
int	maxslp = 0;
int	avefree;	/* 5 sec moving average of free memory */
int	avefree30;	/* 30 sec moving average of free memory */
...
top:
	...
	for (prp = practive; prp != NULL; prp = prp->p_next) {
		...
		if (prp->p_swapcnt) {
			...
			mutex_enter(&prp->p_lock);
			if (prp->p_lwpcnt == 0 ||
			    prp->p_as != NULL && prp->p_as->a_paglck) {
				mutex_exit(&prp->p_lock);
				continue;
			}
	...
	if (swapin_prp == NULL) {
		TRACE_3(TR_FAC_SCHED, TR_RUNOUT,
		    "schedrunout:runout nswapped: %d, avefree: %d freemem: %d",
	...
	if (avail > needs) {
		...
		TRACE_5(TR_FAC_SCHED, TR_SWAPIN_VALUES,
		    "swapin_values:%s[%d] needs %d freemem %d deficit %d",
		...
		if (freemem > needs) {
			TRACE_5(TR_FAC_SCHED, TR_HIGH_DEFICIT,
			    "deficit:%s[%d] needs %d freemem %d deficit %d",
			    PTOU(swapin_prp)->u_comm, swapin_prp->p_pid,
			    needs, freemem, deficit);
			goto block;
		}
	...
	TRACE_5(TR_FAC_SCHED, TR_UNLOAD,
	    "unload:%s[%d] needs %d freemem %d deficit %d",
	    PTOU(swapin_prp)->u_comm, swapin_prp->p_pid,
	    needs, freemem, deficit);
...
unload:
	...
	for (prp = practive; prp != NULL; prp = prp->p_next) {
		...
		if (swapin_prp == NULL || swapin_pri > proc_pri) {
			TRACE_5(TR_FAC_SCHED, TR_CHOOSE_SWAPOUT,
			    "hardswap:%s[%d] pri %d psize %d needs %d",
			    PTOU(prp)->u_comm, prp->p_pid, proc_pri,
	...
	TRACE_3(TR_FAC_SCHED, TR_RUNIN,
	    "schedrunin:runin nswapped: %d, avefree: %d freemem: %d",
	    nswapped, avefree, freemem);
...
/*
 * Swap in lwps.  Returns 1 on success (i.e., if at least one lwp is
		...
		if (thread_pri != -1 && tp->t_state == TS_RUN &&
			...
			if (err) {
				...
				TRACE_4(TR_FAC_SCHED, TR_SWAPIN,
				    "swapin:%s[%d] pages_in: %d freemem: %d",
...
/*
 * Swap out lwps.  Returns 1 on success (i.e., if at least one lwp is
 * swapped out) and 0 on failure.
 */
static int
swapout(pp, swrss, swapflags)
	proc_t *pp;
	u_int *swrss;
	int swapflags;
{
...
top:
	...
	if (pp->p_swapcnt == pp->p_lwpcnt) {
		...
		TRACE_4(TR_FAC_SCHED, TR_SWAPOUT,
		    "swapout:%s[%d] pages_pushed: %d freemem %d",
	...
	klwp_t	*lwp;
{
	...
	if (tp->t_schedflag & TS_SWAPENQ) {
		...
		TRACE_4(TR_FAC_SCHED, TR_SWAPOUT_LWP,
		    "swapout_lwp:%s[%d] freemem %d avefree %d",
...
process_swap_queue()
{
	...
	while ((tp = tswap_queue) != NULL) {
		...
		TRACE_5(TR_FAC_SCHED, TR_SWAPQ_LWP,
		    "swaplist:%s[%d] swapcnt: %d freemem %d avefree %d",
		...
		if (pp->p_swapcnt == pp->p_lwpcnt) {
			...
			TRACE_4(TR_FAC_SCHED, TR_SWAPQ_PROC,
			    "swaplist_proc:%s[%d] pages_pushed: %d freemem %d",
			    PTOU(pp)->u_comm, pp->p_pid, ws_pages, freemem);
			/* Kernel probe */
