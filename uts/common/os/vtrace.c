...
#pragma ident	"@(#)vtrace.c	1.21	94/09/29 SMI"
...
inittrace(void)
{
	...
	mutex_init(&tracing_state_lock, "tracing state lock",
		MUTEX_SPIN_DEFAULT, (void *) ipltospl(SPL7));
	mutex_init(&trace_reset_lock, "trace reset lock",
		MUTEX_DEFAULT, (void *) NULL);
}

/*
 * XXX - hack to get hi-res time in microseconds.  This will go away with
 * vtrace 4.0.0, which will be nanosecond-based.
 * Routine reads the hi-res timer (nanoseconds), then performs division
 * by 1000 without using integer divide (__div64 is not in the kernel yet).
 */
hrtime_t
...
trace_vn_write(struct vnode *vp, struct cred *cr, char *addr, int len)
{
	...
	TRACE_3(TR_FAC_TEST, TR_TRACE_VN_WRITE_START,
		"trace_vn_write_start:vp %x addr %x len %d", vp, addr, len);
	error = vn_rdwr(UIO_WRITE, vp, addr, len, 0, UIO_SYSSPACE,
		FAPPEND, u.u_rlimit[RLIMIT_FSIZE].rlim_cur,
		cr, &resid);
	if (resid)
		error = EFBIG;
	if (error)
...
#ifdef COMPILER_longlong_BUG
	/* the 2.0.1 compiler can't grok this expression with -xcg92 */
	abs_hrtime = ((hrtime_t)hrestime.tv_sec * 1000000) +
		(hrestime.tv_nsec) / 1000;
#else
	{
		register hrtime_t tmp_time;

		tmp_time = hrestime.tv_sec;
		tmp_time *= 1000000;
		tmp_time += hrestime.tv_nsec / 1000;
		abs_hrtime = tmp_time;
	}
#endif
