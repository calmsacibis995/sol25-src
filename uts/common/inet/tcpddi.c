...
#pragma ident	"@(#)tcpddi.c	1.24	95/02/08 SMI"
...
#define	D_SM_FLAGS D_MP|D_MTQPAIR|D_SYNCSTR

extern	int	nulldev();
extern	int	nodev();
extern	void    tcp_random_init( void );

extern krwlock_t tcp_g_lock;
...
_init(void)
{
	...
	rw_init(&tcp_g_lock, "TCP global lock", RW_DEFAULT, NULL);
	error = mod_install(&modlinkage);
	if (error != 0) {
		rw_destroy(&tcp_g_lock);
	}
