...
#ident	"@(#)intp.c	1.17	93/05/05 SMI" /* from S5R4 1.6 */
 ...
 */
static int
getintphead(vp, idatap)
	struct vnode *vp;
	register struct intpdata *idatap;
{
	register int error;
	register char *cp, *linep = idatap->intp;
	int resid;

	/*
	 * Read the entire line and confirm that it starts with '#!'.
	 */
	if (error = vn_rdwr(UIO_READ, vp, linep, INTPSZ, (off_t) 0,
	    UIO_SYSSPACE, 0, (ulong) 0, CRED(), &resid))
		return (error);
...
int
intpexec(vp, uap, args, idatap, level, execsz, setid, exec_file, cred)
	struct vnode *vp;
	struct execa *uap;
	struct uarg *args;
	...
	struct cred *cred;
{
	 ...
	 */
	idata.intp = (caddr_t)kmem_alloc(INTPSZ, KM_SLEEP);
