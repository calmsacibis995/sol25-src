...
#pragma ident	"@(#)audit.h	1.53	95/01/11 SMI"
...
#define	AU_OTHER	0x80000000	/* other category */
#define	AU_ALL		0xffffffff	/* Everything */
 ...
 */
struct au_qctrl {
	u_long	aq_hiwater;	/* kernel audit queue, high water mark */
	u_long  aq_lowater;	/* kernel audit queue, low  water mark */
	u_long  aq_bufsz;	/* kernel audit queue, write size to trail */
	u_long  aq_delay;	/* delay before flushing audit queue */
};
...
struct audit_stat {
	unsigned long as_version;	/* version of kernel audit code */
	unsigned long as_numevent;	/* number of kernel audit events */
	long as_generated;	/* # records processed */
	long as_nonattrib;	/* # non-attributed records produced */
	long as_kernel;	/* # records produced by kernel */
	long as_audit;		/* # records processed by audit(2) */
	long as_auditctl;	/* # records processed by auditctl(2) */
	long as_enqueue;	/* # records put onto audit queue */
	long as_written;	/* # records written to audit trail */
	long as_wblocked;	/* # times write blked on audit queue */
	long as_rblocked;	/* # times read blked on audit queue */
	long as_dropped;	/* # of dropped audit records */
	long as_totalsize;	/* total number bytes of audit data */
	unsigned long as_memused;	/* amount of memory used by auditing */
};
 ...
 */
struct audit_fstat {
	unsigned long af_filesz;
	unsigned long af_currsz;
};
...
void	audit_savepath(struct pathname *, struct vnode *, int);
...
void	audit_strioctl(struct vnode *, int, int, int, int, struct cred *,
		int *);
...
void	audit_exec(u_int, u_int, u_int, int, int);
...
void	audit_start(unsigned int, unsigned int, int, klwp_t *);
...
u_int	audit_fixpath(char *, u_int);
