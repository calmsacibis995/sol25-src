	...
	caddr_t *d;
{
	...
	if (*d == (caddr_t) NULL)
		*d = (caddr_t) m;
		...
		au_append_rec((au_buff_t *) *d, (au_buff_t *)m);
}
...
{
	token_t *au_to_header();	/* creates header token */
	token_t *au_to_trailer();	/* creates trailer token */
	...
	*d = NULL;
	...
	au_append_rec((au_buff_t *)record, (au_buff_t *)dchain);
	if (audit_policy & AUDIT_TRAIL) {
		au_append_rec((au_buff_t *)record,
...
{	/* AU_DOIO */
	...
	unsigned int	off;	/* space used in buffer */
	unsigned int	used;	/* space used in au_membuf */
	 ...
	 */
	unsigned int	sz;
	unsigned int	len;	/* len of data to move, size of AR */
	int		error;	/* error return */
	int		left;	/* data not xfered by write to disk */
	statvfs_t	sb;	/* buffer for statfs */
	unsigned int	curr_sz = 0;	/* amount of data written during now */

ADDTRACE("[%x] au_doio(%x,%x)", vp, limit, 0, 0, 0, 0);

	/*
	 * Check to insure enough free space on audit device.
	 */
ADDTRACE("[%x], vnode %x vp->v_vfsp \n", vp, vp->v_vfsp, 0, 0, 0, 0);
	...
	if (au_queue.bufsz != au_queue.buflen) {
		...
		kmem_free(au_buffer, (u_int)au_queue.buflen);

		/* only if write buffer is non-zero */
		if (au_queue.bufsz) {
			AS_INC(as_memused, (u_int)au_queue.bufsz);

			/* bad, should not sleep here. Testing only */
			au_buffer = (char *) kmem_alloc(
				(u_int)au_queue.bufsz, KM_SLEEP);
		}
		au_queue.buflen = au_queue.bufsz;

ADDTRACE("[%x] new buffer size: %x", au_queue.buflen, 0, 0, 0, 0, 0);
	}
...
ADDTRACE("[%x] buffered writes %x", au_queue.buflen, 0, 0, 0, 0, 0);
...
ADDTRACE("[%x] xfer strt - AR cnt: %x cAR: %x cMB: %x used: %x bp: %x off: %x",
...
debug_enter("HELP ---- au_queue.head is null and we are still here");
			...
			sz  = cMB->len - used; /* data left in au_membuf */
			len = (u_int) min((int) sz,
			    (int) (au_queue.buflen-off)); /* len to move */

ADDTRACE("[%x] bcopy - cp: %x used: %x bp: %x off: %x ln: %x",
	cp, used, bp, off, len, 0);

			/*
			 * move the data
			 */
			bcopy((caddr_t) cp + used, bp+off, len);
			used += len; /* update used au_membuf */
			off  += len; /* update offset into buffer */

ADDTRACE("[%x] new offsets - off: %x cAR: %x sp: %x", off, cAR, sp, 0, 0, 0);

			if (used >= cMB->len) { /* advance to next au_membuf */
				used = 0;
				cMB  = cMB->next_buf;
			}
...
ADDTRACE("[%x] finished? cAR: %x cMB: %x used: %x off: %x",
...
ADDTRACE("[%x] vn_rdwr write - off %x", off, left, 0, 0, 0, 0);

				error = vn_rdwr(UIO_WRITE, vp, au_buffer,
	(int)off, 0, UIO_SYSSPACE, FAPPEND, RLIM_INFINITY, CRED(), &left);

ADDTRACE("[%x] error: %x left: %x", error, left, 0, 0, 0, 0);
...
ADDTRACE("[%x] no FS space - sz: %x = off: %x left: %x",
	sz, off, left, 0, 0, 0);
...
ADDTRACE("[%x] releasing AR - cAR: %x &data_cAR: %x AR_len: %x (sz: %x)",
	cAR, cp, len, sz, 0, 0);
					if (len > sz)
						break;
					sz -= len;
...
ADDTRACE("[%x] flush buffer - AQ.head: %x AQ.len: %x sp: %x",
...
ADDTRACE("[%x] bufs freed - AQ.head: %x aq.cnt: %x",
	au_queue.head, au_queue.cnt, 0, 0, 0, 0);
...
#ifdef NOTYET
				if (!au_nomb()) {
					(void) wakeup((caddr_t)&au_checktime);
				}
#endif
				/* Update sizes */
				curr_sz += off;
				audit_file_stat.af_currsz += off;
				...
				if (limit > (sb.f_bavail - curr_sz/ sb.f_bsize))
					return (ENOSPC);
				if (audit_file_stat.af_filesz &&
...
ADDTRACE("[%x] go to next AR - AQ.head: %x cAR: %x cMB: %x bp: %x",
	...
	memtod(cMB, caddr_t), cMB->len, 0, UIO_SYSSPACE,
	FAPPEND, RLIM_INFINITY, CRED(), &left);
...
{	/* ADDTRACE */
	 ...
	 */
	if (c2_debug)
		printf(f, cnt_save_trace, v1, v2, v3, v4, v5, v6);
	else
		sprintf(buffer, f, cnt_save_trace, v1, v2, v3, v4, v5, v6);
