...
#pragma ident   "@(#)vm_pvn.c 1.141     94/12/02 SMI"
/*	From:	SVr4.0	"kernel:vm/vm_pvn.c	1.29"		*/
 ...
 */
page_t *
pvn_read_kluster(vp, off, seg, addr, offp, lenp, vp_off, vp_len, isra)
	struct vnode *vp;
	register u_int off;
	...
	int isra;
{
	...
	if (bytesavail <= 0) {
		if (isra)
			...
			bytesavail = PAGESIZE;	    /* must return a page */
	}
	...
	TRACE_3(TR_FAC_VM, TR_PVN_READ_KLUSTER,
		"pvn_read_kluster:seg %x addr %x isra %x",
		seg, addr, isra);
	return (plist);
}
 ...
 */
page_t *
pvn_write_kluster(vp, pp, offp, lenp, vp_off, vp_len, flags)
 ...
 */
int	write_free = 1;
int	pages_before_pager = 200;	/* LMXXX */
	...
	register int flags;
{
	...
	while (plist != NULL) {
		pp = plist;
		ASSERT(se_assert(&pp->p_selock) && page_iolock_assert(pp));
		page_sub(&plist, pp);
		...
		if (flags & B_ERROR) {
			...
			TRACE_3(TR_FAC_VM, TR_PAGE_WS_OUT,
				"page_ws_out:pp %x vp %x offset %x",
			...
			if (page_tryupgrade(pp) &&
				 ...
				 *
				 * Note that we look at the "p_mod"
				 * bit and the "p_mapping" field without
				 * holding the "per hat_mutex" as a safe
				 * "optimistic" optimization that we
				 * will verify below.
				 */
				if (!PP_ISMOD(pp) && pp->p_mapping)
					hat_pagesync(pp,
					    HAT_DONTZERO | HAT_STOPON_MOD);
			ck_refmod:
				if (!PP_ISREF(pp) && !PP_ISMOD(pp)) {
					if (pp->p_mapping) {
						/*
					...
					"page_ws_free:pp %x vp %x offset %x",
					pp, pp->p_vnode, pp->p_offset);
				}
			} else {
				/*
 ...
 */
int
pvn_getdirty(pp, flags)
	register page_t *pp;
	int flags;
{
	ASSERT((flags & (B_INVAL | B_FREE)) ?
	    se_excl_assert(&pp->p_selock) : se_shared_assert(&pp->p_selock));
	ASSERT(pp->p_free == 0);
	...
	TRACE_3(TR_FAC_VM, TR_PVN_GETDIRTY,
		"pvn_getdirty:pp %x vp %x offset %x",
		pp, pp->p_vnode, pp->p_offset);
 ...
 */
int
pvn_vplist_dirty(vp, off, putapage, flags, cred)
	register struct vnode *vp;
	u_int		off;
	int		(*putapage)();
	int		flags;
	struct cred	*cred;
{
	...
	if (vp->v_pages) {
		...
		end.p_offset = (u_int)-1;
		end.p_fsdata = 0;
		mark.p_vnode = vp;
		mark.p_offset = (u_int)-2;
		mark.p_fsdata = 0;
...
top:
	/* LINTED lint confusion: used before set: ppnext */
	for (pp = vp->v_pages; pp != &end; pp = ppnext) {
		...
		if (pp->p_free) {
			 ...
			 *
			 * XXX - p_mapping is mostly NULL for pages while
			 * running on a long AIM3 benchmark.  It turns out
			 * that 60 million times out of 60.5 million times
			 * through the loop, pvn_getdirty only returned
			 * true 29 thousand times.  That's 1 in 2000.
	...
	struct cred *cred;
{
	...
	for (o = off; o < eoff; o += PAGESIZE, addr += PAGESIZE,
		...
		err = (*getapage)(vp, o, xlen, protp, ppp, sz, seg, addr,
		    rw, cred);
		if (err) {
			/*
