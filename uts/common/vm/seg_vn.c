...
#ident	"@(#)seg_vn.c	1.145	95/10/20 SMI"
/*	From:	SVr4.0	"kernel:vm/seg_vn.c	1.54"		*/
...
static int	segvn_dup(struct seg *, struct seg *);
static int	segvn_unmap(struct seg *, caddr_t, u_int);
static void	segvn_free(struct seg *);
static faultcode_t segvn_fault(struct hat *, struct seg *, caddr_t, u_int,
		    enum fault_type, enum seg_rw);
static faultcode_t segvn_faulta(struct seg *, caddr_t);
static int	segvn_setprot(struct seg *, caddr_t, u_int, u_int);
static int	segvn_checkprot(struct seg *, caddr_t, u_int, u_int);
static int	segvn_kluster(struct seg *, caddr_t, int);
static u_int	segvn_swapout(struct seg *);
static int	segvn_sync(struct seg *, caddr_t, u_int, int, u_int);
static int	segvn_incore(struct seg *, caddr_t, u_int, char *);
static int	segvn_lockop(struct seg *, caddr_t, u_int, int, int,
		    ulong *, size_t);
static int	segvn_getprot(struct seg *, caddr_t, u_int, u_int *);
static off_t	segvn_getoffset(struct seg *, caddr_t);
static int	segvn_gettype(struct seg *, caddr_t);
static int	segvn_getvp(struct seg *, caddr_t, struct vnode **);
static int	segvn_advise(struct seg *, caddr_t, u_int, int);
static void	segvn_dump(struct seg *);
...
static faultcode_t segvn_fault(/* hat, seg, addr, len, type, rw */);
...
static int	segvn_advise(/* seg, addr, len, behav */);
static void	segvn_dump(/* seg */);
...
u_int segvn_comb_thrshld = UINT_MAX;	/* patchable -- see 1196681 */
...
static int	segvn_extend_prev(struct seg *, struct seg *,
		    struct segvn_crargs *, u_int);
static int	segvn_extend_next(struct seg *, struct seg *,
		    struct segvn_crargs *, u_int);
static void	segvn_softunlock(struct seg *, caddr_t, u_int, enum seg_rw);
static void	segvn_pagelist_rele(page_t **);
static int	segvn_faultpage(struct hat *, struct seg *, caddr_t, u_int,
		    struct vpage *, page_t **, u_int,
		    enum fault_type, enum seg_rw);
static void	segvn_vpage(struct seg *);

#else

static int	segvn_concat(/* seg1, seg2 */);
static int	segvn_extend_prev(/* seg1, seg2, a, swresv */);
static int	segvn_extend_next(/* seg1, seg2, a, swresv */);
static void	segvn_softunlock(/* seg, addr, len, rw */);
static void	segvn_pagelist_rele(/* ppp */);
static int	segvn_faultpage(/* hat, seg, addr, off, vpage, */
			/* pl, vpprot, type, rw */);
...
/*ARGSUSED1*/
static void
segvn_cache_constructor(void *buf, size_t size)
{
	struct segvn_data *svd = buf;

	rw_init(&svd->lock, "segvn rwlock", RW_DEFAULT, DEFAULT_WT);
}

/*ARGSUSED1*/
static void
segvn_cache_destructor(void *buf, size_t size)
{
	struct segvn_data *svd = buf;
	...
	void *argsp;
{
	...
	if (!(a->flags & MAP_NORESERVE) && ((a->vp == NULL && a->amp == NULL) ||
		...
		if (anon_resv(seg->s_size) == 0)
			return (EAGAIN);
		swresv = seg->s_size;
		TRACE_5(TR_FAC_VM, TR_ANON_PROC, "anon proc:%u %u %u %u %u",
	...
	if (error != 0) {
		if (swresv != 0) {
			...
			TRACE_5(TR_FAC_VM, TR_ANON_PROC,
				"anon proc:%u %u %u %u %u",
				curproc->p_pid, seg, seg->s_base, swresv, 0);
		}
		return (error);
	}
	...
	if (a->vp) {
		error = VOP_ADDMAP(a->vp, (offset_t)a->offset & PAGEMASK,
		...
		if (error) {
			...
			crfree(cred);
			hat_unmap(seg->s_as, seg->s_base,
				seg->s_size, HAT_UNMAP);
			return (error);
		}
	}
		...
		if (a->type == MAP_SHARED) {
			...
			for (addr = seg->s_base; addr < eaddr;
				...
				*app = ap;
				...
				hat_memload(seg->s_as->a_hat, seg->s_as, addr,
				    pp, svd->prot & ~PROT_WRITE, HAT_LOAD);
				page_unlock(pp);
			}
			mutex_enter(&amp->lock);
			anon_dup(&amp->anon[anon_num], svd->amp->anon,
			    seg->s_size);
			mutex_exit(&amp->lock);
...
#undef incompat
	...
	if (vpage1 != NULL || vpage2 != NULL) {
		register u_int npages1, npages2;
		register struct vpage *vp, *new_vpage;

		npages1 = seg_pages(seg1);
		npages2 = seg_pages(seg2);
		new_vpage = (struct vpage *)
		    kmem_zalloc((u_int)(vpgtob(npages1 + npages2)), KM_NOSLEEP);
		if (new_vpage == (struct vpage *)NULL)
			return (-1);
		if (vpage1 != NULL)
		...
		if (vpage1 != NULL)
			kmem_free((caddr_t)vpage1, vpgtob(npages1));

		if (vpage2 != NULL) {
			svd2->vpage = NULL;
			kmem_free((caddr_t)vpage2, vpgtob(npages2));
		}
...
segvn_extend_prev(seg1, seg2, a, swresv)
	...
	u_int swresv;
{
	register struct segvn_data *svd1 = (struct segvn_data *)seg1->s_data;
	register u_int size;
	register struct anon_map *amp1;
	register struct vpage *new_vpage;
	...
	if (amp1) {
		...
		if (amp1->size - ctob(svd1->anon_index) <
			...
			aa = (struct anon **)
			    ...
			    sizeof (struct anon *), KM_NOSLEEP);
			if (aa == (struct anon **)NULL) {
				mutex_exit(&amp1->lock);
				return (-1);
			}
	...
	if (svd1->vpage != NULL) {
		new_vpage = (struct vpage *)
		    kmem_zalloc((u_int)(vpgtob(seg_pages(seg1) +
		    seg_pages(seg2))), KM_NOSLEEP);
		if (new_vpage == (struct vpage *)NULL)
			return (-1);
		bcopy((caddr_t)svd1->vpage, (caddr_t)new_vpage,
		    (u_int)vpgtob(seg_pages(seg1)));
		kmem_free((caddr_t)svd1->vpage, vpgtob(seg_pages(seg1)));
		svd1->vpage = new_vpage;
		if (svd1->pageprot) {
	...
	u_int swresv;
{
	...
	if (svd2->vpage != NULL) {
		...
		kmem_free((caddr_t)svd2->vpage, vpgtob(seg_pages(seg2)));
	...
	struct seg *seg, *newseg;
{
	...
	if ((newsvd->amp = svd->amp) == NULL) {
		...
		if (svd->type == MAP_SHARED) {
			 ...
			 * softlocked.
			 */
			if (svd->softlockcnt) {	/* something is softlocked */
				struct anon **anonp, **newanonp;
				int i;
				u_int vpprot;
				page_t *anon_pl[1+1], *pp;
				caddr_t addr;

				newanonp = newsvd->amp->anon;
				anonp = amp->anon;
				i = btopr(seg->s_size);
				addr = seg->s_base;
				while (i-- > 0) {
					if (*anonp) {
					    error = anon_getpage(anonp, &vpprot,
				...
				anon_dup(&amp->anon[svd->anon_index],
				    newsvd->amp->anon, seg->s_size);
				hat_chgprot(seg->s_as, seg->s_base, seg->s_size,
				    ~PROT_WRITE);
			}
		}
	}
	...
	u_int len;
{
	...
	if (addr == seg->s_base) {
		...
		if (svd->swresv) {
			if (svd->flags & MAP_NORESERVE) {
				ASSERT(amp);
				oswresv = svd->swresv;
				svd->swresv = anon_pages(amp->anon,
	...
	if (addr + len == seg->s_base + seg->s_size) {
		...
		if (amp != NULL) {
			...
			if (amp->refcnt == 1 || svd->type == MAP_PRIVATE) {
				 ...
				 * Free up now unused parts of anon_map array
				 */
				app = &amp->anon[svd->anon_index + npages];
				anon_free(app, len);
			}
			mutex_exit(&amp->lock);
		}
	...
	if (amp == NULL) {
		...
		if (amp->refcnt == 1 || svd->type == MAP_PRIVATE) {
			 ...
			 * Free up now unused parts of anon_map array
			 */
			app = &amp->anon[svd->anon_index + opages];
			anon_free(app, len);
		}
		amp->refcnt++;
		mutex_exit(&amp->lock);
	...
	if (svd->swresv) {
		if (svd->flags & MAP_NORESERVE) {
			...
			nsvd->swresv = anon_pages(amp->anon,
	...
	struct seg *seg;
{
	 ...
	 */
	if (svd->vpage != NULL) {
		kmem_free((caddr_t)svd->vpage, vpgtob(npages));
		svd->vpage = NULL;
	}
	if ((amp = svd->amp) != NULL) {
		/*
		...
		if (--amp->refcnt == 0) {
			if (svd->type == MAP_PRIVATE) {
				 ...
				 */
				anon_free(&amp->anon[svd->anon_index],
				    seg->s_size);
			} else {
				/*
	...
	enum seg_rw rw;
{
	...
	for (adr = addr; adr < addr + len; adr += PAGESIZE) {
		...
		if (pp == NULL) {
			cmn_err(CE_PANIC,
			    "segvn_softunlock: addr %x, ap %x, vp %x, off %x",
			    (int)addr, (int)ap, (int)vp, offset);
		}

		if (rw == S_WRITE) {
			hat_setrefmod(pp);
			if (seg->s_as->a_vbits)
				(void) hat_setstat(seg->s_as, addr, 0x3);
		} else {
			if (rw != S_OTHER) {
				hat_setref(pp);
				if (seg->s_as->a_vbits)
					(void) hat_setstat(seg->s_as, addr,
					    0x2);
			}
		}
		TRACE_3(TR_FAC_VM, TR_SEGVN_FAULT,
			"segvn_fault:pp %x vp %x offset %x", pp, vp, offset);
		hat_unlock(seg->s_as->a_hat, seg->s_as, adr, PAGESIZE);
		page_unlock(pp);

		mutex_enter(&freemem_lock); /* for availrmem, pages_pp_kernel */
		availrmem++;
		pages_pp_kernel--;
		svd->softlockcnt--;
 ...
 */
static int
segvn_faultpage(hat, seg, addr, off, vpage, pl, vpprot, type, rw)
	...
	enum seg_rw rw;			/* type of access at fault */
{
	...
	u_int prot;
	int err;
	int cow, serial_lock = 0;
	int steal = 0;
	u_int anon_index;
	struct anon *ap, *oldap, **app;
	struct anon_map *amp;		/* XXX - for locknest */
		...
		if (ap == NULL) {
			...
			*app = ap;
			...
			if (type == F_SOFTLOCK) {
				 ...
				 */
				hat_memload(hat, seg->s_as, addr, pp, prot,
				    HAT_LOCK);
			} else {
				hat_memload(hat, seg->s_as, addr, pp, prot,
				    HAT_LOAD);
	...
	ASSERT(opp != NULL &&
	    (se_shared_assert(&opp->p_selock) ||
	    se_excl_assert(&opp->p_selock)));

	TRACE_3(TR_FAC_VM, TR_SEGVN_FAULT, "segvn_fault:pp %x vp %x offset %x",
	...
	if (cow == 0) {
		if (type == F_SOFTLOCK) {
			 ...
			 * locked and don't unlock the page.
			 */
			hat_memload(hat, seg->s_as, addr,
			    opp, prot & vpprot, HAT_LOCK);
		} else {
			hat_memload(hat, seg->s_as, addr,
			    opp, prot & vpprot, HAT_LOAD);
			page_unlock(opp);
		}
		if (serial_lock)
			mutex_exit(&amp->serial_lock);
	 ...
	 */
	if ((ap = *(app = &amp->anon[anon_index])) == NULL)
		steal = 1;
	 ...
	 */
	if (!PP_ISMOD(opp))
		hat_pagesync(opp, HAT_DONTZERO | HAT_STOPON_MOD);

	if (stealcow && freemem < minfree && steal &&
	    page_tryupgrade(opp) && !PP_ISMOD(opp) &&
		 ...
		 */
		if (!PP_ISMOD(opp) && opp->p_mapping == NULL)
	...
	*app = ap;
	...
	if (type == F_SOFTLOCK) {
		 ...
		 * locked and don't unlock the page.
		 */
		hat_memload(hat, seg->s_as, addr, pp, prot, HAT_LOCK);
	} else {
		hat_memload(hat, seg->s_as, addr, pp, prot, HAT_LOAD);
		page_unlock(pp);
	}
...
top:
	...
	if ((page > 0) && fltadvice) {	/* not if first page in segment */
		struct vpage *vpp;
		register u_int pgoff, fanon_index;
		int fpage;
		u_int fpgoff;
		struct vnode *fvp;
	...
	if (svd->vp != NULL) {
		...
		if (dogetpage) {
			...
			if (len > ptob((sizeof (pl) / sizeof (pl[0])) - 1)) {
				...
				plp = (page_t **)
				    kmem_alloc(pl_alloc_sz, KM_SLEEP);
				plp[0] = (page_t *)NULL;
				plsz = len;
			} else if (rw == S_WRITE && svd->type == MAP_PRIVATE ||
			    rw == S_OTHER ||
			    hat_probe(as->a_hat, as, addr + PAGESIZE)) {
				/*
			...
			TRACE_3(TR_FAC_VM, TR_SEGVN_GETPAGE,
				"segvn_getpage:seg %x addr %x vp %x",
				seg, addr, svd->vp);
	...
	for (ppp = plp; (pp = *ppp) != NULL; ppp++) {
		...
		if (diff >= 0 && diff < seg->s_size) {
			...
			if (amp != NULL) {
				...
				ap = amp->anon[svd->anon_index + page];
				if (ap == NULL) {
					HATOP_MEMLOAD(hat, seg->s_as,
					    seg->s_base + diff, pp, prot,
					    HAT_ADVLOAD);
				}
			} else {
				HATOP_MEMLOAD(hat, seg->s_as,
				    seg->s_base + diff, pp, prot, HAT_ADVLOAD);
			}
		}
		page_unlock(pp);
	}
	...
	caddr_t addr;
{
	...
	if ((amp = svd->amp) != NULL) {
		...
		ap = amp->anon[svd->anon_index + seg_page(seg, addr)];
		if (ap != NULL) {
			err = anon_getpage(&ap, (u_int*)NULL, (page_t **)NULL,
	...
	TRACE_3(TR_FAC_VM, TR_SEGVN_GETPAGE,
		"segvn_getpage:seg %x addr %x vp %x", seg, addr, svd->vp);
	...
	register u_int len, prot;
{
	...
	if (addr == seg->s_base && len == seg->s_size && svd->pageprot == 0) {
		...
		if ((amp = svd->amp) == NULL)
			...
			mutex_enter(&amp->lock);
			app = &amp->anon[svd->anon_index + seg_page(seg, addr)];
		}
	...
	int delta;
{
	...
	pd = delta / (int)PAGESIZE;	/* divide to preserve sign bit */
	page = seg_page(seg, addr);
	...
	if (svd->advice == MADV_RANDOM ||
		...
		if (bvpp->vp_advice == MADV_RANDOM ||
		    evpp->vp_advice == MADV_SEQUENTIAL && delta < 0)
			return (-1);
		if (bvpp->vp_advice != evpp->vp_advice &&
		    evpp->vp_advice == MADV_RANDOM)
			return (-1);
	}
	...
	struct seg *seg;
{
	...
	for (page = 0; page < npages; page++) {
		 ...
		 */
		if (pp->p_lckcnt != 0 || pp->p_cowcnt != 0 || pp->p_mapping) {
			page_unlock(pp);
			continue;
		}
	...
	for (vpp = &svd->vpage[seg_page(seg, addr)]; vpp < evp;
		...
		if ((attr == 0 || vpp->vp_prot == pageprot) &&
			...
			if (op == MC_LOCK && svd->vp == NULL &&
			    ...
			    amp != NULL && amp->anon[anon_index] == NULL) {
				mutex_enter(&amp->serial_lock);
				ap = *(app = &amp->anon[anon_index]);
				if (ap == NULL) {
