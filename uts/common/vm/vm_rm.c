...
#ident	"@(#)vm_rm.c	1.36	94/07/28 SMI"
...
rm_assize(struct as *as)
{
	...
	for (seg = AS_SEGP(as, as->a_segs); seg != NULL;
		...
		if (seg->s_ops == &segvn_ops &&
			...
			register caddr_t addr = seg->s_base;
			register caddr_t eaddr = addr + seg->s_size;
			char incore;
			...
			if (SEGOP_GETVP(seg, addr, &vp) == 0 &&
				...
				if (filesize < offset)
					...
					filesize -= offset;
				filesize = roundup(filesize, PAGESIZE);
				if (segsize > filesize)
					segsize = filesize;
			}
			size += segsize;
...
{
	/*
	 * This field is protected by the hat layer lock.
	 */
