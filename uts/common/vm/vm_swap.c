...
#ident	"@(#)vm_swap.c	1.61	95/02/10 SMI"
 ...
 */
int swap_maxcontig;	/* set by anon_init() */
...
static int	swap_getoff(struct swapinfo *);
static int	swapadd(struct vnode *, u_int, u_int, char *);
static int	swapdel(struct vnode *, int);
static int	swapslot_free(struct vnode *, u_int, struct swapinfo *);
	...
	u_int flags)
{
	...
	do {
		if (sip == NULL)
			break;
		if (!(sip->si_flags & ST_INDEL) && (int)sip->si_nfpgs > 0) {
...
{
	u_int *sp, *ep;
	register int aoff, boff, poff, slotnumber;
	...
	SWAP_PRINT(SW_ALLOC,
	    "swap_getoff: couldn't find slot from hint %d to end\n",
	    sip->si_hint, 0, 0, 0, 0);
	/*
...
foundentry:
	/*
	 * aoff is the page number offset of the si_swapslots
	...
	cmn_err(CE_PANIC, "swap_getoff: didn't find slot in word hint %d\n",
	    sip->si_hint);
...
foundslot:
	...
	SWAP_PRINT(SW_ALLOC, "swap_getoff: allocating slot %d\n",
	    slotnumber, 0, 0, 0, 0);
	poff = ptob(slotnumber);
	if (poff + sip->si_soff >= sip->si_eoff)
		printf("ptob(aoff(%d) + boff(%d))(%d) >= eoff(%d)\n",
		    aoff, boff, ptob(slotnumber), sip->si_eoff);
	...
	do {
		if (sip->si_vp == vp &&
			...
			for (pagenumber = btop(off - sip->si_soff),
				...
				SWAP_PRINT(SW_ALLOC,
				    "swap_phys_free: freeing slot %d on "
				    "sip %x\n",
 ...
 */
struct anon *
swap_anon(register struct vnode *vp, register u_int off)
{
 ...
 */
int
swap_in_range(register struct vnode *vp, register u_int offset, u_int len)
{
...
swapdel_byname(
	...
	register u_int	lowblk)	/* Low block number of area to delete. */
{
...
int
swapctl(register struct swapcmda *uap, rval_t *rvp)
{
	register struct swapinfo *sip, *csip, *tsip;
	register error = 0;
	...
	case SC_AINFO:
		...
		ai.ani_free = anoninfo.ani_free +
		    MAX(availrmem - swapfs_minfree, 0);
		ai.ani_resv = anoninfo.ani_resv;

		if (copyout((caddr_t)&ai, (caddr_t)uap->sc_arg,
		    sizeof (struct anoninfo)) != 0)
			return (EFAULT);
		return (0);
	...
	case SC_LIST:
		 ...
		 */
		csip = (struct swapinfo *)
			kmem_zalloc(length * sizeof (struct swapinfo),
			KM_SLEEP);
retry:
		nlen = length * (gplen += 100);
		...
		for (sip = swapinfo, tsip = csip, tpname = pname, nswap = 0;
		    sip && nswap < length;
		    /* LINTED lint for confusion: used before set: plen */
			...
			*tsip = *sip;
			...
			strcpy(tsip->si_pname, sip->si_pname);
		}
		mutex_exit(&swapinfo_lock);
		...
		for (tsip = csip, cnt = 0; cnt < nswap;  tsip++, ust++, cnt++) {
			if (copyin((caddr_t)ust, (caddr_t)&st,
			    sizeof (swapent_t)) != 0) {
				error = EFAULT;
				goto lout;
			}
			...
			if (copyout((caddr_t)&st, (caddr_t)ust,
			    sizeof (swapent_t)) != 0) {
				error = EFAULT;
				goto lout;
			}
		...
		rvp->r_val1 = nswap;
lout:
		kmem_free((caddr_t)csip, length * sizeof (struct swapinfo));
		kmem_free(pname, nlen);
		return (error);
 ...
 */
int
swapadd(struct vnode *vp, u_int lowblk, u_int nblks, char *swapname)
{
	register struct swapinfo **sipp, *nsip = NULL, *esip = NULL;
	struct vnode *cvp;
	struct vattr vattr;
	...
	SWAP_PRINT(SW_CTL, "swapadd: vp %x lowblk %d nblks %d swapname %s\n",
	...
	error = VOP_PAGEIO(cvp, (page_t *)NULL, 0, 0, 0, CRED());
	...
	if (cvp == rootdir)
		...
		startblk = lowblk ? lowblk : 1; /* Skip 1st page (disk label) */
	...
	SWAP_PRINT(SW_CTL, "swapadd: va_size %d soff %d eoff %d\n",
	...
	eoff = ptob(btop(eoff));
	if (soff >= eoff) {
		SWAP_PRINT(SW_CTL, "swapadd: soff %d >= eoff %d\n",
		    soff, eoff, 0, 0, 0);
		error = EINVAL;
		goto out;
	}
	...
	nsip = (struct swapinfo *)
	    kmem_zalloc(sizeof (struct swapinfo), KM_SLEEP);
	nsip->si_vp = cvp;
	...
	for (i = start; i < end; i++) {
		SWAP_PRINT(SW_CTL, "swap_add: set bit for page %d\n", i,
		    0, 0, 0, 0);
	...
	*sipp = nsip;
	...
	if (anoninfo.ani_resv > anoninfo.ani_max) {
		int returnmem =
		    MIN((int)(anoninfo.ani_resv - anoninfo.ani_max), pages);

		ANI_ADD(-returnmem);
		anoninfo.ani_free -= returnmem;
	...
	mem_resv = MAX((int)anoninfo.ani_resv - (int)anoninfo.ani_max, 0);
	/*
	...
	if (swapfs_minfree < swapfs_desfree) {
		mutex_enter(&freemem_lock);
		if (availrmem > swapfs_desfree || !mem_resv)
			swapfs_minfree = swapfs_desfree;
		mutex_exit(&freemem_lock);
	}

	SWAP_PRINT(SW_CTL, "swapadd: ani_max %d ani_free %d\n",
	    anoninfo.ani_free, anoninfo.ani_free, 0, 0, 0);
...
out:
	if (error || esip) {
		...
		if (nsip) {
			kmem_free(nsip->si_swapslots,
			    (size_t)nsip->si_mapsize);
...
{
	register struct swapinfo **sipp, *osip = NULL;
	 ...
	 */
	mutex_enter(&freemem_lock);
	if ((MAX((int)anoninfo.ani_max - (int)anoninfo.ani_resv, 0) +
	    MAX(availrmem - swapfs_minfree, 0)) < pages) {
		mutex_exit(&freemem_lock);
		error = ENOMEM;
	...
	if ((int)(anoninfo.ani_max - anoninfo.ani_resv) < pages) {
		availrmem -= MIN(pages,
		    pages - (int)(anoninfo.ani_max - anoninfo.ani_resv));
	}
...
top:
		for (ap = *app; ap != NULL; ap = ap->an_hash) {
			if (ap->an_pvp == cvp &&
				...
				if (!error && (osip->si_flags & ST_DOINGDEL)) {
					...
					mutex_enter(&anoninfo_lock);
					anoninfo.ani_max += pages;
					mem_resv = MAX((int)anoninfo.ani_resv -
					    (int)anoninfo.ani_max, 0);
					mutex_exit(&anoninfo_lock);
	...
	*sipp = osip->si_next;
	...
	kmem_free((caddr_t)osip, sizeof (*osip));
	...
	struct swapinfo *sip)
{
	 ...
	 */
	if (error = VOP_GETPAGE(vp, (offset_t)off, ptob(1), (u_int *)NULL,
	    pl, ptob(1), segkmap, (caddr_t)0, S_READ, CRED())) {
		/*
	...
	u_int *poffp)
{
	 ...
	 */
	start =
	    MAX(*offp, offset + PAGESIZE > plen ? offset + PAGESIZE - plen : 0);
