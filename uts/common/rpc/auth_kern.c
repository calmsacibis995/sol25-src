...
#ident	"@(#)auth_kern.c	1.15	95/01/26 SMI"	/* SVr4.0 1.7	*/
...
static bool_t	authkern_refresh(AUTH *, cred_t *);
...
authkern_marshal(AUTH *auth, XDR *xdrs, struct cred *cr)
{
	...
	register int gidlen, credsize, namelen, rounded_namelen;
	register long *ptr;
	...
	namelen = strlen(utsname.nodename);
	...
	if (ptr) {
		...
		IXDR_PUT_LONG(ptr, cr->cr_uid);
		IXDR_PUT_LONG(ptr, cr->cr_gid);
...
/* ARGSUSED */
static bool_t
authkern_refresh(AUTH *auth, cred_t *cr)
{
...
static void
authkern_destroy(register AUTH *auth)
{
