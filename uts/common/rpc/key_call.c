...
#ident	"@(#)key_call.c	1.17	95/10/20 SMI"	/* SVr4.0 1.9	*/
...
static enum clnt_stat key_call(u_long, xdrproc_t, char *, xdrproc_t, char *,
				cred_t *);
	...
	cred_t *cr;
{
	...
	if (stat = key_call((u_long)KEY_NET_GET, xdr_void, (char *) NULL,
			xdr_key_netstres, (char *) &kres, cr) != RPC_SUCCESS) {
		RPCLOG(1, "key_getnetname: timed out? stat = %d\n", stat);
		return (stat);
	}

	if (kres.status != KEY_SUCCESS) {
		RPCLOG(1, "key_getnetname: RPC_FAILED\n", 0);
		return (RPC_FAILED);
	}

	RPCLOG(2, "key_getnetname: RPC_SUCCESS\n", 0);
	return (RPC_SUCCESS);
}
...
key_call(u_long procn, xdrproc_t xdr_args, caddr_t args,
	xdrproc_t xdr_rslt, caddr_t rslt, cred_t *cr)
{
	...
	clnt_destroy(client);
	if (stat != RPC_SUCCESS) {
		RPCLOG(1, "key_call: keyserver clnt_call failed: stat %x",
		    stat);
