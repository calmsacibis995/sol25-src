...
#ident	"@(#)rpc_prot.c	1.15	95/01/26 SMI"	/* SVr4.0 1.2	*/
 ...
 */
bool_t
xdr_opaque_auth(XDR *xdrs, register struct opaque_auth *ap)
{
 ...
 */
bool_t
xdr_des_block(XDR *xdrs, register des_block *blkp)
{
 ...
 */
bool_t
xdr_replymsg_hdr(register XDR *xdrs, register struct rpc_msg *rmsg)
{
	register long *buf;
	register struct accepted_reply *ar;
	register struct opaque_auth *oa;
	...
	if ((buf = XDR_INLINE(xdrs, 6 * BYTES_PER_XDR_UNIT +
	    rmsg->rm_reply.rp_acpt.ar_verf.oa_length)) != NULL) {
		IXDR_PUT_LONG(buf, rmsg->rm_xid);
		IXDR_PUT_ENUM(buf, rmsg->rm_direction);
		IXDR_PUT_ENUM(buf, rmsg->rm_reply.rp_stat);
 ...
 */
bool_t
xdr_replymsg_body(register XDR *xdrs, register struct rpc_msg *rmsg)
{
	register struct accepted_reply *ar;
 ...
 */
bool_t
xdr_rpc_free_verifier(register XDR *xdrs, register struct rpc_msg *msg)
{
