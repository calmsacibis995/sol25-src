...
#pragma ident  "@(#)scsi_resource.c 1.49     94/09/30 SMI"
 ...
 * Callback id
 */
int	scsi_callback_id = 0;



struct buf *
scsi_alloc_consistent_buf(struct scsi_address *ap,
    struct buf *in_bp, int datalen, ulong bflags,
    int (*callback) (), caddr_t callback_arg)
{
	...
	if (datalen) {
		...
		if (ddi_iopb_alloc(pdip, (ddi_dma_lim_t *)0, (u_int)datalen,
		    &bp->b_un.b_addr)) {
			if (!in_bp)
				freerbuf(bp);
			goto no_resource;
...
scsi_init_pkt(struct scsi_address *ap, struct scsi_pkt *in_pktp,
    ...
    int flags, int (*callback)(), caddr_t callback_arg)
{
...
"scsi_init_pkt_start: addr %x in_pktp %x cmdlen %d statuslen %d pplen %d",
