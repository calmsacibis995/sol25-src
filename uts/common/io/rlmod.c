...
#pragma ident	"@(#)rlmod.c	1.12	94/12/16 SMI"
...
static int	rlmodopen(queue_t *q, dev_t *devp, int oflag, int sflag,
			cred_t *credp);
static int	rlmodclose(queue_t *q, int flag, cred_t *credp);
static int	rlmodrput(queue_t *q, mblk_t *mp);
static int	rlmodrsrv(queue_t *q);
static int	rlmodwput(queue_t *q, mblk_t *mp);
static int	rlmodwsrv(queue_t *q);
static int	rlmodrmsg(queue_t *q, mblk_t *mp);
static mblk_t	*make_expmblk(char cntl);
static void 	rlsendnak(queue_t *q, mblk_t *mp, int err);
static void 	rlack(queue_t *q, mblk_t *mp, mblk_t *dp, uint size);
static int 	rlwinctl(queue_t *q, mblk_t *mp);
static mblk_t	*rlwinsetup(queue_t *q, mblk_t *mp, unsigned char *blk);

static void	rlmod_timer(queue_t *q);
static void	rlmod_buffer(queue_t *q);
static void	recover(queue_t *, mblk_t *, int);
static void	recover1(queue_t *, int);
static int	tty_flow();
...
/*ARGSUSED*/
static void dummy_callback(int arg)
{}
...
rlmodopen(queue_t *q, dev_t *devp, int oflag, int sflag, cred_t *cred)
{
	 ...
	 */
	rmip = (struct rlmod_info *)kmem_zalloc(sizeof (*rmip), KM_SLEEP);

	/*
	 * Cross-link.
	 */
	q->q_ptr = (caddr_t)rmip;
	WR(q)->q_ptr = (caddr_t)rmip;
	rmip->rl_expdat = 0;
	...
	while ((bp = allocb(sizeof (union T_primitives), BPRI_HI)) == NULL) {
		...
		id = qbufcall(q, sizeof (union T_primitives), BPRI_HI,
			dummy_callback, 0);
		if (!qwait_sig(q)) {
			qunbufcall(q, id);
			error = EINTR;
...
rlmodrput(queue_t *q, mblk_t *mp)
{
	...
	TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_RPUT_IN, "rlmodrput start: "
	    "q %X, mp %X, db_type 0%o", q, mp, mp->b_datap->db_type);
	...
	if ((mp->b_datap->db_type < QPCTL) &&
		...
		TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_RPUT_OUT,
		    "rlmodrput end: q %X, mp %X, %s", q, mp, "flow");
		return (0);
	}
	...
	case M_PCPROTO:
		...
		default:
			...
			if (rlmoddebug)
				debug_enter(NULL);
#else
			strlog(RLMOD_ID, -1, 0, SL_ERROR,
			    "rlmodrput: got 0x%lx type M_PROTO/M_PCPROTO\n",
			    tip->type);
#endif
			freemsg(mp);
		}
	...
	TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_RPUT_OUT, "rlmodrput end: q %X, "
		"mp %X, %s", q, mp, "done");
	return (0);
}
...
rlmodrsrv(queue_t *q)
{
	...
	TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_RSRV_OUT, "rlmodrsrv end: q %X, "
	    "mp %X, %s", q, mp, "empty");
	...
	case M_PROTO:
		...
		default:
			...
			if (rlmoddebug)
				debug_enter(NULL);
#else
			strlog(RLMOD_ID, -1, 0, SL_ERROR, "rlmodwput: "
...
rlmodwsrv(queue_t *q)
{
	...
	TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_WSRV_OUT, "rlmodwsrv end: q %X, "
	    "mp %X, %s", q, mp, "done");
	return (0);
}
 ...
 */
static void
rlack(queue_t *q, mblk_t *mp, mblk_t *dp, uint size)
{
	struct iocblk  *iocp = (struct iocblk *)mp->b_rptr;
...
rlwinctl(queue_t *q, mblk_t *mp)
{
	...
	TRACE_3(TR_FAC_RLOGINP, TR_RLOGINP_WINCTL_IN, "rlwinctl start: q %X, "
	    "mp %X, db_type 0%o", q, mp, mp->b_datap->db_type);

	rmip->oobdata[0] &= ~TIOCPKT_WINDOW; /* we know he heard */

	if ((rl_msgp = allocb(sizeof (struct iocblk), BPRI_MED)) == NULL) {
		TRACE_2(TR_FAC_RLOGINP, TR_RLOGINP_WINCTL_OUT, "rlwinctl end: "
		    "q %X, mp %X, allocb failed", q, mp);
		return (0);
	}
	/*
	 * create an M_IOCTL message type.
	 */
	rl_msgp->b_datap->db_type = M_IOCTL;
	rl_msgp->b_cont = mp;
	iocbp = (struct iocblk *)rl_msgp->b_rptr;
	rl_msgp->b_wptr = rl_msgp->b_rptr + sizeof (struct iocblk);
	iocbp->ioc_cmd = TIOCSWINSZ;
...
rlwinsetup(queue_t *q, mblk_t *mp, unsigned char *blk)
{
	...
	if (left > 0) {
		 ...
		 */
		bcopy((char *)jmpmp, (char *)blk, left);
		mp->b_wptr = blk + left;
	} else
		mp->b_wptr = blk;
	return (mp);
	...
	int	size;
{
	...
	if (!(id = qbufcall(RD(q), size, BPRI_MED, rlmod_buffer, (long)q))) {
		id = qtimeout(RD(q), rlmod_timer, (caddr_t)q, SIMWAIT);
		if (q->q_flag & QREADR)
			rmip->rtimoutid = id;
		else
			rmip->wtimoutid = id;
