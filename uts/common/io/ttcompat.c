...
#ident	"@(#)ttcompat.c	1.37	95/01/09 SMI"	/* SVr4.0 1.7	*/
...
static int ttcompatopen(queue_t *q, dev_t *devp, int oflag, int sflag,
			cred_t *crp);
static int ttcompatclose(queue_t *q, int flag, cred_t *crp);
static void ttcompatrput(queue_t *q, mblk_t *mp);
static void ttcompatwput(queue_t *q, mblk_t *mp);
...
static void ttcompat_do_ioctl(ttcompat_state_t *tp, queue_t *q, mblk_t *mp);
...
#endif /* HAVE_WIERD_XENIX_FUNCTIONS */
...
/*ARGSUSED*/
static int
ttcompatopen(q, devp, oflag, sflag, crp)
	queue_t *q;
	dev_t *devp;
	int oflag, sflag;
	cred_t *crp;
{
	...
	tp = (ttcompat_state_t *) kmem_zalloc((int)sizeof (ttcompat_state_t),
			KM_SLEEP);
...
#ifdef HAVE_WIERD_XENIX_FUNCTIONS
	case LDSMAP:
		if (emsetmap == NULL)
			/* driver will NAK it */
			putnext(q, mp);
...
allocfailure:
	...
	tp->t_bufcallid = qbufcall(q, (u_int)sizeof (struct iocblk), BPRI_HI,
