...
#pragma ident	"@(#)ppp_diag.c 1.10	95/03/07 SMI"
...
static void *check_param(mblk_t *, int);
	...
	cred_t *credp;
{
	...
	dp = (ppp_diag_struct_t *) kmem_zalloc(sizeof (ppp_diag_struct_t),
	    KM_NOSLEEP);
	...
	ppp_diag_struct_t *dp;
{
	...
	case M_DATA:
		...
		outbuf(PPP_DG_RAW, (direc == UPSTR) ? "RECEIVE " : "SEND ",
		    dp);
		sprintf(idlin, "%d octets: ", msgdsize(mp));
	...
	ppp_diag_struct_t *dp;
{
	if (PPP_DG_MAX_OUTPUT - dp->outputbuflen - 1 <= 0)
		return;
	if (dp->diag_info.debuglevel & debuglevel) {
		strncpy(&dp->outputbuf[dp->outputbuflen], src,
	...
	ppp_diag_struct_t *dp;
{
	...
	if (dp->diag_info.media_type == pppAsync) {
		...
		if (filter_esc(direc, mp, dp) < 0) {
			...
			hexdump(PPP_DG_ERRREP, mp -> b_rptr,
			    msgdsize(mp), dp);
			outputline(dp);
			return;
		}
		...
		trunchexdump(PPP_DG_RFR, bufp, msgdsize(mp), PROTOHEXLIM,
		    dp);
		outbuf(PPP_DG_RFR, ") ", dp);
		bufp = mp-> b_rptr;
		cnt = msgdsize(mp);
 ...
 */
	...
	switch (proto) {
		...
		default:
			sprintf(valstr, "{Unrecognized protocol: %4x ", proto);
	...
	ppp_diag_struct_t *dp;
{
	...
	bcopy((caddr_t) &(*bufp)[2], (caddr_t) &tests, sizeof (tests));
	len = ntohs(tests);
	if (len != *cnt) {
		sprintf(idlin, "{Discr. LEN=%d, CNT=%d} ", len, *cnt);
	...
	sprintf(idlin, " ID=%02x ", id);
	outbuf(PPP_DG_INF, idlin, dp);

	sprintf(idlin, "LEN=%d ", len);
	outbuf(PPP_DG_INF, idlin, dp);
	*bufp += 4;
	*cnt -= 4;
	switch (code) {
	...
	ppp_diag_struct_t *dp;
{
	...
	*cnt -= 4;
	...
	case Authenticate:
		...
		plen = (int) (*bufp)[0];
		sprintf(idlin, "Peer-ID-Length= %d ", plen);
		outbuf(PPP_DG_OPT, idlin, dp);
		(*bufp)++;
		(*cnt)--;
		if (plen > *cnt-1) {
		...
		*cnt -= plen;
		...
		plen = (int) (*bufp)[0];
		sprintf(idlin, "Passwd-Length= %d ", plen);
		outbuf(PPP_DG_OPT, idlin, dp);
		(*bufp)++;
		(*cnt)--;
		if (plen > *cnt) {
	...
	case AuthenticateNak:
		...
		plen = (int) (*bufp)[0];
		sprintf(idlin, "Msg-Length= %d ", plen);
		outbuf(PPP_DG_OPT, idlin, dp);
		(*bufp)++;
		(*cnt)--;
		if (plen > *cnt) {
	...
	ppp_diag_struct_t *dp;
{
	...
	bcopy((caddr_t) (*bufp), (caddr_t) &templqr, sizeof (lqr));

	lqr = ntoh_LQM_pack_t(templqr);

	sprintf(idlin, "Magic-number=	%08x  ", lqr.magic_num);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "LastOutLQRs=	%u  ", lqr.lastOutLQRs);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "LastOutPackets= %u  ", lqr.lastOutPackets);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "LastOutOctets	%u  ", lqr.lastOutOctets);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerInLQRs=	%u  ", lqr.peerInLQRs);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerInPackets=	%u  ", lqr.peerInPackets);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerInDiscards= %u  ", lqr.peerInDiscards);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerInErrors=	%u  ", lqr.peerInErrors);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerInOctets=	%u  ", lqr.peerInOctets);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerOutLQRs=	%u  ", lqr.peerOutLQRs);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerOutPackets= %u  ", lqr.peerOutPackets);
	outbuf(PPP_DG_OPT, idlin, dp);
	sprintf(idlin, "PeerOutOctets=	%u ", lqr.peerOutOctets);
	outbuf(PPP_DG_OPT, idlin, dp);
	...
	*cnt -= 4;
	switch (code) {
		...
		case Response:
			...
			vsize = (int)(*bufp)[0];
			sprintf(idlin, "Value-size= %d ", vsize);
			outbuf(PPP_DG_OPT, idlin, dp);
			(*bufp)++;
			(*cnt)--;
			if (vsize > *cnt) {
	...
	ppp_diag_struct_t *dp;
{
	...
	sprintf(idlin, "MAG#=%08lx ", ntohl(testl));
	outbuf(PPP_DG_OPT, idlin, dp);
	(*bufp) += 4;
	(*cnt) -= 4;
	if (*cnt > 0) {
	...
	ppp_diag_struct_t *dp;
{
	...
	sprintf(idlin, "Rej_proto=%2x ", ntohs(tests));
	outbuf(PPP_DG_OPT, idlin, dp);
	(*bufp) += 2;
	(*cnt) -= 2;
	...
	ppp_diag_struct_t *dp;
{
	...
	while (*cnt > 0) {
		...
		case MaxReceiveUnit:
			...
			sprintf(idlin, "MRU=%d ", ntohs(tests));
			outbuf(PPP_DG_OPT, idlin, dp);
			*bufp += len;
			*cnt -= len;
			break;
		case AsyncControlMap:
			...
			sprintf(idlin, "ACCM=%08lx ", ntohl(testl));
			outbuf(PPP_DG_OPT, idlin, dp);
			*bufp += len;
			*cnt -= len;
			if (dp -> diag_info.media_type == pppAsync) {
		...
		case LinkQualityMon:
			...
			case pppLQM_REPORT:
				...
				bcopy((caddr_t) &(*bufp)[4+i],
				    (caddr_t) &testl, sizeof (testl));
				vall = ntohl(testl);
				sprintf(idlin, "Qual=LQR Rep-period=%lu ",
				    vall);
				outbuf(PPP_DG_OPT, idlin, dp);

				break;
			default:
				sprintf(idlin, "Unknown LQM=%x ", vals);
				outbuf(PPP_DG_ERR, idlin, dp);
			}
			*bufp += len;
			*cnt -= len;
		...
		case AddrCtrlCompress:
			...
			*cnt -= len;
			break;
		default:
			sprintf(idlin, "{Unknown OPTION=%x l=%x} ", type, len);
			outbuf(PPP_DG_ERR, idlin, dp);
			return (-1);
			break;
		}
 ...
 */
	...
	(void) kmem_free(dp, sizeof (ppp_diag_struct_t));
}
