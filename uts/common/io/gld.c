...
#pragma ident "@(#)gld.c	1.28	95/07/19 SMI"
...
_init(void)
{
	...
	rw_init(&gld_device_list.gld_rwlock, "gld device list lock",
	    RW_DRIVER, (void *)-1);
...
gld_register(dev_info_t *devinfo, char *devname, gld_mac_info_t *macinfo)
{
	...
	/* We hold too many mutexes in our interrupt processing */
	...
	if (glddev == NULL) {
		...
		if (glddev == NULL)
			return (DDI_FAILURE);
		strcpy(glddev->gld_name, devname);
		...
		rw_init(&glddev->gld_rwlock, "gld per dev class lock",
			    RW_DRIVER, (void *) -1);
	...
	if (ddi_dev_nregs(devinfo, &nregs) == DDI_SUCCESS) {
	...
	sprintf(minordev, "%s%d", devname, macinfo->gldm_ppa);
	ddi_create_minor_node(devinfo, minordev, S_IFCHR, macinfo->gldm_ppa + 1,
				DDI_NT_NET, 0);
	macinfo->gldm_devinfo = devinfo;
	mutex_init(&macinfo->gldm_maclock, "gld per dev module lock",
			MUTEX_DRIVER, macinfo->gldm_cookie);
	...
	localetheraddr((struct ether_addr *)macinfo->gldm_macaddr, NULL);
...
gld_unregister(gld_mac_info_t *macinfo)
{
	...
	if (ddi_dev_nintrs(devinfo, &nintrs) == DDI_SUCCESS) {
		if (nintrs && macinfo->gldm_intr != NULL)
			ddi_remove_intr(devinfo, macinfo->gldm_irq_index,
					macinfo->gldm_cookie);
	}
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDPROT)
		cmn_err(CE_NOTE, "gld_bind: ok - type = %d", gld->gld_type);
#endif
 ...
 */
int
gld_recv(register gld_mac_info_t *macinfo, mblk_t *mp)
{
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDTRACE)
		cmn_err(CE_NOTE, "gld_recv(%x, %x)", mp, macinfo);
#endif
	...
	case DL_ETHER:
		...
		if (gld_debug & GLDRECV) {
			cmn_err(CE_CONT, "gld_recv: machdr=<");
			for (i = 0; i < (sizeof (struct ether_header) + 4); i++)
				cmn_err(CE_CONT, "%s%x", (i == 0) ? "" : ":",
					*((unsigned char *) ehdr + i));
			cmn_err(CE_CONT, ">\n");
		}
#endif
		break;
	...
	for (gld = glddev->gld_str_next;
		...
		if (gld_debug & GLDRECV)
			cmn_err(CE_NOTE,
			    "gld_recv: type=%d, sap=%x, pkt-dsap=%x",
			    gld->gld_type, gld->gld_sap, msgsap);
#endif
		...
		if (stream_valid &&
		    ...
		    (gld->gld_sap == msgsap) ||
		    (gld->gld_sap < GLD_802_SAP && msgsap < GLD_802_SAP))) {
			/* sap matches */
			if (!canput(gld->gld_qptr)) {
#ifdef GLD_DEBUG
				if (gld_debug & GLDRECV)
					cmn_err(CE_WARN,
					    "gld_recv: canput failed");
#endif
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDRECV)
		cmn_err(CE_NOTE, "gld_recv: queued message to %x (%d)",
			gld->gld_qptr, gld->gld_minor);
#endif
