...
#pragma ident	"@(#)ppp_str.c 1.27	94/03/31 SMI"
...
static void dev_tld(pppLink_t *);
static void memory_retry(queue_t *);
 ...
 */
char ppp_module_name[] = "async/sync ppp v1.27";
...
ppp_initialise()
{
	...
	rw_init(&miblock, "PPP MIB lock", RW_DEFAULT, DEFAULT_WT);
	...
	case M_DATA:
		...
		case pppVJ_UNCOMP_TCP:
			...
			if (lp -> link_up) {
				...
				mutex_exit(&lp->lplock);
				PPP_STRDBG("Sending up packet, size = %d\n",
					msgdsize(mp));
				putnext(q, mp);
				continue;
			}
...
#ifndef _SunOS4
	lp->memouts = timeout(memory_retry, (caddr_t) q, PPP_MEMRETRY);
#else
	lp->memouts++; timeout(memory_retry, (caddr_t) q, PPP_MEMRETRY);
#endif
}
 ...
 */
	mutex_init(&lp->lplock, "PPP link lock", MUTEX_DEFAULT, DEFAULT_WT);
