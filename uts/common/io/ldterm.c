...
#ident	"@(#)ldterm.c	1.68	95/01/12 SMI"	/* SVr4.0 1.36	*/
 ...
 * publication.
 *
 * (c) 1986,1987,1988,1989  Sun Microsystems, Inc (c)
...
static int	ldtermopen(queue_t *q, dev_t *devp, int oflag, int sflag,
				cred_t *crp);
static int	ldtermclose(queue_t *q, int cflag, cred_t *crp);
static void	ldtermrput(queue_t *q, mblk_t *mp);
static void	ldtermrsrv(queue_t *q);
static int	ldtermrmsg(queue_t *q, mblk_t *mp);
static void	ldtermwput(queue_t *q, mblk_t *mp);
static void	ldtermwsrv(queue_t *q);
...
static mblk_t	*ldterm_dononcanon(mblk_t *bp, mblk_t *bpt, int ebsize,
					queue_t *q, ldtermstd_state_t *tp);
static int	ldterm_echo(unsigned char c, queue_t *q, int ebsize,
				ldtermstd_state_t *tp);
static void	ldterm_outchar(unsigned char c, queue_t *q, int bsize,
				ldtermstd_state_t *tp);
static void	ldterm_outstring(unsigned char *cp, int len, queue_t *q,
				int bsize, ldtermstd_state_t *tp);
static mblk_t	*newmsg(ldtermstd_state_t *tp);
static void	ldterm_msg_upstream(queue_t *q, ldtermstd_state_t *tp);
static void	ldterm_wenable(long addr);
static mblk_t	*ldterm_output_msg(queue_t *q, mblk_t *imp, mblk_t **omp,
					ldtermstd_state_t *tp, int bsize,
					int echoing);
static void	ldterm_flush_output(unsigned char c, queue_t *q,
					ldtermstd_state_t *tp);
static void	ldterm_dosig(queue_t *q, int sig, unsigned char c, int mtype,
				int mode);
static void	ldterm_do_ioctl(queue_t *q, mblk_t *mp);
static int	chgstropts(struct termios *oldmodep, ldtermstd_state_t *tp,
				queue_t *q);
static void	ldterm_ioctl_reply(queue_t *q, mblk_t *mp);
static void	vmin_satisfied(queue_t *q, ldtermstd_state_t *tp, int sendup);
static void	vmin_settimer(queue_t *q);
static void	vmin_timed_out(queue_t *q);
static void	ldterm_adjust_modes(ldtermstd_state_t *tp);
static void	ldterm_euc_erase(queue_t *q, int ebsize,
				ldtermstd_state_t *tp);
static void	ldterm_eucwarn(ldtermstd_state_t *tp);
static void	cp_eucwioc(eucioc_t *from, eucioc_t *to, int dir);
static int	ldterm_memwidth(unchar c, eucioc_t *w);
static int	ldterm_dispwidth(unchar c, eucioc_t *w, int mode);
static int	ldterm_codeset(unchar c);
...
ldtermopen(queue_t *q, dev_t *devp, int oflag, int sflag, cred_t *crp)
{
	...
	bzero((caddr_t) & tp->eucwioc, EUCSIZE);
	...
	/*
	 * Formulate an M_CTL message; The first block looks like an
	 * iocblk. Set the command and datasize. The actual data will
	 ...
	 * NODELAY" mode.
	 */
	while ((bp = allocb((int) sizeof (struct stroptions), BPRI_MED)) ==
	    NULL) {
		int id;
...
ldtermclose(queue_t *q, int cflag, cred_t *crp)
{
	...
	while ((datap = allocb((int) sizeof (int), BPRI_MED)) == NULL) {
		...
		id = qbufcall(q, sizeof (int), BPRI_MED, dummy_callback, 0);
		if (!qwait_sig(q)) {
			qunbufcall(q, id);
			freeb(bp);
			goto tidy_up;
...
tidy_up:
	...
	if (tp->t_state & (TS_TBLOCK|TS_FBLOCK)) {
		tp->t_state &= ~(TS_TBLOCK|TS_FBLOCK);
...
ldtermrput(queue_t *q, mblk_t *mp)
{
	...
	case M_CTL:
		...
		case MC_PART_CANON:
			...
			if ((mp->b_cont->b_wptr - mp->b_cont->b_rptr) ==
				...
				bcopy((caddr_t) emodes, (caddr_t) &tp->t_dmodes,
					sizeof (struct termios));
	...
	case M_FLUSH:
		...
		if ((tp->t_modes.c_iflag & IXOFF) &&
		    ...
		    !(tp->t_state & TS_FBLOCK) && q->q_count <= TTXOLO) {
			tp->t_state &= ~TS_TBLOCK;
 ...
 */
static mblk_t *
ldterm_docanon(unchar c, mblk_t *bpt, int ebsize, queue_t *q,
    ldtermstd_state_t *tp)
{
 ...
 */
static void
ldterm_rubout(unchar c, queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
 ...
 */
static void
ldterm_erase(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int c;
 ...
 */
static void
ldterm_werase(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int c;
 ...
 */
static void
ldterm_tokerase(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int c, i;
	register unchar *ip;
...
static void
ldterm_kill(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int c, i;
	register unchar *ip;
 ...
 */
static void
ldterm_reprint(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register mblk_t *bp;
	register unsigned char *readp;
 ...
 */
static mblk_t *
ldterm_dononcanon(mblk_t *bp, mblk_t *bpt, int ebsize, queue_t *q,
    ldtermstd_state_t *tp)
{
	queue_t *wrq = WR(q);
 ...
 */
static int
ldterm_echo(unchar c, queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int i;
 ...
 */
static void
ldterm_outchar(unchar c, queue_t *q, int bsize, ldtermstd_state_t *tp)
{
 ...
 */
static void
ldterm_outstring(unchar *cp, int len, queue_t *q, int bsize,
    ldtermstd_state_t *tp)
{
...
ldtermwmsg(queue_t *q, mblk_t *mp)
{
	...
	tp->t_wbufcid = qbufcall(q, size, BPRI_MED, ldterm_wenable, (long) q);
...
ldterm_output_msg(queue_t *q, mblk_t *imp, mblk_t **omp,
    ldtermstd_state_t *tp, int bsize, int echoing)
{
	...
	case TCSETSF:
		{
			...
			bcopy((caddr_t) tp->t_amodes.c_cc,
				(caddr_t) tp->t_modes.c_cc, NCCS);
	...
	case TCSETAF:
		{
			...
			bcopy((caddr_t) cb->c_cc, (caddr_t) tp->t_modes.c_cc,
			    NCC);
			/* TCGETS returns amodes, so update that too */
			bcopy((caddr_t) cb->c_cc, (caddr_t) tp->t_amodes.c_cc,
			    NCC);
	...
	case EUC_WSET:
		...
		setgetfail2:
			...
			bcopy((caddr_t) mp->b_cont->b_rptr,
				(caddr_t) dmp_cont->b_wptr, EUCSIZE);
	...
	case TCGETA:
		{
			...
			cb->c_line = 0;
			bcopy((caddr_t) tp->t_amodes.c_cc, (caddr_t) cb->c_cc,
			    NCC);
			break;
		}
	}
...
vmin_settimer(queue_t *q)
{
	...
	tp->t_vtid = qtimeout(q, vmin_timed_out, (caddr_t) q,
				(int) (V_TIME * (HZ / 10)));
 ...
 */
static void
ldterm_euc_erase(queue_t *q, int ebsize, ldtermstd_state_t *tp)
{
	register int i, ung;
...
ldterm_eucwarn(ldtermstd_state_t *tp)
{
	...
	if ((tp->t_eucwarn > EUC_WARNCNT) && !(tp->t_state & TS_WARNED)) {
		cmn_err(CE_WARN,
		    "ldterm: tty at addr %x in multi-byte mode --\n",
			(int)tp);
		cmn_err(CE_WARN,
		    "Over %d bad EUC characters this session\n", EUC_WARNCNT);
