 ...
 */
#pragma ident "@(#)llc1.c	1.14	94/03/31 SMI"
/*
...
static int llc1_findminor(llc1dev_t *device);
...
extern void dlbindack(queue_t *, mblk_t *, u_long, u_char *, int, int,
	u_long);
...
llc1_attach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	if (!(llc1_device_list.llc1_status & LLC1_ATTACHED)) {
		...
		rw_init (&llc1_device_list.llc1_rwlock, "llc1 device list lock",
			RW_DRIVER, (void *) -1);
...
llc1_open(queue_t *q, dev_t *dev, int flag, int sflag, cred_t *cred)
{
	...
	if (q->q_ptr == NULL) {
		 ...
		 */
		mp = allocb (sizeof (llc1_t), BPRI_MED);
		llc1 = (llc1_t *) mp->b_rptr;
		bzero ((caddr_t) mp->b_rptr, sizeof (llc1_t));
		if (llc1 != NULL) {
			llc1->llc_mb = mp;
			llc1->llc_qptr = q;
				...
				if ((mcast = llc1->llc_mcast[i]) != NULL) {
					 ...
					 */
				   	if (llc1->llc_mac_info &&
					    llc1->llc_mac_info->llcp_flags & LLC1_AVAILABLE)
					  llc1_send_disable_multi(llc1->llc_mac_info,
							mcast);
		...
		case M_PCPROTO:
			...
			case DL_INFO_ACK:
				if (macinfo->llcp_flags & LLC1_LINKED) {
					...
					macinfo->llcp_maxpkt = prim->info_ack.dl_max_sdu;
					macinfo->llcp_minpkt = prim->info_ack.dl_min_sdu;
					macinfo->llcp_type = prim->info_ack.dl_mac_type;
					if (macinfo->llcp_type == DL_ETHER) {
						macinfo->llcp_type = DL_CSMACD;
						/* size of max header (including SNAP) */
					...
					bcopy ((caddr_t) mp->b_rptr +
					       ...
					       macinfo->llcp_addrlen);
					bcopy ((caddr_t) mp->b_rptr + prim->info_ack.dl_brdcst_addr_offset,
					       (caddr_t) macinfo->llcp_broadcast,
					       prim->info_ack.dl_brdcst_addr_length);

					if (prim->info_ack.dl_current_state == DL_UNBOUND)
					...
					case DL_CSMACD:
						/*
						 * raw mode is optimal so ask for it
						 * we might not get it but that's OK
						 */
						llc1_req_raw (macinfo);
						break;
					default:
						/* don't want raw mode so don't ask for it */
						break;
					}
				} else {
					if (prim->info_ack.dl_current_state == DL_IDLE)
			...
			case DL_ERROR_ACK:
				...
				if (prim->error_ack.dl_error_primitive == DL_BIND_REQ) {
					freemsg(mp);
			...
			case DL_PHYS_ADDR_ACK:
				llc1_find_waiting(macinfo, mp, DL_PHYS_ADDR_REQ);
				break;
			case DL_OK_ACK:
				if (prim->ok_ack.dl_correct_primitive == DL_BIND_REQ)
...
llc1_uwsrv(queue_t *q)
{
	...
	while ((mp = getq (q)) != NULL) {
...
#ifdef LLC1_DEBUG
					if (llc1_debug & LLCERRS)
						printf ("llc1_cmds: nonfatal err=%d\n",
							err);
#endif
					(void) putbq (q, mp);
					return (0);
		...
		case M_DATA:
			...
			if (putnext (lld->llc_mac_info->llcp_queue, mp)) {
				(void) putbq (q, mp);
	   ...
	   switch (lld->llc_type) {
	    case DL_CSMACD: /* 802.3 CSMA/CD */
	      ...
	      if (lld->llc_sap != LLC_NOVELL_SAP) {
		      ...
		      hdr->ether_type = ntohs (msglen + sizeof (struct llchdr));
...
llc1_recv(llc_mac_info_t * macinfo, mblk_t * mp)
{
	...
	if (DB_TYPE (mp) == M_PROTO) {
		...
		addr = (struct ether_addr *)(nmp->b_rptr + udata->dl_dest_addr_offset);
		...
		llchdr = (struct llchdr *) (mp->b_rptr + sizeof (struct ether_header));
	...
	if (msgsap == LLC_NULL_SAP) {
		...
		for (lld = llc1_device_list.llc1_str_next;
			 ...
			 * right MAC layer?
			 */
			if (lld->llc_qptr == NULL || lld->llc_state != DL_IDLE ||
			    lld->llc_mac_info != macinfo) {
				continue;
			}
#ifdef LLC1_DEBUG
			if (llc1_debug & LLCRECV)
				printf ("llc1_recv: type=%d, sap=%x, pkt-dsap=%x\n",
					lld->llc_type, lld->llc_sap,
					msgsap);
#endif
			if (!valid && ismulticast (addr->ether_addr_octet) &&
			    lld->llc_multicnt > 0 && llc1_multicast (addr, lld)) {
				valid |= 4;
			} else if (lld->llc_flags & LLC_PROM) /* promiscuous mode */
				valid = 1;

			if ((lld->llc_flags & LLC_PROM) || /* promiscuous streams */
			    (valid &&
			     (lld->llc_sap == msgsap || msgsap == LLC_GLOBAL_SAP))) {
				/* sap matches */
				if (msgsap == LLC_SNAP_SAP &&
				    (lld->llc_flags & (LLC_SNAP|LLC_PROM)) == LLC_SNAP){
					if (!llc1_snap_match(lld, (struct snaphdr *)(llchdr+1)))
						continue;
				}
				if (!canputnext (RD (lld->llc_qptr))) {
#ifdef LLC1_DEBUG
					if (llc1_debug & LLCRECV)
						printf ("llc1_recv: canput failed\n");
#endif
				...
				case LLC_XID | LLC_P:
					...
					if (lld->llc_flags & LLC1_AUTO_XID) {
						nmp = llc1_xid_reply (macinfo, mp, lld->llc_sap);
					} else {
						/*
				...
				case LLC_TEST | LLC_P:
					...
					if (lld->llc_flags & LLC1_AUTO_TEST) {
						nmp = llc1_test_reply (macinfo, mp, lld->llc_sap);
					} else {
						/*
						 * hand to the user for handling. if
						 * this is a "request", generate a
...
llc1attach(queue_t *q, mblk_t *mp)
{
	...
	for (mac = llc1_device_list.llc1_mac_next;
		...
		if (mac->llcp_ppa == at->dl_ppa) {
			...
			llc1insque(llc, llc1_device_list.llc1_str_prev);
...
llc1unattach(queue_t *q, mblk_t *mp)
{
	...
	llc1remque (llc);	/* removed from list (only if bound) */
	if (llc->llc_mac_info)
...
llc1_req_raw(llc_mac_info_t *macinfo)
{
	...
	DB_TYPE (mp) = M_IOCTL;
	iocp = (struct iocblk *) mp->b_rptr;
	iocp->ioc_cmd = DLIOCRAW;
	iocp->ioc_cr = &cred;
	iocp->ioc_id = 9999;	/* dummy */
	iocp->ioc_count = 0;
	cred.cr_ref = 1;
	putnext (macinfo->llcp_queue, mp);
	macinfo->llcp_flags |= LLC1_RAW_WAIT;
...
llc1_form_udata(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
	   ...
	   bcopy ((caddr_t) hdr->ether_shost.ether_addr_octet,
		  (caddr_t) LLCADDR (udata, udata->dl_src_addr_offset)->llca_addr,
		  macinfo->llcp_addrlen);
	   if (lld->llc_flags & LLC_SNAP) {
		   ...
		   LLCADDR (udata, udata->dl_src_addr_offset)->llca_sap = llchdr->llc_ssap;
	   }
...
llc1_xid_ind_con(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (raw) {
		if (MBLKL (mp) >
			...
			if (nmp->b_cont) {
				nmp->b_cont->b_rptr += sizeof (struct ether_header) +
					sizeof (struct llchdr);
			}
		}
	} else if (mp->b_cont != NULL && MBLKL (mp->b_cont) > sizeof (struct llchdr)) {
		nmp->b_cont = dupmsg (mp->b_cont);
		adjmsg (nmp->b_cont, sizeof (struct llchdr));
	}
...
llc1_xid_req_res(queue_t *q, mblk_t *mp, int req_or_res)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
		hdr = (struct ether_header *) nmp->b_rptr;
		bcopy ((caddr_t) LLCADDR (xid, xid->dl_dest_addr_offset)->llca_addr,
...
llc1_test_ind_con(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (raw) {
		bcopy ((caddr_t) hdr->ether_shost.ether_addr_octet,
		       (caddr_t) LLCADDR (nmp->b_rptr, test->dl_src_addr_offset)->llca_addr,
...
llc1_test_req_res(queue_t *q, mblk_t *mp, int req_or_res)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
		hdr = (struct ether_header *) nmp->b_rptr;
		bcopy ((caddr_t) LLCADDR (test, test->dl_dest_addr_offset)->llca_addr,
		       (caddr_t) hdr->ether_dhost.ether_addr_octet, ETHERADDRL);
		bcopy ((caddr_t) macinfo->llcp_macaddr,
		       (caddr_t) hdr->ether_shost.ether_addr_octet, ETHERADDRL);
		hdr->ether_type = htons (sizeof (struct llchdr) + msgdsize (mp));
...
llc1_init_kstat(llc_mac_info_t *macinfo)
{
	...
	macinfo->llcp_kstatp = kstat_create ("llc", macinfo->llcp_ppa,
					     ...
					     sizeof (struct llc_stats) / sizeof (long),
					     0);
