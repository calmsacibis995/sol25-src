...
#pragma	ident	"@(#)iwscons.c	1.28	95/08/07 SMI"
/* #ident	"@(#)iwscons.c	1.16	92/11/09 SMI" */
...
static int	iwscninfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
		    void **result);
static int	iwscnidentify(dev_info_t *devi);
static int	iwscnattach(dev_info_t *devi, ddi_attach_cmd_t cmd);
...
static int	iwscnioctl(dev_t, int, int, int, cred_t *, int *);
static int	iwscnpoll(dev_t dev, short events, int anyyet,
 ...
 */
static int		srreset(wcd_data_t *wd, int flag, cred_t *cred);
static wcrlist_t	*srrm(wcrlist_t **rwlp, vnode_t *vp, int zap);
static wcd_data_t	*srilookup(int unit);
static wcd_data_t	*srialloc(int unit);
static void		sridealloc(wcd_data_t *wd);
static wcrlist_t	*srpush(wcrlist_t **wlpp, vnode_t *vp);
...
extern struct dev_ops iwscn_ops;
...
iwscnattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	if (!been_here) {
		been_here = 1;
		rw_init(&iwscn_lock, "iwscn", RW_DEFAULT, DEFAULT_WT);
	}
...
static int
iwscnioctl(dev_t dev, int cmd, int arg, int flag, cred_t *cred, int *rvalp)
{
	register wcd_data_t	*wd;
	register int		err = 0;
	file_t			*f;
	...
	case SRIOCSREDIR: {
		 ...
		 */
		mdp = (wcm_data_t *)kmem_alloc(sizeof (*mdp), KM_SLEEP);
		iwscn_wcm_data = mdp;
		 ...
		 */
		if ((err = VOP_IOCTL(f->f_vnode, I_PUSH, (int)"redirmod",
			FREAD|FKIOCTL, cred, rvalp)) != 0) {
			iwscn_thread = NULL;
			kmem_free((caddr_t)mdp, sizeof (*mdp));
			RELEASEF((int)arg);
			break;
		}
	...
	case I_POP: {
		...
		rw_enter(&iwscn_lock, RW_READER);
		wd = srilookup(getminor(dev));
		strioctl(wd->wd_list->wl_vp, I_LOOK, (int) modname, flag,
		    K_TO_K, cred, rvalp);
 ...
 */
static int
srreset(wd, flag, cred)
	wcd_data_t	*wd;
	int	flag;
	cred_t	*cred;
{
	register wcrlist_t	*wlp;
	register int		err = 0;
...
srrm(rwlp, vp, zap)
	...
	int		zap;
{
	register wcrlist_t	**delwlp;
	register wcrlist_t	*wlp;
	register wcm_data_t	*mdp;
...
srilookup(unit)
	int	unit;
{
	register wcd_data_t	*wd = wcddata;
...
srialloc(unit)
	int	unit;
{
	register wcd_data_t	*wdp;
	register wcd_data_t	**wdpp;
	 ...
	 */
	wdp = (wcd_data_t *) kmem_zalloc(sizeof (*wdp), KM_SLEEP);
	wdp->wd_unit = unit;
	wdp->wd_next = *wdpp;
	*wdpp = wdp;
...
srpush(wlpp, vp)
	...
	vnode_t		*vp;
{
	register wcrlist_t	*nwlp;

	ASSERT(RW_WRITE_HELD(&iwscn_lock) || MUTEX_HELD(&iwscn_open_lock));
	if ((nwlp = srrm(wlpp, vp, 0)) == NULL) {
		nwlp = (wcrlist_t *) kmem_zalloc(sizeof (*nwlp), KM_SLEEP);
		nwlp->wl_vp = vp;
		/*
