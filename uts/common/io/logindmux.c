...
#pragma ident	"@(#)logindmux.c	1.10	94/12/06 SMI"
...
static int logdmuxopen(), logdmuxclose(), logdmuxursrv(), logdmuxuwput();
static int logdmuxlrput(), logdmuxlrsrv(), logdmuxlwsrv(), logdmuxuwsrv();

static void logdmuxlink();
static void logdmuxunlink();
static void recover();
static void flushq_dataonly();

#define	LOGDMX_ID	107
#define	SIMWAIT		(1*HZ)

static struct module_info logdmuxm_info =
		{LOGDMX_ID, "logindmux", 0, 256, 512, 256};

static struct qinit logdmuxurinit = { NULL, logdmuxursrv, logdmuxopen,
	logdmuxclose, NULL, &logdmuxm_info, NULL};
static struct qinit logdmuxuwinit = { logdmuxuwput, logdmuxuwsrv, NULL, NULL,
	NULL, &logdmuxm_info, NULL};
static struct qinit logdmuxlrinit = { logdmuxlrput, logdmuxlrsrv, NULL, NULL,
	NULL, &logdmuxm_info, NULL};
static struct qinit logdmuxlwinit = { NULL, logdmuxlwsrv, NULL, NULL, NULL,
	&logdmuxm_info, NULL};

struct streamtab logdmuxinfo = { &logdmuxurinit, &logdmuxuwinit, &logdmuxlrinit,
	&logdmuxlwinit };


static int logdmux_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
		void **result);
...
static 	void	logdmux_sndnak();
static	void	logdmux_sndack();
...
struct tmx {
	...
	int		wbufcid;	/* needed for recovery */
	int		rbufcid;	/* needed for recovery */
	int		wtimoutid;	/* needed for recovery */
	int		rtimoutid;	/* needed for recovery */
};
...
/*ARGSUSED*/
static int
logdmuxopen(q, devp, flag, sflag, crp)
queue_t *q;
dev_t	*devp;
int	flag, sflag;
cred_t	*crp;
...
logdmuxuwput(queue_t *q, mblk_t *mp)
{
	...
	case M_IOCTL:
		...
		case LOGDMX_IOC_QEXCHANGE:
			...
			protop = (struct protocol_arg *)mp->b_cont->b_rptr;
			((struct tmx *)(q->q_ptr))->dev1 =
				getminor(protop->dev);
			minor = ((struct tmx *)(q->q_ptr))->dev1;
			qexchange = protop->flag;
 ...
 */
static void
logdmux_sndack(queue_t *q, mblk_t *mp, mblk_t *dp, uint size)
{
	register struct iocblk	*iocp = (struct iocblk *)mp->b_rptr;
...
static void
recover(queue_t *q, mblk_t *mp, int size)
{
	int	id;
	struct	tmx	*saveq = (struct tmx *)q->q_ptr;
	...
	if (!(id = qbufcall(RD(q), size, BPRI_MED, logdmux_buffer, (long)q))) {
		id = qtimeout(RD(q), logdmux_timer, (caddr_t)q, SIMWAIT);
		if (q->q_flag & QREADR)
			saveq->rtimoutid = id;
		else
			saveq->wtimoutid = id;
