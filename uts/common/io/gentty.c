...
#ident	"@(#)gentty.c	1.24	95/03/25 SMI" /* from S5R4 1.22 */
...
int syopen(dev_t *devp, int flag, int otyp, cred_t *credp);
int syread(dev_t dev, struct uio *uio_p, cred_t *credp);
int sywrite(dev_t dev, struct uio *uio_p, cred_t *credp);
int sypoll(dev_t dev, short events, int anyyet, short *reventsp,
		struct pollhead **phpp);
int syioctl(dev_t dev, int cmd, int arg, int mode, cred_t *credp, int *rvalp);

static int sy_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
		void **result);
static int sy_identify(dev_info_t *devi);
static int sy_attach(dev_info_t *devi, ddi_attach_cmd_t cmd);
	...
	struct cred *cr;
{
	...
	if (IS_STREAM(ttyd))
		error = stropen(ttyvp, &ttyd, flag, cr);
	else
