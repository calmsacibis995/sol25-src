...
#ident	"@(#)tty_common.c	2.47	94/07/03 SMI" /* from SunOS 4.0 X.Y */
...
static int termioval(char **, u_int *, char *);
	...
	int *errorp;
{
	...
	*errorp = 0;	/* no error detected yet */
	...
	case TCGETS: {
		...
		if ((datap = allocb((int)sizeof (struct termios),
		    BPRI_HI)) == NULL) {
	...
	case TCGETA: {
		...
		if ((datap = allocb((int)sizeof (struct termio),
		    BPRI_HI)) == NULL) {
	...
	case TIOCGWINSZ: {
		...
		if ((datap = allocb((int)sizeof (struct winsize),
		    BPRI_HI)) == NULL) {
...
ttyinit()
{
	...
	if (ddi_getlongprop(DDI_DEV_T_ANY, ddi_root_node(), 0,
	    property, (caddr_t) &modesp, &len) != DDI_PROP_SUCCESS) {
		...
		if (ddi_prop_create(DDI_DEV_T_NONE, dip, 0,
		    property, (caddr_t) &default_termios,
	 ...
	 */
	bzero((caddr_t) &new_termios, sizeof (struct termios));
	tp = &new_termios;
	cp = modesp;
	for (i = 0; i < NFIELDS; i++) {
		/*
		 ...
		 */
		if (termioval(&cp, &val, modesp+len) == -1) {
			cmn_err(CE_WARN,
	 ...
	 * autoconfig'ed, we must use e_ddi_prop_modify().
	 */
	if (e_ddi_prop_modify(DDI_DEV_T_NONE, dip, 0, property,
	    (caddr_t) tp, sizeof (struct termios)) != DDI_PROP_SUCCESS) {
		cmn_err(CE_PANIC, "ttyinit: cannot modify '%s' property\n",
		    property);
	}
	kmem_free(modesp, len);
}

/*
 * Convert hex string representation of termios field
 * to a u_int.  Increments string pointer to the next
 * field, and assigns value. Returns -1 if no more fields
 * or an error.
 */

static int
termioval(sp, valp, ep)
	char **sp;
	u_int *valp;
	char *ep;
{
