...
#pragma ident "@(#)ipdcm.c 1.17 95/02/28 Copyright 1993 SMI"
...
ipd_init()
{
	...
	if (ipdpriminfo == NULL) {
		ipdpriminfoinit();
		mutex_init(&ipd_mutex, "IP/dialup lock", MUTEX_DEFAULT,
				DEFAULT_WT);
	}
}
...
/* ARGSUSED */
static int
ipdcminfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
	void **result)
{
	dev_t		dev = (dev_t) arg;
	int		instance, rc;
...
ipd_dlunitdatareq(struct ipd_str *stp, queue_t *wq, mblk_t *mp)
{
	...
	if (!MBLKIN(mp, off, len) || (len != addr_len)) {
		dluderrorind(wq, mp, mp->b_rptr + off, len, DL_BADADDR);
		return (0);
	}
	 ...
	 */
	if (mp->b_cont == NULL) {
		dluderrorind(wq, mp, mp->b_rptr + off, len, DL_BADDATA);
		return (0);
	}
