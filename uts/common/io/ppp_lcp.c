...
#pragma ident	"@(#)ppp_lcp.c 1.15	94/02/18 SMI"
...
pppOption_t	*op;
{
	...
	case MagicNumber:
		bcopy((caddr_t) op -> data, (caddr_t) &templ,
			sizeof (u_int));
	...
	case LinkQualityMon:
		...
		case pppLQM_REPORT:
			bcopy((caddr_t) op -> data,
			    (caddr_t) &templqm, sizeof (templqm));
...
pppOption_t	*op;
{
	...
	bcopy((caddr_t) op -> data, (caddr_t) &tempm,
	    sizeof (tempm));
	...
	if (machp -> linkp-> conf.pppLinkNAKMagicNumber != magic_num) {
		...
		bcopy((caddr_t) &tempm, (caddr_t) op -> data, sizeof (tempm));
...
{
	u_short			tempq;
	lqm_op_t		lqm_data, templqm;

	bcopy((caddr_t) op -> data, (caddr_t) &tempq, sizeof (tempq));

	switch (ntohs(tempq)) {
		case pppLQM_REPORT:
			bcopy((caddr_t) (op -> data), (caddr_t) &templqm,
			    sizeof (templqm));
			lqm_data = ntoh_lqm_op_t(templqm);
			machp -> linkp -> lqmstruct.out_rep_period =
			    lqm_data.rep_period;
			machp -> linkp-> conf.pppLinkOutLQM = pppLQM_REPORT;
			machp -> linkp-> outLQM = pppLQM_REPORT;
			return (OPT_OK);
	}
	return (OPT_NOK);
}
...
pppOption_t	*op;
{
	...
	bcopy((caddr_t) op -> data, (caddr_t) &tempm, sizeof (tempm));
	magic_num = ntohl(tempm);
	...
	if (machp-> linkp -> conf.pppLinkREQMagicNumber != magic_num) {
		...
		machp -> linkp -> looped_back = 1;
		if (machp -> linkp -> conf.pppLinkLoopCount--) {
			magic_num = ppp_rand();
			tempm = htonl(magic_num);
			bcopy((caddr_t) &tempm, (caddr_t) op -> data,
				sizeof (tempm));
 ...
 */
		bcopy((caddr_t) new_opt->data, (caddr_t) &temps,
		    sizeof (temps));
 ...
 * propose new option
 */
		bcopy((caddr_t) new_opt, (caddr_t) op, new_opt->length);
		bcopy((caddr_t) new_opt->data, (caddr_t) &temps,
		    sizeof (temps));
 ...
 */
	bzero((caddr_t) lcp_imp_opts, sizeof (lcp_imp_opts));
