...
#pragma ident	"@(#)telmod.c	1.11	94/12/16 SMI"
...
_init()
{
	...
	ret_code = mod_install(&modlinkage);
	return (ret_code);
}
...
static int	telmodopen(queue_t *q, dev_t *devp, int oflag, int sflag,
			cred_t *credp);
static int	telmodclose(queue_t *q, int flag, cred_t *credp);
static int	telmodrput(queue_t *q, mblk_t *mp);
static int	telmodrsrv(queue_t *q);
static int	telmodwput(queue_t *q, mblk_t *mp);
static int	telmodwsrv(queue_t *q);
static void 	telnak();
static void 	telack();
static int	rcv_parse(queue_t *q, mblk_t *mp);
static int	snd_parse(queue_t *q, mblk_t *mp);
static void	telmod_timer();
static void	telmod_buffer();
...
{
	register struct telmod_info	*tmip;
	register mblk_t *bp;
	 ...
	 */
	tmip = (struct telmod_info *)kmem_zalloc(sizeof (*tmip), KM_SLEEP);

	/*
	 * Cross-link.
	 */
	q->q_ptr = (caddr_t)tmip;
	WR(q)->q_ptr = (caddr_t)tmip;
...
/*ARGSUSED*/
static int
telmodclose(register queue_t *q, int flag, cred_t *credp)
{
	...
	case M_PCPROTO:
		...
		default:
				...
				if (teldebug)
					debug_enter(NULL);
#else
				strlog(TELMOD_ID, -1, 0, SL_ERROR,
				    "telmodrput: unexpected TLI"
				    " primitive 0x%lx msg type\n", tip->type);
				freemsg(mp);
#endif
		}
		break;
 ...
 */
static void
telack(queue_t *q, mblk_t *mp, mblk_t *dp, uint size)
{
	register struct iocblk  *iocp = (struct iocblk *)mp->b_rptr;
		...
		while (tmp < mp->b_wptr) {
			...
			if (tmp[0] == IAC) {
				...
				if (tmp > mp->b_rptr) {
					...
					ASSERT(tmp >= protomp->b_datap->db_base);
...
recover(queue_t *q, mblk_t *mp, int size)
{
	...
	if (!(id = qbufcall(RD(q), size, BPRI_MED, telmod_buffer, (long)q))) {
		id = qtimeout(RD(q), telmod_timer, (caddr_t)q, SIMWAIT);
		if (q->q_flag & QREADR)
			tmip->rtimoutid = id;
		else
			tmip->wtimoutid = id;
