...
#ident	"@(#)fd_asm.s	1.18	95/06/26 SMI"
...
#define	TRIGGER	0x33
	...
	!
	sethi	%hi(fdctlrs), Fdc	! load list of controllers
	ld	[Fdc + %lo(fdctlrs)], Fdc	! get the first in the list...
...
3:	ldub	[Fdc + FD_OPMODE], Tmp2	! load opmode into Tmp2
	...
	bnz	.mutex_enter		! yes!
	nop
	ld	[Fdc + FD_NEXT], Tmp	! Try next ctlr...
	tst	Tmp
	bnz,a	1b
	or	%g0, Tmp, Fdc
					! no more controllers
	or	%g0, 0x2, Tmp2		! must be spurious or "ready" int
.mutex_enter:
	!
	...
	be,a	.opmode3		! yes...
	ld	[Fdc + FD_REG], Reg	! load pointer to h/w registers
