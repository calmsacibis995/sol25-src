...
#ident	"@(#)fpu.c	1.36	95/07/25 SMI"	/* SunOS-4.1 1.9 89/08/07 */
...
static void fp_free(struct fpu *);
static void fp_fork(kthread_id_t, kthread_id_t);

extern void installctx(kthread_id_t, int,
			void(*)(), void(*)(), void(*)(), void(*)());
	...
	kthread_id_t t, ct;
{
	...
	installctx(ct, (int)cfp, fp_save, fp_restore, fp_fork, fp_free);
}
...
fp_installctx(klwp_t *lwp, struct fpu *fp)
{
	...
	installctx(lwptot(lwp), (int)fp, fp_save, fp_restore, fp_fork, fp_free);
}
	...
	struct regs *rp;
{
	...
	if (fpu_exists) {
		...
		if (fp->fpu_en) {
			 ...
			 */
			installctx(curthread, (int)fp, fp_save, fp_restore,
					fp_fork, fp_free);
			CPU->cpu_fpowner = lwp;
			fp_enable(fp);
		}
		...
		if (!fp->fpu_en) {
			...
			fp->fpu_fsr = 0;
			for (i = 0; i < 32; i++)
				fp->fpu_fr.fpu_regs[0] = (u_int)-1; /* NaN */
		}
