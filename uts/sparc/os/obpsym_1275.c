...
#pragma ident	"@(#)obpsym_1275.c	1.8	95/07/12 SMI"
...
ieee_sym_to_value(cell_t *cif)
{
	...
	if (nresults > 1) {
		cif[3 + nargs + 1] = p1275_uint2cell((u_int)symvalue);
...
ieee_value_to_sym(cell_t *cif)
{
	...
	u_int	offset;
	char	*name = symbol;
	extern u_int value_to_name(u_long value, char *symbol);
	...
	*name = (char)0;
	offset = (u_int)-1;
	if (nargs != 0) {
		value = (u_long)p1275_cell2uint(cif[3]); /* argument 0 */
