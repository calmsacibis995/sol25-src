...
#pragma ident	"@(#)rootnex.c	1.30	94/05/09 SMI"	/* SVr4 */
 ...
 * rootnex_identify:
 *
 * 	identify the root nexus for a sun4 machine.
 */
static int
...
rootnex_map_regspec(ddi_map_req_t *mp, caddr_t *vaddrp)
{
	...
	u_int bustype, npages, pfn, pgoffset;
	...
	/*
	 * For this implementation, we have several non OBMEM
	 ...
	 *		bustype = OBMEM;
	 *
	 * if somebody wants to map memory (eg mem.c)
	 */

	bustype = rp->regspec_bustype;
	switch (bustype) {
	case VME_D16:
	case VME_D32:
	case OBIO:
		pfn = MAKE_PGT(bustype) | mmu_btop(base);
		break;
	default:
		return (DDI_ME_INVAL);
	}
	...
	caddr_t int_handler_arg, int kind)
{
	...
	if (kind == IDDI_INTR_TYPE_FAST) {
		...
		/*
		 * Convert soft priority to "fit" with 4m model.
		 *
		 * XXX	Ugh. Child nexi handling 'interrupts' may have
		 *	translated their intrspec_pri's to '4m' form also
		 *	- so we force them back to what we understand on
		 *	this platform.
		 * XXX	One way to fix this is to make the 'interrupts'
		 *	translation propagate to the rootnex where we can
		 *	trim off any bits we don't understand.
		 */
	...
	struct devpage *dp, u_int pfn, u_int prot, u_int lock)
{
	...
	if (seg->s_ops == &segdev_ops) {
		if (hat == NULL) {
			 ...
			 * the hat of the system MMU.  An alternative
...
/*
 * DMA routines- for all sunmmu machines.
 */
...
#define	IOCACHE	ioc
extern int ioc;
static void rnex_ioc_kaddr(u_long, u_int, int);
#define	IOCACHED_SYS_VIRTUAL	\
	((mp->dmai_rflags & (DMP_LKSYSV|DMP_IOCACHE)) == \
	(DMP_LKSYSV|DMP_IOCACHE))
#else

#define	IOCACHE	0
#define	IOC_LINEMASK	0
#define	IOC_LINESIZE	0

#undef	DMP_IOCACHE
#define	DMP_IOCACHE	0

#define	rnex_ioc_kaddr(a, b, c)
#define	check_ioctag(a)
#define	ioc_flush(x)
#define	IOCACHED_SYS_VIRTUAL	0

#endif

extern void check_ioctag(caddr_t);
	...
	struct ddi_dma_req *dmareq, ddi_dma_handle_t *handlep)
{
	...
	/*
	 * If we are going to IO cache something, make sure the minimum
	 * xfer size is at least an IOC_LINESIZE amount.
	 */
	if (IOCACHE && (dmareq->dmar_flags & DDI_DMA_CONSISTENT) == 0) {
		dma_lim->dlim_minxfer =
		    max(dma_lim->dlim_minxfer, IOC_LINESIZE);
	}
	...
	if (handlep) {
		...
		mp = (ddi_dma_impl_t *)kmem_fast_alloc(&dmaimplbase,
		...
		if (vac & VAC_IOCOHERENT) {
			mp->dmai_rflags |= (DMP_USEHAT|DMP_NOSYNC);
		} else {
	 ...
	 *
	 * If we have an IO cache, see whether or not we can mark
	 * the transfer for enabling it.
	 *
	 * An implicit assumption is made that there is no IO cache for
	 * non VAC_IOCOHERENT machines.
	 */

	if (IOCACHE && mp && (mp->dmai_rflags & DDI_DMA_CONSISTENT) == 0 &&
	    ((size | offset) & IOC_LINEMASK) == 0) {
		...
		if (mp) {
			...
			if (klocked == 1) {
				if (mp->dmai_rflags & DMP_IOCACHE) {
					rnex_ioc_kaddr(addr+offset, size, 1);
				}
				mp->dmai_rflags |= DMP_LKSYSV;
			} else {
				/*
				 ...
				 * need syncing.
				 */
				mp->dmai_rflags |= DMP_LKIOPB|DMP_NOSYNC;
			}
	...
	align = (u_int)-1;
	if (vac & VAC_IOCOHERENT) {
		/* see <sun/vm/hat_sunm.c> */
		struct hment *hme;
		extern int vac_mask;

		if (pp != NULL) {
			if ((hme = pp->p_mapping) != NULL) {
				align = hme->hme_impl &
	...
	kaddr = getdvmapages(npages + RED, ALO, AHI, align, CMAX,
	...
	if (DIRECTION == DDI_DMA_WRITE) {
		if (vac & VAC_WRITETHRU) {
			/*
			 * We assume that this only applies to dma_sync
			 * operations- not any flushing implicit in the
			 * mapping done below.
			 */
			mp->dmai_rflags |= DMP_NOCPUSYNC;
		}
	}

	if (mp->dmai_rflags & DMP_USEHAT) {
		/*
		 * We want to use the hat layer to set up the mappings
		 * since for, say, the sun4 I/O goes through the vac and
		 * we want (in this case) the hat layer to manage cache
		 * consistency for us.
		 */
		flags = PTELD_NOSYNC|PTELD_INTREP|PTELD_LOCK;
		if (mp->dmai_rflags & DMP_IOCACHE) {
			flags |= PTELD_IOCACHE;
		}
	} else {
	...
	while (npages > 0) {
		...
		if (flags) {
			...
			if (mp->dmai_rflags & DMP_IOCACHE) {
				check_ioctag((caddr_t)kaddr);
			}
			if (pp) {
				npp = pp;
			} else {
				npp = page_numtopp_nolock(ptep->pg_pfnum);
			}

			if (npp) {
				/*
				 * Make sure the page isn't going anywhere...
				 */
				if (!se_assert(&npp->p_selock) && !panicstr) {
					/*
					 * This is a real panic because it is
					 * very hard at this point to figure
					 * out how much we have already mapped
					 * hence would be difficult to back
					 * out of it and just return an error.
					 * It also is indicative of a gross
					 * logic error somewhere else in the
					 * system.
					 */
					cmn_err(CE_PANIC,
					    "dma_map: unlocked page %x", npp);

					/* NOTREACHED */
				}
			} else {
#ifdef DMADEBUG
				if (dmadebug > 1 && memtype == 0)
					printf(
					    "dma_map: no pp {%x,%x} pte %x\n",
					    as, ORIGVADDR, *((u_int *)ptep));
#endif

				/*
				 * If a write from memory to the device,
				 * do any synchronizations now.
				 *
				 * Also mark for being a vacsync operation at
				 * ddi_dma_sync or ddi_dma_free time.
				 */
				if (DIRECTION & DDI_DMA_WRITE) {
					sunm_vacsync(ptep->pg_pfnum);
				}
				/*
				 * We assume here that the hat layer will
				 * establish the dma mapping uncached since
				 * we have no page pointer.
				 *
				 * However, sunm_vacsync works a little
				 * harder than sunm_pteload when it is
				 * going out to flush things, so even
				 * though the dma mapping will be uncached,
				 * there might be some address reference
				 * to trap.
				 */
				if (memtype == 0)
					mp->dmai_rflags &= ~DMP_NOCPUSYNC;
			}
			sunm_pteload(&kas, (caddr_t)kaddr, npp, *ptep, flags);
		} else {
			/*
			 * We're breaking the rules here a bit.
			 *
			 * Since we're not using the hat layer,
			 * we won't grab hat locks. This is an
	...
	if (RED) {
		/*
		 * We don't have to grab any hat locks here.
		 */
...
out:
	...
	if (mp) {
		DMAPRINT4("dma_map: handle %x flags %x kaddr %x size %x\n", mp,
		    mp->dmai_rflags, mp->dmai_mapping, mp->dmai_size);
	...
	caddr_t *objp, u_int cache_flags)
{
	...
	case DDI_DMA_FREE:
	{
		...
		isdvma = ((mp->dmai_rflags & (DMP_LKIOPB|DMP_LKSYSV)) == 0);
		usehat = mp->dmai_rflags & DMP_USEHAT;

		/*
		 * 'Free' the dma mappings.
		 *
		 * For platforms that use the hat layer to manage things
		 * an implicit ddi_dma_sync(...FORCPU) is done by the act
		 * act of freeing.
		 *
		 * Other platforms (!VAC_IOCOHERENT) have to take an
		 * extra step, but only at a time when we are completely
		 * freeing the mapping, i.e., at ddi_dma_free time
		 * (as opposed to ddi_dma_remap time).
		 */

		if (usehat && isdvma) {
			/*
			 * If appropriate, use hat_unload to unload the
			 * translations without getting the referenced
			 * and modified bits. At the same, time any
			 * vac consistency changes made by the orginal
			 * mapping can be reexamined.
			 *
			 *
			 * Again, note the assumption that no IO cache
			 * will exist without the use of the hat layer
			 * to manage dma mappings.
			 */
			npages = mp->dmai_ndvmapages;
			if (mp->dmai_rflags & DMP_IOCACHE) {
				while (npages-- > 0) {
					extern char DVMA[];
					ioc_flush(mmu_btop(addr-(u_long)DVMA));
					addr += MMU_PAGESIZE;
				}
				npages = mp->dmai_ndvmapages;
				addr = mp->dmai_mapping & ~MMU_PAGEOFFSET;
			}
			hat_unload(&kas, (caddr_t)addr,
				mmu_ptob(npages), HAT_UNLOAD);
		} else if (usehat && IOCACHED_SYS_VIRTUAL) {
			/*
			 * Turn off any ioc settings.
			 *
			 * This really applies only to
			 * the Sunray onboard ethernet.
			 */
			rnex_ioc_kaddr(addr + offset, mp->dmai_size, 0);
		}

		/*
		 * For !VAC_IOCOHERENT machines, now would be the time
		 * to do flushing FORCPU (if appropriate). We use the
		 * rounded size of the entire object just in case we
		 * had been doing a DDI_DMA_PARTIAL type transfer.
		 */

		npages = mmu_btopr(mp->dmai_object.dmao_size + offset);

		if (usehat == 0 && (mp->dmai_rflags & DMP_NOCPUSYNC) == 0) {
		...
		if (isdvma && mp->dmai_ndvmapages) {
			putdvmapages(mp->dmai_mapping & ~MMU_PAGEOFFSET,
			    mp->dmai_ndvmapages + RED);
		}
		...
		if (cache_flags == DDI_DMA_SYNC_FORDEV &&
			...
			if (mp->dmai_rflags & DMP_USEHAT) {
				...
				if (mp->dmai_rflags & DMP_IOCACHE) {
					while (npages-- > 0) {
						extern char DVMA[];
						ioc_flush(mmu_btop(ioaddr-
							(u_long)DVMA));
						ioaddr += MMU_PAGESIZE;
					}
				}
				hat_unload(&kas, (caddr_t)addr,
					mmu_ptob(npf), HAT_UNLOAD);
			}
	...
	case DDI_DMA_NEXTWIN:
	{
		...
		if (mp->dmai_rflags & DDI_DMA_PARTIAL) {
				...
				*nwin = (ddi_dma_win_t)mp;
				...
				if (mp->dmai_rflags & DMP_USEHAT) {
					...
					while (npages > (u_int)0) {
					    extern char DVMA[];
					    ioc_flush(
						mmu_btop(addr-(u_long)DVMA));
					    addr += MMU_PAGESIZE;
					    npages -= 1;
					}
					addr =
					    mp->dmai_mapping & ~MMU_PAGEOFFSET;
					npages = mp->dmai_ndvmapages;
				    }
				    hat_unload(&kas, (caddr_t)addr,
					mmu_ptob(npages), HAT_UNLOAD);
			...
			DMAPRINT1("nextwin: remapping %d pages\n", npages);
				...
				if (mp->dmai_rflags & DMP_USEHAT) {
				    flags =
					PTELD_NOSYNC|PTELD_INTREP|PTELD_LOCK;
				    if (mp->dmai_rflags & DMP_IOCACHE) {
					flags |= PTELD_IOCACHE;
				    }
				} else {
				...
				while (npages > (u_int)0) {
					...
					if (!npp) {
					    ...
					    mp->dmai_rflags &= ~DMP_NOCPUSYNC;
					} else if (!se_assert(&npp->p_selock)) {
						cmn_err(CE_PANIC,
						    "dma_mctl: remap");
						/* NOTREACHED */
					}
					sunm_pteload(&kas, (caddr_t)kaddr, npp,
					    *ptep, flags);
				    } else {
					SETPTE(kaddr, ptep);
				    }
	...
	case DDI_DMA_MOVWIN:
	{
		...
		/*
		 * If appropriate, unload the old mapping.
		 *
		 * This is only necessary for DMP_USEHAT, and
		 * we can assume that it is a mapping across DVMA.
		 */
		...
		if (mp->dmai_rflags & DMP_USEHAT) {
			/*
			 * We want to use the hat layer to set up the mappings
			 * since for, say, the sun4 I/O goes through the vac
			 * and we want (in this case) the hat layer to manage
			 * cache consistency for us.
			 */
			flags = PTELD_NOSYNC|PTELD_INTREP|PTELD_LOCK;
			if (mp->dmai_rflags & DMP_IOCACHE) {
				flags |= PTELD_IOCACHE;
...
#ifdef	DMADEBUG
			if (dmadebug > 1) {
				printf("dma_mctl: remap kaddr %x pte %x\n",
				    kaddr, *((u_int *)ptep));
			}
#endif
			if (flags) {
				...
				if (pp) {
					...
					npp =
					    page_numtopp_nolock(ptep->pg_pfnum);
				}
				/*
				 * Make sure the page isn't going anywhere...
				 */
				if (!npp) {
					...
					cmn_err(CE_PANIC,
					    "dma_mctl: remap unlocked page %x",
					    npp);
					/* NOTREACHED */
				}
				sunm_pteload(&kas, (caddr_t)kaddr, npp,
...
#ifdef	IOC
/*
 * We assume that any necessary flushing has been done
 * prior to converting the ioc attribute of a page
 */
static void
rnex_ioc_kaddr(u_long addr, u_int size, int on)
{
	register u_long saddr, eaddr;

	saddr = addr;
	eaddr = addr + size;

#ifdef	WE_FIXED
	if (saddr & MMU_PAGEOFFSET) {
		saddr = roundup(saddr, MMU_PAGESIZE);
	}
	if (eaddr & MMU_PAGEOFFSET) {
		eaddr &= ~MMU_PAGEOFFSET;
	}
#else
	saddr &= ~MMU_PAGEOFFSET;
	eaddr = roundup(eaddr, MMU_PAGESIZE);
#endif

	while (saddr < eaddr) {
		pte_t pte;
		mutex_enter(&sunm_mutex);	/* XXX - is this needed? */
		GETPTE(saddr, &pte);
		if (on) {
			pte.pg_ioc = 1;
		} else {
...
{
	cmn_err(CE_CONT, "?%s%d at root\n", DEVI(dev)->devi_name,
	    DEVI(dev)->devi_instance);
	return (DDI_SUCCESS);
}

/*
 * For the sun4 rootnexus, we either get a simple list of ipls
 * in the cpu namespace.
 *
 * XXX	Actually, it's not clear that we ever get called here at
 *	all .. though we probably should arrange obio to pass us
 *	its 'interrupts' list for translation.  maybe.
 */
/*ARGSUSED*/
static int
	...
	struct ddi_parent_private_data *pdptr)
{
	...
	static char bad_intr_fmt[] =
	    "rootnex: bad interrupt spec for %s%d - sparc ipl %d\n";
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_IOMIN:
		...
		if (((int)arg) && ioc) {
			/*
			 * Streaming mode access and an IO Cache is present
			 */
			n = maxbit(n, IOC_LINESIZE);
		} else {
			n = maxbit(n, 0x4);
		}
		*((int *)result) = n;
