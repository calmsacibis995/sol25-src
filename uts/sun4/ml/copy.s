...
.bcp_bufchk:
	...
	!
	! sethi %hi(bcopy_res), %o2
	! ld    [%o2 + %lo(bcopy_res)], %o3 ! is hardware available
	! tst   %o3
	! bnz   .bcp_nobuf              ! reserved or not enabled
	! sethi _vac_linesize, %o4
	! ld    [%o4 + %lo(vac_linesize)], %o4 ! cache line size
...
1:
	...
	add     %i1, 8, %i1             ! update dest address
.bcp_buf:
	ldstub  [%o2 + %lo(bcopy_res)], %o3 ! try to grab hardware
	tst     %o3
...
1:      lda     [%i0]ASI_BC, %g0        ! fill buffer
	...
	sethi   %hi(bcopy_cnt), %o3     ! count # times used buffer
	ld      [%o3 + %lo(bcopy_cnt)], %o4
	st      %g0, [%o2 + %lo(bcopy_res)] ! interlock, unlock hardware
	inc     %o4
	b       .bytecp                 ! do remaining bytes, if any
	st      %o4, [%o3 + %lo(bcopy_cnt)]
.bcp_nobuf:
#endif BCOPY_BUF
...
.bzero_bufchk:
	sethi   %hi(bcopy_res), %o2
	ldub    [%o2 + %lo(bcopy_res)], %g2 ! is the bcopy buffer available
	tst     %g2
	bnz     .bzero_nobuf
	cmp     %o1, BCOPY_LIMIT        ! for small counts use software
	bl      .bzero_nobuf
	!
	! set   vac_linesize, %o4
	! ld    [%o4 + %lo(vac_linesize)], %o4
	! sub   %o4, 1, %o3             ! create line mask
	! andcc %o0, %o3, %g0
	!
	mov     32, %o4
	andcc   %o0, 0x1F, %g0          ! check if cache-line aligned
1:      bz,a    .bzero_buf
	ldstub  [%o2 + %lo(bcopy_res)], %g2 ! try to grab bcopy buffer
	std     %g0, [%o0]
	add     %o0, 8, %o0
	sub     %o1, 8, %o1             ! decrement count
	b       1b
	andcc   %o0, 0x1F, %g0          ! check if aligned
...
3:  sta     %g0, [%o0]ASI_BC        ! zero a line
#endif
	sub     %o1, %o4, %o1           ! decrement count
	cmp     %o1, %o4                ! check if done
	bge     3b                      ! loop until done
	add     %o0, %o4, %o0           ! increment address

	sethi   %hi(bzero_cnt), %g2     ! count # times used buffer
	ld      [%g2 + %lo(bzero_cnt)], %o4
	stb     %g0, [%o2 + %lo(bcopy_res)] ! interlock, unlock bcopy buffer
	inc     %o4
	b       .byteclr
	st      %o4, [%g2 + %lo(bzero_cnt)]
#else !BCOPY_BUF
	!
	! obmem, if needed move a word to become double-word aligned.
	!
