...
#ident	"@(#)swtch.s	1.13	93/12/07 SMI"
...
1:
	...
	ld	[%i0 + T_MMUCTX], %l1
	set	CONTEXT_REG, %l0
	tst	%l1			! are we in a temporary context?
	bnz,a	5f			! 	we preempted the hat layer
	stba	%l1, [%l0] ASI_CTL	! setup new context, the temporary
	...
	!
	! Update LRU ctx clock, and setup address space context,
	! if it exists. Otherwise we put the thread in the kernel's
	! context which will cause it to fault if it ever gets back
	! to userland.
	!
	ld	[%o0 + A_HAT], %o0	! check (p->p_as->a_hat)
	tst	%o0
	bz,a	5f			! if zero, skip ahead
	stba	%g0, [%l0] ASI_CTL	! delay - setup kernel context

	ld	[%o0 + HAT_CTX], %o0	! check (p->p_as->a_hat->hat_ctx)
	tst	%o0
	bz	4f			! if zero, skip ahead

	sethi	%hi(ctx_time), %l2	! delay slot
	lduh	[%l2 + %lo(ctx_time)], %o2 ! get context LRU clock
	stb	%g0, [%o0 + C_CLEAN]	! zero the context's clean byte
	add	%o2, 1, %o3		! ++ctx_time
	sth	%o2, [%o0 + C_TIME]	! Set context clock to LRU clock
	sth	%o3, [%l2 + %lo(ctx_time)]
	ldub	[%o0 + C_NUM], %o0	! get context number

4:
	stba	%o0, [%l0] ASI_CTL	! setup context
5:
	!
...
4:
	...
	st	%o1, [%i1 + CPU_LWP]	! delay - set CPU's lwp ptr
	!
...
1:
	...
	! XXX - this should never happen on a uniprocessor.
	!
7:
	ldub	[%i0 + T_LOCK], %o0
1:
