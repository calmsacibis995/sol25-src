...
#pragma ident	"@(#)locore.s	1.83	94/10/20 SMI"
...
#include <sys/clock.h>
#include <sys/cpu.h>
#include <sys/diag.h>	/* SUN4POC: Not in sun4c */
#include <sys/enable.h>
#include <sys/intreg.h>
#include <sys/memerr.h>
#include <sys/eccreg.h>	/* SUN4POC: Not in sun4c */
#include <sys/eeprom.h>
#include <sys/iocache.h>	/* SUN4POC: Not in sun4c */
...
#include <sys/trap.h>
#include <sys/scb.h>
#ifndef	sun4	/* SUN4POC: The auxio for sun4c should not be in locore.s */
#include <sys/auxio.h>
#endif	/* !sun4 */
...
#ifdef	sun4	/* XXX-SUN4POC: sun4c (and maybe sun4m) use a TRAP_MON here */
	BAD_TRAP;				! FF
#else
	TRAP_MON(0xff)				! FF
#endif
 ...
 *
 * XXX-SUN4POC: This should be required for sun4c too.
 */
	.skip   4096
#endif
	!
...
#ifdef	sun4
	/*
	 * XXX-SUN4POC: For sun4, we I'm not sure that we can enable traps.
	 * XXX-SUN4POC: This should be checked out.
	 */
	set	PSR_S|PSR_PIL, %g1	! setup psr
#else
...
2:
	...
	!
	! this used to be done with ldd/sdd. Bad news on a sunray.
	! Reasons unknown. It only would do the first long of the
	! pair of longs in a load or store.
	!
	ld	[%o0], %g2
	st	%g2, [%o1]
	ld	[%o0 + 4], %g2
...
window_overflow_trace:
	...
	!
	TRACE_PTR(%l5, %l7)		! get trace pointer
	mov	%tbr, %l3
...
.st_user:
	...
	ld	[%l6 + CPU_THREAD], %l6	! load thread pointer
	ld	[%l6 + T_STACK], %l7	! %l7 is lwp's kernel stack
	SAVE_GLOBALS(%l7 + MINFRAME)
	...
	mov	%l6, THREAD_REG		! finally move thread pointer to global
	mov	1, FLAG_REG		! set flag reg (%g6) nz for mutex_enter
	ld	[%l6 + T_LWP], %g5	! put kt's lwp in %g5
	sethi	%hi(scb + 0x1f), %l6		! XXX - temp hack
	ldub	[%l6 + %lo(scb + 0x1f)], %l6	! load NW-1
	st	%l0, [%l7 + MINFRAME + PSR*4] ! psr
	st	%l1, [%l7 + MINFRAME + PC*4] ! pc
...
.st_sys_ovf:
	...
	! If lwp->lwp_pcb->pcb_uwm has any bits set, then it is a user
	! which must be saved.
	!
#ifdef	PERFMETER
	...
	ld	[%g5 + PCB_UWM], %g2	! if (lwp_pcb.pcb_uwm)
	tst	%g2			!	user window
	bnz	.st_user_ovf
	nop
1:	
	...
	!
.st_stack_res:
	SAVE_WINDOW(%g4)		! %g4 has user's %sp for shared case
...
.st_user_ovf:
	...
	ld	[%g5 + PCB_SWM], %g4	! test shared window mask
	...
	!
	clr	[%g5 + PCB_SWM]		! clear the shared window mas
	SAVE_WINDOW(%sp)		! save copy of shared window
	ld	[%g5 + LWP_REGS], %g4	! pointer to regs struct at user trap
	ld	[%g4 + SP*4], %g4	! use saved stack pointer for user copy
...
.st_stack_not_res:
	...
	add	%g1, %g5, %g2
	st	%g4, [%g2 + PCB_SPBUF] 	! save sp in %g1 + curthread + PCB_SPBUF
...
{}
	...
	!
	ld	[THREAD_REG+T_LWP], %g5	! get lwp
	ldub	[THREAD_REG + T_ASTFLAG], %g1
	tst	%g1			! test signal or AST pending
	bz	1f
	ld	[%g5 + PCB_WBCNT], %g3	! user regs been saved?
...
.sr_stack_not_res:
	...
	mov	%o1, %psr		! set priority before enabling (IU bug)
	wr	%o1, PSR_ET, %psr	! turn on traps
	mov	%g1, %l2		! delay, save fault address
...
#ifdef  sun4
	mov	BE_PROTERR, %o3		! delay - set protection error
#else
...
.sr_sup:
	...
	! Check for an AST that's posted to the lwp.
	!
	ld	[THREAD_REG + T_CPU], %g5	! delay slot, get lwp
	ldub	[%g5 + CPU_KPRUNRUN], %g5
	tst	%g5
	bz	2f
	nop				! delay
	!
...
#define	ST_OP_SHIFT	21	/* opcode bit that determines ld vs st */
	...
	ld	[%g1 + ME_VADDR], %o2	! get data fault address in mem err reg
 	ld	[%g1], %o5		! get memory error register
 	btst	ER_INTR, %o5		! is this a memory error or a fault?
 	bz,a	1f			! fault, not memory error
 	clr	[%g1 + ME_VADDR]	! clear fault address if not error
...
2:
	...
	nop; nop; nop			! necessary PSR delay (cmp might
	cmp	%o3, BE_INVALID		! fail)
#else
/*
...
/* Calvin type support */
	...
	wr	%l0, PSR_ET, %psr	! enable traps (no priority change)
	mov	S_EXEC, %o4		! assume execute fault
1:					! %psr modified in branch delay slot
	nop; nop; nop			! psr delay
	cmp	%l4, T_TEXT_FAULT	! text fault?
	be,a	2f
	mov	%l1, %o2		! pc is text fault address
...
4:
	...
	mov	%l5, %o4
 	mov	%l6, %o5
 	mov	%l4, %o0		! trap(t, rp, addr, be, rw, merr)
3:
	!
	! Call C trap handler
	!
 ...
 * table are loaded when a driver for this device adds its
 * interrupt (via ddi_addintr()).
 */

#define	ERRV	.word 0, 0, 0

	.global	vme_vector
	.type	vme_vector,#object
vme_vector:			! vector numbers
...
2:
	...
	!
	mov	%g0, %g2		! flag that we are return to .intr_ret
	cmp	%l3, 8			! is this a vme generated interrupt?
	blu,a	vme_interrupt
...
2:
	...
	! Set the interrupted thread's T_SP because if it is the idle thread,
	...
	! interrupt otherwise we get an interrupt routine address
	!
	mov	%l3, %g2		! flag that we return to .int_rtt
	cmp	%l3, 8			! is this a vme generated interrupt?
	blu,a	vme_interrupt
	mov	(14<<PSR_PIL_BIT), %g1
#endif	/* sun4 */
...
.int_rtt:
	...
	mov	TT_INTR_RET2, %g2	! interrupt return
...
#endif	/* lint */
#if	!defined(lint) && defined(sun4)

	!
	! VME interrupt- Read vector and do vectoring.
	!
	! The VME vector is read off the VME bus. If this fails (data fault),
	! trap will check the PC of the fault and set vme_ifault flag below.
	!
	! Entry:
	!	%l0	saved %psr (which we restore)
	!	%l3	Vme Interrupt Level
	!	%l4	Sparc IPL level
	!	%l5	Old %psr with PIL cleared
	!	%g1	Value to OR into %psr (IPL 14)
	!	%g2	Flag as to which interrupt return to go back to
	!
	! Uses:
	!	all of the above, plus %l1, %l2, %l6, %g3
	!
	! Cautions:	Must preserve %l4, %l7
	!

	DGDEF(vme_ifault)
	.word	0
ENTRY_NP(vme_interrupt)
	or	%l5, %g1, %l5		! spl 14
	mov	%l5, %psr		! set level (IU bug)
	wr	%l5, PSR_ET, %psr	! enable traps
	nop; nop;			! psr delay
	sethi	%hi(vme_ifault), %l5	! clear vme_ifault word
	st	%g0, [%l5+%lo(vme_ifault)]
	set	VME_INT_VEC, %g1	! get VME vector from the bus
	sll	%l3, 1, %g3		! turn vme int pri to part of address
	or	%g1, %g3, %g1		! create address to read in alt space
	or	%g1, 1, %g1		! a0 should always be a 1
ALTENTRY(vme_read_vector)		! This is caught in trap()
	lduba	[%g1]ASI_CTL, %l5	! read vector number, acknowledge int.
	sethi	%hi(vme_ifault), %l1	! reload vme_ifault
	ld	[%l1+%lo(vme_ifault)],%g1
	tst	%g1			! to see whether we got a fault
	bne	.vme_asi_intrfault	! error reading the vector
	wr	%l0, PSR_ET, %psr	! set proper interrupt level
	nop; nop;			! psr delay
	mov	%g2, %l0		! Save return type flag (frees %l0)
	cmp	%l5, VEC_MAX		! check vector limits
	bg	.vme_spurious		! spurious vector...
	subcc	%l5, VEC_MIN, %g3	! normalize vector to base
	bl	.vme_spurious		! spurious vector...
	sll	%g3, 2, %g2		! get 4X scale
	sll	%g3, 3, %g3		! get 8X scale
	add	%g3, %g2, %g3		! to end up with 12X scale
	set	vme_vector, %g2		! table of interrupt vectors
	ld	[%g2 + %g3], %l6	! get handler address
	tst	%l6			! make sure it is non-zero
	be	.vme_spurious		! no driver installed
...
1:	call	%l6			! call routine
	...
	mov	%l2, %o0
2:	tst	%l1
	be	.vme_spurious		! driver didn't claim the interrupt
	nop
.vme_intr_ret:
	tst	%l0			! What kind of return?
	be	.intr_ret		! zero means running above LOCK_LEVEL
	nop
	b,a	.int_rtt		! else running a thread
	.empty

.vme_asi_intrfault:
	mov	%g2, %l0		! Save flag as to return type
	mov	-1, %l5			! 'totally spurious' interrupt
	!
	! The vector was illegal or spurious
	!
.vme_spurious:
/*
 * Special hack for buggy ie1 hardware.  This fix is taken from 1009297.
 */
	.seg    "data"
	.global _ie1_intr
_ie1_intr: .word 1
	...
	! Entry (which we care about):
	!	%l0	Return type flag
	!	%l3	VME Interrupt Level
	!	%l5	VME Interrupt Vector
	!
	! Caution: Preserve %l4 and %l7
	!
	mov	%l5, %o2
	set	2f, %o0
...
1:
	...
	.seg	".data"
2:	.ascii	"Spurious VME interrupt at "
	.asciz	"VME level %d vector 0x%x\n"
	.align	4
	.seg	".text"
	SET_SIZE(vme_interrupt)
#endif	/* !defined(lint) && defined(sun4) */
...
.level6:
	...
	set	xlvl6, %l1		! level 6 soft intr. pointer
 ...
 *
 * We enter with %l5 pointing to the hard int list, %l5 pointing to the
 * soft int list, %l3 pointing to the levelN_spurious counter, and %l6 holding
 * our favorite interrupt register bit, and %g3 holding our interrupt level
...
2:	tst	%l0				! success?
	...
	b	.int_rtt			! done
	/*
	 * delay slot, non-spurious interrupt; clear spurious counter
	 */
...
3:	set	INTREG_ADDR, %o0		! was our favorite bit set?
	...
	!	%l1 - software av list pointer (no longer used)
	!
...
5:
	...
	sethi	%hi(clock_type), %l5	! which clock_type?
	ld	[%l5 + %lo(clock_type)], %g1
	cmp	%g1, INTERSIL7170
	bnz	.mostek
	nop
...
#endif	/* !defined(SAS) || !defined(MPSAS) */
...
.mostek:
#if	!defined(SAS) || !defined(MPSAS)
	sethi	%hi(COUNTER_ADDR + CTR_LIMIT10), %l5	! read limit10 reg to
	ld	[%l5 + %lo(COUNTER_ADDR + CTR_LIMIT10)], %g1 ! clear interrupt
#else
	nop
#endif	/* !defined(SAS) || !defined(MPSAS) */
...
.done_clkhrdsvc:
	...
	!
	ld	[THREAD_REG + T_CPU], %g1	! get CPU pointer
	ld	[%g1 + CPU_IDLE_THREAD], %g1	! get idle thread ptr
	cmp	THREAD_REG, %g1			! is that us?
	beq	1f				! Yes. update LEDs
	ldub	[%l5 + LEDCNT], %g1	! Decrement count
	subcc	%g1, 1, %g1		! subtract
...
#ifdef	sun4
	sethi	%hi(clock_type), %l3	! which clock type?
	ld	[%l3 + %lo(clock_type)], %l4
	cmp	%l4, INTERSIL7170
	bnz	1f			! mostek
	nop
 ...
 * If the error is correctable ECC, asyncerr_sun4 will eventually return
 ...
 */
	...
	bz	1f
	nop
	call	asyncerr_sun4		! sometimes returns
	nop
	b,a	3f
#else	/* sun4 */
/*
...
2:
	...
	clr	[%g5 + PCB_SWM]		! clear shared window mask
	b	splx			! splx and return
	mov	%g4, %o0
3:
	retl
...
.clean_windows:
	...
	mov	1, %l4
	stb	%l4, [%l5 + T_POST_SYS]	! so syscalls will clean windows
...
.fix_alignment:
	CPU_ADDR(%l5, %l4)		! load CPU struct addr to %l5 using %l4
	ld	[%l5 + CPU_THREAD], %l5	! load thread pointer
	ld	[%l5 + T_LWP], %l5	! load lwp pointer (pcb is first in lwp)
 ...
 * in user mode.
 */
	.global kadb_tcode, trap_kadb_tcode
...
/*
 * Arrggghhhh!
 * test to discover if this is an early revision of the sun4/200 series
 * CPU which had a bug that caused floating point stores to non-cached
 * pages to not wait for the FPU to finish caculating the result.
 * Must be called after configure which sets the variable fpu_exists.
 * IF it is found that the bug exists in the hardware we turn on
 * the nopagereclaim workaround, and print a message.
 */
#if defined(lint)
...
{ return (0); }
	...
	.align	8
chknums: 
	.word	0x3ff00000,0x0		! operand a
	.word	0x40000000,0x0		! operand b
	.word	0,0			! initialize result reg with this
chknumres:
	.word	0,0			! placeto store result of a/b
	.word	0x3fe00000,0x0		! what the result should be

	.global	old260cpu
old260cpu:
	.word	0			! assume this is a new board
 
	.seg	".text"
	.align	8
	ENTRY_NP(old260_cpu_chk)
	save	%sp, -WINDOWSIZE, %sp
	sethi	%hi(fpu_exists), %o0
	ld	[%o0 + %lo(fpu_exists)], %o0 ! does the fpu exist
	tst	%o0			! believe it or not;
	bnz	1f			! some machines don't have FPUs
	nop				! if no fpu; no one cares
	ret
	restore %g0, %g0, %o0
1:
	set	chknumres, %l0		! address of result words
	lda	[%l0]ASI_PM, %l1	! read the page map
	set	PG_NC, %l2
	or	%l1, %l2, %l3		! turn on the NC bit
	sta	%l3, [%l0]ASI_PM	! now the page is marked non-cacheable
 
	mov	%l0, %o0
	call	vac_pageflush		! flush the page out of the cache
	nop
 
	mov	%psr, %l7		! get current psr value
	set	PSR_EF, %o1
	or 	%l7, %o1, %o2		! turn on the enable fpu bit
	mov	%o2, %psr		! enable the FPU
	set	chknums, %o3		! pointer to data used for check
	set	old260cpu, %o4		! make nonzero if broken hardware
	mov	1, %o0			! value to store and return, if broken
	mov	3, %o5			! loop
threetrys:
	ldd	[%o3], %f0		! read operand a
	ldd	[%o3+8], %f2		! read operand b
	st	%g0, [%o3+16]		! initialize result
	st	%g0, [%o3+20]
	ldd	[%o3+16], %f4
 
	! calculate a/b, have to fake out assembler workarounds
	.word	0x89a009c2		! fdivd %f0, %f2, %f4
	std	%f4, [%o3+24]		! store result
 
	ldd	[%o3+24], %g2		! get stored result
	ldd	[%o3+32], %g4		! get what the result should be
	cmp	%g2, %g4		! compare first half
	bne	1f			! got a bad one
	cmp	%g3, %g5		! compare second half
	bne	1f			! got a bad one
	deccc	%o5
	bnz	threetrys
	nop
	clr	%o0			! everything ok, store and return 0
1:	st	%o0, [%o4]
 
	sta	%l1, [%l0]ASI_PM	! restore page map to original value
	mov	%l7, %psr		! restore original psr
	ret
	restore	%g0, %o0, %o0
	SET_SIZE(old260_cpu_chk)
