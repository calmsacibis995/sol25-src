...
#pragma ident   "@(#)mach_srmmu.c 1.23     95/03/07 SMI"
 ...
 * Static Routines:
 */
static void srmmu_setctx(int ctx);
	...
	int rmkeep)		/* R & M bits to carry over */
{
	...
	if (!srmmu_use_inv) {
		...
		return (mmu_writepte_noinv(pte, entry, va, lvl, cxn, rmkeep));
	}
	...
	int ctx)
{
	...
	/*
	 * if there's nothing do, don't do anything
	 */
	if (srmmu_getctxreg() == ctx)
		return;
...
map_ctxtbl(void)
{
	...
	ulong_t a;		/* kernel map index */
	caddr_t kvaddr;		/* kernel virtual address */

	size = nctxs * sizeof (struct ptp);
	npages = btopr(size);
	a = rmalloc(kernelmap, (size_t)npages);
	if (a == 0)
		prom_panic("map_ctxtbl: kernelmap exhausted");
	kvaddr = kmxtob(a);
	...
	int flags)
{
	...
	if (PTE_ETYPE(old) != MMU_ET_INVALID && cxn != -1)
...
hat_kern_setup(void)
{
	...
	phys_msgbuf = va_to_pfn((caddr_t)&msgbuf) * PAGESIZE;
...
load_l1(struct l1pt *l1pt, pa_t pa)
{
	...
	l1ptbl = kl1ptbl;
	lock_ptbl(l1ptbl, 0, &kas, 0, 1, &l1mtx);
	...
	for (i = 0; i < NL1PTEPERPT; i++, pa += sizeof (struct ptp)) {
		...
		if (nl1ptbl != l1ptbl) {
			...
			lock_ptbl(nl1ptbl, 0, &kas, vaddr, 1, &l1mtx);
			l1ptbl = nl1ptbl;
		}
	...
	for (i = 0; i < NL3PTEPERPT; i++, pa += sizeof (struct pte)) {
		 ...
		 */
			...
			if (((caddr_t)vaddr >= econtig) &&
			    (address_in_memlist(virt_avail, vaddr, 0x1000))) {
#define	MORE_DEBUG
#ifdef MORE_DEBUG
				rp.ptpe_int = ldphys36(pa);
				if (pte_valid((struct pte *)&rp.ptpe_int)) {
					printf("load_l3: invalidating 0x%x\n",
						vaddr);
				}
#endif MORE_DEBUG
				rp.ptpe_int = MMU_ET_INVALID;
