...
#ident	"@(#)cpudelay.c	1.32	95/10/19 SMI"
...
resetcpudelay(void)
{
	...
	/*
	 * Set the target delay ('e' and 'es') to be based on the
	 * resolution of the hires timer 'r' multiplied by 1000. This
	 * creates a large enough delay to limit the impact the timer
	 * resolution has on the Cpudelay calculation at the end of
	 * the loop.
	 */
	cpudelay = Cpudelay;	/* save copy of default setting */
	r = 1000;		/* sun4d hires timer resolution, in ns */
	es = r * 1000;	/* target delay, in ns */
	e = ((es + (NS_PER_US - 1)) / NS_PER_US);	/* round delay to us */
	es = e * NS_PER_US;		/* adjusted target delay, in ns */
	Cpudelay = 16;		/* initial guess (right for 33 MHz) */
	DELAY(1);
	do {
