...
#pragma ident	"@(#)intr15.c	1.60	95/01/10 SMI"
...
#include <sys/t_lock.h>		/* inc mutex.h, inc types.h */
 ...
 * Global Routines:
 */
void bb_ctl_bits_all(u_int bits, u_int enable);
 ...
 */
u_int disable_acfail;		/* override for /etc/system */
u_int disable_tempfail;		/* override for /etc/system */
u_int disable_fanfail;		/* override for /etc/system */
u_int disable_power;		/* override for /etc/system */
u_int disable_sbifail;		/* override for /etc/system */
u_int disable_mxccfail;		/* override for /etc/system */
u_int oven_timeout_sec = OVEN_TIMEOUT_SEC;	/* set to default */
 ...
 * the options node.
 */
static u_int temperature_chamber = 0;
/*
...
#define	ECI_DELAY (HZ/20)	/* 50ms of peace before re-enabling ECI */
static long eci_delay = ECI_DELAY;

#define	ECC_THROTTLE 64	/* log no more than 128 errors/interrupt */
static u_int ecc_throttle = ECC_THROTTLE;
...
#define	TRIGGER_INTR(handler) { \
	register u_int level = (handler).ipl; \
 ...
 * Static Routines:
 */
static void intr15_bbus(u_int cpu_id);
static void intr15_obp();
...
static void bb_ctl_bits(u_int bits, u_int enable, int cpu_id);
...
static void oven_timeout(caddr_t);
...
#define	NO_CPU (u_int)0xDEADBEEF	/* an illegal cpu_id u_int */
...
#ifdef DEBUG
u_int intr15_count[NCPU];
u_int intr15_loops[NCPU];
#endif	/* DEBUG */
...
nvram_get_offset(char *option)
{
	...
	if ((options_dip = ddi_find_devinfo("options", -1, 0)) == NULL) {
		cmn_err(CE_WARN, "nvram_get_offset: "
		    "%s: no options node.", ddi_get_name(options_dip));
		return (0);
	} else {
	...
	sprintf(cmd_buffer, "' %s >body w@ to %s\n", option, option);
...
void
nvram_update_powerfail(u_int pattern)
{
	u_char oldval[4];
	u_char newval[4];
	u_char old_checksum, new_checksum;
	extern struct cpu cpu0;
	u_int nv_cpuid = cpu0.cpu_id;
	int i;
...
level15_init_sbi()
{
	...
	while (dip) {
		...
		if (strcmp("io-unit", name) == 0) {
			...
			devid = ddi_getprop(DDI_DEV_T_ANY, dip,
			    DDI_PROP_DONTPASS,
			    PROP_DEVICE_ID, -1);
			if (devid == -1) {
				cmn_err(CE_WARN,
 ...
 */
void
level15_enable_bbus(u_int cpu_id)
{
	/*
...
u_int intr15_temp_detected[NCPU];

#ifdef DEBUG
u_int intr15_count_temp;
u_int intr15_count_fan;
u_int intr15_count_power;
u_int intr15_fan_still_pend[NCPU];
u_int intr15_ac_detected[NCPU];
u_int intr15_ac_claimed[NCPU];

#endif	/* DEBUG */

static void
intr15_bbus(u_int cpu_id)
{
	u_int status2;
	...
	if (status2 & STATUS2_AC_INT) {
		...
		if (!acfail_detected && lock_try(&acfail_detected)) {
			...
			sprintf(buffer, "POWER FAILURE (detected on board %d),"
				" entering monitor.\n", (CPU->cpu_id) / 2);
...
intr15_mxcc(struct regs *rp)
{
	...
	mxcc_err = intr_mxcc_error_get();

	mxcc_err_hi = (u_int)(mxcc_err >> 32);
...
#ifdef DEBUG
u_int intr15_count_ecc_ce[NCPU];
u_int intr15_count_ecc_ue[NCPU];
u_int intr15_count_sbi[NCPU];
...
intr15_xbus()
{
	...
	if (inttab0 & INTTABLE_SBI_XPT) {
		 ...
		 */
		intr_clear_table(0, INTTABLE_SBI_XPT);
		/*
 ...
 */
static void
bb_ctl_bits(u_int bits, u_int enable, int cpu_id)
{
	u_char old, new;
 ...
 */
void
bb_ctl_bits_all(u_int bits, u_int enable)
{
	u_int cpu_id;
	u_char nfound = 0;
...
#ifdef DEBUG
static u_int syserr_handler_called1[NCPU];
static u_int syserr_handler_called4[NCPU];
static u_int syserr_handler_called8[NCPU];
static u_int syserr_handler_claimed1[NCPU];
static u_int syserr_handler_claimed4[NCPU];
static u_int syserr_handler_claimed8[NCPU];
static u_int syserr_handler_loops1[NCPU];
static u_int syserr_handler_loops4[NCPU];
static u_int syserr_handler_loops8[NCPU];
#endif DEBUG
...
#ifdef DEBUG
static u_int softint_fan_called[NCPU];

#endif	/* DEBUG */

#ifdef DEBUG
static u_int softint_temp_called[NCPU];

#endif	/* DEBUG */

#ifdef DEBUG
static u_int softint_power_called[NCPU];
...
softint_fan(caddr_t arg)
{
	...
	if (!disable_fanfail && (STATUS2_FAN_INT & xdb_bb_status2_get())) {
		...
		(void) timeout((void(*)(caddr_t))softint_fan, NULL,
...
softint_power(caddr_t arg)
{
	...
	if (!disable_power && (STATUS2_PWR_INT & xdb_bb_status2_get())) {
		...
		(void) timeout((void(*)(caddr_t))softint_power, NULL,
...
/*ARGSUSED*/
static void
oven_timeout(caddr_t arg)
{
	int cpu_id;
	static u_int ot_detected_old;	/* bit mask for boards */
	u_int ot_detected_new = 0;
	u_int ot_detected_cnt = 0;
	 ...
	 */
	(void) timeout((void(*)(caddr_t))oven_timeout, NULL,
		(oven_timeout_sec * HZ));
}

#ifdef DEBUG
u_int intr15_count_cc_ae;
u_int intr15_count_cc_cp;
u_int intr15_count_cc_vp;
u_int intr15_count_cc_me;
#endif	/* DEBUG */

#ifdef DEBUG
static u_int softint_mxcc_called[NCPU];
...
{
	u_int ccop, dcmd, err_type, err;
	u_int mxcc_err_hi = (u_int)(mxcc_err >> 32);
	...
	sprintf(buf, "MXCC.ERR=0x%b", mxcc_err_hi, mxcc_err_hi_bits);
...
softint_mxcc(caddr_t arg)
{
	...
	for (err_cpu_id = 0; err_cpu_id < NCPU; ++err_cpu_id) {
		...
		if (mxcc_err_hi & MXCC_ERR_AE) {
			proc_t *p;
			kthread_t *t;
			u_int was_usermode = !(mxcc_psr[err_cpu_id] & PSR_PS);
...
#ifdef DEBUG
static u_int softint_sbi_called[NCPU];
...
softint_sbi(caddr_t arg)
{
	...
	for (i = 0; i < n_sbi; i++) {
		...
		addr =	(u_int *)((sbi_devids[i] << SBI_DEVID_SHIFT) |
...
#ifdef DEBUG
static u_int softint_ecc_called[NCPU];
static u_int softint_ecc_loops;
static u_int softint_ecc_spurious;
static u_int softint_ecc_throttle;
...
softint_ecc(caddr_t type)
{
	...
	if ((u_int)type & MEMERR_FATAL)
		handler = &handler_ecc_ue_async;
	else if ((u_int)type & MEMERR_UE)
