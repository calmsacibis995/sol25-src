 ...
 *
 * level10 calls led_blink_all() via
 * a function pointer (bbus_blink_leds) every 100Hz/led_ticks (25Hz).
 *
 * If you want to disable the LEDs from flashing, write 0 to bbus_blink_leds.
 * If you want bbus_blink_leds() (and thus led_blink_all()) called more or
 * less frequently, decrease or increase the value of led_ticks.
 *
 ...
 * ledpat[] at 100Hz/led_ticks (25Hz).  Else if the CPU is not idle, they
 * cycle through ledpat[] at 100Hz/led_ticks/LED_TICKS2 (1.3Hz)
 *
 * In summary...
 ...
 * Global Data
 */
void (*bbus_blink_leds)(void) = led_blink_all;

u_char led_ticks = 4;   /* level10 calls bbus_blink_leds() @ 100Hz/led_ticks */
u_char led_cnt = 1;
...
bbus_led_update(u_int bbus_id)
{
	...
	if (!CPU_IN_SET(leds, cpu_id))
		...
		if (update_all) {
			if (cpu[cpu_id]->cpu_thread ==
			    cpu[cpu_id]->cpu_idle_thread)
...
bbus_led_boot(u_int bbus_id)
{
	...
	led_set_ecsr(bbus_id, boot_pattern);
}
