...
#ident	"@(#)hwbcopy.c	1.50	95/08/21 SMI"
...
extern void 		bcopy_asm(caddr_t from, caddr_t to, size_t count);
extern void 		bcopy_asm_toio(caddr_t from, caddr_t to, size_t count);
extern void 		bzero_asm(caddr_t addr, size_t resid);
...
#ifdef BCSTAT
		mutex_init(&bcstat.lock, "bcstat_lock", MUTEX_DEFAULT,
			DEFAULT_WT);
#endif
