...
#pragma ident	"@(#)memerr.c	1.44	94/08/18 SMI"
...
#include <sys/promif.h>		/* prom_getprop() */
#include <sys/debug.h>
...
memerr_init(void)
{
	 ...
	 */
	if (prom_getprop((dnode_t)0, "unum-table", (caddr_t)&obp_unum) ==
		sizeof (obp_unum)) {
...
#define	POST_DEBUG
		...
		for (g = 0; g < grp_cnt; g++) {
			...
			case SSIZE_8MB:
				size = btop(0x00800000);
				break;

			case SSIZE_32MB:
				size = btop(0x02000000);
				break;

			case SSIZE_128MB:
				size = btop(0x08000000);
				break;

			case SSIZE_512MB:
				size = btop(0x20000000);
				break;

			case SSIZE_2GB:
				size = btop(0x80000000);
				break;
			}
