...
#pragma ident	"@(#)locore.s	1.299	95/10/19 SMI"
...
#define	INTTABLE_MQH_UCE	(1 << 3)
#define	INTTABLE_MQH_BOTH	(1 << 4)
#define	INTTABLE_SBI		(1 << 5)
 ...
 *
 * "msgbuf" occupies the third and fourth pages.
 * The first 2 pages of KERNELBASE are illegal to act as a redzone
 * during copyin/copyout type operations.
 * The message buffer is allocated in low memory to prevent being
 ...
 */
	...
	.global	.int_rtt, .interrupt_ret, .intr_ret, .setcc
...
firsthighinstr:
	...
	! sun4c version does patch_win_stuff here...
...
ERROR - pcb_wbuf offset to large
	...
	add	%g1, 1, %g1
	st	%g1, [%g5 + MPCB_WBCNT]
	stb	%g1, [THREAD_REG + T_ASTFLAG]	! make sure new syscall sees it
	b	.st_have_window		! finished overflow processing
	restore				! delay slot, back to original window
	SET_SIZE(_sys_trap)
	SET_SIZE(sys_trap)
...
_int_vector:
	...
	.word	.level1		! level 1
	.word	.level2		! level 2,	sbus-1
	.word	.level3		! level 3,	sbus-2
	.word	.level4		! level 4
	.word	.level5		! level 5,	sbus-3
	.word	.level6		! level 6
	.word	.level7		! level 7,	sbus-4
	.word	.level8		! level 8
	.word	.level9		! level 9,	sbus-5
	.word	_level10	! level 10,	cpu_unit: tick timer
	.word	.level11	! level 11,	sbus-6
	.word	.level12	! level 12,	cpu_unit: uart(s)
	.word	.level13	! level 13,	sbus-7
	.word	.level14	! level 14,	cpu_unit: profile timer
...
.interrupt_ret:
	...
	!
	cmp	%l4, LOCK_LEVEL
	bl	.int_rtt		! slow-int, *is* interrupt thread
	nop				! delay slot
...
#endif	/* lint */
/*
 * Level 10, from cpu_unit: tick timer
 * for now, we use _level10 instead!
 */
...
{}
	...
	set	(BW_LOCALBASE+INTR_TABLE), %l3	! %l3 = Address of local BW+1000
	lduha	[%l3]ASI_BW, %l5 		! %l3 holds rest of address.
	andcc	%l5, INTTABLE_PROFILE, %l5	! Is profile intr bit set?
	bz	5f				! no, must be a regular interrupt.
	sethi	%hi(mon_clock_on), %l4		! If obp is watching the keyboard,
	ld	[%l4+%lo(mon_clock_on)], %l4	! then this interrupt needs to be
	tst	%l4				! given to it.
	bz	5f				! 
	nop
	...
	nop
	!
	! fall through if it's kernel profile timer, it's handled in
	! level14_handler() in C.
	!
5:	b	sys_trap			! go do the normal thing.
	mov	(T_INTERRUPT | 14), %l4		! put the correct stuff in %l4
	!
	! no return
	!
 ...
 *
 * We must first save the address of the stack frame on which the interrupted
 * context info was pushed, since it is otherwise not possible for profiling to
...
{}
	...
	ENTRY_NP(.level14)
/*
 * XXX - Need to check performance penalty will be neglible and guard against
 * no profiling structure before including the the new code.  level14_handler
 * will presumably chain to cpu_intr(14).  Can we (should we) use this for hard
 * interrupts?  Maybe we should try and distinguish hard and soft here.  What
 * about using CPUINTR(14)?
 */
 ...
 */
	.common clk_intr, 4, 4
        ...
        .seg    ".data"
	.global hrtime_base, vtrace_time_base, clk10_phyaddr
	.global	hres_lock, hres_last_tick, hrestime, hrestime_adj, timedelta
	/*
	 * WARNING WARNING WARNING
	 *
	 * All of these variables MUST be together on a 128-byte boundary.
	 ...
	 *
	 * In addition, a new variable (soft_hrtime) assists with the
	 * "don't access the h/w timer too fast" logic (see 1222745).
	 * To prevent the cache line which contains it from causing
	 * excessive write-invalidates, it stands alone in its own
	 * cache sub-block and is accessed only with instructions
	 * that cause write-updates.
	 */
	.global soft_hrtime
	.align	128	! two cache lines: the usual plus one for soft_hrtime
hrtime_base:
	.word   0, 0
vtrace_time_base:
	.word   0
...
8:
	...
	!
	ld	[%l4 + %lo(clk10_phyaddr)], %g2	! %g2 = timer address
...
5:
	...
	call	intr_get_table		! requesting devices
	clr	%o0			! delay slot - pseudo level
	and	%o0, INTTABLE_TICK, %o0 ! check to see if it's us
					! anybody home? (assume it was ok?)
	/*
	 * now we'll clean up a bit
	 */
	mov	%o0, %o1		! mask: just tick timer
	call	intr_clear_table	! We can forget it now!
	mov	%g0, %o0		! delay slot - pseudo level
	call	intr_tick_getlimit_local! to clear limit bit
	nop				! delay slot
	...
	! blinking LED stuff ...
	!
	set	bbus_blink_leds, %g2
	ld	[%g2], %g1		! %g1 = *bbus_blink_leds
	tst	%g1			! if(*bbus_blink_leds == 0)
	be	3f
	nop

	set	led_cnt, %g2		! %g2 = &led_cnt
	ldub	[%g2], %g3
	subcc	%g3, 1, %g3		! led_cnt--;
	bge,a	3f			! not zero, don't write LEDs
	stb	%g3, [%g2]		! delay, annul store of zero

	set	led_ticks, %l5		! %l5 = &led_ticks
	ldub	[%l5], %g3		! %g3 = led_ticks
	stb	%g3, [%g2]		! led_cnt = led_ticks; 

	mov	%l0, %psr		! set level (IU bug)
	wr	%l0, PSR_ET, %psr	! enable traps
	nop

	jmpl	%g1, %o7		! (*bbus_blink_leds)()
	nop

	call	syserr_poll		! call syserr_poll()
	clr	%o0			! delay slot void
...
3:
	...
	tst	%l5
#if	FREE_THREAD != 0	/* Save an instruction */
 ...
 * cannot be directly used on the sun4d.  This is because ddi_add_fastintr()
 * is not supported by the sun4d.
 */
 ...
 * in the trap window.
 * the kprof routine is inlined here because poke_cpu
 * may have set a soft int and we have to handle both,
 * kprof must return to SYS_TRAP, with locals in tact
 *
 * This assumes psr has been saved into %l0. It might be better
 * that we integrate this with L14_front_end code. But I'm not
 * sure how this piece code of would be turned on so we leave
 * it here for now.
 */
...
{}
	...
	set	BW_LOCAL_BASE+INTR_TABLE, %l3
	lduha	[%l3]ASI_BW, %l3	! read in intr table.
	andcc	%l3, INTTABLE_PROFILE, %l3 ! check to see if it's us
	bz	knone			! not profile timer intr.
	nop

	!
	! this is profile timer intr.
	!
	set	BW_LOCAL_BASE+PROF_LIMIT, %l3 ! clr timer
	lda	[%l3]ASI_BW, %l3

	set	BW_LOCAL_BASE+INTR_TABLE, %l3
	mov	INTTABLE_PROFILE, %l4
	stha	%l4, [%l3]ASI_BW	! clr BW intr table.

	mov	(1 << T_INT_LEVEL_14), %l4
	set	CC_BASE+INTR_CLEAR, %l3
	stha	%l4, [%l3]ASI_CC	! clr CC
...
void
_db_kernel_breakpoint_handler(void)
{}

void
_db_kernel_breakpoint_handler_new(void)
{}
...
/*
 * remote debugger interface code
 */
	.common _db_kernel_breakpoint_handler_adr, 4, 4

	ENTRY_NP(_db_kernel_breakpoint_handler)
	set _db_kernel_breakpoint_handler_adr, %l4
	ld	[%l4], %l4
	jmpl	%l4, %g0
	nop
	SET_SIZE(_db_kernel_breakpoint_handler)

	.common _db_kernel_breakpoint_handler_new_adr, 4, 4

	ENTRY_NP(_db_kernel_breakpoint_handler_new)
	set _db_kernel_breakpoint_handler_new_adr, %l4
	ld	[%l4], %l4
	jmpl	%l4, %g0
	nop
	SET_SIZE(_db_kernel_breakpoint_handler_new)
