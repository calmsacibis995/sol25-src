...
#ident	"@(#)bootbus.c	1.35	94/11/25 SMI"
...
/*
 * select_mutex - decide which, if any, mutex we should use
 */
static kmutex_t *
	...
	caddr_t int_handler_arg)
{
	...
	int unit = ddi_get_instance(rdip);
	int cpu_id = bbus_parent_cpu(dip);
...
#ifdef	DEBUG
	printf("bbus_add_hard: name=%s, pri=%d, av=0x%x\n", name, pri, av);
#endif	/* DEBUG */
	...
	ddi_iblock_cookie_t iblock_cookiep)
{
	...
	mutex_enter(&av_lock);
	bbus_rem_hard(dip, rdip, intrspec, iblock_cookiep);
	mutex_exit(&av_lock);
}
