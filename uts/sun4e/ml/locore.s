...
#pragma ident	"@(#)locore.s	1.71	94/10/20 SMI"
 ...
 * On the sun4e we put the message buffer in the third and fourth pages.
 ...
 *
 * The DVMA area is not like other sun architectures. On sun4e
 ...
 * trace buffer.
 *
 * TRAPTRACE can be defined in Makefile.sun4e.
 */
 ...
 *
 * KNH: The assumption above is no longer true.  The kernel is now
 *	loaded _directly_ into KERNBASE.  Thus, there is NO NEED to
 *	remap the kernel to high memory, performance implications
 *	notwithstanding.
 *
 ...
 *
 * KNH:  This is no longer necessary.  In fact, the kernel NO LONGER relocates
 *	itself.
 */
.entry:
 ...
 */
	!
	! We are running with correct addresses
...
2:
	lduba	[%l0]ASI_SM, %g1
	cmp	%g1, 256		! 4/E has 8 bits for PMEG
	blu,a	2b
	add	%l0, %g2, %l0
...
{}
	...
	bz	1f
	ld	[%g5 + PCB_WBCNT], %g3	! delay - user regs been saved?
...
1:					! psr written in delay of branch to here
	nop; nop; nop			! psr delay
...
_int_vector:
	...
	.word	.level2		! level 2, sbus level 2, vme level 1
	.word	.level3		! level 3, sbus level 3, vme level 2
	.word	.level4		! level 4, IE register 2
	.word	.level5		! level 5, sbus level 4, vme level 3
	.word	.level6		! level 6, IE register 3
	.word	.level7		! level 7, sbus level 5 
	.word	.level8		! level 8, sbus level 6, vme level 4
	.word	.level9		! level 9, sbus level 7, vme level 5
	.word	_level10	! level 10, normal clock
	.word	vme_level6	! level 11, vme level 6
#if defined(SAS) || defined(MPSAS)
	.word	simcintr	! sas console interrupt
#else
	.word	.level12	! level 12, scc - serial i/o
#endif	/* SAS */
	.word	vme_level7	! level 13, vme level 7
...
.intr_ret:
	...
	ld	[%l3 + CPU_SYSINFO_INTRTHREAD], %g2	! cpu_sysinfo.intrthread++
	inc	%g2
	st	%g2, [%l3 + CPU_SYSINFO_INTRTHREAD]
	...
	!
	! interrupt() branches here if the mutex-fixup hook detects a problem.
	!
intr_mutex_panic:
	set	1f, %o0
	mov	%l0, %psr		! set level (IU bug)
	...
	nop				! psr delay
1:	.asciz	"interrupt: mutex fixup failed"
	.align	4
...
ENTRY_NP(vme_level6)
	mov	%g0, %g2
        sethi   %hi(vme_ifault), %l1    ! clear vme_ifault word
        st      %g0, [%l1+%lo(vme_ifault)]
        sethi   %hi(VME_IACK6), %g1     ! get VME vector from the bus
	b	vme_read_vector
	or	%g1, %lo(VME_IACK6), %g1

ENTRY_NP(vme_level7)
	mov	%g0, %g2
        sethi   %hi(vme_ifault), %l1    ! clear vme_ifault word
        st      %g0, [%l1+%lo(vme_ifault)]
        sethi   %hi(VME_IACK7), %g1     ! get VME vector from the bus
	b	vme_read_vector
	or	%g1, %lo(VME_IACK7), %g1
	
	

	ENTRY_NP(vme_read_vector)		! This is caught in trap()
	ldub	[%g1], %l5	! read vector number, acknowledge int.
	sethi	%hi(vme_ifault), %l1	! reload vme_ifault
	ld	[%l1+%lo(vme_ifault)],%g1
	tst	%g1			! to see whether we got a fault
	bne	.vme_vec_intrfault	! error reading the vector
	nop
...
4:									;\
	...
	nop
/*
 * Macro for autovectored SBus interrupts and vectored VME interrupts.
 * %l4 or %l7 must be preserved in it.
 */


#define	IOINTR_VME(LEVEL) \
	set	level/**/LEVEL,%l5 /* get vector ptr */;\
1:									;\
	ld	[%l5 + AV_VECTOR], %l3 /* get routine address */	;\
	tst	%l3							;\
...
3:\
	...
	b	.int_rtt;\
        clr     [%g1 + %lo(level/**/LEVEL/**/_spurious)]  ;\
4:\
        sethi   %hi(vme_levels), %l1    /* memory refer set 0x2b2c, %l1 */;\
        ld      [%l1 + %lo(vme_levels)], %l1    /* VME interrupt */;\
	mov	1, %g1;\
	sll	%g1, LEVEL, %g1;\
	andcc	%l1, %g1, %g0		/* AND mask & processor level*/ ;\
	bz,a	5f			/* if not a VME level, spurious */ ;\
	nop;\
	/* add sun4e interupt handle support here*/;\
        sethi   %hi(vme_ifault), %l1    /* clear vme_ifault word */;\
        st      %g0, [%l1+%lo(vme_ifault)];\
	set	vme_iackaddr, %l1	/*table of IACK address per PIL*/;\
	set	LEVEL, %g1		/*%g1 <- index into vme_iackaddr[]*/;\
	sll	%g1, 2, %g1		/*scale LEVEL into array index*/ ;\
	ld	[%l1 + %g1], %g1	/*%g1 <-- IACK reg addr*/ ;\
	mov	1, %g2 ; \
        b	vme_read_vector         /*%g1==IACKreg: service VME intr*/ ;\
	nop;\
5:\
	set	level/**/LEVEL/**/_spurious, %o0			;\
        set     busname_vec, %o2                                        ;\
...
/*
 * Level 2 interrupts, from Sbus level 2 and VME level 1.
 */
...
/*
 * Level 3 interrupts, from Sbus level 3 (SCSI/DMA) and VME level 2.
 */
...
/*
 * Level 4 interrupts, IE register 2 and SCSI
 */
...
/*
 * Level 5 interrupts, from Sbus level 4 (ethernet) and VME level 3.
 */
...
/*
 * Level 6 interrupts, IE register 3 and Ethernet
...
/*
 * Level 8 interrupts, from Sbus level 6 and VME level 4.
 */
...
/*
 * Level 9 interrupts, from Sbus level 7 and VME level 5.
 */
...
#endif
/*
 * Level 11 interrupts, from floppy disk and VME level 6.
 */
...
/*
 * Level 13 interrupts, from audio chip and VME level 7.
 */
 ...
 * per second, for SUN4E we call clock at that rate.
 *
...
2:
	stb	%g1, [%l5 + LEDPTR]	! update duty count pointer
        set     AUXIO_ADDR, %g1         ! 4E in search of a diagnostic register
        stb     %g2, [%g1]              ! store it back
3:
#endif	/* SAS */
#else	/* FANCYLED */
	sethi	%hi(_led_on), %l5
...
9:
	...
	!
	! On a Uniprocessor like the sun4e, we're protected by SPL level
