...
#pragma ident	"@(#)bp_map.c	1.8	94/01/21 SMI"
 ...
 * If asked, we also enforce some rules on non-obmem ptes.
 */
int
	...
	int cknonobmem;
{
	...
	while (npf-- > 0) {
		...
		if (cknonobmem && bustype == -1) {
			bustype = pte->pg_type;
#ifdef	here_just_for_comment_different_from_sun4
			if (bustype != OBMEM) {
				if (bustype == VME_D16 || bustype == VME_D32) {
					/*
					 * If the bus address is in the
					 * Sun VME space DVMA area, bounce
					 * the request.
					 */
					if (pte->pg_pfnum < mmu_btop(1<<20)) {
						r = -1;
						break;
					}
				}
				lastpfnum = pte->pg_pfnum & pfnumbits;
			}
#endif
		} else if (cknonobmem) {
...
#ifdef	here_just_for_comment_different_from_sun4
			if (bustype != OBMEM) {
				pte->pg_pfnum &= pfnumbits;
				if (pte->pg_pfnum != lastpfnum + 1) {
					r = -1;
					break;
				}
			}
	...
	if (cknonobmem && r == 0 && bustype != OBMEM)
		r = 1;
	return (r);
}
...
bp_alloc(struct map *map, register struct buf *bp, int size)
{
	...
	if (vac) {
		/*
		 * XXX: this needs to be reexamined for sun4e
		 */
