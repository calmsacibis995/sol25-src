...
#pragma ident	"@(#)autoconf.c	1.45	94/07/05 SMI"
...
setcputype(void)
{
	...
	if (prom_getidprom((caddr_t)&idprom, sizeof (idprom)) == 0 &&
		 ...
		 * Plain ole Paranoia ..
		 */
		prom_printf("Using default machine type Sun4e/120\n");
		idprom.id_format = 0;	/* Make sure later tests fail */
		cputype = CPU_SUN4E_120;
	}
...
static const int slave_only = 4;	/* Slot 2 is slave-only */
static const char compat[] = "sun4e";
...
{ "slave-only",		sizeof (int),		(caddr_t)&slave_only}
};
...
add_root_props(dev_info_t *devi)
{
	...
	if (vac_linesize == 32) {
		...
		if (prom_getproplen(prom_rootnode(), "sun4c-micro-tlb") < 0)
			(void) e_ddi_prop_create(DDI_DEV_T_NONE, devi, 0,
			    "sun4c-micro-tlb", (caddr_t)&tlb, sizeof (int));
	}
...
#if 0	/* Not needed for sun4e (but please don't remove it just yet) */
static void
...
{
	static int first = 1;
	if (cputype == CPU_SUN4E_120 && first) {
    ...
    caddr_t buffer)
{
	 ...
	 * V2high = (V0low >> 26) & 3	(i.e. extract bits 27 and 26)
	 * V2low  = V0low & 3ffffff	(i.e. low 26 bits)
	 */
	if ((prom_getversion() == 0) &&
		...
		for (i = 0; i < n; ++i, ++rp)  {
			rp->regspec_bustype =
			    (rp->regspec_addr >> 26) & 3;
			rp->regspec_addr &= 0x3ffffff;
		}
	}
...
#undef	GOOD_BOOT_PROM
...
#ifdef sun4c
	/*
	 * 1088905: The 'burst-sizes' property was slightly
	...
	if (prom_getversion() == 2 &&
		...
		*(int *)buffer = 0x1f & (*(int *)buffer | 0x8);
	}
#endif
}
 ...
 * We don't know how to remove power from this machine.
 */
void
arch_power_down()
{
