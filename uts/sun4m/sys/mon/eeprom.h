...
#ifndef	_SYS_MON_EEPROM_H
#define	_SYS_MON_EEPROM_H

#pragma ident	"@(#)eeprom.h	1.11	93/11/01 SMI"

#include <sys/mon/keyboard.h>
#include <sys/mon/password.h>
#include <sys/mon/idprom.h>
...
#define	EEC_TYPE_CPU	0x01			/* cpu */
			struct	eec_cpu {
				...
				u_int	eec_cpu_unused:6;
				u_int	eec_cpu_dcp:1;		/* dcp? */
				u_int	eec_cpu_68881:1;	/* 68881? */
...
/* 0x18E */	u_char	ee_kb_id;	/* keybrd id in case of EEPROM table */
...
/* 0x290 */	u_char	  ee_logo[64][8];	/* A 64X64 bit space for */
						/* custom/OEM designed logo */
						/* icon */
#ifdef PASSWORD
...
/* 0x700 */	u_short ees_wrcnt[3];	/* write count (3 copies)  */
		short	ees_nu1;	/* not used		   */
/* 0x708 */	u_char	ees_chksum[3];	/* software area checksum (3 copies) */
/* 0x70b */	char	ees_diagsw;	/* Diagnostic mode switch  */
/* 0x70c */	char	ees_resv[0x7d8-0x70c];
	} ee_soft;

	/*
	 * Idprom contents for Hydra systems
	 */
	struct ee_idprom {
/* 0x7d8 */	u_char	 id_format;	/* format identifier	*/
/* 0x7d9 */	u_char	 id_machine;	/* machine type		*/
/* 0x7da */	u_char	 id_ether[6];	/* ethernet address	*/
/* 0x7df */	long	 id_date;	/* date of manufacture	*/
/* 0x7e0 */	unsigned id_serial:24;	/* serial number	*/
/* 0x7e7 */	u_char	 id_xsum;	/* xor checksum		*/
/* 0x7e8 */	u_char	 id_undef[25];	/* undefined	*/
	};
#endif	/* HYDRA */
#else
