...
#pragma	ident	"@(#)hat_sx.c 1.46	95/03/20 SMI"
...
static	u_int 	sx_mmu_getkpfnum(caddr_t);
...
static	void	sx_mmu_contig_devload(struct hat *, struct as *, caddr_t,
				struct devpage *, u_int, u_int, int, u_int);
...
struct hatops sx_mmu_hatops = {
	...
	sx_mmu_share,
	sx_mmu_unshare,
	sx_mmu_unload		/* alias for sx_mmu_unmap */
};
...
{
	mutex_init(&sxmmu_hment_lock, "sxmmu_hment_lock", MUTEX_DEFAULT, NULL);
}
...
sx_mmu_pageunload(struct page *pp, struct hment *hme)
{
	...
	ASSERT(pp != NULL ? HATLOCK_MLIST_HELD(pp) : 1);
...
sx_mmu_pagecachectl(struct page *pp, u_int flag)
{
	...
	/*
	 * If we are called because a page is being marked non-cached, we
	 * have no work to do, since the work is done by the SRMMU. If someone
	 * is trying to mark the page as CACHED and there is an SX mapping
	 * we panic because that scenario should not happen.
	 */
	if (flag & (HAT_TMPNC | HAT_UNCACHE))
		return;
	/*
	 * The page is being marked cached.
	 * Since we cannot have the page marked as cacheable while there is an
	 * SX mapping to the page, we panic when this happens.
	 */
	ASSERT(pp != NULL ? HATLOCK_MLIST_HELD(pp) : 1);
	...
	for (hme = pp->p_mapping; hme; hme = hme->hme_next) {
		...
		mutex_enter(&hat->hat_mutex);
		if (hat->hat_op == &sx_mmu_hatops)
			cmn_err(CE_PANIC,
				"sx_mmu_pagecachectl: trying to cache SX\n");
		mutex_exit(&hat->hat_mutex);
	}
}
