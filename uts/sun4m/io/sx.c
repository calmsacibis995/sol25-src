...
#pragma	ident		"@(#)sx.c 1.108     95/09/19 SMI"
...
int sx_tmpflag = HAT_TMPUNLOAD;	/* patchable to 0 to turn off TMPUNLOAD */
...
static int sx_ioctl(dev_t dev, int cmd, int arg, int mode,
 ...
 */
static	int	segsx_create(struct seg *, caddr_t);
static	int	segsx_dup(struct seg *, struct seg *);
static	int	segsx_unmap(struct seg *, caddr_t, u_int);
static	void	segsx_free(struct seg *);
static	faultcode_t	segsx_fault(struct hat *, struct seg *,
			    caddr_t, u_int, enum fault_type, enum seg_rw);
static	faultcode_t	segsx_faulta(struct seg *, caddr_t);
static	int	segsx_setprot(struct seg *, caddr_t, u_int, u_int);
static	int	segsx_checkprot(struct seg *, caddr_t, u_int, u_int);
static	int	segsx_kluster(struct seg *, caddr_t, int);
static	u_int	segsx_swapout(struct seg *);
static	int	segsx_sync(struct seg *, caddr_t, u_int, int, u_int);
static	int	segsx_incore(struct seg *, caddr_t, u_int, char *);
static	int	segsx_lockop(struct seg *, caddr_t, u_int,
			int, int, ulong *, size_t);
static	int	segsx_getprot(struct seg *, caddr_t, u_int, u_int *);
static	off_t	segsx_getoffset(struct seg *, caddr_t);
static	int	segsx_gettype(struct seg *, caddr_t);
static	int	segsx_getvp(struct seg *, caddr_t, struct vnode **);
static	int	segsx_advise(struct seg *, caddr_t, u_int, int);
static	void   segsx_dump(struct seg *);
...
static struct modldrv modldrv = {
	...
	"Device/Segment driver for SX V1.108",
	&sx_ops	/* devops */
};
...
sx_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
...

		{
			...
			mutex_init(&sx_aflt_mutex, "SX async fault lock",
				MUTEX_DRIVER, ic);
		}
		mutex_init(&sx_hw_mutex, "SX HW lock", MUTEX_DRIVER, NULL);

		mutex_init(&sx_cntxt_mutex, "SX context lock",
				MUTEX_DRIVER, NULL);

		mutex_init(&sx_tr_lock, "SX trace buffer lock",
				MUTEX_DRIVER, NULL);
...
sx_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		if (sx_aflt_cookie != NULL) {
			aflt_remove_handler(sx_aflt_cookie);
		}
	...
	case DDI_SUSPEND:
		...
		if (sx_current_procp)
			sx_regsave(sx_current_procp->spp_cntxtp);
...
/*ARGSUSED*/
int
sx_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *cred_p, int *rval_p)
{
	int error = 0;
	int cmdarg;
	int cntxtnum;
	...
	case SX_CACHE_CTRL:
		...
		if (copyin((caddr_t)arg, (caddr_t)&sx_cachectrl,
	...
	off_t len, u_int prot, u_int maxprot, u_int flags, cred_t *cred)
{
	...
	u_int pf;
	void map_addr(caddr_t *, u_int, off_t, int);
	...
	case SX_REG_MAP:	  /* SX register set	*/
		...
		if ((p->spp_hat = hat_alloc(as, &sx_mmu_hatops)) == NULL) {
	...
	default:
		...
		if ((pf = hat_getpfnum(as, (caddr_t)offset)) == -1) {
			as_fault(as->a_hat, as, (caddr_t)offset,
...
segsx_dup(struct seg *seg, struct seg *newseg)
{
	...
	if ((p = sx_getproc(sdp->sd_cntxtnum, newseg->s_as)) ==
		...
		/* Allocate a SX HAT for this address space */
		if ((p->spp_hat = hat_alloc(newseg->s_as,
		    &sx_mmu_hatops)) == NULL) {
	...
	if ((error = segsx_create(newseg, (caddr_t)sdp)) != 0) {
		...
		hat_free(p->spp_hat, newseg->s_as);
		sx_freeproc(sdp->sd_cntxtnum, p);
		SX_CNTXT_LOCK_EXIT(&sx_cntxt_mutex);
		return (error);
	}
...
static int
segsx_unmap(struct seg *seg, caddr_t addr, u_int len)
{
	sx_proc_t *sx_procp;
	struct sx_seg_data *sdp;
	...
	if (sdp->sd_orig_vaddr != NULL) {
		...
		if ((sx_unload_orig_xlations) &&
			...
			hat_unload(seg->s_as, sdp->sd_orig_vaddr, len,
				HAT_UNLOAD);
		}
	}
	 ...
	 */
	if (VOP_DELMAP(common_specvp(sdp->sd_vp),
	    (offset_t)0, seg->s_as, addr, len,
	    sdp->sd_prot, sdp->sd_maxprot, MAP_SHARED, CRED()) != 0)
		cmn_err(CE_WARN, "segsx_unmap VOP_DELMAP failure");
	...
	if (--sx_procp->spp_segcnt == 0) {
		...
		for (; sx_hat != NULL; sx_hat = sx_hat->hat_next) {
			if (sx_hat == sx_procp->spp_hat) {
				hat_free(sx_procp->spp_hat, seg->s_as);
				break;
			}
		}
...
top:
	...
	if (sx_current_procp == (struct sx_proc *)0) {
		...
		if ((sx_cntxtswtch(sx_procp)) == SX_REGSAVE_FAIL) {
			...
			SX_CNTXT_LOCK_EXIT(&sx_cntxt_mutex);
			delay(HZ / 50); /* Wait 2 clock ticks */
			goto top;
		}
	...
	case SX_REG_MAP:	/* Fault on mapping to SX register set */
		...
		hat_devload(hat, seg->s_as, addr, NULL,
		    sx_usr_pf, sdp->sd_prot, HAT_LOAD);
		break;
	...
	default:	/* Fault on mapping to D[V]RAM */
		...
		for (; sx_vaddr < addr + len;
			 ...
			 */
			if ((pf = hat_getpfnum(seg->s_as, orig_vaddr)) == -1) {
				/*
...
sx_fault_cmem(struct hat *hat, struct as *as, caddr_t sx_vaddr,
	caddr_t orig_vaddr, struct devpage *dp, u_int len, u_int prot)
{
	u_int pf, sx_pf;
	...
	hat_contig_devload(hat, as, sx_vaddr, dp, sx_pf, prot, HAT_LOAD, len);
	return (0);
}
	...
	u_int len, u_int prot)
{
	...
	for (end_vaddr = sx_vaddr + len; sx_vaddr < end_vaddr;
		 ...
		 * the original virtual address.
		 */
		if ((pf = hat_getpfnum(as, orig_vaddr)) == -1) {
			continue;
		}
		...
		hat_devload(sx_hat, as, sx_vaddr, dp, sx_pf, prot, hat_flags);
...
/*ARGSUSED*/
static int
segsx_setprot(struct seg *seg, caddr_t addr, u_int len, u_int prot)
{
	return (EACCES);
}
...
/*ARGSUSED*/
static int
segsx_checkprot(struct seg *seg, caddr_t addr, u_int len, u_int prot)
{
	int error;
	struct sx_seg_data *sdp = (struct sx_seg_data *)seg->s_data;
...
static	int
segsx_getprot(struct seg * seg, caddr_t addr, u_int len, u_int *protv)
{
	struct sx_seg_data *sdp = (struct sx_seg_data *)seg->s_data;
...
/*ARGSUSED*/
static int
segsx_kluster(struct seg *seg, caddr_t addr, int delta)
{
	return (-1);
}
...
/*ARGSUSED*/
static int
segsx_sync(struct seg *seg, caddr_t addr, u_int len, int attr, u_int flags)
{
	return (0);
}
...
/*ARGSUSED*/
static int
segsx_incore(struct seg *seg, caddr_t addr, u_int len, char *vec)
{
	u_int v = 0;
	struct sx_seg_data *sdp = (struct sx_seg_data *)seg->s_data;
...
/*ARGSUSED*/
static int
segsx_lockop(struct seg *seg, caddr_t addr, u_int len, int attr,
...
/*ARGSUSED*/
static int
segsx_advise(struct seg *seg, caddr_t addr, u_int len, int behav)
{
	return (0);
}
...
sx_orig_unmap(struct as *as, caddr_t vaddr, u_int len)
{
	...
	hat_unmap(sxseg->s_as, sxseg->s_base, sxseg->s_size, HAT_UNMAP);
...
sx_unload(struct sx_proc *p)
{
	...
	ASSERT(SX_HW_LOCK_HELD(&sx_hw_mutex));
	ASSERT(SX_CNTXT_LOCK_HELD(&sx_cntxt_mutex));
	ASSERT(as && AS_READ_HELD(as, &as->a_lock));
	...
	for (seg = AS_SEGP(as, as->a_segs); seg != NULL;
		...
		sdp = (struct sx_seg_data *)seg->s_data;
		if (sdp->sd_valid) {
			hat_unload(seg->s_as, seg->s_base, seg->s_size,
...
sx_vrfy_va(struct sx_valid_va *arg)
{
	...
	if (copyin((caddr_t)arg, (caddr_t)&smv, sizeof (smv)) != 0)
		return (EFAULT);
	drv_getparm(UPROCP, &procp);
	as = ((struct proc *)procp)->p_as;
	...
	for (seg = AS_SEGP(as, as->a_segs); seg != NULL;
		...
		if ((segbase >= saddr && segbase < eaddr) ||
			...
			smv.sp_base_len = (u_int)MIN(eaddr, segend) -
				smv.sp_base_vaddr;
			AS_LOCK_EXIT(as, &as->a_lock);
			if (copyout((caddr_t)&smv, (caddr_t)arg,
				sizeof (smv)) != 0) {
				return (EFAULT);
			}
			return (0);
...
sx_get_orig_va(struct sx_original_va *arg)
{
	...
	if (copyout((caddr_t)&smv, (caddr_t)arg, sizeof (smv)) != 0)
		return (EFAULT);
	return (0);
}
