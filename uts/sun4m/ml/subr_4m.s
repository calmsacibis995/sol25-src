...
#pragma ident	"@(#)subr_4m.s	1.71	95/01/16 SMI"
...
/*ARGSUSED*/
int
wait_z_and_set(int *i)
{ return (0); }

/*ARGSUSED*/
int
wait_nz_and_clr(int *i)
{ return (0); }
...
#ifdef notdef
/*
 * Atomic "test and clear" function
 */
	ENTRY(atomic_tac)
        mov     %o0, %o1
        mov     %g0, %o0
...
1:	retl
	nop
	SET_SIZE(wait_z_and_set)

	ENTRY(wait_nz_and_clr)
	mov	%o0, %o1
3:	mov	%g0, %o0
	swap	[%o1], %o0
2:	tst	%o0
...
/*ARGSUSED*/
void
do_load_ioc(int ioc_addr, int i)
{}

/*ARGSUSED*/
int
do_read_ioc(int ioc_addr)
{ return (0); }

/*ARGSUSED*/
void
do_flush_ioc(int ioc_addr)
{}
...
void
flush_vme_writebuffers(void)
{}

#else	/* lint */

/*
 * Flush VME Writebuffers
 *
 *	void flush_vme_writebuffers()
 */
	ENTRY(flush_vme_writebuffers)
	/*
	...
	retl
	nop
	SET_SIZE(flush_vme_writebuffers)
...
{}
	...
	/*
	 * Since the vme and sbus nexus drivers don't handle writebuffers
	 * properly, we have a flush_writebuffers_to hack that defaults
	 * to this, but changes to flush_vme_writebuffers when vme is
	 * present.
	 */
...
#ifdef sun4m
/*
 * Enable and disable video interrupt. (sun4 only, not sun4m)
 * setintrenable(value)
...
/*
 * Enable and disable video. (sun4 only; not sun4m)
 */
...
/*
 * Read the state of the video. (sun4 only; not sun4m)
 */
