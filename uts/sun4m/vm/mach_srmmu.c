...
#pragma ident   "@(#)mach_srmmu.c 1.30     95/05/11 SMI"
...
va_to_pa(u_int vaddr)
{
	...
	if ((pfn = (u_int)va_to_pfn((caddr_t)vaddr)) != -1)
		return ((pfn << MMU_STD_PAGESHIFT) | (vaddr & MMU_PAGEOFFSET));
	else
		return ((u_int)-1);
}
...
hat_kern_setup(void)
{
	...
	if ((pa = va_to_pa((u_int)contexts)) == -1)
...
scan_l3(struct ptbl *ptbl)
{
	...
	if (taddr >= (caddr_t)SYSBASE && taddr < Syslimit &&
	    pf_is_memory(pfn))
		return (0);
