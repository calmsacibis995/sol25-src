...
#pragma ident   "@(#)mach_4m.c 1.40     95/01/04 SMI"
 ...
 * disabled for a period of INT_OFF_TIME;  (1800 * HZ) = 30 minutes.
 */
#define	INT_OFF_TIME	(1800 * HZ)
...
void log_stovme_err(u_int afsr, u_int afar0);
...
page_giveup(u_int pagenum, struct page *pp)
{
	...
	cmn_err(CE_WARN, "page %x marked out of service.\n", ptob(pagenum));
}
	...
	enum seg_rw rw;
{
	...
	if (mmu_fsr & MMU_SFSR_FATAL) {
		 ...
		 * entirely unmodified ...
		 * Note: we only want to complain in a debug kernel;
		 * this alarms the production users (customers) otherwise.
		 */
		ptpe.ptpe_int = mmu_probe(addr, NULL);
		bt = impl_bustype(ptpe.pte.PhysicalPageNumber);

		if ((type == T_DATA_FAULT) && (bt == BT_VME)) {
#ifdef DEBUG
			printf("VME dropped an INT-ACK cycle\n");
 ...
 */
			if (curthread->t_nofault) {
				...
				if (async_errq_full) {
					...
					cmn_err(CE_CONT, "AFAR0 = 0x%x, ");
					cmn_err(CE_CONT, "AFAR1 = 0x%x\n",
						afsr, afar0, afar1);
 ...
 */
			if (pokefault == -1) {
				...
				handle_aflt(cpuid, AFLT_S_TO_VME, afsr,
				    afar0, afar1);
			}
		}
...
#endif MP
	if (system_fatal) {
		(void) setup_panic("async fault", (va_list)NULL);
		...
		if (module_error) {
			...
			for (i = 0; i < NCPU; i++) {
#endif MP
				if (sys_fatal_flt[i].aflt_type == AFLT_MODULE) {
		...
		case AFLT_M_TO_S:
			...
			break;
		case AFLT_S_TO_VME:
			cmn_err(CE_CONT, "Async Fault from S-to-VME: ");
			cmn_err(CE_CONT, "afsr=%x afar=%x\n",
				afsr, afar0);
...
sun4m_process_aflt()
{
	...
	while (a_head[cpuid] != a_tail[cpuid]) {
		...
		case	AFLT_S_TO_VME:
			 ...
			 * fault.
			 */
			cmn_err(CE_WARN, "VME Asynchronous fault due to user ");
			cmn_err(CE_CONT, "write - non-fatal\n");
			log_stovme_err(afsr, afar0);
			break;
	...
	u_int afar1)	/* Asynchronous Fault Address Register 1 */
{
	...
	if (head == a_tail[cpu_id]) {
		...
		timeout(enable_cei, 0, ceint_off_time);
...
void
log_stovme_err(u_int afsr, u_int afar0)
{
	if (afsr & VFSR_TO) {
		cmn_err(CE_WARN, "\tVME address %x Timed Out\n", afar0);
	} else
	if (afsr & VFSR_BERR) {
		cmn_err(CE_WARN, "\tVME address %x received a BERR.\n", afar0);
	} else
	if (afsr & VFSR_WB) {
		cmn_err(CE_WARN, "\tIO Cache Write-Back Error at ");
		cmn_err(CE_CONT, "address %x\n", afar0);
	} else
		cmn_err(CE_WARN, "\tStrange S-to-VME error at %x\n", afar0);
}
	...
	u_int afar1)	/* Asynchronous Fault Address Register 1 */
{
	 ...
	 * this variable can be set to true if you want the information
	 * printed on the console as well as in the log file.
	 */
	if (afsr & EFSR_CE) {
