...
#pragma ident	"@(#)fillsysinfo.c	1.61	95/03/06 SMI"
...
static char cpu_info_buf[NCPU][80];
...
caddr_t v_counter_addr[5];	/* 4 @ level14, 1 @ level 10 */
struct cpu_intreg *v_interrupt_addr[5];	/* 4 (1 per module) + 1 (system) */
caddr_t v_eeprom_addr;
caddr_t v_sbusctl_addr;
caddr_t v_vmectl_addr[2];	/* Error regs, int vect reg	*/

/*
 * Multiple addresses broken out here to avoid run-time arithmetic
 */
struct sys_intreg *v_sipr_addr;
extern caddr_t v_level10clk_addr;
...
fill_cpu(dnode_t node)
{
	...
	(void) GETPROP(node, "clock-frequency", (caddr_t)&clk_freq);
	...
	if (clk_freq == 0) {
		...
		(void) GETPROP(root, "clock-frequency", (caddr_t)&clk_freq);
	}

	cpunode->clock_MHz = (clk_freq + 500000) / 1000000;
	...
	cpus[ncpunode].cpu_m.cpu_info = &cpu_info_buf[ncpunode][0];
	sprintf(cpus[ncpunode].cpu_m.cpu_info,
	    "cpu%d: %s (mid %d impl 0x%x ver 0x%x clock %d MHz)\n",
	    ncpunode, cpunode->name, cpunode->mid, cpunode->implementation,
	    cpunode->version, cpunode->clock_MHz);
	ncpunode++;
}
...
have_counter(dnode_t node)
{
	...
	v_level10clk_addr = v_counter_addr[size / sizeof (caddr_t) - 1];
}
...
void
have_vme(dnode_t node)
{
	int size;
	extern void (*v_flush_writebuffers_to)(), flush_vme_writebuffers();

	vme = 1;
	if (GETPROPLEN(node, "iocache?") != -1)
		ioc = 1;
	size = GETPROPLEN(node, OBP_ADDRESS);
	if (size == -1 || size > sizeof (v_vmectl_addr))
		cmn_err(CE_PANIC, "vme addr size");
	if (GETPROP(node, OBP_ADDRESS, (caddr_t)v_vmectl_addr) == -1)
		cmn_err(CE_PANIC, "vme address");

	/* hack because the vme and sbus nexi don't handle flushes */
	v_flush_writebuffers_to = flush_vme_writebuffers;
}
