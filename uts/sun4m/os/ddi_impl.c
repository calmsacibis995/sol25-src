...
#pragma ident	"@(#)ddi_impl.c	1.48	94/12/05 SMI"

/*
 * Sun4m specific DDI implementation
 */
	...
	u_int *real_length, ddi_acc_hdl_t *ap)
{
	...
	nocache = !streaming && !(cache & CACHE_IOCOHERENT);
	if (ross_iopb_workaround)
		nocache = 1;	/* ross bug; fixed in A.8 */
	a = kalloca(length, align, nocache, cansleep);
	if ((*kaddrp = a) == 0) {
		return (DDI_FAILURE);
	} else {
		/*
...
i_ddi_mem_free(caddr_t kaddr, int stream)
{
	...
	nocache = !stream && !(cache & CACHE_IOCOHERENT);
	if (ross_iopb_workaround)
		nocache = 1;	/* ross bug; fixed in A.8 */
	kfreea(kaddr, nocache);
}
    ...
    int use_registers)
{
	...
	*ppd = pdptr = (struct ddi_parent_private_data *)
	...
	rgstr_len = 0;
	if (use_registers)
		get_prop(child, "registers", DDI_PROP_NOTPROM,
...
i_impl_ddi_sunbus_initchild(dev_info_t *child, int use_registers)
{
	...
	if ((has_registers) && (ddi_get_nodeid(child) == DEVI_PSEUDO_NODEID)) {
		...
		for (och = devnamesp[major].dn_head;
			...
			need_init = DDI_CF1(och) ? 1 : 0;	/* XXX huh? */
			if (need_init)
