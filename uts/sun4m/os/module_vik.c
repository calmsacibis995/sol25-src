...
#pragma ident	"@(#)module_vik.c	1.54	95/08/07 SMI"
	...
	int kernelregs;
{
	if (reg == 0)
		...
		*val = rgs[reg];
		...
		if (kernelregs)
			*val = rw[reg - 16];
		else if ((*val = fuword((int *)&rw[reg - 16])) == -1 &&
		    fubyte((caddr_t)&rw[reg - 16]) == -1)
			return (-1);
	}
	return (0);
}
